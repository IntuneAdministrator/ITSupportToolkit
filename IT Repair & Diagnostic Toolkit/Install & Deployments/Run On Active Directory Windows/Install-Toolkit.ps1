...
<#
.SYNOPSIS
  Installs IT Repair & Diagnostic Toolkit for Active Directory / GPO (per-machine).
.NOTES
  Host the full toolkit on a DFS/SYSVOL share. Point a Computer Startup script at:
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "\\share\...\Install & Deployments\Run On Active Directory Windows\Install-Toolkit.ps1"
  Or use Install.cmd from the same folder.
#>
[CmdletBinding()]
param()
