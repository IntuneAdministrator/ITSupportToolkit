#!/bin/zsh
# Installs IT Repair & Diagnostic Toolkit for Active Directory–joined Macs (per-machine).
#   sudo zsh "/Volumes/IT-Repair-Scripts/Install & Deployments/Run On Active Directory macOS/Install-Toolkit.sh"
set -euo pipefail
...
if [[ ! -d "$APP_SRC" ]]; then
  ...
  echo "Then copy the full toolkit folder to your AD/SMB share." >&2
  exit 1
fi
