#!/bin/zsh
# ============================================================
# Build IT Repair & Diagnostic Toolkit-Jamf.pkg for Jamf Pro.
# Run ONCE on a Mac after IT Repair & Diagnostic Toolkit.app exists.
# Output: IT Repair & Diagnostic Toolkit-Jamf.pkg next to this script (or -o path).
...
OUT_PKG="$SCRIPT_DIR/IT Repair & Diagnostic Toolkit-Jamf.pkg"
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "ERROR: Build the Jamf .pkg on a Mac." >&2
  exit 1
fi
...
STAGE="$(/usr/bin/mktemp -d /tmp/it-repair-jamf-XXXXXX)"
...
# Jamf receipt-friendly touch (optional inventory hint)
/usr/bin/touch "$DEST/.jamf-installed"
exit 0
POST
...
echo "Upload to Jamf Pro → Settings → Computer Management → Packages"
echo "Policy: Packages → install $OUT_PKG; Scope + Self Service as needed"
echo "Uninstall: Policy → Scripts → Uninstall-Toolkit.sh (upload as Jamf Script)"
echo ""
