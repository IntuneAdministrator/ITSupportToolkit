#!/bin/zsh
# ============================================================
# Script:      screenshot-evidence-capture-mac.sh
# Description: macOS IT Support quick repair for screenshot evidence capture mac.
#              Category: 09_Evidence-Reporting-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh screenshot-evidence-capture-mac.sh
...
FOLDER_NAME='09_Evidence-Reporting-Mac'
...
show_progress 1 "$TOTAL_STEPS" "Start + MDM detection"
detect_mdm_join_state
log_info "Computer: $COMPUTER_NAME | State: $JOIN_STATE | User: $REAL_USER"
add_report_row "Computer" "$COMPUTER_NAME" "INFO" "$JOIN_STATE"
add_report_row "Script" "$SCRIPT_BASE" "INFO" "ITSupport/$FOLDER_NAME"

section_banner "EVIDENCE REPORTING"
show_progress 2 "$TOTAL_STEPS" "Build evidence pack"
PACK="$LOG_DIR/Evidence_${SCRIPT_BASE}_${TIMESTAMP}"
mkdir -p "$PACK"
sw_vers > "$PACK/sw_vers.txt" 2>/dev/null || true
system_profiler SPSoftwareDataType SPHardwareDataType > "$PACK/hardware_software.txt" 2>/dev/null || true
who > "$PACK/who.txt" 2>/dev/null || true
last -10 > "$PACK/last.txt" 2>/dev/null || true
add_report_row "Base snapshot" "$PACK" "PASS" ""

if [[ "$SCRIPT_BASE" == *network* || "$SCRIPT_BASE" == *ticket* || "$SCRIPT_BASE" == *system-snapshot* || "$SCRIPT_BASE" == *session* ]]; then
  ifconfig > "$PACK/ifconfig.txt" 2>/dev/null || true
  netstat -rn > "$PACK/routes.txt" 2>/dev/null || true
  scutil --dns > "$PACK/dns.txt" 2>/dev/null || true
  add_report_row "Network state" "Captured" "PASS" ""
fi

if [[ "$SCRIPT_BASE" == *crash* ]]; then
  mkdir -p "$PACK/crashes"
  cp /Library/Logs/DiagnosticReports/*.{crash,ips,panic} "$PACK/crashes/" 2>/dev/null || true
  cp "$REAL_HOME/Library/Logs/DiagnosticReports/"*.{crash,ips} "$PACK/crashes/" 2>/dev/null || true
  add_report_row "Crash export" "$PACK/crashes" "PASS" ""
fi

if [[ "$SCRIPT_BASE" == *screenshot* ]]; then
  # screencapture requires GUI user session
  if [[ -n "${CONSOLE_USER:-}" && "$CONSOLE_USER" != "root" ]]; then
    sudo -u "$CONSOLE_USER" screencapture -x "$PACK/screenshot.png" 2>/dev/null || true
    [[ -f "$PACK/screenshot.png" ]] && add_report_row "Screenshot" "Saved" "PASS" "" || add_report_row "Screenshot" "Failed (headless?)" "WARN" ""
  else
    add_report_row "Screenshot" "No console user" "WARN" ""
  fi
fi

if [[ "$SCRIPT_BASE" == *before-after* ]]; then
  BASELINE="$LOG_DIR/BeforeAfter_Baseline.txt"
  NOW="time=$(date -u +%Y-%m-%dT%H:%M:%SZ); disk=$(df -h / | tail -1); load=$(uptime)"
  if [[ ! -f "$BASELINE" ]]; then
    echo "$NOW" > "$BASELINE"
    add_report_row "Baseline" "Created" "PASS" "$BASELINE"
  else
    add_report_row "Before" "$(cat "$BASELINE")" "INFO" ""
    add_report_row "After" "$NOW" "INFO" ""
    echo "$NOW" > "$BASELINE"
  fi
fi

(cd "$LOG_DIR" && tar -czf "Evidence_${SCRIPT_BASE}_${TIMESTAMP}.tar.gz" "Evidence_${SCRIPT_BASE}_${TIMESTAMP}" 2>/dev/null) || true
add_report_row "Evidence tar.gz" "$LOG_DIR/Evidence_${SCRIPT_BASE}_${TIMESTAMP}.tar.gz" "PASS" "Attach to ticket"
show_progress 6 "$TOTAL_STEPS" "Evidence done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "screenshot evidence capture mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive