#!/bin/zsh
# ============================================================
# Script:      printer-remove-reinstall-mac.sh
# Descrição:   Remove e reinstala impressora via lpadmin.
# Categoria:   37_Printer-Management-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh printer-remove-reinstall-mac.sh
...
FOLDER_NAME='37_Printer-Management-HD-Mac'
...
section_banner "REMOVER E REINSTALAR IMPRESSORA"
# Uso: sudo PRINTER_NAME=OfficePrinter PRINTER_URI='ipp://printer.company.local/ipp/print' PP_PATH='drv:///sample.drv/generic.ppd' zsh ...
PRINTER_NAME=${PRINTER_NAME:-}
PRINTER_URI=${PRINTER_URI:-}
PP_PATH=${PP_PATH:-drv:///sample.drv/generic.ppd}
show_progress 2 "$TOTAL_STEPS" "Validar"
if [[ -z "$PRINTER_NAME" ]]; then
  add_report_row "PRINTER_NAME" "obrigatório" "FAIL" "ex.: OfficePrinter"
  lpstat -p 2>&1 | tee -a "$LOG_FILE" || true
else
  URI_CUR=$(lpstat -v "$PRINTER_NAME" 2>/dev/null | awk '{print $3}' || true)
  PRINTER_URI=${PRINTER_URI:-$URI_CUR}
  show_progress 4 "$TOTAL_STEPS" "Remover $PRINTER_NAME"
  lpadmin -x "$PRINTER_NAME" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Remover" "$PRINTER_NAME" "PASS" "lpadmin -x"
  if [[ -n "$PRINTER_URI" ]]; then
    show_progress 6 "$TOTAL_STEPS" "Reinstalar"
    lpadmin -p "$PRINTER_NAME" -E -v "$PRINTER_URI" -m "$PP_PATH" 2>&1 | tee -a "$LOG_FILE" || \
      lpadmin -p "$PRINTER_NAME" -E -v "$PRINTER_URI" 2>&1 | tee -a "$LOG_FILE" || true
    cupsenable "$PRINTER_NAME" 2>/dev/null || true
    cupsaccept "$PRINTER_NAME" 2>/dev/null || true
    add_report_row "Reinstalar" "$PRINTER_NAME" "PASS" "$PRINTER_URI"
  else
    add_report_row "Reinstalar" "URI ausente" "WARN" "defina PRINTER_URI"
  fi
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Printer Remove Reinstall" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive