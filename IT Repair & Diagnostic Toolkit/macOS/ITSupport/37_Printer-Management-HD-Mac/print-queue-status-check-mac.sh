#!/bin/zsh
# ============================================================
# Script:      print-queue-status-check-mac.sh
# Descrição:   Verifica status da fila de impressão.
# Categoria:   37_Printer-Management-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh print-queue-status-check-mac.sh
...
section_banner "STATUS DA FILA"
show_progress 2 "$TOTAL_STEPS" "lpstat -o / -p"
lpstat -p 2>&1 | tee -a "$LOG_FILE" || true
lpstat -o 2>&1 | tee -a "$LOG_FILE" || true
JOBS=$(lpstat -o 2>/dev/null | wc -l | tr -d ' ')
if [[ "${JOBS:-0}" -gt 0 ]]; then
  add_report_row "Jobs na fila" "$JOBS" "WARN" "lpstat -o"
else
  add_report_row "Jobs na fila" "0" "PASS" ""
fi
tail -n 40 /var/log/cups/error_log 2>/dev/null | tee -a "$LOG_FILE" || true
add_report_row "CUPS error_log" "tail" "INFO" ""

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Print Queue Status" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive