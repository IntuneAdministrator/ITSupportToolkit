#!/bin/zsh
# ============================================================
# Script:      printers-export-import-list-mac.sh
# Descrição:   Exporta lista de impressoras para reimportação.
# Categoria:   37_Printer-Management-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh printers-export-import-list-mac.sh
...
section_banner "EXPORTAR LISTA DE IMPRESSORAS"
show_progress 2 "$TOTAL_STEPS" "Exportar lpstat/lpadmin data"
EXPORT_FILE="$BACKUP_DIR/printers-export-$TIMESTAMP.txt"
{
  echo "# Printer export $TIMESTAMP"
  echo "# Reimport hint: lpadmin -p NAME -E -v URI -m everywhere"
  lpstat -p -d 2>/dev/null || true
  echo "---"
  lpstat -v 2>/dev/null || true
  echo "---"
  lpoptions -l 2>/dev/null || true
} > "$EXPORT_FILE"
[[ "$REAL_USER" != "root" ]] && chown "$REAL_USER":staff "$EXPORT_FILE" 2>/dev/null || true
cat "$EXPORT_FILE" | tee -a "$LOG_FILE"
add_report_row "Export" "$EXPORT_FILE" "PASS" "use como referência para reimportação"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Printers Export List" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive