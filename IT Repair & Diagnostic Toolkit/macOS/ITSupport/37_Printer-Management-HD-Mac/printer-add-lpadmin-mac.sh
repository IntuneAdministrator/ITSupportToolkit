#!/bin/zsh
# ============================================================
# Script:      printer-add-lpadmin-mac.sh
# Descrição:   Adiciona impressora via lpadmin.
# Categoria:   37_Printer-Management-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh printer-add-lpadmin-mac.sh
...
section_banner "ADICIONAR IMPRESSORA (lpadmin)"
# Uso: sudo PRINTER_NAME=Office PRINTER_URI='ipp://10.0.0.10/ipp/print' PP_PATH='everywhere' zsh ...
PRINTER_NAME=${PRINTER_NAME:-}; PRINTER_URI=${PRINTER_URI:-}; PP_PATH=${PP_PATH:-everywhere}
show_progress 2 "$TOTAL_STEPS" "Validar"
if [[ -z "$PRINTER_NAME" || -z "$PRINTER_URI" ]]; then
  add_report_row "Parâmetros" "PRINTER_NAME e PRINTER_URI obrigatórios" "FAIL" ""
  lpinfo -v 2>&1 | tee -a "$LOG_FILE" || true
else
  show_progress 4 "$TOTAL_STEPS" "lpadmin -p"
  if [[ "$PP_PATH" == "everywhere" ]]; then
    lpadmin -p "$PRINTER_NAME" -E -v "$PRINTER_URI" -m everywhere 2>&1 | tee -a "$LOG_FILE" || true
  else
    lpadmin -p "$PRINTER_NAME" -E -v "$PRINTER_URI" -P "$PP_PATH" 2>&1 | tee -a "$LOG_FILE" || \
      lpadmin -p "$PRINTER_NAME" -E -v "$PRINTER_URI" -m "$PP_PATH" 2>&1 | tee -a "$LOG_FILE" || true
  fi
  cupsenable "$PRINTER_NAME" 2>/dev/null || true
  cupsaccept "$PRINTER_NAME" 2>/dev/null || true
  lpstat -p "$PRINTER_NAME" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Adicionar" "$PRINTER_NAME" "PASS" "$PRINTER_URI"
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Printer Add lpadmin" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive