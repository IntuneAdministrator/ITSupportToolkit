#!/bin/zsh
# ============================================================
# Script:      print-spool-reset-stuck-mac.sh
# Descrição:   Reseta spool/fila de impressão travada.
# Categoria:   37_Printer-Management-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh print-spool-reset-stuck-mac.sh
...
section_banner "RESET SPOOL / FILA TRAVADA"
show_progress 2 "$TOTAL_STEPS" "Parar cupsd e limpar spool"
launchctl stop system/org.cups.cupsd 2>/dev/null || true
cancel -a 2>/dev/null || true
rm -f /var/spool/cups/c* /var/spool/cups/d* 2>/dev/null || true
add_report_row "Fila" "cancel -a + spool limpo" "PASS" "/var/spool/cups"
show_progress 5 "$TOTAL_STEPS" "Reiniciar CUPS"
launchctl kickstart -k system/org.cups.cupsd 2>&1 | tee -a "$LOG_FILE" || true
lpstat -o 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "cupsd" "kickstart" "PASS" ""

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Print Spool Reset" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive