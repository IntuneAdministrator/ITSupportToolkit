#!/bin/zsh
# ============================================================
# Script:      printers-list-installed-mac.sh
# Descrição:   Lista impressoras instaladas.
# Categoria:   37_Printer-Management-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh printers-list-installed-mac.sh
...
section_banner "IMPRESSORAS INSTALADAS"
show_progress 2 "$TOTAL_STEPS" "lpstat -p -d"
lpstat -p -d 2>&1 | tee -a "$LOG_FILE" || true
lpstat -v 2>&1 | tee -a "$LOG_FILE" || true
COUNT=$(lpstat -p 2>/dev/null | grep -c '^printer' || echo 0)
add_report_row "Impressoras" "$COUNT" "INFO" "lpstat -p"
add_report_row "Default" "$(lpstat -d 2>/dev/null || true)" "INFO" ""

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Printers List Installed" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive