#!/bin/zsh
# ============================================================
# Script:      background-update-throttle-mac.sh
# Description: macOS IT Support quick repair for background update throttle mac.
#              Category: 04_Slow-Mac-Fix. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh background-update-throttle-mac.sh
...
section_banner "SLOW MAC FIX"
show_progress 2 "$TOTAL_STEPS" "Pressure snapshot"
add_report_row "Load" "$(uptime)" "INFO" ""
ps aux | sort -nrk 3 | head -n 10 | tee -a "$LOG_FILE" || true
add_report_row "Top CPU" "Logged" "INFO" "ps aux"

if [[ "$SCRIPT_BASE" == *disk* || "$SCRIPT_BASE" == *emergency* || "$SCRIPT_BASE" == *slow-mac* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Free space"
  find /private/tmp -type f -mtime +1 -delete 2>/dev/null || true
  find "$REAL_HOME/Library/Caches" -type f -mtime +7 -delete 2>/dev/null || true
  tmutil thinlocalsnapshots / 10000000000 4 2>/dev/null || true
  add_report_row "Cleanup" "tmp/caches/snapshots trimmed" "PASS" ""
  add_report_row "Disk after" "$(df -h / | tail -n 1)" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *cpu* || "$SCRIPT_BASE" == *kill* ]]; then
  # Only kill obvious hung user helpers - never kernel/system critical
  pkill -f "Microsoft Update\|OneDrive.*Updater\|softwareupdated" 2>/dev/null || true
  add_report_row "Background updaters" "pkill attempted" "INFO" "Safe helpers only"
fi

if [[ "$SCRIPT_BASE" == *startup* || "$SCRIPT_BASE" == *background* ]]; then
  find /Library/LaunchAgents /Library/LaunchDaemons "$REAL_HOME/Library/LaunchAgents" -name '*.plist' 2>/dev/null | wc -l | tee -a "$LOG_FILE"
  add_report_row "Launch items" "Counted" "INFO" "Review Login Items in System Settings"
fi

if [[ "$SCRIPT_BASE" == *spotlight* ]]; then
  mdutil -E / 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Spotlight" "mdutil -E /" "PASS" ""
fi

if [[ "$SCRIPT_BASE" == *ram* || "$SCRIPT_BASE" == *ssd* ]]; then
  sysctl vm.swapusage 2>&1 | tee -a "$LOG_FILE" || true
  system_profiler SPNVMeDataType SPSerialATADataType 2>/dev/null | head -n 40 | tee -a "$LOG_FILE" || true
  add_report_row "RAM/SSD" "Sampled" "INFO" ""
fi
show_progress 6 "$TOTAL_STEPS" "Slow Mac done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "background update throttle mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive