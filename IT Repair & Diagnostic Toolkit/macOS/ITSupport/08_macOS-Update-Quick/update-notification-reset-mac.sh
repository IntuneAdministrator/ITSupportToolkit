#!/bin/zsh
# ============================================================
# Script:      update-notification-reset-mac.sh
# Description: macOS IT Support quick repair for update notification reset mac.
#              Category: 08_macOS-Update-Quick. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh update-notification-reset-mac.sh
...
FOLDER_NAME='08_macOS-Update-Quick'
...
section_banner "macOS UPDATE QUICK"
show_progress 2 "$TOTAL_STEPS" "Update status"
softwareupdate -l 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "softwareupdate -l" "Listed" "INFO" ""

if [[ "$SCRIPT_BASE" == *stuck* || "$SCRIPT_BASE" == *pause* || "$SCRIPT_BASE" == *notification* ]]; then
  rm -rf /Library/Updates/* 2>/dev/null || true
  softwareupdate --clear-catalog 2>/dev/null || true
  add_report_row "Update cache" "Cleared" "PASS" "/Library/Updates"
fi

if [[ "$SCRIPT_BASE" == *force* ]]; then
  softwareupdate -l --force 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Force check" "Invoked" "PASS" "softwareupdate -l --force"
fi

if [[ "$SCRIPT_BASE" == *mdm* ]]; then
  profiles show 2>&1 | head -n 40 | tee -a "$LOG_FILE" || true
  add_report_row "MDM update profiles" "Reviewed sample" "INFO" "Deferral policies in profiles"
fi
show_progress 5 "$TOTAL_STEPS" "Update quick done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "update notification reset mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive