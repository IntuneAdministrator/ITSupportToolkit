#!/bin/zsh
# ============================================================
# Script:      teamviewer-anydesk-detection-mac.sh
# Description: macOS IT Support quick repair for teamviewer anydesk detection mac.
#              Category: 10_Remote-Tools-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh teamviewer-anydesk-detection-mac.sh
...
FOLDER_NAME='10_Remote-Tools-Mac'
...
section_banner "REMOTE TOOLS"
show_progress 2 "$TOTAL_STEPS" "Enable remote support options"

if [[ "$SCRIPT_BASE" == *remote-login* || "$SCRIPT_BASE" == *ssh* || "$SCRIPT_BASE" == *enable-remote-support* ]]; then
  systemsetup -setremotelogin on 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Remote Login (SSH)" "Enabled" "PASS" "systemsetup -setremotelogin on"
fi

if [[ "$SCRIPT_BASE" == *remote-management* || "$SCRIPT_BASE" == *ard* || "$SCRIPT_BASE" == *screen-share* ]]; then
  /System/Library/CoreServices/RemoteManagement/ARDAgent.app/Contents/Resources/kickstart -activate -configure -access -on -restart -agent -privs -all 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "ARD/Remote Management" "kickstart attempted" "PASS" "Review MDM restrictions"
fi

if [[ "$SCRIPT_BASE" == *teamviewer* || "$SCRIPT_BASE" == *anydesk* || "$SCRIPT_BASE" == *detection* ]]; then
  for app in TeamViewer AnyDesk "Chrome Remote Desktop Host"; do
    if [[ -d "/Applications/$app.app" ]] || ls /Applications 2>/dev/null | grep -qi "$app"; then
      add_report_row "$app" "Detected" "INFO" "/Applications"
    else
      add_report_row "$app" "Not found" "INFO" ""
    fi
  done
  ps aux | grep -iE 'TeamViewer|AnyDesk' | grep -v grep | tee -a "$LOG_FILE" || true
fi
show_progress 5 "$TOTAL_STEPS" "Remote tools done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "teamviewer anydesk detection mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive