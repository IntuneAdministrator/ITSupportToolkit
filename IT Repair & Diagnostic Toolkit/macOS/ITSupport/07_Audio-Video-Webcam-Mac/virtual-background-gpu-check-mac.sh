#!/bin/zsh
# ============================================================
# Script:      virtual-background-gpu-check-mac.sh
# Description: macOS IT Support quick repair for virtual background gpu check mac.
#              Category: 07_Audio-Video-Webcam-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh virtual-background-gpu-check-mac.sh
...
FOLDER_NAME='07_Audio-Video-Webcam-Mac'
...
section_banner "AUDIO / VIDEO / WEBCAM"
show_progress 2 "$TOTAL_STEPS" "Reset A/V stack"
killall coreaudiod 2>/dev/null || true
add_report_row "coreaudiod" "Restarted" "PASS" ""

if [[ "$SCRIPT_BASE" == *mic* || "$SCRIPT_BASE" == *webcam* || "$SCRIPT_BASE" == *camera* || "$SCRIPT_BASE" == *facetime* || "$SCRIPT_BASE" == *tcc* || "$SCRIPT_BASE" == *continuity* || "$SCRIPT_BASE" == *teams-zoom* ]]; then
  ...
  add_report_row "Camera/Mic TCC" "Reset + assistants restarted" "PASS" "tccutil"
fi

if [[ "$SCRIPT_BASE" == *monitor* || "$SCRIPT_BASE" == *resolution* || "$SCRIPT_BASE" == *screen* ]]; then
  killall Dock ControlCenter SystemUIServer 2>/dev/null || true
  system_profiler SPDisplaysDataType 2>/dev/null | head -n 40 | tee -a "$LOG_FILE" || true
  add_report_row "Displays" "UI agents restarted" "PASS" ""
fi

if [[ "$SCRIPT_BASE" == *gpu* || "$SCRIPT_BASE" == *virtual-background* ]]; then
  system_profiler SPDisplaysDataType 2>/dev/null | head -n 30 | tee -a "$LOG_FILE" || true
  ps aux | grep -iE 'obs|mmhmm|camtwist|zoom|teams' | grep -v grep | tee -a "$LOG_FILE" || true
  add_report_row "GPU/virtual cam" "Sampled" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "A/V done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "virtual background gpu check mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive