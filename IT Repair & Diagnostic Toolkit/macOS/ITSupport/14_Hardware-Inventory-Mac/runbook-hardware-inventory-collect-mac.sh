...
# ============================================================
# Script: runbook-hardware-inventory-collect-mac.sh | Phase 13 common problems | ITSupport/14_Hardware-Inventory-Mac
# Requires: sudo | Usage: sudo zsh runbook-hardware-inventory-collect-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='14_Hardware-Inventory-Mac'
...
add_report_row "Module" "14_Hardware-Inventory-Mac" "INFO" "ITSupport"
...
if [[ "$SCRIPT_BASE" == *runbook* || "$SCRIPT_BASE" == *jamf-policy-template* || "$SCRIPT_BASE" == *jamf-self-service* || "$SCRIPT_BASE" == *jamf-post-enrollment* || "$SCRIPT_BASE" == *intune-mac-runbook* || "$SCRIPT_BASE" == *intune-mac-remediation* || "$SCRIPT_BASE" == *intune-mac-required* ]]; then
  show_progress 4 "$TOTAL_STEPS" "Runbook orchestration"
  add_report_row "Runbook" "$SCRIPT_BASE" "INFO" "Batch J multi-step"
  if [[ "$SCRIPT_BASE" == *network* || "$SCRIPT_BASE" == *connectivity* || "$SCRIPT_BASE" == *wifi* ]]; then
    dscacheutil -flushcache 2>/dev/null || true
    killall -HUP mDNSResponder 2>/dev/null || true
    add_report_row "Network runbook" "DNS flush + mDNS restart" "PASS" ""
  fi
  if [[ "$SCRIPT_BASE" == *printer* || "$SCRIPT_BASE" == *print* ]]; then
    cancel -a "$REAL_USER" 2>/dev/null || true
    lpstat -p 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "Printer runbook" "Queue cleared; printers listed" "PASS" ""
  fi
  if [[ "$SCRIPT_BASE" == *m365* || "$SCRIPT_BASE" == *office365* ]]; then
    killall "Microsoft Teams" "Microsoft Outlook" 2>/dev/null || true
    add_report_row "M365 runbook" "Teams/Outlook quit" "PASS" ""
  fi
  if [[ "$SCRIPT_BASE" == *slow* || "$SCRIPT_BASE" == *disk-cleanup* || "$SCRIPT_BASE" == *performance* ]]; then
    df -h / 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "Performance runbook" "Disk free space captured" "INFO" ""
  fi
  if [[ "$SCRIPT_BASE" == *security* || "$SCRIPT_BASE" == *baseline* ]]; then
    /usr/libexec/ApplicationFirewall/socketfilterfw --getglobalstate 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "Security runbook" "Firewall state captured" "INFO" ""
  fi
  if [[ "$SCRIPT_BASE" == *update* || "$SCRIPT_BASE" == *macos-update* ]]; then
    softwareupdate -l 2>&1 | head -n 10 | tee -a "$LOG_FILE" || true
    add_report_row "Update runbook" "softwareupdate -l captured" "INFO" ""
  fi
  if [[ "$SCRIPT_BASE" == *backup* || "$SCRIPT_BASE" == *timemachine* ]]; then
    tmutil destinationinfo 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "Backup runbook" "Time Machine destinations listed" "INFO" ""
  fi
  if [[ "$SCRIPT_BASE" == *finder* || "$SCRIPT_BASE" == *spotlight* || "$SCRIPT_BASE" == *browser* || "$SCRIPT_BASE" == *helpdesk-first* ]]; then
    killall Finder 2>/dev/null || true
    add_report_row "Helpdesk runbook" "Finder/shell refresh" "PASS" ""
  fi
  if [[ "$SCRIPT_BASE" == *jamf-policy-template* || "$SCRIPT_BASE" == *jamf-self-service* || "$SCRIPT_BASE" == *jamf-post-enrollment* ]]; then
    add_report_row "Jamf deploy" "Create policy: Scripts > priority  Before/After; map to smart group" "INFO" "Self Service category: IT Repair"
  fi
  if [[ "$SCRIPT_BASE" == *intune-mac-runbook* || "$SCRIPT_BASE" == *intune-mac-remediation* || "$SCRIPT_BASE" == *intune-mac-required* ]]; then
    add_report_row "Intune macOS" "Upload as macOS script; run as admin; assign to device groups" "INFO" "Bundle related scripts per category"
  fi
  if [[ "$SCRIPT_BASE" == *hardware-inventory* ]]; then
    system_profiler SPHardwareDataType SPUSBDataType 2>/dev/null | head -n 40 | tee -a "$LOG_FILE" || true
    add_report_row "Hardware runbook" "Inventory captured" "PASS" ""
  fi
  add_report_row "Runbook complete" "Review log + HTML report" "PASS" ""
fi

show_progress 5 "$TOTAL_STEPS" "Complete"
add_report_row "Script" "$SCRIPT_BASE" "INFO" "14_Hardware-Inventory-Mac"
...
export_html_report "runbook hardware inventory collect mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive