...
# ============================================================
# Script: offboarding-mac-local-wipe-prep.sh | ITSupport/20_Onboarding-Mac-Quick | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh offboarding-mac-local-wipe-prep.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='20_Onboarding-Mac-Quick'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "20_Onboarding-Mac-Quick" "INFO" "ITSupport"
section_banner "IDENTITY PLATFORM SSO"
show_progress 2 "$TOTAL_STEPS" "SSO / tokens"
profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Enrollment" "Captured" "INFO" ""
sysadminctl -secureTokenStatus "$REAL_USER" 2>&1 | tee -a "$LOG_FILE" || true
fdesetup list 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Secure Token / FV users" "Queried" "INFO" ""
if [[ "$SCRIPT_BASE" == *jamf-connect* ]]; then
  [[ -d "/Applications/Jamf Connect.app" ]] && add_report_row "Jamf Connect" "Installed" "PASS" "" || add_report_row "Jamf Connect" "Missing" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *bind* || "$SCRIPT_BASE" == *kerberos* ]]; then
  dsconfigad -show 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "AD bind" "dsconfigad" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Identity done" section_banner "ITSUPPORT MAC PHASE10"
...
export_html_report "offboarding mac local wipe prep" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive