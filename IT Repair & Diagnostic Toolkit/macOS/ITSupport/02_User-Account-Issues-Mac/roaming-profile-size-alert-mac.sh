#!/bin/zsh
# ============================================================
# Script:      roaming-profile-size-alert-mac.sh
# Description: macOS IT Support quick repair for roaming profile size alert mac.
#              Category: 02_User-Account-Issues-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh roaming-profile-size-alert-mac.sh
...
FOLDER_NAME='02_User-Account-Issues-Mac'
...
section_banner "USER ACCOUNT ISSUES"
show_progress 2 "$TOTAL_STEPS" "User context"
add_report_row "Real user" "$REAL_USER" "INFO" "$REAL_HOME"
CONSOLE_USER=$(stat -f%Su /dev/console 2>/dev/null || true)
add_report_row "Console user" "${CONSOLE_USER:-unknown}" "INFO" ""

if [[ "$SCRIPT_BASE" == *password* || "$SCRIPT_BASE" == *locked* || "$SCRIPT_BASE" == *mfa* ]]; then
  add_report_row "Password/MFA" "Reset via SysPrefs/Directory or IdP - not automated" "WARN" "Security: no password collection in script"
  sysadminctl -secureTokenStatus "$REAL_USER" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Secure Token" "Queried" "INFO" "sysadminctl"
fi

if [[ "$SCRIPT_BASE" == *login-loop* || "$SCRIPT_BASE" == *temp-profile* || "$SCRIPT_BASE" == *roaming* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Profile repair"
  if [[ "$REAL_USER" != "root" && -d "$REAL_HOME" ]]; then
    diskutil resetUserPermissions / "$REAL_USER" 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "resetUserPermissions" "$REAL_USER" "PASS" ""
    SIZE=$(du -sh "$REAL_HOME" 2>/dev/null | awk '{print $1}')
    add_report_row "Home size" "$SIZE" "INFO" "$REAL_HOME"
  fi
  rm -f "$REAL_HOME/Library/Preferences/ByHost/com.apple.loginwindow."*.plist 2>/dev/null || true
  add_report_row "loginwindow ByHost" "Cleared if present" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *credential* || "$SCRIPT_BASE" == *sso* ]]; then
  rm -f "$REAL_HOME/Library/Keychains/login.keychain-db.lock" 2>/dev/null || true
  add_report_row "Keychain lock" "Cleared" "PASS" ""
  detect_mdm_join_state
  add_report_row "MDM/SSO context" "$JOIN_STATE" "INFO" "Re-auth via Company Portal / Jamf Connect if used"
fi
show_progress 5 "$TOTAL_STEPS" "Account done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "roaming profile size alert mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive