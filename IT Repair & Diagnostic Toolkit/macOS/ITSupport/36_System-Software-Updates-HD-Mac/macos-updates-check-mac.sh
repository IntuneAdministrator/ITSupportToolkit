#!/bin/zsh
# ============================================================
# Script:      macos-updates-check-mac.sh
# Descrição:   Lista atualizações disponíveis (softwareupdate).
# Categoria:   36_System-Software-Updates-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh macos-updates-check-mac.sh
...
section_banner "VERIFICAR ATUALIZAÇÕES macOS"
show_progress 2 "$TOTAL_STEPS" "softwareupdate -l"
LIST=$(softwareupdate -l 2>&1 || true)
echo "$LIST" | tee -a "$LOG_FILE"
if echo "$LIST" | grep -qi 'No new software available\|No updates available'; then
  add_report_row "Atualizações" "nenhuma" "PASS" ""
elif echo "$LIST" | grep -qi 'Label:\|Title:'; then
  add_report_row "Atualizações" "disponíveis" "WARN" "ver log"
else
  add_report_row "softwareupdate" "saída" "INFO" "$LIST"
fi
sw_vers | tee -a "$LOG_FILE"
add_report_row "sw_vers" "$(sw_vers -productVersion 2>/dev/null)" "INFO" "$(sw_vers -buildVersion 2>/dev/null)"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "macOS Updates Check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive