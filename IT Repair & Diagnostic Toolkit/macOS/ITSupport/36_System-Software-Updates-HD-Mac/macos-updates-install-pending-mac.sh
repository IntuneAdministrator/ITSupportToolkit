#!/bin/zsh
# ============================================================
# Script:      macos-updates-install-pending-mac.sh
# Descrição:   Instala atualizações pendentes do macOS.
# Categoria:   36_System-Software-Updates-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh macos-updates-install-pending-mac.sh
...
section_banner "INSTALAR ATUALIZAÇÕES PENDENTES"
show_progress 2 "$TOTAL_STEPS" "Listar"
softwareupdate -l 2>&1 | tee -a "$LOG_FILE" || true
show_progress 4 "$TOTAL_STEPS" "Instalar todas (pode reiniciar)"
# --agree-to-license necessário em versões recentes
if softwareupdate -i -a --agree-to-license 2>&1 | tee -a "$LOG_FILE"; then
  add_report_row "Instalação" "concluída/solicitada" "PASS" "softwareupdate -i -a"
else
  add_report_row "Instalação" "falha ou reboot pendente" "WARN" "verifique log"
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "macOS Updates Install" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive