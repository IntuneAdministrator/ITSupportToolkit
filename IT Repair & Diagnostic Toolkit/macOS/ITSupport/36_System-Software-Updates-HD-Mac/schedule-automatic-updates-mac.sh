#!/bin/zsh
# ============================================================
# Script:      schedule-automatic-updates-mac.sh
# Descrição:   Habilita atualizações automáticas.
# Categoria:   36_System-Software-Updates-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh schedule-automatic-updates-mac.sh
...
FOLDER_NAME='36_System-Software-Updates-HD-Mac'
...
section_banner "AGENDAR ATUALIZAÇÕES AUTOMÁTICAS"
show_progress 2 "$TOTAL_STEPS" "softwareupdate --schedule / defaults"
# Habilitar download/instalação automática de atualizações de segurança
softwareupdate --schedule on 2>&1 | tee -a "$LOG_FILE" || true
defaults write /Library/Preferences/com.apple.SoftwareUpdate AutomaticCheckEnabled -bool true 2>&1 | tee -a "$LOG_FILE" || true
defaults write /Library/Preferences/com.apple.SoftwareUpdate AutomaticDownload -bool true 2>&1 | tee -a "$LOG_FILE" || true
defaults write /Library/Preferences/com.apple.SoftwareUpdate CriticalUpdateInstall -bool true 2>&1 | tee -a "$LOG_FILE" || true
defaults write /Library/Preferences/com.apple.commerce AutoUpdate -bool true 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Auto check" "on" "PASS" "SoftwareUpdate"
add_report_row "Auto download" "on" "PASS" ""
add_report_row "Critical updates" "on" "PASS" ""
softwareupdate --schedule 2>&1 | tee -a "$LOG_FILE" || true

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Schedule Automatic Updates" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive