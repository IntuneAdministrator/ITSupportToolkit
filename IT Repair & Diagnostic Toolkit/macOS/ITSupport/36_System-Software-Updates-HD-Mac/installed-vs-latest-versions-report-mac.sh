#!/bin/zsh
# ============================================================
# Script:      installed-vs-latest-versions-report-mac.sh
# Descrição:   Relatório versões instaladas vs disponíveis.
# Categoria:   36_System-Software-Updates-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh installed-vs-latest-versions-report-mac.sh
...
section_banner "VERSÕES INSTALADAS vs DISPONÍVEIS"
show_progress 2 "$TOTAL_STEPS" "macOS atual"
PROD=$(sw_vers -productVersion); BUILD=$(sw_vers -buildVersion)
add_report_row "macOS instalado" "$PROD ($BUILD)" "INFO" ""
echo "Installed: $PROD $BUILD" | tee -a "$LOG_FILE"
show_progress 4 "$TOTAL_STEPS" "Atualizações listadas"
softwareupdate -l 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Pendências" "ver log" "INFO" "softwareupdate -l"
BREW="$BREW_PREFIX/bin/brew"; [[ -x "$BREW" ]] || BREW=$(command -v brew 2>/dev/null || true)
RUN_AS=${SUDO_USER:-$REAL_USER}
if [[ -n "$BREW" && "$RUN_AS" != "root" ]]; then
  sudo -u "$RUN_AS" -H "$BREW" outdated --verbose 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "brew outdated" "verbose" "INFO" ""
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Installed vs Latest Report" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive