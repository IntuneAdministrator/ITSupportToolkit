#!/bin/zsh
# ============================================================
# Script:      homebrew-packages-update-all-mac.sh
# Descrição:   Atualiza pacotes Homebrew.
# Categoria:   36_System-Software-Updates-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh homebrew-packages-update-all-mac.sh
...
section_banner "HOMEBREW — ATUALIZAR PACOTES"
show_progress 2 "$TOTAL_STEPS" "Localizar brew"
BREW="$BREW_PREFIX/bin/brew"
if [[ ! -x "$BREW" ]]; then
  BREW=$(command -v brew 2>/dev/null || true)
fi
if [[ -z "$BREW" || ! -x "$BREW" ]]; then
  add_report_row "Homebrew" "não instalado" "WARN" "$BREW_PREFIX"
else
  # brew não deve rodar como root
  RUN_AS=${SUDO_USER:-$REAL_USER}
  if [[ "$RUN_AS" == "root" ]]; then
    add_report_row "brew" "pule — sem usuário não-root" "WARN" ""
  else
    show_progress 4 "$TOTAL_STEPS" "brew update && upgrade"
    sudo -u "$RUN_AS" -H "$BREW" update 2>&1 | tee -a "$LOG_FILE" || true
    sudo -u "$RUN_AS" -H "$BREW" upgrade 2>&1 | tee -a "$LOG_FILE" || true
    sudo -u "$RUN_AS" -H "$BREW" outdated 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "brew upgrade" "executado" "PASS" "$BREW"
  fi
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Homebrew Update All" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive