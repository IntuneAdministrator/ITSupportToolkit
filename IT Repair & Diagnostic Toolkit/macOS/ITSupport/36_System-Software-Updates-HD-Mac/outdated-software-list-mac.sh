#!/bin/zsh
# ============================================================
# Script:      outdated-software-list-mac.sh
# Descrição:   Lista software desatualizado.
# Categoria:   36_System-Software-Updates-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh outdated-software-list-mac.sh
...
section_banner "SOFTWARE DESATUALIZADO"
show_progress 2 "$TOTAL_STEPS" "softwareupdate + brew outdated"
softwareupdate -l 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "macOS updates" "listado" "INFO" "softwareupdate -l"
BREW="$BREW_PREFIX/bin/brew"
[[ -x "$BREW" ]] || BREW=$(command -v brew 2>/dev/null || true)
RUN_AS=${SUDO_USER:-$REAL_USER}
if [[ -n "$BREW" && -x "$BREW" && "$RUN_AS" != "root" ]]; then
  OUT=$(sudo -u "$RUN_AS" -H "$BREW" outdated 2>/dev/null || true)
  echo "$OUT" | tee -a "$LOG_FILE"
  if [[ -z "$OUT" ]]; then add_report_row "brew outdated" "nenhum" "PASS" ""; else add_report_row "brew outdated" "itens" "WARN" "$OUT"; fi
else
  add_report_row "brew" "indisponível" "INFO" ""
fi
system_profiler SPApplicationsDataType 2>/dev/null | grep -E 'Location:|Version:' | head -n 80 | tee -a "$LOG_FILE" || true
add_report_row "Apps sample" "system_profiler" "INFO" "amostra"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Outdated Software List" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive