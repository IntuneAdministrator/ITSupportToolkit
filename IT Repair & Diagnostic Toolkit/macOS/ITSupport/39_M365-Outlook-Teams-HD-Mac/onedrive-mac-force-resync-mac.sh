#!/bin/zsh
# ============================================================
# Script:      onedrive-mac-force-resync-mac.sh
# Descrição:   Força re-sincronização do OneDrive.
# Categoria:   39_M365-Outlook-Teams-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh onedrive-mac-force-resync-mac.sh
...
section_banner "FORÇAR RE-SYNC ONEDRIVE"
show_progress 2 "$TOTAL_STEPS" "Reiniciar OneDrive"
pkill -x "OneDrive" 2>/dev/null || true
pkill -f "OneDrive.app" 2>/dev/null || true
sleep 2
# Limpa caches leves (não remove dados do cloud)
CACHE="$REAL_HOME/Library/Containers/com.microsoft.OneDrive-Mac/Data/Library/Caches"
[[ -d "$CACHE" ]] && rm -rf "$CACHE"/* 2>/dev/null || true
add_report_row "Cache OneDrive" "limpo" "PASS" "$CACHE"
show_progress 5 "$TOTAL_STEPS" "Reabrir OneDrive"
if [[ -d "/Applications/OneDrive.app" ]]; then
  sudo -u "$REAL_USER" open -a "OneDrive" 2>&1 | tee -a "$LOG_FILE" || open -a "OneDrive" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "OneDrive" "reaberto" "PASS" "sync deve retomar"
else
  add_report_row "OneDrive.app" "não encontrado" "WARN" "/Applications"
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "OneDrive Force Resync" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive