#!/bin/zsh
# ============================================================
# Script:      outlook-mac-cache-clear-mac.sh
# Descrição:   Limpa cache do Microsoft Outlook para Mac.
# Categoria:   39_M365-Outlook-Teams-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh outlook-mac-cache-clear-mac.sh
...
section_banner "LIMPAR CACHE OUTLOOK PARA MAC"
show_progress 2 "$TOTAL_STEPS" "Encerrar Outlook"
pkill -x "Microsoft Outlook" 2>/dev/null || true
sleep 1
TARGET_HOME="$REAL_HOME"
show_progress 4 "$TOTAL_STEPS" "Remover caches Office/Outlook"
# Caminhos comuns Outlook/Office 365 Mac
PATHS=(
  "$TARGET_HOME/Library/Containers/com.microsoft.Outlook/Data/Library/Application Support/Microsoft/Office/16.0/OfficeFileCache"
  "$TARGET_HOME/Library/Group Containers/UBF8T346G9.Office/Outlook/Outlook 15 Profiles/Main Profile/Files/Temp"
  "$TARGET_HOME/Library/Caches/com.microsoft.Outlook"
  "$TARGET_HOME/Library/Containers/com.microsoft.Outlook/Data/Library/Caches"
)
for p in "${PATHS[@]}"; do
  if [[ -e "$p" ]]; then
    rm -rf "$p" 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "Limpar" "$p" "PASS" ""
  else
    add_report_row "Ausente" "$p" "INFO" ""
  fi
done
add_report_row "Outlook" "cache limpo" "PASS" "reabra o Outlook"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Outlook Mac Cache Clear" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive