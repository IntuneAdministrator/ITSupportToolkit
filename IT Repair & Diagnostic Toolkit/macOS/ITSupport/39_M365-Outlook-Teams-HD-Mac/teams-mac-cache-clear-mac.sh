#!/bin/zsh
# ============================================================
# Script:      teams-mac-cache-clear-mac.sh
# Descrição:   Limpa cache do Microsoft Teams.
# Categoria:   39_M365-Outlook-Teams-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh teams-mac-cache-clear-mac.sh
...
section_banner "LIMPAR CACHE MICROSOFT TEAMS"
show_progress 2 "$TOTAL_STEPS" "Encerrar Teams"
pkill -x "Microsoft Teams" 2>/dev/null || true
pkill -x "Teams" 2>/dev/null || true
pkill -f "Microsoft Teams" 2>/dev/null || true
sleep 1
show_progress 4 "$TOTAL_STEPS" "Remover caches"
PATHS=(
  "$REAL_HOME/Library/Application Support/Microsoft/Teams/Cache"
  "$REAL_HOME/Library/Application Support/Microsoft/Teams/GPUCache"
  "$REAL_HOME/Library/Application Support/Microsoft/Teams/Code Cache"
  "$REAL_HOME/Library/Application Support/Microsoft/Teams/blob_storage"
  "$REAL_HOME/Library/Group Containers/UBF8T346G9.Office/tmp"
  "$REAL_HOME/Library/Containers/com.microsoft.teams2/Data/Library/Application Support/Microsoft/MSTeams/EBWebView/WV2Profile_tfw/Cache"
  "$REAL_HOME/Library/Caches/com.microsoft.teams"
  "$REAL_HOME/Library/Caches/com.microsoft.teams2"
)
for p in "${PATHS[@]}"; do
  if [[ -e "$p" ]]; then
    rm -rf "$p" 2>/dev/null || true
    add_report_row "Limpar" "$(basename "$p")" "PASS" "$p"
  else
    add_report_row "Ausente" "$p" "INFO" ""
  fi
done

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Teams Mac Cache Clear" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive