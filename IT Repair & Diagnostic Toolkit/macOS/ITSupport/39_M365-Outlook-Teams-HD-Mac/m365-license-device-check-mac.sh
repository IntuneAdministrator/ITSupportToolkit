#!/bin/zsh
# ============================================================
# Script:      m365-license-device-check-mac.sh
# Descrição:   Verifica apps/licença M365 no dispositivo.
# Categoria:   39_M365-Outlook-Teams-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh m365-license-device-check-mac.sh
...
FOLDER_NAME='39_M365-Outlook-Teams-HD-Mac'
...
section_banner "VERIFICAR LICENÇA / APPS M365 NO DISPOSITIVO"
show_progress 2 "$TOTAL_STEPS" "Apps Microsoft instalados"
for app in "Microsoft Outlook" "Microsoft Teams" "Microsoft Word" "Microsoft Excel" "Microsoft PowerPoint" "OneDrive" "Company Portal"; do
  if [[ -d "/Applications/$app.app" ]]; then
    VER=$(defaults read "/Applications/$app.app/Contents/Info" CFBundleShortVersionString 2>/dev/null || echo "?")
    add_report_row "$app" "instalado ($VER)" "PASS" "/Applications/$app.app"
    echo "$app = $VER" | tee -a "$LOG_FILE"
  else
    add_report_row "$app" "não instalado" "INFO" ""
  fi
done
show_progress 5 "$TOTAL_STEPS" "Licenciamento Office (se disponível)"
# officelicensetokengen / licensing não é 100% CLI público; usamos evidências locais
LS=(
  "$REAL_HOME/Library/Group Containers/UBF8T346G9.Office/LicenseInformation"
  "$REAL_HOME/Library/Preferences/com.microsoft.office.licensingV2.plist"
  "/Library/Preferences/com.microsoft.office.licensingV2.plist"
)
for p in "${LS[@]}"; do
  if [[ -e "$p" ]]; then
    add_report_row "Licensing artifact" "presente" "INFO" "$p"
  fi
done
# Conta OneDrive / Company Portal hints
ls "$REAL_HOME/Library/Containers/com.microsoft.OneDrive-Mac" >/dev/null 2>&1 && add_report_row "OneDrive container" "presente" "INFO" "" || true

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "M365 License Device Check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive