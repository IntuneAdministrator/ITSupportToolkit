...
# ============================================================
# Script: company-portal-signin-repair-mac.sh | Role: ITSupport/13_Identity-MFA-Helpdesk-Mac | v1.1.0 expansion
# Requires: sudo | Usage: sudo zsh company-portal-signin-repair-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='13_Identity-MFA-Helpdesk-Mac'
...
add_report_row "Module" "13_Identity-MFA-Helpdesk-Mac" "INFO" "ITSupport"
section_banner "ITSUPPORT MAC EXPANSION"
show_progress 2 "$TOTAL_STEPS" "Quick checks"
add_report_row "User" "$REAL_USER" "INFO" "$REAL_HOME"
if [[ "$SCRIPT_BASE" == *sso* || "$SCRIPT_BASE" == *keychain* || "$SCRIPT_BASE" == *company-portal* || "$SCRIPT_BASE" == *platform-sso* ]]; then
  ...
  add_report_row "SSO/keychain" "Enrollment checked; lock cleared" "PASS" ""
  open -a "Company Portal" 2>/dev/null || true
fi
if [[ "$SCRIPT_BASE" == *serial* || "$SCRIPT_BASE" == *battery* || "$SCRIPT_BASE" == *thunderbolt* || "$SCRIPT_BASE" == *asset* ]]; then
  system_profiler SPHardwareDataType SPPowerDataType SPThunderboltDataType 2>/dev/null | head -n 80 | tee -a "$LOG_FILE" || true
  add_report_row "Hardware profile" "Captured" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *teams* || "$SCRIPT_BASE" == *continuity* || "$SCRIPT_BASE" == *airplay* || "$SCRIPT_BASE" == *calendar* ]]; then
  killall "Microsoft Teams" 2>/dev/null || true
  add_report_row "Collaboration" "Teams quit if running; check Continuity settings" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "HTML report"
export_html_report "company portal signin repair mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive