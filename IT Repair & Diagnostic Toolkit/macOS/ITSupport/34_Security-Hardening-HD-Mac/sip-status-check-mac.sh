#!/bin/zsh
# ============================================================
# Script:      sip-status-check-mac.sh
# Descrição:   Verifica System Integrity Protection (SIP).
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh sip-status-check-mac.sh
...
section_banner "SIP (System Integrity Protection)"
show_progress 2 "$TOTAL_STEPS" "csrutil status"
SIP_OUT=$(csrutil status 2>&1 || true)
echo "$SIP_OUT" | tee -a "$LOG_FILE"
if echo "$SIP_OUT" | grep -qi 'enabled'; then
  log_success "SIP habilitado"
  add_report_row "SIP" "enabled" "PASS" "$SIP_OUT"
elif echo "$SIP_OUT" | grep -qi 'disabled'; then
  log_warn "SIP desabilitado — risco elevado"
  add_report_row "SIP" "disabled" "WARN" "$SIP_OUT"
else
  add_report_row "SIP" "unknown" "INFO" "$SIP_OUT"
fi
show_progress 5 "$TOTAL_STEPS" "Checagens relacionadas"
nvram -p 2>/dev/null | grep -i 'csr-active-config' | tee -a "$LOG_FILE" || true
add_report_row "NVRAM csr" "consultado" "INFO" "csr-active-config se presente"
