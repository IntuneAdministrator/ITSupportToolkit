#!/bin/zsh
# ============================================================
# Script:      open-ports-process-list-mac.sh
# Descrição:   Lista portas abertas e processos associados.
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh open-ports-process-list-mac.sh
...
section_banner "PORTAS ABERTAS"
show_progress 2 "$TOTAL_STEPS" "lsof -iTCP -sTCP:LISTEN"
LISTEN=$(lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null || true)
if [[ -z "$LISTEN" ]]; then
  log_warn "Nenhuma porta TCP em LISTEN encontrada (ou permissão insuficiente)"
  add_report_row "LISTEN ports" "vazio" "WARN" "tente com sudo"
else
  echo "$LISTEN" | tee -a "$LOG_FILE"
  COUNT=$(echo "$LISTEN" | tail -n +2 | wc -l | tr -d ' ')
  add_report_row "LISTEN ports" "$COUNT processos/portas" "INFO" "lsof -iTCP -sTCP:LISTEN"
fi
show_progress 5 "$TOTAL_STEPS" "netstat summary"
netstat -anv -p tcp 2>/dev/null | grep LISTEN | tee -a "$LOG_FILE" || true
add_report_row "netstat LISTEN" "capturado" "INFO" ""

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Open Ports Process List" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive