#!/bin/zsh
# ============================================================
# Script:      security-hardening-system-report-mac.sh
# Descrição:   Relatório geral de segurança do sistema.
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh security-hardening-system-report-mac.sh
...
FOLDER_NAME='34_Security-Hardening-HD-Mac'
...
section_banner "RELATÓRIO GERAL DE SEGURANÇA"
show_progress 2 "$TOTAL_STEPS" "FileVault / SIP / Gatekeeper / Firewall"
FV=$(fdesetup status 2>&1 || true)
SIP=$(csrutil status 2>&1 || true)
GK=$(spctl --status 2>&1 || true)
AF=$(/usr/libexec/ApplicationFirewall/socketfilterfw --getglobalstate 2>&1 || true)
echo "$FV" | tee -a "$LOG_FILE"; echo "$SIP" | tee -a "$LOG_FILE"
echo "$GK" | tee -a "$LOG_FILE"; echo "$AF" | tee -a "$LOG_FILE"
echo "$FV" | grep -qi 'On' && add_report_row "FileVault" "On" "PASS" "" || add_report_row "FileVault" "Off/Unknown" "WARN" "$FV"
echo "$SIP" | grep -qi 'enabled' && add_report_row "SIP" "enabled" "PASS" "" || add_report_row "SIP" "disabled/unknown" "WARN" "$SIP"
echo "$GK" | grep -qi 'enabled' && add_report_row "Gatekeeper" "enabled" "PASS" "" || add_report_row "Gatekeeper" "check" "WARN" "$GK"
echo "$AF" | grep -qi 'enabled' && add_report_row "App Firewall" "enabled" "PASS" "" || add_report_row "App Firewall" "disabled/unknown" "WARN" "$AF"
show_progress 5 "$TOTAL_STEPS" "Secure Token / bootstrap token hints"
diskutil apfs listUsers / 2>&1 | head -n 40 | tee -a "$LOG_FILE" || true
add_report_row "APFS users" "diskutil apfs listUsers" "INFO" ""
softwareupdate --history 2>&1 | tail -n 15 | tee -a "$LOG_FILE" || true
add_report_row "Update history" "tail" "INFO" "softwareupdate --history"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Security Hardening System Report" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive