#!/bin/zsh
# ============================================================
# Script:      users-password-expiry-audit-mac.sh
# Descrição:   Audita usuários locais e políticas de senha.
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh users-password-expiry-audit-mac.sh
...
section_banner "AUDITORIA DE SENHAS DE USUÁRIOS"
show_progress 2 "$TOTAL_STEPS" "Enumerar usuários locais"
USERS=$(dscl . -list /Users UniqueID 2>/dev/null | awk '$2 >= 500 {print $1}')
for u in $USERS; do
  # Contas sem senha (AuthenticationAuthority vazio / ShadowHash sem dados)
  AUTH=$(dscl . -read "/Users/$u" AuthenticationAuthority 2>/dev/null || true)
  PASSWORDABLE=$(dscl . -read "/Users/$u" Password 2>/dev/null || true)
  DETAIL="AuthenticationAuthority presente"
  STATUS="PASS"
  if [[ -z "$AUTH" ]] || echo "$AUTH" | grep -qi 'No such key'; then
    # Pode ser conta sem senha / só SecureToken especial
    if echo "$PASSWORDABLE" | grep -q '\*'; then
      STATUS="WARN"; DETAIL="marcador de senha *; verificar se há senha válida"
      log_warn "Usuário $u — verificar senha"
    fi
  fi
  # policy: max password age se definido via pwpolicy
  POL=$(pwpolicy -u "$u" -getaccountpolicies 2>/dev/null | head -n 5 || true)
  add_report_row "User $u" "$STATUS" "$STATUS" "$DETAIL | policy sample: ${POL:0:80}"
  echo "=== $u ===" | tee -a "$LOG_FILE"
  echo "$AUTH" | head -n 3 | tee -a "$LOG_FILE" || true
done
show_progress 6 "$TOTAL_STEPS" "pwpolicy global"
pwpolicy -getaccountpolicies 2>&1 | head -n 40 | tee -a "$LOG_FILE" || true
add_report_row "pwpolicy global" "capturado" "INFO" ""

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Users Password Expiry Audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive