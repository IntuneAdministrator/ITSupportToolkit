#!/bin/zsh
# ============================================================
# Script:      gatekeeper-status-check-mac.sh
# Descrição:   Verifica status do Gatekeeper (spctl).
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh gatekeeper-status-check-mac.sh
...
section_banner "GATEKEEPER"
show_progress 2 "$TOTAL_STEPS" "spctl --status"
GK=$(spctl --status 2>&1 || true)
echo "$GK" | tee -a "$LOG_FILE"
if echo "$GK" | grep -qi 'assessments enabled'; then
  log_success "Gatekeeper habilitado"
  add_report_row "Gatekeeper" "enabled" "PASS" "$GK"
else
  log_warn "Gatekeeper pode estar desabilitado"
  add_report_row "Gatekeeper" "$GK" "WARN" "spctl --status"
fi
show_progress 4 "$TOTAL_STEPS" "Política de notarização / assessment"
spctl --status --verbose 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Gatekeeper verbose" "spctl --status --verbose" "INFO" ""
show_progress 6 "$TOTAL_STEPS" "XProtect / MRT (se disponível)"
system_profiler SPInstallHistoryDataType 2>/dev/null | grep -i 'XProtect\|MRT' | tail -n 20 | tee -a "$LOG_FILE" || true
add_report_row "XProtect/MRT" "histórico" "INFO" "SPInstallHistoryDataType"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Gatekeeper Status Check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive