#!/bin/zsh
# ============================================================
# Script:      filevault-status-check-mac.sh
# Descrição:   Verifica status da criptografia FileVault.
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh filevault-status-check-mac.sh
...
section_banner "FILEVAULT"
show_progress 2 "$TOTAL_STEPS" "Status do FileVault"
FV_OUT=$(fdesetup status 2>&1 || true)
echo "$FV_OUT" | tee -a "$LOG_FILE"
if echo "$FV_OUT" | grep -qi 'FileVault is On'; then
  log_success "FileVault está ATIVO"
  add_report_row "FileVault" "On" "PASS" "$FV_OUT"
elif echo "$FV_OUT" | grep -qi 'FileVault is Off'; then
  log_warn "FileVault está DESLIGADO"
  add_report_row "FileVault" "Off" "WARN" "$FV_OUT"
else
  log_error "Não foi possível determinar o status do FileVault"
  add_report_row "FileVault" "Unknown" "FAIL" "$FV_OUT"
fi
show_progress 5 "$TOTAL_STEPS" "Listar usuários autorizados (se ativo)"
fdesetup list 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "FileVault users" "fdesetup list" "INFO" "Usuários autorizados a desbloquear"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "FileVault Status Check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive