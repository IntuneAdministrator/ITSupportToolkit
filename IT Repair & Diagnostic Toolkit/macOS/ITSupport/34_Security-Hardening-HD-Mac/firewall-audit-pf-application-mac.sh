#!/bin/zsh
# ============================================================
# Script:      firewall-audit-pf-application-mac.sh
# Descrição:   Auditoria do Application Firewall e pf.
# Categoria:   34_Security-Hardening-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh firewall-audit-pf-application-mac.sh
...
section_banner "FIREWALL (Application Firewall + pf)"
show_progress 2 "$TOTAL_STEPS" "Application Firewall (socketfilterfw)"
SFW="/usr/libexec/ApplicationFirewall/socketfilterfw"
if [[ -x "$SFW" ]]; then
  AF_STATE=$($SFW --getglobalstate 2>&1 || true)
  AF_STEALTH=$($SFW --getstealthmode 2>&1 || true)
  echo "$AF_STATE" | tee -a "$LOG_FILE"
  echo "$AF_STEALTH" | tee -a "$LOG_FILE"
  if echo "$AF_STATE" | grep -qi 'enabled'; then
    add_report_row "Application Firewall" "enabled" "PASS" "$AF_STATE"
  else
    add_report_row "Application Firewall" "disabled/unknown" "WARN" "$AF_STATE"
  fi
  add_report_row "Stealth mode" "$AF_STEALTH" "INFO" ""
  $SFW --listapps 2>&1 | tee -a "$LOG_FILE" || true
else
  add_report_row "socketfilterfw" "não encontrado" "FAIL" ""
fi
show_progress 5 "$TOTAL_STEPS" "pfctl (packet filter)"
PF=$(pfctl -s info 2>&1 || true)
echo "$PF" | tee -a "$LOG_FILE"
if echo "$PF" | grep -qi 'Status: Enabled'; then
  add_report_row "pf" "Enabled" "PASS" "pfctl -s info"
else
  add_report_row "pf" "Disabled/unknown" "INFO" "$PF"
fi
pfctl -s rules 2>&1 | head -n 80 | tee -a "$LOG_FILE" || true
add_report_row "pf rules" "amostra" "INFO" "primeiras regras"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Firewall Audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive