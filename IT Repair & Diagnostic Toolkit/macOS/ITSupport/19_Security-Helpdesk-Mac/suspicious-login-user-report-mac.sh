...
# ============================================================
# Script: suspicious-login-user-report-mac.sh | ITSupport/19_Security-Helpdesk-Mac | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh suspicious-login-user-report-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='19_Security-Helpdesk-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "19_Security-Helpdesk-Mac" "INFO" "ITSupport"
section_banner "NETWORK ENTERPRISE MAC"
show_progress 2 "$TOTAL_STEPS" "Network enterprise"
scutil --proxy 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Proxy (scutil)" "Captured" "INFO" ""
networksetup -listallnetworkservices 2>&1 | tee -a "$LOG_FILE" || true
if [[ "$SCRIPT_BASE" == *8021x* || "$SCRIPT_BASE" == *wifi* ]]; then
  WIFI=$(networksetup -listallhardwareports 2>/dev/null | awk '/Wi-Fi|AirPort/{getline; print $2; exit}')
  [[ -n "${WIFI:-}" ]] && networksetup -getairportnetwork "$WIFI" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Wi-Fi/802.1X" "Status captured" "INFO" "${WIFI:-}"
fi
if [[ "$SCRIPT_BASE" == *vpn* || "$SCRIPT_BASE" == *ikev2* ]]; then
  scutil --nc list 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "VPN configs" "Listed" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *pf* || "$SCRIPT_BASE" == *firewall* ]]; then
  pfctl -s info 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "pf" "Info dumped" "INFO" "may be empty if unused"
fi
if [[ "$SCRIPT_BASE" == *mtu* ]]; then
  ping -D -s 1472 -c 1 1.1.1.1 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "MTU probe" "ping -D sample" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Net enterprise done" section_banner "ITSUPPORT MAC PHASE10"
show_progress 2 "$TOTAL_STEPS" "Helpdesk quick"
add_report_row "User" "$REAL_USER" "INFO" "$REAL_HOME"
if [[ "$SCRIPT_BASE" == *outlook* || "$SCRIPT_BASE" == *mail* || "$SCRIPT_BASE" == *calendar* || "$SCRIPT_BASE" == *focus* ]]; then
  killall "Microsoft Outlook" "Mail" "Calendar" 2>/dev/null || true
  add_report_row "Mail/Calendar apps" "Quit" "PASS" "Relaunch after"
fi
if [[ "$SCRIPT_BASE" == *safari* || "$SCRIPT_BASE" == *edge* || "$SCRIPT_BASE" == *chrome* || "$SCRIPT_BASE" == *browser* || "$SCRIPT_BASE" == *passwords* ]]; then
  killall Safari "Microsoft Edge" "Google Chrome" 2>/dev/null || true
  add_report_row "Browsers" "Quit" "PASS" ""
fi
if [[ "$SCRIPT_BASE" == *clamshell* || "$SCRIPT_BASE" == *usb-c* || "$SCRIPT_BASE" == *hotspot* || "$SCRIPT_BASE" == *captive* || "$SCRIPT_BASE" == *battery* ]]; then
  pmset -g batt 2>&1 | tee -a "$LOG_FILE" || true
  system_profiler SPDisplaysDataType 2>/dev/null | head -n 30 | tee -a "$LOG_FILE" || true
  add_report_row "Power/Display" "Sampled" "INFO" ""
  BODY=$(curl -fsS --max-time 8 http://captive.apple.com/hotspot-detect.html 2>/dev/null || true)
  echo "$BODY" | grep -qi Success && add_report_row "Captive" "Clear" "PASS" "" || add_report_row "Captive" "Check Wi-Fi portal" "WARN" ""
fi
if [[ "$SCRIPT_BASE" == *screen-lock* || "$SCRIPT_BASE" == *filevault* || "$SCRIPT_BASE" == *phishing* || "$SCRIPT_BASE" == *firewall* || "$SCRIPT_BASE" == *suspicious* ]]; then
  fdesetup status 2>&1 | tee -a "$LOG_FILE" || true
  /usr/libexec/ApplicationFirewall/socketfilterfw --getglobalstate 2>&1 | tee -a "$LOG_FILE" || true
  PACK="$LOG_DIR/SecHDMac_$TIMESTAMP"; mkdir -p "$PACK"
  last -10 > "$PACK/last.txt" 2>/dev/null || true
  add_report_row "Security pack" "$PACK" "PASS" ""
fi
if [[ "$SCRIPT_BASE" == *onboarding* || "$SCRIPT_BASE" == *company-portal* || "$SCRIPT_BASE" == *setup* || "$SCRIPT_BASE" == *migration* || "$SCRIPT_BASE" == *offboarding* ]]; then
  profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
  [[ -d "/Applications/Company Portal.app" ]] && open -a "Company Portal" 2>/dev/null || true
  add_report_row "Onboarding/MDM" "Enrollment checked" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *display* || "$SCRIPT_BASE" == *headset* || "$SCRIPT_BASE" == *printer* || "$SCRIPT_BASE" == *sd-card* || "$SCRIPT_BASE" == *magic* ]]; then
  killall coreaudiod 2>/dev/null || true
  launchctl kickstart -k system/org.cups.cupsd 2>/dev/null || true
  add_report_row "Peripherals" "Audio/CUPS refreshed" "PASS" ""
fi
show_progress 5 "$TOTAL_STEPS" "ITSupport Mac P10 done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "suspicious login user report mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive