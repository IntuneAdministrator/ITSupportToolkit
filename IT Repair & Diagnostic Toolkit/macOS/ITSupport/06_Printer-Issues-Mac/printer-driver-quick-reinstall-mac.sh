#!/bin/zsh
# ============================================================
# Script:      printer-driver-quick-reinstall-mac.sh
# Description: macOS IT Support quick repair for printer driver quick reinstall mac.
#              Category: 06_Printer-Issues-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh printer-driver-quick-reinstall-mac.sh
...
FOLDER_NAME='06_Printer-Issues-Mac'
...
section_banner "PRINTER QUICK FIX"
show_progress 2 "$TOTAL_STEPS" "CUPS repair"
...
add_report_row "Queue clear + cupsd" "Done" "PASS" ""
lpstat -p -d 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Printers" "lpstat" "INFO" ""

if [[ "$SCRIPT_BASE" == *airprint* || "$SCRIPT_BASE" == *driver* ]]; then
  lpinfo -v 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Discovery" "lpinfo -v" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *scan* ]]; then
  add_report_row "Scan-to-email" "Verify MFP DNS/SMTP on device" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Printer done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "printer driver quick reinstall mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive