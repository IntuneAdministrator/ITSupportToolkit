#!/bin/zsh
# ============================================================
# Script:      corporate-wifi-cert-repair-mac.sh
# Description: macOS IT Support quick repair for corporate wifi cert repair mac.
#              Category: 03_Connectivity-Issues-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh corporate-wifi-cert-repair-mac.sh
...
FOLDER_NAME='03_Connectivity-Issues-Mac'
...
section_banner "CONNECTIVITY QUICK FIX"
show_progress 2 "$TOTAL_STEPS" "Network basics"
DEFAULT_IF=$(route -n get default 2>/dev/null | awk '/interface:/{print $2}')
add_report_row "Default IF" "${DEFAULT_IF:-none}" "$([[ -n "${DEFAULT_IF:-}" ]] && echo PASS || echo FAIL)" ""

show_progress 3 "$TOTAL_STEPS" "DNS / proxy / captive"
...
add_report_row "Proxies" "Disabled on services" "PASS" "networksetup"

BODY=$(curl -fsS --max-time 10 http://captive.apple.com/hotspot-detect.html 2>/dev/null || true)
echo "$BODY" | grep -qi Success && add_report_row "Captive portal" "Clear" "PASS" "" || add_report_row "Captive portal" "Blocked/unclear" "WARN" "$BODY"

if [[ "$SCRIPT_BASE" == *wifi* || "$SCRIPT_BASE" == *corporate-wifi* ]]; then
  ...
  if [[ -n "${WIFI_IF:-}" ]]; then
    ...
    add_report_row "Wi-Fi cycle" "$WIFI_IF" "PASS" "$(networksetup -getairportnetwork "$WIFI_IF" 2>/dev/null || true)"
  fi
fi

if [[ "$SCRIPT_BASE" == *vpn* || "$SCRIPT_BASE" == *split-tunnel* ]]; then
  scutil --nc list 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "VPN configs" "scutil --nc list" "INFO" "Disconnect/reconnect from UI if needed"
fi

if [[ -n "${DEFAULT_IF:-}" ]]; then
  ipconfig set "$DEFAULT_IF" DHCP 2>/dev/null || true
  add_report_row "DHCP renew" "$DEFAULT_IF" "PASS" ""
fi
show_progress 6 "$TOTAL_STEPS" "Connectivity done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "corporate wifi cert repair mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive