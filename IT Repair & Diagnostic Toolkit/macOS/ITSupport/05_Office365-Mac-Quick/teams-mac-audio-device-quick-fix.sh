#!/bin/zsh
# ============================================================
# Script:      teams-mac-audio-device-quick-fix.sh
# Description: macOS IT Support quick repair for teams mac audio device quick fix.
#              Category: 05_Office365-Mac-Quick. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh teams-mac-audio-device-quick-fix.sh
...
FOLDER_NAME='05_Office365-Mac-Quick'
...
section_banner "M365 MAC QUICK"
show_progress 2 "$TOTAL_STEPS" "Quit + cache repair"
for app in "Microsoft Teams" "Microsoft Outlook" "Microsoft Word" "Microsoft Excel" "OneDrive" "Microsoft Teams (work or school)"; do
  killall "$app" 2>/dev/null || true
done
add_report_row "Office apps" "Quit attempted" "PASS" ""
...
if [[ "$SCRIPT_BASE" == *teams* ]]; then
  ...
  add_report_row "Teams cache" "Cleared" "PASS" ""
  if [[ "$SCRIPT_BASE" == *audio* ]]; then
    killall coreaudiod 2>/dev/null || true
    add_report_row "coreaudiod" "Restarted" "PASS" ""
  fi
fi

if [[ "$SCRIPT_BASE" == *outlook* || "$SCRIPT_BASE" == *calendar* ]]; then
  add_report_row "Outlook" "Quit for repair; reopen after" "INFO" "Clear DisabledItems via Outlook Safe Mode if needed"
fi

if [[ "$SCRIPT_BASE" == *onedrive* ]]; then
  open -a OneDrive 2>/dev/null || true
  add_report_row "OneDrive" "Relaunch" "PASS" ""
fi

if [[ "$SCRIPT_BASE" == *license* || "$SCRIPT_BASE" == *copilot* || "$SCRIPT_BASE" == *wont-open* ]]; then
  [[ -d "/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app" ]] && add_report_row "MAU" "Present" "PASS" "" || add_report_row "MAU" "Missing" "WARN" ""
  open "/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app" 2>/dev/null || true
fi
show_progress 5 "$TOTAL_STEPS" "M365 quick done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "teams mac audio device quick fix" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive