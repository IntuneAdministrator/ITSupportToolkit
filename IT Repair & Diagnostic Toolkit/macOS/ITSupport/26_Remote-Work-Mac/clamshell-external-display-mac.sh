...
# ============================================================
# Script: clamshell-external-display-mac.sh | ITSupport/26_Remote-Work-Mac | v1.3.0 Phase11
# Requires: sudo | Usage: sudo zsh clamshell-external-display-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='26_Remote-Work-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "26_Remote-Work-Mac" "INFO" "ITSupport"
section_banner "VIRTUALIZATION MAC"
show_progress 2 "$TOTAL_STEPS" "Virt"
sysctl -a kern.hv_support 2>/dev/null | tee -a "$LOG_FILE" || true
sysctl -n sysctl.proc_translated 2>/dev/null | tee -a "$LOG_FILE" || true
add_report_row "HV / Rosetta translated" "Queried" "INFO" ""
[[ -d "/Applications/Docker.app" ]] && add_report_row "Docker.app" "Installed" "INFO" "" || add_report_row "Docker.app" "Missing" "INFO" ""
[[ -d "/Applications/Parallels Desktop.app" ]] && add_report_row "Parallels" "Installed" "INFO" "" || true
[[ -d "/Applications/UTM.app" ]] && add_report_row "UTM" "Installed" "INFO" "" || true
pgrep -l qemu 2>/dev/null | head -n 10 | tee -a "$LOG_FILE" || true
show_progress 5 "$TOTAL_STEPS" "Virt done" section_banner "ITSUPPORT MAC PHASE11"
...
export_html_report "clamshell external display mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive