#!/bin/zsh
# ============================================================
# Script:      remote-diagnostics-packager-mac.sh
# Description: macOS IT Support quick repair for remote diagnostics packager mac.
#              Category: 01_Quick-Fix-Toolkit-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh remote-diagnostics-packager-mac.sh
...
FOLDER_NAME='01_Quick-Fix-Toolkit-Mac'
...
section_banner "QUICK FIX TOOLKIT"
show_progress 2 "$TOTAL_STEPS" "Health snapshot"
UPTIME=$(uptime 2>/dev/null || true)
add_report_row "Uptime" "$UPTIME" "INFO" ""
DF=$(df -h / | tail -n 1)
add_report_row "Disk /" "$DF" "INFO" "df -h /"
MEM=$(memory_pressure 2>&1 | head -n 8 || true)
add_report_row "Memory" "Sampled" "INFO" "$MEM"

show_progress 3 "$TOTAL_STEPS" "Safe common repairs"
dscacheutil -flushcache 2>/dev/null || true
killall -HUP mDNSResponder 2>/dev/null || true
add_report_row "DNS flush" "Done" "PASS" "dscacheutil + mDNSResponder"
...
add_report_row "CUPS/queue" "Restarted + cancel -a" "PASS" ""

if [[ "$SCRIPT_BASE" == *packager* || "$SCRIPT_BASE" == *evidence* || "$SCRIPT_BASE" == *triage* || "$SCRIPT_BASE" == *health* ]]; then
  show_progress 4 "$TOTAL_STEPS" "Evidence pack"
  PACK="$LOG_DIR/SupportPack_${TIMESTAMP}"
  mkdir -p "$PACK"
  system_profiler SPSoftwareDataType SPHardwareDataType SPNetworkDataType > "$PACK/system_profiler.txt" 2>/dev/null || true
  ifconfig > "$PACK/ifconfig.txt" 2>/dev/null || true
  profiles status -type enrollment > "$PACK/enrollment.txt" 2>/dev/null || true
  sw_vers > "$PACK/sw_vers.txt" 2>/dev/null || true
  (cd "$LOG_DIR" && tar -czf "SupportPack_${TIMESTAMP}.tar.gz" "SupportPack_${TIMESTAMP}" 2>/dev/null) || true
  add_report_row "Support pack" "$LOG_DIR/SupportPack_${TIMESTAMP}.tar.gz" "PASS" "$PACK"
fi

if [[ "$SCRIPT_BASE" == *escalation* ]]; then
  FREE_PCT=$(df / | awk 'NR==2{gsub(/%/,"",$5); print 100-$5}')
  [[ "${FREE_PCT:-0}" -gt 10 ]] && add_report_row "Disk headroom" "${FREE_PCT}% free" "PASS" "" || add_report_row "Disk headroom" "${FREE_PCT}% free" "FAIL" ""
  ping -c 1 -W 2000 1.1.1.1 >/dev/null 2>&1 && add_report_row "Network" "OK" "PASS" "" || add_report_row "Network" "Down" "FAIL" ""
fi

if [[ "$SCRIPT_BASE" == *menu* ]]; then
  add_report_row "Menu mode" "Non-interactive common repair set" "INFO" "Use MASTER-MENU-GUI.py for picker"
fi
show_progress 6 "$TOTAL_STEPS" "Toolkit done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "remote diagnostics packager mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive