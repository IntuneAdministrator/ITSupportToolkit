...
# ============================================================
# Script: magic-keyboard-reconnect-mac.sh | ITSupport/21_Peripheral-Mac-Quick | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh magic-keyboard-reconnect-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='21_Peripheral-Mac-Quick'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "21_Peripheral-Mac-Quick" "INFO" "ITSupport"
section_banner "ENDPOINT SECURITY MAC DEEP"
show_progress 2 "$TOTAL_STEPS" "XProtect / GK / extensions"
spctl --status 2>&1 | tee -a "$LOG_FILE" || true
csrutil status 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Gatekeeper/SIP" "Captured" "INFO" ""
systemextensionsctl list 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "System extensions" "Listed" "INFO" ""
if [[ "$SCRIPT_BASE" == *tcc* || "$SCRIPT_BASE" == *full-disk* ]]; then
  add_report_row "TCC/FDA" "Review System Settings > Privacy; TCC.db SIP-protected" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *mrt* || "$SCRIPT_BASE" == *malware* || "$SCRIPT_BASE" == *xprotect* ]]; then
  defaults read /System/Library/CoreServices/XProtect.bundle/Contents/Info.plist CFBundleShortVersionString 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "XProtect version" "Queried" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Security deep done" section_banner "ITSUPPORT MAC PHASE10"
...
export_html_report "magic keyboard reconnect mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive