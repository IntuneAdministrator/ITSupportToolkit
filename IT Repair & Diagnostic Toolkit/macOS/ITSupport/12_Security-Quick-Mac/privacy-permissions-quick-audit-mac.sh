#!/bin/zsh
# ============================================================
# Script:      privacy-permissions-quick-audit-mac.sh
# Description: macOS IT Support quick repair for privacy permissions quick audit mac.
#              Category: 12_Security-Quick-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh privacy-permissions-quick-audit-mac.sh
...
FOLDER_NAME='12_Security-Quick-Mac'
...
section_banner "SECURITY QUICK"
show_progress 2 "$TOTAL_STEPS" "Quick posture"
FV=$(fdesetup status 2>&1 || true)
add_report_row "FileVault" "$FV" "$(echo "$FV" | grep -qi On && echo PASS || echo WARN)" ""
GK=$(spctl --status 2>&1 || true)
add_report_row "Gatekeeper" "$GK" "$(echo "$GK" | grep -qi enabled && echo PASS || echo WARN)" ""
SIP=$(csrutil status 2>&1 || true)
add_report_row "SIP" "$SIP" "$(echo "$SIP" | grep -qi enabled && echo PASS || echo WARN)" ""

if [[ "$SCRIPT_BASE" == *firewall* ]]; then
  ...
  add_report_row "Firewall" "Enabled" "PASS" "socketfilterfw"
fi

if [[ "$SCRIPT_BASE" == *login* ]]; then
  last -15 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Recent logins" "last -15" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *privacy* ]]; then
  add_report_row "Privacy/TCC" "Use System Settings > Privacy; tccutil reset <service> if needed" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Security quick done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "privacy permissions quick audit mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive