...
# ============================================================
# Script: password-policy-expiry-check-mac.sh | ITSupport/27_Account-Unlock-Mac | v1.3.0 Phase11
# Requires: sudo | Usage: sudo zsh password-policy-expiry-check-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='27_Account-Unlock-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "27_Account-Unlock-Mac" "INFO" "ITSupport"
section_banner "UPDATE CATALOG DEEP"
show_progress 2 "$TOTAL_STEPS" "softwareupdate"
softwareupdate -l 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "softwareupdate -l" "Ran" "INFO" ""
if [[ "$SCRIPT_BASE" == *force* || "$SCRIPT_BASE" == *catalog* ]]; then
  softwareupdate --dump-state 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "dump-state" "Captured" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *xprotect* || "$SCRIPT_BASE" == *gatekeeper* ]]; then
  defaults read /Library/Preferences/com.apple.SoftwareUpdate 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "SoftwareUpdate prefs" "Read" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *firmware* || "$SCRIPT_BASE" == *bridgeos* ]]; then
  system_profiler SPiBridgeDataType 2>/dev/null | tee -a "$LOG_FILE" || true
  add_report_row "iBridge/firmware" "Profiled" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Update done" section_banner "ITSUPPORT MAC PHASE11"
...
export_html_report "password policy expiry check mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive