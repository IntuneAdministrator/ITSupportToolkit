#!/bin/zsh
# ============================================================
# Script:      user-onboarding-complete-mac.sh
# Descrição:   Onboarding completo: usuário + ambiente.
# Categoria:   35_User-Onboarding-Offboarding-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh user-onboarding-complete-mac.sh
...
section_banner "ONBOARDING COMPLETO"
# Uso: sudo NEW_USER=jdoe FULL_NAME='Jane Doe' USER_PASSWORD='***' GROUPS='staff,_lpadmin' zsh user-onboarding-complete-mac.sh
NEW_USER=${NEW_USER:-}; FULL_NAME=${FULL_NAME:-$NEW_USER}; USER_PASSWORD=${USER_PASSWORD:-}; GROUPS=${GROUPS:-staff}
show_progress 2 "$TOTAL_STEPS" "Criar usuário se necessário"
if [[ -z "$NEW_USER" || -z "$USER_PASSWORD" ]]; then
  add_report_row "Parâmetros" "NEW_USER/USER_PASSWORD obrigatórios" "FAIL" ""
else
  if ! id "$NEW_USER" &>/dev/null; then
    sysadminctl -addUser "$NEW_USER" -fullName "$FULL_NAME" -password "$USER_PASSWORD" -home "/Users/$NEW_USER" 2>&1 | tee -a "$LOG_FILE" || true
  fi
  if id "$NEW_USER" &>/dev/null; then
    add_report_row "Usuário" "$NEW_USER" "PASS" "existe"
    IFS=',' read -rA GARR <<< "$GROUPS"
    for g in "${GARR[@]}"; do
      g=$(echo "$g" | tr -d ' ')
      dseditgroup -o edit -a "$NEW_USER" -t user "$g" 2>/dev/null || true
    done
    # Ambiente padrão: pastas Desktop/Documents + Dock hint
    HOME_DIR=$(dscl . -read "/Users/$NEW_USER" NFSHomeDirectory | awk '{print $2}')
    mkdir -p "$HOME_DIR/Desktop" "$HOME_DIR/Documents" "$HOME_DIR/Downloads"
    chown -R "$NEW_USER":staff "$HOME_DIR" 2>/dev/null || true
    add_report_row "Home/perfil" "$HOME_DIR" "PASS" "pastas padrão"
    # Preferências leves
    sudo -u "$NEW_USER" defaults write com.apple.dock autohide -bool false 2>/dev/null || true
    add_report_row "Ambiente" "configurado" "PASS" "Dock + pastas"
  else
    add_report_row "Usuário" "$NEW_USER" "FAIL" "não criado"
  fi
fi
show_progress 6 "$TOTAL_STEPS" "Resumo onboarding"
id "$NEW_USER" 2>&1 | tee -a "$LOG_FILE" || true

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "User Onboarding Complete" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive