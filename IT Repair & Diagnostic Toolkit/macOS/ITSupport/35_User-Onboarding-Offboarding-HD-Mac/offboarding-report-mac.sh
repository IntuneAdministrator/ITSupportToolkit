#!/bin/zsh
# ============================================================
# Script:      offboarding-report-mac.sh
# Descrição:   Gera relatório de offboarding.
# Categoria:   35_User-Onboarding-Offboarding-HD-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh offboarding-report-mac.sh
...
section_banner "RELATÓRIO DE OFFBOARDING"
# Uso: sudo TARGET_USER=jdoe zsh offboarding-report-mac.sh
TARGET_USER=${TARGET_USER:-}
show_progress 2 "$TOTAL_STEPS" "Coletar evidências"
if [[ -z "$TARGET_USER" ]]; then
  add_report_row "TARGET_USER" "obrigatório" "FAIL" ""
else
  if id "$TARGET_USER" &>/dev/null; then
    id "$TARGET_USER" | tee -a "$LOG_FILE"
    dscl . -read "/Users/$TARGET_USER" 2>&1 | tee -a "$LOG_FILE" || true
    HOME_DIR=$(dscl . -read "/Users/$TARGET_USER" NFSHomeDirectory 2>/dev/null | awk '{print $2}')
    DU=$(du -sh "$HOME_DIR" 2>/dev/null | awk '{print $1}')
    add_report_row "Conta" "existe" "INFO" "$(id "$TARGET_USER")"
    add_report_row "Home size" "${DU:-n/a}" "INFO" "$HOME_DIR"
    HIDDEN=$(dscl . -read "/Users/$TARGET_USER" IsHidden 2>/dev/null || echo "não oculto")
    add_report_row "IsHidden" "$HIDDEN" "INFO" ""
  else
    add_report_row "Conta" "não existe" "WARN" "$TARGET_USER"
  fi
  last "$TARGET_USER" 2>&1 | head -n 10 | tee -a "$LOG_FILE" || true
  add_report_row "last login" "amostra" "INFO" "comando last"
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Offboarding Report" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive