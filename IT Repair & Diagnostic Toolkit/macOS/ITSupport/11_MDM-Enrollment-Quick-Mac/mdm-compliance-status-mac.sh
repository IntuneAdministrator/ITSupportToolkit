#!/bin/zsh
# ============================================================
# Script:      mdm-compliance-status-mac.sh
# Description: macOS IT Support quick repair for mdm compliance status mac.
#              Category: 11_MDM-Enrollment-Quick-Mac. Logs + HTML under
#              ~/Library/Logs/IT-Repair. Intune/Jamf compatible.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh mdm-compliance-status-mac.sh
...
FOLDER_NAME='11_MDM-Enrollment-Quick-Mac'
...
section_banner "MDM QUICK"
show_progress 2 "$TOTAL_STEPS" "Enrollment"
profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
detect_mdm_join_state
add_report_row "MDM state" "$JOIN_STATE" "INFO" ""

if [[ "$SCRIPT_BASE" == *profile-list* || "$SCRIPT_BASE" == *compliance* || "$SCRIPT_BASE" == *ade* ]]; then
  profiles list 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "profiles list" "Captured" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *jamf* ]]; then
  JAMF=$(command -v jamf || true)
  ...
  if [[ -n "${JAMF:-}" ]]; then
    ...
    "$JAMF" recon 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "jamf check-in" "policy + recon" "PASS" "$JAMF"
  else
...
if [[ "$SCRIPT_BASE" == *intune* ]]; then
  ...
  add_report_row "Intune sync" "Company Portal opened if present" "INFO" "User completes sync in app"
fi
show_progress 5 "$TOTAL_STEPS" "MDM quick done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "mdm compliance status mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive