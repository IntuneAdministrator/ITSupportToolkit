#!/bin/zsh
# ============================================================
# Script:      keychain-certificates-list-mac.sh
# Descrição:   Lista certificados instalados no Keychain.
# Categoria:   41_Certificate-Management-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh keychain-certificates-list-mac.sh
...
section_banner "CERTIFICADOS NO KEYCHAIN"
show_progress 2 "$TOTAL_STEPS" "security find-certificate (System)"
security find-certificate -a /Library/Keychains/System.keychain 2>/dev/null | grep -E 'alis|labl|"0x00000000' | head -n 100 | tee -a "$LOG_FILE" || true
add_report_row "System.keychain" "listado" "INFO" "security find-certificate -a"
show_progress 5 "$TOTAL_STEPS" "Login keychain do usuário"
LOGIN_KC="$REAL_HOME/Library/Keychains/login.keychain-db"
[[ -f "$LOGIN_KC" ]] || LOGIN_KC="$REAL_HOME/Library/Keychains/login.keychain"
if [[ -f "$LOGIN_KC" ]]; then
  security find-certificate -a "$LOGIN_KC" 2>/dev/null | grep -E 'alis|labl' | head -n 80 | tee -a "$LOG_FILE" || true
  add_report_row "login.keychain" "listado" "INFO" "$LOGIN_KC"
else
  add_report_row "login.keychain" "não encontrado" "WARN" ""
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Keychain Certificates List" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive