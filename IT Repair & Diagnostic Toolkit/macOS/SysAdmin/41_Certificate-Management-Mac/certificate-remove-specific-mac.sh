#!/bin/zsh
# ============================================================
# Script:      certificate-remove-specific-mac.sh
# Descrição:   Remove certificado específico do Keychain.
# Categoria:   41_Certificate-Management-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh certificate-remove-specific-mac.sh
...
section_banner "REMOVER CERTIFICADO"
# Uso: sudo CERT_COMMON_NAME='My CA' KEYCHAIN=/Library/Keychains/System.keychain zsh ...
CERT_COMMON_NAME=${CERT_COMMON_NAME:-}; KEYCHAIN=${KEYCHAIN:-/Library/Keychains/System.keychain}
show_progress 2 "$TOTAL_STEPS" "Localizar"
if [[ -z "$CERT_COMMON_NAME" ]]; then
  add_report_row "CERT_COMMON_NAME" "obrigatório" "FAIL" ""
else
  security find-certificate -c "$CERT_COMMON_NAME" -a "$KEYCHAIN" 2>&1 | tee -a "$LOG_FILE" || true
  show_progress 5 "$TOTAL_STEPS" "delete-certificate"
  security delete-certificate -c "$CERT_COMMON_NAME" "$KEYCHAIN" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Remover" "$CERT_COMMON_NAME" "PASS" "$KEYCHAIN"
fi

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Certificate Remove" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive