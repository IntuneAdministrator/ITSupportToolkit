#!/bin/zsh
# ============================================================
# Script:      certificates-expiry-30-60-90-mac.sh
# Descrição:   Certificados próximos do vencimento.
# Categoria:   41_Certificate-Management-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh certificates-expiry-30-60-90-mac.sh
...
section_banner "CERTIFICADOS — VENCIMENTO 30/60/90 DIAS"
show_progress 2 "$TOTAL_STEPS" "Varredura System + login"
TMP="$BACKUP_DIR/certs-pem-$TIMESTAMP"
mkdir -p "$TMP"
security find-certificate -a -p /Library/Keychains/System.keychain > "$TMP/system.pem" 2>/dev/null || true
LOGIN_KC="$REAL_HOME/Library/Keychains/login.keychain-db"
[[ -f "$LOGIN_KC" ]] || LOGIN_KC="$REAL_HOME/Library/Keychains/login.keychain"
[[ -f "$LOGIN_KC" ]] && security find-certificate -a -p "$LOGIN_KC" > "$TMP/login.pem" 2>/dev/null || true
NOW=$(date +%s)
for pem in "$TMP"/*.pem; do
  [[ -f "$pem" ]] || continue
  # Divide PEM blocks
  csplit -s -f "$TMP/c-" -b '%03d.pem' "$pem" '/-----BEGIN CERTIFICATE-----/' '{*}' 2>/dev/null || true
done
for c in "$TMP"/c-*.pem; do
  [[ -f "$c" ]] || continue
  grep -q 'BEGIN CERTIFICATE' "$c" || continue
  END=$(openssl x509 -enddate -noout -in "$c" 2>/dev/null | cut -d= -f2 || true)
  SUBJ=$(openssl x509 -subject -noout -in "$c" 2>/dev/null || true)
  [[ -z "$END" ]] && continue
  END_EPOCH=$(date -j -f "%b %d %T %Y %Z" "$END" +%s 2>/dev/null || date -d "$END" +%s 2>/dev/null || echo 0)
  if [[ "$END_EPOCH" -le 0 ]]; then continue; fi
  DAYS=$(( (END_EPOCH - NOW) / 86400 ))
  ST="PASS"
  if [[ $DAYS -lt 0 ]]; then ST="FAIL"; elif [[ $DAYS -le 30 ]]; then ST="WARN"; elif [[ $DAYS -le 90 ]]; then ST="INFO"; fi
  if [[ $DAYS -le 90 ]]; then
    echo "$DAYS days | $END | $SUBJ" | tee -a "$LOG_FILE"
    add_report_row "Cert (${DAYS}d)" "${SUBJ:0:80}" "$ST" "$END"
  fi
done
add_report_row "Varredura" "30/60/90" "INFO" "$TMP"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Certificates Expiry 30/60/90" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive