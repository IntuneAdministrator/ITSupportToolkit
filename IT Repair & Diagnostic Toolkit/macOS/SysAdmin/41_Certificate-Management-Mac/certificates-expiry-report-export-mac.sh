#!/bin/zsh
# ============================================================
# Script:      certificates-expiry-report-export-mac.sh
# Descrição:   Exporta relatório CSV de certificados.
# Categoria:   41_Certificate-Management-Mac
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh certificates-expiry-report-export-mac.sh
...
FOLDER_NAME='41_Certificate-Management-Mac'
...
section_banner "RELATÓRIO DE CERTIFICADOS / EXPIRAÇÃO"
show_progress 2 "$TOTAL_STEPS" "Exportar CSV"
CSV="$REPORT_DIR/certificates-expiry-$TIMESTAMP.csv"
echo "subject,not_after,days_left,keychain" > "$CSV"
NOW=$(date +%s)
dump_kc() {
  local kc="$1" label="$2" tmp
  tmp=$(mktemp)
  security find-certificate -a -p "$kc" > "$tmp" 2>/dev/null || true
  awk 'BEGIN{c=0} /BEGIN CERTIFICATE/{c++; f=sprintf("'"$BACKUP_DIR"'/one-%s-%d.pem","'"$label"'",c)} {print > f}' "$tmp" 2>/dev/null || true
  for f in "$BACKUP_DIR"/one-"$label"-*.pem; do
    [[ -f "$f" ]] || continue
    SUBJ=$(openssl x509 -noout -subject -in "$f" 2>/dev/null | sed 's/"/""/g')
    END=$(openssl x509 -noout -enddate -in "$f" 2>/dev/null | cut -d= -f2)
    END_EPOCH=$(date -j -f "%b %d %T %Y %Z" "$END" +%s 2>/dev/null || echo 0)
    DAYS=$(( (END_EPOCH - NOW) / 86400 ))
    echo "\"$SUBJ\",\"$END\",$DAYS,$label" >> "$CSV"
  done
  rm -f "$tmp"
}
dump_kc /Library/Keychains/System.keychain system
LOGIN_KC="$REAL_HOME/Library/Keychains/login.keychain-db"
[[ -f "$LOGIN_KC" ]] || LOGIN_KC="$REAL_HOME/Library/Keychains/login.keychain"
[[ -f "$LOGIN_KC" ]] && dump_kc "$LOGIN_KC" login
[[ "$REAL_USER" != "root" ]] && chown "$REAL_USER":staff "$CSV" 2>/dev/null || true
ROWS=$(tail -n +2 "$CSV" | wc -l | tr -d ' ')
add_report_row "CSV" "$CSV" "PASS" "$ROWS certificados"
cat "$CSV" | head -n 30 | tee -a "$LOG_FILE"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Exportar relatório HTML"
export_html_report "Certificates Expiry Report" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive