#!/bin/zsh
# ============================================================
# Script:      m365-mac-network-connectivity-test.sh
# Description: Production macOS SysAdmin repair/diagnostic for m365 mac network connectivity test.
#              Category: 05_Microsoft365-Mac. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh m365-mac-network-connectivity-test.sh
...
FOLDER_NAME='05_Microsoft365-Mac'
...
section_banner "MICROSOFT 365 MAC"
show_progress 2 "$TOTAL_STEPS" "Office/Teams presence"
for app in "Microsoft Teams.app" "Microsoft Outlook.app" "Microsoft Word.app" "OneDrive.app"; do
  if [[ -d "/Applications/$app" ]]; then
    add_report_row "$app" "Installed" "PASS" "/Applications"
  else
    add_report_row "$app" "Not found" "WARN" "/Applications"
  fi
done

if [[ "$SCRIPT_BASE" == *teams* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Teams reset"
  killall "Microsoft Teams" "Teams" 2>/dev/null || true
  rm -rf "$REAL_HOME/Library/Application Support/Microsoft/Teams/Cache" 2>/dev/null || true
  rm -rf "$REAL_HOME/Library/Application Support/Microsoft/Teams/GPUCache" 2>/dev/null || true
  rm -rf "$REAL_HOME/Library/Group Containers/UBF8T346G9.Office/Outlook/Outlook 15 Profiles" 2>/dev/null || true
  add_report_row "Teams cache" "Cleared" "PASS" "Application Support caches"
  if [[ "$SCRIPT_BASE" == *webcam* || "$SCRIPT_BASE" == *microphone* ]]; then
    add_report_row "TCC note" "Reset camera/mic via tccutil if needed" "INFO" "tccutil reset Camera/Microphone"
  fi
fi

if [[ "$SCRIPT_BASE" == *outlook* ]]; then
  killall "Microsoft Outlook" 2>/dev/null || true
  add_report_row "Outlook" "Quit for repair" "INFO" "Profile/search rebuild"
  if [[ "$SCRIPT_BASE" == *search* ]]; then
    mdutil -E "$REAL_HOME/Library/Group Containers/UBF8T346G9.Office" 2>/dev/null || true
    add_report_row "Outlook search reindex" "Triggered" "PASS" "mdutil -E Office container"
  fi
fi

if [[ "$SCRIPT_BASE" == *onedrive* ]]; then
  killall OneDrive 2>/dev/null || true
  open -a OneDrive 2>/dev/null || true
  add_report_row "OneDrive" "Restarted" "PASS" "open -a OneDrive"
fi

if [[ "$SCRIPT_BASE" == *license* || "$SCRIPT_BASE" == *activation* || "$SCRIPT_BASE" == *mau* ]]; then
  MAU="/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app"
  if [[ -d "$MAU" ]]; then
    add_report_row "MAU" "Present" "PASS" "$MAU"
    open "$MAU" 2>/dev/null || true
  else
    add_report_row "MAU" "Missing" "WARN" "Microsoft AutoUpdate"
  fi
fi

if [[ "$SCRIPT_BASE" == *network-connectivity* ]]; then
  for u in "https://outlook.office365.com" "https://login.microsoftonline.com" "https://teams.microsoft.com"; do
    CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 15 -I "$u" || echo 000)
    if [[ "$CODE" =~ ^[23] ]]; then
      add_report_row "$u" "$CODE" "PASS" "HTTP"
    else
      add_report_row "$u" "$CODE" "FAIL" "HTTP"
    fi
  done
fi

show_progress 5 "$TOTAL_STEPS" "M365 Mac done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "m365 mac network connectivity test" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
