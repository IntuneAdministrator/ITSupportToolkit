#!/bin/zsh
# ============================================================
# Script:      enrollment-profile-force-remove.sh
# Description: Production macOS SysAdmin repair/diagnostic for enrollment profile force remove.
#              Category: 08_MDM-Enrollment. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh enrollment-profile-force-remove.sh
...
section_banner "MDM ENROLLMENT"
show_progress 2 "$TOTAL_STEPS" "Enrollment status"
profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
detect_mdm_join_state
add_report_row "Join/MDM state" "$JOIN_STATE" "INFO" "profiles"

if [[ "$SCRIPT_BASE" == *jamf* ]]; then
  JAMF=""
  [[ -x /usr/local/bin/jamf ]] && JAMF=/usr/local/bin/jamf
  [[ -x /usr/local/jamf/bin/jamf ]] && JAMF=/usr/local/jamf/bin/jamf
  if [[ -n "$JAMF" ]]; then
    "$JAMF" checkJSSConnection 2>&1 | tee -a "$LOG_FILE" || true
    if [[ "$SCRIPT_BASE" == *policy* || "$SCRIPT_BASE" == *re-enroll* ]]; then
      "$JAMF" policy 2>&1 | tee -a "$LOG_FILE" || true
      add_report_row "jamf policy" "Invoked" "PASS" "$JAMF"
    else
      add_report_row "jamf binary" "$JAMF" "PASS" "present"
    fi
  else
    add_report_row "jamf" "Not installed" "WARN" ""
  fi
fi

if [[ "$SCRIPT_BASE" == *intune* ]]; then
  if [[ -d "/Applications/Company Portal.app" ]]; then
    add_report_row "Company Portal" "Installed" "PASS" ""
    open -a "Company Portal" 2>/dev/null || true
  else
    add_report_row "Company Portal" "Missing" "WARN" "Install from App Store/ABM"
  fi
fi

if [[ "$SCRIPT_BASE" == *profile-audit* || "$SCRIPT_BASE" == *push* || "$SCRIPT_BASE" == *certificate* || "$SCRIPT_BASE" == *supervision* || "$SCRIPT_BASE" == *bootstrap* ]]; then
  profiles list 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "profiles list" "Captured" "INFO" "See log"
fi

if [[ "$SCRIPT_BASE" == *ade* || "$SCRIPT_BASE" == *dep* ]]; then
  profiles show -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "ADE/DEP enrollment" "Shown" "INFO" "profiles show -type enrollment"
fi

if [[ "$SCRIPT_BASE" == *force-remove* ]]; then
  add_report_row "Force remove" "NOT auto-executed (destructive)" "WARN" "Use profiles remove -identifier <id> with approval"
fi

if [[ "$SCRIPT_BASE" == *command-queue* ]]; then
  add_report_row "MDM command queue" "Flush via MDM console / jamf recon" "INFO" "Device-side queue not fully exposed"
  if [[ -x /usr/local/bin/jamf ]]; then /usr/local/bin/jamf recon 2>&1 | tee -a "$LOG_FILE" || true; fi
fi

show_progress 5 "$TOTAL_STEPS" "MDM done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "enrollment profile force remove" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
