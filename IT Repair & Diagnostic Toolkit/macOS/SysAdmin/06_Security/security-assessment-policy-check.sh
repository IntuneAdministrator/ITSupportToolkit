#!/bin/zsh
# ============================================================
# Script:      security-assessment-policy-check.sh
# Description: Production macOS SysAdmin repair/diagnostic for security assessment policy check.
#              Category: 06_Security. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh security-assessment-policy-check.sh
...
show_progress 2 "$TOTAL_STEPS" "Core security status"
GK=$(spctl --status 2>&1 || true)
add_report_row "Gatekeeper" "$GK" "$(echo "$GK" | grep -qi enabled && echo PASS || echo WARN)" "spctl --status"
SIP=$(csrutil status 2>&1 || true)
add_report_row "SIP" "$SIP" "$(echo "$SIP" | grep -qi enabled && echo PASS || echo WARN)" "csrutil"
FV=$(fdesetup status 2>&1 || true)
add_report_row "FileVault" "$FV" "$(echo "$FV" | grep -qi On && echo PASS || echo WARN)" "fdesetup status"

if [[ "$SCRIPT_BASE" == *firewall* || "$SCRIPT_BASE" == *alf* ]]; then
  /usr/libexec/ApplicationFirewall/socketfilterfw --getglobalstate 2>&1 | tee -a "$LOG_FILE" || true
  /usr/libexec/ApplicationFirewall/socketfilterfw --setglobalstate on 2>&1 || true
  add_report_row "Application Firewall" "Enabled" "PASS" "socketfilterfw"
fi

if [[ "$SCRIPT_BASE" == *xprotect* ]]; then
  defaults read /System/Library/CoreServices/XProtect.bundle/Contents/Info.plist CFBundleShortVersionString 2>/dev/null || true
  add_report_row "XProtect" "Version queried" "INFO" "XProtect.bundle Info.plist"
fi

if [[ "$SCRIPT_BASE" == *clam* ]]; then
  if command -v clamscan >/dev/null 2>&1; then
    clamscan -r --bell -i /Users --max-filesize=50M --max-scansize=200M 2>&1 | tee -a "$LOG_FILE" | tail -n 20 || true
    add_report_row "clamscan" "Completed sample scan" "INFO" "/Users limited"
  else
    add_report_row "clamscan" "Not installed" "WARN" "brew install clamav"
  fi
fi

if [[ "$SCRIPT_BASE" == *ssh* || "$SCRIPT_BASE" == *remote-login* ]]; then
  SSH=$(systemsetup -getremotelogin 2>&1 || true)
  add_report_row "Remote Login" "$SSH" "INFO" "systemsetup"
fi

if [[ "$SCRIPT_BASE" == *sudoers* ]]; then
  if visudo -c 2>&1 | tee -a "$LOG_FILE"; then
    add_report_row "sudoers" "OK" "PASS" "visudo -c"
  else
    add_report_row "sudoers" "INVALID" "FAIL" "visudo -c"
  fi
fi

if [[ "$SCRIPT_BASE" == *tcc* || "$SCRIPT_BASE" == *privacy* ]]; then
  add_report_row "TCC" "SIP protects TCC.db; use tccutil reset <service>" "INFO" "privacy permission reset"
fi

if [[ "$SCRIPT_BASE" == *mdm-profile* || "$SCRIPT_BASE" == *notarization* || "$SCRIPT_BASE" == *secure-boot* ]]; then
  profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Enrollment" "See log" "INFO" "profiles status"
fi

if [[ "$SCRIPT_BASE" == *suspicious-login* ]]; then
  LAST=$(last -10 2>/dev/null || true)
  add_report_row "Recent logins" "last -10" "INFO" "$LAST"
fi

show_progress 5 "$TOTAL_STEPS" "Security done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "security assessment policy check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
