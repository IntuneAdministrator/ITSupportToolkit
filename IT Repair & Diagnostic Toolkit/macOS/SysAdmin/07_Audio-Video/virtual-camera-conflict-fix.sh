#!/bin/zsh
# ============================================================
# Script:      virtual-camera-conflict-fix.sh
# Description: Production macOS SysAdmin repair/diagnostic for virtual camera conflict fix.
#              Category: 07_Audio-Video. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh virtual-camera-conflict-fix.sh
...
section_banner "AUDIO / VIDEO"
show_progress 2 "$TOTAL_STEPS" "Core Audio / display"

if [[ "$SCRIPT_BASE" == *coreaudio* || "$SCRIPT_BASE" == *audio* || "$SCRIPT_BASE" == *hdmi-audio* || "$SCRIPT_BASE" == *spatial* || "$SCRIPT_BASE" == *midi* ]]; then
  killall coreaudiod 2>/dev/null || true
  sleep 1
  add_report_row "coreaudiod" "Restarted (killall)" "PASS" "launchd will respawn"
fi

if [[ "$SCRIPT_BASE" == *display* || "$SCRIPT_BASE" == *hdmi-display* || "$SCRIPT_BASE" == *multiple-display* || "$SCRIPT_BASE" == *color-profile* ]]; then
  killall Dock ControlCenter SystemUIServer 2>/dev/null || true
  add_report_row "Display UI agents" "Restarted" "PASS" "Dock/ControlCenter"
  DISPLAYS=$(system_profiler SPDisplaysDataType 2>/dev/null | head -n 40 || true)
  add_report_row "Displays" "Profiled" "INFO" "$DISPLAYS"
fi

if [[ "$SCRIPT_BASE" == *microphone* || "$SCRIPT_BASE" == *camera* || "$SCRIPT_BASE" == *webcam* || "$SCRIPT_BASE" == *facetime* || "$SCRIPT_BASE" == *continuity* ]]; then
  tccutil reset Microphone 2>/dev/null || true
  tccutil reset Camera 2>/dev/null || true
  add_report_row "TCC Camera/Microphone" "Reset attempted" "PASS" "tccutil reset"
  killall VDCAssistant AppleCameraAssistant 2>/dev/null || true
  add_report_row "Camera assistants" "Restarted" "PASS" "VDCAssistant"
fi

if [[ "$SCRIPT_BASE" == *bluetooth-audio* ]]; then
  pkill bluetoothd 2>/dev/null || true
  add_report_row "bluetoothd" "Restarted" "PASS" "pkill bluetoothd"
fi

if [[ "$SCRIPT_BASE" == *virtual-camera* ]]; then
  ps aux | grep -iE 'obs|camtwist|mmhmm|epoccam|virtual' | grep -v grep | tee -a "$LOG_FILE" || true
  add_report_row "Virtual camera procs" "Scanned" "INFO" "See log"
fi

show_progress 5 "$TOTAL_STEPS" "A/V done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "virtual camera conflict fix" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
