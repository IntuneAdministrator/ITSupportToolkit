...
# ============================================================
# Script: icloud-optimize-mac-storage-audit.sh | SysAdmin/22_Storage-APFS-Deep | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh icloud-optimize-mac-storage-audit.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='22_Storage-APFS-Deep'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "22_Storage-APFS-Deep" "INFO" "SysAdmin"
section_banner "STORAGE APFS DEEP"
show_progress 2 "$TOTAL_STEPS" "APFS / space"
df -h / | tee -a "$LOG_FILE" || true
diskutil apfs list 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "APFS list" "Captured" "INFO" ""
if [[ "$SCRIPT_BASE" == *snapshot* || "$SCRIPT_BASE" == *purge* || "$SCRIPT_BASE" == *timemachine* ]]; then
...
if [[ "$SCRIPT_BASE" == *encryption* || "$SCRIPT_BASE" == *secure-token* ]]; then
  fdesetup status 2>&1 | tee -a "$LOG_FILE" || true
  diskutil apfs listUsers / 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "FV/APFS users" "Queried" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "APFS done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "icloud optimize mac storage audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive