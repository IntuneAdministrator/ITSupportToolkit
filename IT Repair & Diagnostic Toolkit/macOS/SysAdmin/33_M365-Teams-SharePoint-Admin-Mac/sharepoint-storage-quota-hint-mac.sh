...
# ============================================================
# Script: sharepoint-storage-quota-hint-mac.sh | SysAdmin/33_M365-Teams-SharePoint-Admin-Mac | v1.4.0 Phase12 M365 cloud
# Usage: zsh sharepoint-storage-quota-hint-mac.sh [user@domain.com]
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='33_M365-Teams-SharePoint-Admin-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "33_M365-Teams-SharePoint-Admin-Mac" "INFO" "SysAdmin Phase12 M365"
...
section_banner "TEAMS / SHAREPOINT ADMIN MAC"
show_progress 3 "$TOTAL_STEPS" "Teams/SPO"
add_report_row "Teams Admin" "https://admin.teams.microsoft.com" "INFO" "Policies, guest, meetings, devices"
add_report_row "SPO Admin" "https://admin.microsoft.com/#/sharepoint" "INFO" "Sharing + storage"
if [[ -n "${TARGET_UPN:-}" ]]; then
  add_report_row "OneDrive URL hint" "https://<tenant>-my.sharepoint.com/personal/<upn>" "INFO" "$TARGET_UPN"
fi
# Client-side Office presence on this Mac
[[ -d "/Applications/Microsoft Teams.app" || -d "/Applications/Microsoft Teams (work or school).app" ]] && add_report_row "Teams.app" "Installed" "PASS" "" || add_report_row "Teams.app" "Missing" "INFO" ""
[[ -d "/Applications/Microsoft Outlook.app" ]] && add_report_row "Outlook.app" "Installed" "PASS" "" || true
[[ -d "/Applications/OneDrive.app" ]] && add_report_row "OneDrive.app" "Installed" "PASS" "" || true
show_progress 5 "$TOTAL_STEPS" "Teams/SPO Mac done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "sharepoint storage quota hint mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive