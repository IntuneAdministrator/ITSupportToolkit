...
# ============================================================
# Script: timemachine-destination-health-mac.sh | SysAdmin/30_Backup-TimeMachine-Deep | v1.3.0 Phase11
# Requires: sudo | Usage: sudo zsh timemachine-destination-health-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='30_Backup-TimeMachine-Deep'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
section_banner "BACKUP TIME MACHINE DEEP"
show_progress 2 "$TOTAL_STEPS" "Time Machine"
tmutil status 2>&1 | tee -a "$LOG_FILE" || true
tmutil destinationinfo 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "TM status/dest" "Captured" "INFO" ""
if [[ "$SCRIPT_BASE" == *snapshot* || "$SCRIPT_BASE" == *thin* ]]; then
  tmutil listlocalsnapshots / 2>&1 | tee -a "$LOG_FILE" || true
  tmutil thinlocalsnapshots / 10000000000 4 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Local snapshots" "Listed/thinned" "PASS" ""
fi
if [[ "$SCRIPT_BASE" == *exclude* ]]; then
  defaults read /Library/Preferences/com.apple.TimeMachine SkipPaths 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "TM SkipPaths" "Read" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Backup done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "timemachine destination health mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive