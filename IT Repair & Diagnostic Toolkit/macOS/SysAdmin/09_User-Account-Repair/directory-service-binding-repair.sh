#!/bin/zsh
# ============================================================
# Script:      directory-service-binding-repair.sh
# Description: Production macOS SysAdmin repair/diagnostic for directory service binding repair.
#              Category: 09_User-Account-Repair. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh directory-service-binding-repair.sh
...
FOLDER_NAME='09_User-Account-Repair'
...
section_banner "USER ACCOUNT REPAIR"
show_progress 2 "$TOTAL_STEPS" "User context"
add_report_row "Real user" "$REAL_USER" "INFO" "$REAL_HOME"
CONSOLE_USER=$(stat -f%Su /dev/console 2>/dev/null || true)
add_report_row "Console user" "$CONSOLE_USER" "INFO" ""

if [[ "$SCRIPT_BASE" == *permissions* || "$SCRIPT_BASE" == *home-folder* || "$SCRIPT_BASE" == *acl* || "$SCRIPT_BASE" == *profile* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Home permissions"
  if [[ -d "$REAL_HOME" && "$REAL_USER" != "root" ]]; then
    diskutil resetUserPermissions / "$REAL_USER" 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "resetUserPermissions" "$REAL_USER" "PASS" "$REAL_HOME"
  else
    add_report_row "resetUserPermissions" "Skipped" "WARN" "No non-root home"
  fi
fi

if [[ "$SCRIPT_BASE" == *keychain* ]]; then
  if [[ "$REAL_USER" != "root" ]]; then
    rm -f "$REAL_HOME/Library/Keychains/login.keychain-db.lock" 2>/dev/null || true
    add_report_row "Keychain lock file" "Cleared if present" "PASS" "login.keychain-db.lock"
  fi
  add_report_row "Keychain First Aid" "Advise user run Keychain Access > First Aid" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *secure-token* || "$SCRIPT_BASE" == *bootstrap-token* || "$SCRIPT_BASE" == *filevault-user* ]]; then
  fdesetup list 2>&1 | tee -a "$LOG_FILE" || true
  sysadminctl -secureTokenStatus "$REAL_USER" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Secure Token status" "Queried" "INFO" "sysadminctl"
fi

if [[ "$SCRIPT_BASE" == *admin* ]]; then
  dscl . -read /Groups/admin GroupMembership 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "admin group" "Listed" "INFO" "dscl"
fi

if [[ "$SCRIPT_BASE" == *login-loop* ]]; then
  rm -f /Library/Preferences/com.apple.loginwindow.plist.bak 2>/dev/null || true
  add_report_row "Login loop" "Check .next / VC / launch agents in user Library" "WARN" "Manual review recommended"
fi

if [[ "$SCRIPT_BASE" == *directory-service* || "$SCRIPT_BASE" == *mobile-account* ]]; then
  dsconfigad -show 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "AD bind" "dsconfigad -show" "INFO" "May be empty on Entra-only"
fi

show_progress 5 "$TOTAL_STEPS" "User repair done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "directory service binding repair" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
