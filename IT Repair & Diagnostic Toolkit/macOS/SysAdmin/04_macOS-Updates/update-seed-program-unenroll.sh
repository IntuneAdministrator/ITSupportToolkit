#!/bin/zsh
# ============================================================
# Script:      update-seed-program-unenroll.sh
# Description: Production macOS SysAdmin repair/diagnostic for update seed program unenroll.
#              Category: 04_macOS-Updates. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh update-seed-program-unenroll.sh
...
section_banner "macOS UPDATES"
show_progress 2 "$TOTAL_STEPS" "softwareupdate status"
PENDING=$(softwareupdate -l 2>&1 || true)
add_report_row "softwareupdate -l" "Listed" "INFO" "$(echo "$PENDING" | head -n 20)"

if [[ "$SCRIPT_BASE" == *cache* || "$SCRIPT_BASE" == *stuck* || "$SCRIPT_BASE" == *repair* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Clear update cache"
  rm -rf /Library/Updates/* 2>/dev/null || true
  rm -rf /Library/Preferences/com.apple.SoftwareUpdate.plist 2>/dev/null || true
  softwareupdate --clear-catalog 2>/dev/null || true
  add_report_row "Update cache clear" "Attempted" "PASS" "/Library/Updates"
fi

if [[ "$SCRIPT_BASE" == *force* || "$SCRIPT_BASE" == *delta* ]]; then
  softwareupdate -ia --force 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "softwareupdate -ia --force" "Invoked" "INFO" "May require reboot"
fi

if [[ "$SCRIPT_BASE" == *seed* || "$SCRIPT_BASE" == *unenroll* ]]; then
  if command -v seedutil >/dev/null 2>&1; then
    seedutil unenroll 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "seedutil unenroll" "Attempted" "PASS" "Beta unenroll"
  else
    add_report_row "seedutil" "Not present" "INFO" "Not enrolled or tool removed"
  fi
fi

if [[ "$SCRIPT_BASE" == *pkg-receipt* ]]; then
  PKG=$(pkgutil --pkgs 2>/dev/null | wc -l | tr -d ' ')
  add_report_row "pkg receipts" "$PKG" "INFO" "pkgutil --pkgs"
fi

if [[ "$SCRIPT_BASE" == *history* || "$SCRIPT_BASE" == *log-analyzer* ]]; then
  LOGS=$(ls -lt /var/log/install.log 2>/dev/null | head -n 5 || true)
  add_report_row "install.log" "Present" "INFO" "$LOGS"
  tail -n 50 /var/log/install.log 2>/dev/null | tee -a "$LOG_FILE" || true
fi

if [[ "$SCRIPT_BASE" == *mdm-override* || "$SCRIPT_BASE" == *staged-rollout* ]]; then
  add_report_row "MDM update deferrals" "Review configuration profiles" "INFO" "profiles show"
  profiles show 2>/dev/null | head -n 40 | tee -a "$LOG_FILE" || true
fi

show_progress 5 "$TOTAL_STEPS" "Updates done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "update seed program unenroll" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
