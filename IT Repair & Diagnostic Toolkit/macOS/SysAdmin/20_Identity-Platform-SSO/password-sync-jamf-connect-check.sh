...
# ============================================================
# Script: password-sync-jamf-connect-check.sh | SysAdmin/20_Identity-Platform-SSO | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh password-sync-jamf-connect-check.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='20_Identity-Platform-SSO'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "20_Identity-Platform-SSO" "INFO" "SysAdmin"
...
show_progress 5 "$TOTAL_STEPS" "Identity done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "password sync jamf connect check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive