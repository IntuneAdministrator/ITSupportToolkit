...
# ============================================================
# Script: quarantine-xattr-bulk-clear.sh | SysAdmin/23_App-Management-Mac | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh quarantine-xattr-bulk-clear.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='23_App-Management-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "23_App-Management-Mac" "INFO" "SysAdmin"
section_banner "APP MANAGEMENT MAC"
show_progress 2 "$TOTAL_STEPS" "Apps / quarantine / MAU"
pkgutil --pkgs 2>/dev/null | wc -l | tee -a "$LOG_FILE" || true
add_report_row "pkg receipts" "Counted" "INFO" ""
if [[ "$SCRIPT_BASE" == *quarantine* ]]; then
  find /Applications -name '*.app' -exec xattr -d com.apple.quarantine {} \; 2>/dev/null || true
  add_report_row "quarantine xattr" "Cleared on Apps sample" "WARN" "Use carefully"
fi
if [[ "$SCRIPT_BASE" == *notarization* || "$SCRIPT_BASE" == *staple* ]]; then
  spctl --assess -v /Applications/Safari.app 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "spctl assess Safari" "Sample" "INFO" ""
fi
if [[ "$SCRIPT_BASE" == *mau* || "$SCRIPT_BASE" == *office* || "$SCRIPT_BASE" == *app-store* ]]; then
  [[ -d "/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app" ]] && open "/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app" 2>/dev/null || true
  softwareupdate -l 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Updates/MAU" "Checked" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "App mgmt done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "quarantine xattr bulk clear" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive