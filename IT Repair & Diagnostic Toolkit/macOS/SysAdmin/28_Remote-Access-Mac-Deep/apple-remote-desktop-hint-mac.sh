...
# ============================================================
# Script: apple-remote-desktop-hint-mac.sh | SysAdmin/28_Remote-Access-Mac-Deep | v1.3.0 Phase11
# Requires: sudo | Usage: sudo zsh apple-remote-desktop-hint-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='28_Remote-Access-Mac-Deep'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "28_Remote-Access-Mac-Deep" "INFO" "SysAdmin"
section_banner "REMOTE ACCESS MAC DEEP"
show_progress 2 "$TOTAL_STEPS" "Remote"
systemsetup -getremotelogin 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Remote Login (SSH)" "Queried" "INFO" ""
kickstart -q -getconfig 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "ARD kickstart" "getconfig" "INFO" ""
/usr/libexec/ApplicationFirewall/socketfilterfw --getglobalstate 2>&1 | tee -a "$LOG_FILE" || true
if [[ "$SCRIPT_BASE" == *vnc* ]]; then
  defaults read /Library/Preferences/com.apple.RemoteManagement 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "RemoteManagement" "Prefs" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Remote done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "apple remote desktop hint mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive