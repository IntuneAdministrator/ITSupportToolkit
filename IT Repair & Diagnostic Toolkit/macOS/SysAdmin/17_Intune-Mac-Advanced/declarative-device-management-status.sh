...
# ============================================================
# Script: declarative-device-management-status.sh | Role: SysAdmin/17_Intune-Mac-Advanced | v1.1.0 expansion
# Requires: sudo | Usage: sudo zsh declarative-device-management-status.sh
# ============================================================
set -euo pipefail
...
section_banner "INTUNE MAC ADVANCED"
show_progress 2 "$TOTAL_STEPS" "Company Portal / agents"
[[ -d "/Applications/Company Portal.app" ]] && add_report_row "Company Portal" "Installed" "PASS" "" || add_report_row "Company Portal" "Missing" "WARN" ""
profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Enrollment" "Captured" "INFO" ""
if [[ "$SCRIPT_BASE" == *log* || "$SCRIPT_BASE" == *artifact* ]]; then
  PACK="$LOG_DIR/IntuneMac_$TIMESTAMP"; mkdir -p "$PACK"
  profiles list > "$PACK/profiles.txt" 2>/dev/null || true
  add_report_row "Artifacts" "$PACK" "PASS" ""
fi
show_progress 5 "$TOTAL_STEPS" "Intune Mac done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "HTML report"
export_html_report "declarative device management status" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive