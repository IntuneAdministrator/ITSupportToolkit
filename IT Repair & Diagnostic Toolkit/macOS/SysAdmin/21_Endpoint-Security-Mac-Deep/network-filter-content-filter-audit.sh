...
# ============================================================
# Script: network-filter-content-filter-audit.sh | SysAdmin/21_Endpoint-Security-Mac-Deep | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh network-filter-content-filter-audit.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='21_Endpoint-Security-Mac-Deep'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "21_Endpoint-Security-Mac-Deep" "INFO" "SysAdmin"
...
show_progress 5 "$TOTAL_STEPS" "Security deep done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "network filter content filter audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive