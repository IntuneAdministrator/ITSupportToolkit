#!/bin/zsh
# ============================================================
# Script:      bluetooth-module-nvram-reset.sh
# Description: Production macOS SysAdmin repair/diagnostic for bluetooth module nvram reset.
#              Category: 11_Bluetooth-Mac. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh bluetooth-module-nvram-reset.sh
...
section_banner "BLUETOOTH"
show_progress 2 "$TOTAL_STEPS" "Bluetooth reset"
pkill bluetoothd 2>/dev/null || true
sleep 1
add_report_row "bluetoothd" "Restarted" "PASS" "pkill"

if [[ "$SCRIPT_BASE" == *plist* || "$SCRIPT_BASE" == *paired* ]]; then
  BTPLIST="/Library/Preferences/com.apple.Bluetooth.plist"
  if [[ -f "$BTPLIST" ]]; then
    cp "$BTPLIST" "$BACKUP_DIR/com.apple.Bluetooth.plist.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
    add_report_row "Bluetooth plist backup" "Saved" "PASS" "$BACKUP_DIR"
  fi
fi

if [[ "$SCRIPT_BASE" == *nvram* ]]; then
  nvram -d bluetoothHostControllerSwitchBehavior 2>/dev/null || true
  add_report_row "BT NVRAM" "Cleared controller switch key if present" "INFO" "nvram -d"
fi

if [[ "$SCRIPT_BASE" == *airdrop* || "$SCRIPT_BASE" == *handoff* || "$SCRIPT_BASE" == *continuity* ]]; then
  defaults write com.apple.NetworkBrowser BrowseAllInterfaces 1 2>/dev/null || true
  killall Finder SystemUIServer 2>/dev/null || true
  add_report_row "AirDrop/Handoff" "Finder refresh" "PASS" "BrowseAllInterfaces"
fi

system_profiler SPBluetoothDataType 2>/dev/null | head -n 50 | tee -a "$LOG_FILE" || true
add_report_row "Bluetooth profile" "Captured" "INFO" "system_profiler"

show_progress 5 "$TOTAL_STEPS" "Bluetooth done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "bluetooth module nvram reset" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
