#!/bin/zsh
# ============================================================
# Script:      cpu-frequency-scaling-audit.sh
# Description: Production macOS SysAdmin repair/diagnostic for cpu frequency scaling audit.
#              Category: 03_Performance. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh cpu-frequency-scaling-audit.sh
...
section_banner "PERFORMANCE"
show_progress 2 "$TOTAL_STEPS" "Memory pressure"
MEM=$(memory_pressure 2>&1 | head -n 20 || true)
add_report_row "memory_pressure" "Sampled" "INFO" "$MEM"

if [[ "$SCRIPT_BASE" == *spotlight* ]]; then
  mdutil -E / 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Spotlight reindex" "Triggered mdutil -E /" "PASS" "Indexing runs in background"
fi

if [[ "$SCRIPT_BASE" == *launch-services* ]]; then
  /System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister -kill -r -domain local -domain system -domain user 2>&1 | tee -a "$LOG_FILE" || true
  killall Finder Dock 2>/dev/null || true
  add_report_row "Launch Services" "Reset" "PASS" "lsregister -kill -r"
fi

if [[ "$SCRIPT_BASE" == *thermal* || "$SCRIPT_BASE" == *fan* ]]; then
  THERM=$(sudo powermetrics --samplers smc -n 1 2>/dev/null | head -n 40 || true)
  add_report_row "Thermal/SMC sample" "Captured" "INFO" "$THERM"
fi

if [[ "$SCRIPT_BASE" == *startup* || "$SCRIPT_BASE" == *background* ]]; then
  LA=$(find /Library/LaunchAgents /Library/LaunchDaemons ~/Library/LaunchAgents -type f -name '*.plist' 2>/dev/null | wc -l | tr -d ' ')
  add_report_row "Launch agents/daemons" "$LA plists" "INFO" "Count across system/user"
fi

if [[ "$SCRIPT_BASE" == *swap* || "$SCRIPT_BASE" == *memory-pressure* ]]; then
  SWAP=$(sysctl vm.swapusage 2>/dev/null || true)
  add_report_row "Swap usage" "$SWAP" "INFO" "sysctl vm.swapusage"
fi

if [[ "$SCRIPT_BASE" == *gpu* || "$SCRIPT_BASE" == *power-usage* || "$SCRIPT_BASE" == *process-priority* ]]; then
  TOP=$(ps aux | sort -nrk 3 | head -n 12)
  add_report_row "Top CPU processes" "Listed" "INFO" "$(echo "$TOP" | head -n 8)"
fi

if [[ "$SCRIPT_BASE" == *disk-io* ]]; then
  IO=$(iostat -c 2 2>/dev/null | tail -n 10 || true)
  add_report_row "iostat" "Sampled" "INFO" "$IO"
fi

show_progress 5 "$TOTAL_STEPS" "Performance done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "cpu frequency scaling audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
