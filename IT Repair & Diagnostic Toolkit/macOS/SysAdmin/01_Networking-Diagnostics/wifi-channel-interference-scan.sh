#!/bin/zsh
# ============================================================
# Script:      wifi-channel-interference-scan.sh
# Description: Production macOS SysAdmin repair/diagnostic for wifi channel interference scan.
#              Category: 01_Networking-Diagnostics. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh wifi-channel-interference-scan.sh
...
FOLDER_NAME='01_Networking-Diagnostics'
...
section_banner "NETWORKING"
show_progress 2 "$TOTAL_STEPS" "Interface inventory"
IFACES=$(ifconfig -l 2>/dev/null || true)
add_report_row "Interfaces" "$IFACES" "INFO" "ifconfig -l"
DEFAULT_IF=$(route -n get default 2>/dev/null | awk '/interface:/{print $2}')
add_report_row "Default interface" "${DEFAULT_IF:-none}" "$([[ -n "$DEFAULT_IF" ]] && echo PASS || echo FAIL)" "route get default"

show_progress 3 "$TOTAL_STEPS" "Connectivity"
if ping -c 2 -W 2000 1.1.1.1 >/dev/null 2>&1; then
  add_report_row "Ping 1.1.1.1" "OK" "PASS" "ICMP"
else
  add_report_row "Ping 1.1.1.1" "Failed" "FAIL" "ICMP"
fi

if [[ "$SCRIPT_BASE" == *dns* || "$SCRIPT_BASE" == *full-network* ]]; then
  show_progress 4 "$TOTAL_STEPS" "DNS flush"
  dscacheutil -flushcache 2>/dev/null || true
  killall -HUP mDNSResponder 2>/dev/null || true
  add_report_row "DNS cache flush" "Flushed" "PASS" "dscacheutil + mDNSResponder HUP"
fi

if [[ "$SCRIPT_BASE" == *wifi* || "$SCRIPT_BASE" == *ssid* ]]; then
  show_progress 4 "$TOTAL_STEPS" "Wi-Fi"
  ...
  if [[ -n "${WIFI_IF:-}" ]]; then
    ...
    SSID=$(networksetup -getairportnetwork "$WIFI_IF" 2>/dev/null || true)
    add_report_row "Wi-Fi reset" "$WIFI_IF" "PASS" "$SSID"
  else
    add_report_row "Wi-Fi interface" "Not found" "WARN" "networksetup"
  fi
fi

if [[ "$SCRIPT_BASE" == *proxy* ]]; then
  show_progress 4 "$TOTAL_STEPS" "Proxy clear"
  ...
  while IFS= read -r svc; do
    ...
    add_report_row "Proxy off" "$svc" "PASS" "web/secure/auto"
  done <<< "$SERVICES"
fi

if [[ "$SCRIPT_BASE" == *arp* ]]; then
  arp -a -d 2>/dev/null || true
  add_report_row "ARP cache" "Flushed" "PASS" "arp -a -d"
fi

if [[ "$SCRIPT_BASE" == *dhcp* ]]; then
  if [[ -n "${DEFAULT_IF:-}" ]]; then
    ipconfig set "$DEFAULT_IF" DHCP 2>/dev/null || true
    add_report_row "DHCP renew" "$DEFAULT_IF" "PASS" "ipconfig set DHCP"
  fi
fi

if [[ "$SCRIPT_BASE" == *captive* ]]; then
  BODY=$(curl -fsS --max-time 10 http://captive.apple.com/hotspot-detect.html 2>/dev/null || true)
  if echo "$BODY" | grep -qi Success; then
    add_report_row "Captive portal" "None detected" "PASS" "captive.apple.com"
  else
    add_report_row "Captive portal" "Possible portal / no net" "WARN" "$BODY"
  fi
fi

if [[ "$SCRIPT_BASE" == *port-connectivity* ]]; then
  for port in 80 443 53; do
    if nc -z -G 3 1.1.1.1 "$port" 2>/dev/null; then
      add_report_row "TCP $port" "Open" "PASS" "1.1.1.1"
    else
      add_report_row "TCP $port" "Closed/filtered" "WARN" "1.1.1.1"
    fi
  done
fi

if [[ "$SCRIPT_BASE" == *network-quality* ]] && command -v networkQuality >/dev/null 2>&1; then
  NQ=$(networkQuality -c 2>/dev/null | head -c 400 || true)
  add_report_row "networkQuality" "Completed" "INFO" "$NQ"
fi

if [[ "$SCRIPT_BASE" == *mdns* || "$SCRIPT_BASE" == *bonjour* ]]; then
  killall -HUP mDNSResponder 2>/dev/null || true
  add_report_row "mDNSResponder" "HUP sent" "PASS" "Bonjour repair"
fi

show_progress 6 "$TOTAL_STEPS" "Network checks done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "wifi channel interference scan" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
