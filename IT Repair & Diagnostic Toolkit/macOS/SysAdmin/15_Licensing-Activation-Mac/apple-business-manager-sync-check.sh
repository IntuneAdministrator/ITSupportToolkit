#!/bin/zsh
# ============================================================
# Script:      apple-business-manager-sync-check.sh
# Description: Production macOS SysAdmin repair/diagnostic for apple business manager sync check.
#              Category: 15_Licensing-Activation-Mac. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh apple-business-manager-sync-check.sh
...
FOLDER_NAME='15_Licensing-Activation-Mac'
...
section_banner "LICENSING / ACTIVATION MAC"
show_progress 2 "$TOTAL_STEPS" "License inventory"
if [[ "$SCRIPT_BASE" == *office* ]]; then
  if [[ -d "/Applications/Microsoft Word.app" ]]; then
    add_report_row "Microsoft Office" "Installed" "PASS" ""
  else
    add_report_row "Microsoft Office" "Not found" "WARN" ""
  fi
fi

if [[ "$SCRIPT_BASE" == *app-store* || "$SCRIPT_BASE" == *receipt* || "$SCRIPT_BASE" == *vpp* || "$SCRIPT_BASE" == *volume* ]]; then
  find /Applications -name 'receipt' 2>/dev/null | head -n 20 | tee -a "$LOG_FILE" || true
  add_report_row "App receipts" "Sampled under /Applications" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *apple-id* || "$SCRIPT_BASE" == *business-manager* || "$SCRIPT_BASE" == *license-audit* ]]; then
  profiles list 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "ABM/VPP" "Review via Business Manager + MDM" "INFO" "Device-side limited"
fi

sw_vers | tee -a "$LOG_FILE"
add_report_row "OS license platform" "$(sw_vers -productVersion)" "INFO" "macOS does not use Windows-style activation"

show_progress 5 "$TOTAL_STEPS" "Licensing done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "apple business manager sync check" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
