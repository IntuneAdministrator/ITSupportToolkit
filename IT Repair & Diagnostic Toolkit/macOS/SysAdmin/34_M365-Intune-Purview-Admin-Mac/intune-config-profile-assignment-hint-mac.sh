...
# ============================================================
# Script: intune-config-profile-assignment-hint-mac.sh | SysAdmin/34_M365-Intune-Purview-Admin-Mac | v1.4.0 Phase12 M365 cloud
# Usage: zsh intune-config-profile-assignment-hint-mac.sh [user@domain.com]
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='34_M365-Intune-Purview-Admin-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "34_M365-Intune-Purview-Admin-Mac" "INFO" "SysAdmin Phase12 M365"
...
section_banner "INTUNE / PURVIEW ADMIN MAC"
show_progress 3 "$TOTAL_STEPS" "Intune/Purview"
add_report_row "Intune" "https://intune.microsoft.com" "INFO" ""
add_report_row "Purview" "https://purview.microsoft.com" "INFO" ""
add_report_row "Secure Score" "https://security.microsoft.com/securescore" "INFO" ""
profiles status -type enrollment 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Local enrollment" "Captured" "INFO" "Compare with Intune device object"
if [[ -n "${GRAPH_TOKEN:-}" && -n "${TARGET_UPN:-}" ]]; then
  ENC=$(python3 -c "import urllib.parse,sys; print(urllib.parse.quote(sys.argv[1]))" "$TARGET_UPN" 2>/dev/null || echo "$TARGET_UPN")
  curl -fsS -H "Authorization: Bearer $GRAPH_TOKEN" "https://graph.microsoft.com/v1.0/users/$ENC/managedDevices" 2>/dev/null | tee -a "$LOG_FILE" || true
  add_report_row "managedDevices" "REST attempted" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "Intune Mac done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "intune config profile assignment hint mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive