...
# ============================================================
# Script: network-location-switch-audit-mac.sh | SysAdmin/29_DNS-Network-Services | v1.3.0 Phase11
# Requires: sudo | Usage: sudo zsh network-location-switch-audit-mac.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='29_DNS-Network-Services'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "29_DNS-Network-Services" "INFO" "SysAdmin"
section_banner "DNS NETWORK SERVICES"
show_progress 2 "$TOTAL_STEPS" "DNS"
scutil --dns 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "scutil --dns" "Dumped" "INFO" ""
if [[ "$SCRIPT_BASE" == *flush* || "$SCRIPT_BASE" == *mdnsresponder* ]]; then
  dscacheutil -flushcache 2>/dev/null || true
  killall -HUP mDNSResponder 2>/dev/null || true
  add_report_row "DNS cache" "Flushed + HUP mDNSResponder" "PASS" ""
fi
if [[ "$SCRIPT_BASE" == *dhcp* || "$SCRIPT_BASE" == *lease* ]]; then
  ipconfig set en0 DHCP 2>/dev/null || true
  add_report_row "DHCP" "ipconfig set attempted (en0)" "INFO" "Adjust interface as needed"
fi
if [[ "$SCRIPT_BASE" == *bonjour* || "$SCRIPT_BASE" == *location* ]]; then
  networksetup -listlocations 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Network locations" "Listed" "INFO" ""
fi
show_progress 5 "$TOTAL_STEPS" "DNS done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "network location switch audit mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive