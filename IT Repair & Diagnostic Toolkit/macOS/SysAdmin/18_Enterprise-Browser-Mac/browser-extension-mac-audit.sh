...
# ============================================================
# Script: browser-extension-mac-audit.sh | Role: SysAdmin/18_Enterprise-Browser-Mac | v1.1.0 expansion
# Requires: sudo | Usage: sudo zsh browser-extension-mac-audit.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='18_Enterprise-Browser-Mac'
...
add_report_row "Module" "18_Enterprise-Browser-Mac" "INFO" "SysAdmin"
section_banner "ENTERPRISE BROWSER MAC"
show_progress 2 "$TOTAL_STEPS" "Browsers"
for app in "Microsoft Edge.app" "Google Chrome.app" "Safari.app"; do
  [[ -d "/Applications/$app" ]] && add_report_row "$app" "Installed" "PASS" "" || add_report_row "$app" "Missing" "INFO" ""
done
if [[ "$SCRIPT_BASE" == *edge* ]]; then
  killall "Microsoft Edge" 2>/dev/null || true
  rm -rf "$REAL_HOME/Library/Caches/Microsoft Edge" 2>/dev/null || true
  add_report_row "Edge cache" "Cleared" "PASS" ""
fi
show_progress 5 "$TOTAL_STEPS" "Browser Mac done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "HTML report"
export_html_report "browser extension mac audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive