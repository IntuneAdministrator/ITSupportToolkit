...
# ============================================================
# Script: exo-shared-mailbox-permissions-mac.sh | SysAdmin/32_M365-Exchange-Online-Admin-Mac | v1.4.0 Phase12 M365 cloud
# Usage: zsh exo-shared-mailbox-permissions-mac.sh [user@domain.com]
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='32_M365-Exchange-Online-Admin-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "32_M365-Exchange-Online-Admin-Mac" "INFO" "SysAdmin Phase12 M365"
...
section_banner "EXCHANGE ONLINE ADMIN MAC"
show_progress 3 "$TOTAL_STEPS" "EXO"
add_report_row "EAC" "https://admin.exchange.microsoft.com" "INFO" ""
if command -v pwsh >/dev/null 2>&1; then
  add_report_row "EXO on Mac" "pwsh: Install-Module ExchangeOnlineManagement; Connect-ExchangeOnline" "PASS" "Supported on PowerShell 7"
  if [[ -n "${TARGET_UPN:-}" ]]; then
    pwsh -NoProfile -Command "
      try {
        if (-not (Get-Module -ListAvailable ExchangeOnlineManagement)) { Write-Output 'MODULE_MISSING'; exit 0 }
        Import-Module ExchangeOnlineManagement
        if (-not (Get-ConnectionInformation -EA SilentlyContinue)) { Write-Output 'NOT_CONNECTED - run Connect-ExchangeOnline first'; exit 0 }
        Get-EXOMailbox -Identity '$TARGET_UPN' | Format-List DisplayName,RecipientTypeDetails,PrimarySmtpAddress
      } catch { Write-Output $_.Exception.Message }
    " 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "EXO query" "Attempted via pwsh" "INFO" "$TARGET_UPN"
  fi
else
  add_report_row "EXO" "Install pwsh or use EAC portal" "WARN" ""
fi
show_progress 5 "$TOTAL_STEPS" "EXO Mac done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "exo shared mailbox permissions mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive