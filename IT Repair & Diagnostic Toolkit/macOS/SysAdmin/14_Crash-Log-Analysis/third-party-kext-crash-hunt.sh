#!/bin/zsh
# ============================================================
# Script:      third-party-kext-crash-hunt.sh
# Description: Production macOS SysAdmin repair/diagnostic for third party kext crash hunt.
#              Category: 14_Crash-Log-Analysis. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh third-party-kext-crash-hunt.sh
...
FOLDER_NAME='14_Crash-Log-Analysis'
...
section_banner "CRASH LOG ANALYSIS"
show_progress 2 "$TOTAL_STEPS" "Collect crash reports"
CRASH_DIRS=(
...
for d in "${CRASH_DIRS[@]}"; do
  if [[ -d "$d" ]]; then
    N=$(find "$d" -type f \( -name '*.crash' -o -name '*.ips' -o -name '*.panic' \) 2>/dev/null | wc -l | tr -d ' ')
    COUNT=$((COUNT+N))
    add_report_row "Crash dir" "$d" "INFO" "$N files"
  fi
done
add_report_row "Total crash-like files" "$COUNT" "$([[ $COUNT -lt 20 ]] && echo PASS || echo WARN)" "last inventory"

if [[ "$SCRIPT_BASE" == *kernel-panic* || "$SCRIPT_BASE" == *gpu-panic* ]]; then
  find /Library/Logs/DiagnosticReports -name '*Panic*' -o -name '*GPU*' 2>/dev/null | head -n 20 | tee -a "$LOG_FILE" || true
  add_report_row "Panic reports" "Listed in log" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *ips* || "$SCRIPT_BASE" == *trend* || "$SCRIPT_BASE" == *digest* || "$SCRIPT_BASE" == *hang* || "$SCRIPT_BASE" == *app-crash* ]]; then
  find /Library/Logs/DiagnosticReports -type f -mtime -14 2>/dev/null | head -n 30 | tee -a "$LOG_FILE" || true
  add_report_row "Reports (14d)" "Sampled" "INFO" "DiagnosticReports"
fi

if [[ "$SCRIPT_BASE" == *kext* ]]; then
  kextstat 2>/dev/null | grep -v com.apple | tee -a "$LOG_FILE" || true
  add_report_row "Non-Apple kexts" "Listed" "INFO" "kextstat (may be empty on modern macOS)"
fi

show_progress 5 "$TOTAL_STEPS" "Crash analysis done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "third party kext crash hunt" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
