#!/bin/zsh
# ============================================================
# Script:      icloud-drive-conflict-resolver.sh
# Description: Production macOS SysAdmin repair/diagnostic for icloud drive conflict resolver.
#              Category: 10_Disk-Storage-Mac. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh icloud-drive-conflict-resolver.sh
...
FOLDER_NAME='10_Disk-Storage-Mac'
...
section_banner "DISK / STORAGE"
show_progress 2 "$TOTAL_STEPS" "Disk space"
DF=$(df -h / | tail -n 1)
add_report_row "Root volume" "$DF" "INFO" "df -h /"
diskutil info / 2>&1 | tee -a "$LOG_FILE" | head -n 30 >/dev/null || true

if [[ "$SCRIPT_BASE" == *apfs-repair* || "$SCRIPT_BASE" == *fusion* ]]; then
  diskutil verifyVolume / 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "verifyVolume" "Completed" "INFO" "See log"
fi

if [[ "$SCRIPT_BASE" == *time-machine* ]]; then
  tmutil status 2>&1 | tee -a "$LOG_FILE" || true
  tmutil destinationinfo 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Time Machine" "Status captured" "INFO" "tmutil"
fi

if [[ "$SCRIPT_BASE" == *trash* ]]; then
  rm -rf "$REAL_HOME/.Trash/"* 2>/dev/null || true
  add_report_row "Trash empty" "$REAL_HOME/.Trash" "PASS" "force empty"
fi

if [[ "$SCRIPT_BASE" == *large-file* || "$SCRIPT_BASE" == *duplicate* ]]; then
  LARGE=$(find "$REAL_HOME" -type f -size +500M 2>/dev/null | head -n 20 || true)
  add_report_row "Files >500MB" "Sample" "INFO" "$LARGE"
fi

if [[ "$SCRIPT_BASE" == *snapshot* ]]; then
  tmutil thinlocalsnapshots / 10000000000 4 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "thinlocalsnapshots" "Attempted" "PASS" "Free space recovery"
fi

if [[ "$SCRIPT_BASE" == *encryption* || "$SCRIPT_BASE" == *filevault* || "$SCRIPT_BASE" == *nvme* ]]; then
  fdesetup status 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Encryption" "fdesetup status" "INFO" ""
  system_profiler SPNVMeDataType 2>/dev/null | head -n 40 | tee -a "$LOG_FILE" || true
fi

if [[ "$SCRIPT_BASE" == *icloud* ]]; then
  add_report_row "iCloud Drive" "Check ~/Library/Mobile Documents conflicts" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *benchmark* ]]; then
  if command -v dd >/dev/null; then
    WRITE=$(dd if=/dev/zero of=/tmp/itspeedtest bs=1m count=256 2>&1 | tail -n 1)
    rm -f /tmp/itspeedtest
    add_report_row "dd write sample" "$WRITE" "INFO" "256MB /tmp"
  fi
fi

show_progress 5 "$TOTAL_STEPS" "Disk done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "icloud drive conflict resolver" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
