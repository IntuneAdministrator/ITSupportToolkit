...
# ============================================================
# Script: jamf-manage-reinstall-hints.sh | Role: SysAdmin/16_Jamf-Advanced | v1.1.0 expansion
# Requires: sudo | Usage: sudo zsh jamf-manage-reinstall-hints.sh
# ============================================================
set -euo pipefail
...
section_banner "JAMF ADVANCED"
JAMF=""; [[ -x /usr/local/bin/jamf ]] && JAMF=/usr/local/bin/jamf; [[ -x /usr/local/jamf/bin/jamf ]] && JAMF=/usr/local/jamf/bin/jamf
if [[ -n "$JAMF" ]]; then
  add_report_row "jamf binary" "$JAMF" "PASS" ""
  show_progress 2 "$TOTAL_STEPS" "Jamf actions"
  "$JAMF" checkJSSConnection 2>&1 | tee -a "$LOG_FILE" || true
  if [[ "$SCRIPT_BASE" == *recon* || "$SCRIPT_BASE" == *policy* || "$SCRIPT_BASE" == *selfheal* || "$SCRIPT_BASE" == *log* ]]; then
    "$JAMF" recon 2>&1 | tee -a "$LOG_FILE" || true
    "$JAMF" policy 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "recon+policy" "Invoked" "PASS" ""
  fi
  if [[ "$SCRIPT_BASE" == *connect* ]]; then
  ...
  if [[ "$SCRIPT_BASE" == *log* ]]; then
    PACK="$LOG_DIR/JamfLogs_$TIMESTAMP"; mkdir -p "$PACK"
    cp /var/log/jamf.log "$PACK/" 2>/dev/null || true
    add_report_row "Jamf logs" "$PACK" "PASS" ""
  fi
else
  add_report_row "jamf" "Not installed" "WARN" ""
fi
show_progress 5 "$TOTAL_STEPS" "Jamf done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "HTML report"
export_html_report "jamf manage reinstall hints" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive