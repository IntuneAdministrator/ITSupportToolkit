...
# ============================================================
# Script: graph-service-health-snapshot-mac.sh | SysAdmin/31_M365-Graph-Tenant-Admin-Mac | v1.4.0 Phase12 M365 cloud
# Usage: zsh graph-service-health-snapshot-mac.sh [user@domain.com]
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='31_M365-Graph-Tenant-Admin-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "31_M365-Graph-Tenant-Admin-Mac" "INFO" "SysAdmin Phase12 M365"
...
show_progress 5 "$TOTAL_STEPS" "Graph Mac done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "graph service health snapshot mac" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive