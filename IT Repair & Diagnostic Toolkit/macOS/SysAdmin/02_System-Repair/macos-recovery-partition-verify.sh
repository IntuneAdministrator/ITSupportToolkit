#!/bin/zsh
# ============================================================
# Script:      macos-recovery-partition-verify.sh
# Description: Production macOS SysAdmin repair/diagnostic for macos recovery partition verify.
#              Category: 02_System-Repair. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh macos-recovery-partition-verify.sh
...
section_banner "SYSTEM REPAIR"
show_progress 2 "$TOTAL_STEPS" "System info"
VER=$(sw_vers -productVersion 2>/dev/null || true)
BUILD=$(sw_vers -buildVersion 2>/dev/null || true)
add_report_row "macOS" "$VER ($BUILD)" "INFO" "sw_vers"
ARCH=$(uname -m)
add_report_row "Arch" "$ARCH" "INFO" "uname -m"

if [[ "$SCRIPT_BASE" == *disk-utility* || "$SCRIPT_BASE" == *apfs* || "$SCRIPT_BASE" == *fsck* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Disk verify"
  DISKUTIL=$(diskutil verifyVolume / 2>&1 | tail -n 20 || true)
  if echo "$DISKUTIL" | grep -qiE 'appear to be OK|Volume appears'; then
    add_report_row "diskutil verifyVolume /" "OK" "PASS" "$DISKUTIL"
  else
    add_report_row "diskutil verifyVolume /" "Review output" "WARN" "$DISKUTIL"
  fi
fi

if [[ "$SCRIPT_BASE" == *permissions* || "$SCRIPT_BASE" == *home-folder* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Permissions"
  # Disk Utility repairDiskPermissions removed post-El Capitan; use home ACL reset guidance
  add_report_row "Permissions model" "SIP-managed system files" "INFO" "Use diskutil resetUserPermissions for user homes"
fi

if [[ "$SCRIPT_BASE" == *sip* || "$SCRIPT_BASE" == *system-integrity* ]]; then
  SIP=$(csrutil status 2>&1 || true)
  if echo "$SIP" | grep -qi enabled; then
    add_report_row "SIP" "Enabled" "PASS" "$SIP"
  else
    add_report_row "SIP" "Disabled/Unknown" "WARN" "$SIP"
  fi
fi

if [[ "$SCRIPT_BASE" == *rosetta* ]]; then
  if [[ "$ARCH" == "arm64" ]]; then
    /usr/sbin/softwareupdate --install-rosetta --agree-to-license 2>&1 | tee -a "$LOG_FILE" || true
    add_report_row "Rosetta 2" "Install attempted" "PASS" "softwareupdate --install-rosetta"
  else
    add_report_row "Rosetta 2" "N/A on Intel" "INFO" "$ARCH"
  fi
fi

if [[ "$SCRIPT_BASE" == *temp-file* ]]; then
  show_progress 3 "$TOTAL_STEPS" "Temp clean"
  BEFORE=$(du -sk /private/tmp 2>/dev/null | awk '{print $1}')
  find /private/tmp -type f -mtime +2 -delete 2>/dev/null || true
  find /var/folders -type f -name '*.tmp' -mtime +3 -delete 2>/dev/null || true
  AFTER=$(du -sk /private/tmp 2>/dev/null | awk '{print $1}')
  add_report_row "Temp cleanup" "${BEFORE}KB -> ${AFTER}KB" "PASS" "/private/tmp"
fi

if [[ "$SCRIPT_BASE" == *launchd* ]]; then
  BAD=0
  while IFS= read -r plist; do
    if ! plutil -lint "$plist" >/dev/null 2>&1; then
      add_report_row "Invalid plist" "$plist" "WARN" "plutil"
      BAD=$((BAD+1))
    fi
  done < <(find /Library/LaunchDaemons /Library/LaunchAgents -name '*.plist' 2>/dev/null | head -n 40)
  add_report_row "launchd plist scan" "$BAD invalid" "$([[ $BAD -eq 0 ]] && echo PASS || echo WARN)" "sample 40"
fi

if [[ "$SCRIPT_BASE" == *snapshot* ]]; then
  SNAP=$(tmutil listlocalsnapshots / 2>&1 || true)
  add_report_row "APFS local snapshots" "Listed" "INFO" "$SNAP"
fi

if [[ "$SCRIPT_BASE" == *system-info* || "$SCRIPT_BASE" == *version-build* ]]; then
  system_profiler SPSoftwareDataType SPHardwareDataType 2>/dev/null | tee -a "$LOG_FILE" | head -n 40 >/dev/null
  add_report_row "system_profiler" "Captured to log" "PASS" "SPSoftwareDataType/SPHardwareDataType"
fi

if [[ "$SCRIPT_BASE" == *dyld* ]]; then
  update_dyld_shared_cache -force 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "dyld shared cache" "Rebuild attempted" "INFO" "May require Recovery on newer macOS"
fi

show_progress 5 "$TOTAL_STEPS" "System repair done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "macos recovery partition verify" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
