#!/bin/zsh
# ============================================================
# Script:      ipp-everywhere-driver-install.sh
# Description: Production macOS SysAdmin repair/diagnostic for ipp everywhere driver install.
#              Category: 12_Printing-Mac. Logs + HTML report under
#              ~/Library/Logs/IT-Repair. Compatible with Intune and Jamf.
# Author:      Allester Padovani
# Version:     1.0.0
# Tested:      macOS Ventura 13.x / Sonoma 14.x / Sequoia 15.x
# Requires:    sudo / root
# Usage:       sudo zsh ipp-everywhere-driver-install.sh
...
section_banner "PRINTING / CUPS"
show_progress 2 "$TOTAL_STEPS" "CUPS service"
launchctl kickstart -k system/org.cups.cupsd 2>/dev/null || true
add_report_row "cupsd" "kickstart" "PASS" "launchctl"

if [[ "$SCRIPT_BASE" == *queue* || "$SCRIPT_BASE" == *cups-reset* ]]; then
  cancel -a 2>/dev/null || true
  rm -f /var/spool/cups/c* /var/spool/cups/d* 2>/dev/null || true
  add_report_row "Print queue" "Cleared" "PASS" "cancel -a + spool cleanup"
fi

if [[ "$SCRIPT_BASE" == *default-printer* ]]; then
  lpstat -d 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Default printer" "lpstat -d" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *airprint* || "$SCRIPT_BASE" == *ipp* ]]; then
  lpinfo -v 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "Printer discovery" "lpinfo -v" "INFO" ""
fi

if [[ "$SCRIPT_BASE" == *sharing* ]]; then
  cupsctl --share-printers 2>&1 || true
  add_report_row "Printer sharing" "Enabled attempt" "PASS" "cupsctl"
fi

if [[ "$SCRIPT_BASE" == *log* ]]; then
  tail -n 80 /var/log/cups/error_log 2>/dev/null | tee -a "$LOG_FILE" || true
  add_report_row "CUPS error_log" "Tailed" "INFO" "/var/log/cups/error_log"
fi

lpstat -p 2>&1 | tee -a "$LOG_FILE" || true
add_report_row "Printers" "lpstat -p" "INFO" ""

show_progress 5 "$TOTAL_STEPS" "Printing done"

show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Export HTML report"
export_html_report "ipp everywhere driver install" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive
