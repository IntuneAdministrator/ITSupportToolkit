...
# ============================================================
# Script: diagnostic-reports-age-audit.sh | SysAdmin/24_Diagnostics-Telemetry-Mac | v1.2.0 Phase10
# Requires: sudo | Usage: sudo zsh diagnostic-reports-age-audit.sh
# ============================================================
set -euo pipefail
...
SCRIPT_BASE=$(basename "$0" .sh); FOLDER_NAME='24_Diagnostics-Telemetry-Mac'; TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
add_report_row "Module" "24_Diagnostics-Telemetry-Mac" "INFO" "SysAdmin"
section_banner "DIAGNOSTICS TELEMETRY MAC"
show_progress 2 "$TOTAL_STEPS" "Diagnostics"
if [[ "$SCRIPT_BASE" == *sysdiagnose* ]]; then
  sysdiagnose -f "$LOG_DIR" -A "ITRepair_$TIMESTAMP" 2>&1 | tee -a "$LOG_FILE" || true
  add_report_row "sysdiagnose" "Triggered (long)" "INFO" "$LOG_DIR"
fi
log show --last 1h --predicate 'eventMessage CONTAINS "error"' 2>/dev/null | head -n 40 | tee -a "$LOG_FILE" || true
add_report_row "unified log errors" "Sample 1h" "INFO" ""
ls -lt /Library/Logs/DiagnosticReports 2>/dev/null | head -n 15 | tee -a "$LOG_FILE" || true
add_report_row "DiagnosticReports" "Listed" "INFO" ""
show_progress 5 "$TOTAL_STEPS" "Diagnostics done"
show_progress "$TOTAL_STEPS" "$TOTAL_STEPS" "Report"
export_html_report "diagnostic reports age audit" "$COMPUTER_NAME" "$JOIN_STATE"
print_completion_summary
open_report_if_interactive