...
<#
.SYNOPSIS
    Common problem fix: NRPT Rule Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\NRPT-Rule-Conflict-Fix
.EXAMPLE
    .\NRPT-Rule-Conflict-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NRPT Rule Conflict Fix'; $totalSteps = 8; $Name = 'NRPT-Rule-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'NRPT Rule Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NRPT Rule Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
