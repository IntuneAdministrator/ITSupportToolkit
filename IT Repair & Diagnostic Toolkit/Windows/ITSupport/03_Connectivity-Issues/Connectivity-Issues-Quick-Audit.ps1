...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-Quick-Audit.ps1
.EXAMPLE
    .\Connectivity-Issues-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues Quick Audit'; $totalSteps = 8; $Name = 'Connectivity-Issues-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
