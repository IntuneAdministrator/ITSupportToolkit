...
<#
.SYNOPSIS
    Common problem fix: APIPA 169 Address Fix
.DESCRIPTION
...
.EXAMPLE
    .\APIPA-169-Address-Fix
.EXAMPLE
    .\APIPA-169-Address-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'APIPA 169 Address Fix'; $totalSteps = 8; $Name = 'APIPA-169-Address-Fix'
try {
    ...
    Export-HtmlReport -Title 'APIPA 169 Address Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'APIPA 169 Address Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
