...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-Helpdesk-Report.ps1
.EXAMPLE
    .\Connectivity-Issues-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues Helpdesk Report'; $totalSteps = 8; $Name = 'Connectivity-Issues-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
