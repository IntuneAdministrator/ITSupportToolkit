...
<#
.SYNOPSIS
    Common problem fix: Ethernet Unidentified Network
.DESCRIPTION
...
.EXAMPLE
    .\Ethernet-Unidentified-Network.ps1
.EXAMPLE
    .\Ethernet-Unidentified-Network.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Ethernet Unidentified Network'; $totalSteps = 8; $Name = 'Ethernet-Unidentified-Network'
try {
    ...
    Export-HtmlReport -Title 'Ethernet Unidentified Network' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Ethernet Unidentified Network' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
