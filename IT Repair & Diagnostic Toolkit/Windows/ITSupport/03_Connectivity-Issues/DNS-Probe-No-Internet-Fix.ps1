...
<#
.SYNOPSIS
    Common problem fix: DNS Probe No Internet Fix
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Probe-No-Internet-Fix.ps1
.EXAMPLE
    .\DNS-Probe-No-Internet-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Probe No Internet Fix'; $totalSteps = 8; $Name = 'DNS-Probe-No-Internet-Fix'
try {
    ...
    Export-HtmlReport -Title 'DNS Probe No Internet Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Probe No Internet Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
