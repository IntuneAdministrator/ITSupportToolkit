...
<#
.SYNOPSIS
    Common problem fix: Airplane Mode Stuck On Fix
.DESCRIPTION
...
.EXAMPLE
    .\Airplane-Mode-Stuck-On-Fix
.EXAMPLE
    .\Airplane-Mode-Stuck-On-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Airplane Mode Stuck On Fix'; $totalSteps = 8; $Name = 'Airplane-Mode-Stuck-On-Fix'
try {
    ...
    Export-HtmlReport -Title 'Airplane Mode Stuck On Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Airplane Mode Stuck On Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
