...
<#
.SYNOPSIS
    Common problem fix: WiFi Certificate 8021X Fix
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Certificate-8021X-Fix
.EXAMPLE
    .\WiFi-Certificate-8021X-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Certificate 8021X Fix'; $totalSteps = 8; $Name = 'WiFi-Certificate-8021X-Fix'
try {
    ...
    Export-HtmlReport -Title 'WiFi Certificate 8021X Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Certificate 8021X Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
