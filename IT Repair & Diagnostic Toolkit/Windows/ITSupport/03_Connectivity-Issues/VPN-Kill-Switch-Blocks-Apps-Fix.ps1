...
<#
.SYNOPSIS
    Common problem fix: VPN Kill Switch Blocks Apps Fix
.DESCRIPTION
...
.EXAMPLE
    .\VPN-Kill-Switch-Blocks-Apps-Fix
.EXAMPLE
    .\VPN-Kill-Switch-Blocks-Apps-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VPN Kill Switch Blocks Apps Fix'; $totalSteps = 8; $Name = 'VPN-Kill-Switch-Blocks-Apps-Fix'
try {
    ...
    Export-HtmlReport -Title 'VPN Kill Switch Blocks Apps Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VPN Kill Switch Blocks Apps Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
