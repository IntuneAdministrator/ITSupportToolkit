...
<#
.SYNOPSIS
    Common problem fix: DNS Suffix Search Order Fix
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Suffix-Search-Order-Fix
.EXAMPLE
    .\DNS-Suffix-Search-Order-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Suffix Search Order Fix'; $totalSteps = 8; $Name = 'DNS-Suffix-Search-Order-Fix'
try {
    ...
    Export-HtmlReport -Title 'DNS Suffix Search Order Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Suffix Search Order Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
