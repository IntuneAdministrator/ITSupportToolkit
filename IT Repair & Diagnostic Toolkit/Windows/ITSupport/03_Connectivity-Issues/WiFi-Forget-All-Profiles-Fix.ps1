...
<#
.SYNOPSIS
    Common problem fix: WiFi Forget All Profiles Fix
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Forget-All-Profiles-Fix
.EXAMPLE
    .\WiFi-Forget-All-Profiles-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Forget All Profiles Fix'; $totalSteps = 8; $Name = 'WiFi-Forget-All-Profiles-Fix'
try {
    ...
    Export-HtmlReport -Title 'WiFi Forget All Profiles Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Forget All Profiles Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
