...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: DNS Quick Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for DNS-Quick-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'DNS-Quick-Fix'
        Add-ReportRow -Check 'Tool' -Result 'DNS-Quick-Fix' -Status 'INFO' -Detail 'ITSupport/03_Connectivity-Issues'
    ...
    Export-HtmlReport -Title 'DNS Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
