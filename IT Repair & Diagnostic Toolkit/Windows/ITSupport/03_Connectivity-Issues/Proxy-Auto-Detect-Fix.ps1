...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Proxy Auto Detect Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Proxy-Auto-Detect-Fix.
...
$exitCode = 0
$Activity = 'Proxy Auto Detect Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Proxy-Auto-Detect-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Proxy-Auto-Detect-Fix' -Status 'INFO' -Detail 'ITSupport/03_Connectivity-Issues'
    ...
    Export-HtmlReport -Title 'Proxy Auto Detect Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Proxy Auto Detect Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
