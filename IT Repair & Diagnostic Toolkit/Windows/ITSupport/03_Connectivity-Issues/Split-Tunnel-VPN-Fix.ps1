...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Split Tunnel VPN Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Split-Tunnel-VPN-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Split-Tunnel-VPN-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Split-Tunnel-VPN-Fix' -Status 'INFO' -Detail 'ITSupport/03_Connectivity-Issues'
    ...
    Export-HtmlReport -Title 'Split Tunnel VPN Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Split Tunnel VPN Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
