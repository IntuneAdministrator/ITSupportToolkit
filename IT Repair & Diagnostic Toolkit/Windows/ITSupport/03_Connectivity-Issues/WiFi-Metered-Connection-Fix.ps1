...
<#
.SYNOPSIS
    Common problem fix: WiFi Metered Connection Fix
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Metered-Connection-Fix
.EXAMPLE
    .\WiFi-Metered-Connection-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Metered Connection Fix'; $totalSteps = 8; $Name = 'WiFi-Metered-Connection-Fix'
try {
    ...
    Export-HtmlReport -Title 'WiFi Metered Connection Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Metered Connection Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
