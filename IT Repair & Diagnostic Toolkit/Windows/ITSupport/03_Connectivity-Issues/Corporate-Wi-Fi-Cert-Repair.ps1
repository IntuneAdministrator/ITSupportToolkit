...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Corporate Wi Fi Cert Repair
.DESCRIPTION
    Production-grade Windows IT Support tool for Corporate-Wi-Fi-Cert-Repair.
Category: 03_Connectivity-Issues. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Corporate-Wi-Fi-Cert-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Corporate-Wi-Fi-Cert-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Corporate Wi Fi Cert Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'CONNECTIVITY ISSUES'
        $Name = 'Corporate-Wi-Fi-Cert-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Corporate-Wi-Fi-Cert-Repair' -Status 'INFO' -Detail 'ITSupport/03_Connectivity-Issues'
        Write-SectionBanner 'CONNECTIVITY QUICK FIX'
        $up = @(Get-NetAdapter | Where-Object Status -eq 'Up')
        Add-ReportRow -Check 'Adapters Up' -Result "$($up.Count)" -Status $(if ($up.Count -gt 0) {'PASS'} else {'FAIL'}) -Detail (($up | ForEach-Object Name) -join ', ')
        Write-ProgressStep 2 $totalSteps $Activity 'Adapters'

        if ($Name -match 'No-Internet|DNS-Quick|Captive-Portal|Proxy') {
            if ($PSCmdlet.ShouldProcess('Network stack', 'Flush DNS / reset WinHTTP proxy / renew DHCP')) {
                ipconfig /flushdns 2>&1 | Out-Null
                Clear-DnsClientCache -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'DNS flush' -Result 'Done' -Status 'PASS' -Detail ''
                & netsh winhttp reset proxy 2>&1 | Out-Null
                Add-ReportRow -Check 'WinHTTP proxy reset' -Result 'Done' -Status 'PASS' -Detail ''
                Backup-RegistryKey -KeyPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
                ...
                Add-ReportRow -Check 'User ProxyEnable' -Result '0' -Status 'PASS' -Detail 'Rollback via .reg'
                ipconfig /release 2>&1 | Out-Null
                ipconfig /renew 2>&1 | Out-Null
                Add-ReportRow -Check 'DHCP renew' -Result 'Done' -Status 'PASS' -Detail ''
            }
            try {
                $ncsi = Invoke-WebRequest -Uri 'http://www.msftconnecttest.com/connecttest.txt' -UseBasicParsing -TimeoutSec 10
                $ok = $ncsi.Content -match 'Microsoft Connect Test'
                Add-ReportRow -Check 'NCSI / captive portal' -Result $(if ($ok) {'Internet OK'} else {'Unexpected - possible portal'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail $ncsi.Content.Trim()
            } catch {
                Add-ReportRow -Check 'NCSI' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
            }
        }

        if ($Name -match 'Wi-Fi-Reconnect|Corporate-Wi-Fi') {
            if ($PSCmdlet.ShouldProcess('WLAN', 'Restart WlanSvc and cycle Wi-Fi adapters')) {
                Restart-Service WlanSvc -Force -ErrorAction SilentlyContinue
                Get-NetAdapter | Where-Object { $_.Name -match 'Wi-Fi|Wireless' -or $_.InterfaceDescription -match 'Wireless|802\.11' } | ForEach-Object {
                    ...
                    Add-ReportRow -Check "Reset $($_.Name)" -Result 'Toggled' -Status 'PASS' -Detail $_.InterfaceDescription
                }
            }
            if ($Name -match 'Cert') {
                $wifiCerts = @(Get-ChildItem Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Where-Object { $_.Subject -match 'WiFi|Wireless|EAP|802' -or $_.EnhancedKeyUsageList -match 'Client Auth' })
                Add-ReportRow -Check 'Client auth certs' -Result "$($wifiCerts.Count)" -Status $(if ($wifiCerts.Count -gt 0) {'PASS'} else {'WARN'}) -Detail 'Machine store My'
            }
        }

        if ($Name -match 'VPN|Split-Tunnel') {
            $vpns = @(Get-VpnConnection -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'VPN profiles' -Result "$($vpns.Count)" -Status 'INFO' -Detail (($vpns | ForEach-Object { "$($_.Name):$($_.ConnectionStatus)" }) -join '; ')
            foreach ($v in $vpns) {
                if ($v.ConnectionStatus -eq 'Connected' -and $Name -match 'Quick-Reset' -and $PSCmdlet.ShouldProcess($v.Name, 'Disconnect then note reconnect')) {
                    rasdial "$($v.Name)" /disconnect 2>&1 | Out-Null
                    Add-ReportRow -Check "Disconnect $($v.Name)" -Result 'Disconnected' -Status 'PASS' -Detail 'User may reconnect from Settings'
                }
                Add-ReportRow -Check "Split tunneling $($v.Name)" -Result "$($v.SplitTunneling)" -Status 'INFO' -Detail $v.TunnelType
            }
            Get-Service | Where-Object { $_.Name -match 'Vpn|RasMan' } | ForEach-Object {
                if ($PSCmdlet.ShouldProcess($_.Name, 'Restart VPN-related service')) {
                    Restart-Service $_.Name -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check "Restart $($_.Name)" -Result (Get-Service $_.Name).Status -Status 'PASS' -Detail ''
                }
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Connectivity'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Corporate Wi Fi Cert Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Corporate Wi Fi Cert Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
