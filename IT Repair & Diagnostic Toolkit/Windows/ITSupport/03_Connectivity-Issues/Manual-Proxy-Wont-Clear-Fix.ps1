...
<#
.SYNOPSIS
    Common problem fix: Manual Proxy Wont Clear Fix
.DESCRIPTION
...
.EXAMPLE
    .\Manual-Proxy-Wont-Clear-Fix
.EXAMPLE
    .\Manual-Proxy-Wont-Clear-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Manual Proxy Wont Clear Fix'; $totalSteps = 8; $Name = 'Manual-Proxy-Wont-Clear-Fix'
try {
    ...
    Export-HtmlReport -Title 'Manual Proxy Wont Clear Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Manual Proxy Wont Clear Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
