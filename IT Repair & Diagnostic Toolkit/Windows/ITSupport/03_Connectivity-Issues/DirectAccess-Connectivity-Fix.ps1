...
<#
.SYNOPSIS
    Common problem fix: DirectAccess Connectivity Fix
.DESCRIPTION
...
.EXAMPLE
    .\DirectAccess-Connectivity-Fix
.EXAMPLE
    .\DirectAccess-Connectivity-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DirectAccess Connectivity Fix'; $totalSteps = 8; $Name = 'DirectAccess-Connectivity-Fix'
try {
    ...
    Export-HtmlReport -Title 'DirectAccess Connectivity Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DirectAccess Connectivity Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
