...
<#
.SYNOPSIS
    Common problem fix: WiFi Hidden SSID Connect Fix
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Hidden-SSID-Connect-Fix
.EXAMPLE
    .\WiFi-Hidden-SSID-Connect-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Hidden SSID Connect Fix'; $totalSteps = 8; $Name = 'WiFi-Hidden-SSID-Connect-Fix'
try {
    ...
    Export-HtmlReport -Title 'WiFi Hidden SSID Connect Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Hidden SSID Connect Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
