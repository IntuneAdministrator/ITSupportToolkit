...
<#
.SYNOPSIS
    Common problem fix: VPN Connected Apps No Access
.DESCRIPTION
...
.EXAMPLE
    .\VPN-Connected-Apps-No-Access.ps1
.EXAMPLE
    .\VPN-Connected-Apps-No-Access.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VPN Connected Apps No Access'; $totalSteps = 8; $Name = 'VPN-Connected-Apps-No-Access'
try {
    ...
    Export-HtmlReport -Title 'VPN Connected Apps No Access' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VPN Connected Apps No Access' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
