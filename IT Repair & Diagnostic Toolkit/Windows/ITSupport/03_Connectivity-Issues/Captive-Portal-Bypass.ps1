...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Captive Portal Bypass
.DESCRIPTION
    Production-grade Windows IT Support tool for Captive-Portal-Bypass.
...
$exitCode = 0
$Activity = 'Captive Portal Bypass'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Captive-Portal-Bypass' -Status 'INFO' -Detail 'ITSupport/03_Connectivity-Issues'
    ...
    Export-HtmlReport -Title 'Captive Portal Bypass' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Captive Portal Bypass' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
