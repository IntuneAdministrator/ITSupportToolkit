...
<#
.SYNOPSIS
    Common problem fix: Ethernet Speed 100Mbps Only Fix
.DESCRIPTION
...
.EXAMPLE
    .\Ethernet-Speed-100Mbps-Only-Fix
.EXAMPLE
    .\Ethernet-Speed-100Mbps-Only-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Ethernet Speed 100Mbps Only Fix'; $totalSteps = 8; $Name = 'Ethernet-Speed-100Mbps-Only-Fix'
try {
    ...
    Export-HtmlReport -Title 'Ethernet Speed 100Mbps Only Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Ethernet Speed 100Mbps Only Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
