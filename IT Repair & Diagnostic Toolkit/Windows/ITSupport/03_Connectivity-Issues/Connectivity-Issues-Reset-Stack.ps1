...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-Reset-Stack.ps1
.EXAMPLE
    .\Connectivity-Issues-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues Reset Stack'; $totalSteps = 8; $Name = 'Connectivity-Issues-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
