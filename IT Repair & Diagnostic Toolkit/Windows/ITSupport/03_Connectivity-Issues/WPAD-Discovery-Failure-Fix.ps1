...
<#
.SYNOPSIS
    Common problem fix: WPAD Discovery Failure Fix
.DESCRIPTION
...
.EXAMPLE
    .\WPAD-Discovery-Failure-Fix
.EXAMPLE
    .\WPAD-Discovery-Failure-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WPAD Discovery Failure Fix'; $totalSteps = 8; $Name = 'WPAD-Discovery-Failure-Fix'
try {
    ...
    Export-HtmlReport -Title 'WPAD Discovery Failure Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WPAD Discovery Failure Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
