...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-Policy-Check.ps1
.EXAMPLE
    .\Connectivity-Issues-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues Policy Check'; $totalSteps = 8; $Name = 'Connectivity-Issues-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
