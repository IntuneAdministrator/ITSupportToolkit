...
<#
.SYNOPSIS
    Common problem fix: IPv6 Only Network Failure Fix
.DESCRIPTION
...
.EXAMPLE
    .\IPv6-Only-Network-Failure-Fix
.EXAMPLE
    .\IPv6-Only-Network-Failure-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'IPv6 Only Network Failure Fix'; $totalSteps = 8; $Name = 'IPv6-Only-Network-Failure-Fix'
try {
    ...
    Export-HtmlReport -Title 'IPv6 Only Network Failure Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IPv6 Only Network Failure Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
