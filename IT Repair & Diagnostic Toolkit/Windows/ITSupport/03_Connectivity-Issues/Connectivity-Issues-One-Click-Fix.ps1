...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-One-Click-Fix.ps1
.EXAMPLE
    .\Connectivity-Issues-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues One Click Fix'; $totalSteps = 8; $Name = 'Connectivity-Issues-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
