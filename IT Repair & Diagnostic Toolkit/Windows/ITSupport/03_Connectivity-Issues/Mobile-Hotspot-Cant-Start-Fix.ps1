...
<#
.SYNOPSIS
    Common problem fix: Mobile Hotspot Cant Start Fix
.DESCRIPTION
...
.EXAMPLE
    .\Mobile-Hotspot-Cant-Start-Fix
.EXAMPLE
    .\Mobile-Hotspot-Cant-Start-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Mobile Hotspot Cant Start Fix'; $totalSteps = 8; $Name = 'Mobile-Hotspot-Cant-Start-Fix'
try {
    ...
    Export-HtmlReport -Title 'Mobile Hotspot Cant Start Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Mobile Hotspot Cant Start Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
