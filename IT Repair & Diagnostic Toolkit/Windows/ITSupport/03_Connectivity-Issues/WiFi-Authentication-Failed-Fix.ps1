...
<#
.SYNOPSIS
    Common problem fix: WiFi Authentication Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Authentication-Failed-Fix
.EXAMPLE
    .\WiFi-Authentication-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Authentication Failed Fix'; $totalSteps = 8; $Name = 'WiFi-Authentication-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'WiFi Authentication Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Authentication Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
