...
<#
.SYNOPSIS
    Common problem fix: Always On VPN Stuck Fix
.DESCRIPTION
...
.EXAMPLE
    .\Always-On-VPN-Stuck-Fix
.EXAMPLE
    .\Always-On-VPN-Stuck-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Always On VPN Stuck Fix'; $totalSteps = 8; $Name = 'Always-On-VPN-Stuck-Fix'
try {
    ...
    Export-HtmlReport -Title 'Always On VPN Stuck Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Always On VPN Stuck Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
