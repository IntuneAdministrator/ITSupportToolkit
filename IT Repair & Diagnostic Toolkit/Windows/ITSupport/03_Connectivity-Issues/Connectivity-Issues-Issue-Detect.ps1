...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-Issue-Detect.ps1
.EXAMPLE
    .\Connectivity-Issues-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues Issue Detect'; $totalSteps = 8; $Name = 'Connectivity-Issues-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
