...
<#
.SYNOPSIS
    Common problem fix: WiFi Random MAC Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Random-MAC-Conflict-Fix
.EXAMPLE
    .\WiFi-Random-MAC-Conflict-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Random MAC Conflict Fix'; $totalSteps = 8; $Name = 'WiFi-Random-MAC-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'WiFi Random MAC Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Random MAC Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
