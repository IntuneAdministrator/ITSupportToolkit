...
<#
.SYNOPSIS
    Common problem fix: Limited Connectivity WiFi Fix
.DESCRIPTION
...
.EXAMPLE
    .\Limited-Connectivity-WiFi-Fix.ps1
.EXAMPLE
    .\Limited-Connectivity-WiFi-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Limited Connectivity WiFi Fix'; $totalSteps = 8; $Name = 'Limited-Connectivity-WiFi-Fix'
try {
    ...
    Export-HtmlReport -Title 'Limited Connectivity WiFi Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Limited Connectivity WiFi Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
