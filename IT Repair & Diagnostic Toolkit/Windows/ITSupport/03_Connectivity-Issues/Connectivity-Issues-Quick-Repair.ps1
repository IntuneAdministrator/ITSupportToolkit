...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-Quick-Repair.ps1
.EXAMPLE
    .\Connectivity-Issues-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues Quick Repair'; $totalSteps = 8; $Name = 'Connectivity-Issues-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
