...
<#
.SYNOPSIS
    Common problem fix: WiFi Connected No Internet
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Connected-No-Internet.ps1
.EXAMPLE
    .\WiFi-Connected-No-Internet.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Connected No Internet'; $totalSteps = 8; $Name = 'WiFi-Connected-No-Internet'
try {
    ...
    Export-HtmlReport -Title 'WiFi Connected No Internet' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Connected No Internet' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
