...
<#
.SYNOPSIS
    Common problem fix: Dual Stack Preference Fix
.DESCRIPTION
...
.EXAMPLE
    .\Dual-Stack-Preference-Fix
.EXAMPLE
    .\Dual-Stack-Preference-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dual Stack Preference Fix'; $totalSteps = 8; $Name = 'Dual-Stack-Preference-Fix'
try {
    ...
    Export-HtmlReport -Title 'Dual Stack Preference Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dual Stack Preference Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
