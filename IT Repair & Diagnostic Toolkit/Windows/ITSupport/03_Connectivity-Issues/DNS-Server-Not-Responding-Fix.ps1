...
<#
.SYNOPSIS
    Common problem fix: DNS Server Not Responding Fix
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Server-Not-Responding-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Server Not Responding Fix'; $totalSteps = 8; $Name = 'DNS-Server-Not-Responding-Fix'
try {
    ...
    Export-HtmlReport -Title 'DNS Server Not Responding Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Server Not Responding Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
