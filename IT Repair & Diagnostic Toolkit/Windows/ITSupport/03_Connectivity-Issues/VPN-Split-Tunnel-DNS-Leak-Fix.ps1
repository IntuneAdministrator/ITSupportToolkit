...
<#
.SYNOPSIS
    Common problem fix: VPN Split Tunnel DNS Leak Fix
.DESCRIPTION
...
.EXAMPLE
    .\VPN-Split-Tunnel-DNS-Leak-Fix
.EXAMPLE
    .\VPN-Split-Tunnel-DNS-Leak-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VPN Split Tunnel DNS Leak Fix'; $totalSteps = 8; $Name = 'VPN-Split-Tunnel-DNS-Leak-Fix'
try {
    ...
    Export-HtmlReport -Title 'VPN Split Tunnel DNS Leak Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VPN Split Tunnel DNS Leak Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
