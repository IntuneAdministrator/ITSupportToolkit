...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: No Internet Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for No-Internet-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'No-Internet-Fix'
        Add-ReportRow -Check 'Tool' -Result 'No-Internet-Fix' -Status 'INFO' -Detail 'ITSupport/03_Connectivity-Issues'
    ...
    Export-HtmlReport -Title 'No Internet Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'No Internet Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
