...
<#
.SYNOPSIS
    Common problem fix: Ethernet Duplex Mismatch Fix
.DESCRIPTION
...
.EXAMPLE
    .\Ethernet-Duplex-Mismatch-Fix
.EXAMPLE
    .\Ethernet-Duplex-Mismatch-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Ethernet Duplex Mismatch Fix'; $totalSteps = 8; $Name = 'Ethernet-Duplex-Mismatch-Fix'
try {
    ...
    Export-HtmlReport -Title 'Ethernet Duplex Mismatch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Ethernet Duplex Mismatch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
