...
<#
.SYNOPSIS
    Common problem fix: Connectivity Issues User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Connectivity-Issues-User-Triage.ps1
.EXAMPLE
    .\Connectivity-Issues-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connectivity Issues User Triage'; $totalSteps = 8; $Name = 'Connectivity-Issues-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Connectivity Issues User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connectivity Issues User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
