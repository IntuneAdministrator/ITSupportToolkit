...
<#
.SYNOPSIS
    Common problem fix: Runbook WiFi Ethernet Full Fix
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 03_Connectivity-Issues). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-WiFi-Ethernet-Full-Fix.ps1
.EXAMPLE
    .\Runbook-WiFi-Ethernet-Full-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook WiFi Ethernet Full Fix'; $totalSteps = 8; $Name = 'Runbook-WiFi-Ethernet-Full-Fix'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '03_Connectivity-Issues' -Status 'INFO' -Detail 'ITSupport'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '03_Connectivity-Issues'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '03_Connectivity-Issues'
    ...
    Export-HtmlReport -Title 'Runbook WiFi Ethernet Full Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook WiFi Ethernet Full Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
