...
<#
.SYNOPSIS
    Enterprise IT tool: Missing Font Install Guide
.DESCRIPTION
    Phase 11 expansion (ITSupport/22_Font-Language-Region). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Missing-Font-Install-Guide.ps1
.EXAMPLE
    .\Missing-Font-Install-Guide.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Missing Font Install Guide'; $totalSteps=8; $Name='Missing-Font-Install-Guide'; $Folder='22_Font-Language-Region'
try {
    ...
    Export-HtmlReport -Title 'Missing Font Install Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Missing Font Install Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
