...
<#
.SYNOPSIS
    Common problem fix: Font Language Region Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 22_Font-Language-Region). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Font-Language-Region-Helpdesk-Report.ps1
.EXAMPLE
    .\Font-Language-Region-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region Helpdesk Report'; $totalSteps = 8; $Name = 'Font-Language-Region-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '22_Font-Language-Region' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '22_Font-Language-Region'
    ...
    Export-HtmlReport -Title 'Font Language Region Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
