...
<#
.SYNOPSIS
    Common problem fix: Font Language Region Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Font-Language-Region-Policy-Check.ps1
.EXAMPLE
    .\Font-Language-Region-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region Policy Check'; $totalSteps = 8; $Name = 'Font-Language-Region-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Font Language Region Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
