...
<#
.SYNOPSIS
    Enterprise IT tool: Region Format Mismatch Fix
.DESCRIPTION
    Phase 11 expansion (ITSupport/22_Font-Language-Region). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Region-Format-Mismatch-Fix.ps1
.EXAMPLE
    .\Region-Format-Mismatch-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Region Format Mismatch Fix'; $totalSteps=8; $Name='Region-Format-Mismatch-Fix'; $Folder='22_Font-Language-Region'
try {
    ...
    Export-HtmlReport -Title 'Region Format Mismatch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Region Format Mismatch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
