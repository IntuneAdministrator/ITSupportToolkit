...
<#
.SYNOPSIS
    Common problem fix: Font Language Region Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Font-Language-Region-Issue-Detect.ps1
.EXAMPLE
    .\Font-Language-Region-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region Issue Detect'; $totalSteps = 8; $Name = 'Font-Language-Region-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Font Language Region Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
