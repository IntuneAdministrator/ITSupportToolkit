...
<#
.SYNOPSIS
    Common problem fix: Font Language Region Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Font-Language-Region-Reset-Stack.ps1
.EXAMPLE
    .\Font-Language-Region-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region Reset Stack'; $totalSteps = 8; $Name = 'Font-Language-Region-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Font Language Region Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
