...
<#
.SYNOPSIS
    Enterprise IT tool: IME Keyboard Layout Reset
.DESCRIPTION
    Phase 11 expansion (ITSupport/22_Font-Language-Region). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\IME-Keyboard-Layout-Reset.ps1
.EXAMPLE
    .\IME-Keyboard-Layout-Reset.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='IME Keyboard Layout Reset'; $totalSteps=8; $Name='IME-Keyboard-Layout-Reset'; $Folder='22_Font-Language-Region'
try {
    ...
    Export-HtmlReport -Title 'IME Keyboard Layout Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IME Keyboard Layout Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
