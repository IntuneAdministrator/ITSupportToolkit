...
<#
.SYNOPSIS
    Common problem fix: Font Language Region One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Font-Language-Region-One-Click-Fix.ps1
.EXAMPLE
    .\Font-Language-Region-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region One Click Fix'; $totalSteps = 8; $Name = 'Font-Language-Region-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Font Language Region One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
