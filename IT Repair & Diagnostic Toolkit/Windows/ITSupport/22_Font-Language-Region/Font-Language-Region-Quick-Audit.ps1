...
<#
.SYNOPSIS
    Common problem fix: Font Language Region Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Font-Language-Region-Quick-Audit.ps1
.EXAMPLE
    .\Font-Language-Region-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region Quick Audit'; $totalSteps = 8; $Name = 'Font-Language-Region-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Font Language Region Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
