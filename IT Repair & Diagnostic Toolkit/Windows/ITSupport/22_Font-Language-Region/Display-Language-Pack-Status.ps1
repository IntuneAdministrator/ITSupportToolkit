...
<#
.SYNOPSIS
    Enterprise IT tool: Display Language Pack Status
.DESCRIPTION
    Phase 11 expansion (ITSupport/22_Font-Language-Region). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Display-Language-Pack-Status.ps1
.EXAMPLE
    .\Display-Language-Pack-Status.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Display Language Pack Status'; $totalSteps=8; $Name='Display-Language-Pack-Status'; $Folder='22_Font-Language-Region'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '22_Font-Language-Region' -Status 'INFO' -Detail 'ITSupport'
    ...
    Export-HtmlReport -Title 'Display Language Pack Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Display Language Pack Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
