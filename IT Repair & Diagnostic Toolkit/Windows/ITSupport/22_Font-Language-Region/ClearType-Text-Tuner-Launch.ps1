...
<#
.SYNOPSIS
    Enterprise IT tool: ClearType Text Tuner Launch
.DESCRIPTION
    Phase 11 expansion (ITSupport/22_Font-Language-Region). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\ClearType-Text-Tuner-Launch.ps1
.EXAMPLE
    .\ClearType-Text-Tuner-Launch.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='ClearType Text Tuner Launch'; $totalSteps=8; $Name='ClearType-Text-Tuner-Launch'; $Folder='22_Font-Language-Region'
try {
    ...
    Export-HtmlReport -Title 'ClearType Text Tuner Launch' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'ClearType Text Tuner Launch' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
