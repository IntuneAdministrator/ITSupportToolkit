...
<#
.SYNOPSIS
    Common problem fix: Font Language Region User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Font-Language-Region-User-Triage.ps1
.EXAMPLE
    .\Font-Language-Region-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Font Language Region User Triage'; $totalSteps = 8; $Name = 'Font-Language-Region-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Font Language Region User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Font Language Region User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
