...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Shared Mailbox Missing Outlook
.DESCRIPTION
    Phase 12 M365 tenant admin (ITSupport/28_M365-Mailbox-Calendar-HD). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\Shared-Mailbox-Missing-Outlook.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Shared-Mailbox-Missing-Outlook.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Shared Mailbox Missing Outlook'; $totalSteps = 8; $Name = 'Shared-Mailbox-Missing-Outlook'; $Folder = '28_M365-Mailbox-Calendar-HD'
$DefaultScopes = @('User.Read.All','Directory.Read.All')
try {
    ...
    Add-ReportRow -Check 'Module' -Result '28_M365-Mailbox-Calendar-HD' -Status 'INFO' -Detail 'ITSupport Phase12 M365'
    Add-M365PortalRows
    ...
    Export-HtmlReport -Title 'Shared Mailbox Missing Outlook' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shared Mailbox Missing Outlook' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
