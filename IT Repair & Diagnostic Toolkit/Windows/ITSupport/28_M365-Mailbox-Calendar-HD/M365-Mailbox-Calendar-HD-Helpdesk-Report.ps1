...
<#
.SYNOPSIS
    Common problem fix: M365 Mailbox Calendar HD Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 28_M365-Mailbox-Calendar-HD). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\M365-Mailbox-Calendar-HD-Helpdesk-Report.ps1
.EXAMPLE
    .\M365-Mailbox-Calendar-HD-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'M365 Mailbox Calendar HD Helpdesk Report'; $totalSteps = 8; $Name = 'M365-Mailbox-Calendar-HD-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '28_M365-Mailbox-Calendar-HD' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '28_M365-Mailbox-Calendar-HD'
    ...
    Export-HtmlReport -Title 'M365 Mailbox Calendar HD Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'M365 Mailbox Calendar HD Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
