...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Mail Forwarding User Confirm
.DESCRIPTION
...
.EXAMPLE
    .\Mail-Forwarding-User-Confirm.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Mail-Forwarding-User-Confirm.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Mail Forwarding User Confirm'; $totalSteps = 8; $Name = 'Mail-Forwarding-User-Confirm'; $Folder = '28_M365-Mailbox-Calendar-HD'
...
try {
    ...
    Export-HtmlReport -Title 'Mail Forwarding User Confirm' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Mail Forwarding User Confirm' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
