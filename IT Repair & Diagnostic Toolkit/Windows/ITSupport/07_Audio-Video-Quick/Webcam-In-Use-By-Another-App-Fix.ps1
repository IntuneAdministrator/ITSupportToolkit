...
<#
.SYNOPSIS
    Common problem fix: Webcam In Use By Another App Fix
.DESCRIPTION
...
.EXAMPLE
    .\Webcam-In-Use-By-Another-App-Fix
.EXAMPLE
    .\Webcam-In-Use-By-Another-App-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Webcam In Use By Another App Fix'; $totalSteps = 8; $Name = 'Webcam-In-Use-By-Another-App-Fix'
try {
    ...
    Export-HtmlReport -Title 'Webcam In Use By Another App Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Webcam In Use By Another App Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
