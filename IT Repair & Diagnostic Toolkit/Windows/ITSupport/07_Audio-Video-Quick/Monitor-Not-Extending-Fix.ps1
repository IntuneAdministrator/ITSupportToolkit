...
<#
.SYNOPSIS
    Common problem fix: Monitor Not Extending Fix
.DESCRIPTION
...
.EXAMPLE
    .\Monitor-Not-Extending-Fix
.EXAMPLE
    .\Monitor-Not-Extending-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Monitor Not Extending Fix'; $totalSteps = 8; $Name = 'Monitor-Not-Extending-Fix'
try {
    ...
    Export-HtmlReport -Title 'Monitor Not Extending Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Monitor Not Extending Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
