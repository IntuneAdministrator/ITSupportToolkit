...
<#
.SYNOPSIS
    Common problem fix: Headset Not Switching Fix
.DESCRIPTION
...
.EXAMPLE
    .\Headset-Not-Switching-Fix
.EXAMPLE
    .\Headset-Not-Switching-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Headset Not Switching Fix'; $totalSteps = 8; $Name = 'Headset-Not-Switching-Fix'
try {
    ...
    Export-HtmlReport -Title 'Headset Not Switching Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Headset Not Switching Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
