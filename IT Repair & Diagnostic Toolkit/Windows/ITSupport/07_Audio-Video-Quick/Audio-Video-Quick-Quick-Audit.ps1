...
<#
.SYNOPSIS
    Common problem fix: Audio Video Quick Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Quick-Quick-Audit.ps1
.EXAMPLE
    .\Audio-Video-Quick-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Quick Quick Audit'; $totalSteps = 8; $Name = 'Audio-Video-Quick-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Quick Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Quick Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
