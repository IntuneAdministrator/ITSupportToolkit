...
<#
.SYNOPSIS
    Common problem fix: Night Light Greyed Out Fix
.DESCRIPTION
...
.EXAMPLE
    .\Night-Light-Greyed-Out-Fix
.EXAMPLE
    .\Night-Light-Greyed-Out-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Night Light Greyed Out Fix'; $totalSteps = 8; $Name = 'Night-Light-Greyed-Out-Fix'
try {
    ...
    Export-HtmlReport -Title 'Night Light Greyed Out Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Night Light Greyed Out Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
