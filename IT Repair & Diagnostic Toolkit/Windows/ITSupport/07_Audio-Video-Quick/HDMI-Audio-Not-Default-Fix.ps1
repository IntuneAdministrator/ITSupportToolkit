...
<#
.SYNOPSIS
    Common problem fix: HDMI Audio Not Default Fix
.DESCRIPTION
...
.EXAMPLE
    .\HDMI-Audio-Not-Default-Fix
.EXAMPLE
    .\HDMI-Audio-Not-Default-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HDMI Audio Not Default Fix'; $totalSteps = 8; $Name = 'HDMI-Audio-Not-Default-Fix'
try {
    ...
    Export-HtmlReport -Title 'HDMI Audio Not Default Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HDMI Audio Not Default Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
