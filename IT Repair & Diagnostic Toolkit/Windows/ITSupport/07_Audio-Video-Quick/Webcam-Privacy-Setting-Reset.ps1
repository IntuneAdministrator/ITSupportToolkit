...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Webcam Privacy Setting Reset
.DESCRIPTION
    Production-grade Windows IT Support tool for Webcam-Privacy-Setting-Reset.
...
.EXAMPLE
    .\Webcam-Privacy-Setting-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-Privacy-Setting-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam Privacy Setting Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Webcam-Privacy-Setting-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-Privacy-Setting-Reset' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
    ...
    Export-HtmlReport -Title 'Webcam Privacy Setting Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Privacy Setting Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
