...
<#
.SYNOPSIS
    Common problem fix: Webcam Driver Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Webcam-Driver-Missing-Fix
.EXAMPLE
    .\Webcam-Driver-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Webcam Driver Missing Fix'; $totalSteps = 8; $Name = 'Webcam-Driver-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Webcam Driver Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Webcam Driver Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
