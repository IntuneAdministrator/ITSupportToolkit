...
<#
.SYNOPSIS
    Common problem fix: HDR Display Problems
.DESCRIPTION
...
.EXAMPLE
    .\HDR-Display-Problems.ps1
.EXAMPLE
    .\HDR-Display-Problems.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HDR Display Problems'; $totalSteps = 8; $Name = 'HDR-Display-Problems'
try {
    ...
    Export-HtmlReport -Title 'HDR Display Problems' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HDR Display Problems' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
