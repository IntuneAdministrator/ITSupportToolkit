...
<#
.SYNOPSIS
    Common problem fix: Camera Flip Mirror Wrong Fix
.DESCRIPTION
...
.EXAMPLE
    .\Camera-Flip-Mirror-Wrong-Fix
.EXAMPLE
    .\Camera-Flip-Mirror-Wrong-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Camera Flip Mirror Wrong Fix'; $totalSteps = 8; $Name = 'Camera-Flip-Mirror-Wrong-Fix'
try {
    ...
    Export-HtmlReport -Title 'Camera Flip Mirror Wrong Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Camera Flip Mirror Wrong Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
