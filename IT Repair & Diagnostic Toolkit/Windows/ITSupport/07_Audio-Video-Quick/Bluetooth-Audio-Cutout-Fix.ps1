...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Audio Cutout Fix
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Audio-Cutout-Fix.ps1
.EXAMPLE
    .\Bluetooth-Audio-Cutout-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Audio Cutout Fix'; $totalSteps = 8; $Name = 'Bluetooth-Audio-Cutout-Fix'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Audio Cutout Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Audio Cutout Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
