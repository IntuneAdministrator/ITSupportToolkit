...
<#
.SYNOPSIS
    Common problem fix: Audio Video Quick Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Quick-Issue-Detect.ps1
.EXAMPLE
    .\Audio-Video-Quick-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Quick Issue Detect'; $totalSteps = 8; $Name = 'Audio-Video-Quick-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Quick Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Quick Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
