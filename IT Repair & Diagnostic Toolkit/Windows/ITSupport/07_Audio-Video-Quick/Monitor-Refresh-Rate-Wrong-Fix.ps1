...
<#
.SYNOPSIS
    Common problem fix: Monitor Refresh Rate Wrong Fix
.DESCRIPTION
...
.EXAMPLE
    .\Monitor-Refresh-Rate-Wrong-Fix
.EXAMPLE
    .\Monitor-Refresh-Rate-Wrong-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Monitor Refresh Rate Wrong Fix'; $totalSteps = 8; $Name = 'Monitor-Refresh-Rate-Wrong-Fix'
try {
    ...
    Export-HtmlReport -Title 'Monitor Refresh Rate Wrong Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Monitor Refresh Rate Wrong Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
