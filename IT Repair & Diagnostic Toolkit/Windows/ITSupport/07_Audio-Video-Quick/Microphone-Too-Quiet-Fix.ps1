...
<#
.SYNOPSIS
    Common problem fix: Microphone Too Quiet Fix
.DESCRIPTION
...
.EXAMPLE
    .\Microphone-Too-Quiet-Fix
.EXAMPLE
    .\Microphone-Too-Quiet-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microphone Too Quiet Fix'; $totalSteps = 8; $Name = 'Microphone-Too-Quiet-Fix'
try {
    ...
    Export-HtmlReport -Title 'Microphone Too Quiet Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microphone Too Quiet Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
