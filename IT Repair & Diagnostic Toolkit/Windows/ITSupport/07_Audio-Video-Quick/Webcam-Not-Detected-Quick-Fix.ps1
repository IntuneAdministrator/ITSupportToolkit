...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Webcam Not Detected Quick Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Webcam-Not-Detected-Quick-Fix.
Category: 07_Audio-Video-Quick. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Webcam-Not-Detected-Quick-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-Not-Detected-Quick-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam Not Detected Quick Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO VIDEO QUICK'
        $Name = 'Webcam-Not-Detected-Quick-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-Not-Detected-Quick-Fix' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
        if ($Name -match 'No-Sound|Mic-Not|Audio') {
            foreach ($s in @('Audiosrv','AudioEndpointBuilder')) {
                if ($PSCmdlet.ShouldProcess($s, 'Restart')) {
                    ...
                    Add-ReportRow -Check $s -Result (Get-Service $s).Status -Status 'PASS' -Detail ''
                }
            }
        }
        if ($Name -match 'Mic|Privacy|Webcam') {
            foreach ($cap in @('microphone','webcam')) {
                $key = "HKCU:\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\$cap"
                if (Test-Path $key) {
                    Backup-RegistryKey -KeyPath $key -BackupDir 'C:\IT-Logs\Backups' | Out-Null
                    if ($PSCmdlet.ShouldProcess($cap, 'Set Allow')) {
                        Set-ItemProperty $key -Name Value -Value 'Allow' -ErrorAction SilentlyContinue
                        Add-ReportRow -Check "$cap consent" -Result 'Allow' -Status 'PASS' -Detail $key
                    }
                } else {
                    Add-ReportRow -Check "$cap consent" -Result 'Key missing' -Status 'WARN' -Detail $key
                }
            }
        }
        if ($Name -match 'Screen-Resolution|Second-Monitor|Monitor') {
            try {
                Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue
                [System.Windows.Forms.Screen]::AllScreens | ForEach-Object {
                    Add-ReportRow -Check $(if ($_.Primary) {'Primary'} else {'Monitor'}) -Result "$($_.Bounds.Width)x$($_.Bounds.Height)" -Status 'INFO' -Detail ''
                }
            } catch {
                Get-CimInstance Win32_VideoController | ForEach-Object {
                    Add-ReportRow -Check $_.Name -Result $_.VideoModeDescription -Status 'INFO' -Detail $_.Status
                }
            }
            if ($PSCmdlet.ShouldProcess('Display', 'Restart Display Policy service')) {
                Restart-Service DispBrokerDesktopSvc -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'DispBrokerDesktopSvc' -Result 'Restarted' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Webcam-Not-Detected|Camera|Teams-Zoom|Virtual-Background|GPU') {
            $cams = @(Get-PnpDevice -Class Camera,Image -EA SilentlyContinue)
            foreach ($c in $cams) {
                Add-ReportRow -Check $c.FriendlyName -Result $c.Status -Status $(if ($c.Status -eq 'OK') {'PASS'} else {'WARN'}) -Detail $c.InstanceId
                if ($c.Status -ne 'OK' -and $PSCmdlet.ShouldProcess($c.FriendlyName, 'Re-enumerate')) {
                    Disable-PnpDevice -InstanceId $c.InstanceId -Confirm:$false -EA SilentlyContinue
                    Start-Sleep -Seconds 1
                    Enable-PnpDevice -InstanceId $c.InstanceId -Confirm:$false -EA SilentlyContinue
                    Add-ReportRow -Check "Reset $($c.FriendlyName)" -Result 'Toggled' -Status 'PASS' -Detail ''
                }
            }
            if ($Name -match 'GPU|Virtual-Background') {
                Get-CimInstance Win32_VideoController | ForEach-Object {
                    Add-ReportRow -Check "GPU $($_.Name)" -Result $_.Status -Status $(if ($_.Status -eq 'OK') {'PASS'} else {'WARN'}) -Detail $_.DriverVersion
                }
            }
            if ($Name -match 'Teams-Zoom') {
                Get-Process -Name Teams,ms-teams,Zoom -EA SilentlyContinue | ForEach-Object {
                    Add-ReportRow -Check "App $($_.ProcessName)" -Result "PID $($_.Id)" -Status 'INFO' -Detail 'May hold camera lock'
                }
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'A/V quick'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Webcam Not Detected Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Not Detected Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
