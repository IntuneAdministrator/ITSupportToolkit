...
<#
.SYNOPSIS
    Common problem fix: Audio Video Quick One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Quick-One-Click-Fix.ps1
.EXAMPLE
    .\Audio-Video-Quick-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Quick One Click Fix'; $totalSteps = 8; $Name = 'Audio-Video-Quick-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Quick One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Quick One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
