...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Second Monitor Not Detected
.DESCRIPTION
    Production-grade Windows IT Support tool for Second-Monitor-Not-Detected.
...
.EXAMPLE
    .\Second-Monitor-Not-Detected.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Second-Monitor-Not-Detected.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Second Monitor Not Detected'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Second-Monitor-Not-Detected'
        Add-ReportRow -Check 'Tool' -Result 'Second-Monitor-Not-Detected' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
    ...
    Export-HtmlReport -Title 'Second Monitor Not Detected' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Second Monitor Not Detected' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
