...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Virtual Background GPU Check
.DESCRIPTION
    Production-grade Windows IT Support tool for Virtual-Background-GPU-Check.
...
.EXAMPLE
    .\Virtual-Background-GPU-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Virtual-Background-GPU-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Virtual Background GPU Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Virtual-Background-GPU-Check'
        Add-ReportRow -Check 'Tool' -Result 'Virtual-Background-GPU-Check' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
    ...
    Export-HtmlReport -Title 'Virtual Background GPU Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Virtual Background GPU Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
