...
<#
.SYNOPSIS
    Common problem fix: Conference Room Camera Fix
.DESCRIPTION
...
.EXAMPLE
    .\Conference-Room-Camera-Fix.ps1
.EXAMPLE
    .\Conference-Room-Camera-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Conference Room Camera Fix'; $totalSteps = 8; $Name = 'Conference-Room-Camera-Fix'
try {
    ...
    Export-HtmlReport -Title 'Conference Room Camera Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Conference Room Camera Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
