...
<#
.SYNOPSIS
    Common problem fix: Audio Video Quick Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Quick-Policy-Check.ps1
.EXAMPLE
    .\Audio-Video-Quick-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Quick Policy Check'; $totalSteps = 8; $Name = 'Audio-Video-Quick-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Quick Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Quick Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
