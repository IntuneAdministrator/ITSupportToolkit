...
<#
.SYNOPSIS
    Common problem fix: Wireless Headset Dongle Pair Fix
.DESCRIPTION
...
.EXAMPLE
    .\Wireless-Headset-Dongle-Pair-Fix.ps1
.EXAMPLE
    .\Wireless-Headset-Dongle-Pair-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Wireless Headset Dongle Pair Fix'; $totalSteps = 8; $Name = 'Wireless-Headset-Dongle-Pair-Fix'
try {
    ...
    Export-HtmlReport -Title 'Wireless Headset Dongle Pair Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Wireless Headset Dongle Pair Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
