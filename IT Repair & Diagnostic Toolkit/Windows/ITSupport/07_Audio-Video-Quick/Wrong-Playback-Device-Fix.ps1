...
<#
.SYNOPSIS
    Common problem fix: Wrong Playback Device Fix
.DESCRIPTION
...
.EXAMPLE
    .\Wrong-Playback-Device-Fix
.EXAMPLE
    .\Wrong-Playback-Device-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Wrong Playback Device Fix'; $totalSteps = 8; $Name = 'Wrong-Playback-Device-Fix'
try {
    ...
    Export-HtmlReport -Title 'Wrong Playback Device Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Wrong Playback Device Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
