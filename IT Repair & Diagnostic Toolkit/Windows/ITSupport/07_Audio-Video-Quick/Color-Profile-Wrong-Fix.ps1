...
<#
.SYNOPSIS
    Common problem fix: Color Profile Wrong Fix
.DESCRIPTION
...
.EXAMPLE
    .\Color-Profile-Wrong-Fix
.EXAMPLE
    .\Color-Profile-Wrong-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Color Profile Wrong Fix'; $totalSteps = 8; $Name = 'Color-Profile-Wrong-Fix'
try {
    ...
    Export-HtmlReport -Title 'Color Profile Wrong Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Color Profile Wrong Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
