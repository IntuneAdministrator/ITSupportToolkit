...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Screen Resolution Reset
.DESCRIPTION
    Production-grade Windows IT Support tool for Screen-Resolution-Reset.
...
.EXAMPLE
    .\Screen-Resolution-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Screen-Resolution-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Screen Resolution Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Screen-Resolution-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Screen-Resolution-Reset' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
    ...
    Export-HtmlReport -Title 'Screen Resolution Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Screen Resolution Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
