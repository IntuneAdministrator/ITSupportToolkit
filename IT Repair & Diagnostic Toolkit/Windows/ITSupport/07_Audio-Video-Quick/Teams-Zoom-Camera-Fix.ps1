...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Teams Zoom Camera Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Teams-Zoom-Camera-Fix.
...
$exitCode = 0
$Activity = 'Teams Zoom Camera Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Teams-Zoom-Camera-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Teams-Zoom-Camera-Fix' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
    ...
    Export-HtmlReport -Title 'Teams Zoom Camera Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams Zoom Camera Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
