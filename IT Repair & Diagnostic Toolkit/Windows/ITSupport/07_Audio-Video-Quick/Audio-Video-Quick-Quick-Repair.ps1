...
<#
.SYNOPSIS
    Common problem fix: Audio Video Quick Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Quick-Quick-Repair.ps1
.EXAMPLE
    .\Audio-Video-Quick-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Quick Quick Repair'; $totalSteps = 8; $Name = 'Audio-Video-Quick-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Quick Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Quick Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
