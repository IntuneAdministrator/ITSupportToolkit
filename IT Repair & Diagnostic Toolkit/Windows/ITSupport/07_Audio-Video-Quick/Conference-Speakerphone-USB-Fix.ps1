...
<#
.SYNOPSIS
    Common problem fix: Conference Speakerphone USB Fix
.DESCRIPTION
...
.EXAMPLE
    .\Conference-Speakerphone-USB-Fix.ps1
.EXAMPLE
    .\Conference-Speakerphone-USB-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Conference Speakerphone USB Fix'; $totalSteps = 8; $Name = 'Conference-Speakerphone-USB-Fix'
try {
    ...
    Export-HtmlReport -Title 'Conference Speakerphone USB Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Conference Speakerphone USB Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
