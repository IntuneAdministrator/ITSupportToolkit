...
<#
.SYNOPSIS
    Common problem fix: Screen Flickering Fix
.DESCRIPTION
...
.EXAMPLE
    .\Screen-Flickering-Fix.ps1
.EXAMPLE
    .\Screen-Flickering-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Screen Flickering Fix'; $totalSteps = 8; $Name = 'Screen-Flickering-Fix'
try {
    ...
    Export-HtmlReport -Title 'Screen Flickering Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Screen Flickering Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
