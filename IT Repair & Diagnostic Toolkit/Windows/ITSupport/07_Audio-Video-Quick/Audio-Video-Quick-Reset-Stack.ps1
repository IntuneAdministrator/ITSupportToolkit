...
<#
.SYNOPSIS
    Common problem fix: Audio Video Quick Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Quick-Reset-Stack.ps1
.EXAMPLE
    .\Audio-Video-Quick-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Quick Reset Stack'; $totalSteps = 8; $Name = 'Audio-Video-Quick-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Quick Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Quick Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
