...
<#
.SYNOPSIS
    Common problem fix: Auto Brightness Stuck Fix
.DESCRIPTION
...
.EXAMPLE
    .\Auto-Brightness-Stuck-Fix
.EXAMPLE
    .\Auto-Brightness-Stuck-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Auto Brightness Stuck Fix'; $totalSteps = 8; $Name = 'Auto-Brightness-Stuck-Fix'
try {
    ...
    Export-HtmlReport -Title 'Auto Brightness Stuck Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Auto Brightness Stuck Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
