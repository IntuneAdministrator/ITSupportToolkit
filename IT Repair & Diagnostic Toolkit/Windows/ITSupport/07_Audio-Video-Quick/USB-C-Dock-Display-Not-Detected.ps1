...
<#
.SYNOPSIS
    Common problem fix: USB C Dock Display Not Detected
.DESCRIPTION
...
.EXAMPLE
    .\USB-C-Dock-Display-Not-Detected.ps1
.EXAMPLE
    .\USB-C-Dock-Display-Not-Detected.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB C Dock Display Not Detected'; $totalSteps = 8; $Name = 'USB-C-Dock-Display-Not-Detected'
try {
    ...
    Export-HtmlReport -Title 'USB C Dock Display Not Detected' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB C Dock Display Not Detected' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
