...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Mic Not Working Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Mic-Not-Working-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Mic-Not-Working-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Mic-Not-Working-Fix' -Status 'INFO' -Detail 'ITSupport/07_Audio-Video-Quick'
        Write-SectionBanner 'AUDIO / VIDEO QUICK'
    ...
    Export-HtmlReport -Title 'Mic Not Working Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Mic Not Working Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
