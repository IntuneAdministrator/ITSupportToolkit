...
<#
.SYNOPSIS
    Common problem fix: Remote Tools User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-User-Triage.ps1
.EXAMPLE
    .\Remote-Tools-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools User Triage'; $totalSteps = 8; $Name = 'Remote-Tools-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
