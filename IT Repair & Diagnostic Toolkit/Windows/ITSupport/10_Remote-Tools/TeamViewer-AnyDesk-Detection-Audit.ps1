...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: TeamViewer AnyDesk Detection Audit
.DESCRIPTION
    Production-grade Windows IT Support tool for TeamViewer-AnyDesk-Detection-Audit.
Category: 10_Remote-Tools. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\TeamViewer-AnyDesk-Detection-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\TeamViewer-AnyDesk-Detection-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'TeamViewer AnyDesk Detection Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE TOOLS'
        $Name = 'TeamViewer-AnyDesk-Detection-Audit'
        Add-ReportRow -Check 'Tool' -Result 'TeamViewer-AnyDesk-Detection-Audit' -Status 'INFO' -Detail 'ITSupport/10_Remote-Tools'
        Write-SectionBanner 'REMOTE SUPPORT TOOLS'
        if ($Name -match 'Enable-Remote-Support|MSRA|Remote-Assistance') {
            Backup-RegistryKey -KeyPath 'HKLM:\SYSTEM\CurrentControlSet\Control\Remote Assistance' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            if ($PSCmdlet.ShouldProcess('Remote Assistance', 'Enable fAllowToGetHelp')) {
                Set-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Remote Assistance' -Name fAllowToGetHelp -Value 1 -Type DWord -Force
                Enable-NetFirewallRule -DisplayGroup 'Remote Assistance' -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Remote Assistance' -Result 'Enabled' -Status 'PASS' -Detail 'Firewall group enabled'
            }
        }
        if ($Name -match 'Quick-Assist') {
            $qa = Get-AppxPackage *QuickAssist* -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Quick Assist package' -Result $(if ($qa) { $qa.Version } else {'Not installed'}) -Status $(if ($qa) {'PASS'} else {'WARN'}) -Detail ''
            if (-not $qa -and $PSCmdlet.ShouldProcess('Quick Assist', 'Try winget install')) {
                if (Get-Command winget -EA SilentlyContinue) {
                    & winget install 9P7BP5VNWKX5 --accept-package-agreements --accept-source-agreements 2>&1 | Out-Null
                    Add-ReportRow -Check 'winget Quick Assist' -Result 'Install attempted' -Status 'INFO' -Detail '9P7BP5VNWKX5'
                }
            }
        }
        if ($Name -match 'Remote-Registry') {
            if ($PSCmdlet.ShouldProcess('RemoteRegistry', 'Enable and start')) {
                Set-Service RemoteRegistry -StartupType Manual -ErrorAction SilentlyContinue
                Start-Service RemoteRegistry -ErrorAction SilentlyContinue
                $s = Get-Service RemoteRegistry
                Add-ReportRow -Check 'RemoteRegistry' -Result "$($s.Status)/$($s.StartType)" -Status $(if ($s.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail ''
            }
            if ($Name -match 'Permission') {
                Add-ReportRow -Check 'Registry permissions' -Result 'Ensure winreg pipe ACL / firewall allow Remote Service Management' -Status 'INFO' -Detail ''
                Enable-NetFirewallRule -DisplayGroup 'Remote Service Management' -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Firewall Remote Service Management' -Result 'Enabled' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Screen-Share-Prep') {
            if ($PSCmdlet.ShouldProcess('RDP/UI', 'Prep for screen share')) {
                ...
                Add-ReportRow -Check 'RDP enable (optional)' -Result 'Allowed' -Status 'INFO' -Detail 'fDenyTSConnections=0'
                Add-ReportRow -Check 'UI tips' -Result 'Close privacy screens / enable notifications as needed' -Status 'INFO' -Detail ''
            }
        }
        if ($Name -match 'TeamViewer|AnyDesk|Detection') {
            $targets = @{
            ...
            foreach ($k in $targets.Keys) {
                $procs = @(Get-Process -Name ($targets[$k] -replace '\*','') -EA SilentlyContinue)
                if ($k -match 'Quick') { $procs = @(Get-Process | Where-Object ProcessName -match 'QuickAssist') }
                $pkg = Get-Package -Name $targets[$k] -EA SilentlyContinue
                $unin = Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*, HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* -EA SilentlyContinue |
                    Where-Object { $_.DisplayName -match ($k -replace ' ','|') }
                Add-ReportRow -Check "$k detection" -Result "Proc=$(@($procs).Count) Uninstall=$(@($unin).Count)" -Status 'INFO' -Detail (($unin | Select-Object -First 3 DisplayName) -join '; ')
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Remote tools'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'TeamViewer AnyDesk Detection Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'TeamViewer AnyDesk Detection Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
