...
<#
.SYNOPSIS
    Common problem fix: Remote Tools Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-Policy-Check.ps1
.EXAMPLE
    .\Remote-Tools-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools Policy Check'; $totalSteps = 8; $Name = 'Remote-Tools-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
