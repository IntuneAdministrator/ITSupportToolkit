...
<#
.SYNOPSIS
    Common problem fix: Remote Tools One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-One-Click-Fix.ps1
.EXAMPLE
    .\Remote-Tools-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools One Click Fix'; $totalSteps = 8; $Name = 'Remote-Tools-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
