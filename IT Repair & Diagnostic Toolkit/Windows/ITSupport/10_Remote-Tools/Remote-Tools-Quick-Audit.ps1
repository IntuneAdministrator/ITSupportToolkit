...
<#
.SYNOPSIS
    Common problem fix: Remote Tools Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-Quick-Audit.ps1
.EXAMPLE
    .\Remote-Tools-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools Quick Audit'; $totalSteps = 8; $Name = 'Remote-Tools-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
