...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: MSRA Remote Assistance Enable
.DESCRIPTION
    Production-grade Windows IT Support tool for MSRA-Remote-Assistance-Enable.
...
.EXAMPLE
    .\MSRA-Remote-Assistance-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\MSRA-Remote-Assistance-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'MSRA Remote Assistance Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'MSRA-Remote-Assistance-Enable'
        Add-ReportRow -Check 'Tool' -Result 'MSRA-Remote-Assistance-Enable' -Status 'INFO' -Detail 'ITSupport/10_Remote-Tools'
    ...
    Export-HtmlReport -Title 'MSRA Remote Assistance Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'MSRA Remote Assistance Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
