...
<#
.SYNOPSIS
    Common problem fix: Remote Tools Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-Quick-Repair.ps1
.EXAMPLE
    .\Remote-Tools-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools Quick Repair'; $totalSteps = 8; $Name = 'Remote-Tools-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
