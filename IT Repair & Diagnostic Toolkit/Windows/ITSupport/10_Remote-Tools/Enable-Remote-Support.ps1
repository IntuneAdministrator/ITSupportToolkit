...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Enable Remote Support
.DESCRIPTION
    Production-grade Windows IT Support tool for Enable-Remote-Support.
...
$exitCode = 0
$Activity = 'Enable Remote Support'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Enable-Remote-Support' -Status 'INFO' -Detail 'ITSupport/10_Remote-Tools'
    ...
    Export-HtmlReport -Title 'Enable Remote Support' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Enable Remote Support' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
