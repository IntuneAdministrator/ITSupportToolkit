...
<#
.SYNOPSIS
    Common problem fix: Remote Tools Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-Issue-Detect.ps1
.EXAMPLE
    .\Remote-Tools-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools Issue Detect'; $totalSteps = 8; $Name = 'Remote-Tools-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
