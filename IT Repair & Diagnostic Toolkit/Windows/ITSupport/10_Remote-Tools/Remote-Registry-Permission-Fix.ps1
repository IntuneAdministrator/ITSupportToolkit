...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Remote Registry Permission Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Remote-Registry-Permission-Fix.
...
.EXAMPLE
    .\Remote-Registry-Permission-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Remote-Registry-Permission-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Remote Registry Permission Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Remote-Registry-Permission-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Remote-Registry-Permission-Fix' -Status 'INFO' -Detail 'ITSupport/10_Remote-Tools'
    ...
    Export-HtmlReport -Title 'Remote Registry Permission Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Registry Permission Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
