...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Remote Registry Enable
.DESCRIPTION
    Production-grade Windows IT Support tool for Remote-Registry-Enable.
...
$exitCode = 0
$Activity = 'Remote Registry Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Remote-Registry-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Remote-Registry-Enable' -Status 'INFO' -Detail 'ITSupport/10_Remote-Tools'
    ...
    Export-HtmlReport -Title 'Remote Registry Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Registry Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
