...
<#
.SYNOPSIS
    Common problem fix: Remote Tools Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Tools-Reset-Stack.ps1
.EXAMPLE
    .\Remote-Tools-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools Reset Stack'; $totalSteps = 8; $Name = 'Remote-Tools-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Remote Tools Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
