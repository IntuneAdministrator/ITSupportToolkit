...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Quick Assist Enable
.DESCRIPTION
    Production-grade Windows IT Support tool for Quick-Assist-Enable.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Quick-Assist-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Quick-Assist-Enable' -Status 'INFO' -Detail 'ITSupport/10_Remote-Tools'
    ...
    Export-HtmlReport -Title 'Quick Assist Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Quick Assist Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
