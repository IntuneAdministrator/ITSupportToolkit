...
<#
.SYNOPSIS
    Common problem fix: Remote Tools Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 10_Remote-Tools). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Remote-Tools-Helpdesk-Report.ps1
.EXAMPLE
    .\Remote-Tools-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Tools Helpdesk Report'; $totalSteps = 8; $Name = 'Remote-Tools-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '10_Remote-Tools' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '10_Remote-Tools'
    ...
    Export-HtmlReport -Title 'Remote Tools Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Tools Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
