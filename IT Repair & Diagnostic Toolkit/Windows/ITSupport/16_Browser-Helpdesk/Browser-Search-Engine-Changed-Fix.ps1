...
<#
.SYNOPSIS
    Common problem fix: Browser Search Engine Changed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Search-Engine-Changed-Fix
.EXAMPLE
    .\Browser-Search-Engine-Changed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Search Engine Changed Fix'; $totalSteps = 8; $Name = 'Browser-Search-Engine-Changed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Search Engine Changed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Search Engine Changed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
