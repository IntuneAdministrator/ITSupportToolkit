...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 16_Browser-Helpdesk). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Browser-Helpdesk-Helpdesk-Report.ps1
.EXAMPLE
    .\Browser-Helpdesk-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk Helpdesk Report'; $totalSteps = 8; $Name = 'Browser-Helpdesk-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '16_Browser-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '16_Browser-Helpdesk'
    ...
    Export-HtmlReport -Title 'Browser Helpdesk Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
