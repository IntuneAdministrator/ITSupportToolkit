...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-One-Click-Fix.ps1
.EXAMPLE
    .\Browser-Helpdesk-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk One Click Fix'; $totalSteps = 8; $Name = 'Browser-Helpdesk-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
