...
<#
.SYNOPSIS
    Enterprise IT tool: Chrome Wont Open Quick
.DESCRIPTION
    Phase 10 expansion (ITSupport/16_Browser-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Chrome-Wont-Open-Quick.ps1
.EXAMPLE
    .\Chrome-Wont-Open-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Chrome Wont Open Quick'; $totalSteps=8; $Name='Chrome-Wont-Open-Quick'; $Folder='16_Browser-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Chrome Wont Open Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chrome Wont Open Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
