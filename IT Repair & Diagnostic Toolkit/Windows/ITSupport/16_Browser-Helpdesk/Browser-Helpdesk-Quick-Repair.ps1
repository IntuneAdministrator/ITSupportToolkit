...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-Quick-Repair.ps1
.EXAMPLE
    .\Browser-Helpdesk-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk Quick Repair'; $totalSteps = 8; $Name = 'Browser-Helpdesk-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
