...
<#
.SYNOPSIS
    Enterprise IT tool: Extension Crash Disable Hint
.DESCRIPTION
    Phase 10 expansion (ITSupport/16_Browser-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Extension-Crash-Disable-Hint.ps1
.EXAMPLE
    .\Extension-Crash-Disable-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Extension Crash Disable Hint'; $totalSteps=8; $Name='Extension-Crash-Disable-Hint'; $Folder='16_Browser-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Extension Crash Disable Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Extension Crash Disable Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
