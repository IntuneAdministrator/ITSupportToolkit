...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-User-Triage.ps1
.EXAMPLE
    .\Browser-Helpdesk-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk User Triage'; $totalSteps = 8; $Name = 'Browser-Helpdesk-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
