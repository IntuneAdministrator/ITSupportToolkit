...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-Policy-Check.ps1
.EXAMPLE
    .\Browser-Helpdesk-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk Policy Check'; $totalSteps = 8; $Name = 'Browser-Helpdesk-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
