...
<#
.SYNOPSIS
    Common problem fix: Browser HSTS Block Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-HSTS-Block-Fix
.EXAMPLE
    .\Browser-HSTS-Block-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser HSTS Block Fix'; $totalSteps = 8; $Name = 'Browser-HSTS-Block-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser HSTS Block Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser HSTS Block Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
