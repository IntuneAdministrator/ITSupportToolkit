...
<#
.SYNOPSIS
    Common problem fix: Chrome ERR CONNECTION Fix
.DESCRIPTION
...
.EXAMPLE
    .\Chrome-ERR-CONNECTION-Fix
.EXAMPLE
    .\Chrome-ERR-CONNECTION-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Chrome ERR CONNECTION Fix'; $totalSteps = 8; $Name = 'Chrome-ERR-CONNECTION-Fix'
try {
    ...
    Export-HtmlReport -Title 'Chrome ERR CONNECTION Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chrome ERR CONNECTION Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
