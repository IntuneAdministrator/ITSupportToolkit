...
<#
.SYNOPSIS
    Enterprise IT tool: Default Browser Reset Quick
.DESCRIPTION
    Phase 10 expansion (ITSupport/16_Browser-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Default-Browser-Reset-Quick.ps1
.EXAMPLE
    .\Default-Browser-Reset-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Default Browser Reset Quick'; $totalSteps=8; $Name='Default-Browser-Reset-Quick'; $Folder='16_Browser-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Default Browser Reset Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Default Browser Reset Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
