...
<#
.SYNOPSIS
    Common problem fix: Edge Wont Start Fix
.DESCRIPTION
...
$exitCode = 0; $Activity = 'Edge Wont Start Fix'; $totalSteps = 8; $Name = 'Edge-Wont-Start-Fix'
try {
    ...
    Export-HtmlReport -Title 'Edge Wont Start Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge Wont Start Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
