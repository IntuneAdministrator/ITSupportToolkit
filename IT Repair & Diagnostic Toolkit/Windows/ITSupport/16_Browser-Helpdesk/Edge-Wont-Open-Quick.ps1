...
$exitCode=0; $Activity='Edge Wont Open Quick'; $totalSteps=8; $Name='Edge-Wont-Open-Quick'; $Folder='16_Browser-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Edge Wont Open Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge Wont Open Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
