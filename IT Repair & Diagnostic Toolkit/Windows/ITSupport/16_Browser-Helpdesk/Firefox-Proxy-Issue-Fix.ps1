...
<#
.SYNOPSIS
    Common problem fix: Firefox Proxy Issue Fix
.DESCRIPTION
...
.EXAMPLE
    .\Firefox-Proxy-Issue-Fix
.EXAMPLE
    .\Firefox-Proxy-Issue-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Firefox Proxy Issue Fix'; $totalSteps = 8; $Name = 'Firefox-Proxy-Issue-Fix'
try {
    ...
    Export-HtmlReport -Title 'Firefox Proxy Issue Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Firefox Proxy Issue Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
