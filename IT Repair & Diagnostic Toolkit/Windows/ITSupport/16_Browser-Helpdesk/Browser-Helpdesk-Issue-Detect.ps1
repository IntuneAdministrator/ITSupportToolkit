...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-Issue-Detect.ps1
.EXAMPLE
    .\Browser-Helpdesk-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk Issue Detect'; $totalSteps = 8; $Name = 'Browser-Helpdesk-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
