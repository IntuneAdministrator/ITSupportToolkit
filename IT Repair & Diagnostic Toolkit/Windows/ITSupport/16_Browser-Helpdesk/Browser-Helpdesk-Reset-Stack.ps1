...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-Reset-Stack.ps1
.EXAMPLE
    .\Browser-Helpdesk-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk Reset Stack'; $totalSteps = 8; $Name = 'Browser-Helpdesk-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
