...
<#
.SYNOPSIS
    Common problem fix: Browser Homepage Hijacked Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Homepage-Hijacked-Fix
.EXAMPLE
    .\Browser-Homepage-Hijacked-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Homepage Hijacked Fix'; $totalSteps = 8; $Name = 'Browser-Homepage-Hijacked-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Homepage Hijacked Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Homepage Hijacked Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
