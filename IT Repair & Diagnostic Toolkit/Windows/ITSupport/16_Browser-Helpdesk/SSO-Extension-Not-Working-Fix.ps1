...
<#
.SYNOPSIS
    Common problem fix: SSO Extension Not Working Fix
.DESCRIPTION
...
.EXAMPLE
    .\SSO-Extension-Not-Working-Fix
.EXAMPLE
    .\SSO-Extension-Not-Working-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SSO Extension Not Working Fix'; $totalSteps = 8; $Name = 'SSO-Extension-Not-Working-Fix'
try {
    ...
    Export-HtmlReport -Title 'SSO Extension Not Working Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SSO Extension Not Working Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
