...
<#
.SYNOPSIS
    Common problem fix: Edge Extensions Crash Fix
.DESCRIPTION
...
.EXAMPLE
    .\Edge-Extensions-Crash-Fix
.EXAMPLE
    .\Edge-Extensions-Crash-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Edge Extensions Crash Fix'; $totalSteps = 8; $Name = 'Edge-Extensions-Crash-Fix'
try {
    ...
    Export-HtmlReport -Title 'Edge Extensions Crash Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge Extensions Crash Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
