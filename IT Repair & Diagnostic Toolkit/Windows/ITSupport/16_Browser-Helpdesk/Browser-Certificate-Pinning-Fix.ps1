...
<#
.SYNOPSIS
    Common problem fix: Browser Certificate Pinning Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Certificate-Pinning-Fix
.EXAMPLE
    .\Browser-Certificate-Pinning-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Certificate Pinning Fix'; $totalSteps = 8; $Name = 'Browser-Certificate-Pinning-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Certificate Pinning Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Certificate Pinning Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
