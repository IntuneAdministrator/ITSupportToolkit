...
<#
.SYNOPSIS
    Common problem fix: Edge Profile Corrupt Repair
.DESCRIPTION
...
.EXAMPLE
    .\Edge-Profile-Corrupt-Repair.ps1
.EXAMPLE
    .\Edge-Profile-Corrupt-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Edge Profile Corrupt Repair'; $totalSteps = 8; $Name = 'Edge-Profile-Corrupt-Repair'
try {
    ...
    Export-HtmlReport -Title 'Edge Profile Corrupt Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge Profile Corrupt Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
