...
<#
.SYNOPSIS
    Common problem fix: Chrome Sync Paused Fix
.DESCRIPTION
...
.EXAMPLE
    .\Chrome-Sync-Paused-Fix.ps1
.EXAMPLE
    .\Chrome-Sync-Paused-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Chrome Sync Paused Fix'; $totalSteps = 8; $Name = 'Chrome-Sync-Paused-Fix'
try {
    ...
    Export-HtmlReport -Title 'Chrome Sync Paused Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chrome Sync Paused Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
