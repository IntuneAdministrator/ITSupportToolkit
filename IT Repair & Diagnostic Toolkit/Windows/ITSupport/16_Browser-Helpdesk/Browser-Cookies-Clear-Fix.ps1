...
<#
.SYNOPSIS
    Common problem fix: Browser Cookies Clear Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Cookies-Clear-Fix
.EXAMPLE
    .\Browser-Cookies-Clear-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Cookies Clear Fix'; $totalSteps = 8; $Name = 'Browser-Cookies-Clear-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Cookies Clear Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Cookies Clear Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
