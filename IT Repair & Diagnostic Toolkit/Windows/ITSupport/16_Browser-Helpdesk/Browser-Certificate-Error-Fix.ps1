...
<#
.SYNOPSIS
    Common problem fix: Browser Certificate Error Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Certificate-Error-Fix.ps1
.EXAMPLE
    .\Browser-Certificate-Error-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Certificate Error Fix'; $totalSteps = 8; $Name = 'Browser-Certificate-Error-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Certificate Error Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Certificate Error Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
