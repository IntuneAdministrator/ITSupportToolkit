...
<#
.SYNOPSIS
    Common problem fix: Browser Helpdesk Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Helpdesk-Quick-Audit.ps1
.EXAMPLE
    .\Browser-Helpdesk-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Helpdesk Quick Audit'; $totalSteps = 8; $Name = 'Browser-Helpdesk-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Browser Helpdesk Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Helpdesk Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
