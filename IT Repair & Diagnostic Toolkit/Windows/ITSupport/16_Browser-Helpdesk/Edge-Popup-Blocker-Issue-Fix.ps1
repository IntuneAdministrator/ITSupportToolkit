...
<#
.SYNOPSIS
    Common problem fix: Edge Popup Blocker Issue Fix
.DESCRIPTION
...
.EXAMPLE
    .\Edge-Popup-Blocker-Issue-Fix
.EXAMPLE
    .\Edge-Popup-Blocker-Issue-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Edge Popup Blocker Issue Fix'; $totalSteps = 8; $Name = 'Edge-Popup-Blocker-Issue-Fix'
try {
    ...
    Export-HtmlReport -Title 'Edge Popup Blocker Issue Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge Popup Blocker Issue Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
