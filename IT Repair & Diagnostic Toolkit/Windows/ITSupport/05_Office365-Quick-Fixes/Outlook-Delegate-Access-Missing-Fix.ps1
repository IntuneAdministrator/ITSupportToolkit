...
<#
.SYNOPSIS
    Common problem fix: Outlook Delegate Access Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Delegate-Access-Missing-Fix
.EXAMPLE
    .\Outlook-Delegate-Access-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Delegate Access Missing Fix'; $totalSteps = 8; $Name = 'Outlook-Delegate-Access-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Delegate Access Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Delegate Access Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
