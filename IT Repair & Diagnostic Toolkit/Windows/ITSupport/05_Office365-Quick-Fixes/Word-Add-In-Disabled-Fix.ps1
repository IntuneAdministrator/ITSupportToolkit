...
<#
.SYNOPSIS
    Common problem fix: Word Add In Disabled Fix
.DESCRIPTION
...
.EXAMPLE
    .\Word-Add-In-Disabled-Fix.ps1
.EXAMPLE
    .\Word-Add-In-Disabled-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Word Add In Disabled Fix'; $totalSteps = 8; $Name = 'Word-Add-In-Disabled-Fix'
try {
    ...
    Export-HtmlReport -Title 'Word Add In Disabled Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Word Add In Disabled Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
