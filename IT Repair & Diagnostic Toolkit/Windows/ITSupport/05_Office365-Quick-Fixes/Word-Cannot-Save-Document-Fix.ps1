...
<#
.SYNOPSIS
    Common problem fix: Word Cannot Save Document Fix
.DESCRIPTION
...
.EXAMPLE
    .\Word-Cannot-Save-Document-Fix
.EXAMPLE
    .\Word-Cannot-Save-Document-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Word Cannot Save Document Fix'; $totalSteps = 8; $Name = 'Word-Cannot-Save-Document-Fix'
try {
    ...
    Export-HtmlReport -Title 'Word Cannot Save Document Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Word Cannot Save Document Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
