...
<#
.SYNOPSIS
    Common problem fix: OneDrive Files On Demand Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-Files-On-Demand-Missing-Fix
.EXAMPLE
    .\OneDrive-Files-On-Demand-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive Files On Demand Missing Fix'; $totalSteps = 8; $Name = 'OneDrive-Files-On-Demand-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'OneDrive Files On Demand Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive Files On Demand Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
