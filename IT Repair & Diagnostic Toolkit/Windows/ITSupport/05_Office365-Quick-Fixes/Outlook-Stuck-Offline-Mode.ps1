...
<#
.SYNOPSIS
    Common problem fix: Outlook Stuck Offline Mode
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Stuck-Offline-Mode.ps1
.EXAMPLE
    .\Outlook-Stuck-Offline-Mode.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Stuck Offline Mode'; $totalSteps = 8; $Name = 'Outlook-Stuck-Offline-Mode'
try {
    ...
    Export-HtmlReport -Title 'Outlook Stuck Offline Mode' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Stuck Offline Mode' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
