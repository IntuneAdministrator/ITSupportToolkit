...
<#
.SYNOPSIS
    Common problem fix: Excel Calculation Mode Stuck
.DESCRIPTION
...
.EXAMPLE
    .\Excel-Calculation-Mode-Stuck.ps1
.EXAMPLE
    .\Excel-Calculation-Mode-Stuck.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Excel Calculation Mode Stuck'; $totalSteps = 8; $Name = 'Excel-Calculation-Mode-Stuck'
try {
    ...
    Export-HtmlReport -Title 'Excel Calculation Mode Stuck' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Excel Calculation Mode Stuck' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
