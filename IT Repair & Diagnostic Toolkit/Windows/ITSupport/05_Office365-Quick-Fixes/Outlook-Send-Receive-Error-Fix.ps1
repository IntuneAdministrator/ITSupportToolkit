...
<#
.SYNOPSIS
    Common problem fix: Outlook Send Receive Error Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Send-Receive-Error-Fix
.EXAMPLE
    .\Outlook-Send-Receive-Error-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Send Receive Error Fix'; $totalSteps = 8; $Name = 'Outlook-Send-Receive-Error-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Send Receive Error Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Send Receive Error Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
