...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Outlook Freezing Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Outlook-Freezing-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Freezing-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Freezing-Fix' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
    ...
    Export-HtmlReport -Title 'Outlook Freezing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Freezing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
