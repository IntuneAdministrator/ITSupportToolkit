...
<#
.SYNOPSIS
    Common problem fix: Outlook Cannot Open OST PST
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Cannot-Open-OST-PST.ps1
.EXAMPLE
    .\Outlook-Cannot-Open-OST-PST.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Cannot Open OST PST'; $totalSteps = 8; $Name = 'Outlook-Cannot-Open-OST-PST'
try {
    ...
    Export-HtmlReport -Title 'Outlook Cannot Open OST PST' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Cannot Open OST PST' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
