...
<#
.SYNOPSIS
    Common problem fix: Office Update Stuck Fix
.DESCRIPTION
...
.EXAMPLE
    .\Office-Update-Stuck-Fix
.EXAMPLE
    .\Office-Update-Stuck-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Office Update Stuck Fix'; $totalSteps = 8; $Name = 'Office-Update-Stuck-Fix'
try {
    ...
    Export-HtmlReport -Title 'Office Update Stuck Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Office Update Stuck Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
