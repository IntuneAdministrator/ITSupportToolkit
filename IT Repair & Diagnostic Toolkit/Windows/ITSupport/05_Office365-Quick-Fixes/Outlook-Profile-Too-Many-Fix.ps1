...
<#
.SYNOPSIS
    Common problem fix: Outlook Profile Too Many Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Profile-Too-Many-Fix
.EXAMPLE
    .\Outlook-Profile-Too-Many-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Profile Too Many Fix'; $totalSteps = 8; $Name = 'Outlook-Profile-Too-Many-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Profile Too Many Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Profile Too Many Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
