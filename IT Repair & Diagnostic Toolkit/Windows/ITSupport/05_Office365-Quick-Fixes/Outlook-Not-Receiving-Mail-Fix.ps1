...
<#
.SYNOPSIS
    Common problem fix: Outlook Not Receiving Mail Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Not-Receiving-Mail-Fix
.EXAMPLE
    .\Outlook-Not-Receiving-Mail-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Not Receiving Mail Fix'; $totalSteps = 8; $Name = 'Outlook-Not-Receiving-Mail-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Not Receiving Mail Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Not Receiving Mail Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
