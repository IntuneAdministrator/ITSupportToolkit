...
<#
.SYNOPSIS
    Common problem fix: Excel Not Responding Fix
.DESCRIPTION
...
.EXAMPLE
    .\Excel-Not-Responding-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Excel Not Responding Fix'; $totalSteps = 8; $Name = 'Excel-Not-Responding-Fix'
try {
    ...
    Export-HtmlReport -Title 'Excel Not Responding Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Excel Not Responding Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
