...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Shared Calendar Not Syncing Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Shared-Calendar-Not-Syncing-Fix.
Category: 05_Office365-Quick-Fixes. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Shared-Calendar-Not-Syncing-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Shared-Calendar-Not-Syncing-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Shared Calendar Not Syncing Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'OFFICE365 QUICK FIXES'
        $Name = 'Shared-Calendar-Not-Syncing-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Shared-Calendar-Not-Syncing-Fix' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
        Write-SectionBanner 'M365 QUICK FIX'
        if ($Name -match 'Teams') {
            Get-Process -Name Teams,ms-teams -ErrorAction SilentlyContinue | ForEach-Object {
                if ($PSCmdlet.ShouldProcess($_.ProcessName, 'Stop Teams')) { Stop-Process -Id $_.Id -Force -EA SilentlyContinue }
            }
            $caches = @(
            ...
            foreach ($c in $caches) {
                if ((Test-Path $c) -and $PSCmdlet.ShouldProcess($c, 'Clear Teams cache')) {
                    Remove-Item "$c\*" -Recurse -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Teams cache' -Result 'Cleared' -Status 'PASS' -Detail $c
                }
            }
            if ($Name -match 'Audio') {
                Restart-Service Audiosrv -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Audiosrv' -Result (Get-Service Audiosrv).Status -Status 'PASS' -Detail 'Restart for Teams devices'
            }
        }

        if ($Name -match 'Outlook') {
            Get-Process OUTLOOK -ErrorAction SilentlyContinue | ForEach-Object {
                if ($PSCmdlet.ShouldProcess('OUTLOOK', 'Stop')) { Stop-Process -Id $_.Id -Force -EA SilentlyContinue }
            }
            ...
            if (Test-Path $disabled) {
                ...
                if ($PSCmdlet.ShouldProcess('Outlook resiliency', 'Clear DisabledItems')) {
                    ...
                    Add-ReportRow -Check 'DisabledItems cleared' -Result 'Done' -Status 'PASS' -Detail ''
                }
            } else {
                Add-ReportRow -Check 'DisabledItems' -Result 'None' -Status 'PASS' -Detail ''
            }
            if ($Name -match 'Calendar|Shared') {
                Add-ReportRow -Check 'Shared calendar' -Result 'Repair: remove/re-add calendar / OST rebuild if needed' -Status 'INFO' -Detail 'Exchange Online connectivity recommended next'
            }
        }

        if ($Name -match 'OneDrive') {
            Get-Process OneDrive -EA SilentlyContinue | ForEach-Object {
                if ($PSCmdlet.ShouldProcess('OneDrive', 'Restart')) { Stop-Process -Id $_.Id -Force -EA SilentlyContinue }
            }
            $od = "$env:LOCALAPPDATA\Microsoft\OneDrive\OneDrive.exe"
            if (Test-Path $od) {
                Start-Process $od
                Add-ReportRow -Check 'OneDrive' -Result 'Restarted' -Status 'PASS' -Detail $od
            } else {
                Add-ReportRow -Check 'OneDrive' -Result 'Not installed' -Status 'WARN' -Detail $od
            }
        }

        if ($Name -match 'Office-Wont-Launch|Copilot|License') {
            $ctr = Get-Service ClickToRunSvc -ErrorAction SilentlyContinue
            if ($ctr) {
                if ($PSCmdlet.ShouldProcess('ClickToRunSvc', 'Restart')) {
                    Restart-Service ClickToRunSvc -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'ClickToRunSvc' -Result (Get-Service ClickToRunSvc).Status -Status 'PASS' -Detail ''
                }
            }
            $ospp = Get-ChildItem "$env:ProgramFiles*\Microsoft Office\Office*\OSPP.VBS" -EA SilentlyContinue | Select-Object -First 1
            if ($ospp) {
                $st = & cscript.exe //Nologo $ospp.FullName /dstatus 2>&1 | Out-String
                Add-ReportRow -Check 'Office license' -Result 'dstatus' -Status 'INFO' -Detail ($st.Substring(0, [Math]::Min(400, $st.Length)))
            }
            if ($Name -match 'Copilot') {
                Add-ReportRow -Check 'Copilot' -Result 'Verify M365 Copilot license + WebView2 + sign-in' -Status 'INFO' -Detail $Join.Label
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'M365 quick fix'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Shared Calendar Not Syncing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Shared Calendar Not Syncing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
