...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: OneDrive Sync Error Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for OneDrive-Sync-Error-Fix.
...
.EXAMPLE
    .\OneDrive-Sync-Error-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\OneDrive-Sync-Error-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'OneDrive Sync Error Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'OneDrive-Sync-Error-Fix'
        Add-ReportRow -Check 'Tool' -Result 'OneDrive-Sync-Error-Fix' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
    ...
    Export-HtmlReport -Title 'OneDrive Sync Error Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'OneDrive Sync Error Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
