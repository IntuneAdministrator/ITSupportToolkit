...
<#
.SYNOPSIS
    Common problem fix: Office Activation Required Fix
.DESCRIPTION
...
.EXAMPLE
    .\Office-Activation-Required-Fix
.EXAMPLE
    .\Office-Activation-Required-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Office Activation Required Fix'; $totalSteps = 8; $Name = 'Office-Activation-Required-Fix'
try {
    ...
    Export-HtmlReport -Title 'Office Activation Required Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Office Activation Required Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
