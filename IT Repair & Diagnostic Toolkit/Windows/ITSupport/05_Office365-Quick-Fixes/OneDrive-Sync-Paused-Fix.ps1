...
<#
.SYNOPSIS
    Common problem fix: OneDrive Sync Paused Fix
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-Sync-Paused-Fix
.EXAMPLE
    .\OneDrive-Sync-Paused-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive Sync Paused Fix'; $totalSteps = 8; $Name = 'OneDrive-Sync-Paused-Fix'
try {
    ...
    Export-HtmlReport -Title 'OneDrive Sync Paused Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive Sync Paused Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
