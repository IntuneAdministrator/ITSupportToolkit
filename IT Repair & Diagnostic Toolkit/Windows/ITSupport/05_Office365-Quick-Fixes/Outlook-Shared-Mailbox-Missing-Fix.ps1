...
<#
.SYNOPSIS
    Common problem fix: Outlook Shared Mailbox Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Shared-Mailbox-Missing-Fix
.EXAMPLE
    .\Outlook-Shared-Mailbox-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Shared Mailbox Missing Fix'; $totalSteps = 8; $Name = 'Outlook-Shared-Mailbox-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Shared Mailbox Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Shared Mailbox Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
