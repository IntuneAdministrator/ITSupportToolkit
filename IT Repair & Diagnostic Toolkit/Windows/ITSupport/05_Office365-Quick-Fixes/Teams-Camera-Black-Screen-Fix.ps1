...
<#
.SYNOPSIS
    Common problem fix: Teams Camera Black Screen Fix
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Camera-Black-Screen-Fix
.EXAMPLE
    .\Teams-Camera-Black-Screen-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Camera Black Screen Fix'; $totalSteps = 8; $Name = 'Teams-Camera-Black-Screen-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams Camera Black Screen Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Camera Black Screen Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
