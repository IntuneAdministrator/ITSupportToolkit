...
<#
.SYNOPSIS
    Common problem fix: Outlook Safe Mode Launch Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Safe-Mode-Launch-Fix
.EXAMPLE
    .\Outlook-Safe-Mode-Launch-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Safe Mode Launch Fix'; $totalSteps = 8; $Name = 'Outlook-Safe-Mode-Launch-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Safe Mode Launch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Safe Mode Launch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
