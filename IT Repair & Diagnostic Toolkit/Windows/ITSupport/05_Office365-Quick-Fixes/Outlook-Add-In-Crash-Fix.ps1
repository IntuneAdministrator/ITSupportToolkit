...
<#
.SYNOPSIS
    Common problem fix: Outlook Add In Crash Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Add-In-Crash-Fix
.EXAMPLE
    .\Outlook-Add-In-Crash-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Add In Crash Fix'; $totalSteps = 8; $Name = 'Outlook-Add-In-Crash-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Add In Crash Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Add In Crash Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
