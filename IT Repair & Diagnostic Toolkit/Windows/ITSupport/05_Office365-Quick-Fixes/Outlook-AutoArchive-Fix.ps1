...
<#
.SYNOPSIS
    Common problem fix: Outlook AutoArchive Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-AutoArchive-Fix
.EXAMPLE
    .\Outlook-AutoArchive-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook AutoArchive Fix'; $totalSteps = 8; $Name = 'Outlook-AutoArchive-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook AutoArchive Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook AutoArchive Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
