...
<#
.SYNOPSIS
    Common problem fix: Teams Background Effects Fail Fix
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Background-Effects-Fail-Fix
.EXAMPLE
    .\Teams-Background-Effects-Fail-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Background Effects Fail Fix'; $totalSteps = 8; $Name = 'Teams-Background-Effects-Fail-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams Background Effects Fail Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Background Effects Fail Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
