...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Copilot Not Loading Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Copilot-Not-Loading-Fix.
...
.EXAMPLE
    .\Copilot-Not-Loading-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Copilot-Not-Loading-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Copilot Not Loading Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Copilot-Not-Loading-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Copilot-Not-Loading-Fix' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
    ...
    Export-HtmlReport -Title 'Copilot Not Loading Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Copilot Not Loading Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
