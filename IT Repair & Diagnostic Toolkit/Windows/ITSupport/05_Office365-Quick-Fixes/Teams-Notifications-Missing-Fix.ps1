...
<#
.SYNOPSIS
    Common problem fix: Teams Notifications Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Notifications-Missing-Fix
.EXAMPLE
    .\Teams-Notifications-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Notifications Missing Fix'; $totalSteps = 8; $Name = 'Teams-Notifications-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams Notifications Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Notifications Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
