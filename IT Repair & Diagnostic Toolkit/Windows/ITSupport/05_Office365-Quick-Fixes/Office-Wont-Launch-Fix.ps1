...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Office Wont Launch Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Office-Wont-Launch-Fix.
...
$exitCode = 0
$Activity = 'Office Wont Launch Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Office-Wont-Launch-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Office-Wont-Launch-Fix' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
    ...
    Export-HtmlReport -Title 'Office Wont Launch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Office Wont Launch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
