...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: M365 License Quick Check
.DESCRIPTION
    Production-grade Windows IT Support tool for M365-License-Quick-Check.
...
.EXAMPLE
    .\M365-License-Quick-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\M365-License-Quick-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'M365 License Quick Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'M365-License-Quick-Check'
        Add-ReportRow -Check 'Tool' -Result 'M365-License-Quick-Check' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
    ...
    Export-HtmlReport -Title 'M365 License Quick Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'M365 License Quick Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
