...
<#
.SYNOPSIS
    Common problem fix: Teams Status Stuck Away Fix
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Status-Stuck-Away-Fix
.EXAMPLE
    .\Teams-Status-Stuck-Away-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Status Stuck Away Fix'; $totalSteps = 8; $Name = 'Teams-Status-Stuck-Away-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams Status Stuck Away Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Status Stuck Away Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
