...
<#
.SYNOPSIS
    Common problem fix: Teams Meeting Join Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Meeting-Join-Failed-Fix
.EXAMPLE
    .\Teams-Meeting-Join-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Meeting Join Failed Fix'; $totalSteps = 8; $Name = 'Teams-Meeting-Join-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams Meeting Join Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Meeting Join Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
