...
<#
.SYNOPSIS
    Common problem fix: OneDrive Selective Sync Fix
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-Selective-Sync-Fix
.EXAMPLE
    .\OneDrive-Selective-Sync-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive Selective Sync Fix'; $totalSteps = 8; $Name = 'OneDrive-Selective-Sync-Fix'
try {
    ...
    Export-HtmlReport -Title 'OneDrive Selective Sync Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive Selective Sync Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
