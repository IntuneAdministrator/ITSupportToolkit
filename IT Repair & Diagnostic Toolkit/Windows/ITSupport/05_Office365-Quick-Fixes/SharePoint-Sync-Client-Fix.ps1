...
<#
.SYNOPSIS
    Common problem fix: SharePoint Sync Client Fix
.DESCRIPTION
...
.EXAMPLE
    .\SharePoint-Sync-Client-Fix
.EXAMPLE
    .\SharePoint-Sync-Client-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SharePoint Sync Client Fix'; $totalSteps = 8; $Name = 'SharePoint-Sync-Client-Fix'
try {
    ...
    Export-HtmlReport -Title 'SharePoint Sync Client Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SharePoint Sync Client Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
