...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Teams Audio Device Quick Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Teams-Audio-Device-Quick-Fix.
...
.EXAMPLE
    .\Teams-Audio-Device-Quick-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Teams-Audio-Device-Quick-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Teams Audio Device Quick Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Teams-Audio-Device-Quick-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Teams-Audio-Device-Quick-Fix' -Status 'INFO' -Detail 'ITSupport/05_Office365-Quick-Fixes'
    ...
    Export-HtmlReport -Title 'Teams Audio Device Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams Audio Device Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
