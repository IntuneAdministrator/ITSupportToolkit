...
<#
.SYNOPSIS
    Common problem fix: Teams Sign In Loop Fix
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Sign-In-Loop-Fix.ps1
.EXAMPLE
    .\Teams-Sign-In-Loop-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Sign In Loop Fix'; $totalSteps = 8; $Name = 'Teams-Sign-In-Loop-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams Sign In Loop Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Sign In Loop Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
