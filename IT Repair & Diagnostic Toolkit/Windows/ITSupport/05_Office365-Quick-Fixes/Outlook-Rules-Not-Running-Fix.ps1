...
<#
.SYNOPSIS
    Common problem fix: Outlook Rules Not Running Fix
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-Rules-Not-Running-Fix
.EXAMPLE
    .\Outlook-Rules-Not-Running-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook Rules Not Running Fix'; $totalSteps = 8; $Name = 'Outlook-Rules-Not-Running-Fix'
try {
    ...
    Export-HtmlReport -Title 'Outlook Rules Not Running Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Rules Not Running Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
