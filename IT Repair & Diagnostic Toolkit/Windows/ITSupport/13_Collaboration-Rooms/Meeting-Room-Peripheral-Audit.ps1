...
<#
.SYNOPSIS
    Enterprise IT tool: Meeting Room Peripheral Audit
.DESCRIPTION
...
.EXAMPLE
    .\Meeting-Room-Peripheral-Audit.ps1
.EXAMPLE
    .\Meeting-Room-Peripheral-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Meeting Room Peripheral Audit'; $totalSteps = 8; $Name = 'Meeting-Room-Peripheral-Audit'
try {
    ...
    Export-HtmlReport -Title 'Meeting Room Peripheral Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Meeting Room Peripheral Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
