...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-User-Triage.ps1
.EXAMPLE
    .\Collaboration-Rooms-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms User Triage'; $totalSteps = 8; $Name = 'Collaboration-Rooms-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
