...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-One-Click-Fix.ps1
.EXAMPLE
    .\Collaboration-Rooms-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms One Click Fix'; $totalSteps = 8; $Name = 'Collaboration-Rooms-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
