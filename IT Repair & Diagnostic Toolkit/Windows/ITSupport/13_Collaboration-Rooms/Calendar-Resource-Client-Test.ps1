...
<#
.SYNOPSIS
    Enterprise IT tool: Calendar Resource Client Test
.DESCRIPTION
...
.EXAMPLE
    .\Calendar-Resource-Client-Test.ps1
.EXAMPLE
    .\Calendar-Resource-Client-Test.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Calendar Resource Client Test'; $totalSteps = 8; $Name = 'Calendar-Resource-Client-Test'
try {
    ...
    Export-HtmlReport -Title 'Calendar Resource Client Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Calendar Resource Client Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
