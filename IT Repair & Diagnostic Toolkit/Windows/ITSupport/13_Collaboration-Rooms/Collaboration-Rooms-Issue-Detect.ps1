...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-Issue-Detect.ps1
.EXAMPLE
    .\Collaboration-Rooms-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms Issue Detect'; $totalSteps = 8; $Name = 'Collaboration-Rooms-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
