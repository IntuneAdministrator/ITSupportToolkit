...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-Quick-Repair.ps1
.EXAMPLE
    .\Collaboration-Rooms-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms Quick Repair'; $totalSteps = 8; $Name = 'Collaboration-Rooms-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
