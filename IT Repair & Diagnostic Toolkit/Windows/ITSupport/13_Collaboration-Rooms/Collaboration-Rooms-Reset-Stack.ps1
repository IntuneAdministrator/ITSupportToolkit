...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-Reset-Stack.ps1
.EXAMPLE
    .\Collaboration-Rooms-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms Reset Stack'; $totalSteps = 8; $Name = 'Collaboration-Rooms-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
