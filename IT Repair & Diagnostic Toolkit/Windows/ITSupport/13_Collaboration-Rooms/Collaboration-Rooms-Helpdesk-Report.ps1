...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 13_Collaboration-Rooms). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Collaboration-Rooms-Helpdesk-Report.ps1
.EXAMPLE
    .\Collaboration-Rooms-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms Helpdesk Report'; $totalSteps = 8; $Name = 'Collaboration-Rooms-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '13_Collaboration-Rooms' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '13_Collaboration-Rooms'
    ...
    Export-HtmlReport -Title 'Collaboration Rooms Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
