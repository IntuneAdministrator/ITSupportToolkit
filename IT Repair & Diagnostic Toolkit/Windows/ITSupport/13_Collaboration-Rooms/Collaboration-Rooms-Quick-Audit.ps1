...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-Quick-Audit.ps1
.EXAMPLE
    .\Collaboration-Rooms-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms Quick Audit'; $totalSteps = 8; $Name = 'Collaboration-Rooms-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
