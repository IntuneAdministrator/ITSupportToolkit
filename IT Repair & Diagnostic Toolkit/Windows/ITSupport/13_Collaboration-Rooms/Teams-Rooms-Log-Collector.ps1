...
<#
.SYNOPSIS
    Enterprise IT tool: Teams Rooms Log Collector
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Rooms-Log-Collector.ps1
.EXAMPLE
    .\Teams-Rooms-Log-Collector.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Rooms Log Collector'; $totalSteps = 8; $Name = 'Teams-Rooms-Log-Collector'
try {
    ...
    Export-HtmlReport -Title 'Teams Rooms Log Collector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Rooms Log Collector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
