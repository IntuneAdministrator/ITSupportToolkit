...
<#
.SYNOPSIS
    Common problem fix: Collaboration Rooms Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Collaboration-Rooms-Policy-Check.ps1
.EXAMPLE
    .\Collaboration-Rooms-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Collaboration Rooms Policy Check'; $totalSteps = 8; $Name = 'Collaboration-Rooms-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Collaboration Rooms Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Collaboration Rooms Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
