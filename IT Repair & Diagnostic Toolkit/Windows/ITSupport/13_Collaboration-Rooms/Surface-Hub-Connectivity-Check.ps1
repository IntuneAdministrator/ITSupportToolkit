...
<#
.SYNOPSIS
    Enterprise IT tool: Surface Hub Connectivity Check
.DESCRIPTION
    Expansion module (ITSupport / 13_Collaboration-Rooms) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Surface-Hub-Connectivity-Check.ps1
.EXAMPLE
    .\Surface-Hub-Connectivity-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Surface Hub Connectivity Check'; $totalSteps = 8; $Name = 'Surface-Hub-Connectivity-Check'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '13_Collaboration-Rooms' -Status 'INFO' -Detail 'ITSupport'
    ...
    Export-HtmlReport -Title 'Surface Hub Connectivity Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Surface Hub Connectivity Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
