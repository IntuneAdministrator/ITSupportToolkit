...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-Reset-Stack.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit Reset Stack'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
