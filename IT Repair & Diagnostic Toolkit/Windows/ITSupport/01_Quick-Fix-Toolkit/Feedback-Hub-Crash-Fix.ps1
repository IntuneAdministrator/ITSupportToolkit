...
<#
.SYNOPSIS
    Common problem fix: Feedback Hub Crash Fix
.DESCRIPTION
...
.EXAMPLE
    .\Feedback-Hub-Crash-Fix
.EXAMPLE
    .\Feedback-Hub-Crash-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Feedback Hub Crash Fix'; $totalSteps = 8; $Name = 'Feedback-Hub-Crash-Fix'
try {
    ...
    Export-HtmlReport -Title 'Feedback Hub Crash Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Feedback Hub Crash Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
