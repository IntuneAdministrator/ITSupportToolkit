...
<#
.SYNOPSIS
    Common problem fix: Digital License Link Fix
.DESCRIPTION
...
.EXAMPLE
    .\Digital-License-Link-Fix
.EXAMPLE
    .\Digital-License-Link-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Digital License Link Fix'; $totalSteps = 8; $Name = 'Digital-License-Link-Fix'
try {
    ...
    Export-HtmlReport -Title 'Digital License Link Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Digital License Link Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
