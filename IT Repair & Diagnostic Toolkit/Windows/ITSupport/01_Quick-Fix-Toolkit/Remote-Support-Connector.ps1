...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Remote Support Connector
.DESCRIPTION
    Production-grade Windows IT Support tool for Remote-Support-Connector.
...
.EXAMPLE
    .\Remote-Support-Connector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Remote-Support-Connector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Remote Support Connector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Remote-Support-Connector'
        Add-ReportRow -Check 'Tool' -Result 'Remote-Support-Connector' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
    ...
    Export-HtmlReport -Title 'Remote Support Connector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Support Connector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
