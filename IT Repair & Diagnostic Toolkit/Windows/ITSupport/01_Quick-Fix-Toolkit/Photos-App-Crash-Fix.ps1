...
<#
.SYNOPSIS
    Common problem fix: Photos App Crash Fix
.DESCRIPTION
...
.EXAMPLE
    .\Photos-App-Crash-Fix
.EXAMPLE
    .\Photos-App-Crash-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Photos App Crash Fix'; $totalSteps = 8; $Name = 'Photos-App-Crash-Fix'
try {
    ...
    Export-HtmlReport -Title 'Photos App Crash Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Photos App Crash Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
