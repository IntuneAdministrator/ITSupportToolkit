...
<#
.SYNOPSIS
    Common problem fix: Context Menu Not Opening Fix
.DESCRIPTION
...
.EXAMPLE
    .\Context-Menu-Not-Opening-Fix
.EXAMPLE
    .\Context-Menu-Not-Opening-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Context Menu Not Opening Fix'; $totalSteps = 8; $Name = 'Context-Menu-Not-Opening-Fix'
try {
    ...
    Export-HtmlReport -Title 'Context Menu Not Opening Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Context Menu Not Opening Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
