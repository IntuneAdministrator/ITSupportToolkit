...
<#
.SYNOPSIS
    Common problem fix: Desktop Icons Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Desktop-Icons-Missing-Fix
.EXAMPLE
    .\Desktop-Icons-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Desktop Icons Missing Fix'; $totalSteps = 8; $Name = 'Desktop-Icons-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Desktop Icons Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Desktop Icons Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
