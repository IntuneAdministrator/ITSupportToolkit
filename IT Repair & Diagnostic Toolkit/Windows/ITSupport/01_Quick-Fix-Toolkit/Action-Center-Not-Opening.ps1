...
<#
.SYNOPSIS
    Common problem fix: Action Center Not Opening
.DESCRIPTION
...
.EXAMPLE
    .\Action-Center-Not-Opening.ps1
.EXAMPLE
    .\Action-Center-Not-Opening.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Action Center Not Opening'; $totalSteps = 8; $Name = 'Action-Center-Not-Opening'
try {
    ...
    Export-HtmlReport -Title 'Action Center Not Opening' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Action Center Not Opening' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
