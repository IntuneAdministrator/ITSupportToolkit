...
<#
.SYNOPSIS
    Common problem fix: Restore Point Create Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Restore-Point-Create-Failed-Fix
.EXAMPLE
    .\Restore-Point-Create-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Restore Point Create Failed Fix'; $totalSteps = 8; $Name = 'Restore-Point-Create-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Restore Point Create Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Restore Point Create Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
