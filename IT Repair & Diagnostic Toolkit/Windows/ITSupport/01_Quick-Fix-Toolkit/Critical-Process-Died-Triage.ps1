...
<#
.SYNOPSIS
    Common problem fix: Critical Process Died Triage
.DESCRIPTION
...
.EXAMPLE
    .\Critical-Process-Died-Triage
.EXAMPLE
    .\Critical-Process-Died-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Critical Process Died Triage'; $totalSteps = 8; $Name = 'Critical-Process-Died-Triage'
try {
    ...
    Export-HtmlReport -Title 'Critical Process Died Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Critical Process Died Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
