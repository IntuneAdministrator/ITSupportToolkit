...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Ticket Evidence Collector
.DESCRIPTION
    Production-grade Windows IT Support tool for Ticket-Evidence-Collector.
...
.EXAMPLE
    .\Ticket-Evidence-Collector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Ticket-Evidence-Collector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Ticket Evidence Collector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Ticket-Evidence-Collector'
        Add-ReportRow -Check 'Tool' -Result 'Ticket-Evidence-Collector' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
    ...
    Export-HtmlReport -Title 'Ticket Evidence Collector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Ticket Evidence Collector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
