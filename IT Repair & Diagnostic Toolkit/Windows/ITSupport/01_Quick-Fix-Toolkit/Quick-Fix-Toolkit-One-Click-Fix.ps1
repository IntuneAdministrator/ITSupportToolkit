...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-One-Click-Fix.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit One Click Fix'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
