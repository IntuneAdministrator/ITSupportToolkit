...
<#
.SYNOPSIS
    Common problem fix: Runbook Slow PC Standard Fix
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Slow-PC-Standard-Fix.ps1
.EXAMPLE
    .\Runbook-Slow-PC-Standard-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Slow PC Standard Fix'; $totalSteps = 8; $Name = 'Runbook-Slow-PC-Standard-Fix'
try {
    ...
    Export-HtmlReport -Title 'Runbook Slow PC Standard Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Slow PC Standard Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
