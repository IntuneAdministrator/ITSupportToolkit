...
<#
.SYNOPSIS
    Common problem fix: Windows Activation Error Triage
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Activation-Error-Triage
.EXAMPLE
    .\Windows-Activation-Error-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Activation Error Triage'; $totalSteps = 8; $Name = 'Windows-Activation-Error-Triage'
try {
    ...
    Export-HtmlReport -Title 'Windows Activation Error Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Activation Error Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
