...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-Quick-Repair.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit Quick Repair'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
