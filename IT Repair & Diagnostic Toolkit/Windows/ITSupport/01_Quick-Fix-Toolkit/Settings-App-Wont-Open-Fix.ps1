...
<#
.SYNOPSIS
    Common problem fix: Settings App Wont Open Fix
.DESCRIPTION
...
.EXAMPLE
    .\Settings-App-Wont-Open-Fix
.EXAMPLE
    .\Settings-App-Wont-Open-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings App Wont Open Fix'; $totalSteps = 8; $Name = 'Settings-App-Wont-Open-Fix'
try {
    ...
    Export-HtmlReport -Title 'Settings App Wont Open Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings App Wont Open Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
