...
<#
.SYNOPSIS
    Common problem fix: Windows Search Reset Quick
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Search-Reset-Quick.ps1
.EXAMPLE
    .\Windows-Search-Reset-Quick.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Search Reset Quick'; $totalSteps = 8; $Name = 'Windows-Search-Reset-Quick'
try {
    ...
    Export-HtmlReport -Title 'Windows Search Reset Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Search Reset Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
