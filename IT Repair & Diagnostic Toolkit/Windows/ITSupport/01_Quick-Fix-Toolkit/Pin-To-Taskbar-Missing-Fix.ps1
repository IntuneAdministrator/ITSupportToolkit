...
<#
.SYNOPSIS
    Common problem fix: Pin To Taskbar Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Pin-To-Taskbar-Missing-Fix
.EXAMPLE
    .\Pin-To-Taskbar-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Pin To Taskbar Missing Fix'; $totalSteps = 8; $Name = 'Pin-To-Taskbar-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Pin To Taskbar Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Pin To Taskbar Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
