...
<#
.SYNOPSIS
    Common problem fix: Helpdesk Autopilot Reset Hint
.DESCRIPTION
...
.EXAMPLE
    .\Helpdesk-Autopilot-Reset-Hint.ps1
.EXAMPLE
    .\Helpdesk-Autopilot-Reset-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Helpdesk Autopilot Reset Hint'; $totalSteps = 8; $Name = 'Helpdesk-Autopilot-Reset-Hint'
try {
    ...
    Export-HtmlReport -Title 'Helpdesk Autopilot Reset Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Helpdesk Autopilot Reset Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
