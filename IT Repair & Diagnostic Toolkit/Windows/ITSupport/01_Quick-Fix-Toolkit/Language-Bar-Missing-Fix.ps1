...
<#
.SYNOPSIS
    Common problem fix: Language Bar Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Language-Bar-Missing-Fix
.EXAMPLE
    .\Language-Bar-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Language Bar Missing Fix'; $totalSteps = 8; $Name = 'Language-Bar-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Language Bar Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Language Bar Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
