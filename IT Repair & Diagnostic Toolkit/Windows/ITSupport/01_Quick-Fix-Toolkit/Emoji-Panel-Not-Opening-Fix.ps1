...
<#
.SYNOPSIS
    Common problem fix: Emoji Panel Not Opening Fix
.DESCRIPTION
...
.EXAMPLE
    .\Emoji-Panel-Not-Opening-Fix
.EXAMPLE
    .\Emoji-Panel-Not-Opening-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Emoji Panel Not Opening Fix'; $totalSteps = 8; $Name = 'Emoji-Panel-Not-Opening-Fix'
try {
    ...
    Export-HtmlReport -Title 'Emoji Panel Not Opening Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Emoji Panel Not Opening Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
