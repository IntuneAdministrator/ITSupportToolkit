...
<#
.SYNOPSIS
    Common problem fix: Microsoft Store Wont Open Fix
.DESCRIPTION
...
.EXAMPLE
    .\Microsoft-Store-Wont-Open-Fix
.EXAMPLE
    .\Microsoft-Store-Wont-Open-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft Store Wont Open Fix'; $totalSteps = 8; $Name = 'Microsoft-Store-Wont-Open-Fix'
try {
    ...
    Export-HtmlReport -Title 'Microsoft Store Wont Open Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft Store Wont Open Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
