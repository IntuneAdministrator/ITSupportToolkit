...
<#
.SYNOPSIS
    Common problem fix: Right Click Menu Slow Fix
.DESCRIPTION
...
.EXAMPLE
    .\Right-Click-Menu-Slow-Fix
.EXAMPLE
    .\Right-Click-Menu-Slow-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Right Click Menu Slow Fix'; $totalSteps = 8; $Name = 'Right-Click-Menu-Slow-Fix'
try {
    ...
    Export-HtmlReport -Title 'Right Click Menu Slow Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Right Click Menu Slow Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
