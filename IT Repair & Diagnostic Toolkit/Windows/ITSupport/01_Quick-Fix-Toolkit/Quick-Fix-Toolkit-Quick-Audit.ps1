...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-Quick-Audit.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit Quick Audit'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
