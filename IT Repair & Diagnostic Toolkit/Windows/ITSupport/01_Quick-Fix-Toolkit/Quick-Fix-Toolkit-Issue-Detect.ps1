...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-Issue-Detect.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit Issue Detect'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
