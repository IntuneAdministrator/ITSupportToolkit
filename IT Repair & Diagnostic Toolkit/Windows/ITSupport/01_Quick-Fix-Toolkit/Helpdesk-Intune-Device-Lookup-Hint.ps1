...
<#
.SYNOPSIS
    Common problem fix: Helpdesk Intune Device Lookup Hint
.DESCRIPTION
...
.EXAMPLE
    .\Helpdesk-Intune-Device-Lookup-Hint.ps1
.EXAMPLE
    .\Helpdesk-Intune-Device-Lookup-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Helpdesk Intune Device Lookup Hint'; $totalSteps = 8; $Name = 'Helpdesk-Intune-Device-Lookup-Hint'
try {
    ...
    Export-HtmlReport -Title 'Helpdesk Intune Device Lookup Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Helpdesk Intune Device Lookup Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
