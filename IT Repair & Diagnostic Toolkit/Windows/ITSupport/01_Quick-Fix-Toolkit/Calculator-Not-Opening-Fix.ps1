...
<#
.SYNOPSIS
    Common problem fix: Calculator Not Opening Fix
.DESCRIPTION
...
.EXAMPLE
    .\Calculator-Not-Opening-Fix
.EXAMPLE
    .\Calculator-Not-Opening-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Calculator Not Opening Fix'; $totalSteps = 8; $Name = 'Calculator-Not-Opening-Fix'
try {
    ...
    Export-HtmlReport -Title 'Calculator Not Opening Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Calculator Not Opening Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
