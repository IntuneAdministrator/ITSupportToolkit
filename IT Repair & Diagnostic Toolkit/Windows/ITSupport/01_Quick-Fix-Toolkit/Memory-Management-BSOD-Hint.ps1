...
<#
.SYNOPSIS
    Common problem fix: Memory Management BSOD Hint
.DESCRIPTION
...
.EXAMPLE
    .\Memory-Management-BSOD-Hint
.EXAMPLE
    .\Memory-Management-BSOD-Hint -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Memory Management BSOD Hint'; $totalSteps = 8; $Name = 'Memory-Management-BSOD-Hint'
try {
    ...
    Export-HtmlReport -Title 'Memory Management BSOD Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Memory Management BSOD Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
