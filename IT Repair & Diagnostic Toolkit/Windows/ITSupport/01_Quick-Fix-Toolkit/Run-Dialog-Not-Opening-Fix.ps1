...
<#
.SYNOPSIS
    Common problem fix: Run Dialog Not Opening Fix
.DESCRIPTION
...
.EXAMPLE
    .\Run-Dialog-Not-Opening-Fix
.EXAMPLE
    .\Run-Dialog-Not-Opening-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Run Dialog Not Opening Fix'; $totalSteps = 8; $Name = 'Run-Dialog-Not-Opening-Fix'
try {
    ...
    Export-HtmlReport -Title 'Run Dialog Not Opening Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Run Dialog Not Opening Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
