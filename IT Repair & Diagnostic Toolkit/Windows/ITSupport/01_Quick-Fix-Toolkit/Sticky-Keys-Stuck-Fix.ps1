...
<#
.SYNOPSIS
    Common problem fix: Sticky Keys Stuck Fix
.DESCRIPTION
...
.EXAMPLE
    .\Sticky-Keys-Stuck-Fix.ps1
.EXAMPLE
    .\Sticky-Keys-Stuck-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Sticky Keys Stuck Fix'; $totalSteps = 8; $Name = 'Sticky-Keys-Stuck-Fix'
try {
    ...
    Export-HtmlReport -Title 'Sticky Keys Stuck Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Sticky Keys Stuck Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
