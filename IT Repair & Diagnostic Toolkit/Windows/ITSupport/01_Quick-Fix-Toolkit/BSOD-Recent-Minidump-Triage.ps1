...
<#
.SYNOPSIS
    Common problem fix: BSOD Recent Minidump Triage
.DESCRIPTION
...
.EXAMPLE
    .\BSOD-Recent-Minidump-Triage
.EXAMPLE
    .\BSOD-Recent-Minidump-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BSOD Recent Minidump Triage'; $totalSteps = 8; $Name = 'BSOD-Recent-Minidump-Triage'
try {
    ...
    Export-HtmlReport -Title 'BSOD Recent Minidump Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BSOD Recent Minidump Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
