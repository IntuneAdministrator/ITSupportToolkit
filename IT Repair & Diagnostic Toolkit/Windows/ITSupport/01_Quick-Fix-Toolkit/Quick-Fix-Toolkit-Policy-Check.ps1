...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-Policy-Check.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit Policy Check'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
