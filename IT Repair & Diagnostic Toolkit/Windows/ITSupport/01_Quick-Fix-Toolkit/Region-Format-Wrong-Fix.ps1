...
<#
.SYNOPSIS
    Common problem fix: Region Format Wrong Fix
.DESCRIPTION
...
.EXAMPLE
    .\Region-Format-Wrong-Fix
.EXAMPLE
    .\Region-Format-Wrong-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Region Format Wrong Fix'; $totalSteps = 8; $Name = 'Region-Format-Wrong-Fix'
try {
    ...
    Export-HtmlReport -Title 'Region Format Wrong Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Region Format Wrong Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
