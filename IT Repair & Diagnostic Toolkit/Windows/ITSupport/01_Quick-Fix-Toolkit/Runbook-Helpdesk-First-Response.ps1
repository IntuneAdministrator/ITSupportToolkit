...
<#
.SYNOPSIS
    Common problem fix: Runbook Helpdesk First Response
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Helpdesk-First-Response.ps1
.EXAMPLE
    .\Runbook-Helpdesk-First-Response.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Helpdesk First Response'; $totalSteps = 8; $Name = 'Runbook-Helpdesk-First-Response'
try {
    ...
    Export-HtmlReport -Title 'Runbook Helpdesk First Response' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Helpdesk First Response' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
