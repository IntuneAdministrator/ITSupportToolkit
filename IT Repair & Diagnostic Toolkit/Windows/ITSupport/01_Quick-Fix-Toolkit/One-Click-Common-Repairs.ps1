...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: One Click Common Repairs
.DESCRIPTION
    Production-grade Windows IT Support tool for One-Click-Common-Repairs.
...
.EXAMPLE
    .\One-Click-Common-Repairs.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\One-Click-Common-Repairs.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'One Click Common Repairs'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'One-Click-Common-Repairs'
        Add-ReportRow -Check 'Tool' -Result 'One-Click-Common-Repairs' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
    ...
    Export-HtmlReport -Title 'One Click Common Repairs' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'One Click Common Repairs' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
