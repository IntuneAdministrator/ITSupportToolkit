...
<#
.SYNOPSIS
    Common problem fix: Previous Version Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Previous-Version-Missing-Fix
.EXAMPLE
    .\Previous-Version-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Previous Version Missing Fix'; $totalSteps = 8; $Name = 'Previous-Version-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Previous Version Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Previous Version Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
