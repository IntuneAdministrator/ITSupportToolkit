...
<#
.SYNOPSIS
    Common problem fix: Clock Wrong Timezone Fix
.DESCRIPTION
...
.EXAMPLE
    .\Clock-Wrong-Timezone-Fix
.EXAMPLE
    .\Clock-Wrong-Timezone-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Clock Wrong Timezone Fix'; $totalSteps = 8; $Name = 'Clock-Wrong-Timezone-Fix'
try {
    ...
    Export-HtmlReport -Title 'Clock Wrong Timezone Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Clock Wrong Timezone Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
