...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-User-Triage.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit User Triage'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
