...
<#
.SYNOPSIS
    Common problem fix: Game Bar Not Opening Fix
.DESCRIPTION
...
.EXAMPLE
    .\Game-Bar-Not-Opening-Fix
.EXAMPLE
    .\Game-Bar-Not-Opening-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Game Bar Not Opening Fix'; $totalSteps = 8; $Name = 'Game-Bar-Not-Opening-Fix'
try {
    ...
    Export-HtmlReport -Title 'Game Bar Not Opening Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Game Bar Not Opening Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
