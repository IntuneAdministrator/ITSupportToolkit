...
<#
.SYNOPSIS
    Common problem fix: Runbook User Account Standard Fix
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 01_Quick-Fix-Toolkit). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-User-Account-Standard-Fix.ps1
.EXAMPLE
    .\Runbook-User-Account-Standard-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook User Account Standard Fix'; $totalSteps = 8; $Name = 'Runbook-User-Account-Standard-Fix'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '01_Quick-Fix-Toolkit' -Status 'INFO' -Detail 'ITSupport'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '01_Quick-Fix-Toolkit'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '01_Quick-Fix-Toolkit'
    ...
    Export-HtmlReport -Title 'Runbook User Account Standard Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook User Account Standard Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
