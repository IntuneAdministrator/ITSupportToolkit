...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Quick Fix Menu
.DESCRIPTION
    Production-grade Windows IT Support tool for Quick-Fix-Menu.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Quick-Fix-Menu'
        Add-ReportRow -Check 'Tool' -Result 'Quick-Fix-Menu' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
    ...
    Export-HtmlReport -Title 'Quick Fix Menu' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Quick Fix Menu' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
