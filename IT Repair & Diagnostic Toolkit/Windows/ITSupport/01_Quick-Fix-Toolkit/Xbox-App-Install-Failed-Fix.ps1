...
<#
.SYNOPSIS
    Common problem fix: Xbox App Install Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Xbox-App-Install-Failed-Fix
.EXAMPLE
    .\Xbox-App-Install-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Xbox App Install Failed Fix'; $totalSteps = 8; $Name = 'Xbox-App-Install-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Xbox App Install Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Xbox App Install Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
