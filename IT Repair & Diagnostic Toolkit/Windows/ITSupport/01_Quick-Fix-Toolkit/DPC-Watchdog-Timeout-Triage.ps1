...
<#
.SYNOPSIS
    Common problem fix: DPC Watchdog Timeout Triage
.DESCRIPTION
...
.EXAMPLE
    .\DPC-Watchdog-Timeout-Triage
.EXAMPLE
    .\DPC-Watchdog-Timeout-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DPC Watchdog Timeout Triage'; $totalSteps = 8; $Name = 'DPC-Watchdog-Timeout-Triage'
try {
    ...
    Export-HtmlReport -Title 'DPC Watchdog Timeout Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DPC Watchdog Timeout Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
