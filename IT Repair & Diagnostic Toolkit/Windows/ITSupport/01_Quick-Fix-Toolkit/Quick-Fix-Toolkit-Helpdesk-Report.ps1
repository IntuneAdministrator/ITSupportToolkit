...
<#
.SYNOPSIS
    Common problem fix: Quick Fix Toolkit Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Fix-Toolkit-Helpdesk-Report.ps1
.EXAMPLE
    .\Quick-Fix-Toolkit-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Fix Toolkit Helpdesk Report'; $totalSteps = 8; $Name = 'Quick-Fix-Toolkit-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Quick Fix Toolkit Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Fix Toolkit Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
