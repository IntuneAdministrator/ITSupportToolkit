...
<#
.SYNOPSIS
    Common problem fix: WHEA Uncorrectable Error Triage
.DESCRIPTION
...
.EXAMPLE
    .\WHEA-Uncorrectable-Error-Triage
.EXAMPLE
    .\WHEA-Uncorrectable-Error-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WHEA Uncorrectable Error Triage'; $totalSteps = 8; $Name = 'WHEA-Uncorrectable-Error-Triage'
try {
    ...
    Export-HtmlReport -Title 'WHEA Uncorrectable Error Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WHEA Uncorrectable Error Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
