...
<#
.SYNOPSIS
    Common problem fix: System Thread Exception Triage
.DESCRIPTION
...
.EXAMPLE
    .\System-Thread-Exception-Triage
.EXAMPLE
    .\System-Thread-Exception-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'System Thread Exception Triage'; $totalSteps = 8; $Name = 'System-Thread-Exception-Triage'
try {
    ...
    Export-HtmlReport -Title 'System Thread Exception Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'System Thread Exception Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
