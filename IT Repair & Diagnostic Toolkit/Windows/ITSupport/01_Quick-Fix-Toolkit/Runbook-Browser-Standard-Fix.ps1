...
<#
.SYNOPSIS
    Common problem fix: Runbook Browser Standard Fix
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Browser-Standard-Fix.ps1
.EXAMPLE
    .\Runbook-Browser-Standard-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Browser Standard Fix'; $totalSteps = 8; $Name = 'Runbook-Browser-Standard-Fix'
try {
    ...
    Export-HtmlReport -Title 'Runbook Browser Standard Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Browser Standard Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
