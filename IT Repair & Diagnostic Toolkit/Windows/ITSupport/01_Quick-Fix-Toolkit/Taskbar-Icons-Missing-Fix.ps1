...
<#
.SYNOPSIS
    Common problem fix: Taskbar Icons Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Taskbar-Icons-Missing-Fix.ps1
.EXAMPLE
    .\Taskbar-Icons-Missing-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Taskbar Icons Missing Fix'; $totalSteps = 8; $Name = 'Taskbar-Icons-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Taskbar Icons Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Taskbar Icons Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
