...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Remote Diagnostics Packager
.DESCRIPTION
    Production-grade Windows IT Support tool for Remote-Diagnostics-Packager.
Category: 01_Quick-Fix-Toolkit. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Remote-Diagnostics-Packager.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Remote-Diagnostics-Packager.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Remote Diagnostics Packager'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'QUICK FIX TOOLKIT'
        $Name = 'Remote-Diagnostics-Packager'
        Add-ReportRow -Check 'Tool' -Result 'Remote-Diagnostics-Packager' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
        # Rapid health snapshot used by menu / one-click / triage / silent runners
        $os = Get-CimInstance Win32_OperatingSystem
        $cs = Get-CimInstance Win32_ComputerSystem
        Add-ReportRow -Check 'Uptime hours' -Result ([int]((Get-Date) - $os.LastBootUpTime).TotalHours) -Status 'INFO' -Detail "Booted $($os.LastBootUpTime)"
        Add-ReportRow -Check 'Free RAM GB' -Result ("{0:N2}" -f ($os.FreePhysicalMemory/1MB)) -Status $(if (($os.FreePhysicalMemory/1MB) -gt 1) {'PASS'} else {'WARN'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'Health snapshot'

        $fixes = [System.Collections.Generic.List[string]]::new()
        if ($PSCmdlet.ShouldProcess('Common repairs', 'Run safe quick fixes')) {
            ...
            if ($spool -and $spool.Status -ne 'Running') {
                ...
                $fixes.Add('Spooler-start')
            }
            # Clear stuck print jobs if emergency path
            if ($Name -match 'One-Click|Silent|All-In-One|Quick-Fix') {
                Stop-Service Spooler -Force -ErrorAction SilentlyContinue
                Remove-Item "$env:SystemRoot\System32\spool\PRINTERS\*" -Force -ErrorAction SilentlyContinue
                Start-Service Spooler -ErrorAction SilentlyContinue
                $fixes.Add('print-queue-clear')
            }
            Add-ReportRow -Check 'Quick repairs applied' -Result ($fixes -join ', ') -Status 'PASS' -Detail 'Safe defaults only'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Quick repairs'

        if ($Name -match 'Ticket-Evidence|Remote-Diagnostics|Escalation|Packager') {
            $pack = Join-Path 'C:\IT-Logs' ("SupportPack_{0}" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
            ...
            Get-NetAdapter | Format-Table -AutoSize | Out-File (Join-Path $pack 'adapters.txt')
            Get-Service | Where-Object Status -ne 'Running' | Select-Object Name,Status,StartType | Export-Csv (Join-Path $pack 'services_not_running.csv') -NoTypeInformation
            dsregcmd /status 2>&1 | Out-File (Join-Path $pack 'dsregcmd.txt')
            try { Get-WinEvent -LogName System -MaxEvents 50 | Select-Object TimeCreated,Id,LevelDisplayName,ProviderName,Message | Export-Csv (Join-Path $pack 'system_events.csv') -NoTypeInformation } catch {}
            Compress-Archive -Path "$pack\*" -DestinationPath "$pack.zip" -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Evidence pack' -Result $(if (Test-Path "$pack.zip") {"$pack.zip"} else {$pack}) -Status 'PASS' -Detail 'Attach to ticket'
        }

        if ($Name -match 'Remote-Support-Connector') {
            $qa = Get-AppxPackage *QuickAssist* -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Quick Assist' -Result $(if ($qa) {'Installed'} else {'Missing'}) -Status $(if ($qa) {'PASS'} else {'WARN'}) -Detail ''
            $ra = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Remote Assistance' -ErrorAction SilentlyContinue).fAllowToGetHelp
            Add-ReportRow -Check 'Remote Assistance allow' -Result "$ra" -Status 'INFO' -Detail 'fAllowToGetHelp'
        }

        if ($Name -match 'Escalation-Readiness') {
            $checks = @{
                'Admin elevation' = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
                'Network Up' = [bool](Get-NetAdapter | Where-Object Status -eq 'Up')
                'Disk free >10%' = [bool]((Get-PSDrive C).Free / ((Get-PSDrive C).Used + (Get-PSDrive C).Free) -gt 0.1)
            }
            foreach ($k in $checks.Keys) {
                Add-ReportRow -Check $k -Result "$($checks[$k])" -Status $(if ($checks[$k]) {'PASS'} else {'FAIL'}) -Detail 'Escalation gate'
            }
        }

        if ($Name -match 'Quick-Fix-Menu') {
            Add-ReportRow -Check 'Menu mode' -Result 'Non-interactive catalog run' -Status 'INFO' -Detail 'Invoked common repair set; MASTER-MENU provides interactive picker'
            Add-ReportRow -Check 'Recommended next' -Result 'One-Click-Common-Repairs / All-In-One-Triage' -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Toolkit complete'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Remote Diagnostics Packager' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Diagnostics Packager' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
