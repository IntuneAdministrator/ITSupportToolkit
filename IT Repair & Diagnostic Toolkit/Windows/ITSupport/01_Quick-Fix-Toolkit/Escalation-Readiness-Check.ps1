...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Escalation Readiness Check
.DESCRIPTION
    Production-grade Windows IT Support tool for Escalation-Readiness-Check.
...
.EXAMPLE
    .\Escalation-Readiness-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Escalation-Readiness-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Escalation Readiness Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Escalation-Readiness-Check'
        Add-ReportRow -Check 'Tool' -Result 'Escalation-Readiness-Check' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
    ...
    Export-HtmlReport -Title 'Escalation Readiness Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Escalation Readiness Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
