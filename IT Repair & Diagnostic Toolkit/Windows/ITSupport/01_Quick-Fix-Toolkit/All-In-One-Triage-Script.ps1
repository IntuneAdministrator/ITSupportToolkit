...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: All In One Triage Script
.DESCRIPTION
    Production-grade Windows IT Support tool for All-In-One-Triage-Script.
...
.EXAMPLE
    .\All-In-One-Triage-Script.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\All-In-One-Triage-Script.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'All In One Triage Script'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'All-In-One-Triage-Script'
        Add-ReportRow -Check 'Tool' -Result 'All-In-One-Triage-Script' -Status 'INFO' -Detail 'ITSupport/01_Quick-Fix-Toolkit'
        Write-SectionBanner 'QUICK FIX TOOLKIT'
    ...
    Export-HtmlReport -Title 'All In One Triage Script' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'All In One Triage Script' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
