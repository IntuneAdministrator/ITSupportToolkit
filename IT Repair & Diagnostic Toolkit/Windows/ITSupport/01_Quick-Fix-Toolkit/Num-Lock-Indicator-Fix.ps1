...
<#
.SYNOPSIS
    Common problem fix: Num Lock Indicator Fix
.DESCRIPTION
...
.EXAMPLE
    .\Num-Lock-Indicator-Fix.ps1
.EXAMPLE
    .\Num-Lock-Indicator-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Num Lock Indicator Fix'; $totalSteps = 8; $Name = 'Num-Lock-Indicator-Fix'
try {
    ...
    Export-HtmlReport -Title 'Num Lock Indicator Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Num Lock Indicator Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
