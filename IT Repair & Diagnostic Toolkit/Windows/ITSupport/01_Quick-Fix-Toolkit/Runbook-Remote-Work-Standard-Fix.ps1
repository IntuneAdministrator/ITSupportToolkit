...
<#
.SYNOPSIS
    Common problem fix: Runbook Remote Work Standard Fix
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Remote-Work-Standard-Fix.ps1
.EXAMPLE
    .\Runbook-Remote-Work-Standard-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Remote Work Standard Fix'; $totalSteps = 8; $Name = 'Runbook-Remote-Work-Standard-Fix'
try {
    ...
    Export-HtmlReport -Title 'Runbook Remote Work Standard Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Remote Work Standard Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
