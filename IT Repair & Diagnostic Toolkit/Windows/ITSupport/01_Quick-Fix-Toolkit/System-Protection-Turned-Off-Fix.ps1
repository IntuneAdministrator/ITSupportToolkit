...
<#
.SYNOPSIS
    Common problem fix: System Protection Turned Off Fix
.DESCRIPTION
...
.EXAMPLE
    .\System-Protection-Turned-Off-Fix
.EXAMPLE
    .\System-Protection-Turned-Off-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'System Protection Turned Off Fix'; $totalSteps = 8; $Name = 'System-Protection-Turned-Off-Fix'
try {
    ...
    Export-HtmlReport -Title 'System Protection Turned Off Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'System Protection Turned Off Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
