...
<#
.SYNOPSIS
    Common problem fix: Product Key Invalid Triage
.DESCRIPTION
...
.EXAMPLE
    .\Product-Key-Invalid-Triage
.EXAMPLE
    .\Product-Key-Invalid-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Product Key Invalid Triage'; $totalSteps = 8; $Name = 'Product-Key-Invalid-Triage'
try {
    ...
    Export-HtmlReport -Title 'Product Key Invalid Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Product Key Invalid Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
