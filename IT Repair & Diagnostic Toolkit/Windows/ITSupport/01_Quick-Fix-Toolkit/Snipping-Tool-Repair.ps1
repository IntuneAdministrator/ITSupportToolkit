...
<#
.SYNOPSIS
    Common problem fix: Snipping Tool Repair
.DESCRIPTION
...
.EXAMPLE
    .\Snipping-Tool-Repair.ps1
.EXAMPLE
    .\Snipping-Tool-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Snipping Tool Repair'; $totalSteps = 8; $Name = 'Snipping-Tool-Repair'
try {
    ...
    Export-HtmlReport -Title 'Snipping Tool Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Snipping Tool Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
