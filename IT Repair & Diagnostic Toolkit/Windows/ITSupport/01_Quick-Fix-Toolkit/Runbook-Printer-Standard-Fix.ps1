...
<#
.SYNOPSIS
    Common problem fix: Runbook Printer Standard Fix
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Printer-Standard-Fix.ps1
.EXAMPLE
    .\Runbook-Printer-Standard-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Printer Standard Fix'; $totalSteps = 8; $Name = 'Runbook-Printer-Standard-Fix'
try {
    ...
    Export-HtmlReport -Title 'Runbook Printer Standard Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Printer Standard Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
