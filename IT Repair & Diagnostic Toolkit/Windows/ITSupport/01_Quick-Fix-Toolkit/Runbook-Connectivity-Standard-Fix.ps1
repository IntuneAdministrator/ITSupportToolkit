...
<#
.SYNOPSIS
    Common problem fix: Runbook Connectivity Standard Fix
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Connectivity-Standard-Fix.ps1
.EXAMPLE
    .\Runbook-Connectivity-Standard-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Connectivity Standard Fix'; $totalSteps = 8; $Name = 'Runbook-Connectivity-Standard-Fix'
try {
    ...
    Export-HtmlReport -Title 'Runbook Connectivity Standard Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Connectivity Standard Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
