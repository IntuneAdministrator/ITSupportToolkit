...
<#
.SYNOPSIS
    Enterprise IT tool: Cached Credential Clear User
.DESCRIPTION
    Phase 11 expansion (ITSupport/26_Account-Unlock-Quick). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Cached-Credential-Clear-User.ps1
.EXAMPLE
    .\Cached-Credential-Clear-User.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Cached Credential Clear User'; $totalSteps=8; $Name='Cached-Credential-Clear-User'; $Folder='26_Account-Unlock-Quick'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '26_Account-Unlock-Quick' -Status 'INFO' -Detail 'ITSupport'
    ...
    Export-HtmlReport -Title 'Cached Credential Clear User' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cached Credential Clear User' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
