...
<#
.SYNOPSIS
    Common problem fix: Account Unlock Quick User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Account-Unlock-Quick-User-Triage.ps1
.EXAMPLE
    .\Account-Unlock-Quick-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Account Unlock Quick User Triage'; $totalSteps = 8; $Name = 'Account-Unlock-Quick-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Account Unlock Quick User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Unlock Quick User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
