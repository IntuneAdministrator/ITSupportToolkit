...
<#
.SYNOPSIS
    Enterprise IT tool: Account Lockout Event Find
.DESCRIPTION
    Phase 11 expansion (ITSupport/26_Account-Unlock-Quick). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Account-Lockout-Event-Find.ps1
.EXAMPLE
    .\Account-Lockout-Event-Find.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Account Lockout Event Find'; $totalSteps=8; $Name='Account-Lockout-Event-Find'; $Folder='26_Account-Unlock-Quick'
try {
    ...
    Export-HtmlReport -Title 'Account Lockout Event Find' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Lockout Event Find' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
