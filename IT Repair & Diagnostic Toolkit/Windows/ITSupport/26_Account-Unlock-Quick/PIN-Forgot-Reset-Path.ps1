...
$exitCode=0; $Activity='PIN Forgot Reset Path'; $totalSteps=8; $Name='PIN-Forgot-Reset-Path'; $Folder='26_Account-Unlock-Quick'
try {
    ...
    Export-HtmlReport -Title 'PIN Forgot Reset Path' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'PIN Forgot Reset Path' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
