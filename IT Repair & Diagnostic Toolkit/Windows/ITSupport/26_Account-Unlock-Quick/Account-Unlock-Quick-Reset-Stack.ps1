...
<#
.SYNOPSIS
    Common problem fix: Account Unlock Quick Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Account-Unlock-Quick-Reset-Stack.ps1
.EXAMPLE
    .\Account-Unlock-Quick-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Account Unlock Quick Reset Stack'; $totalSteps = 8; $Name = 'Account-Unlock-Quick-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Account Unlock Quick Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Unlock Quick Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
