...
<#
.SYNOPSIS
    Enterprise IT tool: Local Account Unlock Guide
.DESCRIPTION
    Phase 11 expansion (ITSupport/26_Account-Unlock-Quick). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Local-Account-Unlock-Guide.ps1
.EXAMPLE
    .\Local-Account-Unlock-Guide.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Local Account Unlock Guide'; $totalSteps=8; $Name='Local-Account-Unlock-Guide'; $Folder='26_Account-Unlock-Quick'
try {
    ...
    Export-HtmlReport -Title 'Local Account Unlock Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Local Account Unlock Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
