...
<#
.SYNOPSIS
    Common problem fix: Account Unlock Quick Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Account-Unlock-Quick-Issue-Detect.ps1
.EXAMPLE
    .\Account-Unlock-Quick-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Account Unlock Quick Issue Detect'; $totalSteps = 8; $Name = 'Account-Unlock-Quick-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Account Unlock Quick Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Unlock Quick Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
