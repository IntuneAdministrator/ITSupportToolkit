...
<#
.SYNOPSIS
    Common problem fix: Account Unlock Quick Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 26_Account-Unlock-Quick). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Account-Unlock-Quick-Helpdesk-Report.ps1
.EXAMPLE
    .\Account-Unlock-Quick-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Account Unlock Quick Helpdesk Report'; $totalSteps = 8; $Name = 'Account-Unlock-Quick-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '26_Account-Unlock-Quick' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '26_Account-Unlock-Quick'
    ...
    Export-HtmlReport -Title 'Account Unlock Quick Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Unlock Quick Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
