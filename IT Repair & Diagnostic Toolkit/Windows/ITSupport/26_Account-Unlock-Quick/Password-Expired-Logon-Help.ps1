...
<#
.SYNOPSIS
    Enterprise IT tool: Password Expired Logon Help
.DESCRIPTION
    Phase 11 expansion (ITSupport/26_Account-Unlock-Quick). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Password-Expired-Logon-Help.ps1
.EXAMPLE
    .\Password-Expired-Logon-Help.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Password Expired Logon Help'; $totalSteps=8; $Name='Password-Expired-Logon-Help'; $Folder='26_Account-Unlock-Quick'
try {
    ...
    Export-HtmlReport -Title 'Password Expired Logon Help' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Password Expired Logon Help' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
