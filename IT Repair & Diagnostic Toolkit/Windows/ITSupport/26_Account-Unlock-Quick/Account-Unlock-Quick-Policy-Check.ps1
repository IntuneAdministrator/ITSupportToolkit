...
<#
.SYNOPSIS
    Common problem fix: Account Unlock Quick Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Account-Unlock-Quick-Policy-Check.ps1
.EXAMPLE
    .\Account-Unlock-Quick-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Account Unlock Quick Policy Check'; $totalSteps = 8; $Name = 'Account-Unlock-Quick-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Account Unlock Quick Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Unlock Quick Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
