...
<#
.SYNOPSIS
    Common problem fix: Mapped Drive Red X Fix
.DESCRIPTION
...
.EXAMPLE
    .\Mapped-Drive-Red-X-Fix
.EXAMPLE
    .\Mapped-Drive-Red-X-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Mapped Drive Red X Fix'; $totalSteps = 8; $Name = 'Mapped-Drive-Red-X-Fix'
try {
    ...
    Export-HtmlReport -Title 'Mapped Drive Red X Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Mapped Drive Red X Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
