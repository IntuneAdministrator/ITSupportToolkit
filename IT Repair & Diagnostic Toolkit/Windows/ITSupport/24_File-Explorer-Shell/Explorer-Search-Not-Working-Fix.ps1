...
<#
.SYNOPSIS
    Common problem fix: Explorer Search Not Working Fix
.DESCRIPTION
...
.EXAMPLE
    .\Explorer-Search-Not-Working-Fix
.EXAMPLE
    .\Explorer-Search-Not-Working-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Explorer Search Not Working Fix'; $totalSteps = 8; $Name = 'Explorer-Search-Not-Working-Fix'
try {
    ...
    Export-HtmlReport -Title 'Explorer Search Not Working Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Explorer Search Not Working Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
