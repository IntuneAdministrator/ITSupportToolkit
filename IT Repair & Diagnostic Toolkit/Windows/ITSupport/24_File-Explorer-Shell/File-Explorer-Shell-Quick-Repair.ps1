...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-Quick-Repair.ps1
.EXAMPLE
    .\File-Explorer-Shell-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell Quick Repair'; $totalSteps = 8; $Name = 'File-Explorer-Shell-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
