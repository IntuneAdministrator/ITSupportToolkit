...
<#
.SYNOPSIS
    Common problem fix: File In Use Cant Delete Fix
.DESCRIPTION
...
.EXAMPLE
    .\File-In-Use-Cant-Delete-Fix
.EXAMPLE
    .\File-In-Use-Cant-Delete-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File In Use Cant Delete Fix'; $totalSteps = 8; $Name = 'File-In-Use-Cant-Delete-Fix'
try {
    ...
    Export-HtmlReport -Title 'File In Use Cant Delete Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File In Use Cant Delete Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
