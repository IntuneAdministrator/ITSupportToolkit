...
<#
.SYNOPSIS
    Enterprise IT tool: Explorer Crash Loop Fix
.DESCRIPTION
    Phase 11 expansion (ITSupport/24_File-Explorer-Shell). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Explorer-Crash-Loop-Fix.ps1
.EXAMPLE
    .\Explorer-Crash-Loop-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Explorer Crash Loop Fix'; $totalSteps=8; $Name='Explorer-Crash-Loop-Fix'; $Folder='24_File-Explorer-Shell'
try {
    ...
    Export-HtmlReport -Title 'Explorer Crash Loop Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Explorer Crash Loop Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
