...
<#
.SYNOPSIS
    Common problem fix: Recycle Bin Corrupt Repair
.DESCRIPTION
...
.EXAMPLE
    .\Recycle-Bin-Corrupt-Repair.ps1
.EXAMPLE
    .\Recycle-Bin-Corrupt-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Recycle Bin Corrupt Repair'; $totalSteps = 8; $Name = 'Recycle-Bin-Corrupt-Repair'
try {
    ...
    Export-HtmlReport -Title 'Recycle Bin Corrupt Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Recycle Bin Corrupt Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
