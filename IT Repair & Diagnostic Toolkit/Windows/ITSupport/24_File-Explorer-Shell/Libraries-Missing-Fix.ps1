...
<#
.SYNOPSIS
    Common problem fix: Libraries Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Libraries-Missing-Fix
.EXAMPLE
    .\Libraries-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Libraries Missing Fix'; $totalSteps = 8; $Name = 'Libraries-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Libraries Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Libraries Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
