...
<#
.SYNOPSIS
    Enterprise IT tool: Thumbnail Cache Rebuild
.DESCRIPTION
    Phase 11 expansion (ITSupport/24_File-Explorer-Shell). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Thumbnail-Cache-Rebuild.ps1
.EXAMPLE
    .\Thumbnail-Cache-Rebuild.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Thumbnail Cache Rebuild'; $totalSteps=8; $Name='Thumbnail-Cache-Rebuild'; $Folder='24_File-Explorer-Shell'
try {
    ...
    Export-HtmlReport -Title 'Thumbnail Cache Rebuild' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Thumbnail Cache Rebuild' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
