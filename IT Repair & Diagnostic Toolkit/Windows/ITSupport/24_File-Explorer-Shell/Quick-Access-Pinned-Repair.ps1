...
<#
.SYNOPSIS
    Enterprise IT tool: Quick Access Pinned Repair
.DESCRIPTION
    Phase 11 expansion (ITSupport/24_File-Explorer-Shell). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Quick-Access-Pinned-Repair.ps1
.EXAMPLE
    .\Quick-Access-Pinned-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Quick Access Pinned Repair'; $totalSteps=8; $Name='Quick-Access-Pinned-Repair'; $Folder='24_File-Explorer-Shell'
try {
    ...
    Export-HtmlReport -Title 'Quick Access Pinned Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Access Pinned Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
