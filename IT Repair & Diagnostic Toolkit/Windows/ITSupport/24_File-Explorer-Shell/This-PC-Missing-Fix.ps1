...
<#
.SYNOPSIS
    Common problem fix: This PC Missing Fix
.DESCRIPTION
...
$exitCode = 0; $Activity = 'This PC Missing Fix'; $totalSteps = 8; $Name = 'This-PC-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'This PC Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'This PC Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
