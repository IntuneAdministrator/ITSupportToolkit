...
<#
.SYNOPSIS
    Common problem fix: SharePoint Shortcut Broken Fix
.DESCRIPTION
...
.EXAMPLE
    .\SharePoint-Shortcut-Broken-Fix
.EXAMPLE
    .\SharePoint-Shortcut-Broken-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SharePoint Shortcut Broken Fix'; $totalSteps = 8; $Name = 'SharePoint-Shortcut-Broken-Fix'
try {
    ...
    Export-HtmlReport -Title 'SharePoint Shortcut Broken Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SharePoint Shortcut Broken Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
