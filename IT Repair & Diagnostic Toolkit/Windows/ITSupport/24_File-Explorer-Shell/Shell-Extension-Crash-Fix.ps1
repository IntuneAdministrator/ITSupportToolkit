...
<#
.SYNOPSIS
    Common problem fix: Shell Extension Crash Fix
.DESCRIPTION
...
.EXAMPLE
    .\Shell-Extension-Crash-Fix
.EXAMPLE
    .\Shell-Extension-Crash-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Shell Extension Crash Fix'; $totalSteps = 8; $Name = 'Shell-Extension-Crash-Fix'
try {
    ...
    Export-HtmlReport -Title 'Shell Extension Crash Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shell Extension Crash Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
