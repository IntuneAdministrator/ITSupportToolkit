...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-Reset-Stack.ps1
.EXAMPLE
    .\File-Explorer-Shell-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell Reset Stack'; $totalSteps = 8; $Name = 'File-Explorer-Shell-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
