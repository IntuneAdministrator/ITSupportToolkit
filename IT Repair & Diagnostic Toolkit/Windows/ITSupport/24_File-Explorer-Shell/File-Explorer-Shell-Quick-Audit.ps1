...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-Quick-Audit.ps1
.EXAMPLE
    .\File-Explorer-Shell-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell Quick Audit'; $totalSteps = 8; $Name = 'File-Explorer-Shell-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
