...
<#
.SYNOPSIS
    Enterprise IT tool: Context Menu Slow Audit
.DESCRIPTION
    Phase 11 expansion (ITSupport/24_File-Explorer-Shell). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Context-Menu-Slow-Audit.ps1
.EXAMPLE
    .\Context-Menu-Slow-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Context Menu Slow Audit'; $totalSteps=8; $Name='Context-Menu-Slow-Audit'; $Folder='24_File-Explorer-Shell'
try {
    ...
    Export-HtmlReport -Title 'Context Menu Slow Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Context Menu Slow Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
