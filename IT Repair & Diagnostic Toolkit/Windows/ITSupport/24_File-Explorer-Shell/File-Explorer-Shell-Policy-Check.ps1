...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-Policy-Check.ps1
.EXAMPLE
    .\File-Explorer-Shell-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell Policy Check'; $totalSteps = 8; $Name = 'File-Explorer-Shell-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
