...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-Helpdesk-Report.ps1
.EXAMPLE
    .\File-Explorer-Shell-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell Helpdesk Report'; $totalSteps = 8; $Name = 'File-Explorer-Shell-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
