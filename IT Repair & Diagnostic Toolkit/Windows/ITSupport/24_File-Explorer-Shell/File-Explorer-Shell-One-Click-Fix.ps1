...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-One-Click-Fix.ps1
.EXAMPLE
    .\File-Explorer-Shell-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell One Click Fix'; $totalSteps = 8; $Name = 'File-Explorer-Shell-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
