...
<#
.SYNOPSIS
    Common problem fix: Path Too Long Fix
.DESCRIPTION
...
$exitCode = 0; $Activity = 'Path Too Long Fix'; $totalSteps = 8; $Name = 'Path-Too-Long-Fix'
try {
    ...
    Export-HtmlReport -Title 'Path Too Long Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Path Too Long Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
