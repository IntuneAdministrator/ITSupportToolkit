...
<#
.SYNOPSIS
    Common problem fix: File Association Reset Default
.DESCRIPTION
...
.EXAMPLE
    .\File-Association-Reset-Default.ps1
.EXAMPLE
    .\File-Association-Reset-Default.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Association Reset Default'; $totalSteps = 8; $Name = 'File-Association-Reset-Default'
try {
    ...
    Export-HtmlReport -Title 'File Association Reset Default' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Association Reset Default' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
