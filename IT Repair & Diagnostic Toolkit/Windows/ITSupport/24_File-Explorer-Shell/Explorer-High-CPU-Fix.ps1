...
<#
.SYNOPSIS
    Common problem fix: Explorer High CPU Fix
.DESCRIPTION
...
.EXAMPLE
    .\Explorer-High-CPU-Fix
.EXAMPLE
    .\Explorer-High-CPU-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Explorer High CPU Fix'; $totalSteps = 8; $Name = 'Explorer-High-CPU-Fix'
try {
    ...
    Export-HtmlReport -Title 'Explorer High CPU Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Explorer High CPU Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
