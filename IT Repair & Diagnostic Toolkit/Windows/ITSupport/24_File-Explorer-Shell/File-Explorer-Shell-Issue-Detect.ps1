...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-Issue-Detect.ps1
.EXAMPLE
    .\File-Explorer-Shell-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell Issue Detect'; $totalSteps = 8; $Name = 'File-Explorer-Shell-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
