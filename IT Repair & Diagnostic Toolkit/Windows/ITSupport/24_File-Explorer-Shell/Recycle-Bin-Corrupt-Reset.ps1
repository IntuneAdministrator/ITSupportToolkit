...
<#
.SYNOPSIS
    Enterprise IT tool: Recycle Bin Corrupt Reset
.DESCRIPTION
    Phase 11 expansion (ITSupport/24_File-Explorer-Shell). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Recycle-Bin-Corrupt-Reset.ps1
.EXAMPLE
    .\Recycle-Bin-Corrupt-Reset.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Recycle Bin Corrupt Reset'; $totalSteps=8; $Name='Recycle-Bin-Corrupt-Reset'; $Folder='24_File-Explorer-Shell'
try {
    ...
    Export-HtmlReport -Title 'Recycle Bin Corrupt Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Recycle Bin Corrupt Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
