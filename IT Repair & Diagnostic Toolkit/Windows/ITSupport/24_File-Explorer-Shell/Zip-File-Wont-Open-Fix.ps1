...
<#
.SYNOPSIS
    Common problem fix: Zip File Wont Open Fix
.DESCRIPTION
...
.EXAMPLE
    .\Zip-File-Wont-Open-Fix
.EXAMPLE
    .\Zip-File-Wont-Open-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Zip File Wont Open Fix'; $totalSteps = 8; $Name = 'Zip-File-Wont-Open-Fix'
try {
    ...
    Export-HtmlReport -Title 'Zip File Wont Open Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Zip File Wont Open Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
