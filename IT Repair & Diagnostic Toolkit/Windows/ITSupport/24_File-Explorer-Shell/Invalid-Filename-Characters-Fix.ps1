...
<#
.SYNOPSIS
    Common problem fix: Invalid Filename Characters Fix
.DESCRIPTION
...
.EXAMPLE
    .\Invalid-Filename-Characters-Fix
.EXAMPLE
    .\Invalid-Filename-Characters-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Invalid Filename Characters Fix'; $totalSteps = 8; $Name = 'Invalid-Filename-Characters-Fix'
try {
    ...
    Export-HtmlReport -Title 'Invalid Filename Characters Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Invalid Filename Characters Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
