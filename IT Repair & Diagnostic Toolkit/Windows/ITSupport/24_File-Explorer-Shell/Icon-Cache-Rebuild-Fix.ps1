...
<#
.SYNOPSIS
    Common problem fix: Icon Cache Rebuild Fix
.DESCRIPTION
...
.EXAMPLE
    .\Icon-Cache-Rebuild-Fix
.EXAMPLE
    .\Icon-Cache-Rebuild-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Icon Cache Rebuild Fix'; $totalSteps = 8; $Name = 'Icon-Cache-Rebuild-Fix'
try {
    ...
    Export-HtmlReport -Title 'Icon Cache Rebuild Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Icon Cache Rebuild Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
