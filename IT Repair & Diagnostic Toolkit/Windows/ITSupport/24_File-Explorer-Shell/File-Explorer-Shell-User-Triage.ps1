...
<#
.SYNOPSIS
    Common problem fix: File Explorer Shell User Triage
.DESCRIPTION
...
.EXAMPLE
    .\File-Explorer-Shell-User-Triage.ps1
.EXAMPLE
    .\File-Explorer-Shell-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Explorer Shell User Triage'; $totalSteps = 8; $Name = 'File-Explorer-Shell-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'File Explorer Shell User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Explorer Shell User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
