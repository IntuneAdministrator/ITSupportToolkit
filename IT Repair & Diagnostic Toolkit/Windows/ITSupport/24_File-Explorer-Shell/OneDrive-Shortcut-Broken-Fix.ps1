...
<#
.SYNOPSIS
    Common problem fix: OneDrive Shortcut Broken Fix
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-Shortcut-Broken-Fix
.EXAMPLE
    .\OneDrive-Shortcut-Broken-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive Shortcut Broken Fix'; $totalSteps = 8; $Name = 'OneDrive-Shortcut-Broken-Fix'
try {
    ...
    Export-HtmlReport -Title 'OneDrive Shortcut Broken Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive Shortcut Broken Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
