...
<#
.SYNOPSIS
    Common problem fix: Quick Access Pins Missing
.DESCRIPTION
...
.EXAMPLE
    .\Quick-Access-Pins-Missing.ps1
.EXAMPLE
    .\Quick-Access-Pins-Missing.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quick Access Pins Missing'; $totalSteps = 8; $Name = 'Quick-Access-Pins-Missing'
try {
    ...
    Export-HtmlReport -Title 'Quick Access Pins Missing' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Access Pins Missing' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
