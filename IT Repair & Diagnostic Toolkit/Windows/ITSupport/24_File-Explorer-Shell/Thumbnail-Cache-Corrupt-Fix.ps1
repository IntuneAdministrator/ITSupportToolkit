...
<#
.SYNOPSIS
    Common problem fix: Thumbnail Cache Corrupt Fix
.DESCRIPTION
...
.EXAMPLE
    .\Thumbnail-Cache-Corrupt-Fix
.EXAMPLE
    .\Thumbnail-Cache-Corrupt-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Thumbnail Cache Corrupt Fix'; $totalSteps = 8; $Name = 'Thumbnail-Cache-Corrupt-Fix'
try {
    ...
    Export-HtmlReport -Title 'Thumbnail Cache Corrupt Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Thumbnail Cache Corrupt Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
