...
<#
.SYNOPSIS
    Common problem fix: Network Drive Disconnect Fix
.DESCRIPTION
...
.EXAMPLE
    .\Network-Drive-Disconnect-Fix.ps1
.EXAMPLE
    .\Network-Drive-Disconnect-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Drive Disconnect Fix'; $totalSteps = 8; $Name = 'Network-Drive-Disconnect-Fix'
try {
    ...
    Export-HtmlReport -Title 'Network Drive Disconnect Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Drive Disconnect Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
