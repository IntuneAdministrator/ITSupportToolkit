...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Security-Awareness-Helpdesk-Reset-Stack.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk Reset Stack'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
