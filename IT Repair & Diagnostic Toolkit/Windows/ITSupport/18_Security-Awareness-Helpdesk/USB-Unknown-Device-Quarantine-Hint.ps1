...
<#
.SYNOPSIS
    Enterprise IT tool: USB Unknown Device Quarantine Hint
.DESCRIPTION
    Phase 10 expansion (ITSupport/18_Security-Awareness-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\USB-Unknown-Device-Quarantine-Hint.ps1
.EXAMPLE
    .\USB-Unknown-Device-Quarantine-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='USB Unknown Device Quarantine Hint'; $totalSteps=8; $Name='USB-Unknown-Device-Quarantine-Hint'; $Folder='18_Security-Awareness-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'USB Unknown Device Quarantine Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Unknown Device Quarantine Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
