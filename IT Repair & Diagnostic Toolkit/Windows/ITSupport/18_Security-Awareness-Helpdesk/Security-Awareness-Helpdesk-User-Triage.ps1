...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Security-Awareness-Helpdesk-User-Triage.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk User Triage'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
