...
<#
.SYNOPSIS
    Enterprise IT tool: BitLocker Recovery Key Locate Guide
.DESCRIPTION
    Phase 10 expansion (ITSupport/18_Security-Awareness-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\BitLocker-Recovery-Key-Locate-Guide.ps1
.EXAMPLE
    .\BitLocker-Recovery-Key-Locate-Guide.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='BitLocker Recovery Key Locate Guide'; $totalSteps=8; $Name='BitLocker-Recovery-Key-Locate-Guide'; $Folder='18_Security-Awareness-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'BitLocker Recovery Key Locate Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BitLocker Recovery Key Locate Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
