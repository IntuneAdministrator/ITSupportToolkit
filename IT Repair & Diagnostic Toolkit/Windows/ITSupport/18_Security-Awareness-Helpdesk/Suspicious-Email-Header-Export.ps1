...
<#
.SYNOPSIS
    Enterprise IT tool: Suspicious Email Header Export
.DESCRIPTION
    Phase 10 expansion (ITSupport/18_Security-Awareness-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Suspicious-Email-Header-Export.ps1
.EXAMPLE
    .\Suspicious-Email-Header-Export.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Suspicious Email Header Export'; $totalSteps=8; $Name='Suspicious-Email-Header-Export'; $Folder='18_Security-Awareness-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Suspicious Email Header Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Suspicious Email Header Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
