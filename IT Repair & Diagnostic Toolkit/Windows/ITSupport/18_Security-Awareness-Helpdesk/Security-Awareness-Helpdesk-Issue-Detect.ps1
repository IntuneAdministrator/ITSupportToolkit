...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Security-Awareness-Helpdesk-Issue-Detect.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk Issue Detect'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
