...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Security-Awareness-Helpdesk-Quick-Audit.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk Quick Audit'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
