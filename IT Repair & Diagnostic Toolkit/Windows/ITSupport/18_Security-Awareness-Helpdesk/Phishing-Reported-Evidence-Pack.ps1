...
<#
.SYNOPSIS
    Enterprise IT tool: Phishing Reported Evidence Pack
.DESCRIPTION
    Phase 10 expansion (ITSupport/18_Security-Awareness-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Phishing-Reported-Evidence-Pack.ps1
.EXAMPLE
    .\Phishing-Reported-Evidence-Pack.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Phishing Reported Evidence Pack'; $totalSteps=8; $Name='Phishing-Reported-Evidence-Pack'; $Folder='18_Security-Awareness-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Phishing Reported Evidence Pack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Phishing Reported Evidence Pack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
