...
$exitCode=0; $Activity='Screen Lock Policy Verify'; $totalSteps=8; $Name='Screen-Lock-Policy-Verify'; $Folder='18_Security-Awareness-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Screen Lock Policy Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Screen Lock Policy Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
