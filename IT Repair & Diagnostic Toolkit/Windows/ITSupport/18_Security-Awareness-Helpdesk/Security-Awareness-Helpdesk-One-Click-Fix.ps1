...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Security-Awareness-Helpdesk-One-Click-Fix.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk One Click Fix'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
