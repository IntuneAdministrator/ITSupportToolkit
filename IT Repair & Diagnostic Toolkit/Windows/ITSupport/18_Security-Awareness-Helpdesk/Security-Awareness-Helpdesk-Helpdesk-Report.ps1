...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 18_Security-Awareness-Helpdesk). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Security-Awareness-Helpdesk-Helpdesk-Report.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk Helpdesk Report'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '18_Security-Awareness-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '18_Security-Awareness-Helpdesk'
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
