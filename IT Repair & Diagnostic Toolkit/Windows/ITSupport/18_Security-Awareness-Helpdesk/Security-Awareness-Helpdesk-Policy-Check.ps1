...
<#
.SYNOPSIS
    Common problem fix: Security Awareness Helpdesk Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Security-Awareness-Helpdesk-Policy-Check.ps1
.EXAMPLE
    .\Security-Awareness-Helpdesk-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Awareness Helpdesk Policy Check'; $totalSteps = 8; $Name = 'Security-Awareness-Helpdesk-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Security Awareness Helpdesk Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Awareness Helpdesk Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
