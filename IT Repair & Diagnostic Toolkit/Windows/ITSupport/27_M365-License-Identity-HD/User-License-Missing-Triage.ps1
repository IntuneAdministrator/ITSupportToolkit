...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: User License Missing Triage
.DESCRIPTION
    Phase 12 M365 tenant admin (ITSupport/27_M365-License-Identity-HD). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\User-License-Missing-Triage.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\User-License-Missing-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User License Missing Triage'; $totalSteps = 8; $Name = 'User-License-Missing-Triage'; $Folder = '27_M365-License-Identity-HD'
$DefaultScopes = @('Organization.Read.All','User.Read.All','Directory.Read.All')
try {
    ...
    Add-ReportRow -Check 'Module' -Result '27_M365-License-Identity-HD' -Status 'INFO' -Detail 'ITSupport Phase12 M365'
    Add-M365PortalRows
    ...
    Export-HtmlReport -Title 'User License Missing Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User License Missing Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
