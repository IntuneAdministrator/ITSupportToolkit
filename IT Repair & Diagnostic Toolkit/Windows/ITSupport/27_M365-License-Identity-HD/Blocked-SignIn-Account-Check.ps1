...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Blocked SignIn Account Check
.DESCRIPTION
...
.EXAMPLE
    .\Blocked-SignIn-Account-Check.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Blocked-SignIn-Account-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Blocked SignIn Account Check'; $totalSteps = 8; $Name = 'Blocked-SignIn-Account-Check'; $Folder = '27_M365-License-Identity-HD'
$DefaultScopes = @('ServiceHealth.Read.All','Directory.Read.All')
try {
    ...
    Export-HtmlReport -Title 'Blocked SignIn Account Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Blocked SignIn Account Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
