...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Password Reset Cloud Path
.DESCRIPTION
...
.EXAMPLE
    .\Password-Reset-Cloud-Path.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Password-Reset-Cloud-Path.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Password Reset Cloud Path'; $totalSteps = 8; $Name = 'Password-Reset-Cloud-Path'; $Folder = '27_M365-License-Identity-HD'
...
try {
    ...
    Export-HtmlReport -Title 'Password Reset Cloud Path' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Password Reset Cloud Path' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
