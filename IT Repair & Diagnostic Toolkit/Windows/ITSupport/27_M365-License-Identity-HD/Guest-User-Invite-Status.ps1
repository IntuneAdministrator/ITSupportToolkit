...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Guest User Invite Status
.DESCRIPTION
...
.EXAMPLE
    .\Guest-User-Invite-Status.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Guest-User-Invite-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Guest User Invite Status'; $totalSteps = 8; $Name = 'Guest-User-Invite-Status'; $Folder = '27_M365-License-Identity-HD'
...
try {
    ...
    Export-HtmlReport -Title 'Guest User Invite Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Guest User Invite Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
