...
<#
.SYNOPSIS
    Enterprise IT tool: Company Portal Reinstall Quick
.DESCRIPTION
    Phase 10 expansion (ITSupport/19_Onboarding-Offboarding-Quick). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Company-Portal-Reinstall-Quick.ps1
.EXAMPLE
    .\Company-Portal-Reinstall-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Company Portal Reinstall Quick'; $totalSteps=8; $Name='Company-Portal-Reinstall-Quick'; $Folder='19_Onboarding-Offboarding-Quick'
try {
    ...
    Export-HtmlReport -Title 'Company Portal Reinstall Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Company Portal Reinstall Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
