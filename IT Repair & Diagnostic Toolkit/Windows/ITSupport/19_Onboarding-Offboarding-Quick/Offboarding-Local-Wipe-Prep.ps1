...
<#
.SYNOPSIS
    Enterprise IT tool: Offboarding Local Wipe Prep
.DESCRIPTION
    Phase 10 expansion (ITSupport/19_Onboarding-Offboarding-Quick). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Offboarding-Local-Wipe-Prep.ps1
.EXAMPLE
    .\Offboarding-Local-Wipe-Prep.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Offboarding Local Wipe Prep'; $totalSteps=8; $Name='Offboarding-Local-Wipe-Prep'; $Folder='19_Onboarding-Offboarding-Quick'
try {
    ...
    Export-HtmlReport -Title 'Offboarding Local Wipe Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Offboarding Local Wipe Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
