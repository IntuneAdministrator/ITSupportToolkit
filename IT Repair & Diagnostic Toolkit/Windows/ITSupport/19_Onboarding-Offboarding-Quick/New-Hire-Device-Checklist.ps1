...
$exitCode=0; $Activity='New Hire Device Checklist'; $totalSteps=8; $Name='New-Hire-Device-Checklist'; $Folder='19_Onboarding-Offboarding-Quick'
try {
    ...
    Export-HtmlReport -Title 'New Hire Device Checklist' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'New Hire Device Checklist' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
