...
<#
.SYNOPSIS
    Common problem fix: Onboarding Offboarding Quick Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Reset-Stack.ps1
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Onboarding Offboarding Quick Reset Stack'; $totalSteps = 8; $Name = 'Onboarding-Offboarding-Quick-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Onboarding Offboarding Quick Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Onboarding Offboarding Quick Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
