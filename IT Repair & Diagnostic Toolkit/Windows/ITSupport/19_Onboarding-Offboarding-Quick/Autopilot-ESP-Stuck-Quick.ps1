...
$exitCode=0; $Activity='Autopilot ESP Stuck Quick'; $totalSteps=8; $Name='Autopilot-ESP-Stuck-Quick'; $Folder='19_Onboarding-Offboarding-Quick'
try {
    ...
    Export-HtmlReport -Title 'Autopilot ESP Stuck Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Autopilot ESP Stuck Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
