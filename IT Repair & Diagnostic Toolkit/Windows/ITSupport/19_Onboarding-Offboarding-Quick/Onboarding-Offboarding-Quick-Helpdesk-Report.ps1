...
<#
.SYNOPSIS
    Common problem fix: Onboarding Offboarding Quick Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 19_Onboarding-Offboarding-Quick). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Helpdesk-Report.ps1
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Onboarding Offboarding Quick Helpdesk Report'; $totalSteps = 8; $Name = 'Onboarding-Offboarding-Quick-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '19_Onboarding-Offboarding-Quick' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '19_Onboarding-Offboarding-Quick'
    ...
    Export-HtmlReport -Title 'Onboarding Offboarding Quick Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Onboarding Offboarding Quick Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
