...
<#
.SYNOPSIS
    Common problem fix: Onboarding Offboarding Quick Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Issue-Detect.ps1
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Onboarding Offboarding Quick Issue Detect'; $totalSteps = 8; $Name = 'Onboarding-Offboarding-Quick-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Onboarding Offboarding Quick Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Onboarding Offboarding Quick Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
