...
<#
.SYNOPSIS
    Common problem fix: Onboarding Offboarding Quick Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Quick-Audit.ps1
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Onboarding Offboarding Quick Quick Audit'; $totalSteps = 8; $Name = 'Onboarding-Offboarding-Quick-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Onboarding Offboarding Quick Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Onboarding Offboarding Quick Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
