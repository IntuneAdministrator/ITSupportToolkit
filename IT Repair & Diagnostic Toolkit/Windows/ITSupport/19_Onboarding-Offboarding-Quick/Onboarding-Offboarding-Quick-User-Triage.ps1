...
<#
.SYNOPSIS
    Common problem fix: Onboarding Offboarding Quick User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Onboarding-Offboarding-Quick-User-Triage.ps1
.EXAMPLE
    .\Onboarding-Offboarding-Quick-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Onboarding Offboarding Quick User Triage'; $totalSteps = 8; $Name = 'Onboarding-Offboarding-Quick-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Onboarding Offboarding Quick User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Onboarding Offboarding Quick User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
