...
<#
.SYNOPSIS
    Common problem fix: Onboarding Offboarding Quick Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Policy-Check.ps1
.EXAMPLE
    .\Onboarding-Offboarding-Quick-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Onboarding Offboarding Quick Policy Check'; $totalSteps = 8; $Name = 'Onboarding-Offboarding-Quick-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Onboarding Offboarding Quick Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Onboarding Offboarding Quick Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
