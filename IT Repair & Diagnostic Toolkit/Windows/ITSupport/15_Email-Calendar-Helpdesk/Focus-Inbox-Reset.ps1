...
$exitCode=0; $Activity='Focus Inbox Reset'; $totalSteps=8; $Name='Focus-Inbox-Reset'; $Folder='15_Email-Calendar-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Focus Inbox Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Focus Inbox Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
