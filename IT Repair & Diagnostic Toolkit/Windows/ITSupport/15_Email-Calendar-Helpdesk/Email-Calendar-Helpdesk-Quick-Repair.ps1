...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Email-Calendar-Helpdesk-Quick-Repair.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk Quick Repair'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
