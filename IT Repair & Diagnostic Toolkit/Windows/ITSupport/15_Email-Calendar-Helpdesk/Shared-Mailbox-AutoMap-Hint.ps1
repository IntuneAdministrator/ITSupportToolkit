...
<#
.SYNOPSIS
    Enterprise IT tool: Shared Mailbox AutoMap Hint
.DESCRIPTION
    Phase 10 expansion (ITSupport/15_Email-Calendar-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Shared-Mailbox-AutoMap-Hint.ps1
.EXAMPLE
    .\Shared-Mailbox-AutoMap-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Shared Mailbox AutoMap Hint'; $totalSteps=8; $Name='Shared-Mailbox-AutoMap-Hint'; $Folder='15_Email-Calendar-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Shared Mailbox AutoMap Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shared Mailbox AutoMap Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
