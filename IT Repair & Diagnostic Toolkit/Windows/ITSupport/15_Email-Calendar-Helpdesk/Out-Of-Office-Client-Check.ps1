...
<#
.SYNOPSIS
    Enterprise IT tool: Out Of Office Client Check
.DESCRIPTION
    Phase 10 expansion (ITSupport/15_Email-Calendar-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Out-Of-Office-Client-Check.ps1
.EXAMPLE
    .\Out-Of-Office-Client-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Out Of Office Client Check'; $totalSteps=8; $Name='Out-Of-Office-Client-Check'; $Folder='15_Email-Calendar-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Out Of Office Client Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Out Of Office Client Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
