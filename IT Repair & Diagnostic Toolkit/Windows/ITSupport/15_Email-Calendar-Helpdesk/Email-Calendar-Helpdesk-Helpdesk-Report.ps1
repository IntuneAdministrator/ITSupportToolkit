...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 15_Email-Calendar-Helpdesk). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Email-Calendar-Helpdesk-Helpdesk-Report.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk Helpdesk Report'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '15_Email-Calendar-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '15_Email-Calendar-Helpdesk'
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
