...
<#
.SYNOPSIS
    Enterprise IT tool: Outlook Search Broken Quick
.DESCRIPTION
    Phase 10 expansion (ITSupport/15_Email-Calendar-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Outlook-Search-Broken-Quick.ps1
.EXAMPLE
    .\Outlook-Search-Broken-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Outlook Search Broken Quick'; $totalSteps=8; $Name='Outlook-Search-Broken-Quick'; $Folder='15_Email-Calendar-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Outlook Search Broken Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook Search Broken Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
