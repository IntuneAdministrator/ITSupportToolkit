...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Email-Calendar-Helpdesk-Quick-Audit.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk Quick Audit'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
