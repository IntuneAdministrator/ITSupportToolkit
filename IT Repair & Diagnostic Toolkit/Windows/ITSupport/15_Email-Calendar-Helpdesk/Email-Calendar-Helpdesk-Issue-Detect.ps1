...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Email-Calendar-Helpdesk-Issue-Detect.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk Issue Detect'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
