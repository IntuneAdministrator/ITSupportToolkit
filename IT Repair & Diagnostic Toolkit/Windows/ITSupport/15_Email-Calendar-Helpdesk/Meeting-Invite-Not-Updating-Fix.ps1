...
<#
.SYNOPSIS
    Enterprise IT tool: Meeting Invite Not Updating Fix
.DESCRIPTION
    Phase 10 expansion (ITSupport/15_Email-Calendar-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Meeting-Invite-Not-Updating-Fix.ps1
.EXAMPLE
    .\Meeting-Invite-Not-Updating-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Meeting Invite Not Updating Fix'; $totalSteps=8; $Name='Meeting-Invite-Not-Updating-Fix'; $Folder='15_Email-Calendar-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Meeting Invite Not Updating Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Meeting Invite Not Updating Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
