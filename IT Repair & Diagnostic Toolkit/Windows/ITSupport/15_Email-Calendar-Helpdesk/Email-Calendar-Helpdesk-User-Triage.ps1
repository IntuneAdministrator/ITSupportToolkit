...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Email-Calendar-Helpdesk-User-Triage.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk User Triage'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
