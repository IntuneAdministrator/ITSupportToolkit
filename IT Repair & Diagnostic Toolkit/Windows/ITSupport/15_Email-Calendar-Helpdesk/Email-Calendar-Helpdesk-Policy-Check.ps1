...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Email-Calendar-Helpdesk-Policy-Check.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk Policy Check'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
