...
<#
.SYNOPSIS
    Enterprise IT tool: Room Finder Not Showing Fix
.DESCRIPTION
    Phase 10 expansion (ITSupport/15_Email-Calendar-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Room-Finder-Not-Showing-Fix.ps1
.EXAMPLE
    .\Room-Finder-Not-Showing-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Room Finder Not Showing Fix'; $totalSteps=8; $Name='Room-Finder-Not-Showing-Fix'; $Folder='15_Email-Calendar-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Room Finder Not Showing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Room Finder Not Showing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
