...
<#
.SYNOPSIS
    Common problem fix: Email Calendar Helpdesk Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Email-Calendar-Helpdesk-Reset-Stack.ps1
.EXAMPLE
    .\Email-Calendar-Helpdesk-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Calendar Helpdesk Reset Stack'; $totalSteps = 8; $Name = 'Email-Calendar-Helpdesk-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Email Calendar Helpdesk Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Calendar Helpdesk Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
