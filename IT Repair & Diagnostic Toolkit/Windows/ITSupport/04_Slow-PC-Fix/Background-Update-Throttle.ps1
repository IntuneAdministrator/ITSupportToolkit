...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Background Update Throttle
.DESCRIPTION
    Production-grade Windows IT Support tool for Background-Update-Throttle.
Category: 04_Slow-PC-Fix. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Background-Update-Throttle.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Background-Update-Throttle.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Background Update Throttle'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SLOW PC FIX'
        $Name = 'Background-Update-Throttle'
        Add-ReportRow -Check 'Tool' -Result 'Background-Update-Throttle' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
        Write-SectionBanner 'SLOW PC QUICK FIX'
        $os = Get-CimInstance Win32_OperatingSystem
        $cpu = Get-CimInstance Win32_Processor | Select-Object -First 1
        Add-ReportRow -Check 'CPU load' -Result "$($cpu.LoadPercentage)%" -Status $(if ($cpu.LoadPercentage -lt 85) {'PASS'} else {'WARN'}) -Detail $cpu.Name
        ...
        Write-ProgressStep 2 $totalSteps $Activity 'Perf snapshot'

        if ($Name -match 'Disk-Space|PC-Slowness|Emergency-Free') {
            $c = Get-PSDrive C
            $pctFree = [int](($c.Free/($c.Free+$c.Used))*100)
            Add-ReportRow -Check 'C: free' -Result "$pctFree%" -Status $(if ($pctFree -gt 15) {'PASS'} elseif ($pctFree -gt 5) {'WARN'} else {'FAIL'}) -Detail ("{0:N1} GB free" -f ($c.Free/1GB))
            if ($PSCmdlet.ShouldProcess('Temp cleanup', 'Clear user/system temp older than 2 days')) {
                $freed = 0L
                foreach ($t in @($env:TEMP, "$env:SystemRoot\Temp")) {
                    Get-ChildItem $t -Recurse -Force -ErrorAction SilentlyContinue |
                        ...
                        ForEach-Object { try { $freed += $_.Length; Remove-Item $_.FullName -Force -EA SilentlyContinue } catch {} }
                }
                Add-ReportRow -Check 'Temp cleanup' -Result ("{0:N1} MB" -f ($freed/1MB)) -Status 'PASS' -Detail ''
            }
        }

        if ($Name -match 'High-CPU|Telemetry-CPU') {
            $top = Get-Process | Sort-Object CPU -Descending | Select-Object -First 10
            foreach ($p in $top) {
                Add-ReportRow -Check "CPU $($p.ProcessName)" -Result ("{0:N1}" -f $p.CPU) -Status $(if ($p.CPU -gt 200) {'WARN'} else {'INFO'}) -Detail "PID=$($p.Id)"
            }
            if ($Name -match 'Telemetry' -and $PSCmdlet.ShouldProcess('DiagTrack', 'Stop/disable heavy telemetry spike')) {
                Stop-Service DiagTrack -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'DiagTrack' -Result (Get-Service DiagTrack -EA SilentlyContinue).Status -Status 'INFO' -Detail 'Stopped for triage; policy may restart'
            }
            if ($Name -match 'Quick-Kill') {
                Add-ReportRow -Check 'Kill policy' -Result 'Only non-responding user apps with -WhatIf safety' -Status 'INFO' -Detail ''
                Get-Process | Where-Object { $_.Responding -eq $false -and $_.ProcessName -notmatch 'System|csrss|winlogon|services|lsass|smss' } | ForEach-Object {
                    if ($PSCmdlet.ShouldProcess("$($_.ProcessName) ($($_.Id))", 'Stop-Process')) {
                        Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue
                        Add-ReportRow -Check "Killed $($_.ProcessName)" -Result 'Stopped' -Status 'PASS' -Detail "PID $($_.Id)"
                    }
                }
            }
        }

        if ($Name -match 'Startup-Bloat') {
            $startup = @(Get-CimInstance Win32_StartupCommand -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'Startup items' -Result "$($startup.Count)" -Status $(if ($startup.Count -lt 12) {'PASS'} else {'WARN'}) -Detail (($startup | Select-Object -First 12 Name) -join '; ')
        }

        if ($Name -match 'SSD-Health|RAM-Pressure') {
            Get-PhysicalDisk | ForEach-Object {
                Add-ReportRow -Check $_.FriendlyName -Result $_.HealthStatus -Status $(if ($_.HealthStatus -eq 'Healthy') {'PASS'} else {'FAIL'}) -Detail $_.MediaType
            }
            Get-Process | Sort-Object WorkingSet64 -Descending | Select-Object -First 8 | ForEach-Object {
                Add-ReportRow -Check "RAM $($_.ProcessName)" -Result ("{0:N0} MB" -f ($_.WorkingSet64/1MB)) -Status 'INFO' -Detail "PID=$($_.Id)"
            }
        }

        if ($Name -match 'Background-Update') {
            if ($PSCmdlet.ShouldProcess('Update orchestration', 'Pause UsoSvc briefly for triage')) {
                Stop-Service UsoSvc -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'UsoSvc' -Result 'Stopped temporarily' -Status 'WARN' -Detail 'Restart when triage complete'
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Slow PC'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Background Update Throttle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Background Update Throttle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
