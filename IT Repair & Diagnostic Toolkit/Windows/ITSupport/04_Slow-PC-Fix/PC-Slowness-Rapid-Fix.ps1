...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: PC Slowness Rapid Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for PC-Slowness-Rapid-Fix.
...
$exitCode = 0
$Activity = 'PC Slowness Rapid Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PC-Slowness-Rapid-Fix'
        Add-ReportRow -Check 'Tool' -Result 'PC-Slowness-Rapid-Fix' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'PC Slowness Rapid Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PC Slowness Rapid Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
