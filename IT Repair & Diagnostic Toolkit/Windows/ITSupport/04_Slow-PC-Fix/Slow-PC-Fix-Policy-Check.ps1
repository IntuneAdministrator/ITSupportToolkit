...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-Policy-Check.ps1
.EXAMPLE
    .\Slow-PC-Fix-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix Policy Check'; $totalSteps = 8; $Name = 'Slow-PC-Fix-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
