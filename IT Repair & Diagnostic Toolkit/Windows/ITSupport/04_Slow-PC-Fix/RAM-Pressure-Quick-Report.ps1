...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: RAM Pressure Quick Report
.DESCRIPTION
    Production-grade Windows IT Support tool for RAM-Pressure-Quick-Report.
...
.EXAMPLE
    .\RAM-Pressure-Quick-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\RAM-Pressure-Quick-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'RAM Pressure Quick Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'RAM-Pressure-Quick-Report'
        Add-ReportRow -Check 'Tool' -Result 'RAM-Pressure-Quick-Report' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'RAM Pressure Quick Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RAM Pressure Quick Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
