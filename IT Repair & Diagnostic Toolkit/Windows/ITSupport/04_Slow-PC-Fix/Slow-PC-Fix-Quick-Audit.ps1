...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-Quick-Audit.ps1
.EXAMPLE
    .\Slow-PC-Fix-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix Quick Audit'; $totalSteps = 8; $Name = 'Slow-PC-Fix-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
