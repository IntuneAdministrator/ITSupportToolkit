...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Telemetry CPU Spike Kill
.DESCRIPTION
    Production-grade Windows IT Support tool for Telemetry-CPU-Spike-Kill.
...
.EXAMPLE
    .\Telemetry-CPU-Spike-Kill.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Telemetry-CPU-Spike-Kill.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Telemetry CPU Spike Kill'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Telemetry-CPU-Spike-Kill'
        Add-ReportRow -Check 'Tool' -Result 'Telemetry-CPU-Spike-Kill' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'Telemetry CPU Spike Kill' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Telemetry CPU Spike Kill' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
