...
<#
.SYNOPSIS
    Common problem fix: Chrome High Memory Fix
.DESCRIPTION
...
.EXAMPLE
    .\Chrome-High-Memory-Fix
.EXAMPLE
    .\Chrome-High-Memory-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Chrome High Memory Fix'; $totalSteps = 8; $Name = 'Chrome-High-Memory-Fix'
try {
    ...
    Export-HtmlReport -Title 'Chrome High Memory Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chrome High Memory Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
