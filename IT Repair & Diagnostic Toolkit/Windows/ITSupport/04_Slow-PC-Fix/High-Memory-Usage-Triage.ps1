...
<#
.SYNOPSIS
    Common problem fix: High Memory Usage Triage
.DESCRIPTION
...
.EXAMPLE
    .\High-Memory-Usage-Triage.ps1
.EXAMPLE
    .\High-Memory-Usage-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'High Memory Usage Triage'; $totalSteps = 8; $Name = 'High-Memory-Usage-Triage'
try {
    ...
    Export-HtmlReport -Title 'High Memory Usage Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'High Memory Usage Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
