...
<#
.SYNOPSIS
    Common problem fix: Transparency Effects Disable Fix
.DESCRIPTION
...
.EXAMPLE
    .\Transparency-Effects-Disable-Fix
.EXAMPLE
    .\Transparency-Effects-Disable-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Transparency Effects Disable Fix'; $totalSteps = 8; $Name = 'Transparency-Effects-Disable-Fix'
try {
    ...
    Export-HtmlReport -Title 'Transparency Effects Disable Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Transparency Effects Disable Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
