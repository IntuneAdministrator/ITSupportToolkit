...
<#
.SYNOPSIS
    Common problem fix: Visual Effects Reduce Fix
.DESCRIPTION
...
.EXAMPLE
    .\Visual-Effects-Reduce-Fix
.EXAMPLE
    .\Visual-Effects-Reduce-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Visual Effects Reduce Fix'; $totalSteps = 8; $Name = 'Visual-Effects-Reduce-Fix'
try {
    ...
    Export-HtmlReport -Title 'Visual Effects Reduce Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Visual Effects Reduce Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
