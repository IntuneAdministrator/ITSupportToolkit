...
<#
.SYNOPSIS
    Common problem fix: Background Apps Limit Fix
.DESCRIPTION
...
.EXAMPLE
    .\Background-Apps-Limit-Fix
.EXAMPLE
    .\Background-Apps-Limit-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Background Apps Limit Fix'; $totalSteps = 8; $Name = 'Background-Apps-Limit-Fix'
try {
    ...
    Export-HtmlReport -Title 'Background Apps Limit Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Background Apps Limit Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
