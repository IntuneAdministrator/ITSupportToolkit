...
<#
.SYNOPSIS
    Common problem fix: Teams High CPU Fix
.DESCRIPTION
...
$exitCode = 0; $Activity = 'Teams High CPU Fix'; $totalSteps = 8; $Name = 'Teams-High-CPU-Fix'
try {
    ...
    Export-HtmlReport -Title 'Teams High CPU Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams High CPU Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
