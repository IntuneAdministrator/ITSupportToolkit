...
<#
.SYNOPSIS
    Common problem fix: Workstation GPU Bottleneck Detect
.DESCRIPTION
...
.EXAMPLE
    .\Workstation-GPU-Bottleneck-Detect.ps1
.EXAMPLE
    .\Workstation-GPU-Bottleneck-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Workstation GPU Bottleneck Detect'; $totalSteps = 8; $Name = 'Workstation-GPU-Bottleneck-Detect'
try {
    ...
    Export-HtmlReport -Title 'Workstation GPU Bottleneck Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Workstation GPU Bottleneck Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
