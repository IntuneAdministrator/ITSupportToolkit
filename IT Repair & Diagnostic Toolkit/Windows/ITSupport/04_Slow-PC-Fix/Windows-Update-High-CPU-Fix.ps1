...
<#
.SYNOPSIS
    Common problem fix: Windows Update High CPU Fix
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Update-High-CPU-Fix
.EXAMPLE
    .\Windows-Update-High-CPU-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update High CPU Fix'; $totalSteps = 8; $Name = 'Windows-Update-High-CPU-Fix'
try {
    ...
    Export-HtmlReport -Title 'Windows Update High CPU Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update High CPU Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
