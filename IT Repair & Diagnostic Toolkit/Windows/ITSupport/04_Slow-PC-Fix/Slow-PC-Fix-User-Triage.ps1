...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-User-Triage.ps1
.EXAMPLE
    .\Slow-PC-Fix-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix User Triage'; $totalSteps = 8; $Name = 'Slow-PC-Fix-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
