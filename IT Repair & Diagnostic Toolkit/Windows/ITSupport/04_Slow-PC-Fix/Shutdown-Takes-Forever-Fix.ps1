...
<#
.SYNOPSIS
    Common problem fix: Shutdown Takes Forever Fix
.DESCRIPTION
...
.EXAMPLE
    .\Shutdown-Takes-Forever-Fix
.EXAMPLE
    .\Shutdown-Takes-Forever-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Shutdown Takes Forever Fix'; $totalSteps = 8; $Name = 'Shutdown-Takes-Forever-Fix'
try {
    ...
    Export-HtmlReport -Title 'Shutdown Takes Forever Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shutdown Takes Forever Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
