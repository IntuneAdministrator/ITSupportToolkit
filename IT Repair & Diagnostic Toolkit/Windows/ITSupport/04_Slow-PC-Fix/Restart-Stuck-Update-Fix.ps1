...
<#
.SYNOPSIS
    Common problem fix: Restart Stuck Update Fix
.DESCRIPTION
...
.EXAMPLE
    .\Restart-Stuck-Update-Fix
.EXAMPLE
    .\Restart-Stuck-Update-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Restart Stuck Update Fix'; $totalSteps = 8; $Name = 'Restart-Stuck-Update-Fix'
try {
    ...
    Export-HtmlReport -Title 'Restart Stuck Update Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Restart Stuck Update Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
