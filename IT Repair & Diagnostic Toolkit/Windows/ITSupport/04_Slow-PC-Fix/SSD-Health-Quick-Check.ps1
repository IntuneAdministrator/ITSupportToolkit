...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: SSD Health Quick Check
.DESCRIPTION
    Production-grade Windows IT Support tool for SSD-Health-Quick-Check.
...
$exitCode = 0
$Activity = 'SSD Health Quick Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SSD-Health-Quick-Check'
        Add-ReportRow -Check 'Tool' -Result 'SSD-Health-Quick-Check' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'SSD Health Quick Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SSD Health Quick Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
