...
<#
.SYNOPSIS
    Common problem fix: Startup Apps Too Many Fix
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Apps-Too-Many-Fix.ps1
.EXAMPLE
    .\Startup-Apps-Too-Many-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Apps Too Many Fix'; $totalSteps = 8; $Name = 'Startup-Apps-Too-Many-Fix'
try {
    ...
    Export-HtmlReport -Title 'Startup Apps Too Many Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Apps Too Many Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
