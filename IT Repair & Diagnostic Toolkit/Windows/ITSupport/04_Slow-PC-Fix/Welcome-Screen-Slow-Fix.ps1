...
<#
.SYNOPSIS
    Common problem fix: Welcome Screen Slow Fix
.DESCRIPTION
...
.EXAMPLE
    .\Welcome-Screen-Slow-Fix
.EXAMPLE
    .\Welcome-Screen-Slow-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Welcome Screen Slow Fix'; $totalSteps = 8; $Name = 'Welcome-Screen-Slow-Fix'
try {
    ...
    Export-HtmlReport -Title 'Welcome Screen Slow Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Welcome Screen Slow Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
