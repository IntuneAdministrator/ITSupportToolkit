...
<#
.SYNOPSIS
    Common problem fix: Background Apps Battery Drain
.DESCRIPTION
...
.EXAMPLE
    .\Background-Apps-Battery-Drain.ps1
.EXAMPLE
    .\Background-Apps-Battery-Drain.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Background Apps Battery Drain'; $totalSteps = 8; $Name = 'Background-Apps-Battery-Drain'
try {
    ...
    Export-HtmlReport -Title 'Background Apps Battery Drain' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Background Apps Battery Drain' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
