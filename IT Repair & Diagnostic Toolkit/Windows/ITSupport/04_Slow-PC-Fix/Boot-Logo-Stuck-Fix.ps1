...
<#
.SYNOPSIS
    Common problem fix: Boot Logo Stuck Fix
.DESCRIPTION
...
$exitCode = 0; $Activity = 'Boot Logo Stuck Fix'; $totalSteps = 8; $Name = 'Boot-Logo-Stuck-Fix'
try {
    ...
    Export-HtmlReport -Title 'Boot Logo Stuck Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Boot Logo Stuck Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
