...
<#
.SYNOPSIS
    Common problem fix: Large Excel Slow Workstation Triage
.DESCRIPTION
...
.EXAMPLE
    .\Large-Excel-Slow-Workstation-Triage.ps1
.EXAMPLE
    .\Large-Excel-Slow-Workstation-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Large Excel Slow Workstation Triage'; $totalSteps = 8; $Name = 'Large-Excel-Slow-Workstation-Triage'
try {
    ...
    Export-HtmlReport -Title 'Large Excel Slow Workstation Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Large Excel Slow Workstation Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
