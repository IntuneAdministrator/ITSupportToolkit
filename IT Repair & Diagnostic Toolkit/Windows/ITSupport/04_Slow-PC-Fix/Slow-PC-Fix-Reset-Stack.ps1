...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-Reset-Stack.ps1
.EXAMPLE
    .\Slow-PC-Fix-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix Reset Stack'; $totalSteps = 8; $Name = 'Slow-PC-Fix-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
