...
<#
.SYNOPSIS
    Common problem fix: Startup Delay Too Long Fix
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Delay-Too-Long-Fix
.EXAMPLE
    .\Startup-Delay-Too-Long-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Delay Too Long Fix'; $totalSteps = 8; $Name = 'Startup-Delay-Too-Long-Fix'
try {
    ...
    Export-HtmlReport -Title 'Startup Delay Too Long Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Delay Too Long Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
