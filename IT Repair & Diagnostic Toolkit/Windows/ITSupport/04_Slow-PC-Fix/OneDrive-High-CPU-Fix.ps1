...
<#
.SYNOPSIS
    Common problem fix: OneDrive High CPU Fix
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-High-CPU-Fix
.EXAMPLE
    .\OneDrive-High-CPU-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive High CPU Fix'; $totalSteps = 8; $Name = 'OneDrive-High-CPU-Fix'
try {
    ...
    Export-HtmlReport -Title 'OneDrive High CPU Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive High CPU Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
