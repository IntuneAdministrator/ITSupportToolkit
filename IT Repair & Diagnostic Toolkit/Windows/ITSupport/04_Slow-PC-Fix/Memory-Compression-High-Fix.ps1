...
<#
.SYNOPSIS
    Common problem fix: Memory Compression High Fix
.DESCRIPTION
...
.EXAMPLE
    .\Memory-Compression-High-Fix
.EXAMPLE
    .\Memory-Compression-High-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Memory Compression High Fix'; $totalSteps = 8; $Name = 'Memory-Compression-High-Fix'
try {
    ...
    Export-HtmlReport -Title 'Memory Compression High Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Memory Compression High Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
