...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-Issue-Detect.ps1
.EXAMPLE
    .\Slow-PC-Fix-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix Issue Detect'; $totalSteps = 8; $Name = 'Slow-PC-Fix-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
