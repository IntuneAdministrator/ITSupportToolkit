...
<#
.SYNOPSIS
    Common problem fix: Pagefile Too Small Fix
.DESCRIPTION
...
.EXAMPLE
    .\Pagefile-Too-Small-Fix
.EXAMPLE
    .\Pagefile-Too-Small-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Pagefile Too Small Fix'; $totalSteps = 8; $Name = 'Pagefile-Too-Small-Fix'
try {
    ...
    Export-HtmlReport -Title 'Pagefile Too Small Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Pagefile Too Small Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
