...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-One-Click-Fix.ps1
.EXAMPLE
    .\Slow-PC-Fix-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix One Click Fix'; $totalSteps = 8; $Name = 'Slow-PC-Fix-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
