...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: High CPU Quick Kill
.DESCRIPTION
    Production-grade Windows IT Support tool for High-CPU-Quick-Kill.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'High-CPU-Quick-Kill'
        Add-ReportRow -Check 'Tool' -Result 'High-CPU-Quick-Kill' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'High CPU Quick Kill' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'High CPU Quick Kill' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
