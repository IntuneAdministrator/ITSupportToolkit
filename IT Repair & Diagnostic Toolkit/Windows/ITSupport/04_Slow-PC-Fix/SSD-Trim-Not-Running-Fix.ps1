...
<#
.SYNOPSIS
    Common problem fix: SSD Trim Not Running Fix
.DESCRIPTION
...
.EXAMPLE
    .\SSD-Trim-Not-Running-Fix
.EXAMPLE
    .\SSD-Trim-Not-Running-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SSD Trim Not Running Fix'; $totalSteps = 8; $Name = 'SSD-Trim-Not-Running-Fix'
try {
    ...
    Export-HtmlReport -Title 'SSD Trim Not Running Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SSD Trim Not Running Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
