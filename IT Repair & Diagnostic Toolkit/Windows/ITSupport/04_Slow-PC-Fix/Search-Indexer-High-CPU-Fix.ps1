...
<#
.SYNOPSIS
    Common problem fix: Search Indexer High CPU Fix
.DESCRIPTION
...
.EXAMPLE
    .\Search-Indexer-High-CPU-Fix
.EXAMPLE
    .\Search-Indexer-High-CPU-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Search Indexer High CPU Fix'; $totalSteps = 8; $Name = 'Search-Indexer-High-CPU-Fix'
try {
    ...
    Export-HtmlReport -Title 'Search Indexer High CPU Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Search Indexer High CPU Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
