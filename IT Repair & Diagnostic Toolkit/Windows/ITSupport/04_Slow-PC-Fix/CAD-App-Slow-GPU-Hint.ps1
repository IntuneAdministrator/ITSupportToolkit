...
<#
.SYNOPSIS
    Common problem fix: CAD App Slow GPU Hint
.DESCRIPTION
...
.EXAMPLE
    .\CAD-App-Slow-GPU-Hint.ps1
.EXAMPLE
    .\CAD-App-Slow-GPU-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CAD App Slow GPU Hint'; $totalSteps = 8; $Name = 'CAD-App-Slow-GPU-Hint'
try {
    ...
    Export-HtmlReport -Title 'CAD App Slow GPU Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CAD App Slow GPU Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
