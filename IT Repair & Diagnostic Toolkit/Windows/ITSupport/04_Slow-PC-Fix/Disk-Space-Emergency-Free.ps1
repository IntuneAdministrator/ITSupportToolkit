...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Disk Space Emergency Free
.DESCRIPTION
    Production-grade Windows IT Support tool for Disk-Space-Emergency-Free.
...
.EXAMPLE
    .\Disk-Space-Emergency-Free.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Disk-Space-Emergency-Free.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Disk Space Emergency Free'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Disk-Space-Emergency-Free'
        Add-ReportRow -Check 'Tool' -Result 'Disk-Space-Emergency-Free' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'Disk Space Emergency Free' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk Space Emergency Free' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
