...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-Quick-Repair.ps1
.EXAMPLE
    .\Slow-PC-Fix-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix Quick Repair'; $totalSteps = 8; $Name = 'Slow-PC-Fix-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
