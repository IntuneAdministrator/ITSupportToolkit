...
<#
.SYNOPSIS
    Common problem fix: HDD Fragmentation Check Fix
.DESCRIPTION
...
.EXAMPLE
    .\HDD-Fragmentation-Check-Fix
.EXAMPLE
    .\HDD-Fragmentation-Check-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HDD Fragmentation Check Fix'; $totalSteps = 8; $Name = 'HDD-Fragmentation-Check-Fix'
try {
    ...
    Export-HtmlReport -Title 'HDD Fragmentation Check Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HDD Fragmentation Check Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
