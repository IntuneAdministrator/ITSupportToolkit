...
<#
.SYNOPSIS
    Common problem fix: Pending Reboot Clear Hint Fix
.DESCRIPTION
...
.EXAMPLE
    .\Pending-Reboot-Clear-Hint-Fix
.EXAMPLE
    .\Pending-Reboot-Clear-Hint-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Pending Reboot Clear Hint Fix'; $totalSteps = 8; $Name = 'Pending-Reboot-Clear-Hint-Fix'
try {
    ...
    Export-HtmlReport -Title 'Pending Reboot Clear Hint Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Pending Reboot Clear Hint Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
