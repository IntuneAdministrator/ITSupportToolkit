...
<#
.SYNOPSIS
    Common problem fix: Slow PC Fix Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Slow-PC-Fix-Helpdesk-Report.ps1
.EXAMPLE
    .\Slow-PC-Fix-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Slow PC Fix Helpdesk Report'; $totalSteps = 8; $Name = 'Slow-PC-Fix-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Slow PC Fix Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Slow PC Fix Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
