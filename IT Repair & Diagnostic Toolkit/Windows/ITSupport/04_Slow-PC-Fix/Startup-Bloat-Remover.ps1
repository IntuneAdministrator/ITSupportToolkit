...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Startup Bloat Remover
.DESCRIPTION
    Production-grade Windows IT Support tool for Startup-Bloat-Remover.
...
$exitCode = 0
$Activity = 'Startup Bloat Remover'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Startup-Bloat-Remover'
        Add-ReportRow -Check 'Tool' -Result 'Startup-Bloat-Remover' -Status 'INFO' -Detail 'ITSupport/04_Slow-PC-Fix'
    ...
    Export-HtmlReport -Title 'Startup Bloat Remover' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Startup Bloat Remover' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
