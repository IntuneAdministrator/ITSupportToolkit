...
<#
.SYNOPSIS
    Common problem fix: Prefetch Superfetch Triage Fix
.DESCRIPTION
...
.EXAMPLE
    .\Prefetch-Superfetch-Triage-Fix
.EXAMPLE
    .\Prefetch-Superfetch-Triage-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Prefetch Superfetch Triage Fix'; $totalSteps = 8; $Name = 'Prefetch-Superfetch-Triage-Fix'
try {
    ...
    Export-HtmlReport -Title 'Prefetch Superfetch Triage Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Prefetch Superfetch Triage Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
