...
<#
.SYNOPSIS
    Common problem fix: Disk 100 Percent Usage Fix
.DESCRIPTION
...
.EXAMPLE
    .\Disk-100-Percent-Usage-Fix
.EXAMPLE
    .\Disk-100-Percent-Usage-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk 100 Percent Usage Fix'; $totalSteps = 8; $Name = 'Disk-100-Percent-Usage-Fix'
try {
    ...
    Export-HtmlReport -Title 'Disk 100 Percent Usage Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk 100 Percent Usage Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
