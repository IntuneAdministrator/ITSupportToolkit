...
<#
.SYNOPSIS
    Enterprise IT tool: Windows Hello Container Reset Guide
.DESCRIPTION
    Expansion module (ITSupport / 11_Identity-MFA-Helpdesk) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Windows-Hello-Container-Reset-Guide.ps1
.EXAMPLE
    .\Windows-Hello-Container-Reset-Guide.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Hello Container Reset Guide'; $totalSteps = 8; $Name = 'Windows-Hello-Container-Reset-Guide'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '11_Identity-MFA-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        Write-SectionBanner 'IT SUPPORT EXPANSION'
        ...
        if ($Name -match 'MFA|Passwordless|FIDO|Hello|SSO|TAP') {
            $ds = & dsregcmd /status 2>&1 | Out-String
            Add-ReportRow -Check 'PRT' -Result $(if ($ds -match 'AzureAdPrt\s*:\s*YES') {'YES'} else {'NO'}) -Status 'INFO' -Detail ''
            Add-ReportRow -Check 'Guidance' -Result 'IdP portal for MFA reset; Hello via Sign-in options' -Status 'WARN' -Detail 'No secrets collected'
            if ($PSCmdlet.ShouldProcess('PRT', 'refresh')) { dsregcmd /refreshprt 2>$null | Out-Null; Add-ReportRow -Check 'PRT refresh' -Result 'OK' -Status 'PASS' -Detail '' }
        }
        if ($Name -match 'Serial|Warranty|Dell|Lenovo|HP|Asset|Dock|Battery-Cycle|Model') {
            $bios = Get-CimInstance Win32_BIOS; $cs = Get-CimInstance Win32_ComputerSystem
            Add-ReportRow -Check 'Manufacturer' -Result $cs.Manufacturer -Status 'INFO' -Detail $cs.Model
            Add-ReportRow -Check 'Serial' -Result $bios.SerialNumber -Status 'INFO' -Detail $bios.SMBIOSBIOSVersion
            $out = Join-Path 'C:\IT-Logs' ("Asset_{0}.csv" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
            [pscustomobject]@{Computer=$env:COMPUTERNAME; Manufacturer=$cs.Manufacturer; Model=$cs.Model; Serial=$bios.SerialNumber; Join=$Join.Label} | Export-Csv $out -NoTypeInformation
            Add-ReportRow -Check 'Asset CSV' -Result $out -Status 'PASS' -Detail ''
        }
        if ($Name -match 'Teams-Rooms|Surface-Hub|Meeting|Calendar-Resource') {
            Get-Process -Name Teams,ms-teams,SkypeRoomSystem -EA SilentlyContinue | ForEach-Object { Add-ReportRow -Check $_.ProcessName -Result "PID $($_.Id)" -Status 'INFO' -Detail '' }
            Add-ReportRow -Check 'Room systems' -Result 'Collect peripheral + calendar resource tests manually' -Status 'INFO' -Detail ''
        }
        if ($Name -match 'OneDrive|SharePoint|Known-Folder|Files-On-Demand') {
            $od = "$env:LOCALAPPDATA\Microsoft\OneDrive\OneDrive.exe"
            Add-ReportRow -Check 'OneDrive' -Result $(if (Test-Path $od) {'Installed'} else {'Missing'}) -Status $(if (Test-Path $od) {'PASS'} else {'WARN'}) -Detail $od
            if ($PSCmdlet.ShouldProcess('OneDrive', 'Restart')) { Get-Process OneDrive -EA SilentlyContinue | Stop-Process -Force -EA SilentlyContinue; if (Test-Path $od) { Start-Process $od }; Add-ReportRow -Check 'OneDrive restart' -Result 'Done' -Status 'PASS' -Detail '' }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'ITSupport expansion done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Hello Container Reset Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Hello Container Reset Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
