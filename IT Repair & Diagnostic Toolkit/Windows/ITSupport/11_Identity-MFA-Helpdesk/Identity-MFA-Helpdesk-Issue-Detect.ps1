...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Identity-MFA-Helpdesk-Issue-Detect.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk Issue Detect'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
