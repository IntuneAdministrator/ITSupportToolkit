...
<#
.SYNOPSIS
    Enterprise IT tool: Passwordless Status Check
.DESCRIPTION
...
.EXAMPLE
    .\Passwordless-Status-Check.ps1
.EXAMPLE
    .\Passwordless-Status-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Passwordless Status Check'; $totalSteps = 8; $Name = 'Passwordless-Status-Check'
try {
    ...
    Export-HtmlReport -Title 'Passwordless Status Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Passwordless Status Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
