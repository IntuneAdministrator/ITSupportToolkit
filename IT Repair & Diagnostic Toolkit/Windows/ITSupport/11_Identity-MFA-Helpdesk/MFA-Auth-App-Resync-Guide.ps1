...
<#
.SYNOPSIS
    Enterprise IT tool: MFA Auth App Resync Guide
.DESCRIPTION
...
.EXAMPLE
    .\MFA-Auth-App-Resync-Guide.ps1
.EXAMPLE
    .\MFA-Auth-App-Resync-Guide.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'MFA Auth App Resync Guide'; $totalSteps = 8; $Name = 'MFA-Auth-App-Resync-Guide'
try {
    ...
    Export-HtmlReport -Title 'MFA Auth App Resync Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'MFA Auth App Resync Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
