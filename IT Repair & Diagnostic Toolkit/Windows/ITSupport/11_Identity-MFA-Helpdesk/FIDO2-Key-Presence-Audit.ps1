...
<#
.SYNOPSIS
    Enterprise IT tool: FIDO2 Key Presence Audit
.DESCRIPTION
...
.EXAMPLE
    .\FIDO2-Key-Presence-Audit.ps1
.EXAMPLE
    .\FIDO2-Key-Presence-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'FIDO2 Key Presence Audit'; $totalSteps = 8; $Name = 'FIDO2-Key-Presence-Audit'
try {
    ...
    Export-HtmlReport -Title 'FIDO2 Key Presence Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'FIDO2 Key Presence Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
