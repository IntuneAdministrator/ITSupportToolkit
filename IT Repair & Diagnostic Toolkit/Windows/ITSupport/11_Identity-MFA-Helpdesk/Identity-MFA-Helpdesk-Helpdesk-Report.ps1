...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 11_Identity-MFA-Helpdesk). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Identity-MFA-Helpdesk-Helpdesk-Report.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk Helpdesk Report'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '11_Identity-MFA-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '11_Identity-MFA-Helpdesk'
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
