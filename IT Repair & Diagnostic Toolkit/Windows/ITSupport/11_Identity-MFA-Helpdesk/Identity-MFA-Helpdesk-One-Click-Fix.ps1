...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Identity-MFA-Helpdesk-One-Click-Fix.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk One Click Fix'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
