...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Identity-MFA-Helpdesk-User-Triage.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk User Triage'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
