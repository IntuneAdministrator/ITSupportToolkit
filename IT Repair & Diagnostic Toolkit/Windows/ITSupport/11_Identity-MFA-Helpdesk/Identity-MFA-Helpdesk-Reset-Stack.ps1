...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Identity-MFA-Helpdesk-Reset-Stack.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk Reset Stack'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
