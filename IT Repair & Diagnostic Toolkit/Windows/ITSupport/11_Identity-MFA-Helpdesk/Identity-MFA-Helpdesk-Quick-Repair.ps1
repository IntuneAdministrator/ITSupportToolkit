...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Identity-MFA-Helpdesk-Quick-Repair.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk Quick Repair'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
