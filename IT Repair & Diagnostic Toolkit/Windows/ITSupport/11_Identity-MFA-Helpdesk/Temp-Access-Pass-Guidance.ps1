...
<#
.SYNOPSIS
    Enterprise IT tool: Temp Access Pass Guidance
.DESCRIPTION
...
.EXAMPLE
    .\Temp-Access-Pass-Guidance.ps1
.EXAMPLE
    .\Temp-Access-Pass-Guidance.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Temp Access Pass Guidance'; $totalSteps = 8; $Name = 'Temp-Access-Pass-Guidance'
try {
    ...
    Export-HtmlReport -Title 'Temp Access Pass Guidance' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Temp Access Pass Guidance' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
