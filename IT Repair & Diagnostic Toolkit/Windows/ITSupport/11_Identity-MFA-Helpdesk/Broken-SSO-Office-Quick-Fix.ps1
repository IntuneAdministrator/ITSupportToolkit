...
<#
.SYNOPSIS
    Enterprise IT tool: Broken SSO Office Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Broken-SSO-Office-Quick-Fix.ps1
.EXAMPLE
    .\Broken-SSO-Office-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Broken SSO Office Quick Fix'; $totalSteps = 8; $Name = 'Broken-SSO-Office-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Broken SSO Office Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Broken SSO Office Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
