...
<#
.SYNOPSIS
    Common problem fix: Identity MFA Helpdesk Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Identity-MFA-Helpdesk-Policy-Check.ps1
.EXAMPLE
    .\Identity-MFA-Helpdesk-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity MFA Helpdesk Policy Check'; $totalSteps = 8; $Name = 'Identity-MFA-Helpdesk-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Identity MFA Helpdesk Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity MFA Helpdesk Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
