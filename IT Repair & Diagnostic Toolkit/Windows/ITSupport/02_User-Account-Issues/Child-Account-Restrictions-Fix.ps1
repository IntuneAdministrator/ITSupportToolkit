...
<#
.SYNOPSIS
    Common problem fix: Child Account Restrictions Fix
.DESCRIPTION
...
.EXAMPLE
    .\Child-Account-Restrictions-Fix
.EXAMPLE
    .\Child-Account-Restrictions-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Child Account Restrictions Fix'; $totalSteps = 8; $Name = 'Child-Account-Restrictions-Fix'
try {
    ...
    Export-HtmlReport -Title 'Child Account Restrictions Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Child Account Restrictions Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
