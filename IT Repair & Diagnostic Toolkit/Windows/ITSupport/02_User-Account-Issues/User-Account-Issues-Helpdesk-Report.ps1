...
<#
.SYNOPSIS
    Common problem fix: User Account Issues Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 02_User-Account-Issues). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\User-Account-Issues-Helpdesk-Report.ps1
.EXAMPLE
    .\User-Account-Issues-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues Helpdesk Report'; $totalSteps = 8; $Name = 'User-Account-Issues-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '02_User-Account-Issues' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '02_User-Account-Issues'
    ...
    Export-HtmlReport -Title 'User Account Issues Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
