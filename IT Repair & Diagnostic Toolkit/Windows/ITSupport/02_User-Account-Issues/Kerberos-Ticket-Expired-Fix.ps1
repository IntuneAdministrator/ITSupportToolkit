...
<#
.SYNOPSIS
    Common problem fix: Kerberos Ticket Expired Fix
.DESCRIPTION
...
.EXAMPLE
    .\Kerberos-Ticket-Expired-Fix
.EXAMPLE
    .\Kerberos-Ticket-Expired-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Kerberos Ticket Expired Fix'; $totalSteps = 8; $Name = 'Kerberos-Ticket-Expired-Fix'
try {
    ...
    Export-HtmlReport -Title 'Kerberos Ticket Expired Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Kerberos Ticket Expired Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
