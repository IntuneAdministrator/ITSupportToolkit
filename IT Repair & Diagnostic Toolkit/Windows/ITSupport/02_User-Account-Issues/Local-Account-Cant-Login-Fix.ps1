...
<#
.SYNOPSIS
    Common problem fix: Local Account Cant Login Fix
.DESCRIPTION
...
.EXAMPLE
    .\Local-Account-Cant-Login-Fix
.EXAMPLE
    .\Local-Account-Cant-Login-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Local Account Cant Login Fix'; $totalSteps = 8; $Name = 'Local-Account-Cant-Login-Fix'
try {
    ...
    Export-HtmlReport -Title 'Local Account Cant Login Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Local Account Cant Login Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
