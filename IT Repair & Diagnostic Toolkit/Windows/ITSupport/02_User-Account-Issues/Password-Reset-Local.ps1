...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Password Reset Local
.DESCRIPTION
    Production-grade Windows IT Support tool for Password-Reset-Local.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Password-Reset-Local' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
    ...
    Export-HtmlReport -Title 'Password Reset Local' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Password Reset Local' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
