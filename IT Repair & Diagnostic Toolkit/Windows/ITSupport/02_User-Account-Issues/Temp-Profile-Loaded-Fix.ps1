...
<#
.SYNOPSIS
    Common problem fix: Temp Profile Loaded Fix
.DESCRIPTION
...
.EXAMPLE
    .\Temp-Profile-Loaded-Fix
.EXAMPLE
    .\Temp-Profile-Loaded-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Temp Profile Loaded Fix'; $totalSteps = 8; $Name = 'Temp-Profile-Loaded-Fix'
try {
    ...
    Export-HtmlReport -Title 'Temp Profile Loaded Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Temp Profile Loaded Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
