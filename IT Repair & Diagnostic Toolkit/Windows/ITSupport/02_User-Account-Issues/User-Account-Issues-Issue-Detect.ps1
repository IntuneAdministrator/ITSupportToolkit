...
<#
.SYNOPSIS
    Common problem fix: User Account Issues Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\User-Account-Issues-Issue-Detect.ps1
.EXAMPLE
    .\User-Account-Issues-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues Issue Detect'; $totalSteps = 8; $Name = 'User-Account-Issues-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'User Account Issues Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
