...
<#
.SYNOPSIS
    Common problem fix: Automatic Login Audit Fix
.DESCRIPTION
...
.EXAMPLE
    .\Automatic-Login-Audit-Fix
.EXAMPLE
    .\Automatic-Login-Audit-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Automatic Login Audit Fix'; $totalSteps = 8; $Name = 'Automatic-Login-Audit-Fix'
try {
    ...
    Export-HtmlReport -Title 'Automatic Login Audit Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Automatic Login Audit Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
