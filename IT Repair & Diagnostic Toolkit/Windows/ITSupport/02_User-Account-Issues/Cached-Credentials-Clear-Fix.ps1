...
<#
.SYNOPSIS
    Common problem fix: Cached Credentials Clear Fix
.DESCRIPTION
...
.EXAMPLE
    .\Cached-Credentials-Clear-Fix
.EXAMPLE
    .\Cached-Credentials-Clear-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cached Credentials Clear Fix'; $totalSteps = 8; $Name = 'Cached-Credentials-Clear-Fix'
try {
    ...
    Export-HtmlReport -Title 'Cached Credentials Clear Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cached Credentials Clear Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
