...
<#
.SYNOPSIS
    Common problem fix: Account Disabled AD Check
.DESCRIPTION
...
.EXAMPLE
    .\Account-Disabled-AD-Check
.EXAMPLE
    .\Account-Disabled-AD-Check -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Account Disabled AD Check'; $totalSteps = 8; $Name = 'Account-Disabled-AD-Check'
try {
    ...
    Export-HtmlReport -Title 'Account Disabled AD Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Disabled AD Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
