...
<#
.SYNOPSIS
    Common problem fix: User Account Issues Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\User-Account-Issues-Quick-Audit.ps1
.EXAMPLE
    .\User-Account-Issues-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues Quick Audit'; $totalSteps = 8; $Name = 'User-Account-Issues-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'User Account Issues Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
