...
<#
.SYNOPSIS
    Common problem fix: UPN Suffix Mismatch Fix
.DESCRIPTION
...
.EXAMPLE
    .\UPN-Suffix-Mismatch-Fix
.EXAMPLE
    .\UPN-Suffix-Mismatch-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'UPN Suffix Mismatch Fix'; $totalSteps = 8; $Name = 'UPN-Suffix-Mismatch-Fix'
try {
    ...
    Export-HtmlReport -Title 'UPN Suffix Mismatch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'UPN Suffix Mismatch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
