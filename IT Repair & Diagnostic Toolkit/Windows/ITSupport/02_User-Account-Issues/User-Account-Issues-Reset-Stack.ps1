...
<#
.SYNOPSIS
    Common problem fix: User Account Issues Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\User-Account-Issues-Reset-Stack.ps1
.EXAMPLE
    .\User-Account-Issues-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues Reset Stack'; $totalSteps = 8; $Name = 'User-Account-Issues-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'User Account Issues Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
