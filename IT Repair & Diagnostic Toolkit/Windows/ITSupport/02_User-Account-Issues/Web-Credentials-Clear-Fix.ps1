...
<#
.SYNOPSIS
    Common problem fix: Web Credentials Clear Fix
.DESCRIPTION
...
.EXAMPLE
    .\Web-Credentials-Clear-Fix
.EXAMPLE
    .\Web-Credentials-Clear-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Web Credentials Clear Fix'; $totalSteps = 8; $Name = 'Web-Credentials-Clear-Fix'
try {
    ...
    Export-HtmlReport -Title 'Web Credentials Clear Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Web Credentials Clear Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
