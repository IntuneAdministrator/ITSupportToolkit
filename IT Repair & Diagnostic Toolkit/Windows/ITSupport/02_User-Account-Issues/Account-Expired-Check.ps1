...
<#
.SYNOPSIS
    Common problem fix: Account Expired Check
.DESCRIPTION
...
$exitCode = 0; $Activity = 'Account Expired Check'; $totalSteps = 8; $Name = 'Account-Expired-Check'
try {
    ...
    Export-HtmlReport -Title 'Account Expired Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Account Expired Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
