...
<#
.SYNOPSIS
    Common problem fix: PIN Login Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\PIN-Login-Failed-Fix
.EXAMPLE
    .\PIN-Login-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'PIN Login Failed Fix'; $totalSteps = 8; $Name = 'PIN-Login-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'PIN Login Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'PIN Login Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
