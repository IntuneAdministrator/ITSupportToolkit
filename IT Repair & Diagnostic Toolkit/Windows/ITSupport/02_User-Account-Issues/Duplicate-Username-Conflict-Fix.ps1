...
<#
.SYNOPSIS
    Common problem fix: Duplicate Username Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\Duplicate-Username-Conflict-Fix
.EXAMPLE
    .\Duplicate-Username-Conflict-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Duplicate Username Conflict Fix'; $totalSteps = 8; $Name = 'Duplicate-Username-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'Duplicate Username Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Duplicate Username Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
