...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Profile Corruption Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Profile-Corruption-Fix.
...
$exitCode = 0
$Activity = 'Profile Corruption Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Profile-Corruption-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Profile-Corruption-Fix' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
    ...
    Export-HtmlReport -Title 'Profile Corruption Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Profile Corruption Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
