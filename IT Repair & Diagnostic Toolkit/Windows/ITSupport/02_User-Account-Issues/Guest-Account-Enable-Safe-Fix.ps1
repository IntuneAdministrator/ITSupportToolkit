...
<#
.SYNOPSIS
    Common problem fix: Guest Account Enable Safe Fix
.DESCRIPTION
...
.EXAMPLE
    .\Guest-Account-Enable-Safe-Fix
.EXAMPLE
    .\Guest-Account-Enable-Safe-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Guest Account Enable Safe Fix'; $totalSteps = 8; $Name = 'Guest-Account-Enable-Safe-Fix'
try {
    ...
    Export-HtmlReport -Title 'Guest Account Enable Safe Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Guest Account Enable Safe Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
