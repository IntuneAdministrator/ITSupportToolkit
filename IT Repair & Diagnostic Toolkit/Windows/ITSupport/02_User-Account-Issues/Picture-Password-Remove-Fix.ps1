...
<#
.SYNOPSIS
    Common problem fix: Picture Password Remove Fix
.DESCRIPTION
...
.EXAMPLE
    .\Picture-Password-Remove-Fix
.EXAMPLE
    .\Picture-Password-Remove-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Picture Password Remove Fix'; $totalSteps = 8; $Name = 'Picture-Password-Remove-Fix'
try {
    ...
    Export-HtmlReport -Title 'Picture Password Remove Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Picture Password Remove Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
