...
<#
.SYNOPSIS
    Common problem fix: Standard User Needs Admin Hint
.DESCRIPTION
...
.EXAMPLE
    .\Standard-User-Needs-Admin-Hint
.EXAMPLE
    .\Standard-User-Needs-Admin-Hint -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Standard User Needs Admin Hint'; $totalSteps = 8; $Name = 'Standard-User-Needs-Admin-Hint'
try {
    ...
    Export-HtmlReport -Title 'Standard User Needs Admin Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Standard User Needs Admin Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
