...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Temp Profile Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Temp-Profile-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Temp-Profile-Fix' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
    ...
    Export-HtmlReport -Title 'Temp Profile Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Temp Profile Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
