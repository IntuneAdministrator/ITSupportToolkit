...
<#
.SYNOPSIS
    Common problem fix: User Account Issues Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\User-Account-Issues-Policy-Check.ps1
.EXAMPLE
    .\User-Account-Issues-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues Policy Check'; $totalSteps = 8; $Name = 'User-Account-Issues-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'User Account Issues Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
