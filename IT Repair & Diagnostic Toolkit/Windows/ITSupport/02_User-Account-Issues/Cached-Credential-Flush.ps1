...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Cached Credential Flush
.DESCRIPTION
    Production-grade Windows IT Support tool for Cached-Credential-Flush.
...
.EXAMPLE
    .\Cached-Credential-Flush.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Cached-Credential-Flush.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Cached Credential Flush'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Cached-Credential-Flush'
        Add-ReportRow -Check 'Tool' -Result 'Cached-Credential-Flush' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
    ...
    Export-HtmlReport -Title 'Cached Credential Flush' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Cached Credential Flush' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
