...
<#
.SYNOPSIS
    Common problem fix: Roaming Profile Slow Login Fix
.DESCRIPTION
...
.EXAMPLE
    .\Roaming-Profile-Slow-Login-Fix
.EXAMPLE
    .\Roaming-Profile-Slow-Login-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Roaming Profile Slow Login Fix'; $totalSteps = 8; $Name = 'Roaming-Profile-Slow-Login-Fix'
try {
    ...
    Export-HtmlReport -Title 'Roaming Profile Slow Login Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Roaming Profile Slow Login Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
