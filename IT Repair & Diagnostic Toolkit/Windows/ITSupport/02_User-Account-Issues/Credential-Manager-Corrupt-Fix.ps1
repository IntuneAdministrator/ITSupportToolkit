...
<#
.SYNOPSIS
    Common problem fix: Credential Manager Corrupt Fix
.DESCRIPTION
...
.EXAMPLE
    .\Credential-Manager-Corrupt-Fix
.EXAMPLE
    .\Credential-Manager-Corrupt-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Credential Manager Corrupt Fix'; $totalSteps = 8; $Name = 'Credential-Manager-Corrupt-Fix'
try {
    ...
    Export-HtmlReport -Title 'Credential Manager Corrupt Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Credential Manager Corrupt Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
