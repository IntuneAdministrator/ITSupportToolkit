...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Locked Account Unlock
.DESCRIPTION
    Production-grade Windows IT Support tool for Locked-Account-Unlock.
...
$exitCode = 0
$Activity = 'Locked Account Unlock'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Locked-Account-Unlock'
        Add-ReportRow -Check 'Tool' -Result 'Locked-Account-Unlock' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
    ...
    Export-HtmlReport -Title 'Locked Account Unlock' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Locked Account Unlock' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
