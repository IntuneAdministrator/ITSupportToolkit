...
<#
.SYNOPSIS
    Common problem fix: User Profile Service Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\User-Profile-Service-Failed-Fix
.EXAMPLE
    .\User-Profile-Service-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Service Failed Fix'; $totalSteps = 8; $Name = 'User-Profile-Service-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'User Profile Service Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Service Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
