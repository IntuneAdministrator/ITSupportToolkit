...
<#
.SYNOPSIS
    Common problem fix: User Account Issues One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\User-Account-Issues-One-Click-Fix.ps1
.EXAMPLE
    .\User-Account-Issues-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues One Click Fix'; $totalSteps = 8; $Name = 'User-Account-Issues-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'User Account Issues One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
