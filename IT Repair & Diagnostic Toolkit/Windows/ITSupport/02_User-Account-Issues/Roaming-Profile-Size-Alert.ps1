...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Roaming Profile Size Alert
.DESCRIPTION
    Production-grade Windows IT Support tool for Roaming-Profile-Size-Alert.
Category: 02_User-Account-Issues. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Roaming-Profile-Size-Alert.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Roaming-Profile-Size-Alert.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Roaming Profile Size Alert'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'USER ACCOUNT ISSUES'
        $Name = 'Roaming-Profile-Size-Alert'
        Add-ReportRow -Check 'Tool' -Result 'Roaming-Profile-Size-Alert' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
        Add-ReportRow -Check 'Current user' -Result $env:USERNAME -Status 'INFO' -Detail $env:USERPROFILE
        Add-ReportRow -Check 'Join state' -Result $Join.Label -Status 'INFO' -Detail $Join.DomainName
        Write-ProgressStep 2 $totalSteps $Activity 'Identity context'

        if ($Name -match 'Password-Reset-Local') {
            Add-ReportRow -Check 'Local password reset' -Result 'Use Set-LocalUser interactively or Intune LAPS' -Status 'WARN' -Detail 'No password accepted via script args (security)'
            try {
                $localUsers = Get-LocalUser | Where-Object Enabled
                foreach ($u in $localUsers) {
                    Add-ReportRow -Check "Local user $($u.Name)" -Result $(if ($u.PasswordRequired) {'Password required'} else {'Check policy'}) -Status 'INFO' -Detail "LastSet=$($u.PasswordLastSet)"
                }
            } catch {
                Add-ReportRow -Check 'Local users' -Result 'Query failed' -Status 'WARN' -Detail $_.Exception.Message
            }
        }

        if ($Name -match 'Locked-Account') {
            if ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
                Add-ReportRow -Check 'Unlock path' -Result 'Domain account - unlock on DC / Entra' -Status 'INFO' -Detail 'Unlock-ADAccount requires RSAT + rights'
                if (Get-Module -ListAvailable ActiveDirectory) {
                    Import-Module ActiveDirectory -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'AD module' -Result 'Available' -Status 'PASS' -Detail 'Run Unlock-ADAccount -Identity <user> from elevated admin session'
                }
            } else {
                try {
                    Get-LocalUser | Where-Object { -not $_.Enabled } | ForEach-Object {
                        Add-ReportRow -Check "Disabled $($_.Name)" -Result 'Disabled' -Status 'WARN' -Detail ''
                        if ($PSCmdlet.ShouldProcess($_.Name, 'Enable-LocalUser')) {
                            Enable-LocalUser -Name $_.Name
                            Add-ReportRow -Check "Enable $($_.Name)" -Result 'Enabled' -Status 'PASS' -Detail ''
                        }
                    }
                } catch {
                    Add-ReportRow -Check 'Local unlock' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message
                }
            }
        }

        if ($Name -match 'Profile-Corruption|Temp-Profile|Login-Loop|Roaming-Profile') {
            $profileList = Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList' | ForEach-Object { Get-ItemProperty $_.PSPath }
            foreach ($p in $profileList) {
                if ($p.ProfileImagePath -notlike 'C:\Users\*') { continue }
                $state = $p.State
                $refCount = $p.RefCount
                Add-ReportRow -Check $p.ProfileImagePath -Result "State=$state RefCount=$refCount" -Status $(if ($state -eq 0) {'PASS'} else {'WARN'}) -Detail $p.PSChildName
            }
            $tempProf = $profileList | Where-Object { $_.ProfileImagePath -match 'TEMP' -or $_.State -eq 0x00000800 }
            if ($Name -match 'Temp-Profile') {
                Add-ReportRow -Check 'Temp profile indicators' -Result "$(@($tempProf).Count)" -Status $(if (@($tempProf).Count -eq 0) {'PASS'} else {'FAIL'}) -Detail 'Backup then remove .bak ProfileList SID keys carefully'
            }
            if ($Name -match 'Roaming-Profile') {
                $userProf = $env:USERPROFILE
                if (Test-Path $userProf) {
                    $size = (Get-ChildItem $userProf -Recurse -Force -ErrorAction SilentlyContinue | Measure-Object Length -Sum).Sum
                    Add-ReportRow -Check 'Profile size' -Result ("{0:N1} MB" -f ($size/1MB)) -Status $(if (($size/1MB) -lt 1024) {'PASS'} else {'WARN'}) -Detail $userProf
                }
            }
            if ($Name -match 'Login-Loop' -and $PSCmdlet.ShouldProcess('Explorer/Shell', 'Restart shell + reset Userinit check')) {
                ...
                Add-ReportRow -Check 'Userinit' -Result "$userinit" -Status $(if ($userinit -match 'userinit.exe') {'PASS'} else {'FAIL'}) -Detail 'Should include userinit.exe,'
                Get-Process explorer -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
                Start-Process explorer.exe
                Add-ReportRow -Check 'Explorer restart' -Result 'Done' -Status 'PASS' -Detail ''
            }
        }

        if ($Name -match 'Cached-Credential|SSO-Break|UPN-Mismatch|MFA-Token') {
            if ($Name -match 'Cached-Credential' -and $PSCmdlet.ShouldProcess('Credential Manager', 'cmdkey list + guidance')) {
                $creds = & cmdkey /list 2>&1 | Out-String
                Add-ReportRow -Check 'cmdkey list' -Result 'Captured' -Status 'INFO' -Detail ($creds.Substring(0, [Math]::Min(400, $creds.Length)))
                Add-ReportRow -Check 'Flush guidance' -Result 'cmdkey /delete:target for stale entries' -Status 'WARN' -Detail 'Also: Settings > Accounts > Email & accounts'
            }
            if ($Name -match 'SSO|UPN|MFA') {
                $ds = & dsregcmd /status 2>&1 | Out-String
                $prt = $ds -match 'AzureAdPrt\s*:\s*YES'
                Add-ReportRow -Check 'PRT' -Result $(if ($prt) {'YES'} else {'NO'}) -Status $(if ($prt) {'PASS'} else {'WARN'}) -Detail 'SSO depends on PRT for Entra'
                if ($PSCmdlet.ShouldProcess('PRT', 'dsregcmd /refreshprt')) {
                    & dsregcmd /refreshprt 2>&1 | Out-Null
                    Add-ReportRow -Check 'PRT refresh' -Result 'Triggered' -Status 'PASS' -Detail ''
                }
                Add-ReportRow -Check 'MFA guidance' -Result 'Reset via Entra admin / Authenticator app re-registration' -Status 'INFO' -Detail 'Script cannot reset cloud MFA secrets'
                if ($Name -match 'UPN') {
                    Add-ReportRow -Check 'USERDNSDOMAIN' -Result "$env:USERDNSDOMAIN" -Status 'INFO' -Detail "USERNAME=$env:USERNAME"
                }
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Account checks'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Roaming Profile Size Alert' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Roaming Profile Size Alert' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
