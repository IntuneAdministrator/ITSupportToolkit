...
<#
.SYNOPSIS
    Common problem fix: User Account Issues User Triage
.DESCRIPTION
...
.EXAMPLE
    .\User-Account-Issues-User-Triage.ps1
.EXAMPLE
    .\User-Account-Issues-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Account Issues User Triage'; $totalSteps = 8; $Name = 'User-Account-Issues-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'User Account Issues User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Account Issues User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
