...
<#
.SYNOPSIS
    Common problem fix: Built In Admin Unlock Guide
.DESCRIPTION
...
.EXAMPLE
    .\Built-In-Admin-Unlock-Guide
.EXAMPLE
    .\Built-In-Admin-Unlock-Guide -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Built In Admin Unlock Guide'; $totalSteps = 8; $Name = 'Built-In-Admin-Unlock-Guide'
try {
    ...
    Export-HtmlReport -Title 'Built In Admin Unlock Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Built In Admin Unlock Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
