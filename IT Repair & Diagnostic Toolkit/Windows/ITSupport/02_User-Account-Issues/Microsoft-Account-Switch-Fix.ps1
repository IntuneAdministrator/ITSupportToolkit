...
<#
.SYNOPSIS
    Common problem fix: Microsoft Account Switch Fix
.DESCRIPTION
...
.EXAMPLE
    .\Microsoft-Account-Switch-Fix
.EXAMPLE
    .\Microsoft-Account-Switch-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft Account Switch Fix'; $totalSteps = 8; $Name = 'Microsoft-Account-Switch-Fix'
try {
    ...
    Export-HtmlReport -Title 'Microsoft Account Switch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft Account Switch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
