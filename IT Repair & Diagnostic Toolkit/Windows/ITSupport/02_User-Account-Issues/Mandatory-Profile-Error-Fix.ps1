...
<#
.SYNOPSIS
    Common problem fix: Mandatory Profile Error Fix
.DESCRIPTION
...
.EXAMPLE
    .\Mandatory-Profile-Error-Fix
.EXAMPLE
    .\Mandatory-Profile-Error-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Mandatory Profile Error Fix'; $totalSteps = 8; $Name = 'Mandatory-Profile-Error-Fix'
try {
    ...
    Export-HtmlReport -Title 'Mandatory Profile Error Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Mandatory Profile Error Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
