...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: MFA Token Reset Guide
.DESCRIPTION
    Production-grade Windows IT Support tool for MFA-Token-Reset-Guide.
...
$exitCode = 0
$Activity = 'MFA Token Reset Guide'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'MFA-Token-Reset-Guide'
        Add-ReportRow -Check 'Tool' -Result 'MFA-Token-Reset-Guide' -Status 'INFO' -Detail 'ITSupport/02_User-Account-Issues'
        Write-SectionBanner 'USER ACCOUNT ISSUES'
    ...
    Export-HtmlReport -Title 'MFA Token Reset Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'MFA Token Reset Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
