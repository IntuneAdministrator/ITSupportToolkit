...
<#
.SYNOPSIS
    Common problem fix: RDP Printer Not Redirected Fix
.DESCRIPTION
...
.EXAMPLE
    .\RDP-Printer-Not-Redirected-Fix
.EXAMPLE
    .\RDP-Printer-Not-Redirected-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RDP Printer Not Redirected Fix'; $totalSteps = 8; $Name = 'RDP-Printer-Not-Redirected-Fix'
try {
    ...
    Export-HtmlReport -Title 'RDP Printer Not Redirected Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Printer Not Redirected Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
