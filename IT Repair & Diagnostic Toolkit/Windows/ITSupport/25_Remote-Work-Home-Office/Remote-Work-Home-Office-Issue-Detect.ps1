...
<#
.SYNOPSIS
    Common problem fix: Remote Work Home Office Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Work-Home-Office-Issue-Detect.ps1
.EXAMPLE
    .\Remote-Work-Home-Office-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Work Home Office Issue Detect'; $totalSteps = 8; $Name = 'Remote-Work-Home-Office-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Remote Work Home Office Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Work Home Office Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
