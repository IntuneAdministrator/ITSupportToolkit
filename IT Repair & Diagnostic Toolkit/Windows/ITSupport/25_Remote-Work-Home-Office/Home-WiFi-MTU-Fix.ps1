...
<#
.SYNOPSIS
    Common problem fix: Home WiFi MTU Fix
.DESCRIPTION
...
.EXAMPLE
    .\Home-WiFi-MTU-Fix.ps1
.EXAMPLE
    .\Home-WiFi-MTU-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Home WiFi MTU Fix'; $totalSteps = 8; $Name = 'Home-WiFi-MTU-Fix'
try {
    ...
    Export-HtmlReport -Title 'Home WiFi MTU Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Home WiFi MTU Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
