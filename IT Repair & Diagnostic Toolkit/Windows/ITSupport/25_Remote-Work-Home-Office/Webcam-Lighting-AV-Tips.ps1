...
$exitCode=0; $Activity='Webcam Lighting AV Tips'; $totalSteps=8; $Name='Webcam-Lighting-AV-Tips'; $Folder='25_Remote-Work-Home-Office'
try {
    ...
    Export-HtmlReport -Title 'Webcam Lighting AV Tips' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Webcam Lighting AV Tips' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
