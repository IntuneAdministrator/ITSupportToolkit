...
<#
.SYNOPSIS
    Common problem fix: RDP Audio Not Redirected Fix
.DESCRIPTION
...
.EXAMPLE
    .\RDP-Audio-Not-Redirected-Fix
.EXAMPLE
    .\RDP-Audio-Not-Redirected-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RDP Audio Not Redirected Fix'; $totalSteps = 8; $Name = 'RDP-Audio-Not-Redirected-Fix'
try {
    ...
    Export-HtmlReport -Title 'RDP Audio Not Redirected Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Audio Not Redirected Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
