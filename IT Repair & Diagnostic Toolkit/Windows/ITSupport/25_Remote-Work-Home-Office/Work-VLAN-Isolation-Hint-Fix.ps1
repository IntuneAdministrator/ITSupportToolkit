...
<#
.SYNOPSIS
    Common problem fix: Work VLAN Isolation Hint Fix
.DESCRIPTION
...
.EXAMPLE
    .\Work-VLAN-Isolation-Hint-Fix
.EXAMPLE
    .\Work-VLAN-Isolation-Hint-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Work VLAN Isolation Hint Fix'; $totalSteps = 8; $Name = 'Work-VLAN-Isolation-Hint-Fix'
try {
    ...
    Export-HtmlReport -Title 'Work VLAN Isolation Hint Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Work VLAN Isolation Hint Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
