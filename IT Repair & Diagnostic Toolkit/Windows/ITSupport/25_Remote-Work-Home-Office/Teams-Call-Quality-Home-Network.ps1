...
<#
.SYNOPSIS
    Common problem fix: Teams Call Quality Home Network
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Call-Quality-Home-Network.ps1
.EXAMPLE
    .\Teams-Call-Quality-Home-Network.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Call Quality Home Network'; $totalSteps = 8; $Name = 'Teams-Call-Quality-Home-Network'
try {
    ...
    Export-HtmlReport -Title 'Teams Call Quality Home Network' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Call Quality Home Network' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
