...
<#
.SYNOPSIS
    Enterprise IT tool: Split Tunnel VPN User Check
.DESCRIPTION
    Phase 11 expansion (ITSupport/25_Remote-Work-Home-Office). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Split-Tunnel-VPN-User-Check.ps1
.EXAMPLE
    .\Split-Tunnel-VPN-User-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Split Tunnel VPN User Check'; $totalSteps=8; $Name='Split-Tunnel-VPN-User-Check'; $Folder='25_Remote-Work-Home-Office'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '25_Remote-Work-Home-Office' -Status 'INFO' -Detail 'ITSupport'
    ...
    Export-HtmlReport -Title 'Split Tunnel VPN User Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Split Tunnel VPN User Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
