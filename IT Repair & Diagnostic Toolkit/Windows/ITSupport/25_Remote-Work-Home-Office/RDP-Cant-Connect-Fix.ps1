...
<#
.SYNOPSIS
    Common problem fix: RDP Cant Connect Fix
.DESCRIPTION
...
.EXAMPLE
    .\RDP-Cant-Connect-Fix
.EXAMPLE
    .\RDP-Cant-Connect-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RDP Cant Connect Fix'; $totalSteps = 8; $Name = 'RDP-Cant-Connect-Fix'
try {
    ...
    Export-HtmlReport -Title 'RDP Cant Connect Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Cant Connect Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
