...
<#
.SYNOPSIS
    Common problem fix: Dual Monitor Scaling Fix
.DESCRIPTION
...
.EXAMPLE
    .\Dual-Monitor-Scaling-Fix.ps1
.EXAMPLE
    .\Dual-Monitor-Scaling-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dual Monitor Scaling Fix'; $totalSteps = 8; $Name = 'Dual-Monitor-Scaling-Fix'
try {
    ...
    Export-HtmlReport -Title 'Dual Monitor Scaling Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dual Monitor Scaling Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
