...
<#
.SYNOPSIS
    Common problem fix: AnyDesk Not Connecting Fix
.DESCRIPTION
...
.EXAMPLE
    .\AnyDesk-Not-Connecting-Fix
.EXAMPLE
    .\AnyDesk-Not-Connecting-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AnyDesk Not Connecting Fix'; $totalSteps = 8; $Name = 'AnyDesk-Not-Connecting-Fix'
try {
    ...
    Export-HtmlReport -Title 'AnyDesk Not Connecting Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AnyDesk Not Connecting Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
