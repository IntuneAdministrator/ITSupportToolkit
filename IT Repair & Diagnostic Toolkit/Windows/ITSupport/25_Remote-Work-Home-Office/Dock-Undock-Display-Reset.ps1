...
<#
.SYNOPSIS
    Enterprise IT tool: Dock Undock Display Reset
.DESCRIPTION
    Phase 11 expansion (ITSupport/25_Remote-Work-Home-Office). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Dock-Undock-Display-Reset.ps1
.EXAMPLE
    .\Dock-Undock-Display-Reset.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Dock Undock Display Reset'; $totalSteps=8; $Name='Dock-Undock-Display-Reset'; $Folder='25_Remote-Work-Home-Office'
try {
    ...
    Export-HtmlReport -Title 'Dock Undock Display Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dock Undock Display Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
