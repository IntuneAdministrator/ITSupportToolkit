...
<#
.SYNOPSIS
    Common problem fix: Home Printer Not Shared Fix
.DESCRIPTION
...
.EXAMPLE
    .\Home-Printer-Not-Shared-Fix
.EXAMPLE
    .\Home-Printer-Not-Shared-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Home Printer Not Shared Fix'; $totalSteps = 8; $Name = 'Home-Printer-Not-Shared-Fix'
try {
    ...
    Export-HtmlReport -Title 'Home Printer Not Shared Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Home Printer Not Shared Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
