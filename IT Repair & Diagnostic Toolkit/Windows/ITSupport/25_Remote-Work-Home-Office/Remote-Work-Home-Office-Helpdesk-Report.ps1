...
<#
.SYNOPSIS
    Common problem fix: Remote Work Home Office Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 25_Remote-Work-Home-Office). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Remote-Work-Home-Office-Helpdesk-Report.ps1
.EXAMPLE
    .\Remote-Work-Home-Office-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Work Home Office Helpdesk Report'; $totalSteps = 8; $Name = 'Remote-Work-Home-Office-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '25_Remote-Work-Home-Office' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '25_Remote-Work-Home-Office'
    ...
    Export-HtmlReport -Title 'Remote Work Home Office Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Work Home Office Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
