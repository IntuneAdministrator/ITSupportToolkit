...
<#
.SYNOPSIS
    Common problem fix: RDP Dual Monitor Fix
.DESCRIPTION
...
.EXAMPLE
    .\RDP-Dual-Monitor-Fix
.EXAMPLE
    .\RDP-Dual-Monitor-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RDP Dual Monitor Fix'; $totalSteps = 8; $Name = 'RDP-Dual-Monitor-Fix'
try {
    ...
    Export-HtmlReport -Title 'RDP Dual Monitor Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Dual Monitor Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
