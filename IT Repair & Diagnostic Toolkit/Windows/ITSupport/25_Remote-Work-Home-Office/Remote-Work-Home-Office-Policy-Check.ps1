...
<#
.SYNOPSIS
    Common problem fix: Remote Work Home Office Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Work-Home-Office-Policy-Check.ps1
.EXAMPLE
    .\Remote-Work-Home-Office-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Work Home Office Policy Check'; $totalSteps = 8; $Name = 'Remote-Work-Home-Office-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Remote Work Home Office Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Work Home Office Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
