...
<#
.SYNOPSIS
    Common problem fix: RDP Black Screen Fix
.DESCRIPTION
...
$exitCode = 0; $Activity = 'RDP Black Screen Fix'; $totalSteps = 8; $Name = 'RDP-Black-Screen-Fix'
try {
    ...
    Export-HtmlReport -Title 'RDP Black Screen Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Black Screen Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
