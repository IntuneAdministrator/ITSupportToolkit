...
<#
.SYNOPSIS
    Enterprise IT tool: Home WiFi DNS Quick Test
.DESCRIPTION
    Phase 11 expansion (ITSupport/25_Remote-Work-Home-Office). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Home-WiFi-DNS-Quick-Test.ps1
.EXAMPLE
    .\Home-WiFi-DNS-Quick-Test.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Home WiFi DNS Quick Test'; $totalSteps=8; $Name='Home-WiFi-DNS-Quick-Test'; $Folder='25_Remote-Work-Home-Office'
try {
    ...
    Export-HtmlReport -Title 'Home WiFi DNS Quick Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Home WiFi DNS Quick Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
