...
<#
.SYNOPSIS
    Common problem fix: Home Network Discovery Off Fix
.DESCRIPTION
...
.EXAMPLE
    .\Home-Network-Discovery-Off-Fix
.EXAMPLE
    .\Home-Network-Discovery-Off-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Home Network Discovery Off Fix'; $totalSteps = 8; $Name = 'Home-Network-Discovery-Off-Fix'
try {
    ...
    Export-HtmlReport -Title 'Home Network Discovery Off Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Home Network Discovery Off Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
