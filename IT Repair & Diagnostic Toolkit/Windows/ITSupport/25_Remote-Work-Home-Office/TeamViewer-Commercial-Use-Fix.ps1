...
<#
.SYNOPSIS
    Common problem fix: TeamViewer Commercial Use Fix
.DESCRIPTION
...
.EXAMPLE
    .\TeamViewer-Commercial-Use-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'TeamViewer Commercial Use Fix'; $totalSteps = 8; $Name = 'TeamViewer-Commercial-Use-Fix'
try {
    ...
    Export-HtmlReport -Title 'TeamViewer Commercial Use Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'TeamViewer Commercial Use Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
