...
<#
.SYNOPSIS
    Enterprise IT tool: Print From Home IPP Guide
.DESCRIPTION
    Phase 11 expansion (ITSupport/25_Remote-Work-Home-Office). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Print-From-Home-IPP-Guide.ps1
.EXAMPLE
    .\Print-From-Home-IPP-Guide.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Print From Home IPP Guide'; $totalSteps=8; $Name='Print-From-Home-IPP-Guide'; $Folder='25_Remote-Work-Home-Office'
try {
    ...
    Export-HtmlReport -Title 'Print From Home IPP Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print From Home IPP Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
