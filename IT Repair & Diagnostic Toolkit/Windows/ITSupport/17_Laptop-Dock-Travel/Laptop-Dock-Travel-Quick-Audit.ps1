...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-Quick-Audit.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel Quick Audit'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
