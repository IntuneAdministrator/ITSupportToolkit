...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-Quick-Repair.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel Quick Repair'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
