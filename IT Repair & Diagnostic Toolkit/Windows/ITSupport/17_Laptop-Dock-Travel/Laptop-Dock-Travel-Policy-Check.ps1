...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-Policy-Check.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel Policy Check'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
