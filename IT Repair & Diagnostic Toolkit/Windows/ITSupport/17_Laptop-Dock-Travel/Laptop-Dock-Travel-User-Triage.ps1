...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-User-Triage.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel User Triage'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
