...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-Reset-Stack.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel Reset Stack'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
