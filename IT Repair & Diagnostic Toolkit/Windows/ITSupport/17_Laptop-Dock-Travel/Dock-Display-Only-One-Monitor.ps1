...
<#
.SYNOPSIS
    Enterprise IT tool: Dock Display Only One Monitor
.DESCRIPTION
    Phase 10 expansion (ITSupport/17_Laptop-Dock-Travel). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Dock-Display-Only-One-Monitor.ps1
.EXAMPLE
    .\Dock-Display-Only-One-Monitor.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Dock Display Only One Monitor'; $totalSteps=8; $Name='Dock-Display-Only-One-Monitor'; $Folder='17_Laptop-Dock-Travel'
try {
    ...
    Export-HtmlReport -Title 'Dock Display Only One Monitor' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dock Display Only One Monitor' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
