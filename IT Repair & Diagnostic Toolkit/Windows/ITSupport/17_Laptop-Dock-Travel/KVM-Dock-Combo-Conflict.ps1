...
<#
.SYNOPSIS
    Common problem fix: KVM Dock Combo Conflict
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 17_Laptop-Dock-Travel). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\KVM-Dock-Combo-Conflict.ps1
.EXAMPLE
    .\KVM-Dock-Combo-Conflict.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'KVM Dock Combo Conflict'; $totalSteps = 8; $Name = 'KVM-Dock-Combo-Conflict'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '17_Laptop-Dock-Travel' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '17_Laptop-Dock-Travel'
    ...
    Export-HtmlReport -Title 'KVM Dock Combo Conflict' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'KVM Dock Combo Conflict' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
