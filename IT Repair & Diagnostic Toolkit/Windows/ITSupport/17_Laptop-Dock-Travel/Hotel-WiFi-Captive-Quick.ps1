...
<#
.SYNOPSIS
    Enterprise IT tool: Hotel WiFi Captive Quick
.DESCRIPTION
    Phase 10 expansion (ITSupport/17_Laptop-Dock-Travel). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Hotel-WiFi-Captive-Quick.ps1
.EXAMPLE
    .\Hotel-WiFi-Captive-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Hotel WiFi Captive Quick'; $totalSteps=8; $Name='Hotel-WiFi-Captive-Quick'; $Folder='17_Laptop-Dock-Travel'
try {
    ...
    Export-HtmlReport -Title 'Hotel WiFi Captive Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hotel WiFi Captive Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
