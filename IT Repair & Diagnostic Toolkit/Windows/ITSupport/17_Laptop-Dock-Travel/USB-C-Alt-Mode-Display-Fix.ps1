...
<#
.SYNOPSIS
    Common problem fix: USB C Alt Mode Display Fix
.DESCRIPTION
...
.EXAMPLE
    .\USB-C-Alt-Mode-Display-Fix.ps1
.EXAMPLE
    .\USB-C-Alt-Mode-Display-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB C Alt Mode Display Fix'; $totalSteps = 8; $Name = 'USB-C-Alt-Mode-Display-Fix'
try {
    ...
    Export-HtmlReport -Title 'USB C Alt Mode Display Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB C Alt Mode Display Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
