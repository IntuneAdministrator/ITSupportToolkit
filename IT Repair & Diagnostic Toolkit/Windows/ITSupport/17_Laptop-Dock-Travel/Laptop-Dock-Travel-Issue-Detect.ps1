...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-Issue-Detect.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel Issue Detect'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
