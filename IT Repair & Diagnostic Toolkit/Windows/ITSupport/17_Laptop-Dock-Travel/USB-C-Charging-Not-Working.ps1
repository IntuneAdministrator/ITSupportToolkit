...
<#
.SYNOPSIS
    Enterprise IT tool: USB C Charging Not Working
.DESCRIPTION
    Phase 10 expansion (ITSupport/17_Laptop-Dock-Travel). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\USB-C-Charging-Not-Working.ps1
.EXAMPLE
    .\USB-C-Charging-Not-Working.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='USB C Charging Not Working'; $totalSteps=8; $Name='USB-C-Charging-Not-Working'; $Folder='17_Laptop-Dock-Travel'
try {
    ...
    Export-HtmlReport -Title 'USB C Charging Not Working' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB C Charging Not Working' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
