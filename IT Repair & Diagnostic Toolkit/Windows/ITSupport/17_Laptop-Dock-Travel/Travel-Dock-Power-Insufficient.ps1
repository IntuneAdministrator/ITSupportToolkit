...
<#
.SYNOPSIS
    Common problem fix: Travel Dock Power Insufficient
.DESCRIPTION
...
.EXAMPLE
    .\Travel-Dock-Power-Insufficient.ps1
.EXAMPLE
    .\Travel-Dock-Power-Insufficient.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Travel Dock Power Insufficient'; $totalSteps = 8; $Name = 'Travel-Dock-Power-Insufficient'
try {
    ...
    Export-HtmlReport -Title 'Travel Dock Power Insufficient' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Travel Dock Power Insufficient' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
