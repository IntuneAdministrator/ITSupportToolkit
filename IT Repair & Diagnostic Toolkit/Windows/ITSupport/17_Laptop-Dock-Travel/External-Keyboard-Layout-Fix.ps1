...
<#
.SYNOPSIS
    Enterprise IT tool: External Keyboard Layout Fix
.DESCRIPTION
    Phase 10 expansion (ITSupport/17_Laptop-Dock-Travel). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\External-Keyboard-Layout-Fix.ps1
.EXAMPLE
    .\External-Keyboard-Layout-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='External Keyboard Layout Fix'; $totalSteps=8; $Name='External-Keyboard-Layout-Fix'; $Folder='17_Laptop-Dock-Travel'
try {
    ...
    Export-HtmlReport -Title 'External Keyboard Layout Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'External Keyboard Layout Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
