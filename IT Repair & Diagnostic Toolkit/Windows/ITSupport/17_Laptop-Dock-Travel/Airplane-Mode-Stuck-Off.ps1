...
<#
.SYNOPSIS
    Enterprise IT tool: Airplane Mode Stuck Off
.DESCRIPTION
    Phase 10 expansion (ITSupport/17_Laptop-Dock-Travel). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Airplane-Mode-Stuck-Off.ps1
.EXAMPLE
    .\Airplane-Mode-Stuck-Off.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Airplane Mode Stuck Off'; $totalSteps=8; $Name='Airplane-Mode-Stuck-Off'; $Folder='17_Laptop-Dock-Travel'
try {
    ...
    Export-HtmlReport -Title 'Airplane Mode Stuck Off' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Airplane Mode Stuck Off' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
