...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-Helpdesk-Report.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel Helpdesk Report'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
