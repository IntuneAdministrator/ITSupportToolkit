...
<#
.SYNOPSIS
    Common problem fix: Laptop Dock Travel One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Dock-Travel-One-Click-Fix.ps1
.EXAMPLE
    .\Laptop-Dock-Travel-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Dock Travel One Click Fix'; $totalSteps = 8; $Name = 'Laptop-Dock-Travel-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Laptop Dock Travel One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Dock Travel One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
