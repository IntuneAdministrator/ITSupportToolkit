...
<#
.SYNOPSIS
    Common problem fix: Laptop Lid Close Dock Behavior
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Lid-Close-Dock-Behavior.ps1
.EXAMPLE
    .\Laptop-Lid-Close-Dock-Behavior.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Lid Close Dock Behavior'; $totalSteps = 8; $Name = 'Laptop-Lid-Close-Dock-Behavior'
try {
    ...
    Export-HtmlReport -Title 'Laptop Lid Close Dock Behavior' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Lid Close Dock Behavior' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
