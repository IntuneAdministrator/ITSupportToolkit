...
<#
.SYNOPSIS
    Common problem fix: Plotter Large Format Driver Hint
.DESCRIPTION
...
.EXAMPLE
    .\Plotter-Large-Format-Driver-Hint.ps1
.EXAMPLE
    .\Plotter-Large-Format-Driver-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Plotter Large Format Driver Hint'; $totalSteps = 8; $Name = 'Plotter-Large-Format-Driver-Hint'
try {
    ...
    Export-HtmlReport -Title 'Plotter Large Format Driver Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Plotter Large Format Driver Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
