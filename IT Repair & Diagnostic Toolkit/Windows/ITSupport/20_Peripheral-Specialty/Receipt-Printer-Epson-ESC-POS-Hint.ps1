...
<#
.SYNOPSIS
    Common problem fix: Receipt Printer Epson ESC POS Hint
.DESCRIPTION
...
.EXAMPLE
    .\Receipt-Printer-Epson-ESC-POS-Hint.ps1
.EXAMPLE
    .\Receipt-Printer-Epson-ESC-POS-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Receipt Printer Epson ESC POS Hint'; $totalSteps = 8; $Name = 'Receipt-Printer-Epson-ESC-POS-Hint'
try {
    ...
    Export-HtmlReport -Title 'Receipt Printer Epson ESC POS Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Receipt Printer Epson ESC POS Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
