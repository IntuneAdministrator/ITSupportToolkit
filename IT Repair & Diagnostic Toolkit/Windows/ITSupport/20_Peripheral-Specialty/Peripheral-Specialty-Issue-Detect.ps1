...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-Issue-Detect.ps1
.EXAMPLE
    .\Peripheral-Specialty-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty Issue Detect'; $totalSteps = 8; $Name = 'Peripheral-Specialty-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
