...
<#
.SYNOPSIS
    Common problem fix: Brother P Touch Label Printer
.DESCRIPTION
...
.EXAMPLE
    .\Brother-P-Touch-Label-Printer.ps1
.EXAMPLE
    .\Brother-P-Touch-Label-Printer.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Brother P Touch Label Printer'; $totalSteps = 8; $Name = 'Brother-P-Touch-Label-Printer'
try {
    ...
    Export-HtmlReport -Title 'Brother P Touch Label Printer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Brother P Touch Label Printer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
