...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-Quick-Repair.ps1
.EXAMPLE
    .\Peripheral-Specialty-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty Quick Repair'; $totalSteps = 8; $Name = 'Peripheral-Specialty-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
