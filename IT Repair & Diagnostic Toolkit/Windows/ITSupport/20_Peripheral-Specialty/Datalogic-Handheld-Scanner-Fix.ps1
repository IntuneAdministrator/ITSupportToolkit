...
<#
.SYNOPSIS
    Common problem fix: Datalogic Handheld Scanner Fix
.DESCRIPTION
...
.EXAMPLE
    .\Datalogic-Handheld-Scanner-Fix.ps1
.EXAMPLE
    .\Datalogic-Handheld-Scanner-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Datalogic Handheld Scanner Fix'; $totalSteps = 8; $Name = 'Datalogic-Handheld-Scanner-Fix'
try {
    ...
    Export-HtmlReport -Title 'Datalogic Handheld Scanner Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Datalogic Handheld Scanner Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
