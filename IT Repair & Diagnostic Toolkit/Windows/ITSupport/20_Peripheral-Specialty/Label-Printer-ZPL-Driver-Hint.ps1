...
<#
.SYNOPSIS
    Common problem fix: Label Printer ZPL Driver Hint
.DESCRIPTION
...
.EXAMPLE
    .\Label-Printer-ZPL-Driver-Hint.ps1
.EXAMPLE
    .\Label-Printer-ZPL-Driver-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Label Printer ZPL Driver Hint'; $totalSteps = 8; $Name = 'Label-Printer-ZPL-Driver-Hint'
try {
    ...
    Export-HtmlReport -Title 'Label Printer ZPL Driver Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Label Printer ZPL Driver Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
