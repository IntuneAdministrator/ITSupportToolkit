...
<#
.SYNOPSIS
    Common problem fix: Medical Dictation Foot Pedal USB
.DESCRIPTION
...
.EXAMPLE
    .\Medical-Dictation-Foot-Pedal-USB.ps1
.EXAMPLE
    .\Medical-Dictation-Foot-Pedal-USB.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Medical Dictation Foot Pedal USB'; $totalSteps = 8; $Name = 'Medical-Dictation-Foot-Pedal-USB'
try {
    ...
    Export-HtmlReport -Title 'Medical Dictation Foot Pedal USB' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Medical Dictation Foot Pedal USB' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
