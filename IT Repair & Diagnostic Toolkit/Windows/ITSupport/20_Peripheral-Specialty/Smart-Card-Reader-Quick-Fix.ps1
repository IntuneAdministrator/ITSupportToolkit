...
<#
.SYNOPSIS
    Enterprise IT tool: Smart Card Reader Quick Fix
.DESCRIPTION
    Phase 10 expansion (ITSupport/20_Peripheral-Specialty). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Smart-Card-Reader-Quick-Fix.ps1
.EXAMPLE
    .\Smart-Card-Reader-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Smart Card Reader Quick Fix'; $totalSteps=8; $Name='Smart-Card-Reader-Quick-Fix'; $Folder='20_Peripheral-Specialty'
try {
    ...
    Export-HtmlReport -Title 'Smart Card Reader Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Smart Card Reader Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
