...
<#
.SYNOPSIS
    Common problem fix: Honeywell Scanner OPOS Driver Hint
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 20_Peripheral-Specialty). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Honeywell-Scanner-OPOS-Driver-Hint.ps1
.EXAMPLE
    .\Honeywell-Scanner-OPOS-Driver-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Honeywell Scanner OPOS Driver Hint'; $totalSteps = 8; $Name = 'Honeywell-Scanner-OPOS-Driver-Hint'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '20_Peripheral-Specialty' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '20_Peripheral-Specialty'
    ...
    Export-HtmlReport -Title 'Honeywell Scanner OPOS Driver Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Honeywell Scanner OPOS Driver Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
