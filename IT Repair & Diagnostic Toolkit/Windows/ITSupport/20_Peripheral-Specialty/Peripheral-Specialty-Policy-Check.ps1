...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-Policy-Check.ps1
.EXAMPLE
    .\Peripheral-Specialty-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty Policy Check'; $totalSteps = 8; $Name = 'Peripheral-Specialty-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
