...
<#
.SYNOPSIS
    Common problem fix: Dymo LabelWriter Driver Reset
.DESCRIPTION
...
.EXAMPLE
    .\Dymo-LabelWriter-Driver-Reset.ps1
.EXAMPLE
    .\Dymo-LabelWriter-Driver-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dymo LabelWriter Driver Reset'; $totalSteps = 8; $Name = 'Dymo-LabelWriter-Driver-Reset'
try {
    ...
    Export-HtmlReport -Title 'Dymo LabelWriter Driver Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dymo LabelWriter Driver Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
