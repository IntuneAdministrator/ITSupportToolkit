...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-Reset-Stack.ps1
.EXAMPLE
    .\Peripheral-Specialty-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty Reset Stack'; $totalSteps = 8; $Name = 'Peripheral-Specialty-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
