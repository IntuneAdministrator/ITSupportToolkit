...
<#
.SYNOPSIS
    Enterprise IT tool: Headset Teams Certification Check
.DESCRIPTION
    Phase 10 expansion (ITSupport/20_Peripheral-Specialty). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Headset-Teams-Certification-Check.ps1
.EXAMPLE
    .\Headset-Teams-Certification-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Headset Teams Certification Check'; $totalSteps=8; $Name='Headset-Teams-Certification-Check'; $Folder='20_Peripheral-Specialty'
try {
    ...
    Export-HtmlReport -Title 'Headset Teams Certification Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Headset Teams Certification Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
