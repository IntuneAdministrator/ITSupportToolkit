...
<#
.SYNOPSIS
    Common problem fix: Foot Pedal HID Peripheral Fix
.DESCRIPTION
...
.EXAMPLE
    .\Foot-Pedal-HID-Peripheral-Fix.ps1
.EXAMPLE
    .\Foot-Pedal-HID-Peripheral-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Foot Pedal HID Peripheral Fix'; $totalSteps = 8; $Name = 'Foot-Pedal-HID-Peripheral-Fix'
try {
    ...
    Export-HtmlReport -Title 'Foot Pedal HID Peripheral Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Foot Pedal HID Peripheral Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
