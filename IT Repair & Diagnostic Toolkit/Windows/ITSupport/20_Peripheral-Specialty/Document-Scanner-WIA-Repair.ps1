...
<#
.SYNOPSIS
    Common problem fix: Document Scanner WIA Repair
.DESCRIPTION
...
.EXAMPLE
    .\Document-Scanner-WIA-Repair.ps1
.EXAMPLE
    .\Document-Scanner-WIA-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Document Scanner WIA Repair'; $totalSteps = 8; $Name = 'Document-Scanner-WIA-Repair'
try {
    ...
    Export-HtmlReport -Title 'Document Scanner WIA Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Document Scanner WIA Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
