...
<#
.SYNOPSIS
    Common problem fix: MagTek Card Reader HID Fix
.DESCRIPTION
...
.EXAMPLE
    .\MagTek-Card-Reader-HID-Fix.ps1
.EXAMPLE
    .\MagTek-Card-Reader-HID-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'MagTek Card Reader HID Fix'; $totalSteps = 8; $Name = 'MagTek-Card-Reader-HID-Fix'
try {
    ...
    Export-HtmlReport -Title 'MagTek Card Reader HID Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'MagTek Card Reader HID Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
