...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-Quick-Audit.ps1
.EXAMPLE
    .\Peripheral-Specialty-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty Quick Audit'; $totalSteps = 8; $Name = 'Peripheral-Specialty-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
