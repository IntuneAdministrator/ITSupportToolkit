...
<#
.SYNOPSIS
    Enterprise IT tool: Barcode Scanner COM Port Fix
.DESCRIPTION
    Phase 10 expansion (ITSupport/20_Peripheral-Specialty). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Barcode-Scanner-COM-Port-Fix.ps1
.EXAMPLE
    .\Barcode-Scanner-COM-Port-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Barcode Scanner COM Port Fix'; $totalSteps=8; $Name='Barcode-Scanner-COM-Port-Fix'; $Folder='20_Peripheral-Specialty'
try {
    ...
    Export-HtmlReport -Title 'Barcode Scanner COM Port Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Barcode Scanner COM Port Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
