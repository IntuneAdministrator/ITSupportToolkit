...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-One-Click-Fix.ps1
.EXAMPLE
    .\Peripheral-Specialty-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty One Click Fix'; $totalSteps = 8; $Name = 'Peripheral-Specialty-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
