...
<#
.SYNOPSIS
    Enterprise IT tool: Label Printer Quick Reset
.DESCRIPTION
    Phase 10 expansion (ITSupport/20_Peripheral-Specialty). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Label-Printer-Quick-Reset.ps1
.EXAMPLE
    .\Label-Printer-Quick-Reset.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Label Printer Quick Reset'; $totalSteps=8; $Name='Label-Printer-Quick-Reset'; $Folder='20_Peripheral-Specialty'
try {
    ...
    Export-HtmlReport -Title 'Label Printer Quick Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Label Printer Quick Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
