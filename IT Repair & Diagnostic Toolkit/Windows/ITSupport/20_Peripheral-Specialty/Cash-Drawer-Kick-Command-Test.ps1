...
<#
.SYNOPSIS
    Common problem fix: Cash Drawer Kick Command Test
.DESCRIPTION
...
.EXAMPLE
    .\Cash-Drawer-Kick-Command-Test.ps1
.EXAMPLE
    .\Cash-Drawer-Kick-Command-Test.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cash Drawer Kick Command Test'; $totalSteps = 8; $Name = 'Cash-Drawer-Kick-Command-Test'
try {
    ...
    Export-HtmlReport -Title 'Cash Drawer Kick Command Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cash Drawer Kick Command Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
