...
<#
.SYNOPSIS
    Common problem fix: 3D Mouse SpacePilot Driver Hint
.DESCRIPTION
...
.EXAMPLE
    .\3D-Mouse-SpacePilot-Driver-Hint.ps1
.EXAMPLE
    .\3D-Mouse-SpacePilot-Driver-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = '3D Mouse SpacePilot Driver Hint'; $totalSteps = 8; $Name = '3D-Mouse-SpacePilot-Driver-Hint'
try {
    ...
    Export-HtmlReport -Title '3D Mouse SpacePilot Driver Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title '3D Mouse SpacePilot Driver Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
