...
<#
.SYNOPSIS
    Common problem fix: Philips Dictation Device Hint
.DESCRIPTION
...
.EXAMPLE
    .\Philips-Dictation-Device-Hint.ps1
.EXAMPLE
    .\Philips-Dictation-Device-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Philips Dictation Device Hint'; $totalSteps = 8; $Name = 'Philips-Dictation-Device-Hint'
try {
    ...
    Export-HtmlReport -Title 'Philips Dictation Device Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Philips Dictation Device Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
