...
<#
.SYNOPSIS
    Common problem fix: USB Serial Adapter FTDI Driver
.DESCRIPTION
...
.EXAMPLE
    .\USB-Serial-Adapter-FTDI-Driver.ps1
.EXAMPLE
    .\USB-Serial-Adapter-FTDI-Driver.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Serial Adapter FTDI Driver'; $totalSteps = 8; $Name = 'USB-Serial-Adapter-FTDI-Driver'
try {
    ...
    Export-HtmlReport -Title 'USB Serial Adapter FTDI Driver' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Serial Adapter FTDI Driver' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
