...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-Helpdesk-Report.ps1
.EXAMPLE
    .\Peripheral-Specialty-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty Helpdesk Report'; $totalSteps = 8; $Name = 'Peripheral-Specialty-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
