...
<#
.SYNOPSIS
    Common problem fix: Pin Pad Payment Device HID Fix
.DESCRIPTION
...
.EXAMPLE
    .\Pin-Pad-Payment-Device-HID-Fix.ps1
.EXAMPLE
    .\Pin-Pad-Payment-Device-HID-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Pin Pad Payment Device HID Fix'; $totalSteps = 8; $Name = 'Pin-Pad-Payment-Device-HID-Fix'
try {
    ...
    Export-HtmlReport -Title 'Pin Pad Payment Device HID Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Pin Pad Payment Device HID Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
