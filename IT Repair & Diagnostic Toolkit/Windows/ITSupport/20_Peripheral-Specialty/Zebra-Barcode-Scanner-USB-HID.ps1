...
<#
.SYNOPSIS
    Common problem fix: Zebra Barcode Scanner USB HID
.DESCRIPTION
...
.EXAMPLE
    .\Zebra-Barcode-Scanner-USB-HID.ps1
.EXAMPLE
    .\Zebra-Barcode-Scanner-USB-HID.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Zebra Barcode Scanner USB HID'; $totalSteps = 8; $Name = 'Zebra-Barcode-Scanner-USB-HID'
try {
    ...
    Export-HtmlReport -Title 'Zebra Barcode Scanner USB HID' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Zebra Barcode Scanner USB HID' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
