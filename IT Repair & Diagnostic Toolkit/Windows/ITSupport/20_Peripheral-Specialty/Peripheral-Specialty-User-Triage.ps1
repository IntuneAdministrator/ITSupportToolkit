...
<#
.SYNOPSIS
    Common problem fix: Peripheral Specialty User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Specialty-User-Triage.ps1
.EXAMPLE
    .\Peripheral-Specialty-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Specialty User Triage'; $totalSteps = 8; $Name = 'Peripheral-Specialty-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Specialty User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Specialty User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
