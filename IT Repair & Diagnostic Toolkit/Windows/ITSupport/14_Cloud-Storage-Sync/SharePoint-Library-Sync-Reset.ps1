...
<#
.SYNOPSIS
    Enterprise IT tool: SharePoint Library Sync Reset
.DESCRIPTION
    Expansion module (ITSupport / 14_Cloud-Storage-Sync) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\SharePoint-Library-Sync-Reset.ps1
.EXAMPLE
    .\SharePoint-Library-Sync-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SharePoint Library Sync Reset'; $totalSteps = 8; $Name = 'SharePoint-Library-Sync-Reset'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '14_Cloud-Storage-Sync' -Status 'INFO' -Detail 'ITSupport'
    ...
    Export-HtmlReport -Title 'SharePoint Library Sync Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SharePoint Library Sync Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
