...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-Storage-Sync-Issue-Detect.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync Issue Detect'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
