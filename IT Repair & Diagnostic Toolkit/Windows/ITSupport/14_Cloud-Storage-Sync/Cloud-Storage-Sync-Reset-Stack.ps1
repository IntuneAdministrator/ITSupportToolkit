...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-Storage-Sync-Reset-Stack.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync Reset Stack'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
