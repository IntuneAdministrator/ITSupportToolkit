...
<#
.SYNOPSIS
    Enterprise IT tool: Known Folder Move Status
.DESCRIPTION
...
.EXAMPLE
    .\Known-Folder-Move-Status.ps1
.EXAMPLE
    .\Known-Folder-Move-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Known Folder Move Status'; $totalSteps = 8; $Name = 'Known-Folder-Move-Status'
try {
    ...
    Export-HtmlReport -Title 'Known Folder Move Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Known Folder Move Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
