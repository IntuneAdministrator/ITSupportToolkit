...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-Storage-Sync-One-Click-Fix.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync One Click Fix'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
