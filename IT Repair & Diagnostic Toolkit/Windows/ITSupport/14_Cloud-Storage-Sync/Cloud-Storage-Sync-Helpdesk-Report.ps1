...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 14_Cloud-Storage-Sync). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Cloud-Storage-Sync-Helpdesk-Report.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync Helpdesk Report'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '14_Cloud-Storage-Sync' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '14_Cloud-Storage-Sync'
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
