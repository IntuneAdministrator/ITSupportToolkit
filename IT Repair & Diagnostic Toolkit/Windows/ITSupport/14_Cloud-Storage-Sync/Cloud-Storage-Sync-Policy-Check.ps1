...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-Storage-Sync-Policy-Check.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync Policy Check'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
