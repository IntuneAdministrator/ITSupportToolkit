...
<#
.SYNOPSIS
    Enterprise IT tool: Files On Demand Toggle Repair
.DESCRIPTION
...
.EXAMPLE
    .\Files-On-Demand-Toggle-Repair.ps1
.EXAMPLE
    .\Files-On-Demand-Toggle-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Files On Demand Toggle Repair'; $totalSteps = 8; $Name = 'Files-On-Demand-Toggle-Repair'
try {
    ...
    Export-HtmlReport -Title 'Files On Demand Toggle Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Files On Demand Toggle Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
