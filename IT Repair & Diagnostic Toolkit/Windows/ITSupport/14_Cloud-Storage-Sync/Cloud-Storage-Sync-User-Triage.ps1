...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-Storage-Sync-User-Triage.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync User Triage'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
