...
<#
.SYNOPSIS
    Enterprise IT tool: OneDrive Quota Quick Check
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-Quota-Quick-Check.ps1
.EXAMPLE
    .\OneDrive-Quota-Quick-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive Quota Quick Check'; $totalSteps = 8; $Name = 'OneDrive-Quota-Quick-Check'
try {
    ...
    Export-HtmlReport -Title 'OneDrive Quota Quick Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive Quota Quick Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
