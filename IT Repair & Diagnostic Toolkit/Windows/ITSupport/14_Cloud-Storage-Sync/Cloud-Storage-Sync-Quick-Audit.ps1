...
<#
.SYNOPSIS
    Common problem fix: Cloud Storage Sync Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-Storage-Sync-Quick-Audit.ps1
.EXAMPLE
    .\Cloud-Storage-Sync-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud Storage Sync Quick Audit'; $totalSteps = 8; $Name = 'Cloud-Storage-Sync-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Cloud Storage Sync Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud Storage Sync Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
