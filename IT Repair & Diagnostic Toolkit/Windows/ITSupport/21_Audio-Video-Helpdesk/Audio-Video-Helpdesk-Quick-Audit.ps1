...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Helpdesk-Quick-Audit.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk Quick Audit'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
