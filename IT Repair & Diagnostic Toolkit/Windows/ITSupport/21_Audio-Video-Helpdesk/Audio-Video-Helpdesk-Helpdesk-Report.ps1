...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 21_Audio-Video-Helpdesk). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Audio-Video-Helpdesk-Helpdesk-Report.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk Helpdesk Report'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '21_Audio-Video-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '21_Audio-Video-Helpdesk'
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
