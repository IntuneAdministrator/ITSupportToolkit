...
<#
.SYNOPSIS
    Enterprise IT tool: Teams Zoom AV Quick Fix
.DESCRIPTION
    Phase 11 expansion (ITSupport/21_Audio-Video-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Teams-Zoom-AV-Quick-Fix.ps1
.EXAMPLE
    .\Teams-Zoom-AV-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Teams Zoom AV Quick Fix'; $totalSteps=8; $Name='Teams-Zoom-AV-Quick-Fix'; $Folder='21_Audio-Video-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Teams Zoom AV Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Zoom AV Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
