...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Helpdesk-Issue-Detect.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk Issue Detect'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
