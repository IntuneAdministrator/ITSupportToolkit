...
<#
.SYNOPSIS
    Enterprise IT tool: Webcam Device Presence Test
.DESCRIPTION
    Phase 11 expansion (ITSupport/21_Audio-Video-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Webcam-Device-Presence-Test.ps1
.EXAMPLE
    .\Webcam-Device-Presence-Test.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Webcam Device Presence Test'; $totalSteps=8; $Name='Webcam-Device-Presence-Test'; $Folder='21_Audio-Video-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Webcam Device Presence Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Webcam Device Presence Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
