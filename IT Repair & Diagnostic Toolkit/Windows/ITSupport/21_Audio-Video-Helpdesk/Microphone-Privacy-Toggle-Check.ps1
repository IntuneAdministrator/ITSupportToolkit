...
<#
.SYNOPSIS
    Enterprise IT tool: Microphone Privacy Toggle Check
.DESCRIPTION
    Phase 11 expansion (ITSupport/21_Audio-Video-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Microphone-Privacy-Toggle-Check.ps1
.EXAMPLE
    .\Microphone-Privacy-Toggle-Check.ps1 -WhatIf
.NOTES
    Author: Allester Padovani | Version: 1.3.0 | Windows
#>
...
$exitCode=0; $Activity='Microphone Privacy Toggle Check'; $totalSteps=8; $Name='Microphone-Privacy-Toggle-Check'; $Folder='21_Audio-Video-Helpdesk'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '21_Audio-Video-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        Write-SectionBanner 'ITSUPPORT WINDOWS PHASE11'
        Write-ProgressStep 2 $totalSteps $Activity 'Helpdesk'
        Add-ReportRow -Check 'User' -Result $env:USERNAME -Status 'INFO' -Detail $env:USERPROFILE
        if ($Name -match 'Audio|Microphone|Webcam|HDMI|Teams-Zoom|AV') {
            Get-PnpDevice -Class MEDIA,AudioEndpoint,Camera,Image -EA SilentlyContinue | Select-Object -First 12 | ForEach-Object {
                Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status $(if($_.Status -eq 'OK'){'PASS'}else{'WARN'}) -Detail $_.InstanceId
            }
            if ($PSCmdlet.ShouldProcess('Audio', 'restart AudioSrv')) {
                Restart-Service Audiosrv,AudioEndpointBuilder -Force -EA SilentlyContinue
                Add-ReportRow -Check 'Audio services' -Result 'Restarted' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Font|Language|Region|IME|ClearType') {
            $cul = Get-Culture; $ui = Get-WinSystemLocale
            Add-ReportRow -Check 'Culture/UI' -Result "$($cul.Name) / $($ui.Name)" -Status 'INFO' -Detail ''
            Add-ReportRow -Check 'Language packs' -Result ((Get-WinUserLanguageList | ForEach-Object LanguageTag) -join ', ') -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Startup|Login|Shell-Folder|Fast-Startup|Welcome') {
            $run = @(Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run' -EA SilentlyContinue).PSObject.Properties | Where-Object { $_.Name -notmatch '^PS' }
            Add-ReportRow -Check 'HKCU Run' -Result "$($run.Count) entries" -Status 'INFO' -Detail ''
            $hiberboot = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power' -Name HiberbootEnabled -EA SilentlyContinue).HiberbootEnabled
            Add-ReportRow -Check 'Fast Startup' -Result "HiberbootEnabled=$hiberboot" -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Explorer|Thumbnail|Context-Menu|Recycle|Quick-Access') {
            if ($PSCmdlet.ShouldProcess('Explorer', 'refresh shell caches')) {
            ...
            Add-ReportRow -Check 'Thumbcache path' -Result $thumb -Status 'INFO' -Detail 'Delete thumbcache_*.db after closing Explorer if rebuilding'
        }
        if ($Name -match 'Home-WiFi|VPN|Dock|Print-From-Home|Webcam-Lighting') {
            $gw = Get-NetRoute -DestinationPrefix '0.0.0.0/0' -EA SilentlyContinue | Sort-Object RouteMetric | Select-Object -First 1
            Add-ReportRow -Check 'Default route' -Result $(if($gw){"via $($gw.NextHop)"}else{'None'}) -Status $(if($gw){'PASS'}else{'FAIL'}) -Detail ''
            Resolve-DnsName www.msftconnecttest.com -EA SilentlyContinue | Out-Null
            Add-ReportRow -Check 'DNS sample' -Result 'msftconnecttest resolved or failed (see log)' -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Account|Unlock|PIN|Password-Expired|Lockout|Cached-Credential') {
            Add-ReportRow -Check 'Identity path' -Result $Join.Label -Status 'INFO' -Detail 'Domain: unlock on DC/Entra; local: lusrmgr/net user'
            if ($Name -match 'Cached-Credential' -and $PSCmdlet.ShouldProcess('Credential Manager', 'guide only')) {
                Start-Process 'control.exe' -ArgumentList '/name','Microsoft.CredentialManager' -EA SilentlyContinue
                Add-ReportRow -Check 'Credential Manager' -Result 'Opened' -Status 'PASS' -Detail ''
            }
            if ($Name -match 'Lockout') {
                $ev = Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4740; StartTime=(Get-Date).AddDays(-7)} -MaxEvents 5 -EA SilentlyContinue
                Add-ReportRow -Check 'Lockout 4740 (7d)' -Result $(if($ev){"$($ev.Count) events"}else{'None/no access'}) -Status 'INFO' -Detail ''
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'ITSupport P11 done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Microphone Privacy Toggle Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microphone Privacy Toggle Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
