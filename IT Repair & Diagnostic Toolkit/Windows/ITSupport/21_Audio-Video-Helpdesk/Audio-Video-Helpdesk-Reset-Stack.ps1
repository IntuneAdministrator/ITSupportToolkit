...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Helpdesk-Reset-Stack.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk Reset Stack'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
