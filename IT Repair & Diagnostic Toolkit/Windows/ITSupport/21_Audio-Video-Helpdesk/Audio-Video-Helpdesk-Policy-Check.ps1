...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Helpdesk-Policy-Check.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk Policy Check'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
