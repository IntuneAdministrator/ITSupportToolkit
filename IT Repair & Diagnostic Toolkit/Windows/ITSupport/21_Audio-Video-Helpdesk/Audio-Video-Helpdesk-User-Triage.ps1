...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Helpdesk-User-Triage.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk User Triage'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
