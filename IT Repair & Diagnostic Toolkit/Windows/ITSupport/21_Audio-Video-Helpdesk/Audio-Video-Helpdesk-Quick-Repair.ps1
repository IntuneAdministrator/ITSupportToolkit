...
<#
.SYNOPSIS
    Common problem fix: Audio Video Helpdesk Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Video-Helpdesk-Quick-Repair.ps1
.EXAMPLE
    .\Audio-Video-Helpdesk-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Video Helpdesk Quick Repair'; $totalSteps = 8; $Name = 'Audio-Video-Helpdesk-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Audio Video Helpdesk Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Video Helpdesk Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
