...
<#
.SYNOPSIS
    Enterprise IT tool: Audio Endpoint Reset Quick
.DESCRIPTION
    Phase 11 expansion (ITSupport/21_Audio-Video-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Audio-Endpoint-Reset-Quick.ps1
.EXAMPLE
    .\Audio-Endpoint-Reset-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Audio Endpoint Reset Quick'; $totalSteps=8; $Name='Audio-Endpoint-Reset-Quick'; $Folder='21_Audio-Video-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Audio Endpoint Reset Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Endpoint Reset Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
