...
<#
.SYNOPSIS
    Enterprise IT tool: HDMI Display Audio Switch
.DESCRIPTION
    Phase 11 expansion (ITSupport/21_Audio-Video-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\HDMI-Display-Audio-Switch.ps1
.EXAMPLE
    .\HDMI-Display-Audio-Switch.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='HDMI Display Audio Switch'; $totalSteps=8; $Name='HDMI-Display-Audio-Switch'; $Folder='21_Audio-Video-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'HDMI Display Audio Switch' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HDMI Display Audio Switch' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
