...
<#
.SYNOPSIS
    Common problem fix: Fast Startup Problems Fix
.DESCRIPTION
...
.EXAMPLE
    .\Fast-Startup-Problems-Fix.ps1
.EXAMPLE
    .\Fast-Startup-Problems-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Fast Startup Problems Fix'; $totalSteps = 8; $Name = 'Fast-Startup-Problems-Fix'
try {
    ...
    Export-HtmlReport -Title 'Fast Startup Problems Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Fast Startup Problems Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
