...
<#
.SYNOPSIS
    Common problem fix: BitLocker Key Not Found Hint
.DESCRIPTION
...
.EXAMPLE
    .\BitLocker-Key-Not-Found-Hint
.EXAMPLE
    .\BitLocker-Key-Not-Found-Hint -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BitLocker Key Not Found Hint'; $totalSteps = 8; $Name = 'BitLocker-Key-Not-Found-Hint'
try {
    ...
    Export-HtmlReport -Title 'BitLocker Key Not Found Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BitLocker Key Not Found Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
