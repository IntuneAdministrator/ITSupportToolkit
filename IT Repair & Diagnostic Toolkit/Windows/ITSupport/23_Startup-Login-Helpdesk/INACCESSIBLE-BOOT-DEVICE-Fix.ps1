...
<#
.SYNOPSIS
    Common problem fix: INACCESSIBLE BOOT DEVICE Fix
.DESCRIPTION
...
.EXAMPLE
    .\INACCESSIBLE-BOOT-DEVICE-Fix
.EXAMPLE
    .\INACCESSIBLE-BOOT-DEVICE-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'INACCESSIBLE BOOT DEVICE Fix'; $totalSteps = 8; $Name = 'INACCESSIBLE-BOOT-DEVICE-Fix'
try {
    ...
    Export-HtmlReport -Title 'INACCESSIBLE BOOT DEVICE Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'INACCESSIBLE BOOT DEVICE Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
