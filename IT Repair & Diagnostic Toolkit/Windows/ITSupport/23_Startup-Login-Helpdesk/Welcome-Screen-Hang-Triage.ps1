...
<#
.SYNOPSIS
    Enterprise IT tool: Welcome Screen Hang Triage
.DESCRIPTION
    Phase 11 expansion (ITSupport/23_Startup-Login-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Welcome-Screen-Hang-Triage.ps1
.EXAMPLE
    .\Welcome-Screen-Hang-Triage.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Welcome Screen Hang Triage'; $totalSteps=8; $Name='Welcome-Screen-Hang-Triage'; $Folder='23_Startup-Login-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Welcome Screen Hang Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Welcome Screen Hang Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
