...
<#
.SYNOPSIS
    Common problem fix: BitLocker Recovery Screen Fix
.DESCRIPTION
...
.EXAMPLE
    .\BitLocker-Recovery-Screen-Fix
.EXAMPLE
    .\BitLocker-Recovery-Screen-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BitLocker Recovery Screen Fix'; $totalSteps = 8; $Name = 'BitLocker-Recovery-Screen-Fix'
try {
    ...
    Export-HtmlReport -Title 'BitLocker Recovery Screen Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BitLocker Recovery Screen Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
