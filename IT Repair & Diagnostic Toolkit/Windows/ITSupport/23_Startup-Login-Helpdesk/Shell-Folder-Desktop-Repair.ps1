...
<#
.SYNOPSIS
    Enterprise IT tool: Shell Folder Desktop Repair
.DESCRIPTION
    Phase 11 expansion (ITSupport/23_Startup-Login-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Shell-Folder-Desktop-Repair.ps1
.EXAMPLE
    .\Shell-Folder-Desktop-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Shell Folder Desktop Repair'; $totalSteps=8; $Name='Shell-Folder-Desktop-Repair'; $Folder='23_Startup-Login-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Shell Folder Desktop Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shell Folder Desktop Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
