...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-Quick-Audit.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk Quick Audit'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
