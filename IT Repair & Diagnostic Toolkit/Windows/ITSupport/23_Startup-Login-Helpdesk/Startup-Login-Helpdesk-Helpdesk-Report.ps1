...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-Helpdesk-Report.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk Helpdesk Report'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
