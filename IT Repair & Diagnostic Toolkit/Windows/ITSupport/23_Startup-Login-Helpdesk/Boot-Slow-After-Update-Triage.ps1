...
<#
.SYNOPSIS
    Common problem fix: Boot Slow After Update Triage
.DESCRIPTION
...
.EXAMPLE
    .\Boot-Slow-After-Update-Triage.ps1
.EXAMPLE
    .\Boot-Slow-After-Update-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Boot Slow After Update Triage'; $totalSteps = 8; $Name = 'Boot-Slow-After-Update-Triage'
try {
    ...
    Export-HtmlReport -Title 'Boot Slow After Update Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Boot Slow After Update Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
