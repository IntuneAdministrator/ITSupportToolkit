...
<#
.SYNOPSIS
    Common problem fix: Boot Device Not Found Fix
.DESCRIPTION
...
.EXAMPLE
    .\Boot-Device-Not-Found-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Boot Device Not Found Fix'; $totalSteps = 8; $Name = 'Boot-Device-Not-Found-Fix'
try {
    ...
    Export-HtmlReport -Title 'Boot Device Not Found Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Boot Device Not Found Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
