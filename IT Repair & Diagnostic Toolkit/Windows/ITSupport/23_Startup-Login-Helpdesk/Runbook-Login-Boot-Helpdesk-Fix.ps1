...
<#
.SYNOPSIS
    Common problem fix: Runbook Login Boot Helpdesk Fix
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 23_Startup-Login-Helpdesk). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-Login-Boot-Helpdesk-Fix.ps1
.EXAMPLE
    .\Runbook-Login-Boot-Helpdesk-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Login Boot Helpdesk Fix'; $totalSteps = 8; $Name = 'Runbook-Login-Boot-Helpdesk-Fix'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '23_Startup-Login-Helpdesk' -Status 'INFO' -Detail 'ITSupport'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '23_Startup-Login-Helpdesk'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '23_Startup-Login-Helpdesk'
    ...
    Export-HtmlReport -Title 'Runbook Login Boot Helpdesk Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Login Boot Helpdesk Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
