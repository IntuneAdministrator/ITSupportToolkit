...
<#
.SYNOPSIS
    Common problem fix: Startup Repair Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Repair-Failed-Fix
.EXAMPLE
    .\Startup-Repair-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Repair Failed Fix'; $totalSteps = 8; $Name = 'Startup-Repair-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Startup Repair Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Repair Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
