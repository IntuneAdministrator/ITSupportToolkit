...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-Quick-Repair.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk Quick Repair'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
