...
<#
.SYNOPSIS
    Enterprise IT tool: Login Script Timeout Hint
.DESCRIPTION
    Phase 11 expansion (ITSupport/23_Startup-Login-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Login-Script-Timeout-Hint.ps1
.EXAMPLE
    .\Login-Script-Timeout-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Login Script Timeout Hint'; $totalSteps=8; $Name='Login-Script-Timeout-Hint'; $Folder='23_Startup-Login-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Login Script Timeout Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Login Script Timeout Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
