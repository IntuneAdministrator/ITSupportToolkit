...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-User-Triage.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk User Triage'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
