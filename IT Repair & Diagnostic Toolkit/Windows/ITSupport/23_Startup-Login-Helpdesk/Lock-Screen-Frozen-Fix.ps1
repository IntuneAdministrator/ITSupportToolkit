...
<#
.SYNOPSIS
    Common problem fix: Lock Screen Frozen Fix
.DESCRIPTION
...
.EXAMPLE
    .\Lock-Screen-Frozen-Fix
.EXAMPLE
    .\Lock-Screen-Frozen-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lock Screen Frozen Fix'; $totalSteps = 8; $Name = 'Lock-Screen-Frozen-Fix'
try {
    ...
    Export-HtmlReport -Title 'Lock Screen Frozen Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lock Screen Frozen Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
