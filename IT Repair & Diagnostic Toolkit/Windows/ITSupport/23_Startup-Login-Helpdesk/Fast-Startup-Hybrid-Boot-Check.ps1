...
<#
.SYNOPSIS
    Enterprise IT tool: Fast Startup Hybrid Boot Check
.DESCRIPTION
    Phase 11 expansion (ITSupport/23_Startup-Login-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Fast-Startup-Hybrid-Boot-Check.ps1
.EXAMPLE
    .\Fast-Startup-Hybrid-Boot-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Fast Startup Hybrid Boot Check'; $totalSteps=8; $Name='Fast-Startup-Hybrid-Boot-Check'; $Folder='23_Startup-Login-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Fast Startup Hybrid Boot Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Fast Startup Hybrid Boot Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
