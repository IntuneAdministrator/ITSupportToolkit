...
<#
.SYNOPSIS
    Enterprise IT tool: Startup App Inventory Quick
.DESCRIPTION
    Phase 11 expansion (ITSupport/23_Startup-Login-Helpdesk). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Startup-App-Inventory-Quick.ps1
.EXAMPLE
    .\Startup-App-Inventory-Quick.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Startup App Inventory Quick'; $totalSteps=8; $Name='Startup-App-Inventory-Quick'; $Folder='23_Startup-Login-Helpdesk'
try {
    ...
    Export-HtmlReport -Title 'Startup App Inventory Quick' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup App Inventory Quick' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
