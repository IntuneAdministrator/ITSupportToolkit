...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-Reset-Stack.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk Reset Stack'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
