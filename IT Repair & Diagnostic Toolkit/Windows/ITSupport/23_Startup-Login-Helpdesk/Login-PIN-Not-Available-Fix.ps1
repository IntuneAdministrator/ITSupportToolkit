...
<#
.SYNOPSIS
    Common problem fix: Login PIN Not Available Fix
.DESCRIPTION
...
.EXAMPLE
    .\Login-PIN-Not-Available-Fix
.EXAMPLE
    .\Login-PIN-Not-Available-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Login PIN Not Available Fix'; $totalSteps = 8; $Name = 'Login-PIN-Not-Available-Fix'
try {
    ...
    Export-HtmlReport -Title 'Login PIN Not Available Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Login PIN Not Available Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
