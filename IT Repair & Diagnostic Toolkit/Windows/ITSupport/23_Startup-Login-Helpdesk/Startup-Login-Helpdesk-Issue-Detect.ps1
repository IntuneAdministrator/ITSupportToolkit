...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-Issue-Detect.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk Issue Detect'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
