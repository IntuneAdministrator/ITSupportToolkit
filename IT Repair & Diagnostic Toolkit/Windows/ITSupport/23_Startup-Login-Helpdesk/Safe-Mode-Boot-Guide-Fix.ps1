...
<#
.SYNOPSIS
    Common problem fix: Safe Mode Boot Guide Fix
.DESCRIPTION
...
.EXAMPLE
    .\Safe-Mode-Boot-Guide-Fix
.EXAMPLE
    .\Safe-Mode-Boot-Guide-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Safe Mode Boot Guide Fix'; $totalSteps = 8; $Name = 'Safe-Mode-Boot-Guide-Fix'
try {
    ...
    Export-HtmlReport -Title 'Safe Mode Boot Guide Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Safe Mode Boot Guide Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
