...
<#
.SYNOPSIS
    Common problem fix: Sign In Options Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Sign-In-Options-Missing-Fix
.EXAMPLE
    .\Sign-In-Options-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Sign In Options Missing Fix'; $totalSteps = 8; $Name = 'Sign-In-Options-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Sign In Options Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Sign In Options Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
