...
<#
.SYNOPSIS
    Common problem fix: Login Screen Spinning Fix
.DESCRIPTION
...
.EXAMPLE
    .\Login-Screen-Spinning-Fix.ps1
.EXAMPLE
    .\Login-Screen-Spinning-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Login Screen Spinning Fix'; $totalSteps = 8; $Name = 'Login-Screen-Spinning-Fix'
try {
    ...
    Export-HtmlReport -Title 'Login Screen Spinning Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Login Screen Spinning Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
