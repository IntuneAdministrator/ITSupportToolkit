...
<#
.SYNOPSIS
    Common problem fix: Startup Login Helpdesk Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Startup-Login-Helpdesk-Policy-Check.ps1
.EXAMPLE
    .\Startup-Login-Helpdesk-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Startup Login Helpdesk Policy Check'; $totalSteps = 8; $Name = 'Startup-Login-Helpdesk-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Startup Login Helpdesk Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Startup Login Helpdesk Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
