...
<#
.SYNOPSIS
    Common problem fix: Login Password Correct Rejected Fix
.DESCRIPTION
...
.EXAMPLE
    .\Login-Password-Correct-Rejected-Fix
.EXAMPLE
    .\Login-Password-Correct-Rejected-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Login Password Correct Rejected Fix'; $totalSteps = 8; $Name = 'Login-Password-Correct-Rejected-Fix'
try {
    ...
    Export-HtmlReport -Title 'Login Password Correct Rejected Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Login Password Correct Rejected Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
