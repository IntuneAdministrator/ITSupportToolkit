...
<#
.SYNOPSIS
    Common problem fix: Secure Boot Violation Fix
.DESCRIPTION
...
.EXAMPLE
    .\Secure-Boot-Violation-Fix
.EXAMPLE
    .\Secure-Boot-Violation-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Secure Boot Violation Fix'; $totalSteps = 8; $Name = 'Secure-Boot-Violation-Fix'
try {
    ...
    Export-HtmlReport -Title 'Secure Boot Violation Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Secure Boot Violation Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
