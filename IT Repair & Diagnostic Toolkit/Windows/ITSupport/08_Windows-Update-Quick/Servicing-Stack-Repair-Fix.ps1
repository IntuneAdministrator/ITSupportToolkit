...
<#
.SYNOPSIS
    Common problem fix: Servicing Stack Repair Fix
.DESCRIPTION
...
.EXAMPLE
    .\Servicing-Stack-Repair-Fix
.EXAMPLE
    .\Servicing-Stack-Repair-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Servicing Stack Repair Fix'; $totalSteps = 8; $Name = 'Servicing-Stack-Repair-Fix'
try {
    ...
    Export-HtmlReport -Title 'Servicing Stack Repair Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Servicing Stack Repair Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
