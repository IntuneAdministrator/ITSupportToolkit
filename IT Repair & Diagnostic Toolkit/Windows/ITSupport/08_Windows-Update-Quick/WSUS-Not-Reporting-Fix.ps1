...
<#
.SYNOPSIS
    Common problem fix: WSUS Not Reporting Fix
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Not-Reporting-Fix
.EXAMPLE
    .\WSUS-Not-Reporting-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Not Reporting Fix'; $totalSteps = 8; $Name = 'WSUS-Not-Reporting-Fix'
try {
    ...
    Export-HtmlReport -Title 'WSUS Not Reporting Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Not Reporting Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
