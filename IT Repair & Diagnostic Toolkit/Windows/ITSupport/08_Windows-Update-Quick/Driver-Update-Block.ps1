...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Driver Update Block
.DESCRIPTION
    Production-grade Windows IT Support tool for Driver-Update-Block.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Driver-Update-Block'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Update-Block' -Status 'INFO' -Detail 'ITSupport/08_Windows-Update-Quick'
        Write-SectionBanner 'WINDOWS UPDATE QUICK'
    ...
    Export-HtmlReport -Title 'Driver Update Block' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Update Block' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
