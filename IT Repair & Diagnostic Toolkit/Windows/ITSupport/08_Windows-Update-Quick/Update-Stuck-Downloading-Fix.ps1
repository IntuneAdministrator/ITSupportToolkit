...
<#
.SYNOPSIS
    Common problem fix: Update Stuck Downloading Fix
.DESCRIPTION
...
.EXAMPLE
    .\Update-Stuck-Downloading-Fix
.EXAMPLE
    .\Update-Stuck-Downloading-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Update Stuck Downloading Fix'; $totalSteps = 8; $Name = 'Update-Stuck-Downloading-Fix'
try {
    ...
    Export-HtmlReport -Title 'Update Stuck Downloading Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Update Stuck Downloading Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
