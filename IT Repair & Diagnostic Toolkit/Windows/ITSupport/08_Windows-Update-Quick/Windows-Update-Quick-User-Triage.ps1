...
<#
.SYNOPSIS
    Common problem fix: Windows Update Quick User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Update-Quick-User-Triage.ps1
.EXAMPLE
    .\Windows-Update-Quick-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update Quick User Triage'; $totalSteps = 8; $Name = 'Windows-Update-Quick-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Windows Update Quick User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update Quick User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
