...
<#
.SYNOPSIS
    Common problem fix: Quality Update Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Quality-Update-Failed-Fix
.EXAMPLE
    .\Quality-Update-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quality Update Failed Fix'; $totalSteps = 8; $Name = 'Quality-Update-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Quality Update Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quality Update Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
