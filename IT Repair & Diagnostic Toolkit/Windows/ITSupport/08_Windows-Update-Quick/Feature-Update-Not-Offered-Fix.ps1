...
<#
.SYNOPSIS
    Common problem fix: Feature Update Not Offered Fix
.DESCRIPTION
...
.EXAMPLE
    .\Feature-Update-Not-Offered-Fix
.EXAMPLE
    .\Feature-Update-Not-Offered-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Feature Update Not Offered Fix'; $totalSteps = 8; $Name = 'Feature-Update-Not-Offered-Fix'
try {
    ...
    Export-HtmlReport -Title 'Feature Update Not Offered Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Feature Update Not Offered Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
