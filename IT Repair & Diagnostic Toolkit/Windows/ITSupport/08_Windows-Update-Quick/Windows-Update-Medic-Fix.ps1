...
<#
.SYNOPSIS
    Common problem fix: Windows Update Medic Fix
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Update-Medic-Fix
.EXAMPLE
    .\Windows-Update-Medic-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update Medic Fix'; $totalSteps = 8; $Name = 'Windows-Update-Medic-Fix'
try {
    ...
    Export-HtmlReport -Title 'Windows Update Medic Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update Medic Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
