...
<#
.SYNOPSIS
    Common problem fix: Update Cache Clear Fix
.DESCRIPTION
...
.EXAMPLE
    .\Update-Cache-Clear-Fix
.EXAMPLE
    .\Update-Cache-Clear-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Update Cache Clear Fix'; $totalSteps = 8; $Name = 'Update-Cache-Clear-Fix'
try {
    ...
    Export-HtmlReport -Title 'Update Cache Clear Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Update Cache Clear Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
