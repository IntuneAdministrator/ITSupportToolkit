...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Update Notification Reset
.DESCRIPTION
    Production-grade Windows IT Support tool for Update-Notification-Reset.
Category: 08_Windows-Update-Quick. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Update-Notification-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Update-Notification-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Update Notification Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'WINDOWS UPDATE QUICK'
        $Name = 'Update-Notification-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Update-Notification-Reset' -Status 'INFO' -Detail 'ITSupport/08_Windows-Update-Quick'
        Write-SectionBanner 'WINDOWS UPDATE QUICK'
        $wu = Get-Service wuauserv -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'wuauserv' -Result $(if ($wu) { "$($wu.Status)/$($wu.StartType)" } else {'N/A'}) -Status 'INFO' -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'WU service'

        if ($Name -match 'Stuck|Quick-Fix' -and $PSCmdlet.ShouldProcess('Windows Update', 'Soft reset SoftwareDistribution')) {
            foreach ($s in @('wuauserv','bits','UsoSvc','dosvc')) { Stop-Service $s -Force -EA SilentlyContinue }
            $sd = "$env:SystemRoot\SoftwareDistribution"
            if (Test-Path $sd) {
                $bak = "SoftwareDistribution.bak_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
                Rename-Item $sd $bak -EA SilentlyContinue
                Add-ReportRow -Check 'SoftwareDistribution' -Result "Renamed to $bak" -Status 'PASS' -Detail ''
            }
            foreach ($s in @('bits','wuauserv','dosvc','UsoSvc')) { Start-Service $s -EA SilentlyContinue }
            Add-ReportRow -Check 'WU services' -Result 'Restarted' -Status 'PASS' -Detail ''
        }

        if ($Name -match 'Force-Update-Check') {
            try {
                UsoClient StartInteractiveScan 2>$null
                Add-ReportRow -Check 'Update scan' -Result 'UsoClient StartInteractiveScan' -Status 'PASS' -Detail ''
            } catch {
                Add-ReportRow -Check 'Update scan' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message
            }
        }

        if ($Name -match 'Pause-Update-Clear|Notification') {
            $pauseKeys = @(
            ...
            foreach ($k in $pauseKeys) {
                if (Test-Path $k) {
                    Backup-RegistryKey -KeyPath $k -BackupDir 'C:\IT-Logs\Backups' | Out-Null
                    Add-ReportRow -Check "WU key $k" -Result 'Backed up' -Status 'INFO' -Detail ''
                }
            }
            if ($PSCmdlet.ShouldProcess('Pause updates', 'Clear pause values if present')) {
                $ux = 'HKLM:\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings'
                if (Test-Path $ux) {
                    Remove-ItemProperty $ux -Name PauseUpdatesExpiryTime -EA SilentlyContinue
                    Remove-ItemProperty $ux -Name PauseFeatureUpdatesEndTime -EA SilentlyContinue
                    Remove-ItemProperty $ux -Name PauseQualityUpdatesEndTime -EA SilentlyContinue
                    Add-ReportRow -Check 'Pause values' -Result 'Cleared if present' -Status 'PASS' -Detail $ux
                }
            }
        }

        if ($Name -match 'Driver-Update-Block') {
            $ex = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
            if (-not (Test-Path $ex)) { New-Item $ex -Force | Out-Null }
            Backup-RegistryKey -KeyPath $ex -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            if ($PSCmdlet.ShouldProcess('WU drivers', 'Exclude drivers from WU')) {
                New-ItemProperty $ex -Name ExcludeWUDriversInQualityUpdate -Value 1 -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'ExcludeWUDriversInQualityUpdate' -Result '1' -Status 'PASS' -Detail 'Rollback via .reg'
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'WU quick'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Update Notification Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Update Notification Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
