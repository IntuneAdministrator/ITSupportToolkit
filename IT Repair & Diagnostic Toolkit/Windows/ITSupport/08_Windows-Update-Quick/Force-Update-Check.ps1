...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Force Update Check
.DESCRIPTION
    Production-grade Windows IT Support tool for Force-Update-Check.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Force-Update-Check' -Status 'INFO' -Detail 'ITSupport/08_Windows-Update-Quick'
        Write-SectionBanner 'WINDOWS UPDATE QUICK'
    ...
    Export-HtmlReport -Title 'Force Update Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Force Update Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
