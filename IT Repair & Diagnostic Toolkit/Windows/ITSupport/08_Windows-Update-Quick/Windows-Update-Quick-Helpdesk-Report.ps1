...
<#
.SYNOPSIS
    Common problem fix: Windows Update Quick Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 08_Windows-Update-Quick). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Windows-Update-Quick-Helpdesk-Report.ps1
.EXAMPLE
    .\Windows-Update-Quick-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update Quick Helpdesk Report'; $totalSteps = 8; $Name = 'Windows-Update-Quick-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '08_Windows-Update-Quick' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '08_Windows-Update-Quick'
    ...
    Export-HtmlReport -Title 'Windows Update Quick Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update Quick Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
