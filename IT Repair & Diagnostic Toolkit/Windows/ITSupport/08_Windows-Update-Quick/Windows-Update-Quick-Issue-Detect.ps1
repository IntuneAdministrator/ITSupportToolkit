...
<#
.SYNOPSIS
    Common problem fix: Windows Update Quick Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Update-Quick-Issue-Detect.ps1
.EXAMPLE
    .\Windows-Update-Quick-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update Quick Issue Detect'; $totalSteps = 8; $Name = 'Windows-Update-Quick-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Windows Update Quick Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update Quick Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
