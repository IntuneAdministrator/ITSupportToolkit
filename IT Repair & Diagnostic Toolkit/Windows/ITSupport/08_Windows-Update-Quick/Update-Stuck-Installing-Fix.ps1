...
<#
.SYNOPSIS
    Common problem fix: Update Stuck Installing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Update-Stuck-Installing-Fix
.EXAMPLE
    .\Update-Stuck-Installing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Update Stuck Installing Fix'; $totalSteps = 8; $Name = 'Update-Stuck-Installing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Update Stuck Installing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Update Stuck Installing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
