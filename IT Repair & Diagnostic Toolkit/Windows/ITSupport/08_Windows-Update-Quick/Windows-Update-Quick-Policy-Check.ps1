...
<#
.SYNOPSIS
    Common problem fix: Windows Update Quick Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Update-Quick-Policy-Check.ps1
.EXAMPLE
    .\Windows-Update-Quick-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update Quick Policy Check'; $totalSteps = 8; $Name = 'Windows-Update-Quick-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Windows Update Quick Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update Quick Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
