...
<#
.SYNOPSIS
    Common problem fix: Windows Update Quick Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Update-Quick-Reset-Stack.ps1
.EXAMPLE
    .\Windows-Update-Quick-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Update Quick Reset Stack'; $totalSteps = 8; $Name = 'Windows-Update-Quick-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Windows Update Quick Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Update Quick Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
