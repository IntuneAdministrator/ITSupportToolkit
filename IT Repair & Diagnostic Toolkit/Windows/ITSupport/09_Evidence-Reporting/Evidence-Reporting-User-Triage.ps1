...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-User-Triage.ps1
.EXAMPLE
    .\Evidence-Reporting-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting User Triage'; $totalSteps = 8; $Name = 'Evidence-Reporting-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
