...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting Helpdesk Report
.DESCRIPTION
    Phase 13 common-problems module (ITSupport / 09_Evidence-Reporting). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Evidence-Reporting-Helpdesk-Report.ps1
.EXAMPLE
    .\Evidence-Reporting-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting Helpdesk Report'; $totalSteps = 8; $Name = 'Evidence-Reporting-Helpdesk-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '09_Evidence-Reporting' -Status 'INFO' -Detail 'ITSupport'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '09_Evidence-Reporting'
    ...
    Export-HtmlReport -Title 'Evidence Reporting Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
