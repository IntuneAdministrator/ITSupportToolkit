...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Event Log Quick Export
.DESCRIPTION
    Production-grade Windows IT Support tool for Event-Log-Quick-Export.
...
$exitCode = 0
$Activity = 'Event Log Quick Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Event-Log-Quick-Export'
        Add-ReportRow -Check 'Tool' -Result 'Event-Log-Quick-Export' -Status 'INFO' -Detail 'ITSupport/09_Evidence-Reporting'
        Write-SectionBanner 'EVIDENCE / REPORTING'
    ...
    Export-HtmlReport -Title 'Event Log Quick Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Event Log Quick Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
