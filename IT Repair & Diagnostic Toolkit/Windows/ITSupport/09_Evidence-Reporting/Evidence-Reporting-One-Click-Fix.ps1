...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-One-Click-Fix.ps1
.EXAMPLE
    .\Evidence-Reporting-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting One Click Fix'; $totalSteps = 8; $Name = 'Evidence-Reporting-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
