...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-Issue-Detect.ps1
.EXAMPLE
    .\Evidence-Reporting-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting Issue Detect'; $totalSteps = 8; $Name = 'Evidence-Reporting-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
