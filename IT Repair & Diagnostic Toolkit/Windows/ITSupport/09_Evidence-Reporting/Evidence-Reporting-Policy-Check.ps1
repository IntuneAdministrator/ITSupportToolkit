...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-Policy-Check.ps1
.EXAMPLE
    .\Evidence-Reporting-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting Policy Check'; $totalSteps = 8; $Name = 'Evidence-Reporting-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
