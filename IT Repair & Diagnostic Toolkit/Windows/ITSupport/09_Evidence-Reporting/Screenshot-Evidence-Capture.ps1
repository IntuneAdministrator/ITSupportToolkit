...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Screenshot Evidence Capture
.DESCRIPTION
    Production-grade Windows IT Support tool for Screenshot-Evidence-Capture.
...
.EXAMPLE
    .\Screenshot-Evidence-Capture.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Screenshot-Evidence-Capture.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Screenshot Evidence Capture'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Screenshot-Evidence-Capture'
        Add-ReportRow -Check 'Tool' -Result 'Screenshot-Evidence-Capture' -Status 'INFO' -Detail 'ITSupport/09_Evidence-Reporting'
        Write-SectionBanner 'EVIDENCE / REPORTING'
    ...
    Export-HtmlReport -Title 'Screenshot Evidence Capture' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Screenshot Evidence Capture' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
