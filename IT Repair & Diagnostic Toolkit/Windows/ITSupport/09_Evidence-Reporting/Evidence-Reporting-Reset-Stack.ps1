...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-Reset-Stack.ps1
.EXAMPLE
    .\Evidence-Reporting-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting Reset Stack'; $totalSteps = 8; $Name = 'Evidence-Reporting-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
