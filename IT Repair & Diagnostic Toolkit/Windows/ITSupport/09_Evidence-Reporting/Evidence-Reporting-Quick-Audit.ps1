...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-Quick-Audit.ps1
.EXAMPLE
    .\Evidence-Reporting-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting Quick Audit'; $totalSteps = 8; $Name = 'Evidence-Reporting-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
