...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: System Snapshot Report
.DESCRIPTION
    Production-grade Windows IT Support tool for System-Snapshot-Report.
...
$exitCode = 0
$Activity = 'System Snapshot Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'System-Snapshot-Report'
        Add-ReportRow -Check 'Tool' -Result 'System-Snapshot-Report' -Status 'INFO' -Detail 'ITSupport/09_Evidence-Reporting'
        Write-SectionBanner 'EVIDENCE / REPORTING'
    ...
    Export-HtmlReport -Title 'System Snapshot Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System Snapshot Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
