...
<#
.SYNOPSIS
    Common problem fix: Evidence Reporting Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Evidence-Reporting-Quick-Repair.ps1
.EXAMPLE
    .\Evidence-Reporting-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Evidence Reporting Quick Repair'; $totalSteps = 8; $Name = 'Evidence-Reporting-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Evidence Reporting Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Evidence Reporting Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
