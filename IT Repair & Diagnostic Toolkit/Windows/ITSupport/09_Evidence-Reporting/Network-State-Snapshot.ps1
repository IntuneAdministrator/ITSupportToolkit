...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Network State Snapshot
.DESCRIPTION
    Production-grade Windows IT Support tool for Network-State-Snapshot.
...
$exitCode = 0
$Activity = 'Network State Snapshot'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Network-State-Snapshot'
        Add-ReportRow -Check 'Tool' -Result 'Network-State-Snapshot' -Status 'INFO' -Detail 'ITSupport/09_Evidence-Reporting'
        Write-SectionBanner 'EVIDENCE / REPORTING'
    ...
    Export-HtmlReport -Title 'Network State Snapshot' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network State Snapshot' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
