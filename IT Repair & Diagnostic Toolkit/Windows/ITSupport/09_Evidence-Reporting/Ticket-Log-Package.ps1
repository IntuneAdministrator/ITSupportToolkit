...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Ticket Log Package
.DESCRIPTION
    Production-grade Windows IT Support tool for Ticket-Log-Package.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Ticket-Log-Package'
        Add-ReportRow -Check 'Tool' -Result 'Ticket-Log-Package' -Status 'INFO' -Detail 'ITSupport/09_Evidence-Reporting'
        Write-SectionBanner 'EVIDENCE / REPORTING'
    ...
    Export-HtmlReport -Title 'Ticket Log Package' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Ticket Log Package' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
