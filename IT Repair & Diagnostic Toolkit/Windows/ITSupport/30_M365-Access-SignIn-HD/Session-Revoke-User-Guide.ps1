...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Session Revoke User Guide
.DESCRIPTION
...
.EXAMPLE
    .\Session-Revoke-User-Guide.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Session-Revoke-User-Guide.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Session Revoke User Guide'; $totalSteps = 8; $Name = 'Session-Revoke-User-Guide'; $Folder = '30_M365-Access-SignIn-HD'
$DefaultScopes = @('User.RevokeSessions.All','User.Read.All')
try {
    ...
    Export-HtmlReport -Title 'Session Revoke User Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Session Revoke User Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
