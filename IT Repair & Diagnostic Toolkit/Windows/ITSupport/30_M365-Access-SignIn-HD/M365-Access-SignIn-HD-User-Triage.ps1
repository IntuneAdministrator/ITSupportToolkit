...
<#
.SYNOPSIS
    Common problem fix: M365 Access SignIn HD User Triage
.DESCRIPTION
...
.EXAMPLE
    .\M365-Access-SignIn-HD-User-Triage.ps1
.EXAMPLE
    .\M365-Access-SignIn-HD-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'M365 Access SignIn HD User Triage'; $totalSteps = 8; $Name = 'M365-Access-SignIn-HD-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'M365 Access SignIn HD User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'M365 Access SignIn HD User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
