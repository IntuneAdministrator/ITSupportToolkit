...
<#
.SYNOPSIS
    Common problem fix: M365 Access SignIn HD Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\M365-Access-SignIn-HD-Reset-Stack.ps1
.EXAMPLE
    .\M365-Access-SignIn-HD-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'M365 Access SignIn HD Reset Stack'; $totalSteps = 8; $Name = 'M365-Access-SignIn-HD-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'M365 Access SignIn HD Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'M365 Access SignIn HD Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
