...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Admin Role PIM Elevation Hint
.DESCRIPTION
    Phase 12 M365 tenant admin (ITSupport/30_M365-Access-SignIn-HD). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\Admin-Role-PIM-Elevation-Hint.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Admin-Role-PIM-Elevation-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Admin Role PIM Elevation Hint'; $totalSteps = 8; $Name = 'Admin-Role-PIM-Elevation-Hint'; $Folder = '30_M365-Access-SignIn-HD'
...
try {
    ...
    Add-ReportRow -Check 'Module' -Result '30_M365-Access-SignIn-HD' -Status 'INFO' -Detail 'ITSupport Phase12 M365'
    Add-M365PortalRows
    ...
    Export-HtmlReport -Title 'Admin Role PIM Elevation Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Admin Role PIM Elevation Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
