...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Legacy Auth Blocked Outlook
.DESCRIPTION
...
.EXAMPLE
    .\Legacy-Auth-Blocked-Outlook.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Legacy-Auth-Blocked-Outlook.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Legacy Auth Blocked Outlook'; $totalSteps = 8; $Name = 'Legacy-Auth-Blocked-Outlook'; $Folder = '30_M365-Access-SignIn-HD'
$DefaultScopes = @('UserAuthenticationMethod.Read.All','User.Read.All')
try {
    ...
    Export-HtmlReport -Title 'Legacy Auth Blocked Outlook' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Legacy Auth Blocked Outlook' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
