...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Scan To Email Test
.DESCRIPTION
    Production-grade Windows IT Support tool for Scan-To-Email-Test.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Scan-To-Email-Test'
        Add-ReportRow -Check 'Tool' -Result 'Scan-To-Email-Test' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
    ...
    Export-HtmlReport -Title 'Scan To Email Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Scan To Email Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
