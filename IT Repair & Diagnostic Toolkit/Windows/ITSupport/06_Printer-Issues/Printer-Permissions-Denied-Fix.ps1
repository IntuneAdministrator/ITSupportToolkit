...
<#
.SYNOPSIS
    Common problem fix: Printer Permissions Denied Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Permissions-Denied-Fix
.EXAMPLE
    .\Printer-Permissions-Denied-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Permissions Denied Fix'; $totalSteps = 8; $Name = 'Printer-Permissions-Denied-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer Permissions Denied Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Permissions Denied Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
