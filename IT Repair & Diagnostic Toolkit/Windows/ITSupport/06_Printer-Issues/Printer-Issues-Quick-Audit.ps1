...
<#
.SYNOPSIS
    Common problem fix: Printer Issues Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-Quick-Audit.ps1
.EXAMPLE
    .\Printer-Issues-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues Quick Audit'; $totalSteps = 8; $Name = 'Printer-Issues-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
