...
<#
.SYNOPSIS
    Common problem fix: Printer IP Changed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-IP-Changed-Fix
.EXAMPLE
    .\Printer-IP-Changed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer IP Changed Fix'; $totalSteps = 8; $Name = 'Printer-IP-Changed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer IP Changed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer IP Changed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
