...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Printer Not Found Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Printer-Not-Found-Fix.
...
$exitCode = 0
$Activity = 'Printer Not Found Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Printer-Not-Found-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Printer-Not-Found-Fix' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
    ...
    Export-HtmlReport -Title 'Printer Not Found Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Printer Not Found Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
