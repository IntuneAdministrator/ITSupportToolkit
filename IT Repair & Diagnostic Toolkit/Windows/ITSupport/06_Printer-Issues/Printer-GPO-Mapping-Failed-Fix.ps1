...
<#
.SYNOPSIS
    Common problem fix: Printer GPO Mapping Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-GPO-Mapping-Failed-Fix
.EXAMPLE
    .\Printer-GPO-Mapping-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer GPO Mapping Failed Fix'; $totalSteps = 8; $Name = 'Printer-GPO-Mapping-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer GPO Mapping Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer GPO Mapping Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
