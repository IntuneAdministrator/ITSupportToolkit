...
<#
.SYNOPSIS
    Common problem fix: Printer Issues Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-Reset-Stack.ps1
.EXAMPLE
    .\Printer-Issues-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues Reset Stack'; $totalSteps = 8; $Name = 'Printer-Issues-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
