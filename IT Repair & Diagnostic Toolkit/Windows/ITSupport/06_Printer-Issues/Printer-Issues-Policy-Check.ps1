...
<#
.SYNOPSIS
    Common problem fix: Printer Issues Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-Policy-Check.ps1
.EXAMPLE
    .\Printer-Issues-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues Policy Check'; $totalSteps = 8; $Name = 'Printer-Issues-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
