...
<#
.SYNOPSIS
    Common problem fix: Star Micronics Receipt Printer
.DESCRIPTION
...
.EXAMPLE
    .\Star-Micronics-Receipt-Printer.ps1
.EXAMPLE
    .\Star-Micronics-Receipt-Printer.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Star Micronics Receipt Printer'; $totalSteps = 8; $Name = 'Star-Micronics-Receipt-Printer'
try {
    ...
    Export-HtmlReport -Title 'Star Micronics Receipt Printer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Star Micronics Receipt Printer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
