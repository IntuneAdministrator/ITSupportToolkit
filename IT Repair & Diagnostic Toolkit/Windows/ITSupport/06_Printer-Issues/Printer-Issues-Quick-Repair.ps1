...
<#
.SYNOPSIS
    Common problem fix: Printer Issues Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-Quick-Repair.ps1
.EXAMPLE
    .\Printer-Issues-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues Quick Repair'; $totalSteps = 8; $Name = 'Printer-Issues-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
