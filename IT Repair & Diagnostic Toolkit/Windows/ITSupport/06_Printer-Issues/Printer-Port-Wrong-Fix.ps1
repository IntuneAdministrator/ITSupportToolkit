...
<#
.SYNOPSIS
    Common problem fix: Printer Port Wrong Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Port-Wrong-Fix
.EXAMPLE
    .\Printer-Port-Wrong-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Port Wrong Fix'; $totalSteps = 8; $Name = 'Printer-Port-Wrong-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer Port Wrong Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Port Wrong Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
