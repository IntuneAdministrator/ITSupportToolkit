...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Print Queue Emergency Clear
.DESCRIPTION
    Production-grade Windows IT Support tool for Print-Queue-Emergency-Clear.
...
.EXAMPLE
    .\Print-Queue-Emergency-Clear.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Print-Queue-Emergency-Clear.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Print Queue Emergency Clear'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Print-Queue-Emergency-Clear' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
    ...
    Export-HtmlReport -Title 'Print Queue Emergency Clear' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Print Queue Emergency Clear' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
