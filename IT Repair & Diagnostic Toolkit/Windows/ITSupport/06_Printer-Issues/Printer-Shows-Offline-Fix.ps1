...
<#
.SYNOPSIS
    Common problem fix: Printer Shows Offline Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Shows-Offline-Fix.ps1
.EXAMPLE
    .\Printer-Shows-Offline-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Shows Offline Fix'; $totalSteps = 8; $Name = 'Printer-Shows-Offline-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer Shows Offline Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Shows Offline Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
