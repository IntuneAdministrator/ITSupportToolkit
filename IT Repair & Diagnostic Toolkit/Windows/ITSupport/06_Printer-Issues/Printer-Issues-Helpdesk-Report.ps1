...
<#
.SYNOPSIS
    Common problem fix: Printer Issues Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-Helpdesk-Report.ps1
.EXAMPLE
    .\Printer-Issues-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues Helpdesk Report'; $totalSteps = 8; $Name = 'Printer-Issues-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
