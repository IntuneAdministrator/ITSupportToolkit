...
<#
.SYNOPSIS
    Common problem fix: Print Quality Poor Triage Fix
.DESCRIPTION
...
.EXAMPLE
    .\Print-Quality-Poor-Triage-Fix
.EXAMPLE
    .\Print-Quality-Poor-Triage-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Quality Poor Triage Fix'; $totalSteps = 8; $Name = 'Print-Quality-Poor-Triage-Fix'
try {
    ...
    Export-HtmlReport -Title 'Print Quality Poor Triage Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Quality Poor Triage Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
