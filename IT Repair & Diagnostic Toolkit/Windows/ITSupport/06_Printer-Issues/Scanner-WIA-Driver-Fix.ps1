...
<#
.SYNOPSIS
    Common problem fix: Scanner WIA Driver Fix
.DESCRIPTION
...
.EXAMPLE
    .\Scanner-WIA-Driver-Fix
.EXAMPLE
    .\Scanner-WIA-Driver-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scanner WIA Driver Fix'; $totalSteps = 8; $Name = 'Scanner-WIA-Driver-Fix'
try {
    ...
    Export-HtmlReport -Title 'Scanner WIA Driver Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scanner WIA Driver Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
