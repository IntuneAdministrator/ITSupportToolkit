...
<#
.SYNOPSIS
    Common problem fix: Universal Print Token Fix
.DESCRIPTION
...
.EXAMPLE
    .\Universal-Print-Token-Fix
.EXAMPLE
    .\Universal-Print-Token-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Universal Print Token Fix'; $totalSteps = 8; $Name = 'Universal-Print-Token-Fix'
try {
    ...
    Export-HtmlReport -Title 'Universal Print Token Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Universal Print Token Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
