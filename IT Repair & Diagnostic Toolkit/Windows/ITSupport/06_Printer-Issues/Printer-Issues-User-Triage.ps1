...
<#
.SYNOPSIS
    Common problem fix: Printer Issues User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-User-Triage.ps1
.EXAMPLE
    .\Printer-Issues-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues User Triage'; $totalSteps = 8; $Name = 'Printer-Issues-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
