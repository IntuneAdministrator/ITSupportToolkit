...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Printer Mapping GPO Force
.DESCRIPTION
    Production-grade Windows IT Support tool for Printer-Mapping-GPO-Force.
...
.EXAMPLE
    .\Printer-Mapping-GPO-Force.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Printer-Mapping-GPO-Force.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Printer Mapping GPO Force'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Printer-Mapping-GPO-Force'
        Add-ReportRow -Check 'Tool' -Result 'Printer-Mapping-GPO-Force' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
    ...
    Export-HtmlReport -Title 'Printer Mapping GPO Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Printer Mapping GPO Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
