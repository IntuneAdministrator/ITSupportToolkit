...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: PCL vs PS Driver Swap
.DESCRIPTION
    Production-grade Windows IT Support tool for PCL-vs-PS-Driver-Swap.
...
$exitCode = 0
$Activity = 'PCL vs PS Driver Swap'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PCL-vs-PS-Driver-Swap'
        Add-ReportRow -Check 'Tool' -Result 'PCL-vs-PS-Driver-Swap' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
    ...
    Export-HtmlReport -Title 'PCL vs PS Driver Swap' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PCL vs PS Driver Swap' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
