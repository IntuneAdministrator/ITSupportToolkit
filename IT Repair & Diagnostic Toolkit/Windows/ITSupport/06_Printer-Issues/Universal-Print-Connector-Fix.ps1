...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Universal Print Connector Fix
.DESCRIPTION
    Production-grade Windows IT Support tool for Universal-Print-Connector-Fix.
Category: 06_Printer-Issues. Optimized for Help Desk / Tier-1/2 rapid remediation.
...
.EXAMPLE
    .\Universal-Print-Connector-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Universal-Print-Connector-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Universal Print Connector Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PRINTER ISSUES'
        $Name = 'Universal-Print-Connector-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Universal-Print-Connector-Fix' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
        Write-SectionBanner 'PRINTER QUICK FIX'
        if ($PSCmdlet.ShouldProcess('Spooler', 'Restart and clear queue if needed')) {
            Stop-Service Spooler -Force -ErrorAction SilentlyContinue
            if ($Name -match 'Emergency-Clear|Not-Found|Default-Printer') {
                Remove-Item "$env:SystemRoot\System32\spool\PRINTERS\*" -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Print queue clear' -Result 'Cleared' -Status 'PASS' -Detail 'PRINTERS folder'
            }
            Start-Service Spooler -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Spooler' -Result (Get-Service Spooler).Status -Status 'PASS' -Detail ''
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Spooler'

        $printers = @(Get-Printer -ErrorAction SilentlyContinue)
        foreach ($p in $printers) {
            Add-ReportRow -Check $p.Name -Result "$($p.PrinterStatus)" -Status 'INFO' -Detail "$($p.DriverName) @ $($p.PortName)"
        }
        if ($Name -match 'Default-Printer' -and $printers.Count -gt 0 -and $PSCmdlet.ShouldProcess('Default printer', 'Set first ready printer default')) {
            $candidate = $printers | Where-Object { $_.PrinterStatus -eq 'Normal' } | Select-Object -First 1
            if ($candidate) {
                Set-Printer -Name $candidate.Name -PermissionSDDL $candidate.PermissionSDDL -ErrorAction SilentlyContinue
                (New-Object -ComObject WScript.Network).SetDefaultPrinter($candidate.Name)
                Add-ReportRow -Check 'Default printer' -Result $candidate.Name -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'GPO-Force') {
            if ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
                gpupdate /force 2>&1 | Out-Null
                Add-ReportRow -Check 'gpupdate' -Result 'Forced' -Status 'PASS' -Detail 'Printer mapping GPO refresh'
            } else {
                Add-ReportRow -Check 'gpupdate' -Result 'Skipped - not domain joined' -Status 'WARN' -Detail $Join.Label
            }
        }
        if ($Name -match 'Universal-Print') {
            $upc = Get-Service | Where-Object { $_.Name -match 'PrintDeviceConfiguration|UniversalPrint' -or $_.DisplayName -match 'Universal Print' }
            Add-ReportRow -Check 'Universal Print services' -Result "$(@($upc).Count)" -Status 'INFO' -Detail (($upc | ForEach-Object Name) -join ', ')
        }
        if ($Name -match 'Scan-To-Email') {
            Add-ReportRow -Check 'Scan-to-email' -Result 'Test SMTP path / MFP address book manually' -Status 'INFO' -Detail 'Verify device DNS and mail relay'
        }
        if ($Name -match 'PCL|PS-Driver') {
            $drivers = @(Get-PrinterDriver -EA SilentlyContinue)
            Add-ReportRow -Check 'Drivers' -Result "$($drivers.Count)" -Status 'INFO' -Detail (($drivers | Select-Object -First 10 Name) -join '; ')
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Printers'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Universal Print Connector Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Universal Print Connector Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
