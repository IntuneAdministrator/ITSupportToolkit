...
<#
.SYNOPSIS
    Common problem fix: Default Printer Keeps Changing
.DESCRIPTION
...
.EXAMPLE
    .\Default-Printer-Keeps-Changing.ps1
.EXAMPLE
    .\Default-Printer-Keeps-Changing.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Default Printer Keeps Changing'; $totalSteps = 8; $Name = 'Default-Printer-Keeps-Changing'
try {
    ...
    Export-HtmlReport -Title 'Default Printer Keeps Changing' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Default Printer Keeps Changing' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
