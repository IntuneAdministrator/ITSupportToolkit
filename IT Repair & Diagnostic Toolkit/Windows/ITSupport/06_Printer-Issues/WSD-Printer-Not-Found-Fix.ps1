...
<#
.SYNOPSIS
    Common problem fix: WSD Printer Not Found Fix
.DESCRIPTION
...
.EXAMPLE
    .\WSD-Printer-Not-Found-Fix
.EXAMPLE
    .\WSD-Printer-Not-Found-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSD Printer Not Found Fix'; $totalSteps = 8; $Name = 'WSD-Printer-Not-Found-Fix'
try {
    ...
    Export-HtmlReport -Title 'WSD Printer Not Found Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSD Printer Not Found Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
