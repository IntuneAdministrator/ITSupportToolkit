...
<#
.SYNOPSIS
    Common problem fix: Print Queue Stuck Clear
.DESCRIPTION
...
.EXAMPLE
    .\Print-Queue-Stuck-Clear.ps1
.EXAMPLE
    .\Print-Queue-Stuck-Clear.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Queue Stuck Clear'; $totalSteps = 8; $Name = 'Print-Queue-Stuck-Clear'
try {
    ...
    Export-HtmlReport -Title 'Print Queue Stuck Clear' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Queue Stuck Clear' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
