...
<#
.SYNOPSIS
    Common problem fix: Printer Roaming Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Roaming-Failed-Fix
.EXAMPLE
    .\Printer-Roaming-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Roaming Failed Fix'; $totalSteps = 8; $Name = 'Printer-Roaming-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer Roaming Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Roaming Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
