...
<#
.SYNOPSIS
    Common problem fix: Zebra Label Printer Calibration
.DESCRIPTION
...
.EXAMPLE
    .\Zebra-Label-Printer-Calibration.ps1
.EXAMPLE
    .\Zebra-Label-Printer-Calibration.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Zebra Label Printer Calibration'; $totalSteps = 8; $Name = 'Zebra-Label-Printer-Calibration'
try {
    ...
    Export-HtmlReport -Title 'Zebra Label Printer Calibration' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Zebra Label Printer Calibration' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
