...
<#
.SYNOPSIS
    Common problem fix: Printer Issues Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Issues-Issue-Detect.ps1
.EXAMPLE
    .\Printer-Issues-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Issues Issue Detect'; $totalSteps = 8; $Name = 'Printer-Issues-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Printer Issues Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Issues Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
