...
<#
.SYNOPSIS
    Common problem fix: Printer Driver Not Found Fix
.DESCRIPTION
...
.EXAMPLE
    .\Printer-Driver-Not-Found-Fix
.EXAMPLE
    .\Printer-Driver-Not-Found-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Printer Driver Not Found Fix'; $totalSteps = 8; $Name = 'Printer-Driver-Not-Found-Fix'
try {
    ...
    Export-HtmlReport -Title 'Printer Driver Not Found Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Driver Not Found Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
