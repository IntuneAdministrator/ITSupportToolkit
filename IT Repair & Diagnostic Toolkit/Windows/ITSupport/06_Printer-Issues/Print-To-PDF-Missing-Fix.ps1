...
<#
.SYNOPSIS
    Common problem fix: Print To PDF Missing Fix
.DESCRIPTION
...
.EXAMPLE
    .\Print-To-PDF-Missing-Fix
.EXAMPLE
    .\Print-To-PDF-Missing-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print To PDF Missing Fix'; $totalSteps = 8; $Name = 'Print-To-PDF-Missing-Fix'
try {
    ...
    Export-HtmlReport -Title 'Print To PDF Missing Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print To PDF Missing Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
