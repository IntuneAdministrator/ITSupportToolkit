...
<#
.SYNOPSIS
    IT Support quick repair/diagnostic: Default Printer Reset
.DESCRIPTION
    Production-grade Windows IT Support tool for Default-Printer-Reset.
...
$exitCode = 0
$Activity = 'Default Printer Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Default-Printer-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Default-Printer-Reset' -Status 'INFO' -Detail 'ITSupport/06_Printer-Issues'
    ...
    Export-HtmlReport -Title 'Default Printer Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Default Printer Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
