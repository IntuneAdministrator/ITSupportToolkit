...
<#
.SYNOPSIS
    Common problem fix: Surface Device Warranty Check
.DESCRIPTION
...
.EXAMPLE
    .\Surface-Device-Warranty-Check.ps1
.EXAMPLE
    .\Surface-Device-Warranty-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Surface Device Warranty Check'; $totalSteps = 8; $Name = 'Surface-Device-Warranty-Check'
try {
    ...
    Export-HtmlReport -Title 'Surface Device Warranty Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Surface Device Warranty Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
