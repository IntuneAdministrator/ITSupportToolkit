...
<#
.SYNOPSIS
    Common problem fix: CMDB Software Asset Baseline Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Software-Asset-Baseline-Export.ps1
.EXAMPLE
    .\CMDB-Software-Asset-Baseline-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Software Asset Baseline Export'; $totalSteps = 8; $Name = 'CMDB-Software-Asset-Baseline-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB Software Asset Baseline Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Software Asset Baseline Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
