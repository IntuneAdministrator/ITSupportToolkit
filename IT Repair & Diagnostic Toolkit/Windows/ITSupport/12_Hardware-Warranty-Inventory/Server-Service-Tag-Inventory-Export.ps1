...
<#
.SYNOPSIS
    Common problem fix: Server Service Tag Inventory Export
.DESCRIPTION
...
.EXAMPLE
    .\Server-Service-Tag-Inventory-Export.ps1
.EXAMPLE
    .\Server-Service-Tag-Inventory-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Server Service Tag Inventory Export'; $totalSteps = 8; $Name = 'Server-Service-Tag-Inventory-Export'
try {
    ...
    Export-HtmlReport -Title 'Server Service Tag Inventory Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Server Service Tag Inventory Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
