...
<#
.SYNOPSIS
    Common problem fix: Rack Asset Location Audit
.DESCRIPTION
...
.EXAMPLE
    .\Rack-Asset-Location-Audit.ps1
.EXAMPLE
    .\Rack-Asset-Location-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Rack Asset Location Audit'; $totalSteps = 8; $Name = 'Rack-Asset-Location-Audit'
try {
    ...
    Export-HtmlReport -Title 'Rack Asset Location Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Rack Asset Location Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
