...
<#
.SYNOPSIS
    Enterprise IT tool: Dell Lenovo HP Model Report
.DESCRIPTION
...
.EXAMPLE
    .\Dell-Lenovo-HP-Model-Report.ps1
.EXAMPLE
    .\Dell-Lenovo-HP-Model-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell Lenovo HP Model Report'; $totalSteps = 8; $Name = 'Dell-Lenovo-HP-Model-Report'
try {
    ...
    Export-HtmlReport -Title 'Dell Lenovo HP Model Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell Lenovo HP Model Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
