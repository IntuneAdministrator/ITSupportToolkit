...
<#
.SYNOPSIS
    Common problem fix: CMDB Intune Sync Validation
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Intune-Sync-Validation.ps1
.EXAMPLE
    .\CMDB-Intune-Sync-Validation.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Intune Sync Validation'; $totalSteps = 8; $Name = 'CMDB-Intune-Sync-Validation'
try {
    ...
    Export-HtmlReport -Title 'CMDB Intune Sync Validation' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Intune Sync Validation' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
