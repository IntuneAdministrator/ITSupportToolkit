...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory Policy Check
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-Policy-Check.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory Policy Check'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-Policy-Check'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
