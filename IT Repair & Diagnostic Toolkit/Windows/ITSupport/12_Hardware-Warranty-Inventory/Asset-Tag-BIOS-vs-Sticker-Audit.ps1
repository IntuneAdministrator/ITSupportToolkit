...
<#
.SYNOPSIS
    Common problem fix: Asset Tag BIOS vs Sticker Audit
.DESCRIPTION
...
.EXAMPLE
    .\Asset-Tag-BIOS-vs-Sticker-Audit.ps1
.EXAMPLE
    .\Asset-Tag-BIOS-vs-Sticker-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Asset Tag BIOS vs Sticker Audit'; $totalSteps = 8; $Name = 'Asset-Tag-BIOS-vs-Sticker-Audit'
try {
    ...
    Export-HtmlReport -Title 'Asset Tag BIOS vs Sticker Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Asset Tag BIOS vs Sticker Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
