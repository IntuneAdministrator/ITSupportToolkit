...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory User Triage
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-User-Triage.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-User-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory User Triage'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-User-Triage'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory User Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory User Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
