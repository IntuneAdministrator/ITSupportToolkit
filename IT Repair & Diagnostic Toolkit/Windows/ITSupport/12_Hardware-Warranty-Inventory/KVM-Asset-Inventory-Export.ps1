...
<#
.SYNOPSIS
    Common problem fix: KVM Asset Inventory Export
.DESCRIPTION
...
.EXAMPLE
    .\KVM-Asset-Inventory-Export.ps1
.EXAMPLE
    .\KVM-Asset-Inventory-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'KVM Asset Inventory Export'; $totalSteps = 8; $Name = 'KVM-Asset-Inventory-Export'
try {
    ...
    Export-HtmlReport -Title 'KVM Asset Inventory Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'KVM Asset Inventory Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
