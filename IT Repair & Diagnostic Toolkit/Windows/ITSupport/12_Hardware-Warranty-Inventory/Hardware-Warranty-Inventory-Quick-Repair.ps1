...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory Quick Repair
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-Quick-Repair.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-Quick-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory Quick Repair'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-Quick-Repair'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory Quick Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory Quick Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
