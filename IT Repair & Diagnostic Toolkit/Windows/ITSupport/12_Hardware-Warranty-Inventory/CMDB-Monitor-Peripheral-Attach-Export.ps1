...
<#
.SYNOPSIS
    Common problem fix: CMDB Monitor Peripheral Attach Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Monitor-Peripheral-Attach-Export.ps1
.EXAMPLE
    .\CMDB-Monitor-Peripheral-Attach-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Monitor Peripheral Attach Export'; $totalSteps = 8; $Name = 'CMDB-Monitor-Peripheral-Attach-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB Monitor Peripheral Attach Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Monitor Peripheral Attach Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
