...
<#
.SYNOPSIS
    Enterprise IT tool: Battery Cycle Quick Report
.DESCRIPTION
...
.EXAMPLE
    .\Battery-Cycle-Quick-Report.ps1
.EXAMPLE
    .\Battery-Cycle-Quick-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Battery Cycle Quick Report'; $totalSteps = 8; $Name = 'Battery-Cycle-Quick-Report'
try {
    ...
    Export-HtmlReport -Title 'Battery Cycle Quick Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Battery Cycle Quick Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
