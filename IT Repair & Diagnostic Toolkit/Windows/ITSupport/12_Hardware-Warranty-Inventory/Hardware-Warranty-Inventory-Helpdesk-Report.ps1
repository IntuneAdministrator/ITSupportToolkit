...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory Helpdesk Report
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-Helpdesk-Report.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-Helpdesk-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory Helpdesk Report'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-Helpdesk-Report'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory Helpdesk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory Helpdesk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
