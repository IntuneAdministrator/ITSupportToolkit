...
<#
.SYNOPSIS
    Enterprise IT tool: Asset Tag BIOS Read
.DESCRIPTION
...
.EXAMPLE
    .\Asset-Tag-BIOS-Read.ps1
.EXAMPLE
    .\Asset-Tag-BIOS-Read.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Asset Tag BIOS Read'; $totalSteps = 8; $Name = 'Asset-Tag-BIOS-Read'
try {
    ...
    Export-HtmlReport -Title 'Asset Tag BIOS Read' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Asset Tag BIOS Read' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
