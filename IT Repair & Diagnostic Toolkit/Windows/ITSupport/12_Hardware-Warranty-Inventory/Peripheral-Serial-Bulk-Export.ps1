...
<#
.SYNOPSIS
    Common problem fix: Peripheral Serial Bulk Export
.DESCRIPTION
...
.EXAMPLE
    .\Peripheral-Serial-Bulk-Export.ps1
.EXAMPLE
    .\Peripheral-Serial-Bulk-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Peripheral Serial Bulk Export'; $totalSteps = 8; $Name = 'Peripheral-Serial-Bulk-Export'
try {
    ...
    Export-HtmlReport -Title 'Peripheral Serial Bulk Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Peripheral Serial Bulk Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
