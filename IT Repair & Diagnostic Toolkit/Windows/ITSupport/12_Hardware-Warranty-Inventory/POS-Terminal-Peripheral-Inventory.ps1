...
<#
.SYNOPSIS
    Common problem fix: POS Terminal Peripheral Inventory
.DESCRIPTION
...
.EXAMPLE
    .\POS-Terminal-Peripheral-Inventory.ps1
.EXAMPLE
    .\POS-Terminal-Peripheral-Inventory.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'POS Terminal Peripheral Inventory'; $totalSteps = 8; $Name = 'POS-Terminal-Peripheral-Inventory'
try {
    ...
    Export-HtmlReport -Title 'POS Terminal Peripheral Inventory' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'POS Terminal Peripheral Inventory' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
