...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory Issue Detect
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-Issue-Detect.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-Issue-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory Issue Detect'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-Issue-Detect'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory Issue Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory Issue Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
