...
<#
.SYNOPSIS
    Enterprise IT tool: Dock Monitor Inventory
.DESCRIPTION
...
.EXAMPLE
    .\Dock-Monitor-Inventory.ps1
.EXAMPLE
    .\Dock-Monitor-Inventory.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dock Monitor Inventory'; $totalSteps = 8; $Name = 'Dock-Monitor-Inventory'
try {
    ...
    Export-HtmlReport -Title 'Dock Monitor Inventory' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dock Monitor Inventory' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
