...
<#
.SYNOPSIS
    Common problem fix: HP Serial Warranty Export
.DESCRIPTION
...
.EXAMPLE
    .\HP-Serial-Warranty-Export.ps1
.EXAMPLE
    .\HP-Serial-Warranty-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HP Serial Warranty Export'; $totalSteps = 8; $Name = 'HP-Serial-Warranty-Export'
try {
    ...
    Export-HtmlReport -Title 'HP Serial Warranty Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HP Serial Warranty Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
