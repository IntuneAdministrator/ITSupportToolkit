...
<#
.SYNOPSIS
    Common problem fix: Workstation Asset GPU Spec Export
.DESCRIPTION
...
.EXAMPLE
    .\Workstation-Asset-GPU-Spec-Export.ps1
.EXAMPLE
    .\Workstation-Asset-GPU-Spec-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Workstation Asset GPU Spec Export'; $totalSteps = 8; $Name = 'Workstation-Asset-GPU-Spec-Export'
try {
    ...
    Export-HtmlReport -Title 'Workstation Asset GPU Spec Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Workstation Asset GPU Spec Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
