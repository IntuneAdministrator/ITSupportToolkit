...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory One Click Fix
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-One-Click-Fix.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-One-Click-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory One Click Fix'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-One-Click-Fix'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory One Click Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory One Click Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
