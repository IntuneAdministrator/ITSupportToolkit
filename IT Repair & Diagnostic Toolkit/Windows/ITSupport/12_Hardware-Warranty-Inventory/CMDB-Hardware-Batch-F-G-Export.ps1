...
<#
.SYNOPSIS
    Common problem fix: CMDB Hardware Batch F G Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Hardware-Batch-F-G-Export.ps1
.EXAMPLE
    .\CMDB-Hardware-Batch-F-G-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Hardware Batch F G Export'; $totalSteps = 8; $Name = 'CMDB-Hardware-Batch-F-G-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB Hardware Batch F G Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Hardware Batch F G Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
