...
<#
.SYNOPSIS
    Common problem fix: Monitor Serial Inventory Export
.DESCRIPTION
...
.EXAMPLE
    .\Monitor-Serial-Inventory-Export.ps1
.EXAMPLE
    .\Monitor-Serial-Inventory-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Monitor Serial Inventory Export'; $totalSteps = 8; $Name = 'Monitor-Serial-Inventory-Export'
try {
    ...
    Export-HtmlReport -Title 'Monitor Serial Inventory Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Monitor Serial Inventory Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
