...
<#
.SYNOPSIS
    Common problem fix: CMDB Full Endpoint Inventory CSV
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Full-Endpoint-Inventory-CSV.ps1
.EXAMPLE
    .\CMDB-Full-Endpoint-Inventory-CSV.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Full Endpoint Inventory CSV'; $totalSteps = 8; $Name = 'CMDB-Full-Endpoint-Inventory-CSV'
try {
    ...
    Export-HtmlReport -Title 'CMDB Full Endpoint Inventory CSV' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Full Endpoint Inventory CSV' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
