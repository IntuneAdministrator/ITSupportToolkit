...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory Reset Stack
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-Reset-Stack.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-Reset-Stack.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory Reset Stack'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-Reset-Stack'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory Reset Stack' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory Reset Stack' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
