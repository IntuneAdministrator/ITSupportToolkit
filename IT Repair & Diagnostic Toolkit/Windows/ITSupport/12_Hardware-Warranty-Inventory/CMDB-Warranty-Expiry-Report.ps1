...
<#
.SYNOPSIS
    Common problem fix: CMDB Warranty Expiry Report
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Warranty-Expiry-Report.ps1
.EXAMPLE
    .\CMDB-Warranty-Expiry-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Warranty Expiry Report'; $totalSteps = 8; $Name = 'CMDB-Warranty-Expiry-Report'
try {
    ...
    Export-HtmlReport -Title 'CMDB Warranty Expiry Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Warranty Expiry Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
