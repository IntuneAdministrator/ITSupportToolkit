...
<#
.SYNOPSIS
    Enterprise IT tool: Serial Number Warranty Export
.DESCRIPTION
    Expansion module (ITSupport / 12_Hardware-Warranty-Inventory) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Serial-Number-Warranty-Export.ps1
.EXAMPLE
    .\Serial-Number-Warranty-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Serial Number Warranty Export'; $totalSteps = 8; $Name = 'Serial-Number-Warranty-Export'
try {
    ...
    Export-HtmlReport -Title 'Serial Number Warranty Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Serial Number Warranty Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
