...
<#
.SYNOPSIS
    Common problem fix: Hardware Warranty Inventory Quick Audit
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Warranty-Inventory-Quick-Audit.ps1
.EXAMPLE
    .\Hardware-Warranty-Inventory-Quick-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Warranty Inventory Quick Audit'; $totalSteps = 8; $Name = 'Hardware-Warranty-Inventory-Quick-Audit'
try {
    ...
    Export-HtmlReport -Title 'Hardware Warranty Inventory Quick Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Warranty Inventory Quick Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
