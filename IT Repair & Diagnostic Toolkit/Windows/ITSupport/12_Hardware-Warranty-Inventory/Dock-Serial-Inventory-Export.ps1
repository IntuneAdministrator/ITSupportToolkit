...
<#
.SYNOPSIS
    Common problem fix: Dock Serial Inventory Export
.DESCRIPTION
...
.EXAMPLE
    .\Dock-Serial-Inventory-Export.ps1
.EXAMPLE
    .\Dock-Serial-Inventory-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dock Serial Inventory Export'; $totalSteps = 8; $Name = 'Dock-Serial-Inventory-Export'
try {
    ...
    Export-HtmlReport -Title 'Dock Serial Inventory Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dock Serial Inventory Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
