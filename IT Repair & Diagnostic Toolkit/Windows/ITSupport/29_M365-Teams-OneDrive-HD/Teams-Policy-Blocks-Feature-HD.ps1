...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Teams Policy Blocks Feature HD
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Policy-Blocks-Feature-HD.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Teams-Policy-Blocks-Feature-HD.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Policy Blocks Feature HD'; $totalSteps = 8; $Name = 'Teams-Policy-Blocks-Feature-HD'; $Folder = '29_M365-Teams-OneDrive-HD'
...
try {
    ...
    Export-HtmlReport -Title 'Teams Policy Blocks Feature HD' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Policy Blocks Feature HD' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
