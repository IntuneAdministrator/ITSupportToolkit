...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: SharePoint Access Denied Triage
.DESCRIPTION
    Phase 12 M365 tenant admin (ITSupport/29_M365-Teams-OneDrive-HD). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\SharePoint-Access-Denied-Triage.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\SharePoint-Access-Denied-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SharePoint Access Denied Triage'; $totalSteps = 8; $Name = 'SharePoint-Access-Denied-Triage'; $Folder = '29_M365-Teams-OneDrive-HD'
$DefaultScopes = @('User.Read.All','Directory.Read.All','Organization.Read.All')
try {
    ...
    Add-ReportRow -Check 'Module' -Result '29_M365-Teams-OneDrive-HD' -Status 'INFO' -Detail 'ITSupport Phase12 M365'
    Add-M365PortalRows
    ...
    if ($UPN) { Add-ReportRow -Check 'Target UPN' -Result $UPN -Status 'INFO' -Detail '' }
        ...
        Write-SectionBanner 'M365 HELPDESK CLOUD'
        Write-ProgressStep 3 $totalSteps $Activity 'Helpdesk triage'
        if (-not $UPN) {
            Add-ReportRow -Check 'Need UPN' -Result 'Required for most HD cloud checks' -Status 'WARN' -Detail '.\Script.ps1 -UserPrincipalName user@contoso.com'
        }
        if ($Name -match 'License-Missing' -and $connected -and $UPN) {
            try {
                $u = Get-MgUser -UserId $UPN -Property assignedLicenses,accountEnabled,displayName -EA Stop
                Add-ReportRow -Check $u.DisplayName -Result "$(@($u.AssignedLicenses).Count) license(s)" -Status $(if(@($u.AssignedLicenses).Count -gt 0){'PASS'}else{'FAIL'}) -Detail "Enabled=$($u.AccountEnabled)"
                Add-ReportRow -Check 'Next' -Result 'Admin Center > Users > Licenses / Group-based licensing' -Status 'INFO' -Detail ''
            } catch { Add-ReportRow -Check 'License check' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($Name -match 'MFA-Methods' -and $connected -and $UPN) {
            try {
                $methods = Get-MgUserAuthenticationMethod -UserId $UPN -EA Stop
                Add-ReportRow -Check 'Auth methods' -Result "$(@($methods).Count)" -Status $(if(@($methods).Count -gt 0){'PASS'}else{'WARN'}) -Detail 'Entra > User > Authentication methods'
            } catch { Add-ReportRow -Check 'Auth methods' -Result 'Need UserAuthenticationMethod.Read.All' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($Name -match 'Password-Reset|Blocked-SignIn|Guest-User') {
            Add-ReportRow -Check 'Entra user blade' -Result "https://entra.microsoft.com/#view/Microsoft_AAD_UsersAndTenants/UserProfileMenuBlade/~/overview/userId/$UPN" -Status 'INFO' -Detail 'Or search user in Entra'
            if ($connected -and $UPN -and $Name -match 'Blocked-SignIn') {
                try {
                    $u = Get-MgUser -UserId $UPN -Property accountEnabled,displayName -EA Stop
                    Add-ReportRow -Check 'AccountEnabled' -Result "$($u.AccountEnabled)" -Status $(if($u.AccountEnabled){'PASS'}else{'FAIL'}) -Detail $u.DisplayName
                } catch { Add-ReportRow -Check 'User' -Result 'Lookup failed' -Status 'WARN' -Detail $_.Exception.Message }
            }
        }
        if ($Name -match 'Mailbox-Full|Shared-Mailbox|Calendar-Delegate|Forwarding|Recoverable') {
            Add-ReportRow -Check 'Exchange Admin' -Result 'https://admin.exchange.microsoft.com' -Status 'INFO' -Detail 'Quota, shared perms, forwarding, recoverable items'
            $exo = Get-Module -ListAvailable ExchangeOnlineManagement -EA SilentlyContinue
            if ($exo -and $UPN -and $PSCmdlet.ShouldProcess('EXO','quick mailbox stats')) {
                try {
                    Import-Module ExchangeOnlineManagement -EA SilentlyContinue
                    if (-not (Get-ConnectionInformation -EA SilentlyContinue)) { Connect-ExchangeOnline -ShowBanner:$false -EA Stop }
                    $st = Get-EXOMailboxStatistics -Identity $UPN -EA Stop
                    Add-ReportRow -Check 'Size' -Result "$($st.TotalItemSize)" -Status 'INFO' -Detail "Items=$($st.ItemCount)"
                } catch { Add-ReportRow -Check 'EXO stats' -Result 'Use EAC portal' -Status 'WARN' -Detail $_.Exception.Message }
            }
        }
        if ($Name -match 'Teams-Policy|OneDrive-Site|SharePoint-Access|External-Chat|ODSP-Sync') {
            Add-ReportRow -Check 'Teams/SPO HD' -Result 'Confirm license + policy + site provisioning' -Status 'INFO' -Detail 'OneDrive provision can take minutes after license assign'
            Add-ReportRow -Check 'Known issue path' -Result 'Service health + client reset scripts in 08_Microsoft365-Repair' -Status 'INFO' -Detail ''
        }
        if ($Name -match 'CA-Block|Legacy-Auth|Session-Revoke|Service-Health|PIM') {
            Add-ReportRow -Check 'Sign-in troubleshoot' -Result 'Entra sign-in logs + CA What If' -Status 'INFO' -Detail 'Legacy auth blocks break basic Outlook auth'
            if ($Name -match 'Session-Revoke' -and $connected -and $UPN -and $PSCmdlet.ShouldProcess($UPN,'Revoke sign-in sessions')) {
                try {
                    Revoke-MgUserSignInSession -UserId $UPN -EA Stop
                    Add-ReportRow -Check 'Revoke sessions' -Result 'Requested' -Status 'PASS' -Detail 'User must re-auth'
                } catch { Add-ReportRow -Check 'Revoke' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
            }
            if ($Name -match 'Service-Health') {
                Add-ReportRow -Check 'Open' -Result 'https://admin.microsoft.com/#/servicehealth' -Status 'INFO' -Detail 'Check before long client repairs'
            }
            if ($Name -match 'PIM') {
                Add-ReportRow -Check 'PIM' -Result 'https://entra.microsoft.com/#view/Microsoft_Azure_PIMCommon/ResourceMenuBlade' -Status 'INFO' -Detail 'Activate eligible admin roles before Graph writes'
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'HD cloud done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'SharePoint Access Denied Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SharePoint Access Denied Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
