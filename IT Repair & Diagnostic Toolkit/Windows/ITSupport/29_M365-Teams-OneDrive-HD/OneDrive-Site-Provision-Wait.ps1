...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: OneDrive Site Provision Wait
.DESCRIPTION
...
.EXAMPLE
    .\OneDrive-Site-Provision-Wait.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\OneDrive-Site-Provision-Wait.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OneDrive Site Provision Wait'; $totalSteps = 8; $Name = 'OneDrive-Site-Provision-Wait'; $Folder = '29_M365-Teams-OneDrive-HD'
...
try {
    ...
    Export-HtmlReport -Title 'OneDrive Site Provision Wait' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OneDrive Site Provision Wait' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
