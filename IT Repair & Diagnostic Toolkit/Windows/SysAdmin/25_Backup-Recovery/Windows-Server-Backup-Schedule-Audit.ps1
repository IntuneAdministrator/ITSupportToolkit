...
<#
.SYNOPSIS
    Common problem fix: Windows Server Backup Schedule Audit
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 25_Backup-Recovery). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Windows-Server-Backup-Schedule-Audit.ps1
.EXAMPLE
    .\Windows-Server-Backup-Schedule-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Server Backup Schedule Audit'; $totalSteps = 8; $Name = 'Windows-Server-Backup-Schedule-Audit'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '25_Backup-Recovery' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Windows Server Backup Schedule Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Server Backup Schedule Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
