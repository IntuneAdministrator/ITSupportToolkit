...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Recovery Point Objective Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Recovery-Point-Objective-Report.
Category: 25_Backup-Recovery. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Recovery-Point-Objective-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Recovery-Point-Objective-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Recovery Point Objective Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'BACKUP RECOVERY'
        $Name = 'Recovery-Point-Objective-Report'
        Add-ReportRow -Check 'Tool' -Result 'Recovery-Point-Objective-Report' -Status 'INFO' -Detail '25_Backup-Recovery'
        Write-SectionBanner 'BACKUP / VSS / RECOVERY'
        foreach ($s in @('VSS','SWPRV','wbengine','fhsvc')) {
            $svc = Get-Service -Name $s -ErrorAction SilentlyContinue
            if ($svc) {
                Add-ReportRow -Check "Service $s" -Result "$($svc.Status)/$($svc.StartType)" -Status $(if ($svc.Status -eq 'Running' -or $svc.StartType -eq 'Manual') {'PASS'} else {'WARN'}) -Detail $svc.DisplayName
            }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Backup services'

        if ($Name -match 'VSS|Shadow-Copy') {
            if ($PSCmdlet.ShouldProcess('VSS', 'Restart VSS and list writers')) {
                Restart-Service VSS -Force -ErrorAction SilentlyContinue
                $writers = & vssadmin list writers 2>&1 | Out-String
                $failed = @([regex]::Matches($writers, 'State:\s*\[(?!1\].*Stable)[^\]]+\]'))
                Add-ReportRow -Check 'VSS writers' -Result 'Listed' -Status $(if ($writers -match 'Failed|Waiting for completion') {'WARN'} else {'PASS'}) -Detail ($writers.Substring(0, [Math]::Min(500, $writers.Length)))
            }
            $shadows = @(Get-CimInstance Win32_ShadowCopy -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'Shadow copies' -Result "$($shadows.Count)" -Status 'INFO' -Detail ''
        }

        if ($Name -match 'System-Restore') {
            if ($PSCmdlet.ShouldProcess('System Restore', 'Create restore point')) {
                try {
                    Checkpoint-Computer -Description 'IT-Repair-Scripts' -RestorePointType MODIFY_SETTINGS -ErrorAction Stop
                    Add-ReportRow -Check 'Restore point' -Result 'Created' -Status 'PASS' -Detail 'IT-Repair-Scripts'
                } catch {
                    Add-ReportRow -Check 'Restore point' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message
                }
            }
        }

        if ($Name -match 'File-History') {
            $fh = Get-Service fhsvc -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'File History service' -Result $(if ($fh) { $fh.Status } else {'N/A'}) -Status 'INFO' -Detail ''
            if ($PSCmdlet.ShouldProcess('File History', 'Restart fhsvc')) {
                Restart-Service fhsvc -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'fhsvc restart' -Result 'Done' -Status 'PASS' -Detail ''
            }
        }

        if ($Name -match 'Azure-Backup|DPM|Cloud-Backup|Windows-Backup|Windows-Server-Backup|Bare-Metal|Retention|RPO|Encryption-Key|Backup-Job') {
            $ob = Get-Service OBEngine -ErrorAction SilentlyContinue
            $dpm = Get-Service DPMRA -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Azure Backup OBEngine' -Result $(if ($ob) { $ob.Status } else {'Not installed'}) -Status 'INFO' -Detail ''
            Add-ReportRow -Check 'DPMRA' -Result $(if ($dpm) { $dpm.Status } else {'Not installed'}) -Status 'INFO' -Detail ''
            try {
                $wb = Get-WBSummary -ErrorAction SilentlyContinue
                if ($wb) {
                    Add-ReportRow -Check 'Windows Server Backup' -Result "$($wb.NumberOfVersions) versions" -Status 'INFO' -Detail "Last success=$($wb.LastSuccessfulBackupTime)"
                }
            } catch {
                Add-ReportRow -Check 'Windows Server Backup' -Result 'Module/feature N/A' -Status 'INFO' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Backup'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Recovery Point Objective Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Recovery Point Objective Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
