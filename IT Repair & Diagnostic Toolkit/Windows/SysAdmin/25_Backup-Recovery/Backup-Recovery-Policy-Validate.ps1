...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Policy-Validate.ps1
.EXAMPLE
    .\Backup-Recovery-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Policy Validate'; $totalSteps = 8; $Name = 'Backup-Recovery-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
