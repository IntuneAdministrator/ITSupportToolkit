...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Deep-Audit.ps1
.EXAMPLE
    .\Backup-Recovery-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Deep Audit'; $totalSteps = 8; $Name = 'Backup-Recovery-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
