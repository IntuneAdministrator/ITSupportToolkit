...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Health-Triage.ps1
.EXAMPLE
    .\Backup-Recovery-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Health Triage'; $totalSteps = 8; $Name = 'Backup-Recovery-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
