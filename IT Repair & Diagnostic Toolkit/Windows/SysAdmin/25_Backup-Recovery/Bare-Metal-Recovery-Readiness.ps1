...
<#
.SYNOPSIS
    Common problem fix: Bare Metal Recovery Readiness
.DESCRIPTION
...
.EXAMPLE
    .\Bare-Metal-Recovery-Readiness.ps1
.EXAMPLE
    .\Bare-Metal-Recovery-Readiness.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bare Metal Recovery Readiness'; $totalSteps = 8; $Name = 'Bare-Metal-Recovery-Readiness'
try {
    ...
    Export-HtmlReport -Title 'Bare Metal Recovery Readiness' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bare Metal Recovery Readiness' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
