...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Backup Retention Policy Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Backup-Retention-Policy-Audit.
...
.EXAMPLE
    .\Backup-Retention-Policy-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Backup-Retention-Policy-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Backup Retention Policy Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Backup-Retention-Policy-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Backup-Retention-Policy-Audit' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Backup Retention Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Backup Retention Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
