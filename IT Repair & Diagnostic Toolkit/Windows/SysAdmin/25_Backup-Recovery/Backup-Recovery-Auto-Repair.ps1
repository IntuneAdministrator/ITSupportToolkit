...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Auto-Repair.ps1
.EXAMPLE
    .\Backup-Recovery-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Auto Repair'; $totalSteps = 8; $Name = 'Backup-Recovery-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
