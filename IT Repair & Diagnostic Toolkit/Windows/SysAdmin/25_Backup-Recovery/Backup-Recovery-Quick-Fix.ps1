...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Quick-Fix.ps1
.EXAMPLE
    .\Backup-Recovery-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Quick Fix'; $totalSteps = 8; $Name = 'Backup-Recovery-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
