...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System Restore Point Create
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System-Restore-Point-Create.
...
.EXAMPLE
    .\System-Restore-Point-Create.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\System-Restore-Point-Create.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'System Restore Point Create'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'System-Restore-Point-Create'
        Add-ReportRow -Check 'Tool' -Result 'System-Restore-Point-Create' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'System Restore Point Create' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System Restore Point Create' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
