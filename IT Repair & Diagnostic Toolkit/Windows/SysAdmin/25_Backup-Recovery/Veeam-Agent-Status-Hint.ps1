...
<#
.SYNOPSIS
    Common problem fix: Veeam Agent Status Hint
.DESCRIPTION
...
.EXAMPLE
    .\Veeam-Agent-Status-Hint.ps1
.EXAMPLE
    .\Veeam-Agent-Status-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Veeam Agent Status Hint'; $totalSteps = 8; $Name = 'Veeam-Agent-Status-Hint'
try {
    ...
    Export-HtmlReport -Title 'Veeam Agent Status Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Veeam Agent Status Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
