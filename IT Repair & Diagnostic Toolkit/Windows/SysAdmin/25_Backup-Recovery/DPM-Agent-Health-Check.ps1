...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DPM Agent Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DPM-Agent-Health-Check.
...
$exitCode = 0
$Activity = 'DPM Agent Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'DPM-Agent-Health-Check'
        Add-ReportRow -Check 'Tool' -Result 'DPM-Agent-Health-Check' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'DPM Agent Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DPM Agent Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
