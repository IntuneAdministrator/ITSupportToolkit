...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Stack-Reset.ps1
.EXAMPLE
    .\Backup-Recovery-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Stack Reset'; $totalSteps = 8; $Name = 'Backup-Recovery-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
