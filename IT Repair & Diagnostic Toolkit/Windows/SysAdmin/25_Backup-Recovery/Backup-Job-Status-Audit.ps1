...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Backup Job Status Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Backup-Job-Status-Audit.
...
.EXAMPLE
    .\Backup-Job-Status-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Backup-Job-Status-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Backup Job Status Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Backup-Job-Status-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Backup-Job-Status-Audit' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Backup Job Status Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Backup Job Status Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
