...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Backup Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Backup-Repair.
...
$exitCode = 0
$Activity = 'Windows Backup Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Backup-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Backup-Repair' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Windows Backup Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Backup Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
