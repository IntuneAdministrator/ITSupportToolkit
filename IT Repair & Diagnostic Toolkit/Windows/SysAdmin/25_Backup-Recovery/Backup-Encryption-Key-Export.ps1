...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Backup Encryption Key Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Backup-Encryption-Key-Export.
...
.EXAMPLE
    .\Backup-Encryption-Key-Export.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Backup-Encryption-Key-Export.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Backup Encryption Key Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Backup-Encryption-Key-Export'
        Add-ReportRow -Check 'Tool' -Result 'Backup-Encryption-Key-Export' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Backup Encryption Key Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Backup Encryption Key Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
