...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Shadow Copy Restore
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Shadow-Copy-Restore.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Shadow-Copy-Restore'
        Add-ReportRow -Check 'Tool' -Result 'Shadow-Copy-Restore' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Shadow Copy Restore' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Shadow Copy Restore' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
