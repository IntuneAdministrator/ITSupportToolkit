...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Anomaly-Detect.ps1
.EXAMPLE
    .\Backup-Recovery-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Anomaly Detect'; $totalSteps = 8; $Name = 'Backup-Recovery-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
