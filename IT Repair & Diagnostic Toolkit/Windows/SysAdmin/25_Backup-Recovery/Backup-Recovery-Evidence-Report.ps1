...
<#
.SYNOPSIS
    Common problem fix: Backup Recovery Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Backup-Recovery-Evidence-Report.ps1
.EXAMPLE
    .\Backup-Recovery-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup Recovery Evidence Report'; $totalSteps = 8; $Name = 'Backup-Recovery-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Backup Recovery Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Recovery Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
