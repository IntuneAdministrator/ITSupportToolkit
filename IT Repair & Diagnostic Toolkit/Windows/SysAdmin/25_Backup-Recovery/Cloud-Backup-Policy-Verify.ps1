...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Cloud Backup Policy Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Cloud-Backup-Policy-Verify.
...
.EXAMPLE
    .\Cloud-Backup-Policy-Verify.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Cloud-Backup-Policy-Verify.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Cloud Backup Policy Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Cloud-Backup-Policy-Verify'
        Add-ReportRow -Check 'Tool' -Result 'Cloud-Backup-Policy-Verify' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Cloud Backup Policy Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Cloud Backup Policy Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
