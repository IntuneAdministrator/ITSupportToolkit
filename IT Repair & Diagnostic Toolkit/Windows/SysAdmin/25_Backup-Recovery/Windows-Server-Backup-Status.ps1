...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Server Backup Status
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Server-Backup-Status.
...
.EXAMPLE
    .\Windows-Server-Backup-Status.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Server-Backup-Status.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Server Backup Status'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Server-Backup-Status'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Server-Backup-Status' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Windows Server Backup Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Server Backup Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
