...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Azure Backup Agent Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Azure-Backup-Agent-Repair.
...
.EXAMPLE
    .\Azure-Backup-Agent-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Azure-Backup-Agent-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Azure Backup Agent Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Azure-Backup-Agent-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Azure-Backup-Agent-Repair' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Azure Backup Agent Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Azure Backup Agent Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
