...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bare Metal Recovery Prep
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bare-Metal-Recovery-Prep.
...
.EXAMPLE
    .\Bare-Metal-Recovery-Prep.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bare-Metal-Recovery-Prep.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bare Metal Recovery Prep'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bare-Metal-Recovery-Prep'
        Add-ReportRow -Check 'Tool' -Result 'Bare-Metal-Recovery-Prep' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'Bare Metal Recovery Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bare Metal Recovery Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
