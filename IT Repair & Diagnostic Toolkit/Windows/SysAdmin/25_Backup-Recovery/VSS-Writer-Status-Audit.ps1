...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VSS Writer Status Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VSS-Writer-Status-Audit.
...
.EXAMPLE
    .\VSS-Writer-Status-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\VSS-Writer-Status-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'VSS Writer Status Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'VSS-Writer-Status-Audit'
        Add-ReportRow -Check 'Tool' -Result 'VSS-Writer-Status-Audit' -Status 'INFO' -Detail '25_Backup-Recovery'
    ...
    Export-HtmlReport -Title 'VSS Writer Status Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VSS Writer Status Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
