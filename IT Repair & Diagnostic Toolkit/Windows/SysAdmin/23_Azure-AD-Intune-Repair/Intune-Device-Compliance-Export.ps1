...
<#
.SYNOPSIS
    Common problem fix: Intune Device Compliance Export
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Device-Compliance-Export.ps1
.EXAMPLE
    .\Intune-Device-Compliance-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Device Compliance Export'; $totalSteps = 8; $Name = 'Intune-Device-Compliance-Export'
try {
    ...
    Export-HtmlReport -Title 'Intune Device Compliance Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Device Compliance Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
