...
<#
.SYNOPSIS
    Common problem fix: Company Portal Sign In Fix
.DESCRIPTION
...
.EXAMPLE
    .\Company-Portal-Sign-In-Fix
.EXAMPLE
    .\Company-Portal-Sign-In-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Company Portal Sign In Fix'; $totalSteps = 8; $Name = 'Company-Portal-Sign-In-Fix'
try {
    ...
    Export-HtmlReport -Title 'Company Portal Sign In Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Company Portal Sign In Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
