...
<#
.SYNOPSIS
    Common problem fix: Intune Enrollment Failed Triage
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Enrollment-Failed-Triage
.EXAMPLE
    .\Intune-Enrollment-Failed-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Enrollment Failed Triage'; $totalSteps = 8; $Name = 'Intune-Enrollment-Failed-Triage'
try {
    ...
    Export-HtmlReport -Title 'Intune Enrollment Failed Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Enrollment Failed Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
