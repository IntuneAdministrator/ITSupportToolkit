...
<#
.SYNOPSIS
    Common problem fix: Intune Autopilot Deployment Status
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Autopilot-Deployment-Status.ps1
.EXAMPLE
    .\Intune-Autopilot-Deployment-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Autopilot Deployment Status'; $totalSteps = 8; $Name = 'Intune-Autopilot-Deployment-Status'
try {
    ...
    Export-HtmlReport -Title 'Intune Autopilot Deployment Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Autopilot Deployment Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
