...
<#
.SYNOPSIS
    Common problem fix: Intune BitLocker Recovery Key Export
.DESCRIPTION
...
.EXAMPLE
    .\Intune-BitLocker-Recovery-Key-Export.ps1
.EXAMPLE
    .\Intune-BitLocker-Recovery-Key-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune BitLocker Recovery Key Export'; $totalSteps = 8; $Name = 'Intune-BitLocker-Recovery-Key-Export'
try {
    ...
    Export-HtmlReport -Title 'Intune BitLocker Recovery Key Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune BitLocker Recovery Key Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
