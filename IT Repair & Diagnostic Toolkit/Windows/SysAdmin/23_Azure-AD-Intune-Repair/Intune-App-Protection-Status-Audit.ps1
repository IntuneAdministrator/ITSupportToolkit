...
<#
.SYNOPSIS
    Common problem fix: Intune App Protection Status Audit
.DESCRIPTION
...
.EXAMPLE
    .\Intune-App-Protection-Status-Audit.ps1
.EXAMPLE
    .\Intune-App-Protection-Status-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune App Protection Status Audit'; $totalSteps = 8; $Name = 'Intune-App-Protection-Status-Audit'
try {
    ...
    Export-HtmlReport -Title 'Intune App Protection Status Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune App Protection Status Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
