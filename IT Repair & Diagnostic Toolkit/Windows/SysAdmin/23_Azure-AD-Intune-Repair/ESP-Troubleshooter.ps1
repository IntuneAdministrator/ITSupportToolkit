...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ESP Troubleshooter
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ESP-Troubleshooter.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'ESP-Troubleshooter'
        Add-ReportRow -Check 'Tool' -Result 'ESP-Troubleshooter' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'ESP Troubleshooter' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ESP Troubleshooter' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
