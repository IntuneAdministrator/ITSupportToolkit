...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Conditional Access Diagnostic
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Conditional-Access-Diagnostic.
...
.EXAMPLE
    .\Conditional-Access-Diagnostic.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Conditional-Access-Diagnostic.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Conditional Access Diagnostic'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Conditional-Access-Diagnostic'
        Add-ReportRow -Check 'Tool' -Result 'Conditional-Access-Diagnostic' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Conditional Access Diagnostic' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Conditional Access Diagnostic' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
