...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Intune MDM Re Enroll
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Intune-MDM-Re-Enroll.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Intune-MDM-Re-Enroll'
        Add-ReportRow -Check 'Tool' -Result 'Intune-MDM-Re-Enroll' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Intune MDM Re Enroll' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Intune MDM Re Enroll' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
