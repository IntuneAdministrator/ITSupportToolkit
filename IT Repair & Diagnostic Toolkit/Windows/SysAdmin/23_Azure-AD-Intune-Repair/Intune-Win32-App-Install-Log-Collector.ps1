...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Intune Win32 App Install Log Collector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Intune-Win32-App-Install-Log-Collector.
...
.EXAMPLE
    .\Intune-Win32-App-Install-Log-Collector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Intune-Win32-App-Install-Log-Collector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Intune Win32 App Install Log Collector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Intune-Win32-App-Install-Log-Collector'
        Add-ReportRow -Check 'Tool' -Result 'Intune-Win32-App-Install-Log-Collector' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Intune Win32 App Install Log Collector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Intune Win32 App Install Log Collector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
