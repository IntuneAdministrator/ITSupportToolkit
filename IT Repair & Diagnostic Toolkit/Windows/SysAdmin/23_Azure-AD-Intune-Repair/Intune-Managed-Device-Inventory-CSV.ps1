...
<#
.SYNOPSIS
    Common problem fix: Intune Managed Device Inventory CSV
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Managed-Device-Inventory-CSV.ps1
.EXAMPLE
    .\Intune-Managed-Device-Inventory-CSV.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Managed Device Inventory CSV'; $totalSteps = 8; $Name = 'Intune-Managed-Device-Inventory-CSV'
try {
    ...
    Export-HtmlReport -Title 'Intune Managed Device Inventory CSV' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Managed Device Inventory CSV' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
