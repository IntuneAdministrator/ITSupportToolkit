...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Azure AD Device Registration Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Azure-AD-Device-Registration-Repair.
...
.EXAMPLE
    .\Azure-AD-Device-Registration-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Azure-AD-Device-Registration-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Azure AD Device Registration Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Azure-AD-Device-Registration-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Azure-AD-Device-Registration-Repair' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Azure AD Device Registration Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Azure AD Device Registration Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
