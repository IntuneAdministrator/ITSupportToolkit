...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Entra ID Connect Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Entra-ID-Connect-Health-Check.
...
.EXAMPLE
    .\Entra-ID-Connect-Health-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Entra-ID-Connect-Health-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Entra ID Connect Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Entra-ID-Connect-Health-Check'
        Add-ReportRow -Check 'Tool' -Result 'Entra-ID-Connect-Health-Check' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Entra ID Connect Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Entra ID Connect Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
