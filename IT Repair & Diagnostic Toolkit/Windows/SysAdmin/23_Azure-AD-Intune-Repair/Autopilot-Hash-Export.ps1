...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Autopilot Hash Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Autopilot-Hash-Export.
...
$exitCode = 0
$Activity = 'Autopilot Hash Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Autopilot-Hash-Export'
        Add-ReportRow -Check 'Tool' -Result 'Autopilot-Hash-Export' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Autopilot Hash Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Autopilot Hash Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
