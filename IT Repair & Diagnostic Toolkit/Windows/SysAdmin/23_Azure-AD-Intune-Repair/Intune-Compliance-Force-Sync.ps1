...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Intune Compliance Force Sync
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Intune-Compliance-Force-Sync.
...
.EXAMPLE
    .\Intune-Compliance-Force-Sync.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Intune-Compliance-Force-Sync.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Intune Compliance Force Sync'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Intune-Compliance-Force-Sync'
        Add-ReportRow -Check 'Tool' -Result 'Intune-Compliance-Force-Sync' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Intune Compliance Force Sync' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Intune Compliance Force Sync' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
