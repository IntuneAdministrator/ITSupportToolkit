...
<#
.SYNOPSIS
    Common problem fix: Entra Join Failed Triage
.DESCRIPTION
...
.EXAMPLE
    .\Entra-Join-Failed-Triage
.EXAMPLE
    .\Entra-Join-Failed-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Entra Join Failed Triage'; $totalSteps = 8; $Name = 'Entra-Join-Failed-Triage'
try {
    ...
    Export-HtmlReport -Title 'Entra Join Failed Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Entra Join Failed Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
