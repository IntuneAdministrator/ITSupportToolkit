...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Autopilot Profile Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Autopilot-Profile-Verify.
...
.EXAMPLE
    .\Windows-Autopilot-Profile-Verify.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Autopilot-Profile-Verify.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Autopilot Profile Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Autopilot-Profile-Verify'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Autopilot-Profile-Verify' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Windows Autopilot Profile Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Autopilot Profile Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
