...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Device Compliance Policy Force
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Device-Compliance-Policy-Force.
...
.EXAMPLE
    .\Device-Compliance-Policy-Force.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Device-Compliance-Policy-Force.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Device Compliance Policy Force'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Device-Compliance-Policy-Force'
        Add-ReportRow -Check 'Tool' -Result 'Device-Compliance-Policy-Force' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Device Compliance Policy Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Device Compliance Policy Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
