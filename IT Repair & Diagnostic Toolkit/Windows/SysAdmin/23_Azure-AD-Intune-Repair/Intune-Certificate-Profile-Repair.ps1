...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Intune Certificate Profile Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Intune-Certificate-Profile-Repair.
...
.EXAMPLE
    .\Intune-Certificate-Profile-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Intune-Certificate-Profile-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Intune Certificate Profile Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Intune-Certificate-Profile-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Intune-Certificate-Profile-Repair' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Intune Certificate Profile Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Intune Certificate Profile Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
