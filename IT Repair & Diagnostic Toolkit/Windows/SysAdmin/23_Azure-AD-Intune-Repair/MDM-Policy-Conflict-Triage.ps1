...
<#
.SYNOPSIS
    Common problem fix: MDM Policy Conflict Triage
.DESCRIPTION
...
.EXAMPLE
    .\MDM-Policy-Conflict-Triage
.EXAMPLE
    .\MDM-Policy-Conflict-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'MDM Policy Conflict Triage'; $totalSteps = 8; $Name = 'MDM-Policy-Conflict-Triage'
try {
    ...
    Export-HtmlReport -Title 'MDM Policy Conflict Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'MDM Policy Conflict Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
