...
<#
.SYNOPSIS
    Common problem fix: Autopilot ESP Failure Triage
.DESCRIPTION
...
.EXAMPLE
    .\Autopilot-ESP-Failure-Triage
.EXAMPLE
    .\Autopilot-ESP-Failure-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Autopilot ESP Failure Triage'; $totalSteps = 8; $Name = 'Autopilot-ESP-Failure-Triage'
try {
    ...
    Export-HtmlReport -Title 'Autopilot ESP Failure Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Autopilot ESP Failure Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
