...
<#
.SYNOPSIS
    Common problem fix: Intune Proactive Remediation Runbook Bundle
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 23_Azure-AD-Intune-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Intune-Proactive-Remediation-Runbook-Bundle.ps1
.EXAMPLE
    .\Intune-Proactive-Remediation-Runbook-Bundle.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Proactive Remediation Runbook Bundle'; $totalSteps = 8; $Name = 'Intune-Proactive-Remediation-Runbook-Bundle'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '23_Azure-AD-Intune-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'KVM|ATEN|Raritan|Avocent|PIV|CAC|Gemalto|Zebra|Honeywell|Datalogic|RFID|Receipt|Cash-Drawer|Pin-Pad|PowerMic|Stream-Deck|Dictation|COM-Port|Serial|FTDI|Dymo|P-Touch|MagTek|SupportAssist|Sure-Admin|vPro|AMT|MEBx|Toughbook|Getac|Windows-Hello|Biometric|NFC|ESC-POS|Star-Micronics|POS-Terminal|Conference-Speakerphone|Document-Camera|Elgato|Philips-Dictation|Handheld-RFID|Peripheral-Serial|KVM-Asset|IP-KVM|RDP-Redirect|Medical-Device|Hearing-Aid|Client-Command|BIOS-Configuration|Commercial-Vantage|HID-Global|Proximity-Card|Label-Printer-Calibration|KVM-Dock|Payment-Device') {
            Write-ProgressStep 4 $totalSteps $Activity 'Niche peripheral / KVM / vendor deep'
            if ($Name -match 'KVM|ATEN|Raritan|Avocent|IP-KVM|KVM-Dock|Dual-Monitor-KVM') {
                Get-PnpDevice -Class Monitor,Keyboard,Mouse,USB -ErrorAction SilentlyContinue | Select-Object -First 15 |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail $_.Class }
                Add-ReportRow -Check 'KVM guidance' -Result 'Reseat cables; disable USB selective suspend; test direct attach; update KVM firmware' -Status 'WARN' -Detail 'Batch H'
                if ($Name -match 'EDID|DisplayPort|Black-Screen') {
                    Add-ReportRow -Check 'EDID' -Result 'Use EDID emulator dongle if monitor lost on switch' -Status 'WARN' -Detail ''
                }
            }
            if ($Name -match 'PIV|CAC|YubiKey|Gemalto|SmartCard|HID-Global') {
                Get-PnpDevice -Class SmartCardReader -ErrorAction SilentlyContinue |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status $(if ($_.Status -eq 'OK') {'PASS'} else {'WARN'}) -Detail $_.InstanceId }
                certutil -scinfo 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'Smart card' -Result 'certutil -scinfo' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(180,$_.Length)) }
            }
            if ($Name -match 'Windows-Hello|Biometric|Fingerprint|NFC|Proximity') {
                Get-PnpDevice -Class Biometric -ErrorAction SilentlyContinue |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail 'Settings > Accounts > Sign-in options' }
                Get-WmiObject -Class Win32_Biometric -ErrorAction SilentlyContinue |
                    ForEach-Object { Add-ReportRow -Check $_.Description -Result $_.Status -Status 'INFO' -Detail $_.Manufacturer }
            }
            if ($Name -match 'Zebra|Honeywell|Datalogic|RFID|Barcode|Handheld|Scanner') {
                Get-PnpDevice -Class HIDClass,USB -ErrorAction SilentlyContinue | Where-Object { $_.FriendlyName -match 'Zebra|Honeywell|Datalogic|Symbol|Scanner|RFID' } |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail 'USB HID or OPOS driver' }
                if ($PSCmdlet.ShouldProcess('HID', 'Rescan devices')) { pnputil /scan-devices 2>&1 | Out-Null; Add-ReportRow -Check 'PnP rescan' -Result 'Done' -Status 'PASS' -Detail '' }
            }
            if ($Name -match 'Receipt|Cash-Drawer|Pin-Pad|ESC-POS|Star-Micronics|MagTek|Payment|POS') {
                Get-Printer -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'Receipt|Star|Epson|POS|TM-' } |
                    ForEach-Object { Add-ReportRow -Check $_.Name -Result $_.PrinterStatus -Status 'INFO' -Detail 'POS peripheral' }
                Get-PnpDevice -Class HIDClass -ErrorAction SilentlyContinue | Where-Object { $_.FriendlyName -match 'MagTek|Pin|Payment|Card' } |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail '' }
            }
            if ($Name -match 'PowerMic|Dictation|Philips|Foot-Pedal|Stream-Deck|Elgato|Conference-Speakerphone|Document-Camera|Dictation-Microphone') {
                Get-PnpDevice -Class AudioEndpoint,Media -ErrorAction SilentlyContinue | Select-Object -First 12 |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail 'AV peripheral' }
                Get-PnpDevice -Class HIDClass -ErrorAction SilentlyContinue | Where-Object { $_.FriendlyName -match 'Elgato|PowerMic|Pedal|Stream' } |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail '' }
            }
            if ($Name -match 'COM-Port|Serial|FTDI|RDP-Redirect') {
                Get-CimInstance Win32_SerialPort -ErrorAction SilentlyContinue |
                    ForEach-Object { Add-ReportRow -Check $_.Name -Result $_.Status -Status 'INFO' -Detail $_.DeviceID }
                Get-PnpDevice -ErrorAction SilentlyContinue | Where-Object { $_.FriendlyName -match 'FTDI|Serial|COM|Prolific' } |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail $_.InstanceId }
            }
            if ($Name -match 'Brother|P-Touch|Dymo|Label') {
                Get-Printer -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'Brother|Dymo|Label|P-Touch' } |
                    ForEach-Object { Add-ReportRow -Check $_.Name -Result $_.PrinterStatus -Status 'INFO' -Detail 'Label printer' }
            }
            if ($Name -match 'SupportAssist|Client-Command|Sure-Admin|BIOS-Configuration|Commercial-Vantage|vPro|AMT|MEBx|Toughbook|Getac|Surface-Enterprise|Zebra-Enterprise') {
                $cs = Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue
                if ($cs) { Add-ReportRow -Check 'OEM' -Result $cs.Manufacturer -Status 'INFO' -Detail $cs.Model }
                if ($Name -match 'Dell|SupportAssist|Client-Command') {
                    $sa = Get-Service -Name 'SupportAssistAgent' -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'SupportAssist' -Result $(if ($sa) { $sa.Status } else { 'Not installed' }) -Status 'INFO' -Detail 'Dell enterprise tooling'
                }
                if ($Name -match 'vPro|AMT|MEBx|Sure-Admin') {
                    Get-CimInstance -Namespace root/Intel_AMT -ClassName AMT_GeneralSettings -ErrorAction SilentlyContinue |
                        ForEach-Object { Add-ReportRow -Check 'Intel AMT' -Result 'Detected' -Status 'PASS' -Detail 'vPro manageability' }
                    if (-not (Get-CimInstance -Namespace root/Intel_AMT -ClassName AMT_GeneralSettings -ErrorAction SilentlyContinue)) {
                        Add-ReportRow -Check 'Intel AMT' -Result 'Not detected or not provisioned' -Status 'WARN' -Detail 'Ctrl+P MEBx for setup'
                    }
                }
            }
            if ($Name -match 'Medical-Device|Hearing-Aid|Multipoint') {
                Get-PnpDevice -Class Bluetooth -ErrorAction SilentlyContinue |
                    ForEach-Object { Add-ReportRow -Check $_.FriendlyName -Result $_.Status -Status 'INFO' -Detail 'Bluetooth peripheral' }
                if ($PSCmdlet.ShouldProcess('Bluetooth', 'Restart bthserv')) {
                    Restart-Service bthserv -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Bluetooth service' -Result 'Restarted' -Status 'PASS' -Detail ''
                }
            }
            if ($Name -match 'Peripheral-Serial|KVM-Asset|POS-Terminal') {
                Get-CimInstance Win32_BIOS -ErrorAction SilentlyContinue | ForEach-Object {
                    Add-ReportRow -Check 'Host serial' -Result $_.SerialNumber -Status 'INFO' -Detail 'Attach peripheral serials in CMDB'
                }
            }
        }

        if ($Name -match 'Intune|Autopilot|Graph|Entra|CMDB|SnipeIT|ServiceNow|Lansweeper|Asset-Panda|PDQ|SCCM|Co-Management|Endpoint-Analytics|Compliance|Remediation|Jamf|MDM|Tenant-|Purview|BitLocker-Recovery|Win32-App|Proactive-Remediation|Configuration-Profile|Helpdesk-Intune|Helpdesk-Autopilot|Hardware-Batch') {
            Write-ProgressStep 4 $totalSteps $Activity 'Intune / CMDB / deploy'
            $Join = Get-JoinState
            Add-ReportRow -Check 'Join state' -Result $Join.Label -Status 'INFO' -Detail 'Intune eligibility'
            $cs = Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue
            $bios = Get-CimInstance Win32_BIOS -ErrorAction SilentlyContinue
            $os = Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue
            if ($cs) { Add-ReportRow -Check 'Hostname' -Result $env:COMPUTERNAME -Status 'INFO' -Detail $cs.Model }
            if ($bios) { Add-ReportRow -Check 'Serial' -Result $bios.SerialNumber -Status 'INFO' -Detail $bios.Manufacturer }
            if ($os) { Add-ReportRow -Check 'OS' -Result $os.Caption -Status 'INFO' -Detail $os.Version }

            if ($Name -match 'CMDB|SnipeIT|ServiceNow|Lansweeper|Asset-Panda|PDQ|Inventory|Hardware-Export|Warranty-Expiry|Peripheral-Attach|Software-Asset|Batch-F-G') {
                $exportPath = Join-Path $env:TEMP ("cmdb_export_$env:COMPUTERNAME_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv")
                $rows = @([PSCustomObject]@{
                    ComputerName = $env:COMPUTERNAME; Serial = $bios.SerialNumber; Manufacturer = $cs.Manufacturer
                    Model = $cs.Model; User = $env:USERNAME; OS = $os.Caption; JoinState = $Join.Label
                ...
                Add-ReportRow -Check 'CMDB CSV' -Result $exportPath -Status 'PASS' -Detail 'Import to Snipe-IT/ServiceNow/Lansweeper'
                Get-PnpDevice -Class Monitor,USB -ErrorAction SilentlyContinue | Select-Object -First 10 |
                    ForEach-Object { Add-ReportRow -Check "Peripheral $($_.FriendlyName)" -Result $_.Status -Status 'INFO' -Detail $_.Class }
            }

            if ($Name -match 'Intune|Graph|Entra|Autopilot|Endpoint-Analytics|Compliance|Remediation|Tenant|Managed-Device|BitLocker|Win32|Configuration-Profile|Security-Baseline|Defender-AV|Purview|Co-Management|Helpdesk-Intune|Helpdesk-Autopilot') {
                dsregcmd /status 2>&1 | Out-String | ForEach-Object {
                    Add-ReportRow -Check 'dsregcmd' -Result 'Captured' -Status 'INFO' -Detail 'AzureAdJoined / MDM URLs'
                }
                $ime = Get-Service -Name 'IntuneManagementExtension' -ErrorAction SilentlyContinue
                if ($ime) { Add-ReportRow -Check 'IME service' -Result $ime.Status -Status 'PASS' -Detail $ime.StartType }
                else { Add-ReportRow -Check 'IME service' -Result 'Not found' -Status 'WARN' -Detail 'Device may not be Intune-managed' }
                if ($Name -match 'BitLocker') {
                    manage-bde -status C: 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'BitLocker C:' -Result 'Status captured' -Status 'INFO' -Detail 'Escrow key via Intune portal' }
                }
                if ($Name -match 'Graph|Tenant|Managed-App|Registered-Owner|Hardware-Inventory-Graph') {
                    Add-ReportRow -Check 'Microsoft Graph' -Result 'Use Graph Explorer or Connect-MgGraph' -Status 'WARN' -Detail 'GET /deviceManagement/managedDevices'
                }
                if ($Name -match 'Autopilot|Device-Preparation|Profile-Assignment') {
                    Add-ReportRow -Check 'Autopilot' -Result 'Verify in Entra > Devices > Enroll > Autopilot' -Status 'INFO' -Detail 'Deployment profile assignment'
                }
                if ($Name -match 'Remediation|Platform-Script|Script-Deploy|Custom-Compliance|Filter-Assignment') {
                    Add-ReportRow -Check 'Intune scripts' -Result 'Upload as Platform Script or Proactive Remediation' -Status 'INFO' -Detail 'Run as SYSTEM; 64-bit PS; test with -WhatIf locally first'
                }
            }

            if ($Name -match 'SCCM|Co-Management') {
                $ccm = Get-Service -Name CcmExec -ErrorAction SilentlyContinue
                if ($ccm) { Add-ReportRow -Check 'ConfigMgr client' -Result $ccm.Status -Status 'PASS' -Detail 'CcmExec' }
                else { Add-ReportRow -Check 'ConfigMgr client' -Result 'Not installed' -Status 'WARN' -Detail '' }
                $cm = Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\CCM' -ErrorAction SilentlyContinue
                if ($cm) { Add-ReportRow -Check 'Co-management' -Result 'Check CCM co-management handler in console' -Status 'INFO' -Detail '' }
            }
        }

        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            Write-ProgressStep 4 $totalSteps $Activity 'Runbook orchestration'
            $repoRoot = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
            if (-not (Test-Path $repoRoot)) { $repoRoot = 'C:\IT-Repair-Scripts' }
            Add-ReportRow -Check 'Runbook' -Result $Name -Status 'INFO' -Detail 'Batch J multi-step orchestration'
            $steps = @()

            if ($Name -match 'Network|Connectivity|WiFi|Ethernet|DNS-DHCP') {
                $steps = @('ipconfig /flushdns','netsh winsock reset','Restart-Service WSearch -EA SilentlyContinue')
                if ($PSCmdlet.ShouldProcess('Network', 'Run network runbook steps')) {
                    ipconfig /flushdns 2>&1 | Out-Null; netsh winsock reset 2>&1 | Out-Null
                    Add-ReportRow -Check 'Network runbook' -Result 'DNS flush + winsock reset' -Status 'PASS' -Detail 'Reboot if needed'
                }
            }
            if ($Name -match 'Printer|Print|Spooler') {
                if ($PSCmdlet.ShouldProcess('Spooler', 'Run printer runbook')) {
                    Stop-Service Spooler -Force -ErrorAction SilentlyContinue
                    Remove-Item "$env:SystemRoot\System32\spool\PRINTERS\*" -Force -ErrorAction SilentlyContinue
                    Start-Service Spooler -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Printer runbook' -Result 'Spooler cycle complete' -Status 'PASS' -Detail ''
                }
            }
            if ($Name -match 'M365|Office365|Office-365') {
                Get-Process ms-teams, Teams, OUTLOOK, WINWORD, EXCEL -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'M365 runbook' -Result 'Office apps stopped' -Status 'PASS' -Detail 'Clear Teams/Outlook cache if issue persists'
            }
            if ($Name -match 'Slow-PC|Performance|Disk-Cleanup') {
                $disk = Get-CimInstance Win32_LogicalDisk -Filter "DeviceID='C:'"
                Add-ReportRow -Check 'C: free before' -Result ("{0:N1} GB" -f ($disk.FreeSpace/1GB)) -Status 'INFO' -Detail ''
                if ($PSCmdlet.ShouldProcess('Temp', 'Clean temp folders')) {
                    Remove-Item "$env:TEMP\*" -Recurse -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Temp cleanup' -Result 'Attempted' -Status 'PASS' -Detail $env:TEMP
                }
            }
            if ($Name -match 'Security-Hardening|Security-Baseline') {
                $fw = Get-NetFirewallProfile -ErrorAction SilentlyContinue
                foreach ($p in $fw) { Add-ReportRow -Check "Firewall $($p.Name)" -Result $p.Enabled -Status 'INFO' -Detail '' }
            }
            if ($Name -match 'Windows-Update|Update-Full') {
                UsoClient StartScan 2>&1 | Out-Null
                Add-ReportRow -Check 'Update scan' -Result 'UsoClient StartScan triggered' -Status 'PASS' -Detail 'Settings > Windows Update'
            }
            if ($Name -match 'Boot|Recovery|Boot-Loop') {
                bcdedit /enum 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'BCD' -Result 'Captured' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(120,$_.Length)) }
            }
            if ($Name -match 'Hardware|Driver-Refresh|Inventory-Collect|Hardware-Pack') {
                pnputil /scan-devices 2>&1 | Out-Null
                Get-CimInstance Win32_BIOS -ErrorAction SilentlyContinue | ForEach-Object { Add-ReportRow -Check 'Serial' -Result $_.SerialNumber -Status 'INFO' -Detail $_.Manufacturer }
            }
            if ($Name -match 'IntuneWin|Win32-App|Detection-Script|Install-Command|Uninstall-Command') {
                $iw = @('C:\Program Files (x86)\Microsoft Intune Win32 Content Prep Tool\IntuneWinAppUtil.exe','.\IntuneWinAppUtil.exe') | Where-Object { Test-Path  } | Select-Object -First 1
                if ($iw) { Add-ReportRow -Check 'IntuneWinAppUtil' -Result $iw -Status 'PASS' -Detail 'intunewinapputil -c source -s setup.exe -o output' }
                else { Add-ReportRow -Check 'IntuneWinAppUtil' -Result 'Download from GitHub microsoft/Microsoft-Win32-Content-Prep-Tool' -Status 'WARN' -Detail 'Package .intunewin for Win32 apps' }
                Add-ReportRow -Check 'Detection template' -Result 'if (Test-Path ...) { exit 0 } else { exit 1 }' -Status 'INFO' -Detail 'Use in Win32 app detection rule'
            }
            if ($Name -match 'SCCM-Application|SCCM-Task-Sequence|Remediation-Runbook|Required-Script') {
                Add-ReportRow -Check 'Deploy runbook' -Result 'Assign to pilot group; monitor Intune/SCCM reporting' -Status 'INFO' -Detail 'Test locally with -WhatIf first'
            }
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '23_Azure-AD-Intune-Repair'
            }
            Add-ReportRow -Check 'Runbook complete' -Result "$($steps.Count) automated steps" -Status 'PASS' -Detail 'Review HTML report; reboot if prompted'
        }

        Write-ProgressStep 5 $totalSteps $Activity 'Common problem module complete'
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Intune Proactive Remediation Runbook Bundle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Proactive Remediation Runbook Bundle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
