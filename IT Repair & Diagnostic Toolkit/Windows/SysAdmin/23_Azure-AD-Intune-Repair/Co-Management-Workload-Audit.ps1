...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Co Management Workload Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Co-Management-Workload-Audit.
...
.EXAMPLE
    .\Co-Management-Workload-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Co-Management-Workload-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Co Management Workload Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Co-Management-Workload-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Co-Management-Workload-Audit' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Co Management Workload Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Co Management Workload Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
