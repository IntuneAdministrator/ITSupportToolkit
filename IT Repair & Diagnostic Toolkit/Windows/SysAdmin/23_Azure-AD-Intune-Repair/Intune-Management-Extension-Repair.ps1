...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Intune Management Extension Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Intune-Management-Extension-Repair.
...
.EXAMPLE
    .\Intune-Management-Extension-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Intune-Management-Extension-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Intune Management Extension Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Intune-Management-Extension-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Intune-Management-Extension-Repair' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Intune Management Extension Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Intune Management Extension Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
