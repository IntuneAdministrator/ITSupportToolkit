...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Hybrid Join Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Hybrid-Join-Repair.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Hybrid-Join-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Hybrid-Join-Repair' -Status 'INFO' -Detail '23_Azure-AD-Intune-Repair'
    ...
    Export-HtmlReport -Title 'Hybrid Join Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Hybrid Join Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
