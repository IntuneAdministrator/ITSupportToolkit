...
<#
.SYNOPSIS
    Common problem fix: Hybrid Join Failed Triage
.DESCRIPTION
...
.EXAMPLE
    .\Hybrid-Join-Failed-Triage
.EXAMPLE
    .\Hybrid-Join-Failed-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hybrid Join Failed Triage'; $totalSteps = 8; $Name = 'Hybrid-Join-Failed-Triage'
try {
    ...
    Export-HtmlReport -Title 'Hybrid Join Failed Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hybrid Join Failed Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
