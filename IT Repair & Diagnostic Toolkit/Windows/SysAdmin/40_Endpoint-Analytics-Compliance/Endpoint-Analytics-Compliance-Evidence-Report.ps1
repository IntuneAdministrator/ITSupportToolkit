...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Evidence-Report.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Evidence Report'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
