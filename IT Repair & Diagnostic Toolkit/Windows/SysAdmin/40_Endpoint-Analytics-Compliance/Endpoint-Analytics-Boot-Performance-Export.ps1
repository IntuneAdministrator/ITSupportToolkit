...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Boot Performance Export
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 40_Endpoint-Analytics-Compliance). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Endpoint-Analytics-Boot-Performance-Export.ps1
.EXAMPLE
    .\Endpoint-Analytics-Boot-Performance-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Boot Performance Export'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Boot-Performance-Export'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '40_Endpoint-Analytics-Compliance' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '40_Endpoint-Analytics-Compliance'
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Boot Performance Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Boot Performance Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
