...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Policy-Validate.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Policy Validate'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
