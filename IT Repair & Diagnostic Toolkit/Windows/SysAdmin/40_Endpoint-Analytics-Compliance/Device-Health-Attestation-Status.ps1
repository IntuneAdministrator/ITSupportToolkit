...
<#
.SYNOPSIS
    Enterprise IT tool: Device Health Attestation Status
.DESCRIPTION
...
.EXAMPLE
    .\Device-Health-Attestation-Status.ps1
.EXAMPLE
    .\Device-Health-Attestation-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Device Health Attestation Status'; $totalSteps = 8; $Name = 'Device-Health-Attestation-Status'
try {
    ...
    Export-HtmlReport -Title 'Device Health Attestation Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Device Health Attestation Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
