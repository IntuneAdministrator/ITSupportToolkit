...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Quick-Fix.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Quick Fix'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
