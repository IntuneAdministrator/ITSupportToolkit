...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Anomaly-Detect.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Anomaly Detect'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
