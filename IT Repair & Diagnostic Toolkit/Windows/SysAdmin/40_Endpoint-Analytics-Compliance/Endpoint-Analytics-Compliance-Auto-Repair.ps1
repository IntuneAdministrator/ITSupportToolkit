...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Auto-Repair.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Auto Repair'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
