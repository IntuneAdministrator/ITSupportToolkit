...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Startup Impact Export
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Startup-Impact-Export.ps1
.EXAMPLE
    .\Endpoint-Analytics-Startup-Impact-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Startup Impact Export'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Startup-Impact-Export'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Startup Impact Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Startup Impact Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
