...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics App Reliability Export
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-App-Reliability-Export.ps1
.EXAMPLE
    .\Endpoint-Analytics-App-Reliability-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics App Reliability Export'; $totalSteps = 8; $Name = 'Endpoint-Analytics-App-Reliability-Export'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics App Reliability Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics App Reliability Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
