...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Health-Triage.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Health Triage'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
