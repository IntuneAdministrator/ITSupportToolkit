...
<#
.SYNOPSIS
    Common problem fix: Defender AV Status Intune Sync
.DESCRIPTION
...
.EXAMPLE
    .\Defender-AV-Status-Intune-Sync.ps1
.EXAMPLE
    .\Defender-AV-Status-Intune-Sync.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Defender AV Status Intune Sync'; $totalSteps = 8; $Name = 'Defender-AV-Status-Intune-Sync'
try {
    ...
    Export-HtmlReport -Title 'Defender AV Status Intune Sync' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Defender AV Status Intune Sync' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
