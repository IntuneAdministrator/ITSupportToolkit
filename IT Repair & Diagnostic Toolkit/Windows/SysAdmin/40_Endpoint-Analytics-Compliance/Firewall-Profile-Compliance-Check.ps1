...
<#
.SYNOPSIS
    Enterprise IT tool: Firewall Profile Compliance Check
.DESCRIPTION
...
.EXAMPLE
    .\Firewall-Profile-Compliance-Check.ps1
.EXAMPLE
    .\Firewall-Profile-Compliance-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Firewall Profile Compliance Check'; $totalSteps = 8; $Name = 'Firewall-Profile-Compliance-Check'
try {
    ...
    Export-HtmlReport -Title 'Firewall Profile Compliance Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Firewall Profile Compliance Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
