...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Deep-Audit.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Deep Audit'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
