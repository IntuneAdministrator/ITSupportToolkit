...
<#
.SYNOPSIS
    Enterprise IT tool: BitLocker Escrow Azure Verify
.DESCRIPTION
...
.EXAMPLE
    .\BitLocker-Escrow-Azure-Verify.ps1
.EXAMPLE
    .\BitLocker-Escrow-Azure-Verify.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BitLocker Escrow Azure Verify'; $totalSteps = 8; $Name = 'BitLocker-Escrow-Azure-Verify'
try {
    ...
    Export-HtmlReport -Title 'BitLocker Escrow Azure Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BitLocker Escrow Azure Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
