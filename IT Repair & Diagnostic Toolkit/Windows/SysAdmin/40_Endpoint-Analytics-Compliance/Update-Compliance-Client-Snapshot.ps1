...
<#
.SYNOPSIS
    Enterprise IT tool: Update Compliance Client Snapshot
.DESCRIPTION
    Expansion module (SysAdmin / 40_Endpoint-Analytics-Compliance) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Update-Compliance-Client-Snapshot.ps1
.EXAMPLE
    .\Update-Compliance-Client-Snapshot.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Update Compliance Client Snapshot'; $totalSteps = 8; $Name = 'Update-Compliance-Client-Snapshot'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '40_Endpoint-Analytics-Compliance' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'ENDPOINT ANALYTICS / COMPLIANCE'
        try { $tpm = Get-Tpm; Add-ReportRow -Check 'TPM Ready' -Result "$($tpm.TpmReady)" -Status $(if ($tpm.TpmReady) {'PASS'} else {'FAIL'}) -Detail '' } catch { Add-ReportRow -Check 'TPM' -Result 'Error' -Status 'WARN' -Detail $_.Exception.Message }
        try { Confirm-SecureBootUEFI -EA Stop | Out-Null; Add-ReportRow -Check 'Secure Boot' -Result 'On' -Status 'PASS' -Detail '' } catch { Add-ReportRow -Check 'Secure Boot' -Result 'Off/N/A' -Status 'WARN' -Detail $_.Exception.Message }
        Write-ProgressStep 2 $totalSteps $Activity 'Platform'
        try {
            Get-BitLockerVolume -EA Stop | ForEach-Object {
                Add-ReportRow -Check "BitLocker $($_.MountPoint)" -Result $_.ProtectionStatus -Status $(if ($_.ProtectionStatus -eq 'On') {'PASS'} else {'WARN'}) -Detail $_.VolumeStatus
            }
        } catch { Add-ReportRow -Check 'BitLocker' -Result 'N/A' -Status 'WARN' -Detail $_.Exception.Message }
        try {
            $mp = Get-MpComputerStatus
            Add-ReportRow -Check 'Defender RT' -Result "$($mp.RealTimeProtectionEnabled)" -Status $(if ($mp.RealTimeProtectionEnabled) {'PASS'} else {'FAIL'}) -Detail ''
            $asr = (Get-MpPreference).AttackSurfaceReductionRules_Ids
            Add-ReportRow -Check 'ASR rules' -Result "$(@($asr).Count)" -Status 'INFO' -Detail ''
        } catch { Add-ReportRow -Check 'Defender' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message }
        Get-NetFirewallProfile | ForEach-Object {
            Add-ReportRow -Check "FW $($_.Name)" -Result $(if ($_.Enabled) {'On'} else {'Off'}) -Status $(if ($_.Enabled) {'PASS'} else {'FAIL'}) -Detail ''
        }
        Add-ReportRow -Check 'Join compliance context' -Result $Join.Label -Status 'INFO' -Detail 'Map to Intune compliance policies'
        Write-ProgressStep 5 $totalSteps $Activity 'Compliance snapshot done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Update Compliance Client Snapshot' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Update Compliance Client Snapshot' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
