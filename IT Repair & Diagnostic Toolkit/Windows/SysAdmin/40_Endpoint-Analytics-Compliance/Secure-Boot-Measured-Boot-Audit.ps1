...
<#
.SYNOPSIS
    Enterprise IT tool: Secure Boot Measured Boot Audit
.DESCRIPTION
...
.EXAMPLE
    .\Secure-Boot-Measured-Boot-Audit.ps1
.EXAMPLE
    .\Secure-Boot-Measured-Boot-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Secure Boot Measured Boot Audit'; $totalSteps = 8; $Name = 'Secure-Boot-Measured-Boot-Audit'
try {
    ...
    Export-HtmlReport -Title 'Secure Boot Measured Boot Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Secure Boot Measured Boot Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
