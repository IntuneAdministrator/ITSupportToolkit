...
<#
.SYNOPSIS
    Enterprise IT tool: Defender ASR Status Export
.DESCRIPTION
...
.EXAMPLE
    .\Defender-ASR-Status-Export.ps1
.EXAMPLE
    .\Defender-ASR-Status-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Defender ASR Status Export'; $totalSteps = 8; $Name = 'Defender-ASR-Status-Export'
try {
    ...
    Export-HtmlReport -Title 'Defender ASR Status Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Defender ASR Status Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
