...
<#
.SYNOPSIS
    Enterprise IT tool: Password Protection DC Agent Hint
.DESCRIPTION
...
.EXAMPLE
    .\Password-Protection-DC-Agent-Hint.ps1
.EXAMPLE
    .\Password-Protection-DC-Agent-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Password Protection DC Agent Hint'; $totalSteps = 8; $Name = 'Password-Protection-DC-Agent-Hint'
try {
    ...
    Export-HtmlReport -Title 'Password Protection DC Agent Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Password Protection DC Agent Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
