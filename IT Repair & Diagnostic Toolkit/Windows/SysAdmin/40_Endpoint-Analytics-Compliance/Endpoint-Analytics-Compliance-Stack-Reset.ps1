...
<#
.SYNOPSIS
    Common problem fix: Endpoint Analytics Compliance Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Stack-Reset.ps1
.EXAMPLE
    .\Endpoint-Analytics-Compliance-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Endpoint Analytics Compliance Stack Reset'; $totalSteps = 8; $Name = 'Endpoint-Analytics-Compliance-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Endpoint Analytics Compliance Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Endpoint Analytics Compliance Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
