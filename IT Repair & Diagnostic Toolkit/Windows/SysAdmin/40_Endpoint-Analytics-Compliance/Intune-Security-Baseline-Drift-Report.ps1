...
<#
.SYNOPSIS
    Common problem fix: Intune Security Baseline Drift Report
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Security-Baseline-Drift-Report.ps1
.EXAMPLE
    .\Intune-Security-Baseline-Drift-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Security Baseline Drift Report'; $totalSteps = 8; $Name = 'Intune-Security-Baseline-Drift-Report'
try {
    ...
    Export-HtmlReport -Title 'Intune Security Baseline Drift Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Security Baseline Drift Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
