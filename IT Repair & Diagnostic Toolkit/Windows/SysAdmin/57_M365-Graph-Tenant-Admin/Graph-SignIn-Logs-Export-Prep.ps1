...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Graph SignIn Logs Export Prep
.DESCRIPTION
...
.EXAMPLE
    .\Graph-SignIn-Logs-Export-Prep.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Graph-SignIn-Logs-Export-Prep.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph SignIn Logs Export Prep'; $totalSteps = 8; $Name = 'Graph-SignIn-Logs-Export-Prep'; $Folder = '57_M365-Graph-Tenant-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Graph SignIn Logs Export Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph SignIn Logs Export Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
