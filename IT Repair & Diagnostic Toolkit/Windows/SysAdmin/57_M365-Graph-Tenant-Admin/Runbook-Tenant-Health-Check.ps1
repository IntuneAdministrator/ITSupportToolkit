...
<#
.SYNOPSIS
    Common problem fix: Runbook Tenant Health Check
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 57_M365-Graph-Tenant-Admin). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-Tenant-Health-Check.ps1
.EXAMPLE
    .\Runbook-Tenant-Health-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Tenant Health Check'; $totalSteps = 8; $Name = 'Runbook-Tenant-Health-Check'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '57_M365-Graph-Tenant-Admin' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '57_M365-Graph-Tenant-Admin'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '57_M365-Graph-Tenant-Admin'
    ...
    Export-HtmlReport -Title 'Runbook Tenant Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Tenant Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
