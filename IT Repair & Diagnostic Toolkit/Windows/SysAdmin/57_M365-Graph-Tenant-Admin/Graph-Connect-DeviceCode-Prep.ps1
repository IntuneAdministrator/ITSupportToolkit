...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Graph Connect DeviceCode Prep
.DESCRIPTION
    Phase 12 M365 tenant admin (SysAdmin/57_M365-Graph-Tenant-Admin). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\Graph-Connect-DeviceCode-Prep.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Graph-Connect-DeviceCode-Prep.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Connect DeviceCode Prep'; $totalSteps = 8; $Name = 'Graph-Connect-DeviceCode-Prep'; $Folder = '57_M365-Graph-Tenant-Admin'
...
try {
    ...
    Add-ReportRow -Check 'Module' -Result '57_M365-Graph-Tenant-Admin' -Status 'INFO' -Detail 'SysAdmin Phase12 M365'
    Add-M365PortalRows
    ...
    if ($UPN) { Add-ReportRow -Check 'Target UPN' -Result $UPN -Status 'INFO' -Detail '' }
        ...
        Write-SectionBanner 'GRAPH TENANT ADMIN'
        Write-ProgressStep 3 $totalSteps $Activity 'Tenant ops'
        if ($Name -match 'Connect-DeviceCode') {
            Add-ReportRow -Check 'Device code tip' -Result 'Connect-MgGraph -UseDeviceCode -Scopes User.Read.All,Directory.Read.All' -Status 'INFO' -Detail 'Use on jump boxes without browser'
            if ($connected) { Add-ReportRow -Check 'Session' -Result 'Active' -Status 'PASS' -Detail '' }
        }
        if ($connected -and $Name -match 'User-Lookup' -and $UPN) {
            try {
                Import-Module Microsoft.Graph.Users -EA SilentlyContinue
                $u = Get-MgUser -UserId $UPN -Property id,displayName,userPrincipalName,accountEnabled,assignedLicenses,jobTitle,department -EA Stop
                Add-ReportRow -Check 'DisplayName' -Result $u.DisplayName -Status 'PASS' -Detail $u.Id
                Add-ReportRow -Check 'Enabled' -Result "$($u.AccountEnabled)" -Status $(if($u.AccountEnabled){'PASS'}else{'WARN'}) -Detail ''
                Add-ReportRow -Check 'Licenses' -Result "$($u.AssignedLicenses.Count) SKU(s)" -Status $(if($u.AssignedLicenses.Count -gt 0){'PASS'}else{'WARN'}) -Detail ''
            } catch { Add-ReportRow -Check 'Get-MgUser' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($connected -and $Name -match 'License-SKU') {
            try {
                Import-Module Microsoft.Graph.Identity.DirectoryManagement -EA SilentlyContinue
                $skus = Get-MgSubscribedSku -EA Stop
                foreach ($s in ($skus | Select-Object -First 12)) {
                    $avail = $s.PrepaidUnits.Enabled - $s.ConsumedUnits
                    Add-ReportRow -Check $s.SkuPartNumber -Result "consumed=$($s.ConsumedUnits) avail~$avail" -Status $(if($avail -le 0){'WARN'}else{'PASS'}) -Detail $s.SkuId
                }
                if ($UPN) {
                    Import-Module Microsoft.Graph.Users -EA SilentlyContinue
                    $u = Get-MgUser -UserId $UPN -Property assignedLicenses -EA SilentlyContinue
                    Add-ReportRow -Check "User $UPN SKUs" -Result "$($u.AssignedLicenses.Count)" -Status 'INFO' -Detail ''
                }
            } catch { Add-ReportRow -Check 'SubscribedSku' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($connected -and $Name -match 'Group-Membership' -and $UPN) {
            try {
                Import-Module Microsoft.Graph.Users -EA SilentlyContinue
                $groups = Get-MgUserMemberOf -UserId $UPN -EA Stop | Select-Object -First 25
                Add-ReportRow -Check 'MemberOf (sample)' -Result "$($groups.Count) listed" -Status 'PASS' -Detail (($groups | ForEach-Object { $_.AdditionalProperties.displayName }) -join '; ')
            } catch { Add-ReportRow -Check 'MemberOf' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($Name -match 'SignIn-Logs') {
            Add-ReportRow -Check 'Sign-in logs' -Result 'Entra > Monitoring > Sign-in logs' -Status 'INFO' -Detail 'Needs AuditLog.Read.All; export via Graph beta/reports or portal'
            if ($UPN) { Add-ReportRow -Check 'Filter' -Result "userPrincipalName eq '$UPN'" -Status 'INFO' -Detail '' }
        }
        if ($connected -and $Name -match 'Conditional-Access') {
            try {
                Import-Module Microsoft.Graph.Identity.SignIns -EA SilentlyContinue
                $cas = Get-MgIdentityConditionalAccessPolicy -EA Stop
                Add-ReportRow -Check 'CA policies' -Result "$($cas.Count)" -Status 'PASS' -Detail (($cas | Select-Object -First 8 DisplayName) -join '; ')
            } catch { Add-ReportRow -Check 'CA list' -Result 'Failed/need Policy.Read.All' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($connected -and $Name -match 'App-Consent') {
            try {
                Import-Module Microsoft.Graph.Applications -EA SilentlyContinue
                $apps = Get-MgServicePrincipal -Top 15 -EA Stop
                Add-ReportRow -Check 'Service principals sample' -Result "$($apps.Count)" -Status 'INFO' -Detail 'Review enterprise apps consent in Entra'
            } catch { Add-ReportRow -Check 'App consent' -Result 'Use Entra Enterprise apps' -Status 'INFO' -Detail $_.Exception.Message }
        }
        if ($connected -and $Name -match 'Service-Health') {
            try {
                Import-Module Microsoft.Graph.DeviceManagement.Administration -EA SilentlyContinue
                # serviceAnnouncements often needs ServiceHealth.Read.All
                $health = Invoke-MgGraphRequest -Method GET -Uri 'https://graph.microsoft.com/v1.0/admin/serviceAnnouncement/healthOverviews' -EA Stop
                $vals = @($health.value)
                foreach ($h in ($vals | Select-Object -First 10)) {
                    Add-ReportRow -Check $h.service -Result $h.status -Status $(if($h.status -eq 'serviceOperational'){'PASS'}else{'WARN'}) -Detail ''
                }
            } catch { Add-ReportRow -Check 'Service health API' -Result 'Portal fallback' -Status 'WARN' -Detail 'https://admin.microsoft.com/#/servicehealth' }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Graph tenant done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Graph Connect DeviceCode Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Connect DeviceCode Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
