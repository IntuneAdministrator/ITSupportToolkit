...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Graph Conditional Access List
.DESCRIPTION
...
.EXAMPLE
    .\Graph-Conditional-Access-List.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Graph-Conditional-Access-List.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Conditional Access List'; $totalSteps = 8; $Name = 'Graph-Conditional-Access-List'; $Folder = '57_M365-Graph-Tenant-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Graph Conditional Access List' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Conditional Access List' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
