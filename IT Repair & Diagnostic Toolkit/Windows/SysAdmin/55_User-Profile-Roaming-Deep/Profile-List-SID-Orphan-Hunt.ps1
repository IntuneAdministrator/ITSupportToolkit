...
<#
.SYNOPSIS
    Enterprise IT tool: Profile List SID Orphan Hunt
.DESCRIPTION
    Phase 11 expansion (SysAdmin/55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Profile-List-SID-Orphan-Hunt.ps1
.EXAMPLE
    .\Profile-List-SID-Orphan-Hunt.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Profile List SID Orphan Hunt'; $totalSteps=8; $Name='Profile-List-SID-Orphan-Hunt'; $Folder='55_User-Profile-Roaming-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '55_User-Profile-Roaming-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'USER PROFILE / ROAMING DEEP'
        Write-ProgressStep 2 $totalSteps $Activity 'Profiles'
        $profSvc = Get-Service ProfSvc -EA SilentlyContinue
        Add-ReportRow -Check 'ProfSvc' -Result $(if($profSvc){"$($profSvc.Status)"}else{'N/A'}) -Status $(if($profSvc -and $profSvc.Status -eq 'Running'){'PASS'}else{'FAIL'}) -Detail ''
        $list = Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList' -EA SilentlyContinue
        $temp = @($list | Where-Object { (Get-ItemProperty $_.PSPath -EA SilentlyContinue).State -eq 0x8000 -or $_.PSChildName -match '\.bak$' })
        Add-ReportRow -Check 'ProfileList SIDs' -Result "$($list.Count) ($($temp.Count) bak/temp hints)" -Status $(if($temp.Count -eq 0){'PASS'}else{'WARN'}) -Detail ''
        if ($Name -match 'Temp-Profile') {
            Add-ReportRow -Check 'Temp profile' -Result 'Check Event 1511/1515 User Profile Service; rename .bak SID keys carefully' -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Folder-Redirection') {
            $fr = Get-ChildItem 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders' -EA SilentlyContinue
            Add-ReportRow -Check 'Shell folders' -Result $(if($fr){'Present'}else{'N/A'}) -Status 'INFO' -Detail 'Verify redirects under User Shell Folders'
        }
        if ($Name -match 'AppData|Bloat|Roaming-Profile-Size') {
            $roam = Join-Path $env:USERPROFILE 'AppData\Roaming'
            if (Test-Path $roam) {
                $size = (Get-ChildItem $roam -Recurse -Force -EA SilentlyContinue | Measure-Object Length -Sum).Sum
                Add-ReportRow -Check 'AppData\Roaming size' -Result ("{0:N1} MB" -f ($size/1MB)) -Status $(if($size -gt 1GB){'WARN'}else{'PASS'}) -Detail $roam
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Profile done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Profile List SID Orphan Hunt' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Profile List SID Orphan Hunt' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
