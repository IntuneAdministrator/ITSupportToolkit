...
<#
.SYNOPSIS
    Common problem fix: User Profile Roaming Deep Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\User-Profile-Roaming-Deep-Stack-Reset.ps1
.EXAMPLE
    .\User-Profile-Roaming-Deep-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Roaming Deep Stack Reset'; $totalSteps = 8; $Name = 'User-Profile-Roaming-Deep-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'User Profile Roaming Deep Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Roaming Deep Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
