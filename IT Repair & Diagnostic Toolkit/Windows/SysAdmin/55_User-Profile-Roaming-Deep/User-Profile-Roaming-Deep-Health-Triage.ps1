...
<#
.SYNOPSIS
    Common problem fix: User Profile Roaming Deep Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\User-Profile-Roaming-Deep-Health-Triage.ps1
.EXAMPLE
    .\User-Profile-Roaming-Deep-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Roaming Deep Health Triage'; $totalSteps = 8; $Name = 'User-Profile-Roaming-Deep-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'User Profile Roaming Deep Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Roaming Deep Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
