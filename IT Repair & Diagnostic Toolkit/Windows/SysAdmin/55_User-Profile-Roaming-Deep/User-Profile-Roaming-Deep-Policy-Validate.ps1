...
<#
.SYNOPSIS
    Common problem fix: User Profile Roaming Deep Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\User-Profile-Roaming-Deep-Policy-Validate.ps1
.EXAMPLE
    .\User-Profile-Roaming-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Roaming Deep Policy Validate'; $totalSteps = 8; $Name = 'User-Profile-Roaming-Deep-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '55_User-Profile-Roaming-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '55_User-Profile-Roaming-Deep'
    ...
    Export-HtmlReport -Title 'User Profile Roaming Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Roaming Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
