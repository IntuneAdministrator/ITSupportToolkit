...
<#
.SYNOPSIS
    Enterprise IT tool: Temp Profile Detect Repair
.DESCRIPTION
    Phase 11 expansion (SysAdmin/55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Temp-Profile-Detect-Repair.ps1
.EXAMPLE
    .\Temp-Profile-Detect-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Temp Profile Detect Repair'; $totalSteps=8; $Name='Temp-Profile-Detect-Repair'; $Folder='55_User-Profile-Roaming-Deep'
try {
    ...
    Export-HtmlReport -Title 'Temp Profile Detect Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Temp Profile Detect Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
