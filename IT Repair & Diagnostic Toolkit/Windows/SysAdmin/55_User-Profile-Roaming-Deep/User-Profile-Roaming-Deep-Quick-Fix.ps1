...
<#
.SYNOPSIS
    Common problem fix: User Profile Roaming Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\User-Profile-Roaming-Deep-Quick-Fix.ps1
.EXAMPLE
    .\User-Profile-Roaming-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Roaming Deep Quick Fix'; $totalSteps = 8; $Name = 'User-Profile-Roaming-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'User Profile Roaming Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Roaming Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
