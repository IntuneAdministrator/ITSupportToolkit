...
<#
.SYNOPSIS
    Enterprise IT tool: User Profile Service Repair
.DESCRIPTION
    Phase 11 expansion (SysAdmin/55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\User-Profile-Service-Repair.ps1
.EXAMPLE
    .\User-Profile-Service-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='User Profile Service Repair'; $totalSteps=8; $Name='User-Profile-Service-Repair'; $Folder='55_User-Profile-Roaming-Deep'
try {
    ...
    Export-HtmlReport -Title 'User Profile Service Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Service Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
