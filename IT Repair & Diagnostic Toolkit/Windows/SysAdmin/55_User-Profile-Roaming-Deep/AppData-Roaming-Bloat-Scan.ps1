...
<#
.SYNOPSIS
    Enterprise IT tool: AppData Roaming Bloat Scan
.DESCRIPTION
    Phase 11 expansion (SysAdmin/55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\AppData-Roaming-Bloat-Scan.ps1
.EXAMPLE
    .\AppData-Roaming-Bloat-Scan.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='AppData Roaming Bloat Scan'; $totalSteps=8; $Name='AppData-Roaming-Bloat-Scan'; $Folder='55_User-Profile-Roaming-Deep'
try {
    ...
    Export-HtmlReport -Title 'AppData Roaming Bloat Scan' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AppData Roaming Bloat Scan' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
