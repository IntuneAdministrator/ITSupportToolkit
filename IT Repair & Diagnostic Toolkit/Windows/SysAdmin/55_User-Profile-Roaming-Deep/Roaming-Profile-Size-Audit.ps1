...
<#
.SYNOPSIS
    Enterprise IT tool: Roaming Profile Size Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Roaming-Profile-Size-Audit.ps1
.EXAMPLE
    .\Roaming-Profile-Size-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Roaming Profile Size Audit'; $totalSteps=8; $Name='Roaming-Profile-Size-Audit'; $Folder='55_User-Profile-Roaming-Deep'
try {
    ...
    Export-HtmlReport -Title 'Roaming Profile Size Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Roaming Profile Size Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
