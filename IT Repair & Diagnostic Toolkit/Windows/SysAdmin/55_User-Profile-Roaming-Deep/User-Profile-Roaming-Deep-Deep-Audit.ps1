...
<#
.SYNOPSIS
    Common problem fix: User Profile Roaming Deep Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\User-Profile-Roaming-Deep-Deep-Audit.ps1
.EXAMPLE
    .\User-Profile-Roaming-Deep-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Roaming Deep Deep Audit'; $totalSteps = 8; $Name = 'User-Profile-Roaming-Deep-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'User Profile Roaming Deep Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Roaming Deep Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
