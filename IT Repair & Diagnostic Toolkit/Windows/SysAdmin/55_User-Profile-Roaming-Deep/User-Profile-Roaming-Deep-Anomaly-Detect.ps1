...
<#
.SYNOPSIS
    Common problem fix: User Profile Roaming Deep Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\User-Profile-Roaming-Deep-Anomaly-Detect.ps1
.EXAMPLE
    .\User-Profile-Roaming-Deep-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'User Profile Roaming Deep Anomaly Detect'; $totalSteps = 8; $Name = 'User-Profile-Roaming-Deep-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'User Profile Roaming Deep Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Profile Roaming Deep Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
