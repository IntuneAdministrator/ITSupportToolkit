...
<#
.SYNOPSIS
    Enterprise IT tool: Folder Redirection Health
.DESCRIPTION
    Phase 11 expansion (SysAdmin/55_User-Profile-Roaming-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Folder-Redirection-Health.ps1
.EXAMPLE
    .\Folder-Redirection-Health.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Folder Redirection Health'; $totalSteps=8; $Name='Folder-Redirection-Health'; $Folder='55_User-Profile-Roaming-Deep'
try {
    ...
    Export-HtmlReport -Title 'Folder Redirection Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Folder Redirection Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
