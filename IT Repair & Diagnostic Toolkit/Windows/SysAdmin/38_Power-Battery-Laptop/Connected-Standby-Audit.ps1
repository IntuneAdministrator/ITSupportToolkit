...
<#
.SYNOPSIS
    Enterprise IT tool: Connected Standby Audit
.DESCRIPTION
...
.EXAMPLE
    .\Connected-Standby-Audit.ps1
.EXAMPLE
    .\Connected-Standby-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Connected Standby Audit'; $totalSteps = 8; $Name = 'Connected-Standby-Audit'
try {
    ...
    Export-HtmlReport -Title 'Connected Standby Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connected Standby Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
