...
<#
.SYNOPSIS
    Enterprise IT tool: Battery Health Report
.DESCRIPTION
...
.EXAMPLE
    .\Battery-Health-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Battery Health Report'; $totalSteps = 8; $Name = 'Battery-Health-Report'
try {
    ...
    Export-HtmlReport -Title 'Battery Health Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Battery Health Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
