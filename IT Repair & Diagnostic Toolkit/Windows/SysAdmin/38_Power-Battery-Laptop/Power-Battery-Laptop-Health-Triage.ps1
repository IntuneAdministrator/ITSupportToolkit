...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Health-Triage.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Health Triage'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
