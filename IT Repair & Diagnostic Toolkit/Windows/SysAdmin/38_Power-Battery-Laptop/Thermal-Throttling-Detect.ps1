...
<#
.SYNOPSIS
    Common problem fix: Thermal Throttling Detect
.DESCRIPTION
...
.EXAMPLE
    .\Thermal-Throttling-Detect.ps1
.EXAMPLE
    .\Thermal-Throttling-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Thermal Throttling Detect'; $totalSteps = 8; $Name = 'Thermal-Throttling-Detect'
try {
    ...
    Export-HtmlReport -Title 'Thermal Throttling Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Thermal Throttling Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
