...
<#
.SYNOPSIS
    Enterprise IT tool: Power Plan Enterprise Enforce
.DESCRIPTION
    Expansion module (SysAdmin / 38_Power-Battery-Laptop) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Power-Plan-Enterprise-Enforce.ps1
.EXAMPLE
    .\Power-Plan-Enterprise-Enforce.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Plan Enterprise Enforce'; $totalSteps = 8; $Name = 'Power-Plan-Enterprise-Enforce'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '38_Power-Battery-Laptop' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'POWER / BATTERY'
        $batt = @(Get-CimInstance Win32_Battery -EA SilentlyContinue)
        foreach ($b in $batt) {
            Add-ReportRow -Check "Battery $($b.DeviceID)" -Result "$($b.EstimatedChargeRemaining)%" -Status 'INFO' -Detail "Status=$($b.BatteryStatus)"
        }
        if (-not $batt) { Add-ReportRow -Check 'Battery' -Result 'None (desktop?)' -Status 'INFO' -Detail '' }
        Write-ProgressStep 2 $totalSteps $Activity 'Battery'
        powercfg /list 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'Power schemes' -Result 'Listed' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(300,$_.Length)) }
        if ($Name -match 'Battery-Health|Sleep-Study' -and $PSCmdlet.ShouldProcess('powercfg', 'battery report / sleepstudy')) {
            $out = Join-Path 'C:\IT-Logs' ("battery-report_{0}.html" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
            powercfg /batteryreport /output $out 2>$null
            Add-ReportRow -Check 'Battery report' -Result $(if (Test-Path $out) {$out} else {'Failed'}) -Status $(if (Test-Path $out) {'PASS'} else {'WARN'}) -Detail ''
            if ($Name -match 'Sleep-Study') { powercfg /sleepstudy /output (Join-Path 'C:\IT-Logs' 'sleepstudy.html') 2>$null; Add-ReportRow -Check 'Sleep study' -Result 'Generated if supported' -Status 'INFO' -Detail '' }
        }
        if ($Name -match 'Power-Plan|Enforce' -and $PSCmdlet.ShouldProcess('Power', 'High Performance / Balanced')) {
            powercfg /setactive 381b4222-f694-41f0-9685-ff5bb260df2e 2>$null
            Add-ReportRow -Check 'Balanced plan' -Result 'Activated (GUID)' -Status 'PASS' -Detail '381b4222-f694-41f0-9685-ff5bb260df2e'
        }
        if ($Name -match 'Lid|Dock|USB|Standby|Throttle') {
            powercfg /q SCHEME_CURRENT SUB_BUTTONS 2>&1 | Select-Object -First 15 | Out-String | ForEach-Object { Add-ReportRow -Check 'Lid/button settings' -Result 'Queried' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(200,$_.Length)) }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Power done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Power Plan Enterprise Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Plan Enterprise Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
