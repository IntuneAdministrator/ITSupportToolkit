...
<#
.SYNOPSIS
    Common problem fix: Dell Power Manager Hint
.DESCRIPTION
...
.EXAMPLE
    .\Dell-Power-Manager-Hint.ps1
.EXAMPLE
    .\Dell-Power-Manager-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell Power Manager Hint'; $totalSteps = 8; $Name = 'Dell-Power-Manager-Hint'
try {
    ...
    Export-HtmlReport -Title 'Dell Power Manager Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell Power Manager Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
