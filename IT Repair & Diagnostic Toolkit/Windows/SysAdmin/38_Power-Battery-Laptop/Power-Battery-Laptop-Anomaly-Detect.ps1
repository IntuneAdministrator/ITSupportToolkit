...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Anomaly-Detect.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Anomaly Detect'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
