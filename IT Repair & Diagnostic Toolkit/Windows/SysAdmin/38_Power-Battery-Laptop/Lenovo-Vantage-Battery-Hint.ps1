...
<#
.SYNOPSIS
    Common problem fix: Lenovo Vantage Battery Hint
.DESCRIPTION
...
.EXAMPLE
    .\Lenovo-Vantage-Battery-Hint.ps1
.EXAMPLE
    .\Lenovo-Vantage-Battery-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lenovo Vantage Battery Hint'; $totalSteps = 8; $Name = 'Lenovo-Vantage-Battery-Hint'
try {
    ...
    Export-HtmlReport -Title 'Lenovo Vantage Battery Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lenovo Vantage Battery Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
