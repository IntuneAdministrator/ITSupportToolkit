...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Auto-Repair.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Auto Repair'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
