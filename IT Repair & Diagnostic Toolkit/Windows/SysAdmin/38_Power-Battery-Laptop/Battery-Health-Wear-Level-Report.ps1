...
<#
.SYNOPSIS
    Common problem fix: Battery Health Wear Level Report
.DESCRIPTION
...
.EXAMPLE
    .\Battery-Health-Wear-Level-Report.ps1
.EXAMPLE
    .\Battery-Health-Wear-Level-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Battery Health Wear Level Report'; $totalSteps = 8; $Name = 'Battery-Health-Wear-Level-Report'
try {
    ...
    Export-HtmlReport -Title 'Battery Health Wear Level Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Battery Health Wear Level Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
