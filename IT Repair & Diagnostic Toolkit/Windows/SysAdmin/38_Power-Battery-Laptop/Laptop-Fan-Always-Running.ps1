...
<#
.SYNOPSIS
    Common problem fix: Laptop Fan Always Running
.DESCRIPTION
...
.EXAMPLE
    .\Laptop-Fan-Always-Running.ps1
.EXAMPLE
    .\Laptop-Fan-Always-Running.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Laptop Fan Always Running'; $totalSteps = 8; $Name = 'Laptop-Fan-Always-Running'
try {
    ...
    Export-HtmlReport -Title 'Laptop Fan Always Running' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Laptop Fan Always Running' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
