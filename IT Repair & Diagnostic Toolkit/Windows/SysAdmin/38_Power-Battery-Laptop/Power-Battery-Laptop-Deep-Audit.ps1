...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Deep-Audit.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Deep Audit'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
