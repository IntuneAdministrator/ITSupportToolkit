...
<#
.SYNOPSIS
    Enterprise IT tool: Modern Standby Sleep Study
.DESCRIPTION
...
.EXAMPLE
    .\Modern-Standby-Sleep-Study.ps1
.EXAMPLE
    .\Modern-Standby-Sleep-Study.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Modern Standby Sleep Study'; $totalSteps = 8; $Name = 'Modern-Standby-Sleep-Study'
try {
    ...
    Export-HtmlReport -Title 'Modern Standby Sleep Study' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Modern Standby Sleep Study' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
