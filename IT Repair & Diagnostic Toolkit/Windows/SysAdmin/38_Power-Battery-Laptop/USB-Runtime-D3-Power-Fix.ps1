...
<#
.SYNOPSIS
    Enterprise IT tool: USB Runtime D3 Power Fix
.DESCRIPTION
...
.EXAMPLE
    .\USB-Runtime-D3-Power-Fix.ps1
.EXAMPLE
    .\USB-Runtime-D3-Power-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Runtime D3 Power Fix'; $totalSteps = 8; $Name = 'USB-Runtime-D3-Power-Fix'
try {
    ...
    Export-HtmlReport -Title 'USB Runtime D3 Power Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Runtime D3 Power Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
