...
<#
.SYNOPSIS
    Common problem fix: USB C PD Charging Profile Audit
.DESCRIPTION
...
.EXAMPLE
    .\USB-C-PD-Charging-Profile-Audit.ps1
.EXAMPLE
    .\USB-C-PD-Charging-Profile-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB C PD Charging Profile Audit'; $totalSteps = 8; $Name = 'USB-C-PD-Charging-Profile-Audit'
try {
    ...
    Export-HtmlReport -Title 'USB C PD Charging Profile Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB C PD Charging Profile Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
