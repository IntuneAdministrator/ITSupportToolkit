...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Policy-Validate.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Policy Validate'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
