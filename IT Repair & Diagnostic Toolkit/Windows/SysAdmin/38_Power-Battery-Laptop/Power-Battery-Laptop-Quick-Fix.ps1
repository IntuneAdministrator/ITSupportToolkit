...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Quick-Fix.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Quick Fix'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
