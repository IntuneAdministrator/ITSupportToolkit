...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 38_Power-Battery-Laptop). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Power-Battery-Laptop-Evidence-Report.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Evidence Report'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '38_Power-Battery-Laptop' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '38_Power-Battery-Laptop'
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
