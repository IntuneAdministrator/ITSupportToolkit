...
<#
.SYNOPSIS
    Common problem fix: Power Battery Laptop Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Power-Battery-Laptop-Stack-Reset.ps1
.EXAMPLE
    .\Power-Battery-Laptop-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Power Battery Laptop Stack Reset'; $totalSteps = 8; $Name = 'Power-Battery-Laptop-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Power Battery Laptop Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Power Battery Laptop Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
