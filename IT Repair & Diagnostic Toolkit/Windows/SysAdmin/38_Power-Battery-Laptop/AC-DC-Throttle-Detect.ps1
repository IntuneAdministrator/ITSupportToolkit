...
<#
.SYNOPSIS
    Enterprise IT tool: AC DC Throttle Detect
.DESCRIPTION
...
.EXAMPLE
    .\AC-DC-Throttle-Detect.ps1
.EXAMPLE
    .\AC-DC-Throttle-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AC DC Throttle Detect'; $totalSteps = 8; $Name = 'AC-DC-Throttle-Detect'
try {
    ...
    Export-HtmlReport -Title 'AC DC Throttle Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AC DC Throttle Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
