...
<#
.SYNOPSIS
    Enterprise IT tool: Docking Station Power Audit
.DESCRIPTION
...
.EXAMPLE
    .\Docking-Station-Power-Audit.ps1
.EXAMPLE
    .\Docking-Station-Power-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Docking Station Power Audit'; $totalSteps = 8; $Name = 'Docking-Station-Power-Audit'
try {
    ...
    Export-HtmlReport -Title 'Docking Station Power Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Docking Station Power Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
