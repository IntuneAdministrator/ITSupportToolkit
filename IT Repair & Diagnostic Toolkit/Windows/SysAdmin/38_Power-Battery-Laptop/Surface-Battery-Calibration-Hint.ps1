...
<#
.SYNOPSIS
    Common problem fix: Surface Battery Calibration Hint
.DESCRIPTION
...
.EXAMPLE
    .\Surface-Battery-Calibration-Hint.ps1
.EXAMPLE
    .\Surface-Battery-Calibration-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Surface Battery Calibration Hint'; $totalSteps = 8; $Name = 'Surface-Battery-Calibration-Hint'
try {
    ...
    Export-HtmlReport -Title 'Surface Battery Calibration Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Surface Battery Calibration Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
