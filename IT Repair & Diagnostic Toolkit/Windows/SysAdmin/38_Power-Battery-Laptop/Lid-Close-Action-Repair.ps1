...
<#
.SYNOPSIS
    Enterprise IT tool: Lid Close Action Repair
.DESCRIPTION
...
.EXAMPLE
    .\Lid-Close-Action-Repair.ps1
.EXAMPLE
    .\Lid-Close-Action-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lid Close Action Repair'; $totalSteps = 8; $Name = 'Lid-Close-Action-Repair'
try {
    ...
    Export-HtmlReport -Title 'Lid Close Action Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lid Close Action Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
