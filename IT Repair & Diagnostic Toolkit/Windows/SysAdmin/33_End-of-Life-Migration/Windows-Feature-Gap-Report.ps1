...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Feature Gap Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Feature-Gap-Report.
...
.EXAMPLE
    .\Windows-Feature-Gap-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Feature-Gap-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Feature Gap Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Feature-Gap-Report'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Feature-Gap-Report' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'Windows Feature Gap Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Feature Gap Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
