...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: IE Mode Site List Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for IE-Mode-Site-List-Export.
...
.EXAMPLE
    .\IE-Mode-Site-List-Export.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\IE-Mode-Site-List-Export.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'IE Mode Site List Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'IE-Mode-Site-List-Export'
        Add-ReportRow -Check 'Tool' -Result 'IE-Mode-Site-List-Export' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'IE Mode Site List Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'IE Mode Site List Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
