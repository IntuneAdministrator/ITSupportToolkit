...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Migration Readiness Full Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Migration-Readiness-Full-Report.
...
.EXAMPLE
    .\Migration-Readiness-Full-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Migration-Readiness-Full-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Migration Readiness Full Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Migration-Readiness-Full-Report'
        Add-ReportRow -Check 'Tool' -Result 'Migration-Readiness-Full-Report' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'Migration Readiness Full Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Migration Readiness Full Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
