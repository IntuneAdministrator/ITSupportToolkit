...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows10 to 11 Readiness
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows10-to-11-Readiness.
...
.EXAMPLE
    .\Windows10-to-11-Readiness.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows10-to-11-Readiness.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows10 to 11 Readiness'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows10-to-11-Readiness'
        Add-ReportRow -Check 'Tool' -Result 'Windows10-to-11-Readiness' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'Windows10 to 11 Readiness' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows10 to 11 Readiness' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
