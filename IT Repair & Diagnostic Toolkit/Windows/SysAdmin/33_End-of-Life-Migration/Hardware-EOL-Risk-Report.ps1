...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Hardware EOL Risk Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Hardware-EOL-Risk-Report.
...
.EXAMPLE
    .\Hardware-EOL-Risk-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Hardware-EOL-Risk-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Hardware EOL Risk Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Hardware-EOL-Risk-Report'
        Add-ReportRow -Check 'Tool' -Result 'Hardware-EOL-Risk-Report' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'Hardware EOL Risk Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Hardware EOL Risk Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
