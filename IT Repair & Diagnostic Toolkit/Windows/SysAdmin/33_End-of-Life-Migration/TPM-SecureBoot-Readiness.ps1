...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: TPM SecureBoot Readiness
.DESCRIPTION
    Production-grade Windows SysAdmin tool for TPM-SecureBoot-Readiness.
...
.EXAMPLE
    .\TPM-SecureBoot-Readiness.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\TPM-SecureBoot-Readiness.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'TPM SecureBoot Readiness'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'TPM-SecureBoot-Readiness'
        Add-ReportRow -Check 'Tool' -Result 'TPM-SecureBoot-Readiness' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'TPM SecureBoot Readiness' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'TPM SecureBoot Readiness' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
