...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPO ADMX Compatibility Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPO-ADMX-Compatibility-Check.
...
.EXAMPLE
    .\GPO-ADMX-Compatibility-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\GPO-ADMX-Compatibility-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'GPO ADMX Compatibility Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'GPO-ADMX-Compatibility-Check'
        Add-ReportRow -Check 'Tool' -Result 'GPO-ADMX-Compatibility-Check' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'GPO ADMX Compatibility Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPO ADMX Compatibility Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
