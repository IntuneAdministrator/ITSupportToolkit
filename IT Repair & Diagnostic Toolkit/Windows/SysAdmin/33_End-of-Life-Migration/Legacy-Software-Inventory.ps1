...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Legacy Software Inventory
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Legacy-Software-Inventory.
...
.EXAMPLE
    .\Legacy-Software-Inventory.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Legacy-Software-Inventory.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Legacy Software Inventory'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Legacy-Software-Inventory'
        Add-ReportRow -Check 'Tool' -Result 'Legacy-Software-Inventory' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'Legacy Software Inventory' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Legacy Software Inventory' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
