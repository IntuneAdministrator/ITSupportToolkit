...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Driver Compatibility Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Driver-Compatibility-Report.
...
.EXAMPLE
    .\Driver-Compatibility-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Driver-Compatibility-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Driver Compatibility Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Driver-Compatibility-Report'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Compatibility-Report' -Status 'INFO' -Detail '33_End-of-Life-Migration'
    ...
    Export-HtmlReport -Title 'Driver Compatibility Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Compatibility Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
