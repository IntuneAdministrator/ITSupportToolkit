...
<#
.SYNOPSIS
    Enterprise IT tool: Wireless 8021X Profile Export
.DESCRIPTION
    Phase 10 expansion (SysAdmin/45_Network-Advanced-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Wireless-8021X-Profile-Export.ps1
.EXAMPLE
    .\Wireless-8021X-Profile-Export.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Wireless 8021X Profile Export'; $totalSteps=8; $Name='Wireless-8021X-Profile-Export'; $Folder='45_Network-Advanced-Enterprise'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '45_Network-Advanced-Enterprise' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'NETWORK ADVANCED ENTERPRISE'
        try {
            $doh = Get-DnsClientDohServerAddress -EA SilentlyContinue
            Add-ReportRow -Check 'DoH servers' -Result $(if($doh){($doh|% ServerAddress)-join', '}else{'None'}) -Status 'INFO' -Detail ''
        } catch { Add-ReportRow -Check 'DoH' -Result 'Cmdlet N/A' -Status 'WARN' -Detail $_.Exception.Message }
        Write-ProgressStep 2 $totalSteps $Activity 'DoH'
        $wpad = (Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' -EA SilentlyContinue).AutoConfigURL
        Add-ReportRow -Check 'PAC/AutoConfigURL' -Result $(if($wpad){$wpad}else{'Not set'}) -Status 'INFO' -Detail ''
        Get-NetAdapterHardwareInfo -EA SilentlyContinue | Select-Object -First 5 | ForEach-Object {
            Add-ReportRow -Check "NIC $($_.Name)" -Result "Slot/Bus info" -Status 'INFO' -Detail "$($_.Slot)/$($_.PciDevice)"
        }
        if ($Name -match 'TLS|Schannel') {
            $ev = @(Get-WinEvent -FilterHashtable @{LogName='System'; ProviderName='Schannel'; StartTime=(Get-Date).AddDays(-3)} -MaxEvents 15 -EA SilentlyContinue)
            Add-ReportRow -Check 'Schannel events' -Result "$($ev.Count)" -Status 'INFO' -Detail ''
        }
        if ($Name -match 'IPsec|QoS|RSS|RSC|8021X|Wireless') {
            Get-NetAdapterAdvancedProperty -EA SilentlyContinue | Where-Object { $_.DisplayName -match 'RSS|Recv|RSC|QoS' } | Select-Object -First 10 | ForEach-Object {
                Add-ReportRow -Check $_.DisplayName -Result $_.DisplayValue -Status 'INFO' -Detail $_.Name
            }
            if ($Name -match '8021X|Wireless') {
                netsh wlan show profiles 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'WLAN profiles' -Result 'Listed' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(300,$_.Length)) }
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Net advanced done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Wireless 8021X Profile Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Wireless 8021X Profile Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
