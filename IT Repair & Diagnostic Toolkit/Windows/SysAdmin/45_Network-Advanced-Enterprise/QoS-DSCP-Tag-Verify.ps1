...
$exitCode=0; $Activity='QoS DSCP Tag Verify'; $totalSteps=8; $Name='QoS-DSCP-Tag-Verify'; $Folder='45_Network-Advanced-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'QoS DSCP Tag Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'QoS DSCP Tag Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
