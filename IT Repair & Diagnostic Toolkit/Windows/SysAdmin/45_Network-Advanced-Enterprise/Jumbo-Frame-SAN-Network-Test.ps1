...
<#
.SYNOPSIS
    Common problem fix: Jumbo Frame SAN Network Test
.DESCRIPTION
...
.EXAMPLE
    .\Jumbo-Frame-SAN-Network-Test.ps1
.EXAMPLE
    .\Jumbo-Frame-SAN-Network-Test.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Jumbo Frame SAN Network Test'; $totalSteps = 8; $Name = 'Jumbo-Frame-SAN-Network-Test'
try {
    ...
    Export-HtmlReport -Title 'Jumbo Frame SAN Network Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Jumbo Frame SAN Network Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
