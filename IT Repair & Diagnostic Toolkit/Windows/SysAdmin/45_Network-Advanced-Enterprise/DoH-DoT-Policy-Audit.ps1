...
$exitCode=0; $Activity='DoH DoT Policy Audit'; $totalSteps=8; $Name='DoH-DoT-Policy-Audit'; $Folder='45_Network-Advanced-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'DoH DoT Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DoH DoT Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
