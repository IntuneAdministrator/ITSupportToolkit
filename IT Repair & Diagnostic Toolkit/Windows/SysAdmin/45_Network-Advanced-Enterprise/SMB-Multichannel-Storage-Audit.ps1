...
<#
.SYNOPSIS
    Common problem fix: SMB Multichannel Storage Audit
.DESCRIPTION
...
.EXAMPLE
    .\SMB-Multichannel-Storage-Audit.ps1
.EXAMPLE
    .\SMB-Multichannel-Storage-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SMB Multichannel Storage Audit'; $totalSteps = 8; $Name = 'SMB-Multichannel-Storage-Audit'
try {
    ...
    Export-HtmlReport -Title 'SMB Multichannel Storage Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SMB Multichannel Storage Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
