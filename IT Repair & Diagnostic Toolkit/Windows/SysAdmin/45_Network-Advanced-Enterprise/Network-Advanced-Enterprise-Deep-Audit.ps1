...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Network-Advanced-Enterprise-Deep-Audit.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Deep Audit'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
