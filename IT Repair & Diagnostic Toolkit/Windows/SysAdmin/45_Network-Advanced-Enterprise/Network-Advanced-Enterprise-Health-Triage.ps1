...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Network-Advanced-Enterprise-Health-Triage.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Health Triage'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
