...
$exitCode=0; $Activity='IPsec Policy Audit'; $totalSteps=8; $Name='IPsec-Policy-Audit'; $Folder='45_Network-Advanced-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'IPsec Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IPsec Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
