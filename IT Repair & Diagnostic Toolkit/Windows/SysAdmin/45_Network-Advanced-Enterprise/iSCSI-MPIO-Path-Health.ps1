...
<#
.SYNOPSIS
    Common problem fix: iSCSI MPIO Path Health
.DESCRIPTION
...
.EXAMPLE
    .\iSCSI-MPIO-Path-Health.ps1
.EXAMPLE
    .\iSCSI-MPIO-Path-Health.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'iSCSI MPIO Path Health'; $totalSteps = 8; $Name = 'iSCSI-MPIO-Path-Health'
try {
    ...
    Export-HtmlReport -Title 'iSCSI MPIO Path Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'iSCSI MPIO Path Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
