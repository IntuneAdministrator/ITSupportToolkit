...
$exitCode=0; $Activity='NIC RSS RSC Status'; $totalSteps=8; $Name='NIC-RSS-RSC-Status'; $Folder='45_Network-Advanced-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'NIC RSS RSC Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NIC RSS RSC Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
