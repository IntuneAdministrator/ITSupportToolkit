...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Network-Advanced-Enterprise-Quick-Fix.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Quick Fix'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
