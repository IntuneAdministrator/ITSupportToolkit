...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Network-Advanced-Enterprise-Stack-Reset.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Stack Reset'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
