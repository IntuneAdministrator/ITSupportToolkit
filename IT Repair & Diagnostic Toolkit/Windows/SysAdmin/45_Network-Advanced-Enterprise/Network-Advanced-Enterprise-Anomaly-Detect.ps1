...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Network-Advanced-Enterprise-Anomaly-Detect.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Anomaly Detect'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
