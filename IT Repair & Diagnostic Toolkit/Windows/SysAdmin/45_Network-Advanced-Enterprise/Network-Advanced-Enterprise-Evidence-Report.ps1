...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 45_Network-Advanced-Enterprise). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Network-Advanced-Enterprise-Evidence-Report.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Evidence Report'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '45_Network-Advanced-Enterprise' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '45_Network-Advanced-Enterprise'
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
