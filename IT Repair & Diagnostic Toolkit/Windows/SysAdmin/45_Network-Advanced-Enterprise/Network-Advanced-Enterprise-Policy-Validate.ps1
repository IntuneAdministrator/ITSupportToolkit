...
<#
.SYNOPSIS
    Common problem fix: Network Advanced Enterprise Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Network-Advanced-Enterprise-Policy-Validate.ps1
.EXAMPLE
    .\Network-Advanced-Enterprise-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Network Advanced Enterprise Policy Validate'; $totalSteps = 8; $Name = 'Network-Advanced-Enterprise-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Network Advanced Enterprise Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Network Advanced Enterprise Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
