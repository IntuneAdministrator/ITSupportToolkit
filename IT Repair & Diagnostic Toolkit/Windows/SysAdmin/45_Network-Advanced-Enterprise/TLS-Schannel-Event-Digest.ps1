...
<#
.SYNOPSIS
    Enterprise IT tool: TLS Schannel Event Digest
.DESCRIPTION
    Phase 10 expansion (SysAdmin/45_Network-Advanced-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\TLS-Schannel-Event-Digest.ps1
.EXAMPLE
    .\TLS-Schannel-Event-Digest.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='TLS Schannel Event Digest'; $totalSteps=8; $Name='TLS-Schannel-Event-Digest'; $Folder='45_Network-Advanced-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'TLS Schannel Event Digest' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'TLS Schannel Event Digest' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
