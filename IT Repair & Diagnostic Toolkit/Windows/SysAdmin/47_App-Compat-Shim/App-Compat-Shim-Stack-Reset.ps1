...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Stack-Reset.ps1
.EXAMPLE
    .\App-Compat-Shim-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Stack Reset'; $totalSteps = 8; $Name = 'App-Compat-Shim-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
