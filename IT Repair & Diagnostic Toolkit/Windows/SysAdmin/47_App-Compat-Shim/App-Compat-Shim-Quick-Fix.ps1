...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Quick-Fix.ps1
.EXAMPLE
    .\App-Compat-Shim-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Quick Fix'; $totalSteps = 8; $Name = 'App-Compat-Shim-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
