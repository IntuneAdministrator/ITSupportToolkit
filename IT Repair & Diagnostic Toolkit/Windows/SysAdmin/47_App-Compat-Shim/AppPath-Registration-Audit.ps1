...
<#
.SYNOPSIS
    Enterprise IT tool: AppPath Registration Audit
.DESCRIPTION
    Phase 10 expansion (SysAdmin/47_App-Compat-Shim). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\AppPath-Registration-Audit.ps1
.EXAMPLE
    .\AppPath-Registration-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='AppPath Registration Audit'; $totalSteps=8; $Name='AppPath-Registration-Audit'; $Folder='47_App-Compat-Shim'
try {
    ...
    Export-HtmlReport -Title 'AppPath Registration Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AppPath Registration Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
