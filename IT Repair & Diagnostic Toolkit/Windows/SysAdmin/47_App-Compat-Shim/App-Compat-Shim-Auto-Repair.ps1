...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Auto-Repair.ps1
.EXAMPLE
    .\App-Compat-Shim-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Auto Repair'; $totalSteps = 8; $Name = 'App-Compat-Shim-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
