...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Health-Triage.ps1
.EXAMPLE
    .\App-Compat-Shim-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Health Triage'; $totalSteps = 8; $Name = 'App-Compat-Shim-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
