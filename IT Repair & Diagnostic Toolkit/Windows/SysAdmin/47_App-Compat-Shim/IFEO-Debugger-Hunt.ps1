...
$exitCode=0; $Activity='IFEO Debugger Hunt'; $totalSteps=8; $Name='IFEO-Debugger-Hunt'; $Folder='47_App-Compat-Shim'
try {
    ...
    Export-HtmlReport -Title 'IFEO Debugger Hunt' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IFEO Debugger Hunt' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
