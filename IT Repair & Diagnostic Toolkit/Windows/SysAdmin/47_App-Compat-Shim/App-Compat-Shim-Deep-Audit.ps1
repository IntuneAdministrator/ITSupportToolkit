...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Deep-Audit.ps1
.EXAMPLE
    .\App-Compat-Shim-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Deep Audit'; $totalSteps = 8; $Name = 'App-Compat-Shim-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
