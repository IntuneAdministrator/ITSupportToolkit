...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 47_App-Compat-Shim). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\App-Compat-Shim-Policy-Validate.ps1
.EXAMPLE
    .\App-Compat-Shim-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Policy Validate'; $totalSteps = 8; $Name = 'App-Compat-Shim-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '47_App-Compat-Shim' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '47_App-Compat-Shim'
    ...
    Export-HtmlReport -Title 'App Compat Shim Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
