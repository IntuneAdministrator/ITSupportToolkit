...
$exitCode=0; $Activity='AppCompat Flag Audit'; $totalSteps=8; $Name='AppCompat-Flag-Audit'; $Folder='47_App-Compat-Shim'
try {
    ...
    Export-HtmlReport -Title 'AppCompat Flag Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AppCompat Flag Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
