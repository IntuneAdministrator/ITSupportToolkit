...
<#
.SYNOPSIS
    Enterprise IT tool: Program Compatibility Assistant Reset
.DESCRIPTION
    Phase 10 expansion (SysAdmin/47_App-Compat-Shim). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Program-Compatibility-Assistant-Reset.ps1
.EXAMPLE
    .\Program-Compatibility-Assistant-Reset.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Program Compatibility Assistant Reset'; $totalSteps=8; $Name='Program-Compatibility-Assistant-Reset'; $Folder='47_App-Compat-Shim'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '47_App-Compat-Shim' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'APP COMPAT / RUNTIME'
        $ifeo = Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options' -EA SilentlyContinue |
            Where-Object { (Get-ItemProperty $_.PSPath -EA SilentlyContinue).Debugger }
        Add-ReportRow -Check 'IFEO debuggers' -Result "$(@($ifeo).Count)" -Status $(if(@($ifeo).Count -eq 0){'PASS'}else{'WARN'}) -Detail (($ifeo|Select-Object -First 8 PSChildName)-join', ')
        Write-ProgressStep 2 $totalSteps $Activity 'IFEO'
        $ndp = Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP' -Recurse -EA SilentlyContinue |
            Get-ItemProperty -Name Version -EA SilentlyContinue | Where-Object Version | Select-Object -First 10
        foreach ($n in $ndp) { Add-ReportRow -Check '.NET' -Result $n.Version -Status 'INFO' -Detail $n.PSPath }
        if ($Name -match 'VCRedist|Runtime') {
            $vc = Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*,HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* -EA SilentlyContinue |
                Where-Object { $_.DisplayName -match 'Visual C\+\+' } | Select-Object -First 15
            foreach ($v in $vc) { Add-ReportRow -Check $v.DisplayName -Result $v.DisplayVersion -Status 'INFO' -Detail '' }
        }
        if ($Name -match 'Java') {
            $java = Get-Command java -EA SilentlyContinue
            Add-ReportRow -Check 'java.exe' -Result $(if($java){$java.Source}else{'Not in PATH'}) -Status 'INFO' -Detail ''
        }
        if ($Name -match 'PCA|Compatibility' -and $PSCmdlet.ShouldProcess('PCA','Restart PcaSvc')) {
            Restart-Service PcaSvc -Force -EA SilentlyContinue
            Add-ReportRow -Check 'PcaSvc' -Result (Get-Service PcaSvc -EA SilentlyContinue).Status -Status 'PASS' -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'AppCompat done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Program Compatibility Assistant Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Program Compatibility Assistant Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
