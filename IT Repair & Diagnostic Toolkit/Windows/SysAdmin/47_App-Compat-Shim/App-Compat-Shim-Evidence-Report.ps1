...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Evidence-Report.ps1
.EXAMPLE
    .\App-Compat-Shim-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Evidence Report'; $totalSteps = 8; $Name = 'App-Compat-Shim-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
