...
<#
.SYNOPSIS
    Common problem fix: App Compat Shim Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\App-Compat-Shim-Anomaly-Detect.ps1
.EXAMPLE
    .\App-Compat-Shim-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'App Compat Shim Anomaly Detect'; $totalSteps = 8; $Name = 'App-Compat-Shim-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'App Compat Shim Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'App Compat Shim Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
