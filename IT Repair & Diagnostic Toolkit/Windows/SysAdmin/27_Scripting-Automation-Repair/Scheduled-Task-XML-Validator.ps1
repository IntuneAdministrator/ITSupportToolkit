...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Scheduled Task XML Validator
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Scheduled-Task-XML-Validator.
...
.EXAMPLE
    .\Scheduled-Task-XML-Validator.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Scheduled-Task-XML-Validator.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Scheduled Task XML Validator'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Scheduled-Task-XML-Validator'
        Add-ReportRow -Check 'Tool' -Result 'Scheduled-Task-XML-Validator' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'Scheduled Task XML Validator' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Scheduled Task XML Validator' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
