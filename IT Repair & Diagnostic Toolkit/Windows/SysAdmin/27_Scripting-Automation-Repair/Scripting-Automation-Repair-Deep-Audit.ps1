...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Deep Audit'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
