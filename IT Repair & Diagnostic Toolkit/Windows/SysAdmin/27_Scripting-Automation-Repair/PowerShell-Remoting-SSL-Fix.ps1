...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PowerShell Remoting SSL Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PowerShell-Remoting-SSL-Fix.
...
.EXAMPLE
    .\PowerShell-Remoting-SSL-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PowerShell-Remoting-SSL-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PowerShell Remoting SSL Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PowerShell-Remoting-SSL-Fix'
        Add-ReportRow -Check 'Tool' -Result 'PowerShell-Remoting-SSL-Fix' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'PowerShell Remoting SSL Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PowerShell Remoting SSL Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
