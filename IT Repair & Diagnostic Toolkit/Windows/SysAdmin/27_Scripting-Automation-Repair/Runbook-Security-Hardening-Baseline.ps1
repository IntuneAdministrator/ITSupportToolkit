...
<#
.SYNOPSIS
    Common problem fix: Runbook Security Hardening Baseline
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 27_Scripting-Automation-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-Security-Hardening-Baseline.ps1
.EXAMPLE
    .\Runbook-Security-Hardening-Baseline.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Security Hardening Baseline'; $totalSteps = 8; $Name = 'Runbook-Security-Hardening-Baseline'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '27_Scripting-Automation-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '27_Scripting-Automation-Repair'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'Runbook Security Hardening Baseline' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Security Hardening Baseline' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
