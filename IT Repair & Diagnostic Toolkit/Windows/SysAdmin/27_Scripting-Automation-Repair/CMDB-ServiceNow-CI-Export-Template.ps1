...
<#
.SYNOPSIS
    Common problem fix: CMDB ServiceNow CI Export Template
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-ServiceNow-CI-Export-Template.ps1
.EXAMPLE
    .\CMDB-ServiceNow-CI-Export-Template.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB ServiceNow CI Export Template'; $totalSteps = 8; $Name = 'CMDB-ServiceNow-CI-Export-Template'
try {
    ...
    Export-HtmlReport -Title 'CMDB ServiceNow CI Export Template' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB ServiceNow CI Export Template' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
