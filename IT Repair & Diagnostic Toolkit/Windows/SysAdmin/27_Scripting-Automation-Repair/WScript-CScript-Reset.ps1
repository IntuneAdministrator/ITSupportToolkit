...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WScript CScript Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WScript-CScript-Reset.
...
$exitCode = 0
$Activity = 'WScript CScript Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WScript-CScript-Reset'
        Add-ReportRow -Check 'Tool' -Result 'WScript-CScript-Reset' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'WScript CScript Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WScript CScript Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
