...
<#
.SYNOPSIS
    Common problem fix: Runbook Backup VSS Health Check
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Backup-VSS-Health-Check.ps1
.EXAMPLE
    .\Runbook-Backup-VSS-Health-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Backup VSS Health Check'; $totalSteps = 8; $Name = 'Runbook-Backup-VSS-Health-Check'
try {
    ...
    Export-HtmlReport -Title 'Runbook Backup VSS Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Backup VSS Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
