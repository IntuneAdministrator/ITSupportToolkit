...
<#
.SYNOPSIS
    Common problem fix: Runbook Network Stack Full Repair
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Network-Stack-Full-Repair.ps1
.EXAMPLE
    .\Runbook-Network-Stack-Full-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Network Stack Full Repair'; $totalSteps = 8; $Name = 'Runbook-Network-Stack-Full-Repair'
try {
    ...
    Export-HtmlReport -Title 'Runbook Network Stack Full Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Network Stack Full Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
