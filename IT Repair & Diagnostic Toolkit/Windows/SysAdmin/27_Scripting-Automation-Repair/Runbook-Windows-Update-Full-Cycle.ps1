...
<#
.SYNOPSIS
    Common problem fix: Runbook Windows Update Full Cycle
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Windows-Update-Full-Cycle.ps1
.EXAMPLE
    .\Runbook-Windows-Update-Full-Cycle.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Windows Update Full Cycle'; $totalSteps = 8; $Name = 'Runbook-Windows-Update-Full-Cycle'
try {
    ...
    Export-HtmlReport -Title 'Runbook Windows Update Full Cycle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Windows Update Full Cycle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
