...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DSC Local Config Manager Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DSC-Local-Config-Manager-Repair.
...
.EXAMPLE
    .\DSC-Local-Config-Manager-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DSC-Local-Config-Manager-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DSC Local Config Manager Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'DSC-Local-Config-Manager-Repair'
        Add-ReportRow -Check 'Tool' -Result 'DSC-Local-Config-Manager-Repair' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'DSC Local Config Manager Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DSC Local Config Manager Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
