...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Automation Account Credential Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Automation-Account-Credential-Test.
Category: 27_Scripting-Automation-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Automation-Account-Credential-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Automation-Account-Credential-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Automation Account Credential Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SCRIPTING AUTOMATION REPAIR'
        $Name = 'Automation-Account-Credential-Test'
        Add-ReportRow -Check 'Tool' -Result 'Automation-Account-Credential-Test' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
        Write-SectionBanner 'SCRIPTING / AUTOMATION'
        Add-ReportRow -Check 'PS version' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        $ep = Get-ExecutionPolicy -List
        foreach ($e in $ep) {
            Add-ReportRow -Check "ExecutionPolicy $($e.Scope)" -Result "$($e.ExecutionPolicy)" -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 2 $totalSteps $Activity 'PS policy'

        if ($Name -match 'ExecutionPolicy' -and $PSCmdlet.ShouldProcess('ExecutionPolicy', 'Set LocalMachine RemoteSigned')) {
            Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope LocalMachine -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Set RemoteSigned' -Result (Get-ExecutionPolicy -Scope LocalMachine) -Status 'PASS' -Detail 'LocalMachine'
        }

        if ($Name -match 'PS-Module|NuGet|PSGallery') {
            try {
                Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force -ErrorAction SilentlyContinue | Out-Null
                Add-ReportRow -Check 'NuGet provider' -Result 'Ensured' -Status 'PASS' -Detail ''
            } catch {
                Add-ReportRow -Check 'NuGet provider' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message
            }
            if ($PSCmdlet.ShouldProcess('PSGallery', 'Set trusted')) {
                Set-PSRepository -Name PSGallery -InstallationPolicy Trusted -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'PSGallery' -Result 'Trusted' -Status 'PASS' -Detail ''
            }
        }

        if ($Name -match 'Task-Scheduler|Scheduled-Task') {
            if ($PSCmdlet.ShouldProcess('Schedule', 'Restart Task Scheduler')) {
                Restart-Service Schedule -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Schedule service' -Result (Get-Service Schedule).Status -Status 'PASS' -Detail ''
            }
            if ($Name -match 'XML') {
                $bad = @()
                Get-ScheduledTask -ErrorAction SilentlyContinue | Select-Object -First 30 | ForEach-Object {
                    try { Export-ScheduledTask -TaskName $_.TaskName -TaskPath $_.TaskPath | Out-Null } catch { $bad += $_.TaskName }
                }
                Add-ReportRow -Check 'Task XML export sample' -Result "$(30 - $bad.Count) OK / $($bad.Count) fail" -Status $(if ($bad.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ($bad -join ', ')
            }
        }

        if ($Name -match 'Remoting|DSC|Profile|COM|WScript|Signing|Credential|Version-Compatibility') {
            if ($Name -match 'Remoting' -and $PSCmdlet.ShouldProcess('WinRM', 'Enable PSRemoting')) {
                Enable-PSRemoting -Force -SkipNetworkProfileCheck -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'PSRemoting' -Result 'Enabled' -Status 'PASS' -Detail ''
            }
            if ($Name -match 'DSC') {
                try {
                    $lcm = Get-DscLocalConfigurationManager -ErrorAction Stop
                    Add-ReportRow -Check 'LCM RefreshMode' -Result $lcm.RefreshMode -Status 'INFO' -Detail "ConfigurationMode=$($lcm.ConfigurationMode)"
                } catch {
                    Add-ReportRow -Check 'DSC LCM' -Result 'N/A' -Status 'INFO' -Detail $_.Exception.Message
                }
            }
            if ($Name -match 'Profile') {
                ...
                foreach ($p in $profiles) {
                    if ($p -and (Test-Path $p)) {
                        Add-ReportRow -Check 'Profile present' -Result $p -Status 'INFO' -Detail ("{0} bytes" -f (Get-Item $p).Length)
                    }
                }
            }
            if ($Name -match 'WScript') {
                $wsh = 'HKLM:\SOFTWARE\Microsoft\Windows Script Host\Settings'
                Add-ReportRow -Check 'WSH settings' -Result $(if (Test-Path $wsh) {'Present'} else {'Default'}) -Status 'INFO' -Detail $wsh
            }
            if ($Name -match 'Signing') {
                $codeCerts = @(Get-ChildItem Cert:\CurrentUser\My -CodeSigningCert -ErrorAction SilentlyContinue)
                Add-ReportRow -Check 'Code signing certs (CU)' -Result "$($codeCerts.Count)" -Status $(if ($codeCerts.Count -gt 0) {'PASS'} else {'WARN'}) -Detail ''
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Scripting'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Automation Account Credential Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Automation Account Credential Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
