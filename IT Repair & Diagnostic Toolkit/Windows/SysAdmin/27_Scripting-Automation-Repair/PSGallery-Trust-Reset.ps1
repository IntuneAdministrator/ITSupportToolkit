...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PSGallery Trust Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PSGallery-Trust-Reset.
...
$exitCode = 0
$Activity = 'PSGallery Trust Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PSGallery-Trust-Reset'
        Add-ReportRow -Check 'Tool' -Result 'PSGallery-Trust-Reset' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'PSGallery Trust Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PSGallery Trust Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
