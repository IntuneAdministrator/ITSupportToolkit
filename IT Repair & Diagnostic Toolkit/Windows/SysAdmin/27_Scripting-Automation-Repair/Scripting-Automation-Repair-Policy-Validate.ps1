...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Policy Validate'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
