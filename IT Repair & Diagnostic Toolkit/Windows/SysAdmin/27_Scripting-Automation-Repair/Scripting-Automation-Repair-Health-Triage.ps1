...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Health-Triage.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Health Triage'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
