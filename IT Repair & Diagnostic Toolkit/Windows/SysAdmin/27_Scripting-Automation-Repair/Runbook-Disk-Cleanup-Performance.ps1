...
<#
.SYNOPSIS
    Common problem fix: Runbook Disk Cleanup Performance
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Disk-Cleanup-Performance.ps1
.EXAMPLE
    .\Runbook-Disk-Cleanup-Performance.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Disk Cleanup Performance'; $totalSteps = 8; $Name = 'Runbook-Disk-Cleanup-Performance'
try {
    ...
    Export-HtmlReport -Title 'Runbook Disk Cleanup Performance' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Disk Cleanup Performance' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
