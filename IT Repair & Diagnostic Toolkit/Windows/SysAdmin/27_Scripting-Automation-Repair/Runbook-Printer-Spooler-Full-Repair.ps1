...
<#
.SYNOPSIS
    Common problem fix: Runbook Printer Spooler Full Repair
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Printer-Spooler-Full-Repair.ps1
.EXAMPLE
    .\Runbook-Printer-Spooler-Full-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Printer Spooler Full Repair'; $totalSteps = 8; $Name = 'Runbook-Printer-Spooler-Full-Repair'
try {
    ...
    Export-HtmlReport -Title 'Runbook Printer Spooler Full Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Printer Spooler Full Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
