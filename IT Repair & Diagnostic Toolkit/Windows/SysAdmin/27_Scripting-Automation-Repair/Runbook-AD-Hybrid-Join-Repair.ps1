...
<#
.SYNOPSIS
    Common problem fix: Runbook AD Hybrid Join Repair
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-AD-Hybrid-Join-Repair.ps1
.EXAMPLE
    .\Runbook-AD-Hybrid-Join-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook AD Hybrid Join Repair'; $totalSteps = 8; $Name = 'Runbook-AD-Hybrid-Join-Repair'
try {
    ...
    Export-HtmlReport -Title 'Runbook AD Hybrid Join Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook AD Hybrid Join Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
