...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
