...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PS Version Compatibility Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PS-Version-Compatibility-Check.
...
.EXAMPLE
    .\PS-Version-Compatibility-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PS-Version-Compatibility-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PS Version Compatibility Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PS-Version-Compatibility-Check'
        Add-ReportRow -Check 'Tool' -Result 'PS-Version-Compatibility-Check' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'PS Version Compatibility Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PS Version Compatibility Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
