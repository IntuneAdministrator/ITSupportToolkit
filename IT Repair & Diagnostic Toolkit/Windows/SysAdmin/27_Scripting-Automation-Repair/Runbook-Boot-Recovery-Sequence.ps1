...
<#
.SYNOPSIS
    Common problem fix: Runbook Boot Recovery Sequence
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Boot-Recovery-Sequence.ps1
.EXAMPLE
    .\Runbook-Boot-Recovery-Sequence.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Boot Recovery Sequence'; $totalSteps = 8; $Name = 'Runbook-Boot-Recovery-Sequence'
try {
    ...
    Export-HtmlReport -Title 'Runbook Boot Recovery Sequence' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Boot Recovery Sequence' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
