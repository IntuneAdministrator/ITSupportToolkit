...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PowerShell ExecutionPolicy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PowerShell-ExecutionPolicy-Fix.
...
.EXAMPLE
    .\PowerShell-ExecutionPolicy-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PowerShell-ExecutionPolicy-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PowerShell ExecutionPolicy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PowerShell-ExecutionPolicy-Fix'
        Add-ReportRow -Check 'Tool' -Result 'PowerShell-ExecutionPolicy-Fix' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'PowerShell ExecutionPolicy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PowerShell ExecutionPolicy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
