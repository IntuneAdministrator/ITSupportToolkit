...
<#
.SYNOPSIS
    Common problem fix: CMDB SCCM Hardware Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-SCCM-Hardware-Export.ps1
.EXAMPLE
    .\CMDB-SCCM-Hardware-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB SCCM Hardware Export'; $totalSteps = 8; $Name = 'CMDB-SCCM-Hardware-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB SCCM Hardware Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB SCCM Hardware Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
