...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Quick Fix'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
