...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Auto Repair'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
