...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Script Signing Certificate Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Script-Signing-Certificate-Repair.
...
.EXAMPLE
    .\Script-Signing-Certificate-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Script-Signing-Certificate-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Script Signing Certificate Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Script-Signing-Certificate-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Script-Signing-Certificate-Repair' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'Script Signing Certificate Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Script Signing Certificate Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
