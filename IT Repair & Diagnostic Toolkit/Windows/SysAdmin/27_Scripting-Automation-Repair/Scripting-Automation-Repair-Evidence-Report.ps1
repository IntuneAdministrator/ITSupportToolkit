...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Evidence Report'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
