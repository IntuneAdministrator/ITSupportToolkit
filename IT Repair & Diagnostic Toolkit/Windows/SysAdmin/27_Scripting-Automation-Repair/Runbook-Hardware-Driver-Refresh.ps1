...
<#
.SYNOPSIS
    Common problem fix: Runbook Hardware Driver Refresh
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Hardware-Driver-Refresh.ps1
.EXAMPLE
    .\Runbook-Hardware-Driver-Refresh.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Hardware Driver Refresh'; $totalSteps = 8; $Name = 'Runbook-Hardware-Driver-Refresh'
try {
    ...
    Export-HtmlReport -Title 'Runbook Hardware Driver Refresh' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Hardware Driver Refresh' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
