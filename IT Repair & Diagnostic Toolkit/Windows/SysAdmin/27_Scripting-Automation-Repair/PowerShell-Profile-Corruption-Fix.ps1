...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PowerShell Profile Corruption Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PowerShell-Profile-Corruption-Fix.
...
.EXAMPLE
    .\PowerShell-Profile-Corruption-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PowerShell-Profile-Corruption-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PowerShell Profile Corruption Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PowerShell-Profile-Corruption-Fix'
        Add-ReportRow -Check 'Tool' -Result 'PowerShell-Profile-Corruption-Fix' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'PowerShell Profile Corruption Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PowerShell Profile Corruption Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
