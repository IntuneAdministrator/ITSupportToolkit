...
<#
.SYNOPSIS
    Common problem fix: CMDB SnipeIT Import Format Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-SnipeIT-Import-Format-Export.ps1
.EXAMPLE
    .\CMDB-SnipeIT-Import-Format-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB SnipeIT Import Format Export'; $totalSteps = 8; $Name = 'CMDB-SnipeIT-Import-Format-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB SnipeIT Import Format Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB SnipeIT Import Format Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
