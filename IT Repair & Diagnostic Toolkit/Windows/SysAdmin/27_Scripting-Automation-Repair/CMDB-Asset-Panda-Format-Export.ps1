...
<#
.SYNOPSIS
    Common problem fix: CMDB Asset Panda Format Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Asset-Panda-Format-Export.ps1
.EXAMPLE
    .\CMDB-Asset-Panda-Format-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Asset Panda Format Export'; $totalSteps = 8; $Name = 'CMDB-Asset-Panda-Format-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB Asset Panda Format Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Asset Panda Format Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
