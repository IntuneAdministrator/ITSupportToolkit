...
<#
.SYNOPSIS
    Common problem fix: Scripting Automation Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Scripting-Automation-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Scripting-Automation-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Scripting Automation Repair Stack Reset'; $totalSteps = 8; $Name = 'Scripting-Automation-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Scripting Automation Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Scripting Automation Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
