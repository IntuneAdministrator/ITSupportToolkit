...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NuGet Provider Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NuGet-Provider-Repair.
...
$exitCode = 0
$Activity = 'NuGet Provider Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'NuGet-Provider-Repair'
        Add-ReportRow -Check 'Tool' -Result 'NuGet-Provider-Repair' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'NuGet Provider Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NuGet Provider Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
