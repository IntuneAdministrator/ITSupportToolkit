...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: COM Object Registration Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for COM-Object-Registration-Repair.
...
.EXAMPLE
    .\COM-Object-Registration-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\COM-Object-Registration-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'COM Object Registration Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'COM-Object-Registration-Repair'
        Add-ReportRow -Check 'Tool' -Result 'COM-Object-Registration-Repair' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'COM Object Registration Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'COM Object Registration Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
