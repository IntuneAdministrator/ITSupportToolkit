...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Task Scheduler Service Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Task-Scheduler-Service-Reset.
...
.EXAMPLE
    .\Task-Scheduler-Service-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Task-Scheduler-Service-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Task Scheduler Service Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Task-Scheduler-Service-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Task-Scheduler-Service-Reset' -Status 'INFO' -Detail '27_Scripting-Automation-Repair'
    ...
    Export-HtmlReport -Title 'Task Scheduler Service Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Task Scheduler Service Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
