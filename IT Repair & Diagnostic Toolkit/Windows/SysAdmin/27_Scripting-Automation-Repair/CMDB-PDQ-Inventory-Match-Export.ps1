...
<#
.SYNOPSIS
    Common problem fix: CMDB PDQ Inventory Match Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-PDQ-Inventory-Match-Export.ps1
.EXAMPLE
    .\CMDB-PDQ-Inventory-Match-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB PDQ Inventory Match Export'; $totalSteps = 8; $Name = 'CMDB-PDQ-Inventory-Match-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB PDQ Inventory Match Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB PDQ Inventory Match Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
