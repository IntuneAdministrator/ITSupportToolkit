...
<#
.SYNOPSIS
    Common problem fix: CMDB Lansweeper Format Export
.DESCRIPTION
...
.EXAMPLE
    .\CMDB-Lansweeper-Format-Export.ps1
.EXAMPLE
    .\CMDB-Lansweeper-Format-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CMDB Lansweeper Format Export'; $totalSteps = 8; $Name = 'CMDB-Lansweeper-Format-Export'
try {
    ...
    Export-HtmlReport -Title 'CMDB Lansweeper Format Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CMDB Lansweeper Format Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
