...
<#
.SYNOPSIS
    Common problem fix: M365 Teams SharePoint Admin Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 59_M365-Teams-SharePoint-Admin). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\M365-Teams-SharePoint-Admin-Policy-Validate.ps1
.EXAMPLE
    .\M365-Teams-SharePoint-Admin-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'M365 Teams SharePoint Admin Policy Validate'; $totalSteps = 8; $Name = 'M365-Teams-SharePoint-Admin-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '59_M365-Teams-SharePoint-Admin' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '59_M365-Teams-SharePoint-Admin'
    ...
    Export-HtmlReport -Title 'M365 Teams SharePoint Admin Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'M365 Teams SharePoint Admin Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
