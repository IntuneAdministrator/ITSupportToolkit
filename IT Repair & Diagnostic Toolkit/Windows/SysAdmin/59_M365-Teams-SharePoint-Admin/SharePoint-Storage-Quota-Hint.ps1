...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: SharePoint Storage Quota Hint
.DESCRIPTION
    Phase 12 M365 tenant admin (SysAdmin/59_M365-Teams-SharePoint-Admin). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\SharePoint-Storage-Quota-Hint.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\SharePoint-Storage-Quota-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SharePoint Storage Quota Hint'; $totalSteps = 8; $Name = 'SharePoint-Storage-Quota-Hint'; $Folder = '59_M365-Teams-SharePoint-Admin'
...
try {
    ...
    Add-ReportRow -Check 'Module' -Result '59_M365-Teams-SharePoint-Admin' -Status 'INFO' -Detail 'SysAdmin Phase12 M365'
    Add-M365PortalRows
    ...
    if ($UPN) { Add-ReportRow -Check 'Target UPN' -Result $UPN -Status 'INFO' -Detail '' }
        ...
        Write-SectionBanner 'TEAMS / SHAREPOINT ADMIN'
        Write-ProgressStep 3 $totalSteps $Activity 'Teams/SPO'
        Add-ReportRow -Check 'Teams Admin Center' -Result 'https://admin.teams.microsoft.com' -Status 'INFO' -Detail 'Policies, meetings, guest, devices'
        Add-ReportRow -Check 'SPO Admin' -Result 'https://admin.microsoft.com/#/sharepoint' -Status 'INFO' -Detail 'Sites, sharing, storage'
        if ($connected -and $UPN -and $Name -match 'Teams-User-Policy|Meeting-Policy|Guest') {
            try {
                # Teams PowerShell is separate; Graph for some settings
                Add-ReportRow -Check 'Teams PowerShell' -Result 'Install-Module MicrosoftTeams' -Status 'INFO' -Detail 'Connect-MicrosoftTeams; Get-CsOnlineUser / Get-CsUserPolicyAssignment'
                $u = Get-MgUser -UserId $UPN -Property displayName,userPrincipalName -EA SilentlyContinue
                if ($u) { Add-ReportRow -Check 'User exists' -Result $u.DisplayName -Status 'PASS' -Detail 'Confirm Teams license + policy in TAC' }
            } catch { Add-ReportRow -Check 'Teams user' -Result 'Check TAC' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($Name -match 'OneDrive-Admin-URL' -and $UPN) {
            $adminHint = ($UPN -replace '@','-') 
            Add-ReportRow -Check 'OneDrive path hint' -Result "https://TENANT-my.sharepoint.com/personal/UPN_underscored" -Status 'INFO' -Detail "UPN=$UPN - resolve tenant from Get-MgOrganization"
            if ($connected) {
                try {
                    $org = Get-MgOrganization -EA SilentlyContinue | Select-Object -First 1
                    Add-ReportRow -Check 'Tenant' -Result $org.DisplayName -Status 'PASS' -Detail (($org.VerifiedDomains | Where-Object IsDefault).Name)
                } catch {}
            }
        }
        if ($Name -match 'SPO-Site|External-Sharing|Storage-Quota') {
            Add-ReportRow -Check 'PnP / SPO' -Result 'Install-Module PnP.PowerShell (modern auth)' -Status 'INFO' -Detail 'Or use SharePoint Admin Center'
        }
        if ($Name -match 'Devices-Rooms') {
            Add-ReportRow -Check 'Teams Rooms' -Result 'TAC > Devices / Rooms' -Status 'INFO' -Detail 'Also collect local Teams Rooms logs via ITSupport rooms scripts'
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Teams/SPO done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'SharePoint Storage Quota Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SharePoint Storage Quota Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
