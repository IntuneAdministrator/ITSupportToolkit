...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Teams Guest Access Status
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Guest-Access-Status.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Teams-Guest-Access-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Guest Access Status'; $totalSteps = 8; $Name = 'Teams-Guest-Access-Status'; $Folder = '59_M365-Teams-SharePoint-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Teams Guest Access Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Guest Access Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
