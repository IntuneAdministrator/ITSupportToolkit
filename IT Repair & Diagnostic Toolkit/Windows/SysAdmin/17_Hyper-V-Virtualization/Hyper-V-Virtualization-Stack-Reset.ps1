...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtualization-Stack-Reset.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Stack Reset'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
