...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: HyperV Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for HyperV-Health-Check.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'HyperV-Health-Check'
        Add-ReportRow -Check 'Tool' -Result 'HyperV-Health-Check' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'HyperV Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'HyperV Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
