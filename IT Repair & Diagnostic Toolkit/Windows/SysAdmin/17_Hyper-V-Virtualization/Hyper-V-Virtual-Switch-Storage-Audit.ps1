...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtual Switch Storage Audit
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtual-Switch-Storage-Audit.ps1
.EXAMPLE
    .\Hyper-V-Virtual-Switch-Storage-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtual Switch Storage Audit'; $totalSteps = 8; $Name = 'Hyper-V-Virtual-Switch-Storage-Audit'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtual Switch Storage Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtual Switch Storage Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
