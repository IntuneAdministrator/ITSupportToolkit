...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtualization-Anomaly-Detect.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Anomaly Detect'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
