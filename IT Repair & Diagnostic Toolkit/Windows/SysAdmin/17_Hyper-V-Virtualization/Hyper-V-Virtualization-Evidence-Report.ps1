...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtualization-Evidence-Report.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Evidence Report'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
