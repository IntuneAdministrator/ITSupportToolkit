...
<#
.SYNOPSIS
    Common problem fix: Hyper V Cluster Shared Volume Check
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Cluster-Shared-Volume-Check.ps1
.EXAMPLE
    .\Hyper-V-Cluster-Shared-Volume-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Cluster Shared Volume Check'; $totalSteps = 8; $Name = 'Hyper-V-Cluster-Shared-Volume-Check'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Cluster Shared Volume Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Cluster Shared Volume Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
