...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 17_Hyper-V-Virtualization). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Hyper-V-Virtualization-Policy-Validate.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Policy Validate'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '17_Hyper-V-Virtualization' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
