...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: HyperV Event Log Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for HyperV-Event-Log-Analyzer.
...
.EXAMPLE
    .\HyperV-Event-Log-Analyzer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\HyperV-Event-Log-Analyzer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'HyperV Event Log Analyzer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'HyperV-Event-Log-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'HyperV-Event-Log-Analyzer' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'HyperV Event Log Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'HyperV Event Log Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
