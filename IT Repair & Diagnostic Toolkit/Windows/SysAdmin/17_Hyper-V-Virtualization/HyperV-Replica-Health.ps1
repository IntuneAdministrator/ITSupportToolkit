...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: HyperV Replica Health
.DESCRIPTION
    Production-grade Windows SysAdmin tool for HyperV-Replica-Health.
...
$exitCode = 0
$Activity = 'HyperV Replica Health'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'HyperV-Replica-Health'
        Add-ReportRow -Check 'Tool' -Result 'HyperV-Replica-Health' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'HyperV Replica Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'HyperV Replica Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
