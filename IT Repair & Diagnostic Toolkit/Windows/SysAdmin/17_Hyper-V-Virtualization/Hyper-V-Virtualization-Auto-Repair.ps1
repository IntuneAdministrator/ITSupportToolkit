...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtualization-Auto-Repair.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Auto Repair'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
