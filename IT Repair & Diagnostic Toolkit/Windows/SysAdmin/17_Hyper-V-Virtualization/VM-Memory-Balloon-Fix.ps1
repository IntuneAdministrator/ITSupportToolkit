...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VM Memory Balloon Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VM-Memory-Balloon-Fix.
...
$exitCode = 0
$Activity = 'VM Memory Balloon Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'VM-Memory-Balloon-Fix'
        Add-ReportRow -Check 'Tool' -Result 'VM-Memory-Balloon-Fix' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'VM Memory Balloon Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VM Memory Balloon Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
