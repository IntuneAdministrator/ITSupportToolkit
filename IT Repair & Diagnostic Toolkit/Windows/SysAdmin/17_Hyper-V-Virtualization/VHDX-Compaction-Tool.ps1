...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VHDX Compaction Tool
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VHDX-Compaction-Tool.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'VHDX-Compaction-Tool'
        Add-ReportRow -Check 'Tool' -Result 'VHDX-Compaction-Tool' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'VHDX Compaction Tool' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VHDX Compaction Tool' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
