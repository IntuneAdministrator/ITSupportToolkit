...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VM Export Integrity Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VM-Export-Integrity-Check.
...
.EXAMPLE
    .\VM-Export-Integrity-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\VM-Export-Integrity-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'VM Export Integrity Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'VM-Export-Integrity-Check'
        Add-ReportRow -Check 'Tool' -Result 'VM-Export-Integrity-Check' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'VM Export Integrity Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VM Export Integrity Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
