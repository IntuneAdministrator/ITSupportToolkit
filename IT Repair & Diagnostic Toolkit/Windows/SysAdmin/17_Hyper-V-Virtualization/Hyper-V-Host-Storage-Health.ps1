...
<#
.SYNOPSIS
    Common problem fix: Hyper V Host Storage Health
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Host-Storage-Health.ps1
.EXAMPLE
    .\Hyper-V-Host-Storage-Health.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Host Storage Health'; $totalSteps = 8; $Name = 'Hyper-V-Host-Storage-Health'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Host Storage Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Host Storage Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
