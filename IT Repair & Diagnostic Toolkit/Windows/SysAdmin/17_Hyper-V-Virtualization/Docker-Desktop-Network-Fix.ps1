...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Docker Desktop Network Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Docker-Desktop-Network-Fix.
...
.EXAMPLE
    .\Docker-Desktop-Network-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Docker-Desktop-Network-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Docker Desktop Network Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Docker-Desktop-Network-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Docker-Desktop-Network-Fix' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
        Write-SectionBanner 'WSL / DOCKER NETWORK'
        $wsl = Get-Command wsl.exe -ErrorAction SilentlyContinue
        if ($wsl) {
            $list = & wsl.exe -l -v 2>&1 | Out-String
            Add-ReportRow -Check 'WSL distros' -Result 'Listed' -Status 'PASS' -Detail ($list.Substring(0, [Math]::Min(400, $list.Length)))
            if ($PSCmdlet.ShouldProcess('WSL', 'wsl --shutdown')) {
                & wsl.exe --shutdown 2>&1 | Out-Null
                Add-ReportRow -Check 'WSL shutdown' -Result 'Done' -Status 'PASS' -Detail 'Restarts on next wsl launch'
            }
        } else {
            Add-ReportRow -Check 'WSL' -Result 'Not installed' -Status 'WARN' -Detail ''
        }
        if ($Name -match 'Docker') {
            $docker = Get-Service com.docker.service -ErrorAction SilentlyContinue
            if ($docker) {
                Add-ReportRow -Check 'Docker service' -Result $docker.Status -Status 'INFO' -Detail ''
                if ($PSCmdlet.ShouldProcess('Docker', 'Restart com.docker.service')) {
                    Restart-Service com.docker.service -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Docker restart' -Result 'Done' -Status 'PASS' -Detail ''
                }
            } else {
                Add-ReportRow -Check 'Docker service' -Result 'Not found' -Status 'WARN' -Detail ''
            }
        }
        if ($Name -match 'Network' -and $PSCmdlet.ShouldProcess('HNS', 'Restart hns + Hx* networking')) {
            Restart-Service hns -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'HNS restart' -Result 'Attempted' -Status 'PASS' -Detail 'Host Network Service'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'WSL/Docker'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Docker Desktop Network Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Docker Desktop Network Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
