...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtualization-Quick-Fix.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Quick Fix'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
