...
<#
.SYNOPSIS
    Common problem fix: Hyper V Virtualization Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Virtualization-Deep-Audit.ps1
.EXAMPLE
    .\Hyper-V-Virtualization-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Virtualization Deep Audit'; $totalSteps = 8; $Name = 'Hyper-V-Virtualization-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Virtualization Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Virtualization Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
