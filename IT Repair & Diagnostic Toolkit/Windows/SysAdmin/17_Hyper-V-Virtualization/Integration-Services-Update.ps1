...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Integration Services Update
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Integration-Services-Update.
...
.EXAMPLE
    .\Integration-Services-Update.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Integration-Services-Update.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Integration Services Update'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Integration-Services-Update'
        Add-ReportRow -Check 'Tool' -Result 'Integration-Services-Update' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'Integration Services Update' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Integration Services Update' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
