...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Nested Virtualisation Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Nested-Virtualisation-Enable.
Category: 17_Hyper-V-Virtualization. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Nested-Virtualisation-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Nested-Virtualisation-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Nested Virtualisation Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'HYPER V VIRTUALIZATION'
        $Name = 'Nested-Virtualisation-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Nested-Virtualisation-Enable' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
        Write-SectionBanner 'HYPER-V'
        $hv = Get-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V-All -ErrorAction SilentlyContinue
        if (-not $hv) { $hv = Get-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V -ErrorAction SilentlyContinue }
        Add-ReportRow -Check 'Hyper-V feature' -Result $(if ($hv) { $hv.State } else {'Unknown'}) -Status $(if ($hv -and $hv.State -eq 'Enabled') {'PASS'} else {'WARN'}) -Detail ''

        try {
            Import-Module Hyper-V -ErrorAction Stop
            $vms = @(Get-VM -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'VM count' -Result "$($vms.Count)" -Status 'INFO' -Detail ''
            foreach ($vm in $vms) {
                Add-ReportRow -Check "VM $($vm.Name)" -Result $vm.State -Status $(if ($vm.State -eq 'Running') {'PASS'} else {'INFO'}) -Detail "Gen=$($vm.Generation) CPU=$($vm.ProcessorCount) MemMB=$($vm.MemoryAssigned/1MB)"
            }
            if ($Name -match 'Virtual-Switch') {
                Get-VMSwitch | ForEach-Object {
                    Add-ReportRow -Check "Switch $($_.Name)" -Result $_.SwitchType -Status 'INFO' -Detail $_.NetAdapterInterfaceDescription
                }
            }
            if ($Name -match 'Checkpoint|Snapshot' -and $PSCmdlet.ShouldProcess('Checkpoints', 'Report and optionally remove collapsed checkpoints')) {
                foreach ($vm in $vms) {
                    $cps = @(Get-VMCheckpoint -VMName $vm.Name -ErrorAction SilentlyContinue)
                    Add-ReportRow -Check "Checkpoints $($vm.Name)" -Result "$($cps.Count)" -Status $(if ($cps.Count -gt 5) {'WARN'} else {'INFO'}) -Detail (($cps | ForEach-Object Name) -join '; ')
                }
            }
            if ($Name -match 'Nested' -and $PSCmdlet.ShouldProcess('Nested virt', 'Enable on first VM if present')) {
                $target = $vms | Select-Object -First 1
                if ($target) {
                    Set-VMProcessor -VMName $target.Name -ExposeVirtualizationExtensions $true -ErrorAction SilentlyContinue
                    Add-ReportRow -Check "Nested virt $($target.Name)" -Result 'Enabled' -Status 'PASS' -Detail 'VM must be Off'
                }
            }
        } catch {
            Add-ReportRow -Check 'Hyper-V module' -Result 'Not available' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Hyper-V'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Nested Virtualisation Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Nested Virtualisation Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
