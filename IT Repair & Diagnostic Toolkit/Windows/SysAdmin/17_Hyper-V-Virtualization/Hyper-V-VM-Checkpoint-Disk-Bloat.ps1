...
<#
.SYNOPSIS
    Common problem fix: Hyper V VM Checkpoint Disk Bloat
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-VM-Checkpoint-Disk-Bloat.ps1
.EXAMPLE
    .\Hyper-V-VM-Checkpoint-Disk-Bloat.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V VM Checkpoint Disk Bloat'; $totalSteps = 8; $Name = 'Hyper-V-VM-Checkpoint-Disk-Bloat'
try {
    ...
    Export-HtmlReport -Title 'Hyper V VM Checkpoint Disk Bloat' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V VM Checkpoint Disk Bloat' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
