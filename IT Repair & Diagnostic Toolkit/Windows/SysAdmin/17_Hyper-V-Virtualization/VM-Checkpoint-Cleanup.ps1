...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VM Checkpoint Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VM-Checkpoint-Cleanup.
...
$exitCode = 0
$Activity = 'VM Checkpoint Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'VM-Checkpoint-Cleanup'
        Add-ReportRow -Check 'Tool' -Result 'VM-Checkpoint-Cleanup' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'VM Checkpoint Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VM Checkpoint Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
