...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VM Snapshot Orphan Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VM-Snapshot-Orphan-Cleanup.
...
.EXAMPLE
    .\VM-Snapshot-Orphan-Cleanup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\VM-Snapshot-Orphan-Cleanup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'VM Snapshot Orphan Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'VM-Snapshot-Orphan-Cleanup'
        Add-ReportRow -Check 'Tool' -Result 'VM-Snapshot-Orphan-Cleanup' -Status 'INFO' -Detail '17_Hyper-V-Virtualization'
    ...
    Export-HtmlReport -Title 'VM Snapshot Orphan Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VM Snapshot Orphan Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
