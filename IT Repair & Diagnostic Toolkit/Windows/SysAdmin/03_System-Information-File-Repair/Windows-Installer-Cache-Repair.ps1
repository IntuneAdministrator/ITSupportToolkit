...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Installer Cache Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Installer-Cache-Repair.
...
.EXAMPLE
    .\Windows-Installer-Cache-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Installer-Cache-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Installer Cache Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Windows-Installer-Cache-Repair'
    ...
    Export-HtmlReport -Title 'Windows Installer Cache Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Installer Cache Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
