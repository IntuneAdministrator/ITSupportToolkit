...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Junction Symlink Auditor
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Junction-Symlink-Auditor.
...
.EXAMPLE
    .\Junction-Symlink-Auditor.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Junction-Symlink-Auditor.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Junction Symlink Auditor'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Junction-Symlink-Auditor'
    ...
    Export-HtmlReport -Title 'Junction Symlink Auditor' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Junction Symlink Auditor' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
