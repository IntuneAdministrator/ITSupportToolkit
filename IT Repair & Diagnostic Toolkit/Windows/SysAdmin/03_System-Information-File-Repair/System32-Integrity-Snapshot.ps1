...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System32 Integrity Snapshot
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System32-Integrity-Snapshot.
...
.EXAMPLE
    .\System32-Integrity-Snapshot.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\System32-Integrity-Snapshot.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'System32 Integrity Snapshot'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — System32-Integrity-Snapshot'
    ...
    Export-HtmlReport -Title 'System32 Integrity Snapshot' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System32 Integrity Snapshot' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
