...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Hosts File Integrity Restore
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Hosts-File-Integrity-Restore.
...
.EXAMPLE
    .\Hosts-File-Integrity-Restore.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Hosts-File-Integrity-Restore.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Hosts File Integrity Restore'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Hosts-File-Integrity-Restore'
        ...
        $backupDir = 'C:\IT-Logs\Backups'
        New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
        Copy-Item $hosts (Join-Path $backupDir "hosts_$(Get-Date -Format 'yyyyMMdd_HHmmss').bak") -Force
        $content = Get-Content $hosts -ErrorAction SilentlyContinue
        $suspicious = @($content | Where-Object { $_ -match '^\s*\d' -and $_ -notmatch '^\s*#' -and $_ -notmatch 'localhost|broadcasthost' })
        Add-ReportRow -Check 'Non-default hosts entries' -Result "$($suspicious.Count)" -Status $(if ($suspicious.Count -eq 0) {'PASS'} else {'WARN'}) -Detail (($suspicious | Select-Object -First 10) -join ' | ')
        Write-ProgressStep 3 $totalSteps $Activity 'Hosts audit'
    ...
    Export-HtmlReport -Title 'Hosts File Integrity Restore' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Hosts File Integrity Restore' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
