...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System File Permissions Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System-File-Permissions-Reset.
...
.EXAMPLE
    .\System-File-Permissions-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\System-File-Permissions-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'System File Permissions Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — System-File-Permissions-Reset'
    ...
    Export-HtmlReport -Title 'System File Permissions Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System File Permissions Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
