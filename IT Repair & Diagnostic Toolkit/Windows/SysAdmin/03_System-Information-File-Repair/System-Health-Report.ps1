...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System Health Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System-Health-Report.
...
.EXAMPLE
    .\System-Health-Report.ps1 -WhatIf
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — System-Health-Report'
    ...
    Export-HtmlReport -Title 'System Health Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System Health Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
