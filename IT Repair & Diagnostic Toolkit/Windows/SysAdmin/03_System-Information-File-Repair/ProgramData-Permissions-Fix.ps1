...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ProgramData Permissions Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ProgramData-Permissions-Fix.
...
.EXAMPLE
    .\ProgramData-Permissions-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\ProgramData-Permissions-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'ProgramData Permissions Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — ProgramData-Permissions-Fix'
    ...
    Export-HtmlReport -Title 'ProgramData Permissions Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ProgramData Permissions Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
