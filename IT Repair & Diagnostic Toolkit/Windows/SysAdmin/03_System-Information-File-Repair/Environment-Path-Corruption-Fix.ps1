...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Environment Path Corruption Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Environment-Path-Corruption-Fix.
...
.EXAMPLE
    .\Environment-Path-Corruption-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Environment-Path-Corruption-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Environment Path Corruption Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Environment-Path-Corruption-Fix'
        $os = Get-CimInstance Win32_OperatingSystem
        Add-ReportRow -Check 'OS' -Result "$($os.Caption) Build $($os.BuildNumber)" -Status 'INFO' -Detail $os.Version
        Write-ProgressStep 2 $totalSteps $Activity 'System info'
        $machinePath = [Environment]::GetEnvironmentVariable('Path','Machine')
        $parts = $machinePath -split ';' | Where-Object { $_ }
        $missing = @($parts | Where-Object { -not (Test-Path $_) })
        $dupes = @($parts | Group-Object | Where-Object Count -gt 1)
        Add-ReportRow -Check 'PATH entries' -Result "$($parts.Count)" -Status 'INFO' -Detail "Missing=$($missing.Count) Dupes=$($dupes.Count)"
        if ($missing.Count -gt 0 -and $PSCmdlet.ShouldProcess('Machine PATH', 'Remove missing directories')) {
            $backupDir = 'C:\IT-Logs\Backups'
            New-Item $backupDir -ItemType Directory -Force | Out-Null
            Set-Content (Join-Path $backupDir "Path_Machine_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt") -Value $machinePath
            $newPath = ($parts | Where-Object { Test-Path $_ } | Select-Object -Unique) -join ';'
            [Environment]::SetEnvironmentVariable('Path', $newPath, 'Machine')
            Add-ReportRow -Check 'PATH cleanup' -Result 'Missing entries removed' -Status 'PASS' -Detail 'Rollback: restore backup txt to Machine Path'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'PATH fix'
        Write-ProgressStep $($totalSteps - 1) $totalSteps $Activity 'Health summary'
        $free = [math]::Round(($os.FreePhysicalMemory/1MB),2)
        Add-ReportRow -Check 'Free memory (GB approx)' -Result "$free" -Status $(if ($free -gt 1) {'PASS'} else {'WARN'}) -Detail 'Win32_OperatingSystem.FreePhysicalMemory'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Environment Path Corruption Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Environment Path Corruption Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
