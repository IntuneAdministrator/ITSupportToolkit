...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WMI Repository Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WMI-Repository-Repair.
...
$exitCode = 0
$Activity = 'WMI Repository Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — WMI-Repository-Repair'
        ...
        if ($PSCmdlet.ShouldProcess('WMI repository', 'winmgmt /salvagerepository + /resetrepository if needed')) {
            $salvage = & winmgmt /salvagerepository 2>&1 | Out-String
            Add-ReportRow -Check 'WMI salvagerepository' -Result 'Ran' -Status 'INFO' -Detail $salvage.Trim()
            $consist = & winmgmt /verifyrepository 2>&1 | Out-String
            $ok = $consist -match 'consistent'
            Add-ReportRow -Check 'WMI verifyrepository' -Result $(if ($ok) {'Consistent'} else {'Inconsistent'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail $consist.Trim()
            if (-not $ok) {
                & net stop winmgmt /y 2>&1 | Out-Null
                & winmgmt /resetrepository 2>&1 | Out-Null
                & net start winmgmt 2>&1 | Out-Null
                Add-ReportRow -Check 'WMI resetrepository' -Result 'Executed' -Status 'WARN' -Detail 'Repository reset performed'
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'WMI repair'
    ...
    Export-HtmlReport -Title 'WMI Repository Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WMI Repository Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
