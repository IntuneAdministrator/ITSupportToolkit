...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AppData Corruption Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AppData-Corruption-Detector.
...
.EXAMPLE
    .\AppData-Corruption-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AppData-Corruption-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AppData Corruption Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — AppData-Corruption-Detector'
    ...
    Export-HtmlReport -Title 'AppData Corruption Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AppData Corruption Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
