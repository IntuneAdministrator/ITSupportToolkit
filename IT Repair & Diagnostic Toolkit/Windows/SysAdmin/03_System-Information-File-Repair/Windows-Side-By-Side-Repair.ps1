...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Side By Side Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Side-By-Side-Repair.
...
.EXAMPLE
    .\Windows-Side-By-Side-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Side-By-Side-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Side By Side Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Windows-Side-By-Side-Repair'
        ...
        if ($PSCmdlet.ShouldProcess('Component store', 'DISM AnalyzeComponentStore')) {
            $anal = & dism.exe /Online /Cleanup-Image /AnalyzeComponentStore 2>&1 | Out-String
            Add-ReportRow -Check 'AnalyzeComponentStore' -Result 'Completed' -Status 'INFO' -Detail $anal.Substring(0, [Math]::Min(500, $anal.Length))
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Component store'
    ...
    Export-HtmlReport -Title 'Windows Side By Side Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Side By Side Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
