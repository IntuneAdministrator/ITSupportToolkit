...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DLL Registration Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DLL-Registration-Repair.
...
.EXAMPLE
    .\DLL-Registration-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DLL-Registration-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DLL Registration Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — DLL-Registration-Repair'
    ...
    Export-HtmlReport -Title 'DLL Registration Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DLL Registration Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
