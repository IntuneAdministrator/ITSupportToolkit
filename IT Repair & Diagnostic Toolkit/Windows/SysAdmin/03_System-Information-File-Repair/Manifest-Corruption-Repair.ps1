...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Manifest Corruption Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Manifest-Corruption-Repair.
...
.EXAMPLE
    .\Manifest-Corruption-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Manifest-Corruption-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Manifest Corruption Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Manifest-Corruption-Repair'
    ...
    Export-HtmlReport -Title 'Manifest Corruption Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Manifest Corruption Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
