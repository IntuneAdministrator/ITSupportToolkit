...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SFC DISM Full Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SFC-DISM-Full-Repair.
Category: 03_System-Information-File-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISM HEALTH'
        if ($PSCmdlet.ShouldProcess('DISM', 'CheckHealth / ScanHealth / RestoreHealth')) {
            Write-ProgressStep 2 $totalSteps $Activity 'DISM CheckHealth'
            $check = & dism.exe /Online /Cleanup-Image /CheckHealth 2>&1 | Out-String
            Add-ReportRow -Check 'DISM CheckHealth' -Result 'Completed' -Status $(if ($LASTEXITCODE -eq 0) {'PASS'} else {'WARN'}) -Detail $check.Substring(0, [Math]::Min(400, $check.Length))

            Write-ProgressStep 3 $totalSteps $Activity 'DISM ScanHealth'
            $scan = & dism.exe /Online /Cleanup-Image /ScanHealth 2>&1 | Out-String
            Add-ReportRow -Check 'DISM ScanHealth' -Result 'Completed' -Status $(if ($LASTEXITCODE -eq 0) {'PASS'} else {'WARN'}) -Detail $scan.Substring(0, [Math]::Min(400, $scan.Length))

            Write-ProgressStep 4 $totalSteps $Activity 'DISM RestoreHealth'
            $restore = & dism.exe /Online /Cleanup-Image /RestoreHealth 2>&1 | Out-String
            Add-ReportRow -Check 'DISM RestoreHealth' -Result 'Completed' -Status $(if ($LASTEXITCODE -eq 0) {'PASS'} else {'FAIL'}) -Detail $restore.Substring(0, [Math]::Min(400, $restore.Length))
        } else {
            Add-ReportRow -Check 'DISM' -Result 'WhatIf skipped' -Status 'INFO' -Detail 'No DISM changes'
        }

        Write-SectionBanner 'SFC SCANNOW'
        Write-ProgressStep 5 $totalSteps $Activity 'SFC /scannow'
        if ($PSCmdlet.ShouldProcess('SFC', 'Run sfc /scannow')) {
            $sfc = & sfc.exe /scannow 2>&1 | Out-String
            $ok = $sfc -match 'did not find any integrity violations|successfully repaired'
            Add-ReportRow -Check 'SFC /scannow' -Result $(if ($ok) {'Clean/Repaired'} else {'Review output'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail $sfc.Substring(0, [Math]::Min(500, $sfc.Length))
        }

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'SFC DISM Full Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SFC DISM Full Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
