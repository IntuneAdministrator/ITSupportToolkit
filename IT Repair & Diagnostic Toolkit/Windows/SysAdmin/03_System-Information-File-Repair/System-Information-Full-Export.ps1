...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System Information Full Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System-Information-Full-Export.
...
.EXAMPLE
    .\System-Information-Full-Export.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\System-Information-Full-Export.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'System Information Full Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — System-Information-Full-Export'
        ...
        Add-ReportRow -Check 'BIOS' -Result $bios.SMBIOSBIOSVersion -Status 'INFO' -Detail $bios.SerialNumber
        Get-CimInstance Win32_LogicalDisk -Filter "DriveType=3" | ForEach-Object {
            $freePct = [int](($_.FreeSpace/$_.Size)*100)
            Add-ReportRow -Check "Disk $($_.DeviceID)" -Result "$freePct% free" -Status $(if ($freePct -gt 15) {'PASS'} else {'WARN'}) -Detail ("Size={0:N1}GB" -f ($_.Size/1GB))
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Inventory'
    ...
    Export-HtmlReport -Title 'System Information Full Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System Information Full Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
