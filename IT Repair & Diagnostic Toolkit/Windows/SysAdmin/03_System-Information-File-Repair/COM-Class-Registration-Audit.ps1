...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: COM Class Registration Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for COM-Class-Registration-Audit.
...
.EXAMPLE
    .\COM-Class-Registration-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\COM-Class-Registration-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'COM Class Registration Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — COM-Class-Registration-Audit'
        ...
        foreach ($d in $dlls) {
            if ($PSCmdlet.ShouldProcess($d, 'regsvr32 /s')) {
                if (Test-Path $d) {
                    & regsvr32.exe /s $d
                    Add-ReportRow -Check "regsvr32 $(Split-Path $d -Leaf)" -Result 'Registered' -Status 'PASS' -Detail $d
                } else {
                    Add-ReportRow -Check "regsvr32 $(Split-Path $d -Leaf)" -Result 'Missing' -Status 'FAIL' -Detail $d
                }
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'DLL registration'
    ...
    Export-HtmlReport -Title 'COM Class Registration Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'COM Class Registration Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
