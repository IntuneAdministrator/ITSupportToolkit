...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Runtime Library Repair VCRedist
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Runtime-Library-Repair-VCRedist.
...
.EXAMPLE
    .\Runtime-Library-Repair-VCRedist.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Runtime-Library-Repair-VCRedist.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Runtime Library Repair VCRedist'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Runtime-Library-Repair-VCRedist'
    ...
    Export-HtmlReport -Title 'Runtime Library Repair VCRedist' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Runtime Library Repair VCRedist' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
