...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Temp File Deep Clean
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Temp-File-Deep-Clean.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Temp-File-Deep-Clean'
        ...
        foreach ($t in $targets) {
            ...
            if ($PSCmdlet.ShouldProcess($t, 'Remove old temp files (>3 days)')) {
                Get-ChildItem $t -Recurse -Force -ErrorAction SilentlyContinue |
                    ...
                    ForEach-Object {
                        try { $freed += $_.Length; Remove-Item $_.FullName -Force -ErrorAction SilentlyContinue } catch {}
                    }
            }
        }
        Add-ReportRow -Check 'Temp cleanup' -Result ("{0:N1} MB queued/removed" -f ($freed/1MB)) -Status 'PASS' -Detail ($targets -join '; ')
        Write-ProgressStep 3 $totalSteps $Activity 'Temp clean'
    ...
    Export-HtmlReport -Title 'Temp File Deep Clean' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Temp File Deep Clean' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
