...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Registry Backup Restore
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Registry-Backup-Restore.
...
.EXAMPLE
    .\Registry-Backup-Restore.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Registry-Backup-Restore.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Registry Backup Restore'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SYSTEM / FILE REPAIR — Registry-Backup-Restore'
    ...
    Export-HtmlReport -Title 'Registry Backup Restore' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Registry Backup Restore' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
