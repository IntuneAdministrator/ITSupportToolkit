...
<#
.SYNOPSIS
    Enterprise IT tool: Dedup Status Client Hint
.DESCRIPTION
    Phase 10 expansion (SysAdmin/46_Storage-Spaces-ReFS-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Dedup-Status-Client-Hint.ps1
.EXAMPLE
    .\Dedup-Status-Client-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Dedup Status Client Hint'; $totalSteps=8; $Name='Dedup-Status-Client-Hint'; $Folder='46_Storage-Spaces-ReFS-Deep'
try {
    ...
    Export-HtmlReport -Title 'Dedup Status Client Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dedup Status Client Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
