...
$exitCode=0; $Activity='Storage Bus Cache Check'; $totalSteps=8; $Name='Storage-Bus-Cache-Check'; $Folder='46_Storage-Spaces-ReFS-Deep'
try {
    ...
    Export-HtmlReport -Title 'Storage Bus Cache Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Bus Cache Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
