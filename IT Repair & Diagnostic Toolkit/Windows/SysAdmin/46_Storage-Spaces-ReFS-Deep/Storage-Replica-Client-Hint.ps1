...
<#
.SYNOPSIS
    Enterprise IT tool: Storage Replica Client Hint
.DESCRIPTION
    Phase 10 expansion (SysAdmin/46_Storage-Spaces-ReFS-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Storage-Replica-Client-Hint.ps1
.EXAMPLE
    .\Storage-Replica-Client-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Storage Replica Client Hint'; $totalSteps=8; $Name='Storage-Replica-Client-Hint'; $Folder='46_Storage-Spaces-ReFS-Deep'
try {
    ...
    Export-HtmlReport -Title 'Storage Replica Client Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Replica Client Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
