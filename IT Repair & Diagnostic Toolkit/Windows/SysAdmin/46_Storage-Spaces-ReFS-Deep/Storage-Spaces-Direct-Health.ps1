...
<#
.SYNOPSIS
    Common problem fix: Storage Spaces Direct Health
.DESCRIPTION
...
.EXAMPLE
    .\Storage-Spaces-Direct-Health.ps1
.EXAMPLE
    .\Storage-Spaces-Direct-Health.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Spaces Direct Health'; $totalSteps = 8; $Name = 'Storage-Spaces-Direct-Health'
try {
    ...
    Export-HtmlReport -Title 'Storage Spaces Direct Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Spaces Direct Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
