...
<#
.SYNOPSIS
    Common problem fix: Tiering Policy Audit
.DESCRIPTION
...
.EXAMPLE
    .\Tiering-Policy-Audit.ps1
.EXAMPLE
    .\Tiering-Policy-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Tiering Policy Audit'; $totalSteps = 8; $Name = 'Tiering-Policy-Audit'
try {
    ...
    Export-HtmlReport -Title 'Tiering Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Tiering Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
