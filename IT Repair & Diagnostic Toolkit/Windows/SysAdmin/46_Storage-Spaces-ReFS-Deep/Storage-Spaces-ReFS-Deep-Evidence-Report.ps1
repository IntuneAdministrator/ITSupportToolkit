...
<#
.SYNOPSIS
    Common problem fix: Storage Spaces ReFS Deep Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Storage-Spaces-ReFS-Deep-Evidence-Report.ps1
.EXAMPLE
    .\Storage-Spaces-ReFS-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Spaces ReFS Deep Evidence Report'; $totalSteps = 8; $Name = 'Storage-Spaces-ReFS-Deep-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Storage Spaces ReFS Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Spaces ReFS Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
