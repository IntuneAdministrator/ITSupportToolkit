...
<#
.SYNOPSIS
    Enterprise IT tool: Storage Pool Health Report
.DESCRIPTION
    Phase 10 expansion (SysAdmin/46_Storage-Spaces-ReFS-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Storage-Pool-Health-Report.ps1
.EXAMPLE
    .\Storage-Pool-Health-Report.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Storage Pool Health Report'; $totalSteps=8; $Name='Storage-Pool-Health-Report'; $Folder='46_Storage-Spaces-ReFS-Deep'
try {
    ...
    Export-HtmlReport -Title 'Storage Pool Health Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Pool Health Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
