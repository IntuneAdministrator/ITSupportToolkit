...
<#
.SYNOPSIS
    Common problem fix: Cluster Shared Volume Health
.DESCRIPTION
...
.EXAMPLE
    .\Cluster-Shared-Volume-Health.ps1
.EXAMPLE
    .\Cluster-Shared-Volume-Health.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cluster Shared Volume Health'; $totalSteps = 8; $Name = 'Cluster-Shared-Volume-Health'
try {
    ...
    Export-HtmlReport -Title 'Cluster Shared Volume Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cluster Shared Volume Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
