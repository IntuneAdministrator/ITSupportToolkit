...
<#
.SYNOPSIS
    Common problem fix: Storage Spaces ReFS Deep Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 46_Storage-Spaces-ReFS-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Storage-Spaces-ReFS-Deep-Policy-Validate.ps1
.EXAMPLE
    .\Storage-Spaces-ReFS-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Spaces ReFS Deep Policy Validate'; $totalSteps = 8; $Name = 'Storage-Spaces-ReFS-Deep-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '46_Storage-Spaces-ReFS-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '46_Storage-Spaces-ReFS-Deep'
    ...
    Export-HtmlReport -Title 'Storage Spaces ReFS Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Spaces ReFS Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
