...
<#
.SYNOPSIS
    Enterprise IT tool: ReFS Integrity Stream Audit
.DESCRIPTION
    Phase 10 expansion (SysAdmin/46_Storage-Spaces-ReFS-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\ReFS-Integrity-Stream-Audit.ps1
.EXAMPLE
    .\ReFS-Integrity-Stream-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='ReFS Integrity Stream Audit'; $totalSteps=8; $Name='ReFS-Integrity-Stream-Audit'; $Folder='46_Storage-Spaces-ReFS-Deep'
try {
    ...
    Export-HtmlReport -Title 'ReFS Integrity Stream Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'ReFS Integrity Stream Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
