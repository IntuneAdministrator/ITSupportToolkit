...
<#
.SYNOPSIS
    Common problem fix: ReFS Integrity Scan Trigger
.DESCRIPTION
...
.EXAMPLE
    .\ReFS-Integrity-Scan-Trigger.ps1
.EXAMPLE
    .\ReFS-Integrity-Scan-Trigger.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'ReFS Integrity Scan Trigger'; $totalSteps = 8; $Name = 'ReFS-Integrity-Scan-Trigger'
try {
    ...
    Export-HtmlReport -Title 'ReFS Integrity Scan Trigger' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'ReFS Integrity Scan Trigger' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
