...
<#
.SYNOPSIS
    Common problem fix: Storage Spaces ReFS Deep Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Storage-Spaces-ReFS-Deep-Stack-Reset.ps1
.EXAMPLE
    .\Storage-Spaces-ReFS-Deep-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Spaces ReFS Deep Stack Reset'; $totalSteps = 8; $Name = 'Storage-Spaces-ReFS-Deep-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Storage Spaces ReFS Deep Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Spaces ReFS Deep Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
