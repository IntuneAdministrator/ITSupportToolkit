...
<#
.SYNOPSIS
    Common problem fix: Storage Spaces Repair Volume
.DESCRIPTION
...
.EXAMPLE
    .\Storage-Spaces-Repair-Volume.ps1
.EXAMPLE
    .\Storage-Spaces-Repair-Volume.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Spaces Repair Volume'; $totalSteps = 8; $Name = 'Storage-Spaces-Repair-Volume'
try {
    ...
    Export-HtmlReport -Title 'Storage Spaces Repair Volume' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Spaces Repair Volume' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
