...
<#
.SYNOPSIS
    Enterprise IT tool: Virtual Disk Repair Trigger
.DESCRIPTION
    Phase 10 expansion (SysAdmin/46_Storage-Spaces-ReFS-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Virtual-Disk-Repair-Trigger.ps1
.EXAMPLE
    .\Virtual-Disk-Repair-Trigger.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Virtual Disk Repair Trigger'; $totalSteps=8; $Name='Virtual-Disk-Repair-Trigger'; $Folder='46_Storage-Spaces-ReFS-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '46_Storage-Spaces-ReFS-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'STORAGE SPACES / ReFS / iSCSI'
        try {
            Get-StoragePool -EA SilentlyContinue | ForEach-Object {
                Add-ReportRow -Check "Pool $($_.FriendlyName)" -Result $_.HealthStatus -Status $(if($_.HealthStatus -eq 'Healthy'){'PASS'}else{'FAIL'}) -Detail "Primordial=$($_.IsPrimordial)"
            }
            Get-VirtualDisk -EA SilentlyContinue | ForEach-Object {
                Add-ReportRow -Check "vDisk $($_.FriendlyName)" -Result "$($_.OperationalStatus)/$($_.HealthStatus)" -Status $(if($_.HealthStatus -eq 'Healthy'){'PASS'}else{'WARN'}) -Detail $_.ResiliencySettingName
            }
        } catch { Add-ReportRow -Check 'Storage cmdlets' -Result 'Error' -Status 'WARN' -Detail $_.Exception.Message }
        Write-ProgressStep 2 $totalSteps $Activity 'Pools'
        Get-Volume | Where-Object FileSystem -eq 'ReFS' | ForEach-Object {
            Add-ReportRow -Check "ReFS $($_.DriveLetter):" -Result $_.HealthStatus -Status 'INFO' -Detail "SizeGB=$([math]::Round($_.Size/1GB,1))"
        }
        if ($Name -match 'iSCSI') {
            try {
                Get-IscsiConnection -EA SilentlyContinue | ForEach-Object { Add-ReportRow -Check "iSCSI $($_.TargetAddress)" -Result $_.ConnectionIdentifier -Status 'INFO' -Detail '' }
            } catch { Add-ReportRow -Check 'iSCSI' -Result 'N/A' -Status 'INFO' -Detail $_.Exception.Message }
        }
        if ($Name -match 'Repair' -and $PSCmdlet.ShouldProcess('VirtualDisk','Repair-VirtualDisk where unhealthy')) {
            Get-VirtualDisk | Where-Object HealthStatus -ne 'Healthy' | ForEach-Object {
                Repair-VirtualDisk -FriendlyName $_.FriendlyName -EA SilentlyContinue
                Add-ReportRow -Check "Repair $($_.FriendlyName)" -Result 'Triggered' -Status 'WARN' -Detail ''
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Storage deep done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Virtual Disk Repair Trigger' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Virtual Disk Repair Trigger' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
