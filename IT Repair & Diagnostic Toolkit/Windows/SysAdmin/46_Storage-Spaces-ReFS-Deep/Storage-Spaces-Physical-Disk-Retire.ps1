...
<#
.SYNOPSIS
    Common problem fix: Storage Spaces Physical Disk Retire
.DESCRIPTION
...
.EXAMPLE
    .\Storage-Spaces-Physical-Disk-Retire.ps1
.EXAMPLE
    .\Storage-Spaces-Physical-Disk-Retire.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Spaces Physical Disk Retire'; $totalSteps = 8; $Name = 'Storage-Spaces-Physical-Disk-Retire'
try {
    ...
    Export-HtmlReport -Title 'Storage Spaces Physical Disk Retire' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Spaces Physical Disk Retire' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
