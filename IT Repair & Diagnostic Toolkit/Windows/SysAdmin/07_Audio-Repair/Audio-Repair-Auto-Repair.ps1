...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Audio-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Auto Repair'; $totalSteps = 8; $Name = 'Audio-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
