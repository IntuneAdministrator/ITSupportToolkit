...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Audio-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Deep Audit'; $totalSteps = 8; $Name = 'Audio-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
