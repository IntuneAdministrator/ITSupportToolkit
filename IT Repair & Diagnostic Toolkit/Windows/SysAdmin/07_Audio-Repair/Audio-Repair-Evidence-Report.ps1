...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Audio-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Evidence Report'; $totalSteps = 8; $Name = 'Audio-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
