...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Audio-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Stack Reset'; $totalSteps = 8; $Name = 'Audio-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
