...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Sonic Dolby Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Sonic-Dolby-Reset.
...
.EXAMPLE
    .\Windows-Sonic-Dolby-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Sonic-Dolby-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Sonic Dolby Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Windows-Sonic-Dolby-Reset'
    ...
    Export-HtmlReport -Title 'Windows Sonic Dolby Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Sonic Dolby Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
