...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Realtek IDT Codec Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Realtek-IDT-Codec-Reset.
...
.EXAMPLE
    .\Realtek-IDT-Codec-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Realtek-IDT-Codec-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Realtek IDT Codec Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Realtek-IDT-Codec-Reset'
    ...
    Export-HtmlReport -Title 'Realtek IDT Codec Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Realtek IDT Codec Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
