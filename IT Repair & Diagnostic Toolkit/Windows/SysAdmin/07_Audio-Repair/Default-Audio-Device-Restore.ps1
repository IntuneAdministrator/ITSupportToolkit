...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Default Audio Device Restore
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Default-Audio-Device-Restore.
...
.EXAMPLE
    .\Default-Audio-Device-Restore.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Default-Audio-Device-Restore.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Default Audio Device Restore'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Default-Audio-Device-Restore'
    ...
    Export-HtmlReport -Title 'Default Audio Device Restore' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Default Audio Device Restore' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
