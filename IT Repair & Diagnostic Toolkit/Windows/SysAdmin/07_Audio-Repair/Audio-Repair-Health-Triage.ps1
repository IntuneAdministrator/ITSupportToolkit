...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Health-Triage.ps1
.EXAMPLE
    .\Audio-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Health Triage'; $totalSteps = 8; $Name = 'Audio-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
