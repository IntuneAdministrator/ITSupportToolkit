...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: HDMI Audio Output Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for HDMI-Audio-Output-Fix.
...
$exitCode = 0
$Activity = 'HDMI Audio Output Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — HDMI-Audio-Output-Fix'
    ...
    Export-HtmlReport -Title 'HDMI Audio Output Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'HDMI Audio Output Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
