...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Audio-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Audio-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
