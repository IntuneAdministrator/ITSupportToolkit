...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audio Session Isolation Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audio-Session-Isolation-Fix.
...
.EXAMPLE
    .\Audio-Session-Isolation-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audio-Session-Isolation-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audio Session Isolation Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Audio-Session-Isolation-Fix'
    ...
    Export-HtmlReport -Title 'Audio Session Isolation Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audio Session Isolation Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
