...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Webcam Audio Sync Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Webcam-Audio-Sync-Fix.
...
$exitCode = 0
$Activity = 'Webcam Audio Sync Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Webcam-Audio-Sync-Fix'
    ...
    Export-HtmlReport -Title 'Webcam Audio Sync Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Audio Sync Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
