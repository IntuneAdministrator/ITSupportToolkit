...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Audio Device Re Enumerate
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Audio-Device-Re-Enumerate.
Category: 07_Audio-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\USB-Audio-Device-Re-Enumerate.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Audio-Device-Re-Enumerate.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Audio Device Re Enumerate'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — USB-Audio-Device-Re-Enumerate'
        $audioSvcs = @('Audiosrv','AudioEndpointBuilder','RpcSs')
        foreach ($s in $audioSvcs) {
            $svc = Get-Service -Name $s -ErrorAction SilentlyContinue
            if ($svc) {
                Add-ReportRow -Check "Service $s" -Result $svc.Status -Status $(if ($svc.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail $svc.StartType
            }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Audio services'
        $audioDevs = @(Get-PnpDevice -Class Media,AudioEndpoint -ErrorAction SilentlyContinue | Where-Object Status -ne 'OK')
        if (-not $audioDevs) {
            $audioDevs = @(Get-PnpDevice -Class Media -ErrorAction SilentlyContinue | Select-Object -First 5)
            Add-ReportRow -Check 'Media devices' -Result "$($audioDevs.Count) sampled" -Status 'PASS' -Detail (($audioDevs | ForEach-Object FriendlyName) -join '; ')
        }
        foreach ($d in $audioDevs) {
            ...
            if ($d.Status -ne 'OK' -and $PSCmdlet.ShouldProcess($d.FriendlyName, 'Disable/Enable PnP device')) {
                Disable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 2
                Enable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                Add-ReportRow -Check "Re-enum $($d.FriendlyName)" -Result 'Toggled' -Status 'PASS' -Detail $d.InstanceId
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Audio devices'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'USB Audio Device Re Enumerate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Audio Device Re Enumerate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
