...
<#
.SYNOPSIS
    Common problem fix: Audio Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Audio-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Audio-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Audio Repair Quick Fix'; $totalSteps = 8; $Name = 'Audio-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Audio Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Audio Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
