...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audio Endpoint Builder Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audio-Endpoint-Builder-Reset.
...
.EXAMPLE
    .\Audio-Endpoint-Builder-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audio-Endpoint-Builder-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audio Endpoint Builder Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Audio-Endpoint-Builder-Reset'
    ...
    Export-HtmlReport -Title 'Audio Endpoint Builder Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audio Endpoint Builder Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
