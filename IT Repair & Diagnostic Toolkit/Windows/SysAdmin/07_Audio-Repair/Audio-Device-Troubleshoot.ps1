...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audio Device Troubleshoot
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audio-Device-Troubleshoot.
...
.EXAMPLE
    .\Audio-Device-Troubleshoot.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audio-Device-Troubleshoot.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audio Device Troubleshoot'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Audio-Device-Troubleshoot'
    ...
    Export-HtmlReport -Title 'Audio Device Troubleshoot' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audio Device Troubleshoot' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
