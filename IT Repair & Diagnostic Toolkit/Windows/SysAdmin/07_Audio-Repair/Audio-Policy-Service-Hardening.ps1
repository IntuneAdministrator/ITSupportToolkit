...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audio Policy Service Hardening
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audio-Policy-Service-Hardening.
...
.EXAMPLE
    .\Audio-Policy-Service-Hardening.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audio-Policy-Service-Hardening.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audio Policy Service Hardening'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Audio-Policy-Service-Hardening'
        ...
        foreach ($s in $svcs) {
            if ($PSCmdlet.ShouldProcess($s, 'Restart service')) {
                Restart-Service $s -Force -ErrorAction SilentlyContinue
                $st = (Get-Service $s).Status
                Add-ReportRow -Check "Restart $s" -Result $st -Status $(if ($st -eq 'Running') {'PASS'} else {'FAIL'}) -Detail ''
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Audio services restart'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Audio Policy Service Hardening' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audio Policy Service Hardening' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
