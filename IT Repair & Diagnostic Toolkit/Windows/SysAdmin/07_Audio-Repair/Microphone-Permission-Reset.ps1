...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Microphone Permission Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Microphone-Permission-Reset.
...
.EXAMPLE
    .\Microphone-Permission-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Microphone-Permission-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Microphone Permission Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Microphone-Permission-Reset'
        ...
        if (Test-Path $micKey) {
            Backup-RegistryKey -KeyPath $micKey -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            $val = (Get-ItemProperty $micKey -Name Value -ErrorAction SilentlyContinue).Value
            Add-ReportRow -Check 'Microphone consent' -Result "$val" -Status 'INFO' -Detail $micKey
            if ($PSCmdlet.ShouldProcess('Microphone consent', 'Set to Allow')) {
                Set-ItemProperty $micKey -Name Value -Value 'Allow' -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Microphone Allow' -Result 'Set' -Status 'PASS' -Detail 'Rollback via backup .reg'
            }
        } else {
            Add-ReportRow -Check 'Microphone consent' -Result 'Key missing' -Status 'WARN' -Detail $micKey
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Mic permission'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Microphone Permission Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Microphone Permission Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
