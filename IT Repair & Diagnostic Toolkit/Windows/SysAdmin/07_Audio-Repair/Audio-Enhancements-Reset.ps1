...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audio Enhancements Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audio-Enhancements-Reset.
...
.EXAMPLE
    .\Audio-Enhancements-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audio-Enhancements-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audio Enhancements Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Audio-Enhancements-Reset'
    ...
    Export-HtmlReport -Title 'Audio Enhancements Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audio Enhancements Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
