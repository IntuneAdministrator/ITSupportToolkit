...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: App Audio Permission Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for App-Audio-Permission-Audit.
...
.EXAMPLE
    .\App-Audio-Permission-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\App-Audio-Permission-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'App Audio Permission Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — App-Audio-Permission-Audit'
    ...
    Export-HtmlReport -Title 'App Audio Permission Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'App Audio Permission Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
