...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth Audio Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-Audio-Repair.
...
$exitCode = 0
$Activity = 'Bluetooth Audio Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'AUDIO REPAIR — Bluetooth-Audio-Repair'
    ...
    Export-HtmlReport -Title 'Bluetooth Audio Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth Audio Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
