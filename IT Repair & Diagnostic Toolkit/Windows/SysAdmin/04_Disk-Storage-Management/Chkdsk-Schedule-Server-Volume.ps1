...
<#
.SYNOPSIS
    Common problem fix: Chkdsk Schedule Server Volume
.DESCRIPTION
...
.EXAMPLE
    .\Chkdsk-Schedule-Server-Volume.ps1
.EXAMPLE
    .\Chkdsk-Schedule-Server-Volume.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Chkdsk Schedule Server Volume'; $totalSteps = 8; $Name = 'Chkdsk-Schedule-Server-Volume'
try {
    ...
    Export-HtmlReport -Title 'Chkdsk Schedule Server Volume' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chkdsk Schedule Server Volume' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
