...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NVMe SSD Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NVMe-SSD-Health-Check.
...
$exitCode = 0
$Activity = 'NVMe SSD Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — NVMe-SSD-Health-Check'
    ...
    Export-HtmlReport -Title 'NVMe SSD Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NVMe SSD Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
