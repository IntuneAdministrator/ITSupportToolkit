...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Disk Encryption Status Full Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Disk-Encryption-Status-Full-Report.
...
.EXAMPLE
    .\Disk-Encryption-Status-Full-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Disk-Encryption-Status-Full-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Disk Encryption Status Full Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Disk-Encryption-Status-Full-Report'
        ...
        try {
            Get-BitLockerVolume -ErrorAction Stop | ForEach-Object {
                Add-ReportRow -Check "BitLocker $($_.MountPoint)" -Result $_.VolumeStatus -Status $(if ($_.ProtectionStatus -eq 'On') {'PASS'} else {'WARN'}) -Detail "Protection=$($_.ProtectionStatus) Percent=$($_.EncryptionPercentage)"
            }
        } catch {
            Add-ReportRow -Check 'BitLocker' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 3 $totalSteps $Activity 'BitLocker'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Disk Encryption Status Full Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk Encryption Status Full Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
