...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Shadow Copy Management
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Shadow-Copy-Management.
...
$exitCode = 0
$Activity = 'Shadow Copy Management'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Shadow-Copy-Management'
    ...
    Export-HtmlReport -Title 'Shadow Copy Management' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Shadow Copy Management' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
