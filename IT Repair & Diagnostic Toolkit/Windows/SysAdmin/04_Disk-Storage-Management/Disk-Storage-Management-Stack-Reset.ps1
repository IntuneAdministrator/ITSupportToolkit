...
<#
.SYNOPSIS
    Common problem fix: Disk Storage Management Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Storage-Management-Stack-Reset.ps1
.EXAMPLE
    .\Disk-Storage-Management-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Storage Management Stack Reset'; $totalSteps = 8; $Name = 'Disk-Storage-Management-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Disk Storage Management Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Storage Management Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
