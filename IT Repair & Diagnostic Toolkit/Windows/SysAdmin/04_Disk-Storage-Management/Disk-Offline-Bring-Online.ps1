...
<#
.SYNOPSIS
    Common problem fix: Disk Offline Bring Online
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Offline-Bring-Online.ps1
.EXAMPLE
    .\Disk-Offline-Bring-Online.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Offline Bring Online'; $totalSteps = 8; $Name = 'Disk-Offline-Bring-Online'
try {
    ...
    Export-HtmlReport -Title 'Disk Offline Bring Online' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Offline Bring Online' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
