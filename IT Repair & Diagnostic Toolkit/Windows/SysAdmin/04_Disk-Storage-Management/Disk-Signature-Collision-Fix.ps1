...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Disk Signature Collision Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Disk-Signature-Collision-Fix.
...
.EXAMPLE
    .\Disk-Signature-Collision-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Disk-Signature-Collision-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Disk Signature Collision Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Disk-Signature-Collision-Fix'
    ...
    Export-HtmlReport -Title 'Disk Signature Collision Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk Signature Collision Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
