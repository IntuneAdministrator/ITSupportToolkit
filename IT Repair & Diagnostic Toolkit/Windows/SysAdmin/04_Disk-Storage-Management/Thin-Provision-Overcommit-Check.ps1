...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Thin Provision Overcommit Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Thin-Provision-Overcommit-Check.
...
.EXAMPLE
    .\Thin-Provision-Overcommit-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Thin-Provision-Overcommit-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Thin Provision Overcommit Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Thin-Provision-Overcommit-Check'
    ...
    Export-HtmlReport -Title 'Thin Provision Overcommit Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Thin Provision Overcommit Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
