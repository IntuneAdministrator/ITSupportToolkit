...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Disk Cleanup Advanced
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Disk-Cleanup-Advanced.
...
$exitCode = 0
$Activity = 'Disk Cleanup Advanced'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Disk-Cleanup-Advanced'
    ...
    Export-HtmlReport -Title 'Disk Cleanup Advanced' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk Cleanup Advanced' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
