...
<#
.SYNOPSIS
    Common problem fix: LSI MegaRAID Health Check
.DESCRIPTION
...
.EXAMPLE
    .\LSI-MegaRAID-Health-Check.ps1
.EXAMPLE
    .\LSI-MegaRAID-Health-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'LSI MegaRAID Health Check'; $totalSteps = 8; $Name = 'LSI-MegaRAID-Health-Check'
try {
    ...
    Export-HtmlReport -Title 'LSI MegaRAID Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'LSI MegaRAID Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
