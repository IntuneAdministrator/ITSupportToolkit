...
<#
.SYNOPSIS
    Common problem fix: Storage Pool Health Check
.DESCRIPTION
...
.EXAMPLE
    .\Storage-Pool-Health-Check.ps1
.EXAMPLE
    .\Storage-Pool-Health-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage Pool Health Check'; $totalSteps = 8; $Name = 'Storage-Pool-Health-Check'
try {
    ...
    Export-HtmlReport -Title 'Storage Pool Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage Pool Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
