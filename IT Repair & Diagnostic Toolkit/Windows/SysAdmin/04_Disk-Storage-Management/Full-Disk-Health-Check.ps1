...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Full Disk Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Full-Disk-Health-Check.
...
$exitCode = 0
$Activity = 'Full Disk Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Full-Disk-Health-Check'
        ...
        Get-PhysicalDisk | ForEach-Object {
            Add-ReportRow -Check "Disk $($_.FriendlyName)" -Result $_.HealthStatus -Status $(if ($_.HealthStatus -eq 'Healthy') {'PASS'} else {'FAIL'}) -Detail "Media=$($_.MediaType) Bus=$($_.BusType) SizeGB=$([math]::Round($_.Size/1GB,0))"
        }
        try {
            Get-Disk | ForEach-Object {
                Add-ReportRow -Check "Disk# $($_.Number)" -Result $_.OperationalStatus -Status $(if ($_.OperationalStatus -eq 'Online') {'PASS'} else {'WARN'}) -Detail "PartitionStyle=$($_.PartitionStyle)"
            }
        } catch {}
        Write-ProgressStep 3 $totalSteps $Activity 'Disk health'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Full Disk Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Full Disk Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
