...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NTFS Permissions Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NTFS-Permissions-Audit.
...
$exitCode = 0
$Activity = 'NTFS Permissions Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — NTFS-Permissions-Audit'
    ...
    Export-HtmlReport -Title 'NTFS Permissions Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NTFS Permissions Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
