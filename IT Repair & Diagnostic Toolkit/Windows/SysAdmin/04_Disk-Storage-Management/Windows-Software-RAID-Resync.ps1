...
<#
.SYNOPSIS
    Common problem fix: Windows Software RAID Resync
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Software-RAID-Resync.ps1
.EXAMPLE
    .\Windows-Software-RAID-Resync.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Software RAID Resync'; $totalSteps = 8; $Name = 'Windows-Software-RAID-Resync'
try {
    ...
    Export-HtmlReport -Title 'Windows Software RAID Resync' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Software RAID Resync' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
