...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Volume Shadow Copy Size Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Volume-Shadow-Copy-Size-Audit.
...
.EXAMPLE
    .\Volume-Shadow-Copy-Size-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Volume-Shadow-Copy-Size-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Volume Shadow Copy Size Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Volume-Shadow-Copy-Size-Audit'
        ...
        foreach ($s in $vss) {
            Add-ReportRow -Check "Service $($s.Name)" -Result $s.Status -Status $(if ($s.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail $s.StartType
        }
        $shadows = @(Get-CimInstance Win32_ShadowCopy -ErrorAction SilentlyContinue)
        Add-ReportRow -Check 'Shadow copies' -Result "$($shadows.Count)" -Status 'INFO' -Detail 'Win32_ShadowCopy'
        Write-ProgressStep 3 $totalSteps $Activity 'VSS'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Volume Shadow Copy Size Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Volume Shadow Copy Size Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
