...
<#
.SYNOPSIS
    Common problem fix: Disk Spindle SMART Collect
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Spindle-SMART-Collect.ps1
.EXAMPLE
    .\Disk-Spindle-SMART-Collect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Spindle SMART Collect'; $totalSteps = 8; $Name = 'Disk-Spindle-SMART-Collect'
try {
    ...
    Export-HtmlReport -Title 'Disk Spindle SMART Collect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Spindle SMART Collect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
