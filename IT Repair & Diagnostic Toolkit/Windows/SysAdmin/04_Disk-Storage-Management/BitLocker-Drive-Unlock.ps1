...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: BitLocker Drive Unlock
.DESCRIPTION
    Production-grade Windows SysAdmin tool for BitLocker-Drive-Unlock.
...
$exitCode = 0
$Activity = 'BitLocker Drive Unlock'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — BitLocker-Drive-Unlock'
    ...
    Export-HtmlReport -Title 'BitLocker Drive Unlock' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'BitLocker Drive Unlock' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
