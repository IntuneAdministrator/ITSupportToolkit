...
<#
.SYNOPSIS
    Common problem fix: Disk Storage Management Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Storage-Management-Quick-Fix.ps1
.EXAMPLE
    .\Disk-Storage-Management-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Storage Management Quick Fix'; $totalSteps = 8; $Name = 'Disk-Storage-Management-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Disk Storage Management Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Storage Management Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
