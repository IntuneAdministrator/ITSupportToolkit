...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: OneDrive Storage Sense Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for OneDrive-Storage-Sense-Fix.
...
.EXAMPLE
    .\OneDrive-Storage-Sense-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\OneDrive-Storage-Sense-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'OneDrive Storage Sense Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — OneDrive-Storage-Sense-Fix'
    ...
    Export-HtmlReport -Title 'OneDrive Storage Sense Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'OneDrive Storage Sense Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
