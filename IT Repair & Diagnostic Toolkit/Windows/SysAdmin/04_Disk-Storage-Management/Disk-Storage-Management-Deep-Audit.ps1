...
<#
.SYNOPSIS
    Common problem fix: Disk Storage Management Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Storage-Management-Deep-Audit.ps1
.EXAMPLE
    .\Disk-Storage-Management-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Storage Management Deep Audit'; $totalSteps = 8; $Name = 'Disk-Storage-Management-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Disk Storage Management Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Storage Management Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
