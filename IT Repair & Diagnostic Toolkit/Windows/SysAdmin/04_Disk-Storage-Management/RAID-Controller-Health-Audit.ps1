...
<#
.SYNOPSIS
    Common problem fix: RAID Controller Health Audit
.DESCRIPTION
...
.EXAMPLE
    .\RAID-Controller-Health-Audit.ps1
.EXAMPLE
    .\RAID-Controller-Health-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RAID Controller Health Audit'; $totalSteps = 8; $Name = 'RAID-Controller-Health-Audit'
try {
    ...
    Export-HtmlReport -Title 'RAID Controller Health Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RAID Controller Health Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
