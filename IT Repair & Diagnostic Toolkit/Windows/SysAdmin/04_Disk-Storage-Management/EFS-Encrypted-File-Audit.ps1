...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: EFS Encrypted File Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for EFS-Encrypted-File-Audit.
...
.EXAMPLE
    .\EFS-Encrypted-File-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\EFS-Encrypted-File-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'EFS Encrypted File Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — EFS-Encrypted-File-Audit'
    ...
    Export-HtmlReport -Title 'EFS Encrypted File Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'EFS Encrypted File Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
