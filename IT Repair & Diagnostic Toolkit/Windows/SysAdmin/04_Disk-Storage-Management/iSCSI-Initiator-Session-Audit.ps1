...
<#
.SYNOPSIS
    Common problem fix: iSCSI Initiator Session Audit
.DESCRIPTION
...
.EXAMPLE
    .\iSCSI-Initiator-Session-Audit.ps1
.EXAMPLE
    .\iSCSI-Initiator-Session-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'iSCSI Initiator Session Audit'; $totalSteps = 8; $Name = 'iSCSI-Initiator-Session-Audit'
try {
    ...
    Export-HtmlReport -Title 'iSCSI Initiator Session Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'iSCSI Initiator Session Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
