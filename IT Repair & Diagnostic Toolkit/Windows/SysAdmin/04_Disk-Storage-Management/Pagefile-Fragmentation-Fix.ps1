...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Pagefile Fragmentation Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Pagefile-Fragmentation-Fix.
...
.EXAMPLE
    .\Pagefile-Fragmentation-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Pagefile-Fragmentation-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Pagefile Fragmentation Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Pagefile-Fragmentation-Fix'
    ...
    Export-HtmlReport -Title 'Pagefile Fragmentation Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Pagefile Fragmentation Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
