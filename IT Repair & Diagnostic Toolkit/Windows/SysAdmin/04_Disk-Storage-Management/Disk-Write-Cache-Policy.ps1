...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Disk Write Cache Policy
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Disk-Write-Cache-Policy.
...
.EXAMPLE
    .\Disk-Write-Cache-Policy.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Disk-Write-Cache-Policy.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Disk Write Cache Policy'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Disk-Write-Cache-Policy'
    ...
    Export-HtmlReport -Title 'Disk Write Cache Policy' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk Write Cache Policy' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
