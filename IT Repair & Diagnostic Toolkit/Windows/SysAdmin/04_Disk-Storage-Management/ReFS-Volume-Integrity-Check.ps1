...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ReFS Volume Integrity Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ReFS-Volume-Integrity-Check.
...
.EXAMPLE
    .\ReFS-Volume-Integrity-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\ReFS-Volume-Integrity-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'ReFS Volume Integrity Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — ReFS-Volume-Integrity-Check'
    ...
    Export-HtmlReport -Title 'ReFS Volume Integrity Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ReFS Volume Integrity Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
