...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Disk Defrag Optimizer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Disk-Defrag-Optimizer.
...
$exitCode = 0
$Activity = 'Disk Defrag Optimizer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Disk-Defrag-Optimizer'
    ...
    Export-HtmlReport -Title 'Disk Defrag Optimizer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk Defrag Optimizer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
