...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Storage Spaces Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Storage-Spaces-Repair.
...
$exitCode = 0
$Activity = 'Storage Spaces Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Storage-Spaces-Repair'
    ...
    Export-HtmlReport -Title 'Storage Spaces Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Storage Spaces Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
