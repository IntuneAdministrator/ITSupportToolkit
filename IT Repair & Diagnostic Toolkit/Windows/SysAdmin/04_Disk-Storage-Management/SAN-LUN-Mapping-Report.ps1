...
<#
.SYNOPSIS
    Common problem fix: SAN LUN Mapping Report
.DESCRIPTION
...
.EXAMPLE
    .\SAN-LUN-Mapping-Report.ps1
.EXAMPLE
    .\SAN-LUN-Mapping-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SAN LUN Mapping Report'; $totalSteps = 8; $Name = 'SAN-LUN-Mapping-Report'
try {
    ...
    Export-HtmlReport -Title 'SAN LUN Mapping Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SAN LUN Mapping Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
