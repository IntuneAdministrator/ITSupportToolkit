...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Disk IO Bottleneck Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Disk-IO-Bottleneck-Detector.
...
.EXAMPLE
    .\Disk-IO-Bottleneck-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Disk-IO-Bottleneck-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Disk IO Bottleneck Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Disk-IO-Bottleneck-Detector'
    ...
    Export-HtmlReport -Title 'Disk IO Bottleneck Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Disk IO Bottleneck Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
