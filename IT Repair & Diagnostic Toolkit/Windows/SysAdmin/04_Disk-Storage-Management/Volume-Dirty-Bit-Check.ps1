...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Volume Dirty Bit Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Volume-Dirty-Bit-Check.
...
$exitCode = 0
$Activity = 'Volume Dirty Bit Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Volume-Dirty-Bit-Check'
        ...
        Get-Volume | Where-Object DriveLetter | ForEach-Object {
            $info = & fsutil dirty query "$($_.DriveLetter):" 2>&1 | Out-String
            $dirty = $info -match 'is dirty'
            Add-ReportRow -Check "Dirty bit $($_.DriveLetter):" -Result $(if ($dirty) {'DIRTY'} else {'Clean'}) -Status $(if ($dirty) {'FAIL'} else {'PASS'}) -Detail $info.Trim()
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Dirty bit'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Volume Dirty Bit Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Volume Dirty Bit Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
