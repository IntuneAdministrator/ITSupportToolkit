...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Large File Finder
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Large-File-Finder.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Large-File-Finder'
        ...
        $large = Get-ChildItem "$drive\" -Recurse -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Length -gt 500MB } |
            Sort-Object Length -Descending |
            Select-Object -First 25
        foreach ($f in $large) {
            Add-ReportRow -Check 'Large file' -Result ("{0:N1} GB" -f ($f.Length/1GB)) -Status 'INFO' -Detail $f.FullName
        }
        if (-not $large) { Add-ReportRow -Check 'Large files' -Result 'None >500MB in shallow scan window' -Status 'PASS' -Detail "Scan root $drive (may be truncated by errors)" }
        Write-ProgressStep 3 $totalSteps $Activity 'Large files'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Large File Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Large File Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
