...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: CHKDSK Automation
.DESCRIPTION
    Production-grade Windows SysAdmin tool for CHKDSK-Automation.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — CHKDSK-Automation'
        ...
        Get-Volume | Where-Object DriveLetter | ForEach-Object {
            $dl = $_.DriveLetter
            if ($PSCmdlet.ShouldProcess("${dl}:", 'chkdsk /scan')) {
                $out = & chkdsk.exe "${dl}:" /scan 2>&1 | Out-String
                Add-ReportRow -Check "CHKDSK ${dl}:" -Result 'Scan completed' -Status $(if ($LASTEXITCODE -le 1) {'PASS'} else {'WARN'}) -Detail $out.Substring(0, [Math]::Min(300, $out.Length))
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'CHKDSK'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'CHKDSK Automation' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'CHKDSK Automation' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
