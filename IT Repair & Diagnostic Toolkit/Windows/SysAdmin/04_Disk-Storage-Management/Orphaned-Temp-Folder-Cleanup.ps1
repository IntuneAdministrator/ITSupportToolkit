...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Orphaned Temp Folder Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Orphaned-Temp-Folder-Cleanup.
...
.EXAMPLE
    .\Orphaned-Temp-Folder-Cleanup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Orphaned-Temp-Folder-Cleanup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Orphaned Temp Folder Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — Orphaned-Temp-Folder-Cleanup'
        ...
        if ($PSCmdlet.ShouldProcess('Disk cleanup', 'Cleanmgr autopilot + DISM StartComponentCleanup')) {
            Start-Process cleanmgr.exe -ArgumentList '/sagerun:1' -WindowStyle Hidden -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'cleanmgr' -Result 'Triggered sagerun:1' -Status 'INFO' -Detail 'Configure sage set 1 previously for full effect'
            $dism = & dism.exe /Online /Cleanup-Image /StartComponentCleanup 2>&1 | Out-String
            Add-ReportRow -Check 'StartComponentCleanup' -Result 'Completed' -Status $(if ($LASTEXITCODE -eq 0) {'PASS'} else {'WARN'}) -Detail $dism.Substring(0, [Math]::Min(300, $dism.Length))
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Cleanup'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Orphaned Temp Folder Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Orphaned Temp Folder Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
