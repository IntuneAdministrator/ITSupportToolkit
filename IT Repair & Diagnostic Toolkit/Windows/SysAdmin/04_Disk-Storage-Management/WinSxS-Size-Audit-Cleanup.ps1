...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WinSxS Size Audit Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WinSxS-Size-Audit-Cleanup.
...
.EXAMPLE
    .\WinSxS-Size-Audit-Cleanup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WinSxS-Size-Audit-Cleanup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WinSxS Size Audit Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — WinSxS-Size-Audit-Cleanup'
    ...
    Export-HtmlReport -Title 'WinSxS Size Audit Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WinSxS Size Audit Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
