...
<#
.SYNOPSIS
    Common problem fix: Disk Storage Management Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Storage-Management-Evidence-Report.ps1
.EXAMPLE
    .\Disk-Storage-Management-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Storage Management Evidence Report'; $totalSteps = 8; $Name = 'Disk-Storage-Management-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Disk Storage Management Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Storage Management Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
