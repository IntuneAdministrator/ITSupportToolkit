...
<#
.SYNOPSIS
    Common problem fix: Disk Storage Management Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Storage-Management-Policy-Validate.ps1
.EXAMPLE
    .\Disk-Storage-Management-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Storage Management Policy Validate'; $totalSteps = 8; $Name = 'Disk-Storage-Management-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Disk Storage Management Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Storage Management Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
