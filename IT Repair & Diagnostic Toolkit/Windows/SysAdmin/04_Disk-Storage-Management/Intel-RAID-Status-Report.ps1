...
<#
.SYNOPSIS
    Common problem fix: Intel RAID Status Report
.DESCRIPTION
...
.EXAMPLE
    .\Intel-RAID-Status-Report.ps1
.EXAMPLE
    .\Intel-RAID-Status-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intel RAID Status Report'; $totalSteps = 8; $Name = 'Intel-RAID-Status-Report'
try {
    ...
    Export-HtmlReport -Title 'Intel RAID Status Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intel RAID Status Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
