...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SMART Threshold Alert
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SMART-Threshold-Alert.
...
$exitCode = 0
$Activity = 'SMART Threshold Alert'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DISK / STORAGE — SMART-Threshold-Alert'
    ...
    Export-HtmlReport -Title 'SMART Threshold Alert' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SMART Threshold Alert' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
