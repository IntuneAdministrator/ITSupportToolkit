...
<#
.SYNOPSIS
    Common problem fix: Disk Storage Management Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Storage-Management-Anomaly-Detect.ps1
.EXAMPLE
    .\Disk-Storage-Management-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Storage Management Anomaly Detect'; $totalSteps = 8; $Name = 'Disk-Storage-Management-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Disk Storage Management Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Storage Management Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
