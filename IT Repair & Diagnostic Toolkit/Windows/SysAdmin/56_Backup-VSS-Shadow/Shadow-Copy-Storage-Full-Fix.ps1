...
<#
.SYNOPSIS
    Common problem fix: Shadow Copy Storage Full Fix
.DESCRIPTION
...
.EXAMPLE
    .\Shadow-Copy-Storage-Full-Fix.ps1
.EXAMPLE
    .\Shadow-Copy-Storage-Full-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Shadow Copy Storage Full Fix'; $totalSteps = 8; $Name = 'Shadow-Copy-Storage-Full-Fix'
try {
    ...
    Export-HtmlReport -Title 'Shadow Copy Storage Full Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shadow Copy Storage Full Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
