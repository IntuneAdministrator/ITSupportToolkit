...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Deep-Audit.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Deep Audit'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
