...
<#
.SYNOPSIS
    Enterprise IT tool: VSS Writer Status Report
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\VSS-Writer-Status-Report.ps1
.EXAMPLE
    .\VSS-Writer-Status-Report.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='VSS Writer Status Report'; $totalSteps=8; $Name='VSS-Writer-Status-Report'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'VSS Writer Status Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VSS Writer Status Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
