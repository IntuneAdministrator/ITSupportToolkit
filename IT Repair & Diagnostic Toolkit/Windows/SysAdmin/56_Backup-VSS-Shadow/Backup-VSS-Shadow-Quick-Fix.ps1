...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Quick-Fix.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Quick Fix'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
