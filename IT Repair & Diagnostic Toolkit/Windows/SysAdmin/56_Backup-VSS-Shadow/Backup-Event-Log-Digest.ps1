...
<#
.SYNOPSIS
    Enterprise IT tool: Backup Event Log Digest
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Backup-Event-Log-Digest.ps1
.EXAMPLE
    .\Backup-Event-Log-Digest.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Backup Event Log Digest'; $totalSteps=8; $Name='Backup-Event-Log-Digest'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'Backup Event Log Digest' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup Event Log Digest' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
