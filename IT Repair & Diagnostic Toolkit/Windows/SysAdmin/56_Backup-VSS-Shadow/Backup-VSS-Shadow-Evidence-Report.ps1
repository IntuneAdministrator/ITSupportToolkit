...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 56_Backup-VSS-Shadow). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Backup-VSS-Shadow-Evidence-Report.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Evidence Report'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '56_Backup-VSS-Shadow' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '56_Backup-VSS-Shadow'
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
