...
<#
.SYNOPSIS
    Enterprise IT tool: Shadow Copy Storage Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Shadow-Copy-Storage-Audit.ps1
.EXAMPLE
    .\Shadow-Copy-Storage-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Shadow Copy Storage Audit'; $totalSteps=8; $Name='Shadow-Copy-Storage-Audit'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'Shadow Copy Storage Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Shadow Copy Storage Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
