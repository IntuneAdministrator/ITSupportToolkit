...
<#
.SYNOPSIS
    Enterprise IT tool: File History Client Repair
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\File-History-Client-Repair.ps1
.EXAMPLE
    .\File-History-Client-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='File History Client Repair'; $totalSteps=8; $Name='File-History-Client-Repair'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'File History Client Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File History Client Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
