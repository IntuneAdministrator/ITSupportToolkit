...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Auto-Repair.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Auto Repair'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
