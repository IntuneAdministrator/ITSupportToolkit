...
<#
.SYNOPSIS
    Enterprise IT tool: VSS Admin Resize Guide
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\VSS-Admin-Resize-Guide.ps1
.EXAMPLE
    .\VSS-Admin-Resize-Guide.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='VSS Admin Resize Guide'; $totalSteps=8; $Name='VSS-Admin-Resize-Guide'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'VSS Admin Resize Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VSS Admin Resize Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
