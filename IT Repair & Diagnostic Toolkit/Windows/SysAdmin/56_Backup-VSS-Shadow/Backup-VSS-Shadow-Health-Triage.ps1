...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Health-Triage.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Health Triage'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
