...
<#
.SYNOPSIS
    Enterprise IT tool: Previous Versions Tab Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Previous-Versions-Tab-Check.ps1
.EXAMPLE
    .\Previous-Versions-Tab-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Previous Versions Tab Check'; $totalSteps=8; $Name='Previous-Versions-Tab-Check'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'Previous Versions Tab Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Previous Versions Tab Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
