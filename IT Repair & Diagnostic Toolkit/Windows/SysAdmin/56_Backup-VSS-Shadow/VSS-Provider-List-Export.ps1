...
<#
.SYNOPSIS
    Common problem fix: VSS Provider List Export
.DESCRIPTION
...
.EXAMPLE
    .\VSS-Provider-List-Export.ps1
.EXAMPLE
    .\VSS-Provider-List-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VSS Provider List Export'; $totalSteps = 8; $Name = 'VSS-Provider-List-Export'
try {
    ...
    Export-HtmlReport -Title 'VSS Provider List Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VSS Provider List Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
