...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Policy-Validate.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Policy Validate'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
