...
$exitCode=0; $Activity='WBAdmin Local Status'; $totalSteps=8; $Name='WBAdmin-Local-Status'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Export-HtmlReport -Title 'WBAdmin Local Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WBAdmin Local Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
