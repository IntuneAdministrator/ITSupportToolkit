...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Stack-Reset.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Stack Reset'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
