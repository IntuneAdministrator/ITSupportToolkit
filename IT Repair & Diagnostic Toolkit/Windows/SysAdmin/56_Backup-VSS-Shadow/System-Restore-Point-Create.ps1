...
<#
.SYNOPSIS
    Enterprise IT tool: System Restore Point Create
.DESCRIPTION
    Phase 11 expansion (SysAdmin/56_Backup-VSS-Shadow). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
...
$exitCode=0; $Activity='System Restore Point Create'; $totalSteps=8; $Name='System-Restore-Point-Create'; $Folder='56_Backup-VSS-Shadow'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '56_Backup-VSS-Shadow' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'BACKUP / VSS / SHADOW'
        Write-ProgressStep 2 $totalSteps $Activity 'VSS'
        $vss = Get-Service VSS -EA SilentlyContinue
        Add-ReportRow -Check 'VSS service' -Result $(if($vss){"$($vss.Status)"}else{'N/A'}) -Status $(if($vss){'PASS'}else{'WARN'}) -Detail ''
        if ($PSCmdlet.ShouldProcess('VSS', 'list writers')) {
            $writers = vssadmin list writers 2>&1 | Out-String
            $bad = ([regex]::Matches($writers, 'State:\s*\[(?!1\].*Stable)[^\]]+\]')).Count
            Add-ReportRow -Check 'VSS writers' -Result $(if($bad -gt 0){"Issues hinted: $bad"}else{'Listed (see log)'}) -Status $(if($bad -gt 0){'WARN'}else{'PASS'}) -Detail $writers.Substring(0,[Math]::Min(300,$writers.Length))
        }
        if ($Name -match 'Shadow-Copy|Storage|Resize') {
            $shadow = vssadmin list shadowstorage 2>&1 | Out-String
            Add-ReportRow -Check 'Shadow storage' -Result 'Captured' -Status 'INFO' -Detail $shadow.Substring(0,[Math]::Min(250,$shadow.Length))
        }
        if ($Name -match 'Restore-Point|Create') {
            if ($PSCmdlet.ShouldProcess('System Restore', 'checkpoint')) {
                Checkpoint-Computer -Description "IT-Repair-$stamp" -RestorePointType MODIFY_SETTINGS -EA SilentlyContinue
                Add-ReportRow -Check 'Restore point' -Result 'Requested' -Status 'INFO' -Detail 'May be disabled by policy'
            }
        }
        if ($Name -match 'File-History') {
            $fh = Get-Service fhsvc -EA SilentlyContinue
            Add-ReportRow -Check 'File History' -Result $(if($fh){"$($fh.Status)"}else{'N/A'}) -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'VSS done'
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'System Restore Point Create' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
