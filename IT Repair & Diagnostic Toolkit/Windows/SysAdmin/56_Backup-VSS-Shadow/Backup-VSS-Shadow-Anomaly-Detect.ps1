...
<#
.SYNOPSIS
    Common problem fix: Backup VSS Shadow Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Backup-VSS-Shadow-Anomaly-Detect.ps1
.EXAMPLE
    .\Backup-VSS-Shadow-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Backup VSS Shadow Anomaly Detect'; $totalSteps = 8; $Name = 'Backup-VSS-Shadow-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Backup VSS Shadow Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Backup VSS Shadow Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
