...
<#
.SYNOPSIS
    Common problem fix: VSS Writer Health Audit
.DESCRIPTION
...
.EXAMPLE
    .\VSS-Writer-Health-Audit.ps1
.EXAMPLE
    .\VSS-Writer-Health-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VSS Writer Health Audit'; $totalSteps = 8; $Name = 'VSS-Writer-Health-Audit'
try {
    ...
    Export-HtmlReport -Title 'VSS Writer Health Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VSS Writer Health Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
