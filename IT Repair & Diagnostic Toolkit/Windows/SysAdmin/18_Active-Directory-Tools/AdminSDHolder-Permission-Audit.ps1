...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AdminSDHolder Permission Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AdminSDHolder-Permission-Audit.
...
.EXAMPLE
    .\AdminSDHolder-Permission-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AdminSDHolder-Permission-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AdminSDHolder Permission Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AdminSDHolder-Permission-Audit'
        Add-ReportRow -Check 'Tool' -Result 'AdminSDHolder-Permission-Audit' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'AdminSDHolder Permission Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AdminSDHolder Permission Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
