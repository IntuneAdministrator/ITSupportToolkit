...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AD Password Policy Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AD-Password-Policy-Report.
...
.EXAMPLE
    .\AD-Password-Policy-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AD-Password-Policy-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AD Password Policy Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AD-Password-Policy-Report'
        Add-ReportRow -Check 'Tool' -Result 'AD-Password-Policy-Report' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'AD Password Policy Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AD Password Policy Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
