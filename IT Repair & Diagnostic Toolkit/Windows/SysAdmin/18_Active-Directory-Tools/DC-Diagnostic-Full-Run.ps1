...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DC Diagnostic Full Run
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DC-Diagnostic-Full-Run.
...
$exitCode = 0
$Activity = 'DC Diagnostic Full Run'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'DC-Diagnostic-Full-Run'
        Add-ReportRow -Check 'Tool' -Result 'DC-Diagnostic-Full-Run' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'DC Diagnostic Full Run' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DC Diagnostic Full Run' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
