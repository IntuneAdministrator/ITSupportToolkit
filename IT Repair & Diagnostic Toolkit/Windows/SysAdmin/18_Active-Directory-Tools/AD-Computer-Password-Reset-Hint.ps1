...
<#
.SYNOPSIS
    Common problem fix: AD Computer Password Reset Hint
.DESCRIPTION
...
.EXAMPLE
    .\AD-Computer-Password-Reset-Hint
.EXAMPLE
    .\AD-Computer-Password-Reset-Hint -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AD Computer Password Reset Hint'; $totalSteps = 8; $Name = 'AD-Computer-Password-Reset-Hint'
try {
    ...
    Export-HtmlReport -Title 'AD Computer Password Reset Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AD Computer Password Reset Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
