...
<#
.SYNOPSIS
    Common problem fix: Active Directory Tools Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Active-Directory-Tools-Deep-Audit.ps1
.EXAMPLE
    .\Active-Directory-Tools-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Active Directory Tools Deep Audit'; $totalSteps = 8; $Name = 'Active-Directory-Tools-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Active Directory Tools Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Active Directory Tools Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
