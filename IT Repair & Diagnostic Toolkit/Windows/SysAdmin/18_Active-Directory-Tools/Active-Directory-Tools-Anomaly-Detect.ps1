...
<#
.SYNOPSIS
    Common problem fix: Active Directory Tools Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Active-Directory-Tools-Anomaly-Detect.ps1
.EXAMPLE
    .\Active-Directory-Tools-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Active Directory Tools Anomaly Detect'; $totalSteps = 8; $Name = 'Active-Directory-Tools-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Active Directory Tools Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Active Directory Tools Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
