...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Kerberos Clock Skew Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Kerberos-Clock-Skew-Detect.
...
.EXAMPLE
    .\Kerberos-Clock-Skew-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Kerberos-Clock-Skew-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Kerberos Clock Skew Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Kerberos-Clock-Skew-Detect'
        Add-ReportRow -Check 'Tool' -Result 'Kerberos-Clock-Skew-Detect' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Kerberos Clock Skew Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Kerberos Clock Skew Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
