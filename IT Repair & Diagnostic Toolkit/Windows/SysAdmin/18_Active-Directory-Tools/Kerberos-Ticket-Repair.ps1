...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Kerberos Ticket Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Kerberos-Ticket-Repair.
...
$exitCode = 0
$Activity = 'Kerberos Ticket Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Kerberos-Ticket-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Kerberos-Ticket-Repair' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Kerberos Ticket Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Kerberos Ticket Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
