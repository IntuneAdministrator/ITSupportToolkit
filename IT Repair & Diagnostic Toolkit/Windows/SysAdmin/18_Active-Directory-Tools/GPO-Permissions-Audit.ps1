...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPO Permissions Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPO-Permissions-Audit.
...
$exitCode = 0
$Activity = 'GPO Permissions Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'GPO-Permissions-Audit' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'GPO Permissions Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPO Permissions Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
