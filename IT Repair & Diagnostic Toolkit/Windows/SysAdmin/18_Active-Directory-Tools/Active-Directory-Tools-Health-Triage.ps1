...
<#
.SYNOPSIS
    Common problem fix: Active Directory Tools Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Active-Directory-Tools-Health-Triage.ps1
.EXAMPLE
    .\Active-Directory-Tools-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Active Directory Tools Health Triage'; $totalSteps = 8; $Name = 'Active-Directory-Tools-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Active Directory Tools Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Active Directory Tools Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
