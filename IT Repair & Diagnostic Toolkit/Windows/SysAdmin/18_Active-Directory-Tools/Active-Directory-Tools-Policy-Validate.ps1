...
<#
.SYNOPSIS
    Common problem fix: Active Directory Tools Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 18_Active-Directory-Tools). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Active-Directory-Tools-Policy-Validate.ps1
.EXAMPLE
    .\Active-Directory-Tools-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Active Directory Tools Policy Validate'; $totalSteps = 8; $Name = 'Active-Directory-Tools-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '18_Active-Directory-Tools' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Active Directory Tools Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Active Directory Tools Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
