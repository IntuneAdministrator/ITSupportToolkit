...
<#
.SYNOPSIS
    Common problem fix: Trust Relationship Failed Fix
.DESCRIPTION
...
.EXAMPLE
    .\Trust-Relationship-Failed-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Trust Relationship Failed Fix'; $totalSteps = 8; $Name = 'Trust-Relationship-Failed-Fix'
try {
    ...
    Export-HtmlReport -Title 'Trust Relationship Failed Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Trust Relationship Failed Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
