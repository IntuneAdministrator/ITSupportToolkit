...
<#
.SYNOPSIS
    Common problem fix: Active Directory Tools Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Active-Directory-Tools-Quick-Fix.ps1
.EXAMPLE
    .\Active-Directory-Tools-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Active Directory Tools Quick Fix'; $totalSteps = 8; $Name = 'Active-Directory-Tools-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Active Directory Tools Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Active Directory Tools Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
