...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS AD Integration Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-AD-Integration-Test.
...
.EXAMPLE
    .\DNS-AD-Integration-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-AD-Integration-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS AD Integration Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'DNS-AD-Integration-Test'
        Add-ReportRow -Check 'Tool' -Result 'DNS-AD-Integration-Test' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'DNS AD Integration Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS AD Integration Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
