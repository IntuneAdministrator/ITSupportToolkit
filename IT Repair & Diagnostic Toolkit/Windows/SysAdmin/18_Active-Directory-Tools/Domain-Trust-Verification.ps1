...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Domain Trust Verification
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Domain-Trust-Verification.
...
.EXAMPLE
    .\Domain-Trust-Verification.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Domain-Trust-Verification.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Domain Trust Verification'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Domain-Trust-Verification'
        Add-ReportRow -Check 'Tool' -Result 'Domain-Trust-Verification' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Domain Trust Verification' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Domain Trust Verification' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
