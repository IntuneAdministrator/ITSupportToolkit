...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SPN Duplicate Finder
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SPN-Duplicate-Finder.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SPN-Duplicate-Finder'
        Add-ReportRow -Check 'Tool' -Result 'SPN-Duplicate-Finder' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'SPN Duplicate Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SPN Duplicate Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
