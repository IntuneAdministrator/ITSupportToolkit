...
<#
.SYNOPSIS
    Common problem fix: Kerberos Delegation Audit
.DESCRIPTION
...
.EXAMPLE
    .\Kerberos-Delegation-Audit -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Kerberos Delegation Audit'; $totalSteps = 8; $Name = 'Kerberos-Delegation-Audit'
try {
    ...
    Export-HtmlReport -Title 'Kerberos Delegation Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Kerberos Delegation Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
