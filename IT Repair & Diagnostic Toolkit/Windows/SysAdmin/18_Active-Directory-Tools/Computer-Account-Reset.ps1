...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Computer Account Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Computer-Account-Reset.
...
$exitCode = 0
$Activity = 'Computer Account Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Computer-Account-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Computer-Account-Reset' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Computer Account Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Computer Account Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
