...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AD Recycle Bin Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AD-Recycle-Bin-Enable.
...
$exitCode = 0
$Activity = 'AD Recycle Bin Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AD-Recycle-Bin-Enable'
        Add-ReportRow -Check 'Tool' -Result 'AD-Recycle-Bin-Enable' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'AD Recycle Bin Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AD Recycle Bin Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
