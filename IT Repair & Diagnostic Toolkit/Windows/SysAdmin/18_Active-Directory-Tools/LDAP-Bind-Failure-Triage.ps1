...
<#
.SYNOPSIS
    Common problem fix: LDAP Bind Failure Triage
.DESCRIPTION
...
.EXAMPLE
    .\LDAP-Bind-Failure-Triage
.EXAMPLE
    .\LDAP-Bind-Failure-Triage -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'LDAP Bind Failure Triage'; $totalSteps = 8; $Name = 'LDAP-Bind-Failure-Triage'
try {
    ...
    Export-HtmlReport -Title 'LDAP Bind Failure Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'LDAP Bind Failure Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
