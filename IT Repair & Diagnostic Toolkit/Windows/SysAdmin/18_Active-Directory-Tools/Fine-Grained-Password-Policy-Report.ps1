...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Fine Grained Password Policy Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Fine-Grained-Password-Policy-Report.
...
.EXAMPLE
    .\Fine-Grained-Password-Policy-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Fine-Grained-Password-Policy-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Fine Grained Password Policy Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Fine-Grained-Password-Policy-Report'
        Add-ReportRow -Check 'Tool' -Result 'Fine-Grained-Password-Policy-Report' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Fine Grained Password Policy Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Fine Grained Password Policy Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
