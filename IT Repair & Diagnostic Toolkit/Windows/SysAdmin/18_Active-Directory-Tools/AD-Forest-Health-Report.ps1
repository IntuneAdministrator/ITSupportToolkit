...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AD Forest Health Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AD-Forest-Health-Report.
...
.EXAMPLE
    .\AD-Forest-Health-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AD-Forest-Health-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AD Forest Health Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AD-Forest-Health-Report'
        Add-ReportRow -Check 'Tool' -Result 'AD-Forest-Health-Report' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'AD Forest Health Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AD Forest Health Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
