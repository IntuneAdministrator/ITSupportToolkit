...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPO Conflict Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPO-Conflict-Detector.
...
$exitCode = 0
$Activity = 'GPO Conflict Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'GPO-Conflict-Detector'
        Add-ReportRow -Check 'Tool' -Result 'GPO-Conflict-Detector' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'GPO Conflict Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPO Conflict Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
