...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AD User Lockout Finder
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AD-User-Lockout-Finder.
...
$exitCode = 0
$Activity = 'AD User Lockout Finder'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AD-User-Lockout-Finder'
        Add-ReportRow -Check 'Tool' -Result 'AD-User-Lockout-Finder' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'AD User Lockout Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AD User Lockout Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
