...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Privileged Account Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Privileged-Account-Audit.
...
.EXAMPLE
    .\Privileged-Account-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Privileged Account Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Privileged-Account-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Privileged-Account-Audit' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Privileged Account Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Privileged Account Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
