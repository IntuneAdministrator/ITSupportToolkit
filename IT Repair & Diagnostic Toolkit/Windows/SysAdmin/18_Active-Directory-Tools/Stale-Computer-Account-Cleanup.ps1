...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Stale Computer Account Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Stale-Computer-Account-Cleanup.
...
.EXAMPLE
    .\Stale-Computer-Account-Cleanup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Stale-Computer-Account-Cleanup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Stale Computer Account Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Stale-Computer-Account-Cleanup'
        Add-ReportRow -Check 'Tool' -Result 'Stale-Computer-Account-Cleanup' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'Stale Computer Account Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Stale Computer Account Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
