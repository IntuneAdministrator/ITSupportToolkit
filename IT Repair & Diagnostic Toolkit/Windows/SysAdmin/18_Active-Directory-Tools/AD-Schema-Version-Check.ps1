...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AD Schema Version Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AD-Schema-Version-Check.
...
.EXAMPLE
    .\AD-Schema-Version-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AD-Schema-Version-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AD Schema Version Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AD-Schema-Version-Check'
        Add-ReportRow -Check 'Tool' -Result 'AD-Schema-Version-Check' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'AD Schema Version Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AD Schema Version Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
