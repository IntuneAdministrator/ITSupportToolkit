...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SYSVOL Replication Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SYSVOL-Replication-Check.
...
.EXAMPLE
    .\SYSVOL-Replication-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SYSVOL-Replication-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SYSVOL Replication Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SYSVOL-Replication-Check'
        Add-ReportRow -Check 'Tool' -Result 'SYSVOL-Replication-Check' -Status 'INFO' -Detail '18_Active-Directory-Tools'
    ...
    Export-HtmlReport -Title 'SYSVOL Replication Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SYSVOL Replication Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
