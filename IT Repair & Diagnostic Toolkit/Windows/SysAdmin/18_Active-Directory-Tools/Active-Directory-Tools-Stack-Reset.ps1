...
<#
.SYNOPSIS
    Common problem fix: Active Directory Tools Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Active-Directory-Tools-Stack-Reset.ps1
.EXAMPLE
    .\Active-Directory-Tools-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Active Directory Tools Stack Reset'; $totalSteps = 8; $Name = 'Active-Directory-Tools-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Active Directory Tools Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Active Directory Tools Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
