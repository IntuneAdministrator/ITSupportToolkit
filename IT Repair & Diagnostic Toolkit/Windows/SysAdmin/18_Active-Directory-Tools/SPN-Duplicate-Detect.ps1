...
<#
.SYNOPSIS
    Common problem fix: SPN Duplicate Detect
.DESCRIPTION
...
.EXAMPLE
    .\SPN-Duplicate-Detect
.EXAMPLE
    .\SPN-Duplicate-Detect -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SPN Duplicate Detect'; $totalSteps = 8; $Name = 'SPN-Duplicate-Detect'
try {
    ...
    Export-HtmlReport -Title 'SPN Duplicate Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SPN Duplicate Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
