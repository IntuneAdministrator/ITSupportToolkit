...
<#
.SYNOPSIS
    Common problem fix: GPO Resultant Set Hint
.DESCRIPTION
...
.EXAMPLE
    .\GPO-Resultant-Set-Hint
.EXAMPLE
    .\GPO-Resultant-Set-Hint -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'GPO Resultant Set Hint'; $totalSteps = 8; $Name = 'GPO-Resultant-Set-Hint'
try {
    ...
    Export-HtmlReport -Title 'GPO Resultant Set Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'GPO Resultant Set Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
