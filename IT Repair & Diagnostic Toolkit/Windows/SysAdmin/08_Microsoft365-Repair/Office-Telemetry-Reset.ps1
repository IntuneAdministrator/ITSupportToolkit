...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Office Telemetry Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Office-Telemetry-Reset.
...
$exitCode = 0
$Activity = 'Office Telemetry Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Office-Telemetry-Reset'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Office Telemetry Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Office Telemetry Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
