...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Teams Meeting Addin Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Teams-Meeting-Addin-Repair.
...
.EXAMPLE
    .\Teams-Meeting-Addin-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Teams-Meeting-Addin-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Teams Meeting Addin Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Teams-Meeting-Addin-Repair'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Teams Meeting Addin Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams Meeting Addin Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
