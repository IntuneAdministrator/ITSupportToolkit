...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: M365 Proxy Bypass Config
.DESCRIPTION
    Production-grade Windows SysAdmin tool for M365-Proxy-Bypass-Config.
...
.EXAMPLE
    .\M365-Proxy-Bypass-Config.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\M365-Proxy-Bypass-Config.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'M365 Proxy Bypass Config'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — M365-Proxy-Bypass-Config'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'M365 Proxy Bypass Config' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'M365 Proxy Bypass Config' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
