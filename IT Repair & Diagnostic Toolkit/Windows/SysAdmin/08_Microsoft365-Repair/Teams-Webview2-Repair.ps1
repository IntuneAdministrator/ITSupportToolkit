...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Teams Webview2 Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Teams-Webview2-Repair.
...
$exitCode = 0
$Activity = 'Teams Webview2 Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Teams-Webview2-Repair'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Teams Webview2 Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams Webview2 Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
