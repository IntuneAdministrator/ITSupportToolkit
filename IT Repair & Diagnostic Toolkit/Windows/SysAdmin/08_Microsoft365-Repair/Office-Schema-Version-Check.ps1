...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Office Schema Version Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Office-Schema-Version-Check.
Category: 08_Microsoft365-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Office-Schema-Version-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Office-Schema-Version-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Office Schema Version Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Office-Schema-Version-Check'
        $officeRoots = @(
        ...
        $foundOffice = $false
        foreach ($p in $officeRoots) {
            if (Test-Path $p) { $foundOffice = $true; Add-ReportRow -Check 'Office path' -Result $p -Status 'INFO' -Detail 'Detected' }
        }
        if (-not $foundOffice) { Add-ReportRow -Check 'Office path' -Result 'Not detected' -Status 'WARN' -Detail 'M365 Apps may use Click-to-Run under Program Files\Microsoft Office' }
        Write-ProgressStep 2 $totalSteps $Activity 'Office detect'
        $ospp = Get-ChildItem "$env:ProgramFiles*\Microsoft Office\Office*\OSPP.VBS" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($ospp) {
            $lic = & cscript.exe //Nologo $ospp.FullName /dstatus 2>&1 | Out-String
            Add-ReportRow -Check 'OSPP dstatus' -Result 'Collected' -Status 'INFO' -Detail $lic.Substring(0, [Math]::Min(500, $lic.Length))
        } else {
            Add-ReportRow -Check 'OSPP.VBS' -Result 'Not found' -Status 'WARN' -Detail 'Click-to-Run may use different licensing'
        }
        $ctr = Get-Service ClickToRunSvc -ErrorAction SilentlyContinue
        if ($ctr) {
            Add-ReportRow -Check 'ClickToRunSvc' -Result $ctr.Status -Status $(if ($ctr.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail $ctr.StartType
            if ($Name -match 'Click-To-Run|Full-Repair' -and $PSCmdlet.ShouldProcess('ClickToRunSvc', 'Restart')) {
                Restart-Service ClickToRunSvc -Force -ErrorAction SilentlyContinue
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Licensing'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Office Schema Version Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Office Schema Version Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
