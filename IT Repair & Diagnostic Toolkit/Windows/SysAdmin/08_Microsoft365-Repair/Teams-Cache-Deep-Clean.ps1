...
<#
.SYNOPSIS
    Common problem fix: Teams Cache Deep Clean
.DESCRIPTION
...
.EXAMPLE
    .\Teams-Cache-Deep-Clean.ps1
.EXAMPLE
    .\Teams-Cache-Deep-Clean.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Teams Cache Deep Clean'; $totalSteps = 8; $Name = 'Teams-Cache-Deep-Clean'
try {
    ...
    Export-HtmlReport -Title 'Teams Cache Deep Clean' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Teams Cache Deep Clean' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
