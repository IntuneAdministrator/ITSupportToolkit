...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SharePoint Mapped Drive Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SharePoint-Mapped-Drive-Repair.
...
.EXAMPLE
    .\SharePoint-Mapped-Drive-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SharePoint-Mapped-Drive-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SharePoint Mapped Drive Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — SharePoint-Mapped-Drive-Repair'
        $officeRoots = @(
        ...
        $officeProc = @('WINWORD','EXCEL','POWERPNT','OUTLOOK','Teams','ms-teams','OneDrive')
        foreach ($op in $officeProc) {
            $p = @(Get-Process -Name $op -ErrorAction SilentlyContinue)
            if ($p) { Add-ReportRow -Check "Running $op" -Result "$($p.Count) process(es)" -Status 'INFO' -Detail (($p | ForEach-Object Id) -join ',') }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'M365 processes'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'SharePoint Mapped Drive Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SharePoint Mapped Drive Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
