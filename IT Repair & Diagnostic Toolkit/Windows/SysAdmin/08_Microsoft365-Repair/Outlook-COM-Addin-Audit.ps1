...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook COM Addin Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-COM-Addin-Audit.
...
.EXAMPLE
    .\Outlook-COM-Addin-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-COM-Addin-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook COM Addin Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Outlook-COM-Addin-Audit'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Outlook COM Addin Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook COM Addin Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
