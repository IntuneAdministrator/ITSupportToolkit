...
<#
.SYNOPSIS
    Common problem fix: Office Click To Run Repair
.DESCRIPTION
...
.EXAMPLE
    .\Office-Click-To-Run-Repair.ps1
.EXAMPLE
    .\Office-Click-To-Run-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Office Click To Run Repair'; $totalSteps = 8; $Name = 'Office-Click-To-Run-Repair'
try {
    ...
    Export-HtmlReport -Title 'Office Click To Run Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Office Click To Run Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
