...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Azure AD Token Clear
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Azure-AD-Token-Clear.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Azure-AD-Token-Clear'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Azure AD Token Clear' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Azure AD Token Clear' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
