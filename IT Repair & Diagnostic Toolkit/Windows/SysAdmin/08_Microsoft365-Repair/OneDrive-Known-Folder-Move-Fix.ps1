...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: OneDrive Known Folder Move Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for OneDrive-Known-Folder-Move-Fix.
...
.EXAMPLE
    .\OneDrive-Known-Folder-Move-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\OneDrive-Known-Folder-Move-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'OneDrive Known Folder Move Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — OneDrive-Known-Folder-Move-Fix'
        $officeRoots = @(
        ...
        Get-Process OneDrive -ErrorAction SilentlyContinue | ForEach-Object {
            if ($PSCmdlet.ShouldProcess('OneDrive', 'Restart')) { Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue }
        }
        $od = "$env:LOCALAPPDATA\Microsoft\OneDrive\OneDrive.exe"
        if (Test-Path $od) {
            if ($PSCmdlet.ShouldProcess($od, 'Start OneDrive')) { Start-Process $od }
            Add-ReportRow -Check 'OneDrive restart' -Result 'Started' -Status 'PASS' -Detail $od
        } else {
            Add-ReportRow -Check 'OneDrive' -Result 'Not installed' -Status 'WARN' -Detail $od
        }
        Write-ProgressStep 3 $totalSteps $Activity 'OneDrive'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'OneDrive Known Folder Move Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'OneDrive Known Folder Move Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
