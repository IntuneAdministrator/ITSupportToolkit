...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Copilot Credential Refresh
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Copilot-Credential-Refresh.
...
.EXAMPLE
    .\Copilot-Credential-Refresh.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Copilot-Credential-Refresh.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Copilot Credential Refresh'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Copilot-Credential-Refresh'
        $officeRoots = @(
        ...
        if ($PSCmdlet.ShouldProcess('AAD tokens', 'Clear Token Broker / Workplace join artifacts hint')) {
            $tb = "$env:LOCALAPPDATA\Microsoft\TokenBroker\Cache"
            if (Test-Path $tb) {
                Add-ReportRow -Check 'TokenBroker cache' -Result 'Present' -Status 'INFO' -Detail $tb
            }
            & dsregcmd /status 2>&1 | Out-Null
            Add-ReportRow -Check 'PRT / join' -Result $Join.Label -Status 'INFO' -Detail 'Use AAD-PRT-Token-Repair for deep PRT repair'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Tokens'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Copilot Credential Refresh' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Copilot Credential Refresh' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
