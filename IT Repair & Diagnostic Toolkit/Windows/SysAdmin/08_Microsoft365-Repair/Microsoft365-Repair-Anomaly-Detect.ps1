...
<#
.SYNOPSIS
    Common problem fix: Microsoft365 Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Microsoft365-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Microsoft365-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft365 Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Microsoft365-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Microsoft365 Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft365 Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
