...
<#
.SYNOPSIS
    Common problem fix: Microsoft365 Repair Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 08_Microsoft365-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Microsoft365-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Microsoft365-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft365 Repair Policy Validate'; $totalSteps = 8; $Name = 'Microsoft365-Repair-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '08_Microsoft365-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '08_Microsoft365-Repair'
    ...
    Export-HtmlReport -Title 'Microsoft365 Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft365 Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
