...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Teams Cache Purge
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Teams-Cache-Purge.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Teams-Cache-Purge'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Teams Cache Purge' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams Cache Purge' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
