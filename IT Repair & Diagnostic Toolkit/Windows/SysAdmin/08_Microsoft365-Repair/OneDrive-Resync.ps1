...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: OneDrive Resync
.DESCRIPTION
    Production-grade Windows SysAdmin tool for OneDrive-Resync.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — OneDrive-Resync'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'OneDrive Resync' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'OneDrive Resync' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
