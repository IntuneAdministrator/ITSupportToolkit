...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Teams Policy Assignment Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Teams-Policy-Assignment-Audit.
...
.EXAMPLE
    .\Teams-Policy-Assignment-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Teams-Policy-Assignment-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Teams Policy Assignment Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Teams-Policy-Assignment-Audit'
        $officeRoots = @(
        ...
        Get-Process -Name Teams,ms-teams -ErrorAction SilentlyContinue | ForEach-Object {
            if ($PSCmdlet.ShouldProcess($_.ProcessName, 'Stop Teams process')) { Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue }
        }
        foreach ($p in $teamsPaths) {
            if (Test-Path $p) {
                $cache = Join-Path $p 'Cache'
                if ((Test-Path $cache) -and $PSCmdlet.ShouldProcess($cache, 'Clear Teams cache')) {
                    Remove-Item "$cache\*" -Recurse -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Teams cache clear' -Result 'Cleared' -Status 'PASS' -Detail $cache
                } else {
                    Add-ReportRow -Check 'Teams path' -Result 'Present' -Status 'INFO' -Detail $p
                }
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Teams'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Teams Policy Assignment Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams Policy Assignment Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
