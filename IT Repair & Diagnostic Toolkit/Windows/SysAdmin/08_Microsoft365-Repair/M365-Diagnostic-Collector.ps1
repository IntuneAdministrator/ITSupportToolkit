...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: M365 Diagnostic Collector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for M365-Diagnostic-Collector.
...
.EXAMPLE
    .\M365-Diagnostic-Collector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\M365-Diagnostic-Collector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'M365 Diagnostic Collector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — M365-Diagnostic-Collector'
        $officeRoots = @(
        ...
        Get-Process OUTLOOK -ErrorAction SilentlyContinue | ForEach-Object {
            if ($PSCmdlet.ShouldProcess('OUTLOOK', 'Stop process')) { Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue }
        }
        $ost = Get-ChildItem "$env:LOCALAPPDATA\Microsoft\Outlook" -Filter *.ost -ErrorAction SilentlyContinue
        foreach ($o in $ost) {
            Add-ReportRow -Check 'OST file' -Result ("{0:N1} MB" -f ($o.Length/1MB)) -Status 'INFO' -Detail $o.FullName
        }
        ...
        Add-ReportRow -Check 'SCANPST' -Result $(if ($scanpst) {'Found'} else {'Not found'}) -Status 'INFO' -Detail $scanpst.FullName
        Write-ProgressStep 3 $totalSteps $Activity 'Outlook'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'M365 Diagnostic Collector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'M365 Diagnostic Collector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
