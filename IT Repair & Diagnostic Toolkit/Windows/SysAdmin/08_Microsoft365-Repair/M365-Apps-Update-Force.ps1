...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: M365 Apps Update Force
.DESCRIPTION
    Production-grade Windows SysAdmin tool for M365-Apps-Update-Force.
...
$exitCode = 0
$Activity = 'M365 Apps Update Force'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — M365-Apps-Update-Force'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'M365 Apps Update Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'M365 Apps Update Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
