...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Office Activation Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Office-Activation-Repair.
...
.EXAMPLE
    .\Office-Activation-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Office-Activation-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Office Activation Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Office-Activation-Repair'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Office Activation Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Office Activation Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
