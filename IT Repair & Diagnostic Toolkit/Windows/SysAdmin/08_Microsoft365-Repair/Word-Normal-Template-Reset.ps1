...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Word Normal Template Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Word-Normal-Template-Reset.
...
.EXAMPLE
    .\Word-Normal-Template-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Word-Normal-Template-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Word Normal Template Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Word-Normal-Template-Reset'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Word Normal Template Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Word Normal Template Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
