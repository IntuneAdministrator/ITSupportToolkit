...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Profile Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Profile-Repair.
...
$exitCode = 0
$Activity = 'Outlook Profile Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Outlook-Profile-Repair'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Outlook Profile Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Profile Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
