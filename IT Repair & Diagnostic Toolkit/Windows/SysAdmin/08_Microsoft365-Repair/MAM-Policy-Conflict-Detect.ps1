...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: MAM Policy Conflict Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for MAM-Policy-Conflict-Detect.
...
.EXAMPLE
    .\MAM-Policy-Conflict-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\MAM-Policy-Conflict-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'MAM Policy Conflict Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — MAM-Policy-Conflict-Detect'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'MAM Policy Conflict Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'MAM Policy Conflict Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
