...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Planner Cache Clear
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Planner-Cache-Clear.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Planner-Cache-Clear'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Planner Cache Clear' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Planner Cache Clear' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
