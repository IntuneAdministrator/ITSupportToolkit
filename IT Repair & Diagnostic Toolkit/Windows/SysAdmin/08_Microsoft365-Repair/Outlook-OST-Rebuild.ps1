...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook OST Rebuild
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-OST-Rebuild.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Outlook-OST-Rebuild'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Outlook OST Rebuild' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook OST Rebuild' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
