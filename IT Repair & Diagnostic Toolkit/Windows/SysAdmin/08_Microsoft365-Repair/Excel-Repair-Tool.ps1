...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Excel Repair Tool
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Excel-Repair-Tool.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Excel-Repair-Tool'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Excel Repair Tool' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Excel Repair Tool' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
