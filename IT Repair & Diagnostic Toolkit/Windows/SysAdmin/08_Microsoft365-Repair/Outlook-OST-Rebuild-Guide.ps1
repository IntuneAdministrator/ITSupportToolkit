...
<#
.SYNOPSIS
    Common problem fix: Outlook OST Rebuild Guide
.DESCRIPTION
...
.EXAMPLE
    .\Outlook-OST-Rebuild-Guide.ps1
.EXAMPLE
    .\Outlook-OST-Rebuild-Guide.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Outlook OST Rebuild Guide'; $totalSteps = 8; $Name = 'Outlook-OST-Rebuild-Guide'
try {
    ...
    Export-HtmlReport -Title 'Outlook OST Rebuild Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Outlook OST Rebuild Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
