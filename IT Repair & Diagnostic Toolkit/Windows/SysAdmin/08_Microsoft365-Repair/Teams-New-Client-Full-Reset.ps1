...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Teams New Client Full Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Teams-New-Client-Full-Reset.
...
.EXAMPLE
    .\Teams-New-Client-Full-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Teams-New-Client-Full-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Teams New Client Full Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Teams-New-Client-Full-Reset'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Teams New Client Full Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Teams New Client Full Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
