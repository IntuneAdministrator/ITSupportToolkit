...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SharePoint Client Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SharePoint-Client-Reset.
...
.EXAMPLE
    .\SharePoint-Client-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SharePoint-Client-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SharePoint Client Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — SharePoint-Client-Reset'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'SharePoint Client Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SharePoint Client Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
