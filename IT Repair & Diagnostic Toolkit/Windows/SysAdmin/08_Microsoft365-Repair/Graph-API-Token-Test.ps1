...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Graph API Token Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Graph-API-Token-Test.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — Graph-API-Token-Test'
        $officeRoots = @(
    ...
    Export-HtmlReport -Title 'Graph API Token Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Graph API Token Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
