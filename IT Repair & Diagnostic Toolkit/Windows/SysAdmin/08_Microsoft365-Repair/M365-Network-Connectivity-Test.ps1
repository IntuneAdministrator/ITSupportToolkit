...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: M365 Network Connectivity Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for M365-Network-Connectivity-Test.
...
.EXAMPLE
    .\M365-Network-Connectivity-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\M365-Network-Connectivity-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'M365 Network Connectivity Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'MICROSOFT 365 — M365-Network-Connectivity-Test'
        $officeRoots = @(
        ...
        foreach ($u in $urls) {
            try {
                ...
                Add-ReportRow -Check $u -Result $r.StatusCode -Status $(if ($r.StatusCode -lt 400) {'PASS'} else {'WARN'}) -Detail 'HEAD'
            } catch {
                Add-ReportRow -Check $u -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'M365 endpoints'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'M365 Network Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'M365 Network Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
