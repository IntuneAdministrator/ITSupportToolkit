...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Storage Sense Policy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Storage-Sense-Policy-Fix.
...
.EXAMPLE
    .\Storage-Sense-Policy-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Storage-Sense-Policy-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Storage Sense Policy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Storage-Sense-Policy-Fix'
    ...
    Export-HtmlReport -Title 'Storage Sense Policy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Storage Sense Policy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
