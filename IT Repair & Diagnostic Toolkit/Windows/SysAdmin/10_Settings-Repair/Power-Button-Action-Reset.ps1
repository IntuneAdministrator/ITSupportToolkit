...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Power Button Action Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Power-Button-Action-Reset.
...
.EXAMPLE
    .\Power-Button-Action-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Power-Button-Action-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Power Button Action Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Power-Button-Action-Reset'
    ...
    Export-HtmlReport -Title 'Power Button Action Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Power Button Action Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
