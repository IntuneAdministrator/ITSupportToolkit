...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Environment Variables Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Environment-Variables-Audit.
...
.EXAMPLE
    .\Environment-Variables-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Environment-Variables-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Environment Variables Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Environment-Variables-Audit'
        ...
        foreach ($scope in @('Machine','User')) {
            ...
            $bad = @($parts | Where-Object { -not (Test-Path $_) })
            Add-ReportRow -Check "$scope PATH" -Result "$($parts.Count) entries" -Status $(if ($bad.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ("Missing: " + ($bad -join '; '))
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Env audit'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Environment Variables Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Environment Variables Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
