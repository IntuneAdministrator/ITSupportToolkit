...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: User Profile Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for User-Profile-Repair.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — User-Profile-Repair'
        ...
        foreach ($p in $profileList) {
            $state = $p.State
            Add-ReportRow -Check $p.ProfileImagePath -Result "State=$state" -Status $(if ($state -eq 0) {'PASS'} else {'WARN'}) -Detail $p.PSChildName
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Profiles'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'User Profile Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'User Profile Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
