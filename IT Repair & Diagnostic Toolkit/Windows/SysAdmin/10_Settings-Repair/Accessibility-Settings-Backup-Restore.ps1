...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Accessibility Settings Backup Restore
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Accessibility-Settings-Backup-Restore.
...
.EXAMPLE
    .\Accessibility-Settings-Backup-Restore.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Accessibility-Settings-Backup-Restore.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Accessibility Settings Backup Restore'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Accessibility-Settings-Backup-Restore'
        ...
        Add-ReportRow -Check 'User locale' -Result (Get-WinSystemLocale).Name -Status 'INFO' -Detail (Get-Culture).DisplayName
        Add-ReportRow -Check 'UI language' -Result (Get-WinUserLanguageList | Select-Object -First 1).LanguageTag -Status 'INFO' -Detail ''
        Write-ProgressStep 3 $totalSteps $Activity 'Locale'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Accessibility Settings Backup Restore' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Accessibility Settings Backup Restore' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
