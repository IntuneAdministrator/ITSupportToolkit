...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Settings-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Quick Fix'; $totalSteps = 8; $Name = 'Settings-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
