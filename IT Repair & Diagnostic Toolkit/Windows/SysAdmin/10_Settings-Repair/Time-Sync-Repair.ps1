...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Time Sync Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Time-Sync-Repair.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Time-Sync-Repair'
        ...
        if ($PSCmdlet.ShouldProcess('W32Time', 'Restart and resync')) {
            ...
            Add-ReportRow -Check 'Time sync' -Result 'Resync attempted' -Status 'PASS' -Detail $st.Substring(0, [Math]::Min(300, $st.Length))
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Time'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Time Sync Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Time Sync Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
