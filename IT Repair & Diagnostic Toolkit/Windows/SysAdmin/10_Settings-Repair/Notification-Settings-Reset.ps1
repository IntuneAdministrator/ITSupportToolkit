...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Notification Settings Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Notification-Settings-Reset.
...
.EXAMPLE
    .\Notification-Settings-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Notification-Settings-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Notification Settings Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Notification-Settings-Reset'
    ...
    Export-HtmlReport -Title 'Notification Settings Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Notification Settings Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
