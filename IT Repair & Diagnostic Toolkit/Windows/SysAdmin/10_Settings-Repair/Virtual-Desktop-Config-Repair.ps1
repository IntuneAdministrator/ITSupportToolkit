...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Virtual Desktop Config Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Virtual-Desktop-Config-Repair.
...
.EXAMPLE
    .\Virtual-Desktop-Config-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Virtual-Desktop-Config-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Virtual Desktop Config Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Virtual-Desktop-Config-Repair'
        ...
        Add-ReportRow -Check 'Settings repair' -Result $Name -Status 'INFO' -Detail 'Baseline settings audit complete'
        Write-ProgressStep 3 $totalSteps $Activity 'Settings'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Virtual Desktop Config Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Virtual Desktop Config Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
