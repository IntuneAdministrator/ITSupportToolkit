...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Settings-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Policy Validate'; $totalSteps = 8; $Name = 'Settings-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
