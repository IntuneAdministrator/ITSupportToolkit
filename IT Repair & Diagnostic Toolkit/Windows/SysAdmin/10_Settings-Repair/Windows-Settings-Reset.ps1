...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Settings Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Settings-Reset.
...
$exitCode = 0
$Activity = 'Windows Settings Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Windows-Settings-Reset'
    ...
    Export-HtmlReport -Title 'Windows Settings Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Settings Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
