...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Default App Association Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Default-App-Association-Reset.
...
.EXAMPLE
    .\Default-App-Association-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Default-App-Association-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Default App Association Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Default-App-Association-Reset'
    ...
    Export-HtmlReport -Title 'Default App Association Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Default App Association Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
