...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Health-Triage.ps1
.EXAMPLE
    .\Settings-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Health Triage'; $totalSteps = 8; $Name = 'Settings-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
