...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Night Light Config Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Night-Light-Config-Repair.
...
.EXAMPLE
    .\Night-Light-Config-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Night-Light-Config-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Night Light Config Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Night-Light-Config-Repair'
    ...
    Export-HtmlReport -Title 'Night Light Config Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Night Light Config Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
