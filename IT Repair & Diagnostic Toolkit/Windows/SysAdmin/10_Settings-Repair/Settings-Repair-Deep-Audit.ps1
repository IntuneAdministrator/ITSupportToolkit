...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Settings-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Deep Audit'; $totalSteps = 8; $Name = 'Settings-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
