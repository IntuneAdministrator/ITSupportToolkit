...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Search Indexer Policy Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Search-Indexer-Policy-Reset.
...
.EXAMPLE
    .\Search-Indexer-Policy-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Search-Indexer-Policy-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Search Indexer Policy Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Search-Indexer-Policy-Reset'
        ...
        Backup-RegistryKey -KeyPath $explorerKey -BackupDir $backupDir | Out-Null
        if ($PSCmdlet.ShouldProcess('Explorer', 'Restart shell to refresh UI settings')) {
            Get-Process explorer -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
            Start-Process explorer.exe
            Add-ReportRow -Check 'Explorer restart' -Result 'Done' -Status 'PASS' -Detail 'UI settings refresh'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Explorer refresh'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Search Indexer Policy Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Search Indexer Policy Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
