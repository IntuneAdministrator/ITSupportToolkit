...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Settings-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Settings-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
