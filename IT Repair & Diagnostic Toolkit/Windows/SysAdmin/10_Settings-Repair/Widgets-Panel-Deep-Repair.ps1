...
<#
.SYNOPSIS
    Common problem fix: Widgets Panel Deep Repair
.DESCRIPTION
...
.EXAMPLE
    .\Widgets-Panel-Deep-Repair.ps1
.EXAMPLE
    .\Widgets-Panel-Deep-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Widgets Panel Deep Repair'; $totalSteps = 8; $Name = 'Widgets-Panel-Deep-Repair'
try {
    ...
    Export-HtmlReport -Title 'Widgets Panel Deep Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Widgets Panel Deep Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
