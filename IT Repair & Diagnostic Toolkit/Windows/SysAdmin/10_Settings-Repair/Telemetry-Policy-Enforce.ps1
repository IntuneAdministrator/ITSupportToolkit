...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Telemetry Policy Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Telemetry-Policy-Enforce.
...
.EXAMPLE
    .\Telemetry-Policy-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Telemetry-Policy-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Telemetry Policy Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Telemetry-Policy-Enforce'
        ...
        if (-not (Test-Path $tel)) { New-Item $tel -Force | Out-Null }
        Backup-RegistryKey -KeyPath $tel -BackupDir $backupDir | Out-Null
        if ($PSCmdlet.ShouldProcess('Telemetry', 'Set AllowTelemetry=1 (Enterprise minimum)')) {
            New-ItemProperty -Path $tel -Name AllowTelemetry -Value 1 -PropertyType DWord -Force | Out-Null
            Add-ReportRow -Check 'AllowTelemetry' -Result '1' -Status 'PASS' -Detail 'Policy enforced; rollback via .reg backup'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Telemetry'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Telemetry Policy Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Telemetry Policy Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
