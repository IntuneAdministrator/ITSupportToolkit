...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: File Explorer Options Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for File-Explorer-Options-Reset.
...
.EXAMPLE
    .\File-Explorer-Options-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\File-Explorer-Options-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'File Explorer Options Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — File-Explorer-Options-Reset'
    ...
    Export-HtmlReport -Title 'File Explorer Options Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'File Explorer Options Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
