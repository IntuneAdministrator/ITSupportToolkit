...
<#
.SYNOPSIS
    Common problem fix: Focus Assist Stuck On
.DESCRIPTION
...
.EXAMPLE
    .\Focus-Assist-Stuck-On.ps1
.EXAMPLE
    .\Focus-Assist-Stuck-On.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Focus Assist Stuck On'; $totalSteps = 8; $Name = 'Focus-Assist-Stuck-On'
try {
    ...
    Export-HtmlReport -Title 'Focus Assist Stuck On' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Focus Assist Stuck On' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
