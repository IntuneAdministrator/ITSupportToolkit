...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Ink Workspace Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Ink-Workspace-Reset.
...
.EXAMPLE
    .\Windows-Ink-Workspace-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Ink-Workspace-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Ink Workspace Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Windows-Ink-Workspace-Reset'
    ...
    Export-HtmlReport -Title 'Windows Ink Workspace Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Ink Workspace Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
