...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Settings-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Auto Repair'; $totalSteps = 8; $Name = 'Settings-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
