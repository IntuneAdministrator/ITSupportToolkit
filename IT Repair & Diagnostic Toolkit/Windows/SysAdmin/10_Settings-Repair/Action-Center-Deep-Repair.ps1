...
<#
.SYNOPSIS
    Common problem fix: Action Center Deep Repair
.DESCRIPTION
...
.EXAMPLE
    .\Action-Center-Deep-Repair.ps1
.EXAMPLE
    .\Action-Center-Deep-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Action Center Deep Repair'; $totalSteps = 8; $Name = 'Action-Center-Deep-Repair'
try {
    ...
    Export-HtmlReport -Title 'Action Center Deep Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Action Center Deep Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
