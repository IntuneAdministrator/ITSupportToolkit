...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Input Method Editor Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Input-Method-Editor-Repair.
...
.EXAMPLE
    .\Input-Method-Editor-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Input-Method-Editor-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Input Method Editor Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Input-Method-Editor-Repair'
    ...
    Export-HtmlReport -Title 'Input Method Editor Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Input Method Editor Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
