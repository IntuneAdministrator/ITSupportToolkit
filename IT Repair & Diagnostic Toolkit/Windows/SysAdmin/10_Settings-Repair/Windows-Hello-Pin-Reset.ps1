...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Hello Pin Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Hello-Pin-Reset.
...
.EXAMPLE
    .\Windows-Hello-Pin-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Hello-Pin-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Hello Pin Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Windows-Hello-Pin-Reset'
        Add-ReportRow -Check 'User' -Result $env:USERNAME -Status 'INFO' -Detail $env:USERPROFILE
        Write-ProgressStep 2 $totalSteps $Activity 'Profile context'
        $ngc = "$env:SystemRoot\ServiceProfiles\LocalService\AppData\Local\Microsoft\Ngc"
        Add-ReportRow -Check 'NGC folder' -Result $(if (Test-Path $ngc) {'Present'} else {'Absent'}) -Status 'INFO' -Detail 'PIN reset usually requires Settings > Accounts > Sign-in options'
        Add-ReportRow -Check 'Guidance' -Result 'Use Settings UI or dsregcmd /leave only with approval' -Status 'WARN' -Detail 'Destructive PIN reset not forced automatically'
        Write-ProgressStep 3 $totalSteps $Activity 'Hello/PIN'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Hello Pin Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Hello Pin Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
