...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Taskbar Start Menu Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Taskbar-Start-Menu-Repair.
...
.EXAMPLE
    .\Taskbar-Start-Menu-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Taskbar-Start-Menu-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Taskbar Start Menu Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Taskbar-Start-Menu-Repair'
    ...
    Export-HtmlReport -Title 'Taskbar Start Menu Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Taskbar Start Menu Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
