...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Recents Jump List Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Recents-Jump-List-Repair.
...
.EXAMPLE
    .\Recents-Jump-List-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Recents-Jump-List-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Recents Jump List Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Recents-Jump-List-Repair'
    ...
    Export-HtmlReport -Title 'Recents Jump List Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Recents Jump List Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
