...
<#
.SYNOPSIS
    Common problem fix: Settings Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Settings-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Settings-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Settings Repair Stack Reset'; $totalSteps = 8; $Name = 'Settings-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Settings Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Settings Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
