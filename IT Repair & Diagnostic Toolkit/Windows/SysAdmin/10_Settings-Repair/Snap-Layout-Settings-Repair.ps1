...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Snap Layout Settings Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Snap-Layout-Settings-Repair.
...
.EXAMPLE
    .\Snap-Layout-Settings-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Snap-Layout-Settings-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Snap Layout Settings Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SETTINGS REPAIR — Snap-Layout-Settings-Repair'
    ...
    Export-HtmlReport -Title 'Snap Layout Settings Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Snap Layout Settings Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
