...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Group Policy Force Refresh
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Group-Policy-Force-Refresh.
Category: 10_Settings-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Group-Policy-Force-Refresh.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Group-Policy-Force-Refresh.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Group Policy Force Refresh'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'JOIN STATE BRANCH'
        if ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
            Add-ReportRow -Check 'GPO applicability' -Result 'Domain/Hybrid' -Status 'PASS' -Detail $Join.Label
            Write-ProgressStep 2 $totalSteps $Activity 'gpupdate'
            if ($PSCmdlet.ShouldProcess('Group Policy', 'gpupdate /force')) {
                $out = & gpupdate.exe /force 2>&1 | Out-String
                $ok = $out -match 'successfully'
                Add-ReportRow -Check 'gpupdate /force' -Result $(if ($ok) {'Success'} else {'Partial'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail $out.Trim()
            }
            Write-ProgressStep 3 $totalSteps $Activity 'RSOP summary'
            try {
                $gpsvc = Get-Service gpsvc
                Add-ReportRow -Check 'Group Policy service' -Result $gpsvc.Status -Status $(if ($gpsvc.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail $gpsvc.StartType
            } catch {
                Add-ReportRow -Check 'Group Policy service' -Result 'Error' -Status 'FAIL' -Detail $_.Exception.Message
            }
        } elseif ($Join.IsAzureADJoined) {
            Add-ReportRow -Check 'GPO applicability' -Result 'Entra-only — use Intune/MDM' -Status 'WARN' -Detail 'Classic GPO not applied on cloud-only join'
            Write-ProgressStep 2 $totalSteps $Activity 'MDM sync hint'
            try {
                Start-Process -FilePath "$env:windir\system32\deviceenroller.exe" -ArgumentList '/c','/AutoEnrollMDM' -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'MDM re-enroll trigger' -Result 'Invoked' -Status 'INFO' -Detail 'deviceenroller.exe /c /AutoEnrollMDM'
            } catch {
                Add-ReportRow -Check 'MDM re-enroll trigger' -Result 'Skipped' -Status 'WARN' -Detail $_.Exception.Message
            }
        } else {
            Add-ReportRow -Check 'GPO applicability' -Result 'Workgroup' -Status 'WARN' -Detail 'No domain GPO; local policy only'
            if ($PSCmdlet.ShouldProcess('Local policy', 'gpupdate /force')) {
                $out = & gpupdate.exe /force 2>&1 | Out-String
                Add-ReportRow -Check 'Local gpupdate' -Result 'Ran' -Status 'INFO' -Detail $out.Trim()
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Complete'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Group Policy Force Refresh' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Group Policy Force Refresh' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
