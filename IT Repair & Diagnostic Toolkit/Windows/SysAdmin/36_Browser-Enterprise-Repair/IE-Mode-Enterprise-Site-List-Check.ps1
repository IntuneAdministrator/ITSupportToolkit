...
<#
.SYNOPSIS
    Enterprise IT tool: IE Mode Enterprise Site List Check
.DESCRIPTION
...
.EXAMPLE
    .\IE-Mode-Enterprise-Site-List-Check.ps1
.EXAMPLE
    .\IE-Mode-Enterprise-Site-List-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'IE Mode Enterprise Site List Check'; $totalSteps = 8; $Name = 'IE-Mode-Enterprise-Site-List-Check'
try {
    ...
    Export-HtmlReport -Title 'IE Mode Enterprise Site List Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IE Mode Enterprise Site List Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
