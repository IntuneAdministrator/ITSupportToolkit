...
<#
.SYNOPSIS
    Enterprise IT tool: Secure DNS Browser Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\Secure-DNS-Browser-Conflict-Fix.ps1
.EXAMPLE
    .\Secure-DNS-Browser-Conflict-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Secure DNS Browser Conflict Fix'; $totalSteps = 8; $Name = 'Secure-DNS-Browser-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'Secure DNS Browser Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Secure DNS Browser Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
