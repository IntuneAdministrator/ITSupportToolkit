...
<#
.SYNOPSIS
    Enterprise IT tool: Edge Enterprise Reset
.DESCRIPTION
...
.EXAMPLE
    .\Edge-Enterprise-Reset.ps1
.EXAMPLE
    .\Edge-Enterprise-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Edge Enterprise Reset'; $totalSteps = 8; $Name = 'Edge-Enterprise-Reset'
try {
    ...
    Export-HtmlReport -Title 'Edge Enterprise Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge Enterprise Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
