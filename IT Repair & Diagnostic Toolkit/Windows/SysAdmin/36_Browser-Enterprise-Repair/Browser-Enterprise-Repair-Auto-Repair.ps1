...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Auto Repair'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
