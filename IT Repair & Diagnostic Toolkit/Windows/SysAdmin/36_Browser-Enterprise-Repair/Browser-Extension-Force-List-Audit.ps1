...
<#
.SYNOPSIS
    Enterprise IT tool: Browser Extension Force List Audit
.DESCRIPTION
    Expansion module (SysAdmin / 36_Browser-Enterprise-Repair) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Browser-Extension-Force-List-Audit.ps1
.EXAMPLE
    .\Browser-Extension-Force-List-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Extension Force List Audit'; $totalSteps = 8; $Name = 'Browser-Extension-Force-List-Audit'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '36_Browser-Enterprise-Repair' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'BROWSER ENTERPRISE'
        $edge = "${env:ProgramFiles(x86)}\Microsoft\Edge\Application\msedge.exe"
        if (-not (Test-Path $edge)) { $edge = "$env:ProgramFiles\Microsoft\Edge\Application\msedge.exe" }
        Add-ReportRow -Check 'Edge' -Result $(if (Test-Path $edge) {'Installed'} else {'Missing'}) -Status $(if (Test-Path $edge) {'PASS'} else {'WARN'}) -Detail $edge
        Write-ProgressStep 2 $totalSteps $Activity 'Browser detect'
        if ($Name -match 'Edge' -and $PSCmdlet.ShouldProcess('Edge', 'Reset utility / kill + clear cache sample')) {
            ...
            if (Test-Path $cache) { Remove-Item "$cache\*" -Recurse -Force -EA SilentlyContinue; Add-ReportRow -Check 'Edge cache' -Result 'Cleared Default\Cache' -Status 'PASS' -Detail $cache }
        }
        if ($Name -match 'WebView2') {
            $wv = Get-ItemProperty 'HKLM:\SOFTWARE\WOW6432Node\Microsoft\EdgeUpdate\Clients\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}' -EA SilentlyContinue
            Add-ReportRow -Check 'WebView2 runtime' -Result $(if ($wv) {'Registered'} else {'Check install'}) -Status 'INFO' -Detail 'Evergreen WebView2'
        }
        if ($Name -match 'Chrome') {
            $chromePol = 'HKLM:\SOFTWARE\Policies\Google\Chrome'
            Add-ReportRow -Check 'Chrome policy key' -Result $(if (Test-Path $chromePol) {'Present'} else {'None'}) -Status 'INFO' -Detail $chromePol
        }
        if ($Name -match 'IE-Mode|Extension|Secure-DNS|PDF') {
            $edgePol = 'HKLM:\SOFTWARE\Policies\Microsoft\Edge'
            Add-ReportRow -Check 'Edge policy key' -Result $(if (Test-Path $edgePol) {'Present'} else {'None'}) -Status 'INFO' -Detail $edgePol
            Backup-RegistryKey -KeyPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\FileExts\.pdf' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Browser done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Browser Extension Force List Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Extension Force List Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
