...
<#
.SYNOPSIS
    Enterprise IT tool: Chrome Enterprise Policy Audit
.DESCRIPTION
...
.EXAMPLE
    .\Chrome-Enterprise-Policy-Audit.ps1
.EXAMPLE
    .\Chrome-Enterprise-Policy-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Chrome Enterprise Policy Audit'; $totalSteps = 8; $Name = 'Chrome-Enterprise-Policy-Audit'
try {
    ...
    Export-HtmlReport -Title 'Chrome Enterprise Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chrome Enterprise Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
