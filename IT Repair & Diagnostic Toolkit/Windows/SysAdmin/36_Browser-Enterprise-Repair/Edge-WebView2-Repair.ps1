...
<#
.SYNOPSIS
    Enterprise IT tool: Edge WebView2 Repair
.DESCRIPTION
...
.EXAMPLE
    .\Edge-WebView2-Repair.ps1
.EXAMPLE
    .\Edge-WebView2-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Edge WebView2 Repair'; $totalSteps = 8; $Name = 'Edge-WebView2-Repair'
try {
    ...
    Export-HtmlReport -Title 'Edge WebView2 Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Edge WebView2 Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
