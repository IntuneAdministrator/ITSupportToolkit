...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Health-Triage.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Health Triage'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
