...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 36_Browser-Enterprise-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Browser-Enterprise-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Policy Validate'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '36_Browser-Enterprise-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '36_Browser-Enterprise-Repair'
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
