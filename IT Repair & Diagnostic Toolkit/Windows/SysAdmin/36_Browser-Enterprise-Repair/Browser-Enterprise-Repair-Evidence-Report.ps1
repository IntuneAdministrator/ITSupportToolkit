...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Evidence Report'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
