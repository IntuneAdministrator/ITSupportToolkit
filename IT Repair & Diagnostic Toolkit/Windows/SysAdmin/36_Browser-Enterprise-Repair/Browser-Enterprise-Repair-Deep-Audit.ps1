...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Deep Audit'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
