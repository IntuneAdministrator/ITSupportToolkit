...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Quick Fix'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
