...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Stack Reset'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
