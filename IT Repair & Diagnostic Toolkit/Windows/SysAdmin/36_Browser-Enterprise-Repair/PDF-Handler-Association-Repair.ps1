...
<#
.SYNOPSIS
    Enterprise IT tool: PDF Handler Association Repair
.DESCRIPTION
...
.EXAMPLE
    .\PDF-Handler-Association-Repair.ps1
.EXAMPLE
    .\PDF-Handler-Association-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'PDF Handler Association Repair'; $totalSteps = 8; $Name = 'PDF-Handler-Association-Repair'
try {
    ...
    Export-HtmlReport -Title 'PDF Handler Association Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'PDF Handler Association Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
