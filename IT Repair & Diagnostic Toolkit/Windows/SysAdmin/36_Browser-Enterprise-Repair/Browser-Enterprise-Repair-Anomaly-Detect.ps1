...
<#
.SYNOPSIS
    Common problem fix: Browser Enterprise Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Enterprise-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Browser-Enterprise-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Enterprise Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Browser-Enterprise-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Browser Enterprise Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Enterprise Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
