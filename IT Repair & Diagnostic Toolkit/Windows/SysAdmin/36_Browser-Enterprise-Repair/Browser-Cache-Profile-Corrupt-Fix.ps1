...
<#
.SYNOPSIS
    Enterprise IT tool: Browser Cache Profile Corrupt Fix
.DESCRIPTION
...
.EXAMPLE
    .\Browser-Cache-Profile-Corrupt-Fix.ps1
.EXAMPLE
    .\Browser-Cache-Profile-Corrupt-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Browser Cache Profile Corrupt Fix'; $totalSteps = 8; $Name = 'Browser-Cache-Profile-Corrupt-Fix'
try {
    ...
    Export-HtmlReport -Title 'Browser Cache Profile Corrupt Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Browser Cache Profile Corrupt Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
