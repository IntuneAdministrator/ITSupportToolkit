...
<#
.SYNOPSIS
    Common problem fix: Hyper V Client Virt Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Hyper-V-Client-Virt-Deep-Audit.ps1
.EXAMPLE
    .\Hyper-V-Client-Virt-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Client Virt Deep Audit'; $totalSteps = 8; $Name = 'Hyper-V-Client-Virt-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Client Virt Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Client Virt Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
