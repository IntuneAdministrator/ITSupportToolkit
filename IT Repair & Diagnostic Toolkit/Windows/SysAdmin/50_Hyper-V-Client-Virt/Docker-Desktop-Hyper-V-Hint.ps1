...
<#
.SYNOPSIS
    Enterprise IT tool: Docker Desktop Hyper V Hint
.DESCRIPTION
    Phase 11 expansion (SysAdmin/50_Hyper-V-Client-Virt). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Docker-Desktop-Hyper-V-Hint.ps1
.EXAMPLE
    .\Docker-Desktop-Hyper-V-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Docker Desktop Hyper V Hint'; $totalSteps=8; $Name='Docker-Desktop-Hyper-V-Hint'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Export-HtmlReport -Title 'Docker Desktop Hyper V Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Docker Desktop Hyper V Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
