...
<#
.SYNOPSIS
    Enterprise IT tool: Nested Virtualization Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/50_Hyper-V-Client-Virt). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Nested-Virtualization-Check.ps1
.EXAMPLE
    .\Nested-Virtualization-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Nested Virtualization Check'; $totalSteps=8; $Name='Nested-Virtualization-Check'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Export-HtmlReport -Title 'Nested Virtualization Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Nested Virtualization Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
