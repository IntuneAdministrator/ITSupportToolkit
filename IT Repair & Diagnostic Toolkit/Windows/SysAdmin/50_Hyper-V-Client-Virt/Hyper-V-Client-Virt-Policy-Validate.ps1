...
<#
.SYNOPSIS
    Common problem fix: Hyper V Client Virt Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 50_Hyper-V-Client-Virt). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Hyper-V-Client-Virt-Policy-Validate.ps1
.EXAMPLE
    .\Hyper-V-Client-Virt-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hyper V Client Virt Policy Validate'; $totalSteps = 8; $Name = 'Hyper-V-Client-Virt-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '50_Hyper-V-Client-Virt' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '50_Hyper-V-Client-Virt'
    ...
    Export-HtmlReport -Title 'Hyper V Client Virt Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Client Virt Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
