...
$exitCode=0; $Activity='Hyper V Feature Status'; $totalSteps=8; $Name='Hyper-V-Feature-Status'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Export-HtmlReport -Title 'Hyper V Feature Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Feature Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
