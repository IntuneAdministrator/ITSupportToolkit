...
<#
.SYNOPSIS
    Enterprise IT tool: VM Switch External Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/50_Hyper-V-Client-Virt). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\VM-Switch-External-Audit.ps1
.EXAMPLE
    .\VM-Switch-External-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='VM Switch External Audit'; $totalSteps=8; $Name='VM-Switch-External-Audit'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Export-HtmlReport -Title 'VM Switch External Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VM Switch External Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
