...
<#
.SYNOPSIS
    Enterprise IT tool: WSL2 Hyper V Conflict Fix
.DESCRIPTION
    Phase 11 expansion (SysAdmin/50_Hyper-V-Client-Virt). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\WSL2-Hyper-V-Conflict-Fix.ps1
.EXAMPLE
    .\WSL2-Hyper-V-Conflict-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='WSL2 Hyper V Conflict Fix'; $totalSteps=8; $Name='WSL2-Hyper-V-Conflict-Fix'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Export-HtmlReport -Title 'WSL2 Hyper V Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSL2 Hyper V Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
