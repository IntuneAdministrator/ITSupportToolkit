...
<#
.SYNOPSIS
    Enterprise IT tool: Hyper V Host Compute Service
.DESCRIPTION
    Phase 11 expansion (SysAdmin/50_Hyper-V-Client-Virt). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Hyper-V-Host-Compute-Service.ps1
.EXAMPLE
    .\Hyper-V-Host-Compute-Service.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Hyper V Host Compute Service'; $totalSteps=8; $Name='Hyper-V-Host-Compute-Service'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '50_Hyper-V-Client-Virt' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Hyper-V feature' -Result $(if($hv){"$($hv.State)"}else{'Query failed'}) -Status 'INFO' -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'Virt stack'
        $vmms = Get-Service vmms -EA SilentlyContinue
        Add-ReportRow -Check 'vmms' -Result $(if($vmms){"$($vmms.Status)"}else{'Not installed'}) -Status 'INFO' -Detail ''
        if ($Name -match 'WSL2|Docker') {
            $wsl = wsl --status 2>&1 | Out-String
            Add-ReportRow -Check 'WSL' -Result 'Status captured' -Status 'INFO' -Detail $wsl.Substring(0,[Math]::Min(200,$wsl.Length))
        }
        if ($Name -match 'HVCI|Guard') {
            $dg = Get-CimInstance -ClassName Win32_DeviceGuard -Namespace root\Microsoft\Windows\DeviceGuard -EA SilentlyContinue
            Add-ReportRow -Check 'Device Guard / HVCI' -Result $(if($dg){"VBS=$($dg.VirtualizationBasedSecurityStatus)"}else{'N/A'}) -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Nested') {
            Add-ReportRow -Check 'Nested virt' -Result 'Requires firmware VT-x/AMD-V + Set-VMProcessor ExposeVirtualizationExtensions' -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Virt done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Hyper V Host Compute Service' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hyper V Host Compute Service' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
