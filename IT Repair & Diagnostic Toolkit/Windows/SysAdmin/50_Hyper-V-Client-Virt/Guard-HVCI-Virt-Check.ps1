...
$exitCode=0; $Activity='Guard HVCI Virt Check'; $totalSteps=8; $Name='Guard-HVCI-Virt-Check'; $Folder='50_Hyper-V-Client-Virt'
try {
    ...
    Export-HtmlReport -Title 'Guard HVCI Virt Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Guard HVCI Virt Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
