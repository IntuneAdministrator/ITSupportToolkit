...
<#
.SYNOPSIS
    Common problem fix: HP SSA Array Controller Audit
.DESCRIPTION
...
.EXAMPLE
    .\HP-SSA-Array-Controller-Audit.ps1
.EXAMPLE
    .\HP-SSA-Array-Controller-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HP SSA Array Controller Audit'; $totalSteps = 8; $Name = 'HP-SSA-Array-Controller-Audit'
try {
    ...
    Export-HtmlReport -Title 'HP SSA Array Controller Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HP SSA Array Controller Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
