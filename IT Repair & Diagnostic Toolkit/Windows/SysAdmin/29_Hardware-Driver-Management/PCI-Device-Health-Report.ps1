...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PCI Device Health Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PCI-Device-Health-Report.
...
.EXAMPLE
    .\PCI-Device-Health-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PCI-Device-Health-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PCI Device Health Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PCI-Device-Health-Report'
        Add-ReportRow -Check 'Tool' -Result 'PCI-Device-Health-Report' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'PCI Device Health Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PCI Device Health Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
