...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Driver Update History Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Driver-Update-History-Report.
...
.EXAMPLE
    .\Driver-Update-History-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Driver-Update-History-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Driver Update History Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Driver-Update-History-Report'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Update-History-Report' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Driver Update History Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Update History Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
