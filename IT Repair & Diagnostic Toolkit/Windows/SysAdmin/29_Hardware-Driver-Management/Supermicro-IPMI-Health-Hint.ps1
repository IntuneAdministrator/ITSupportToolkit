...
<#
.SYNOPSIS
    Common problem fix: Supermicro IPMI Health Hint
.DESCRIPTION
...
.EXAMPLE
    .\Supermicro-IPMI-Health-Hint.ps1
.EXAMPLE
    .\Supermicro-IPMI-Health-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Supermicro IPMI Health Hint'; $totalSteps = 8; $Name = 'Supermicro-IPMI-Health-Hint'
try {
    ...
    Export-HtmlReport -Title 'Supermicro IPMI Health Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Supermicro IPMI Health Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
