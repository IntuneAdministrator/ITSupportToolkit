...
<#
.SYNOPSIS
    Common problem fix: Runbook Hardware Pack F G H Deploy
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-Hardware-Pack-F-G-H-Deploy.ps1
.EXAMPLE
    .\Runbook-Hardware-Pack-F-G-H-Deploy.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Hardware Pack F G H Deploy'; $totalSteps = 8; $Name = 'Runbook-Hardware-Pack-F-G-H-Deploy'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '29_Hardware-Driver-Management' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '29_Hardware-Driver-Management'
            }
    ...
    Export-HtmlReport -Title 'Runbook Hardware Pack F G H Deploy' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Hardware Pack F G H Deploy' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
