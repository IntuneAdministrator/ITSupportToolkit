...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Quick-Fix.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Quick Fix'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
