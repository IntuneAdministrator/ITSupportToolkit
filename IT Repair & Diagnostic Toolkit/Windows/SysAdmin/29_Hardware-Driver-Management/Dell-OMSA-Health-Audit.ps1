...
<#
.SYNOPSIS
    Common problem fix: Dell OMSA Health Audit
.DESCRIPTION
...
.EXAMPLE
    .\Dell-OMSA-Health-Audit.ps1
.EXAMPLE
    .\Dell-OMSA-Health-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell OMSA Health Audit'; $totalSteps = 8; $Name = 'Dell-OMSA-Health-Audit'
try {
    ...
    Export-HtmlReport -Title 'Dell OMSA Health Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell OMSA Health Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
