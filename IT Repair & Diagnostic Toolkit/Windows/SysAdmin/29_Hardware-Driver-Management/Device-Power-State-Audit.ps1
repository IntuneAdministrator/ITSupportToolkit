...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Device Power State Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Device-Power-State-Audit.
...
.EXAMPLE
    .\Device-Power-State-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Device-Power-State-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Device Power State Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Device-Power-State-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Device-Power-State-Audit' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Device Power State Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Device Power State Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
