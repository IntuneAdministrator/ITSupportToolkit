...
<#
.SYNOPSIS
    Common problem fix: Getac Rugged Device Hint
.DESCRIPTION
...
.EXAMPLE
    .\Getac-Rugged-Device-Hint.ps1
.EXAMPLE
    .\Getac-Rugged-Device-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Getac Rugged Device Hint'; $totalSteps = 8; $Name = 'Getac-Rugged-Device-Hint'
try {
    ...
    Export-HtmlReport -Title 'Getac Rugged Device Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Getac Rugged Device Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
