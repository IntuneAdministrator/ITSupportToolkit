...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Unknown Device Finder
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Unknown-Device-Finder.
...
$exitCode = 0
$Activity = 'Unknown Device Finder'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Unknown-Device-Finder'
        Add-ReportRow -Check 'Tool' -Result 'Unknown-Device-Finder' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Unknown Device Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Unknown Device Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
