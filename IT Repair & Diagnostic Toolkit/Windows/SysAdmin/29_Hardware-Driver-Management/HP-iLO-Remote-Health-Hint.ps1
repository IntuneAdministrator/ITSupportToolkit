...
<#
.SYNOPSIS
    Common problem fix: HP iLO Remote Health Hint
.DESCRIPTION
...
.EXAMPLE
    .\HP-iLO-Remote-Health-Hint.ps1
.EXAMPLE
    .\HP-iLO-Remote-Health-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HP iLO Remote Health Hint'; $totalSteps = 8; $Name = 'HP-iLO-Remote-Health-Hint'
try {
    ...
    Export-HtmlReport -Title 'HP iLO Remote Health Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HP iLO Remote Health Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
