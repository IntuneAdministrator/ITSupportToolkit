...
<#
.SYNOPSIS
    Common problem fix: Surface Device Driver Pack Hint
.DESCRIPTION
...
.EXAMPLE
    .\Surface-Device-Driver-Pack-Hint.ps1
.EXAMPLE
    .\Surface-Device-Driver-Pack-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Surface Device Driver Pack Hint'; $totalSteps = 8; $Name = 'Surface-Device-Driver-Pack-Hint'
try {
    ...
    Export-HtmlReport -Title 'Surface Device Driver Pack Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Surface Device Driver Pack Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
