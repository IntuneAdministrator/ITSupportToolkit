...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Auto-Repair.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Auto Repair'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
