...
<#
.SYNOPSIS
    Common problem fix: Panasonic Toughbook Driver Hint
.DESCRIPTION
...
.EXAMPLE
    .\Panasonic-Toughbook-Driver-Hint.ps1
.EXAMPLE
    .\Panasonic-Toughbook-Driver-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Panasonic Toughbook Driver Hint'; $totalSteps = 8; $Name = 'Panasonic-Toughbook-Driver-Hint'
try {
    ...
    Export-HtmlReport -Title 'Panasonic Toughbook Driver Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Panasonic Toughbook Driver Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
