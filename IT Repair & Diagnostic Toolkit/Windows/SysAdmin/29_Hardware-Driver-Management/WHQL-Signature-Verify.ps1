...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WHQL Signature Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WHQL-Signature-Verify.
...
$exitCode = 0
$Activity = 'WHQL Signature Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WHQL-Signature-Verify'
        Add-ReportRow -Check 'Tool' -Result 'WHQL-Signature-Verify' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'WHQL Signature Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WHQL Signature Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
