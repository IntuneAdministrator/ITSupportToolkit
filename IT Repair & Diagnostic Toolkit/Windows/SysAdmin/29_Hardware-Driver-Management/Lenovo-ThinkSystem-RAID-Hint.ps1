...
<#
.SYNOPSIS
    Common problem fix: Lenovo ThinkSystem RAID Hint
.DESCRIPTION
...
.EXAMPLE
    .\Lenovo-ThinkSystem-RAID-Hint.ps1
.EXAMPLE
    .\Lenovo-ThinkSystem-RAID-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lenovo ThinkSystem RAID Hint'; $totalSteps = 8; $Name = 'Lenovo-ThinkSystem-RAID-Hint'
try {
    ...
    Export-HtmlReport -Title 'Lenovo ThinkSystem RAID Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lenovo ThinkSystem RAID Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
