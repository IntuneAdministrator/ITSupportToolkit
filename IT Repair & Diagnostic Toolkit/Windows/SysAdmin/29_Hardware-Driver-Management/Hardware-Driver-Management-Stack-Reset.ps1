...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Stack-Reset.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Stack Reset'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
