...
<#
.SYNOPSIS
    Common problem fix: Fujitsu ServerView Hint
.DESCRIPTION
...
.EXAMPLE
    .\Fujitsu-ServerView-Hint.ps1
.EXAMPLE
    .\Fujitsu-ServerView-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Fujitsu ServerView Hint'; $totalSteps = 8; $Name = 'Fujitsu-ServerView-Hint'
try {
    ...
    Export-HtmlReport -Title 'Fujitsu ServerView Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Fujitsu ServerView Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
