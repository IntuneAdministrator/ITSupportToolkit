...
<#
.SYNOPSIS
    Common problem fix: BMC Network Reachability Test
.DESCRIPTION
...
.EXAMPLE
    .\BMC-Network-Reachability-Test.ps1
.EXAMPLE
    .\BMC-Network-Reachability-Test.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BMC Network Reachability Test'; $totalSteps = 8; $Name = 'BMC-Network-Reachability-Test'
try {
    ...
    Export-HtmlReport -Title 'BMC Network Reachability Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BMC Network Reachability Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
