...
<#
.SYNOPSIS
    Common problem fix: HP Image Assistant Hint
.DESCRIPTION
...
.EXAMPLE
    .\HP-Image-Assistant-Hint.ps1
.EXAMPLE
    .\HP-Image-Assistant-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HP Image Assistant Hint'; $totalSteps = 8; $Name = 'HP-Image-Assistant-Hint'
try {
    ...
    Export-HtmlReport -Title 'HP Image Assistant Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HP Image Assistant Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
