...
<#
.SYNOPSIS
    Common problem fix: Dell iDRAC Remote Health Hint
.DESCRIPTION
...
.EXAMPLE
    .\Dell-iDRAC-Remote-Health-Hint.ps1
.EXAMPLE
    .\Dell-iDRAC-Remote-Health-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell iDRAC Remote Health Hint'; $totalSteps = 8; $Name = 'Dell-iDRAC-Remote-Health-Hint'
try {
    ...
    Export-HtmlReport -Title 'Dell iDRAC Remote Health Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell iDRAC Remote Health Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
