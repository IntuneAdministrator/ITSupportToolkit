...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Driver Conflict Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Driver-Conflict-Detector.
...
.EXAMPLE
    .\Driver-Conflict-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Driver-Conflict-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Driver Conflict Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Driver-Conflict-Detector'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Conflict-Detector' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Driver Conflict Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Conflict Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
