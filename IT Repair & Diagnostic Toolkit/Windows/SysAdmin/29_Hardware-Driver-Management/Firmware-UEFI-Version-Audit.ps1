...
<#
.SYNOPSIS
    Common problem fix: Firmware UEFI Version Audit
.DESCRIPTION
...
.EXAMPLE
    .\Firmware-UEFI-Version-Audit.ps1
.EXAMPLE
    .\Firmware-UEFI-Version-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Firmware UEFI Version Audit'; $totalSteps = 8; $Name = 'Firmware-UEFI-Version-Audit'
try {
    ...
    Export-HtmlReport -Title 'Firmware UEFI Version Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Firmware UEFI Version Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
