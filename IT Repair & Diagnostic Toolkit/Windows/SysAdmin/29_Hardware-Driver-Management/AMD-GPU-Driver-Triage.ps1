...
<#
.SYNOPSIS
    Common problem fix: AMD GPU Driver Triage
.DESCRIPTION
...
.EXAMPLE
    .\AMD-GPU-Driver-Triage.ps1
.EXAMPLE
    .\AMD-GPU-Driver-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AMD GPU Driver Triage'; $totalSteps = 8; $Name = 'AMD-GPU-Driver-Triage'
try {
    ...
    Export-HtmlReport -Title 'AMD GPU Driver Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AMD GPU Driver Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
