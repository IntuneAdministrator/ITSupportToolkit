...
<#
.SYNOPSIS
    Common problem fix: Intel vPro AMT Status Audit
.DESCRIPTION
...
.EXAMPLE
    .\Intel-vPro-AMT-Status-Audit.ps1
.EXAMPLE
    .\Intel-vPro-AMT-Status-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intel vPro AMT Status Audit'; $totalSteps = 8; $Name = 'Intel-vPro-AMT-Status-Audit'
try {
    ...
    Export-HtmlReport -Title 'Intel vPro AMT Status Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intel vPro AMT Status Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
