...
<#
.SYNOPSIS
    Common problem fix: Dell SupportAssist Enterprise Audit
.DESCRIPTION
...
.EXAMPLE
    .\Dell-SupportAssist-Enterprise-Audit.ps1
.EXAMPLE
    .\Dell-SupportAssist-Enterprise-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell SupportAssist Enterprise Audit'; $totalSteps = 8; $Name = 'Dell-SupportAssist-Enterprise-Audit'
try {
    ...
    Export-HtmlReport -Title 'Dell SupportAssist Enterprise Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell SupportAssist Enterprise Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
