...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Thunderbolt Security Level Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Thunderbolt-Security-Level-Audit.
Category: 29_Hardware-Driver-Management. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Thunderbolt-Security-Level-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Thunderbolt-Security-Level-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Thunderbolt Security Level Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'HARDWARE DRIVER MANAGEMENT'
        $Name = 'Thunderbolt-Security-Level-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Thunderbolt-Security-Level-Audit' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
        Write-SectionBanner 'HARDWARE / DRIVERS'
        $problem = @(Get-PnpDevice -ErrorAction SilentlyContinue | Where-Object { $_.Status -ne 'OK' })
        Add-ReportRow -Check 'Problem devices' -Result "$($problem.Count)" -Status $(if ($problem.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ''
        foreach ($d in ($problem | Select-Object -First 25)) {
            Add-ReportRow -Check $d.FriendlyName -Result "$($d.Status) [$($d.ConfigManagerErrorCode)]" -Status 'WARN' -Detail "$($d.Class) $($d.InstanceId)"
        }
        Write-ProgressStep 2 $totalSteps $Activity 'PnP problems'

        if ($Name -match 'PnP-Device-Rescan|Unknown-Device|Device-Manager|Driver-Rollback|Driver-Conflict|Driver-Store|INF|WHQL|Hardware-ID') {
            if ($Name -match 'Rescan' -and $PSCmdlet.ShouldProcess('PnP', 'Scan for hardware changes')) {
                & pnputil /scan-devices 2>&1 | Out-Null
                Add-ReportRow -Check 'pnputil /scan-devices' -Result 'Done' -Status 'PASS' -Detail ''
            }
            if ($Name -match 'Unknown') {
                $unk = @($problem | Where-Object { $_.FriendlyName -match 'Unknown|Other devices' -or $_.Class -eq 'Unknown' })
                Add-ReportRow -Check 'Unknown devices' -Result "$($unk.Count)" -Status $(if ($unk.Count -eq 0) {'PASS'} else {'WARN'}) -Detail (($unk | ForEach-Object InstanceId) -join '; ')
            }
            if ($Name -match 'Driver-Store|Cleanup' -and $PSCmdlet.ShouldProcess('Driver store', 'pnputil /enum-drivers')) {
                $drv = & pnputil /enum-drivers 2>&1 | Out-String
                Add-ReportRow -Check 'Driver store enum' -Result 'Captured' -Status 'INFO' -Detail ($drv.Substring(0, [Math]::Min(400, $drv.Length)))
            }
        }

        if ($Name -match 'BIOS|Firmware|Hardware-Inventory|PCI|Thunderbolt|Power-State|Update-History|Verifier') {
            $cs = Get-CimInstance Win32_ComputerSystem
            $bios = Get-CimInstance Win32_BIOS
            Add-ReportRow -Check 'System' -Result "$($cs.Manufacturer) $($cs.Model)" -Status 'INFO' -Detail "Serial=$($bios.SerialNumber)"
            Add-ReportRow -Check 'BIOS' -Result $bios.SMBIOSBIOSVersion -Status 'INFO' -Detail $bios.ReleaseDate
            if ($Name -match 'Inventory|PCI') {
                Get-CimInstance Win32_PnPEntity | Where-Object { $_.PNPClass -eq 'System' -or $_.Name -match 'PCI' } | Select-Object -First 20 | ForEach-Object {
                    Add-ReportRow -Check $_.Name -Result $_.Status -Status 'INFO' -Detail $_.DeviceID
                }
            }
            if ($Name -match 'Verifier') {
                $vq = & verifier /query 2>&1 | Out-String
                Add-ReportRow -Check 'Driver Verifier' -Result 'Query' -Status 'INFO' -Detail ($vq.Substring(0, [Math]::Min(300, $vq.Length)))
            }
            if ($Name -match 'Hardware-Inventory') {
                $out = Join-Path 'C:\IT-Logs' ("HardwareInventory_{0}.csv" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
                ...
                Add-ReportRow -Check 'Inventory export' -Result 'CSV saved' -Status 'PASS' -Detail $out
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Hardware'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Thunderbolt Security Level Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Thunderbolt Security Level Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
