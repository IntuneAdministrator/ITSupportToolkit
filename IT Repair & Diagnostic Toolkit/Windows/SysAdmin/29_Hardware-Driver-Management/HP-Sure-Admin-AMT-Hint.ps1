...
<#
.SYNOPSIS
    Common problem fix: HP Sure Admin AMT Hint
.DESCRIPTION
...
.EXAMPLE
    .\HP-Sure-Admin-AMT-Hint.ps1
.EXAMPLE
    .\HP-Sure-Admin-AMT-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HP Sure Admin AMT Hint'; $totalSteps = 8; $Name = 'HP-Sure-Admin-AMT-Hint'
try {
    ...
    Export-HtmlReport -Title 'HP Sure Admin AMT Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HP Sure Admin AMT Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
