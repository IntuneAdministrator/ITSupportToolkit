...
<#
.SYNOPSIS
    Common problem fix: Storage NVMe Temperature Audit
.DESCRIPTION
...
.EXAMPLE
    .\Storage-NVMe-Temperature-Audit.ps1
.EXAMPLE
    .\Storage-NVMe-Temperature-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Storage NVMe Temperature Audit'; $totalSteps = 8; $Name = 'Storage-NVMe-Temperature-Audit'
try {
    ...
    Export-HtmlReport -Title 'Storage NVMe Temperature Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Storage NVMe Temperature Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
