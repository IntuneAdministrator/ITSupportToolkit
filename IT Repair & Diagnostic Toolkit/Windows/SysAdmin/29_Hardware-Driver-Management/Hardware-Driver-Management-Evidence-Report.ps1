...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Evidence-Report.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Evidence Report'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
