...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Driver Verifier Config
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Driver-Verifier-Config.
...
$exitCode = 0
$Activity = 'Driver Verifier Config'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Driver-Verifier-Config'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Verifier-Config' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Driver Verifier Config' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Verifier Config' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
