...
<#
.SYNOPSIS
    Common problem fix: Intel ARC GPU Status
.DESCRIPTION
...
.EXAMPLE
    .\Intel-ARC-GPU-Status.ps1
.EXAMPLE
    .\Intel-ARC-GPU-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intel ARC GPU Status'; $totalSteps = 8; $Name = 'Intel-ARC-GPU-Status'
try {
    ...
    Export-HtmlReport -Title 'Intel ARC GPU Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intel ARC GPU Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
