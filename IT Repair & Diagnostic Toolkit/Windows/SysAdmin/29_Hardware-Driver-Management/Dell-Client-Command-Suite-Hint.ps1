...
<#
.SYNOPSIS
    Common problem fix: Dell Client Command Suite Hint
.DESCRIPTION
...
.EXAMPLE
    .\Dell-Client-Command-Suite-Hint.ps1
.EXAMPLE
    .\Dell-Client-Command-Suite-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell Client Command Suite Hint'; $totalSteps = 8; $Name = 'Dell-Client-Command-Suite-Hint'
try {
    ...
    Export-HtmlReport -Title 'Dell Client Command Suite Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell Client Command Suite Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
