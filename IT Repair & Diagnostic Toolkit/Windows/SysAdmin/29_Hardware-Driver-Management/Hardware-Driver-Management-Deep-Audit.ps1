...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Deep-Audit.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Deep Audit'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
