...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Policy-Validate.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Policy Validate'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
