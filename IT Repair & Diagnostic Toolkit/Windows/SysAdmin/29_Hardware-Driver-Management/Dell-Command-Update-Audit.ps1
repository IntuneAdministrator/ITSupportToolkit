...
<#
.SYNOPSIS
    Common problem fix: Dell Command Update Audit
.DESCRIPTION
...
.EXAMPLE
    .\Dell-Command-Update-Audit.ps1
.EXAMPLE
    .\Dell-Command-Update-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell Command Update Audit'; $totalSteps = 8; $Name = 'Dell-Command-Update-Audit'
try {
    ...
    Export-HtmlReport -Title 'Dell Command Update Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell Command Update Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
