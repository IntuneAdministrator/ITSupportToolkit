...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Hardware Inventory Full Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Hardware-Inventory-Full-Export.
...
.EXAMPLE
    .\Hardware-Inventory-Full-Export.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Hardware-Inventory-Full-Export.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Hardware Inventory Full Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Hardware-Inventory-Full-Export'
        Add-ReportRow -Check 'Tool' -Result 'Hardware-Inventory-Full-Export' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Hardware Inventory Full Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Hardware Inventory Full Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
