...
<#
.SYNOPSIS
    Common problem fix: Lenovo System Update Audit
.DESCRIPTION
...
.EXAMPLE
    .\Lenovo-System-Update-Audit.ps1
.EXAMPLE
    .\Lenovo-System-Update-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lenovo System Update Audit'; $totalSteps = 8; $Name = 'Lenovo-System-Update-Audit'
try {
    ...
    Export-HtmlReport -Title 'Lenovo System Update Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lenovo System Update Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
