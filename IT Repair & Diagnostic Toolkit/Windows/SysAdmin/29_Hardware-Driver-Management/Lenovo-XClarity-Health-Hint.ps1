...
<#
.SYNOPSIS
    Common problem fix: Lenovo XClarity Health Hint
.DESCRIPTION
...
.EXAMPLE
    .\Lenovo-XClarity-Health-Hint.ps1
.EXAMPLE
    .\Lenovo-XClarity-Health-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lenovo XClarity Health Hint'; $totalSteps = 8; $Name = 'Lenovo-XClarity-Health-Hint'
try {
    ...
    Export-HtmlReport -Title 'Lenovo XClarity Health Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lenovo XClarity Health Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
