...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Device Manager Error Scan
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Device-Manager-Error-Scan.
...
.EXAMPLE
    .\Device-Manager-Error-Scan.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Device-Manager-Error-Scan.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Device Manager Error Scan'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Device-Manager-Error-Scan'
        Add-ReportRow -Check 'Tool' -Result 'Device-Manager-Error-Scan' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'Device Manager Error Scan' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Device Manager Error Scan' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
