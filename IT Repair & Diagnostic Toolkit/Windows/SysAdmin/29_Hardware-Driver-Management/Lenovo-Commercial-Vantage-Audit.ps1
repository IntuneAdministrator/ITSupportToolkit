...
<#
.SYNOPSIS
    Common problem fix: Lenovo Commercial Vantage Audit
.DESCRIPTION
...
.EXAMPLE
    .\Lenovo-Commercial-Vantage-Audit.ps1
.EXAMPLE
    .\Lenovo-Commercial-Vantage-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lenovo Commercial Vantage Audit'; $totalSteps = 8; $Name = 'Lenovo-Commercial-Vantage-Audit'
try {
    ...
    Export-HtmlReport -Title 'Lenovo Commercial Vantage Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lenovo Commercial Vantage Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
