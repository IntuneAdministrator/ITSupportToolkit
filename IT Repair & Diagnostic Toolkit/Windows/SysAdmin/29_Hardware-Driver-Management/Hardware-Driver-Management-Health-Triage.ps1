...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Health-Triage.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Health Triage'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
