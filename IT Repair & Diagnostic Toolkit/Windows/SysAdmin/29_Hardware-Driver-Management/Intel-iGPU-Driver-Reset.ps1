...
<#
.SYNOPSIS
    Common problem fix: Intel iGPU Driver Reset
.DESCRIPTION
...
.EXAMPLE
    .\Intel-iGPU-Driver-Reset.ps1
.EXAMPLE
    .\Intel-iGPU-Driver-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intel iGPU Driver Reset'; $totalSteps = 8; $Name = 'Intel-iGPU-Driver-Reset'
try {
    ...
    Export-HtmlReport -Title 'Intel iGPU Driver Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intel iGPU Driver Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
