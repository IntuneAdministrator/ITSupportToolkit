...
<#
.SYNOPSIS
    Common problem fix: Intel MEBx Reset Hint
.DESCRIPTION
...
.EXAMPLE
    .\Intel-MEBx-Reset-Hint.ps1
.EXAMPLE
    .\Intel-MEBx-Reset-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intel MEBx Reset Hint'; $totalSteps = 8; $Name = 'Intel-MEBx-Reset-Hint'
try {
    ...
    Export-HtmlReport -Title 'Intel MEBx Reset Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intel MEBx Reset Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
