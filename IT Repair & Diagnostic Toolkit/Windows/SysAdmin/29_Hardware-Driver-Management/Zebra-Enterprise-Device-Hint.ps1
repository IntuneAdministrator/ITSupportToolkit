...
<#
.SYNOPSIS
    Common problem fix: Zebra Enterprise Device Hint
.DESCRIPTION
...
.EXAMPLE
    .\Zebra-Enterprise-Device-Hint.ps1
.EXAMPLE
    .\Zebra-Enterprise-Device-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Zebra Enterprise Device Hint'; $totalSteps = 8; $Name = 'Zebra-Enterprise-Device-Hint'
try {
    ...
    Export-HtmlReport -Title 'Zebra Enterprise Device Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Zebra Enterprise Device Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
