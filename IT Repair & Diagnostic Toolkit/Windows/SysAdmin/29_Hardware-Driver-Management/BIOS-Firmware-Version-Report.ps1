...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: BIOS Firmware Version Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for BIOS-Firmware-Version-Report.
...
.EXAMPLE
    .\BIOS-Firmware-Version-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\BIOS-Firmware-Version-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'BIOS Firmware Version Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'BIOS-Firmware-Version-Report'
        Add-ReportRow -Check 'Tool' -Result 'BIOS-Firmware-Version-Report' -Status 'INFO' -Detail '29_Hardware-Driver-Management'
    ...
    Export-HtmlReport -Title 'BIOS Firmware Version Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'BIOS Firmware Version Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
