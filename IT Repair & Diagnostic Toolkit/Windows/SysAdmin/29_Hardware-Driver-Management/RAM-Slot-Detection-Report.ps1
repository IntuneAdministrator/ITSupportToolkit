...
<#
.SYNOPSIS
    Common problem fix: RAM Slot Detection Report
.DESCRIPTION
...
.EXAMPLE
    .\RAM-Slot-Detection-Report.ps1
.EXAMPLE
    .\RAM-Slot-Detection-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RAM Slot Detection Report'; $totalSteps = 8; $Name = 'RAM-Slot-Detection-Report'
try {
    ...
    Export-HtmlReport -Title 'RAM Slot Detection Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RAM Slot Detection Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
