...
<#
.SYNOPSIS
    Common problem fix: TPM 2 Chip Health Audit
.DESCRIPTION
...
.EXAMPLE
    .\TPM-2-Chip-Health-Audit.ps1
.EXAMPLE
    .\TPM-2-Chip-Health-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'TPM 2 Chip Health Audit'; $totalSteps = 8; $Name = 'TPM-2-Chip-Health-Audit'
try {
    ...
    Export-HtmlReport -Title 'TPM 2 Chip Health Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'TPM 2 Chip Health Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
