...
<#
.SYNOPSIS
    Common problem fix: NVIDIA GPU Health Report
.DESCRIPTION
...
.EXAMPLE
    .\NVIDIA-GPU-Health-Report.ps1
.EXAMPLE
    .\NVIDIA-GPU-Health-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NVIDIA GPU Health Report'; $totalSteps = 8; $Name = 'NVIDIA-GPU-Health-Report'
try {
    ...
    Export-HtmlReport -Title 'NVIDIA GPU Health Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NVIDIA GPU Health Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
