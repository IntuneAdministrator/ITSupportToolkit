...
<#
.SYNOPSIS
    Common problem fix: Hardware Driver Management Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Driver-Management-Anomaly-Detect.ps1
.EXAMPLE
    .\Hardware-Driver-Management-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Driver Management Anomaly Detect'; $totalSteps = 8; $Name = 'Hardware-Driver-Management-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Hardware Driver Management Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Driver Management Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
