...
<#
.SYNOPSIS
    Enterprise IT tool: IKE EAP Cert VPN Check
.DESCRIPTION
...
.EXAMPLE
    .\IKE-EAP-Cert-VPN-Check.ps1
.EXAMPLE
    .\IKE-EAP-Cert-VPN-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'IKE EAP Cert VPN Check'; $totalSteps = 8; $Name = 'IKE-EAP-Cert-VPN-Check'
try {
    ...
    Export-HtmlReport -Title 'IKE EAP Cert VPN Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IKE EAP Cert VPN Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
