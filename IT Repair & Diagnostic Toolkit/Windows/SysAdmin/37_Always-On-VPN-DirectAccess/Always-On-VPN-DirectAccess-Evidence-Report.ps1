...
<#
.SYNOPSIS
    Common problem fix: Always On VPN DirectAccess Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 37_Always-On-VPN-DirectAccess). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Always-On-VPN-DirectAccess-Evidence-Report.ps1
.EXAMPLE
    .\Always-On-VPN-DirectAccess-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Always On VPN DirectAccess Evidence Report'; $totalSteps = 8; $Name = 'Always-On-VPN-DirectAccess-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '37_Always-On-VPN-DirectAccess' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '37_Always-On-VPN-DirectAccess'
    ...
    Export-HtmlReport -Title 'Always On VPN DirectAccess Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Always On VPN DirectAccess Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
