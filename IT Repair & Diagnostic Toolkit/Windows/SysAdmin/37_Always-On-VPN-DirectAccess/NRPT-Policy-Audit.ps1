...
<#
.SYNOPSIS
    Enterprise IT tool: NRPT Policy Audit
.DESCRIPTION
...
.EXAMPLE
    .\NRPT-Policy-Audit.ps1
.EXAMPLE
    .\NRPT-Policy-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NRPT Policy Audit'; $totalSteps = 8; $Name = 'NRPT-Policy-Audit'
try {
    ...
    Export-HtmlReport -Title 'NRPT Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NRPT Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
