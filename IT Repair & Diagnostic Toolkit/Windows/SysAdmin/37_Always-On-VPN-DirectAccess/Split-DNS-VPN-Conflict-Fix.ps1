...
<#
.SYNOPSIS
    Enterprise IT tool: Split DNS VPN Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\Split-DNS-VPN-Conflict-Fix.ps1
.EXAMPLE
    .\Split-DNS-VPN-Conflict-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Split DNS VPN Conflict Fix'; $totalSteps = 8; $Name = 'Split-DNS-VPN-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'Split DNS VPN Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Split DNS VPN Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
