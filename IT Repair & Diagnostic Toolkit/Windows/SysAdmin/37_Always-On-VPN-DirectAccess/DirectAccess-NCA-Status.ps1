...
<#
.SYNOPSIS
    Enterprise IT tool: DirectAccess NCA Status
.DESCRIPTION
...
.EXAMPLE
    .\DirectAccess-NCA-Status.ps1
.EXAMPLE
    .\DirectAccess-NCA-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DirectAccess NCA Status'; $totalSteps = 8; $Name = 'DirectAccess-NCA-Status'
try {
    ...
    Export-HtmlReport -Title 'DirectAccess NCA Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DirectAccess NCA Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
