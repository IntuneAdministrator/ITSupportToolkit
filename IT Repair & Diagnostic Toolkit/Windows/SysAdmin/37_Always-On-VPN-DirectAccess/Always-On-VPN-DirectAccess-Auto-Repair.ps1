...
<#
.SYNOPSIS
    Common problem fix: Always On VPN DirectAccess Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Always-On-VPN-DirectAccess-Auto-Repair.ps1
.EXAMPLE
    .\Always-On-VPN-DirectAccess-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Always On VPN DirectAccess Auto Repair'; $totalSteps = 8; $Name = 'Always-On-VPN-DirectAccess-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Always On VPN DirectAccess Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Always On VPN DirectAccess Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
