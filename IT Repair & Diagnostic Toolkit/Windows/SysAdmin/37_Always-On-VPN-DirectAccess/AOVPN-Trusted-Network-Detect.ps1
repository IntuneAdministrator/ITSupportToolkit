...
<#
.SYNOPSIS
    Enterprise IT tool: AOVPN Trusted Network Detect
.DESCRIPTION
...
.EXAMPLE
    .\AOVPN-Trusted-Network-Detect.ps1
.EXAMPLE
    .\AOVPN-Trusted-Network-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AOVPN Trusted Network Detect'; $totalSteps = 8; $Name = 'AOVPN-Trusted-Network-Detect'
try {
    ...
    Export-HtmlReport -Title 'AOVPN Trusted Network Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AOVPN Trusted Network Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
