...
<#
.SYNOPSIS
    Enterprise IT tool: Always On VPN Profile Repair
.DESCRIPTION
    Expansion module (SysAdmin / 37_Always-On-VPN-DirectAccess) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\Always-On-VPN-Profile-Repair.ps1
.EXAMPLE
    .\Always-On-VPN-Profile-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Always On VPN Profile Repair'; $totalSteps = 8; $Name = 'Always-On-VPN-Profile-Repair'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '37_Always-On-VPN-DirectAccess' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'ALWAYS ON VPN / DIRECTACCESS'
        $vpns = @(Get-VpnConnection -AllUserConnection -EA SilentlyContinue) + @(Get-VpnConnection -EA SilentlyContinue)
        Add-ReportRow -Check 'VPN profiles' -Result "$($vpns.Count)" -Status 'INFO' -Detail (($vpns | ForEach-Object Name) -join '; ')
        Write-ProgressStep 2 $totalSteps $Activity 'VPN profiles'
        Get-Service RasMan,EapHost,PolicyAgent -EA SilentlyContinue | ForEach-Object {
            Add-ReportRow -Check $_.Name -Result "$($_.Status)/$($_.StartType)" -Status $(if ($_.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail $_.DisplayName
        }
        if ($Name -match 'Repair|Device-Tunnel|Split|NRPT' -and $PSCmdlet.ShouldProcess('RasMan', 'Restart VPN stack')) {
            Restart-Service RasMan -Force -EA SilentlyContinue
            Add-ReportRow -Check 'RasMan restart' -Result 'Done' -Status 'PASS' -Detail ''
        }
        if ($Name -match 'NRPT|Split-DNS') {
            try {
                $nrpt = Get-DnsClientNrptPolicy -EA SilentlyContinue
                Add-ReportRow -Check 'NRPT policies' -Result "$(@($nrpt).Count)" -Status 'INFO' -Detail ''
            } catch { Add-ReportRow -Check 'NRPT' -Result 'N/A' -Status 'INFO' -Detail $_.Exception.Message }
        }
        if ($Name -match 'DirectAccess|NCA') {
            $nca = Get-Service NcaSvc -EA SilentlyContinue
            Add-ReportRow -Check 'NcaSvc' -Result $(if ($nca) { $nca.Status } else {'Not present'}) -Status 'INFO' -Detail 'Network Connectivity Assistant'
        }
        if ($Name -match 'Event-Log|IKE|EAP|Cert') {
            $ev = @(Get-WinEvent -FilterHashtable @{LogName='Application'; ProviderName='RasClient'; StartTime=(Get-Date).AddDays(-7)} -MaxEvents 20 -EA SilentlyContinue)
            Add-ReportRow -Check 'RasClient events' -Result "$($ev.Count)" -Status 'INFO' -Detail ''
            $clientAuth = @(Get-ChildItem Cert:\LocalMachine\My -EA SilentlyContinue | Where-Object { $_.EnhancedKeyUsageList -match 'Client Authentication' })
            Add-ReportRow -Check 'Client auth certs' -Result "$($clientAuth.Count)" -Status $(if ($clientAuth.Count -gt 0) {'PASS'} else {'WARN'}) -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'VPN done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Always On VPN Profile Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Always On VPN Profile Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
