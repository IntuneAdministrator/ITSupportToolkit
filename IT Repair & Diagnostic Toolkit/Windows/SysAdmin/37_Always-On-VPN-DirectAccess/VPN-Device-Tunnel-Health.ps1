...
<#
.SYNOPSIS
    Enterprise IT tool: VPN Device Tunnel Health
.DESCRIPTION
...
.EXAMPLE
    .\VPN-Device-Tunnel-Health.ps1
.EXAMPLE
    .\VPN-Device-Tunnel-Health.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VPN Device Tunnel Health'; $totalSteps = 8; $Name = 'VPN-Device-Tunnel-Health'
try {
    ...
    Export-HtmlReport -Title 'VPN Device Tunnel Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VPN Device Tunnel Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
