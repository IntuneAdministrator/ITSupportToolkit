...
<#
.SYNOPSIS
    Enterprise IT tool: VPN Event Log Digest
.DESCRIPTION
...
.EXAMPLE
    .\VPN-Event-Log-Digest.ps1
.EXAMPLE
    .\VPN-Event-Log-Digest.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'VPN Event Log Digest'; $totalSteps = 8; $Name = 'VPN-Event-Log-Digest'
try {
    ...
    Export-HtmlReport -Title 'VPN Event Log Digest' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VPN Event Log Digest' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
