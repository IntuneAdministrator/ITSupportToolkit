...
<#
.SYNOPSIS
    Enterprise IT tool: Machine Cert Template List
.DESCRIPTION
    Phase 10 expansion (SysAdmin/42_Certificate-AutoEnroll-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Machine-Cert-Template-List.ps1
.EXAMPLE
    .\Machine-Cert-Template-List.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Machine Cert Template List'; $totalSteps=8; $Name='Machine-Cert-Template-List'; $Folder='42_Certificate-AutoEnroll-Deep'
try {
    ...
    Export-HtmlReport -Title 'Machine Cert Template List' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Machine Cert Template List' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
