...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Evidence-Report.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Evidence Report'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
