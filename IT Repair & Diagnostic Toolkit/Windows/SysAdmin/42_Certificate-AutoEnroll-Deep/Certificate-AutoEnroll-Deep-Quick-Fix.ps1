...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Quick-Fix.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Quick Fix'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
