...
<#
.SYNOPSIS
    Enterprise IT tool: User Cert AutoEnroll Force
.DESCRIPTION
    Phase 10 expansion (SysAdmin/42_Certificate-AutoEnroll-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\User-Cert-AutoEnroll-Force.ps1
.EXAMPLE
    .\User-Cert-AutoEnroll-Force.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='User Cert AutoEnroll Force'; $totalSteps=8; $Name='User-Cert-AutoEnroll-Force'; $Folder='42_Certificate-AutoEnroll-Deep'
try {
    ...
    Export-HtmlReport -Title 'User Cert AutoEnroll Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'User Cert AutoEnroll Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
