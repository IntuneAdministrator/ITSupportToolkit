...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Stack-Reset.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Stack Reset'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
