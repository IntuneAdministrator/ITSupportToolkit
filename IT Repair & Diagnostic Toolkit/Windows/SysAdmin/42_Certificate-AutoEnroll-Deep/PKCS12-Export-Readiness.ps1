...
$exitCode=0; $Activity='PKCS12 Export Readiness'; $totalSteps=8; $Name='PKCS12-Export-Readiness'; $Folder='42_Certificate-AutoEnroll-Deep'
try {
    ...
    Export-HtmlReport -Title 'PKCS12 Export Readiness' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'PKCS12 Export Readiness' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
