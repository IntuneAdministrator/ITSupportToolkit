...
$exitCode=0; $Activity='Code Signing Cert Audit'; $totalSteps=8; $Name='Code-Signing-Cert-Audit'; $Folder='42_Certificate-AutoEnroll-Deep'
try {
    ...
    Export-HtmlReport -Title 'Code Signing Cert Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Code Signing Cert Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
