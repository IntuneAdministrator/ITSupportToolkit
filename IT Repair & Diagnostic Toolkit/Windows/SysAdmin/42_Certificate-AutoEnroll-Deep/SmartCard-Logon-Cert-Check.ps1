...
<#
.SYNOPSIS
    Enterprise IT tool: SmartCard Logon Cert Check
.DESCRIPTION
    Phase 10 expansion (SysAdmin/42_Certificate-AutoEnroll-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\SmartCard-Logon-Cert-Check.ps1
.EXAMPLE
    .\SmartCard-Logon-Cert-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='SmartCard Logon Cert Check'; $totalSteps=8; $Name='SmartCard-Logon-Cert-Check'; $Folder='42_Certificate-AutoEnroll-Deep'
try {
    ...
    Export-HtmlReport -Title 'SmartCard Logon Cert Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SmartCard Logon Cert Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
