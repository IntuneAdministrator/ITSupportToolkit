...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 42_Certificate-AutoEnroll-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Policy-Validate.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Policy Validate'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '42_Certificate-AutoEnroll-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '42_Certificate-AutoEnroll-Deep'
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
