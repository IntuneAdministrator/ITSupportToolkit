...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Anomaly-Detect.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Anomaly Detect'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
