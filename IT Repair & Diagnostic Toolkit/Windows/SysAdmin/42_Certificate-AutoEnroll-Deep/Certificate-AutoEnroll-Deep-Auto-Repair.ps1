...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Auto-Repair.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Auto Repair'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
