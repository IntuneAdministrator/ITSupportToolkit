...
<#
.SYNOPSIS
    Enterprise IT tool: Expired Intermediate CA Hunt
.DESCRIPTION
    Phase 10 expansion (SysAdmin/42_Certificate-AutoEnroll-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Expired-Intermediate-CA-Hunt.ps1
.EXAMPLE
    .\Expired-Intermediate-CA-Hunt.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Expired Intermediate CA Hunt'; $totalSteps=8; $Name='Expired-Intermediate-CA-Hunt'; $Folder='42_Certificate-AutoEnroll-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '42_Certificate-AutoEnroll-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'CERTIFICATE AUTOENROLL DEEP'
        $my = @(Get-ChildItem Cert:\LocalMachine\My -EA SilentlyContinue)
        $exp = @($my | Where-Object { $_.NotAfter -lt (Get-Date).AddDays(30) })
        Add-ReportRow -Check 'LM\My certs' -Result "$($my.Count) ($($exp.Count) expiring<30d)" -Status $(if($exp.Count -eq 0){'PASS'}else{'WARN'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'Store'
        if ($PSCmdlet.ShouldProcess('certutil', 'pulse / verifykeys')) {
            certutil -pulse 2>&1 | Out-Null
            Add-ReportRow -Check 'certutil -pulse' -Result 'Triggered' -Status 'PASS' -Detail 'Auto-enroll pulse'
            $vk = certutil -verifykeys 2>&1 | Out-String
            Add-ReportRow -Check 'verifykeys' -Result 'Ran' -Status 'INFO' -Detail $vk.Substring(0,[Math]::Min(300,$vk.Length))
        }
        if ($Name -match 'SmartCard|Client') {
            $sc = @($my | Where-Object { $_.EnhancedKeyUsageList -match 'Smart Card|Client Auth' })
            Add-ReportRow -Check 'SmartCard/ClientAuth' -Result "$($sc.Count)" -Status 'INFO' -Detail ''
        }
        if ($Name -match 'NDES|SCEP') {
            Add-ReportRow -Check 'SCEP/NDES' -Result 'Probe corporate NDES URL from Intune/ADCS docs' -Status 'INFO' -Detail $Join.Label
        }
        if ($Name -match 'Intermediate|Expired') {
            $ca = @(Get-ChildItem Cert:\LocalMachine\CA -EA SilentlyContinue | Where-Object { $_.NotAfter -lt (Get-Date) })
            Add-ReportRow -Check 'Expired intermediate' -Result "$($ca.Count)" -Status $(if($ca.Count -eq 0){'PASS'}else{'FAIL'}) -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Cert deep done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Expired Intermediate CA Hunt' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Expired Intermediate CA Hunt' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
