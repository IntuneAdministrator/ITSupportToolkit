...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Health-Triage.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Health Triage'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
