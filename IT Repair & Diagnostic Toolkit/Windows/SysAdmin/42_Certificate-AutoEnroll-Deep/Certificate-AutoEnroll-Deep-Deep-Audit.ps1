...
<#
.SYNOPSIS
    Common problem fix: Certificate AutoEnroll Deep Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Deep-Audit.ps1
.EXAMPLE
    .\Certificate-AutoEnroll-Deep-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate AutoEnroll Deep Deep Audit'; $totalSteps = 8; $Name = 'Certificate-AutoEnroll-Deep-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Certificate AutoEnroll Deep Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate AutoEnroll Deep Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
