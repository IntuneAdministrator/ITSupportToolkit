...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Startup Repair Trigger
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Startup-Repair-Trigger.
...
$exitCode = 0
$Activity = 'Startup Repair Trigger'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Startup-Repair-Trigger'
        Add-ReportRow -Check 'Tool' -Result 'Startup-Repair-Trigger' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Startup Repair Trigger' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Startup Repair Trigger' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
