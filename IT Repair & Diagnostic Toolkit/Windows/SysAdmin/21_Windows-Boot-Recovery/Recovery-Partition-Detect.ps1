...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Recovery Partition Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Recovery-Partition-Detect.
...
.EXAMPLE
    .\Recovery-Partition-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Recovery-Partition-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Recovery Partition Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Recovery-Partition-Detect'
        Add-ReportRow -Check 'Tool' -Result 'Recovery-Partition-Detect' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Recovery Partition Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Recovery Partition Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
