...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: EFI Partition Integrity Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for EFI-Partition-Integrity-Check.
...
.EXAMPLE
    .\EFI-Partition-Integrity-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\EFI-Partition-Integrity-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'EFI Partition Integrity Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'EFI-Partition-Integrity-Check'
        Add-ReportRow -Check 'Tool' -Result 'EFI-Partition-Integrity-Check' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'EFI Partition Integrity Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'EFI Partition Integrity Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
