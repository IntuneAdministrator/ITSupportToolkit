...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Health-Triage.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Health Triage'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
