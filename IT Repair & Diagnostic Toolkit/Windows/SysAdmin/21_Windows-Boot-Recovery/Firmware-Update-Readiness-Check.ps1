...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Firmware Update Readiness Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Firmware-Update-Readiness-Check.
...
.EXAMPLE
    .\Firmware-Update-Readiness-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Firmware-Update-Readiness-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Firmware Update Readiness Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Firmware-Update-Readiness-Check'
        Add-ReportRow -Check 'Tool' -Result 'Firmware-Update-Readiness-Check' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Firmware Update Readiness Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Firmware Update Readiness Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
