...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: MBR GPT Boot Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for MBR-GPT-Boot-Fix.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'MBR-GPT-Boot-Fix'
        Add-ReportRow -Check 'Tool' -Result 'MBR-GPT-Boot-Fix' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'MBR GPT Boot Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'MBR GPT Boot Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
