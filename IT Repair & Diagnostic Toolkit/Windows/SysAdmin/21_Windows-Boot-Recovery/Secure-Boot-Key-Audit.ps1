...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Secure Boot Key Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Secure-Boot-Key-Audit.
...
$exitCode = 0
$Activity = 'Secure Boot Key Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Secure-Boot-Key-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Secure-Boot-Key-Audit' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Secure Boot Key Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Secure Boot Key Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
