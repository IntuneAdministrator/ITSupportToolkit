...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Boot Log Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Boot-Log-Analyzer.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Boot-Log-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'Boot-Log-Analyzer' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Boot Log Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Boot Log Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
