...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Boot Performance Diagnostics
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Boot-Performance-Diagnostics.
Category: 21_Windows-Boot-Recovery. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Windows-Boot-Performance-Diagnostics.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Boot-Performance-Diagnostics.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Boot Performance Diagnostics'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'WINDOWS BOOT RECOVERY'
        $Name = 'Windows-Boot-Performance-Diagnostics'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Boot-Performance-Diagnostics' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
        Write-SectionBanner 'BOOT / RECOVERY'
        $bcd = "$env:SystemRoot\System32\bcdedit.exe"
        if (Test-Path $bcd) {
            $out = & $bcd /enum 2>&1 | Out-String
            Add-ReportRow -Check 'BCD enum' -Result 'Captured' -Status 'PASS' -Detail ($out.Substring(0, [Math]::Min(500, $out.Length)))
        } else {
            Add-ReportRow -Check 'bcdedit' -Result 'Missing' -Status 'FAIL' -Detail $bcd
        }
        Write-ProgressStep 2 $totalSteps $Activity 'BCD'

        if ($Name -match 'BCD-Repair|Bootloader|MBR-GPT|Boot-Configuration-Backup') {
            $bakDir = 'C:\IT-Logs\Backups\BCD'
            New-Item -ItemType Directory -Path $bakDir -Force | Out-Null
            $bak = Join-Path $bakDir ("bcd_{0}.txt" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
            if ($PSCmdlet.ShouldProcess('BCD', 'Backup bcdedit /enum all')) {
                & bcdedit /enum all > $bak 2>&1
                Add-ReportRow -Check 'BCD backup' -Result 'Saved' -Status 'PASS' -Detail $bak
            }
            if ($Name -match 'BCD-Repair|Bootloader' -and $PSCmdlet.ShouldProcess('BCD', 'bootrec /rebuildbcd attempt')) {
                # bootrec often needs WinRE; attempt and report
                $br = & bootrec /scanos 2>&1 | Out-String
                Add-ReportRow -Check 'bootrec /scanos' -Result 'Ran' -Status 'INFO' -Detail ($br.Substring(0, [Math]::Min(300, $br.Length)))
            }
        }

        if ($Name -match 'WinRE|Recovery-Partition|Push-Button|Automatic-Repair') {
            $re = & reagentc /info 2>&1 | Out-String
            $enabled = $re -match 'Enabled'
            Add-ReportRow -Check 'WinRE' -Result $(if ($enabled) {'Enabled'} else {'Disabled/Unknown'}) -Status $(if ($enabled) {'PASS'} else {'WARN'}) -Detail ($re.Substring(0, [Math]::Min(400, $re.Length)))
            if ($Name -match 'WinRE-Recovery-Enable' -and -not $enabled -and $PSCmdlet.ShouldProcess('WinRE', 'reagentc /enable')) {
                $en = & reagentc /enable 2>&1 | Out-String
                Add-ReportRow -Check 'Enable WinRE' -Result 'Attempted' -Status 'INFO' -Detail $en.Trim()
            }
            $parts = Get-Partition -ErrorAction SilentlyContinue | Where-Object { $_.Type -match 'Recovery|Reserved' -or $_.Size -lt 2GB }
            foreach ($p in @($parts | Select-Object -First 10)) {
                Add-ReportRow -Check "Partition #$($p.PartitionNumber)" -Result "$([math]::Round($p.Size/1MB,0)) MB" -Status 'INFO' -Detail "Disk=$($p.DiskNumber) Type=$($p.Type)"
            }
        }

        if ($Name -match 'Safe-Mode') {
            Add-ReportRow -Check 'Safe Mode guidance' -Result 'Use msconfig or bcdedit /set {current} safeboot minimal' -Status 'WARN' -Detail 'Not auto-enabled (requires reboot and explicit approval)'
            if ($PSCmdlet.ShouldProcess('Safe Mode', 'Report only - no auto enable')) {
                $cur = & bcdedit /enum '{current}' 2>&1 | Out-String
                Add-ReportRow -Check 'Current BCD' -Result 'Enumerated' -Status 'INFO' -Detail ($cur.Substring(0, [Math]::Min(300, $cur.Length)))
                Add-ReportRow -Check 'Safe Mode note' -Result 'Not auto-enabled' -Status 'WARN' -Detail 'bcdedit /set {current} safeboot minimal requires explicit ops approval'
            }
        }

        if ($Name -match 'EFI|Secure-Boot|Firmware') {
            try {
                Confirm-SecureBootUEFI -ErrorAction Stop | Out-Null
                Add-ReportRow -Check 'Secure Boot' -Result 'Enabled' -Status 'PASS' -Detail ''
            } catch {
                Add-ReportRow -Check 'Secure Boot' -Result 'Disabled or N/A' -Status 'WARN' -Detail $_.Exception.Message
            }
            $efi = Get-Partition | Where-Object IsSystem -eq $true
            foreach ($e in @($efi)) {
                Add-ReportRow -Check 'EFI/System partition' -Result "Disk $($e.DiskNumber) Part $($e.PartitionNumber)" -Status 'PASS' -Detail "$([math]::Round($e.Size/1MB,0)) MB"
            }
            $bios = Get-CimInstance Win32_BIOS
            Add-ReportRow -Check 'BIOS/UEFI version' -Result $bios.SMBIOSBIOSVersion -Status 'INFO' -Detail $bios.Manufacturer
        }

        if ($Name -match 'Boot-Log|Boot-Performance') {
            $bt = (Get-CimInstance Win32_OperatingSystem).LastBootUpTime
            Add-ReportRow -Check 'Last boot' -Result "$bt" -Status 'INFO' -Detail ''
            try {
                $bootEv = @(Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Diagnostics-Performance/Operational'; Id=100; StartTime=(Get-Date).AddDays(-14)} -MaxEvents 10 -ErrorAction SilentlyContinue)
                Add-ReportRow -Check 'Boot perf events (100)' -Result "$($bootEv.Count)" -Status 'INFO' -Detail ''
            } catch {
                Add-ReportRow -Check 'Boot perf log' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
            }
        }

        if ($Name -match 'Dual-Boot') {
            $enum = & bcdedit /enum firmware 2>&1 | Out-String
            Add-ReportRow -Check 'Firmware BCD entries' -Result 'Listed' -Status 'INFO' -Detail ($enum.Substring(0, [Math]::Min(400, $enum.Length)))
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Boot checks'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Boot Performance Diagnostics' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Boot Performance Diagnostics' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
