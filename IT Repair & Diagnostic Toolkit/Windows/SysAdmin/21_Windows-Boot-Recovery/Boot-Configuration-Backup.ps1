...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Boot Configuration Backup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Boot-Configuration-Backup.
...
.EXAMPLE
    .\Boot-Configuration-Backup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Boot-Configuration-Backup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Boot Configuration Backup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Boot-Configuration-Backup'
        Add-ReportRow -Check 'Tool' -Result 'Boot-Configuration-Backup' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Boot Configuration Backup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Boot Configuration Backup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
