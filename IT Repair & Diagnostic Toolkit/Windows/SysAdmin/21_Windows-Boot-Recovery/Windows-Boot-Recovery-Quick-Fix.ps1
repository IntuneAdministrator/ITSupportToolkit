...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Quick-Fix.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Quick Fix'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
