...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Auto-Repair.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Auto Repair'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
