...
<#
.SYNOPSIS
    Common problem fix: Runbook Boot Loop Recovery
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 21_Windows-Boot-Recovery). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-Boot-Loop-Recovery.ps1
.EXAMPLE
    .\Runbook-Boot-Loop-Recovery.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook Boot Loop Recovery'; $totalSteps = 8; $Name = 'Runbook-Boot-Loop-Recovery'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '21_Windows-Boot-Recovery' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '21_Windows-Boot-Recovery'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Runbook Boot Loop Recovery' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook Boot Loop Recovery' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
