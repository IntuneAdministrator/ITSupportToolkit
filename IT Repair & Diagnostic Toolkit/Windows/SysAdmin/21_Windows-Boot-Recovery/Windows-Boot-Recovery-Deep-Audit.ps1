...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Deep-Audit.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Deep Audit'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
