...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Anomaly-Detect.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Anomaly Detect'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
