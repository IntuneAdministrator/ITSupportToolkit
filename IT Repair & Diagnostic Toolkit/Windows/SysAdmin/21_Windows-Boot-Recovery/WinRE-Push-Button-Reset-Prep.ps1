...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WinRE Push Button Reset Prep
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WinRE-Push-Button-Reset-Prep.
...
.EXAMPLE
    .\WinRE-Push-Button-Reset-Prep.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WinRE-Push-Button-Reset-Prep.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WinRE Push Button Reset Prep'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WinRE-Push-Button-Reset-Prep'
        Add-ReportRow -Check 'Tool' -Result 'WinRE-Push-Button-Reset-Prep' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'WinRE Push Button Reset Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WinRE Push Button Reset Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
