...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Policy-Validate.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Policy Validate'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
