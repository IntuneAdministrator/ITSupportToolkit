...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Dual Boot Entry Manager
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Dual-Boot-Entry-Manager.
...
.EXAMPLE
    .\Dual-Boot-Entry-Manager.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Dual-Boot-Entry-Manager.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Dual Boot Entry Manager'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Dual-Boot-Entry-Manager'
        Add-ReportRow -Check 'Tool' -Result 'Dual-Boot-Entry-Manager' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'Dual Boot Entry Manager' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Dual Boot Entry Manager' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
