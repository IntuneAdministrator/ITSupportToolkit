...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Stack-Reset.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Stack Reset'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
