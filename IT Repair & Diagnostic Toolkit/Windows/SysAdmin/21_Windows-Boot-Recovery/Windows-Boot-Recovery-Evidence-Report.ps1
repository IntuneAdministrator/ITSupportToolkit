...
<#
.SYNOPSIS
    Common problem fix: Windows Boot Recovery Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Boot-Recovery-Evidence-Report.ps1
.EXAMPLE
    .\Windows-Boot-Recovery-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Boot Recovery Evidence Report'; $totalSteps = 8; $Name = 'Windows-Boot-Recovery-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Windows Boot Recovery Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Boot Recovery Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
