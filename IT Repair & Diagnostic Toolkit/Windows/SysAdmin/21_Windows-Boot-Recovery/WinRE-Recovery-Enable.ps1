...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WinRE Recovery Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WinRE-Recovery-Enable.
...
$exitCode = 0
$Activity = 'WinRE Recovery Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WinRE-Recovery-Enable'
        Add-ReportRow -Check 'Tool' -Result 'WinRE-Recovery-Enable' -Status 'INFO' -Detail '21_Windows-Boot-Recovery'
    ...
    Export-HtmlReport -Title 'WinRE Recovery Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WinRE Recovery Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
