...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Interface Speed Duplex Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Interface-Speed-Duplex-Audit.
...
.EXAMPLE
    .\Network-Interface-Speed-Duplex-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Interface-Speed-Duplex-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Interface Speed Duplex Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Network-Interface-Speed-Duplex-Audit'
        ...
        Add-ReportRow -Check 'Default gateway present' -Result $(if ($cfg) {'Yes'} else {'No'}) -Status $(if ($cfg) {'PASS'} else {'FAIL'}) -Detail (($cfg | ForEach-Object InterfaceAlias) -join ', ')
        Write-ProgressStep 4 $totalSteps $Activity 'Gateway check'
    ...
    Export-HtmlReport -Title 'Network Interface Speed Duplex Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Interface Speed Duplex Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
