...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ICMP Traceroute Analysis
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ICMP-Traceroute-Analysis.
...
.EXAMPLE
    .\ICMP-Traceroute-Analysis.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\ICMP-Traceroute-Analysis.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'ICMP Traceroute Analysis'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — ICMP-Traceroute-Analysis'
    ...
    Export-HtmlReport -Title 'ICMP Traceroute Analysis' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ICMP Traceroute Analysis' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
