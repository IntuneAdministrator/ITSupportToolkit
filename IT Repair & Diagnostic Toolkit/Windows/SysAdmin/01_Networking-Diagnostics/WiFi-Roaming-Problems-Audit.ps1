...
<#
.SYNOPSIS
    Common problem fix: WiFi Roaming Problems Audit
.DESCRIPTION
...
.EXAMPLE
    .\WiFi-Roaming-Problems-Audit.ps1
.EXAMPLE
    .\WiFi-Roaming-Problems-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WiFi Roaming Problems Audit'; $totalSteps = 8; $Name = 'WiFi-Roaming-Problems-Audit'
try {
    ...
    Export-HtmlReport -Title 'WiFi Roaming Problems Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WiFi Roaming Problems Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
