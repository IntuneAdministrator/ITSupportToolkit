...
<#
.SYNOPSIS
    Common problem fix: Corporate Proxy PAC Failure
.DESCRIPTION
...
.EXAMPLE
    .\Corporate-Proxy-PAC-Failure.ps1
.EXAMPLE
    .\Corporate-Proxy-PAC-Failure.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Corporate Proxy PAC Failure'; $totalSteps = 8; $Name = 'Corporate-Proxy-PAC-Failure'
try {
    ...
    Export-HtmlReport -Title 'Corporate Proxy PAC Failure' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Corporate Proxy PAC Failure' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
