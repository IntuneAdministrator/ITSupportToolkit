...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: 802 1X Auth Status Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for 802-1X-Auth-Status-Check.
...
.EXAMPLE
    .\802-1X-Auth-Status-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\802-1X-Auth-Status-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = '802 1X Auth Status Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — 802-1X-Auth-Status-Check'
    ...
    Export-HtmlReport -Title '802 1X Auth Status Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title '802 1X Auth Status Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
