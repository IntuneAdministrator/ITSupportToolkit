...
<#
.SYNOPSIS
    Common problem fix: LLMNR NetBIOS Leak Audit
.DESCRIPTION
...
.EXAMPLE
    .\LLMNR-NetBIOS-Leak-Audit
.EXAMPLE
    .\LLMNR-NetBIOS-Leak-Audit -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'LLMNR NetBIOS Leak Audit'; $totalSteps = 8; $Name = 'LLMNR-NetBIOS-Leak-Audit'
try {
    ...
    Export-HtmlReport -Title 'LLMNR NetBIOS Leak Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'LLMNR NetBIOS Leak Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
