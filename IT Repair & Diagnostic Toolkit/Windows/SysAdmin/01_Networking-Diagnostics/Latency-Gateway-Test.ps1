...
<#
.SYNOPSIS
    Common problem fix: Latency Gateway Test
.DESCRIPTION
...
.EXAMPLE
    .\Latency-Gateway-Test
.EXAMPLE
    .\Latency-Gateway-Test -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Latency Gateway Test'; $totalSteps = 8; $Name = 'Latency-Gateway-Test'
try {
    ...
    Export-HtmlReport -Title 'Latency Gateway Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Latency Gateway Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
