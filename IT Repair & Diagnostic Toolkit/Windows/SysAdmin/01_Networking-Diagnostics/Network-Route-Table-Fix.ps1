...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Route Table Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Route-Table-Fix.
...
.EXAMPLE
    .\Network-Route-Table-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Route-Table-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Route Table Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Network-Route-Table-Fix'
        ...
        Add-ReportRow -Check 'Default routes' -Result "$($routes.Count)" -Status $(if ($routes.Count -eq 1) {'PASS'} elseif ($routes.Count -gt 1) {'WARN'} else {'FAIL'}) -Detail (($routes | ForEach-Object { "if=$($_.InterfaceAlias) metric=$($_.RouteMetric) nh=$($_.NextHop)" }) -join '; ')
        Write-ProgressStep 4 $totalSteps $Activity 'Routes'
    ...
    Export-HtmlReport -Title 'Network Route Table Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Route Table Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
