...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Stack-Reset.ps1
.EXAMPLE
    .\Networking-Diagnostics-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Stack Reset'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
