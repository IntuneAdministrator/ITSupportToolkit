...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Deep-Audit.ps1
.EXAMPLE
    .\Networking-Diagnostics-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Deep Audit'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
