...
<#
.SYNOPSIS
    Common problem fix: Packet Loss WiFi Audit
.DESCRIPTION
...
.EXAMPLE
    .\Packet-Loss-WiFi-Audit
.EXAMPLE
    .\Packet-Loss-WiFi-Audit -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Packet Loss WiFi Audit'; $totalSteps = 8; $Name = 'Packet-Loss-WiFi-Audit'
try {
    ...
    Export-HtmlReport -Title 'Packet Loss WiFi Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Packet Loss WiFi Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
