...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DHCP Lease Renew Force
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DHCP-Lease-Renew-Force.
...
$exitCode = 0
$Activity = 'DHCP Lease Renew Force'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — DHCP-Lease-Renew-Force'
        ...
        if ($PSCmdlet.ShouldProcess('DHCP', 'Release and renew all adapters')) {
            ipconfig /release 2>&1 | Out-Null
            ipconfig /renew 2>&1 | Out-Null
            $addrs = @(Get-NetIPAddress -AddressFamily IPv4 | Where-Object PrefixOrigin -eq 'Dhcp')
            Add-ReportRow -Check 'DHCP renew' -Result "$($addrs.Count) DHCP addresses" -Status $(if ($addrs.Count -gt 0) {'PASS'} else {'WARN'}) -Detail (($addrs | ForEach-Object { $_.IPAddress }) -join ', ')
        }
        Write-ProgressStep 4 $totalSteps $Activity 'DHCP renew'
    ...
    Export-HtmlReport -Title 'DHCP Lease Renew Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DHCP Lease Renew Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
