...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Proxy Settings Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Proxy-Settings-Repair.
...
$exitCode = 0
$Activity = 'Proxy Settings Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Proxy-Settings-Repair'
        ...
        $backupDir = 'C:\IT-Logs\Backups'
        Backup-RegistryKey -KeyPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' -BackupDir $backupDir | Out-Null
        $proxy = Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'ProxyEnable' -Result "$($proxy.ProxyEnable)" -Status 'INFO' -Detail $proxy.ProxyServer
        if ($PSCmdlet.ShouldProcess('Proxy settings', 'Disable user proxy and reset WinHTTP')) {
            ...
            Add-ReportRow -Check 'Proxy reset' -Result 'Cleared user + WinHTTP' -Status 'PASS' -Detail 'Rollback via reg import backup'
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Proxy repair'
    ...
    Export-HtmlReport -Title 'Proxy Settings Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Proxy Settings Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
