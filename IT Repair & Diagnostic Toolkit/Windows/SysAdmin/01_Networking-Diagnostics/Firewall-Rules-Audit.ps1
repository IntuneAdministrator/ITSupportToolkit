...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Firewall Rules Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Firewall-Rules-Audit.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'FIREWALL PROFILE AUDIT'
        $profiles = @(Get-NetFirewallProfile -ErrorAction SilentlyContinue)
        foreach ($p in $profiles) {
            Add-ReportRow -Check "Profile $($p.Name)" -Result $(if ($p.Enabled) {'Enabled'} else {'Disabled'}) -Status $(if ($p.Enabled) {'PASS'} else {'WARN'}) -Detail "Inbound=$($p.DefaultInboundAction) Outbound=$($p.DefaultOutboundAction)"
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Profiles'

        Write-SectionBanner 'HIGH-RISK RULE SCAN'
        $risky = @(Get-NetFirewallRule -Enabled True -Direction Inbound -Action Allow -ErrorAction SilentlyContinue |
            Where-Object { $_.Profile -match 'Any|Public' } |
            Select-Object -First 50)
        Add-ReportRow -Check 'Public/Any inbound allow rules' -Result "$($risky.Count) sampled" -Status $(if ($risky.Count -gt 20) {'WARN'} else {'INFO'}) -Detail (($risky | Select-Object -First 10 DisplayName) -join '; ')
        Write-ProgressStep 3 $totalSteps $Activity 'Risky rules'

        $disabled = (Get-NetFirewallProfile | Where-Object { -not $_.Enabled }).Count
        if ($disabled -gt 0 -and $PSCmdlet.ShouldProcess('Firewall profiles', 'Warn only — enable manually if required')) {
            Add-ReportRow -Check 'Disabled profiles' -Result "$disabled disabled" -Status 'WARN' -Detail 'Review before enabling in production'
        }

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Firewall Rules Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Firewall Rules Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
