...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Quick-Fix.ps1
.EXAMPLE
    .\Networking-Diagnostics-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Quick Fix'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
