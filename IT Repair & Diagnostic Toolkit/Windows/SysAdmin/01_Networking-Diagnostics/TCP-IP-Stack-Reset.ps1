...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: TCP IP Stack Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for TCP-IP-Stack-Reset.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'TCP/IP STACK RESET'
        $cmds = @(
            @{ N='Winsock reset'; C={ netsh winsock reset } },
            @{ N='IP stack reset'; C={ netsh int ip reset } },
            @{ N='IPv6 reset'; C={ netsh int ipv6 reset } },
            @{ N='Flush DNS'; C={ ipconfig /flushdns } },
            @{ N='Release DHCP'; C={ ipconfig /release } },
            @{ N='Renew DHCP'; C={ ipconfig /renew } }
        )
        $i = 2
        foreach ($item in $cmds) {
            Write-ProgressStep $i $totalSteps $Activity $item.N
            if ($PSCmdlet.ShouldProcess($item.N, 'Execute network reset command')) {
                try {
                    & $item.C 2>&1 | Out-Null
                    Add-ReportRow -Check $item.N -Result 'Executed' -Status 'PASS' -Detail 'Reboot may be required for full effect'
                } catch {
                    Add-ReportRow -Check $item.N -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
            } else {
                Add-ReportRow -Check $item.N -Result 'WhatIf skipped' -Status 'INFO' -Detail 'No change'
            }
            $i++
        }
        Add-ReportRow -Check 'Post-reset note' -Result 'Reboot recommended' -Status 'WARN' -Detail 'Winsock/IP reset often requires restart'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'TCP IP Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'TCP IP Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
