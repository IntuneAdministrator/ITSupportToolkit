...
<#
.SYNOPSIS
    Common problem fix: Multicast Stream Block Detect
.DESCRIPTION
...
.EXAMPLE
    .\Multicast-Stream-Block-Detect
.EXAMPLE
    .\Multicast-Stream-Block-Detect -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Multicast Stream Block Detect'; $totalSteps = 8; $Name = 'Multicast-Stream-Block-Detect'
try {
    ...
    Export-HtmlReport -Title 'Multicast Stream Block Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Multicast Stream Block Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
