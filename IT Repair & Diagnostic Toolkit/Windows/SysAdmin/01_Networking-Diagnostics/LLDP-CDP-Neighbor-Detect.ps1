...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: LLDP CDP Neighbor Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for LLDP-CDP-Neighbor-Detect.
...
.EXAMPLE
    .\LLDP-CDP-Neighbor-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\LLDP-CDP-Neighbor-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'LLDP CDP Neighbor Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — LLDP-CDP-Neighbor-Detect'
    ...
    Export-HtmlReport -Title 'LLDP CDP Neighbor Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'LLDP CDP Neighbor Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
