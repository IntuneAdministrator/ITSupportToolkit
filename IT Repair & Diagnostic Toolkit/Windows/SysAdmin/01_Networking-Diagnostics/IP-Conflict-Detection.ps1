...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: IP Conflict Detection
.DESCRIPTION
    Production-grade Windows SysAdmin tool for IP-Conflict-Detection.
...
$exitCode = 0
$Activity = 'IP Conflict Detection'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — IP-Conflict-Detection'
        ...
        $dupes = ($arp -split "`n" | Select-String 'dynamic' | ForEach-Object { ($_ -split '\s+')[1] } | Group-Object | Where-Object Count -gt 1)
        Add-ReportRow -Check 'ARP duplicate IPs' -Result "$(@($dupes).Count) candidates" -Status $(if (@($dupes).Count -eq 0) {'PASS'} else {'WARN'}) -Detail (($dupes | ForEach-Object Name) -join ', ')
        Write-ProgressStep 4 $totalSteps $Activity 'ARP conflict scan'
    ...
    Export-HtmlReport -Title 'IP Conflict Detection' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'IP Conflict Detection' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
