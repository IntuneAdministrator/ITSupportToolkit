...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: IPv6 Disable Toggle
.DESCRIPTION
    Production-grade Windows SysAdmin tool for IPv6-Disable-Toggle.
Category: 01_Networking-Diagnostics. Detects AD / Entra ID / Hybrid join state at runtime,
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — IPv6-Disable-Toggle'
        $adapters = @(Get-NetAdapter -ErrorAction SilentlyContinue)
        Add-ReportRow -Check 'Net adapters' -Result "$($adapters.Count) found" -Status $(if ($adapters.Count -gt 0) {'PASS'} else {'FAIL'}) -Detail (($adapters | Select-Object -First 8 Name,Status,LinkSpeed | Out-String).Trim())
        Write-ProgressStep 2 $totalSteps $Activity 'Adapters'

        $ip = @(Get-NetIPAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object { $_.IPAddress -notlike '127.*' })
        Add-ReportRow -Check 'IPv4 addresses' -Result "$($ip.Count) addresses" -Status $(if ($ip.Count -gt 0) {'PASS'} else {'WARN'}) -Detail (($ip | ForEach-Object { "$($_.InterfaceAlias)=$($_.IPAddress)" }) -join '; ')
        Write-ProgressStep 3 $totalSteps $Activity 'IP'

        # Script-specific network actions
        $backupDir = 'C:\IT-Logs\Backups'
        Backup-RegistryKey -KeyPath 'HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters' -BackupDir $backupDir | Out-Null
        $paramPath = 'HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters'
        $current = (Get-ItemProperty $paramPath -Name DisabledComponents -ErrorAction SilentlyContinue).DisabledComponents
        Add-ReportRow -Check 'DisabledComponents' -Result $(if ($null -eq $current) {'Not set'} else {"0x{0:X}" -f $current}) -Status 'INFO' -Detail '0xFF=disable all IPv6 tunnel + interfaces (reboot)'
        if ($PSCmdlet.ShouldProcess('IPv6', 'Toggle DisabledComponents between 0 and 0xFF')) {
            if ($current -eq 0xFF) {
                Set-ItemProperty $paramPath -Name DisabledComponents -Value 0 -Type DWord
                Add-ReportRow -Check 'IPv6 toggle' -Result 'Enabled (0)' -Status 'PASS' -Detail 'Reboot required'
            } else {
                New-ItemProperty $paramPath -Name DisabledComponents -Value 0xFF -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'IPv6 toggle' -Result 'Disabled (0xFF)' -Status 'PASS' -Detail 'Reboot required; rollback via backup .reg'
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'IPv6 toggle'

        Write-ProgressStep $($totalSteps - 1) $totalSteps $Activity 'Connectivity verify'
        $ping = Test-Connection -ComputerName 1.1.1.1 -Count 1 -Quiet -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'Internet ICMP' -Result $(if ($ping) {'OK'} else {'Fail'}) -Status $(if ($ping) {'PASS'} else {'WARN'}) -Detail '1.1.1.1'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'IPv6 Disable Toggle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'IPv6 Disable Toggle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
