...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bandwidth Saturation Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bandwidth-Saturation-Test.
...
.EXAMPLE
    .\Bandwidth-Saturation-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bandwidth-Saturation-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bandwidth Saturation Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Bandwidth-Saturation-Test'
        ...
        try {
            $before = Get-Counter '\Network Interface(*)\Bytes Total/sec' -ErrorAction SilentlyContinue
            Start-Sleep -Seconds 3
            $after = Get-Counter '\Network Interface(*)\Bytes Total/sec' -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Bandwidth sample' -Result 'Counters collected' -Status 'INFO' -Detail 'Bytes Total/sec over 3s window'
        } catch {
            Add-ReportRow -Check 'Bandwidth sample' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Bandwidth'
    ...
    Export-HtmlReport -Title 'Bandwidth Saturation Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bandwidth Saturation Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
