...
<#
.SYNOPSIS
    Common problem fix: Bonjour mDNS Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\Bonjour-mDNS-Conflict-Fix
.EXAMPLE
    .\Bonjour-mDNS-Conflict-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bonjour mDNS Conflict Fix'; $totalSteps = 8; $Name = 'Bonjour-mDNS-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'Bonjour mDNS Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bonjour mDNS Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
