...
<#
.SYNOPSIS
    Common problem fix: Runbook DNS DHCP Client Repair
.DESCRIPTION
...
.EXAMPLE
    .\Runbook-DNS-DHCP-Client-Repair.ps1
.EXAMPLE
    .\Runbook-DNS-DHCP-Client-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook DNS DHCP Client Repair'; $totalSteps = 8; $Name = 'Runbook-DNS-DHCP-Client-Repair'
try {
    ...
    Export-HtmlReport -Title 'Runbook DNS DHCP Client Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook DNS DHCP Client Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
