...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Link Layer Topology Discovery
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Link-Layer-Topology-Discovery.
...
.EXAMPLE
    .\Link-Layer-Topology-Discovery.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Link-Layer-Topology-Discovery.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Link Layer Topology Discovery'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Link-Layer-Topology-Discovery'
    ...
    Export-HtmlReport -Title 'Link Layer Topology Discovery' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Link Layer Topology Discovery' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
