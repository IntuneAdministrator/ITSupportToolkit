...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Anomaly-Detect.ps1
.EXAMPLE
    .\Networking-Diagnostics-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Anomaly Detect'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
