...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Captive Portal Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Captive-Portal-Detect.
...
$exitCode = 0
$Activity = 'Captive Portal Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Captive-Portal-Detect'
        ...
        try {
            $r = Invoke-WebRequest -Uri 'http://www.msftconnecttest.com/connecttest.txt' -UseBasicParsing -TimeoutSec 10 -MaximumRedirection 0 -ErrorAction SilentlyContinue
            $ok = $r.Content -match 'Microsoft Connect Test'
            Add-ReportRow -Check 'Captive portal' -Result $(if ($ok) {'None detected'} else {'Possible portal'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail "Status=$($r.StatusCode)"
        } catch {
            Add-ReportRow -Check 'Captive portal' -Result 'Check failed / redirect' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Captive portal'
    ...
    Export-HtmlReport -Title 'Captive Portal Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Captive Portal Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
