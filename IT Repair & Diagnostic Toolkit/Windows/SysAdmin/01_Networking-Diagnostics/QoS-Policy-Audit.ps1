...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: QoS Policy Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for QoS-Policy-Audit.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — QoS-Policy-Audit'
    ...
    Export-HtmlReport -Title 'QoS Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'QoS Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
