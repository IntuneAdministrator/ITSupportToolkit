...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ARP Cache Flush
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ARP-Cache-Flush.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — ARP-Cache-Flush'
        ...
        if ($PSCmdlet.ShouldProcess('ARP cache', 'netsh interface ip delete arpcache')) {
            & netsh interface ip delete arpcache 2>&1 | Out-Null
            Add-ReportRow -Check 'ARP flush' -Result 'Cleared' -Status 'PASS' -Detail 'netsh interface ip delete arpcache'
        }
        Write-ProgressStep 4 $totalSteps $Activity 'ARP flush'
    ...
    Export-HtmlReport -Title 'ARP Cache Flush' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ARP Cache Flush' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
