...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Port Connectivity Scanner
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Port-Connectivity-Scanner.
...
.EXAMPLE
    .\Port-Connectivity-Scanner.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Port-Connectivity-Scanner.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Port Connectivity Scanner'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Port-Connectivity-Scanner'
        ...
        $ports = @(80,443,53,445,3389,5985)
        foreach ($port in $ports) {
            $tnc = Test-NetConnection -ComputerName 1.1.1.1 -Port $port -WarningAction SilentlyContinue -ErrorAction SilentlyContinue
            # 1.1.1.1 only meaningfully answers 80/443; others may fail — mark INFO for non-web
            $status = if ($port -in 80,443) { if ($tnc.TcpTestSucceeded) {'PASS'} else {'FAIL'} } else { 'INFO' }
            Add-ReportRow -Check "TCP $port to 1.1.1.1" -Result $(if ($tnc.TcpTestSucceeded) {'Open'} else {'Closed/Filtered'}) -Status $status -Detail "Latency=$($tnc.PingReplyDetails.RoundtripTime)ms"
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Port scan sample'
    ...
    Export-HtmlReport -Title 'Port Connectivity Scanner' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Port Connectivity Scanner' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
