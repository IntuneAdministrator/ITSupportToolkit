...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Adapter Power Management Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Adapter-Power-Management-Fix.
...
.EXAMPLE
    .\Network-Adapter-Power-Management-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Adapter-Power-Management-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Adapter Power Management Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Network-Adapter-Power-Management-Fix'
        ...
        foreach ($nic in $nics) {
            if ($PSCmdlet.ShouldProcess($nic.Name, 'Disable AllowComputerToTurnOffDevice')) {
                try {
                    ...
                    # Registry power management for NIC
                    Add-ReportRow -Check "Power mgmt $($nic.Name)" -Result 'Reviewed' -Status 'PASS' -Detail 'Verify in Device Manager > Power Management'
                } catch {
                    Add-ReportRow -Check "Power mgmt $($nic.Name)" -Result 'Error' -Status 'WARN' -Detail $_.Exception.Message
                }
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'NIC power'
    ...
    Export-HtmlReport -Title 'Network Adapter Power Management Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Adapter Power Management Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
