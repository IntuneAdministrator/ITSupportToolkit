...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: MTU Path Discovery
.DESCRIPTION
    Production-grade Windows SysAdmin tool for MTU-Path-Discovery.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — MTU-Path-Discovery'
        ...
        $sizes = @(1500,1472,1400,1300,1200)
        $best = $null
        foreach ($s in $sizes) {
            $ping = & ping -f -l $s -n 1 1.1.1.1 2>&1 | Out-String
            if ($ping -match 'Reply from') { $best = $s; break }
        }
        Add-ReportRow -Check 'Path MTU probe' -Result $(if ($best) {"Max payload $best"} else {'All fragmented'}) -Status $(if ($best -ge 1472) {'PASS'} else {'WARN'}) -Detail 'ping -f -l against 1.1.1.1'
        Write-ProgressStep 4 $totalSteps $Activity 'MTU'
    ...
    Export-HtmlReport -Title 'MTU Path Discovery' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'MTU Path Discovery' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
