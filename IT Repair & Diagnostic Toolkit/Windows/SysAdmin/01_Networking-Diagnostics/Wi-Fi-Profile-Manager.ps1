...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wi Fi Profile Manager
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wi-Fi-Profile-Manager.
...
$exitCode = 0
$Activity = 'Wi Fi Profile Manager'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Wi-Fi-Profile-Manager'
        ...
        Add-ReportRow -Check 'WLAN profiles' -Result 'Listed' -Status 'INFO' -Detail ($profiles.Substring(0, [Math]::Min(500, $profiles.Length)))
        if ($PSCmdlet.ShouldProcess('WLAN autoconfig', 'Restart WlanSvc')) {
            Restart-Service WlanSvc -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'WlanSvc restart' -Result 'Done' -Status 'PASS' -Detail 'WLAN AutoConfig'
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Wi-Fi profiles'
    ...
    Export-HtmlReport -Title 'Wi Fi Profile Manager' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wi Fi Profile Manager' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
