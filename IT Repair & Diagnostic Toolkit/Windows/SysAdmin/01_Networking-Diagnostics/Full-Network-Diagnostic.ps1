...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Full Network Diagnostic
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Full-Network-Diagnostic.
...
.EXAMPLE
    .\Full-Network-Diagnostic.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Full-Network-Diagnostic.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Full Network Diagnostic'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'ADAPTER INVENTORY'
        $adapters = @(Get-NetAdapter -ErrorAction SilentlyContinue)
        Add-ReportRow -Check 'Adapter count' -Result "$($adapters.Count) adapters" -Status $(if ($adapters.Count -gt 0) {'PASS'} else {'FAIL'}) -Detail (($adapters | ForEach-Object { "$($_.Name):$($_.Status)" }) -join '; ')
        Write-ProgressStep 2 $totalSteps $Activity 'Adapter inventory'

        Write-SectionBanner 'IP CONFIGURATION'
        $ipcfg = @(Get-NetIPConfiguration -ErrorAction SilentlyContinue)
        foreach ($cfg in $ipcfg) {
            $ipv4 = ($cfg.IPv4Address | Select-Object -ExpandProperty IPAddress) -join ', '
            $gw   = ($cfg.IPv4DefaultGateway | Select-Object -ExpandProperty NextHop) -join ', '
            $dns  = ($cfg.DNSServer.ServerAddresses) -join ', '
            $ok = [bool]$ipv4
            Add-ReportRow -Check "IP $($cfg.InterfaceAlias)" -Result $(if ($ok) {"$ipv4 gw=$gw"} else {'No IPv4'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail "DNS=$dns"
        }
        Write-ProgressStep 3 $totalSteps $Activity 'IP configuration'

        Write-SectionBanner 'CONNECTIVITY TESTS'
        $targets = @('1.1.1.1','8.8.8.8','dns.msftncsi.com')
        foreach ($t in $targets) {
            $ping = Test-Connection -ComputerName $t -Count 2 -Quiet -ErrorAction SilentlyContinue
            Add-ReportRow -Check "Ping $t" -Result $(if ($ping) {'Reachable'} else {'Unreachable'}) -Status $(if ($ping) {'PASS'} else {'FAIL'}) -Detail 'ICMP'
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Connectivity'

        Write-SectionBanner 'DNS RESOLUTION'
        foreach ($name in @('www.microsoft.com','login.microsoftonline.com')) {
            try {
                $r = Resolve-DnsName -Name $name -ErrorAction Stop | Select-Object -First 1
                Add-ReportRow -Check "Resolve $name" -Result "$($r.IPAddress)$($r.NameHost)" -Status 'PASS' -Detail $r.Type
            } catch {
                Add-ReportRow -Check "Resolve $name" -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'DNS'

        Write-SectionBanner 'PUBLIC / NCSI'
        try {
            ...
            Add-ReportRow -Check 'NCSI captive portal check' -Result $(if ($ok) {'Internet OK'} else {'Unexpected response'}) -Status $(if ($ok) {'PASS'} else {'WARN'}) -Detail $ncsi.Content.Trim()
        } catch {
            Add-ReportRow -Check 'NCSI captive portal check' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
        }
        Write-ProgressStep 6 $totalSteps $Activity 'NCSI'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Full Network Diagnostic' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Full Network Diagnostic' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
