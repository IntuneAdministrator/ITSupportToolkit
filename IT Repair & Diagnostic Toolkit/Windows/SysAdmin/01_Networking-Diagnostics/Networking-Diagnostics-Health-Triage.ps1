...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Health-Triage.ps1
.EXAMPLE
    .\Networking-Diagnostics-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Health Triage'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
