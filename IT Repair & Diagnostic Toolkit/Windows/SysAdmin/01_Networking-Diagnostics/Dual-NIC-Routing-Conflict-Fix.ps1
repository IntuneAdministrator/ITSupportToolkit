...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Dual NIC Routing Conflict Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Dual-NIC-Routing-Conflict-Fix.
...
.EXAMPLE
    .\Dual-NIC-Routing-Conflict-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Dual-NIC-Routing-Conflict-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Dual NIC Routing Conflict Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Dual-NIC-Routing-Conflict-Fix'
    ...
    Export-HtmlReport -Title 'Dual NIC Routing Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Dual NIC Routing Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
