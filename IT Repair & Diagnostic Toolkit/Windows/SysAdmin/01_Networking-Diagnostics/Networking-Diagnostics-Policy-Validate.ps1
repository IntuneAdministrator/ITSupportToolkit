...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Policy-Validate.ps1
.EXAMPLE
    .\Networking-Diagnostics-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Policy Validate'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
