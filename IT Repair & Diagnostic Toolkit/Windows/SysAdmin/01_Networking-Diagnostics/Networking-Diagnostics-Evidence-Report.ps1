...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Evidence-Report.ps1
.EXAMPLE
    .\Networking-Diagnostics-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Evidence Report'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
