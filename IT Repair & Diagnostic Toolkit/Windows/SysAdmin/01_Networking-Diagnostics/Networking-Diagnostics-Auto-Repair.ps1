...
<#
.SYNOPSIS
    Common problem fix: Networking Diagnostics Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Networking-Diagnostics-Auto-Repair.ps1
.EXAMPLE
    .\Networking-Diagnostics-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Networking Diagnostics Auto Repair'; $totalSteps = 8; $Name = 'Networking-Diagnostics-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Networking Diagnostics Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Networking Diagnostics Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
