...
<#
.SYNOPSIS
    Common problem fix: Jitter VoIP Quality Test
.DESCRIPTION
...
.EXAMPLE
    .\Jitter-VoIP-Quality-Test
.EXAMPLE
    .\Jitter-VoIP-Quality-Test -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Jitter VoIP Quality Test'; $totalSteps = 8; $Name = 'Jitter-VoIP-Quality-Test'
try {
    ...
    Export-HtmlReport -Title 'Jitter VoIP Quality Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Jitter VoIP Quality Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
