...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Netstat Established Connection Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Netstat-Established-Connection-Report.
...
.EXAMPLE
    .\Netstat-Established-Connection-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Netstat-Established-Connection-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Netstat Established Connection Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Netstat-Established-Connection-Report'
        ...
        Add-ReportRow -Check 'Established TCP' -Result "$($conns.Count)" -Status 'INFO' -Detail (($conns | Select-Object -First 15 RemoteAddress,RemotePort,OwningProcess | Out-String).Trim())
        Write-ProgressStep 4 $totalSteps $Activity 'Netstat'
    ...
    Export-HtmlReport -Title 'Netstat Established Connection Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Netstat Established Connection Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
