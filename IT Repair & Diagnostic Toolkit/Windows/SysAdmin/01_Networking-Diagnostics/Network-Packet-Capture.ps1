...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Packet Capture
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Packet-Capture.
...
$exitCode = 0
$Activity = 'Network Packet Capture'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — Network-Packet-Capture'
        ...
        $capDir = 'C:\IT-Logs\Captures'
        New-Item -ItemType Directory -Path $capDir -Force | Out-Null
        $capFile = Join-Path $capDir ("netcap_{0}.etl" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
        if ($PSCmdlet.ShouldProcess($capFile, 'Short netsh trace capture (15s)')) {
            try {
                Start-Process netsh -ArgumentList "trace start capture=yes tracefile=`"$capFile`" maxSize=50" -Wait -WindowStyle Hidden
                Start-Sleep -Seconds 15
                Start-Process netsh -ArgumentList 'trace stop' -Wait -WindowStyle Hidden
                Add-ReportRow -Check 'Packet capture' -Result $(if (Test-Path $capFile) {'Saved'} else {'Check ETL'}) -Status 'PASS' -Detail $capFile
            } catch {
                Add-ReportRow -Check 'Packet capture' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Capture'
    ...
    Export-HtmlReport -Title 'Network Packet Capture' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Packet Capture' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
