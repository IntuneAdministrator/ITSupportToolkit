...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Adapter Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Adapter-Reset.
...
$exitCode = 0
$Activity = 'Network Adapter Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK ADAPTER RESET'
        $adapters = @(Get-NetAdapter | Where-Object { $_.HardwareInterface -eq $true })
        $i = 2
        foreach ($a in $adapters) {
            Write-ProgressStep $i $totalSteps $Activity "Reset $($a.Name)"
            if ($PSCmdlet.ShouldProcess($a.Name, 'Disable then Enable adapter')) {
                try {
                    ...
                    $re = Get-NetAdapter -Name $a.Name
                    Add-ReportRow -Check "Reset $($a.Name)" -Result $re.Status -Status $(if ($re.Status -eq 'Up') {'PASS'} else {'WARN'}) -Detail "IfIndex=$($re.ifIndex)"
                } catch {
                    Add-ReportRow -Check "Reset $($a.Name)" -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
            } else {
                Add-ReportRow -Check "Reset $($a.Name)" -Result 'WhatIf' -Status 'INFO' -Detail $a.InterfaceDescription
            }
            $i++
        }
        if ($adapters.Count -eq 0) {
            Add-ReportRow -Check 'Hardware adapters' -Result 'None found' -Status 'WARN' -Detail 'Virtual-only environment?'
        }

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Network Adapter Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Adapter Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
