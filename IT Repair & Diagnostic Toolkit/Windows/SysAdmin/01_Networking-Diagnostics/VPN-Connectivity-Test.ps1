...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: VPN Connectivity Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for VPN-Connectivity-Test.
...
.EXAMPLE
    .\VPN-Connectivity-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'VPN Connectivity Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'NETWORK DIAGNOSTIC — VPN-Connectivity-Test'
        ...
        Add-ReportRow -Check 'VPN profiles' -Result "$($vpn.Count)" -Status 'INFO' -Detail (($vpn | ForEach-Object { "$($_.Name):$($_.ConnectionStatus)" }) -join '; ')
        foreach ($v in $vpn) {
            if ($v.ConnectionStatus -ne 'Connected') {
                Add-ReportRow -Check "VPN $($v.Name)" -Result $v.ConnectionStatus -Status 'WARN' -Detail $v.TunnelType
            } else {
                Add-ReportRow -Check "VPN $($v.Name)" -Result 'Connected' -Status 'PASS' -Detail $v.ServerAddress
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'VPN profiles'
    ...
    Export-HtmlReport -Title 'VPN Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'VPN Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
