...
<#
.SYNOPSIS
    Enterprise IT tool: LSA Protected Users Hint
.DESCRIPTION
    Phase 10 expansion (SysAdmin/44_Identity-Credential-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\LSA-Protected-Users-Hint.ps1
.EXAMPLE
    .\LSA-Protected-Users-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='LSA Protected Users Hint'; $totalSteps=8; $Name='LSA-Protected-Users-Hint'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Export-HtmlReport -Title 'LSA Protected Users Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'LSA Protected Users Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
