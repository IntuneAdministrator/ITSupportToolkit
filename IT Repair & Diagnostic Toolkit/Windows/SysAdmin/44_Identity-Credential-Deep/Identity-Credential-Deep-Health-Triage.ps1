...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Health-Triage.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Health Triage'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
