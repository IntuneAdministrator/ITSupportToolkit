...
<#
.SYNOPSIS
    Enterprise IT tool: Credential Manager Export Guide
.DESCRIPTION
    Phase 10 expansion (SysAdmin/44_Identity-Credential-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Credential-Manager-Export-Guide.ps1
.EXAMPLE
    .\Credential-Manager-Export-Guide.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Credential Manager Export Guide'; $totalSteps=8; $Name='Credential-Manager-Export-Guide'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '44_Identity-Credential-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'IDENTITY / CREDENTIAL DEEP'
        $wl = Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' -EA SilentlyContinue
        Add-ReportRow -Check 'CachedLogonsCount' -Result "$($wl.CachedLogonsCount)" -Status 'INFO' -Detail 'Winlogon'
        Write-ProgressStep 2 $totalSteps $Activity 'Winlogon'
        $credssp = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' -Name DisableRestrictedAdmin -EA SilentlyContinue).DisableRestrictedAdmin
        Add-ReportRow -Check 'CredSSP/RestrictedAdmin related' -Result "DisableRestrictedAdmin=$credssp" -Status 'INFO' -Detail 'LSA'
        if ($Name -match 'NGC|Hello') {
            $ngc = "$env:SystemRoot\ServiceProfiles\LocalService\AppData\Local\Microsoft\Ngc"
            Add-ReportRow -Check 'NGC folder' -Result $(if(Test-Path $ngc){'Present'}else{'Absent'}) -Status 'INFO' -Detail 'Windows Hello container store'
        }
        if ($Name -match 'Kerberos|NTLM|Protected-Users|DPAPI') {
            $k = & klist 2>&1 | Out-String
            Add-ReportRow -Check 'klist' -Result 'Tickets' -Status 'INFO' -Detail $k.Substring(0,[Math]::Min(350,$k.Length))
            if ($Name -match 'Kerberos' -and $PSCmdlet.ShouldProcess('Kerberos','klist purge')) { klist purge 2>$null | Out-Null; Add-ReportRow -Check 'klist purge' -Result 'Done' -Status 'PASS' -Detail '' }
        }
        if ($Name -match 'Credential-Manager') {
            cmdkey /list 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'cmdkey' -Result 'Listed' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(400,$_.Length)) }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Identity deep done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Credential Manager Export Guide' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Credential Manager Export Guide' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
