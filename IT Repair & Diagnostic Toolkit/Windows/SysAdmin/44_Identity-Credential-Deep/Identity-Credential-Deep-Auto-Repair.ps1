...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Auto-Repair.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Auto Repair'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
