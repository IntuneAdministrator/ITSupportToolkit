...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Policy-Validate.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Policy Validate'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
