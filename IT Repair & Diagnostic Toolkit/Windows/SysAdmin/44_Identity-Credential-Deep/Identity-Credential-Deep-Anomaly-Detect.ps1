...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Anomaly-Detect.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Anomaly Detect'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
