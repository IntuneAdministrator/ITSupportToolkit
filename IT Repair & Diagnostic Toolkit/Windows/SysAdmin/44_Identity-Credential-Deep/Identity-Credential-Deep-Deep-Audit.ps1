...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Deep-Audit.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Deep Audit'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
