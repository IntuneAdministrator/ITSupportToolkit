...
<#
.SYNOPSIS
    Common problem fix: Windows Hello Facial Reset
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Hello-Facial-Reset.ps1
.EXAMPLE
    .\Windows-Hello-Facial-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Hello Facial Reset'; $totalSteps = 8; $Name = 'Windows-Hello-Facial-Reset'
try {
    ...
    Export-HtmlReport -Title 'Windows Hello Facial Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Hello Facial Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
