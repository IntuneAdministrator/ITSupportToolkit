...
$exitCode=0; $Activity='Kerberos Armor Check'; $totalSteps=8; $Name='Kerberos-Armor-Check'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Export-HtmlReport -Title 'Kerberos Armor Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Kerberos Armor Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
