...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Stack-Reset.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Stack Reset'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
