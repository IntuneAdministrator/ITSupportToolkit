...
$exitCode=0; $Activity='DPAPI MasterKey Audit'; $totalSteps=8; $Name='DPAPI-MasterKey-Audit'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Export-HtmlReport -Title 'DPAPI MasterKey Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DPAPI MasterKey Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
