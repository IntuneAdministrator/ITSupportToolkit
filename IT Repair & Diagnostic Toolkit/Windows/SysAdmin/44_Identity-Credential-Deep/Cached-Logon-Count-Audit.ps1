...
<#
.SYNOPSIS
    Enterprise IT tool: Cached Logon Count Audit
.DESCRIPTION
    Phase 10 expansion (SysAdmin/44_Identity-Credential-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Cached-Logon-Count-Audit.ps1
.EXAMPLE
    .\Cached-Logon-Count-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Cached Logon Count Audit'; $totalSteps=8; $Name='Cached-Logon-Count-Audit'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Export-HtmlReport -Title 'Cached Logon Count Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cached Logon Count Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
