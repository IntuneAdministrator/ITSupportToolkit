...
<#
.SYNOPSIS
    Common problem fix: Fingerprint Enrollment Triage
.DESCRIPTION
...
.EXAMPLE
    .\Fingerprint-Enrollment-Triage.ps1
.EXAMPLE
    .\Fingerprint-Enrollment-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Fingerprint Enrollment Triage'; $totalSteps = 8; $Name = 'Fingerprint-Enrollment-Triage'
try {
    ...
    Export-HtmlReport -Title 'Fingerprint Enrollment Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Fingerprint Enrollment Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
