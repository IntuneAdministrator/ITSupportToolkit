...
<#
.SYNOPSIS
    Common problem fix: NFC Reader Proximity Card Audit
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 44_Identity-Credential-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\NFC-Reader-Proximity-Card-Audit.ps1
.EXAMPLE
    .\NFC-Reader-Proximity-Card-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NFC Reader Proximity Card Audit'; $totalSteps = 8; $Name = 'NFC-Reader-Proximity-Card-Audit'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '44_Identity-Credential-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '44_Identity-Credential-Deep'
    ...
    Export-HtmlReport -Title 'NFC Reader Proximity Card Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NFC Reader Proximity Card Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
