...
<#
.SYNOPSIS
    Common problem fix: Biometric Driver Health Audit
.DESCRIPTION
...
.EXAMPLE
    .\Biometric-Driver-Health-Audit.ps1
.EXAMPLE
    .\Biometric-Driver-Health-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Biometric Driver Health Audit'; $totalSteps = 8; $Name = 'Biometric-Driver-Health-Audit'
try {
    ...
    Export-HtmlReport -Title 'Biometric Driver Health Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Biometric Driver Health Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
