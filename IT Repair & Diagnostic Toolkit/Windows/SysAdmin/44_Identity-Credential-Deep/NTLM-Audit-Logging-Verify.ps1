...
<#
.SYNOPSIS
    Enterprise IT tool: NTLM Audit Logging Verify
.DESCRIPTION
    Phase 10 expansion (SysAdmin/44_Identity-Credential-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\NTLM-Audit-Logging-Verify.ps1
.EXAMPLE
    .\NTLM-Audit-Logging-Verify.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='NTLM Audit Logging Verify'; $totalSteps=8; $Name='NTLM-Audit-Logging-Verify'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Export-HtmlReport -Title 'NTLM Audit Logging Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NTLM Audit Logging Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
