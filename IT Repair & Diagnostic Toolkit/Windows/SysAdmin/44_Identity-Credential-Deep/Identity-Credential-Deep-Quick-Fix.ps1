...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Quick-Fix.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Quick Fix'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
