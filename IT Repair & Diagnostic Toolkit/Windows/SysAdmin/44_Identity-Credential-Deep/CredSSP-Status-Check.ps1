...
$exitCode=0; $Activity='CredSSP Status Check'; $totalSteps=8; $Name='CredSSP-Status-Check'; $Folder='44_Identity-Credential-Deep'
try {
    ...
    Export-HtmlReport -Title 'CredSSP Status Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CredSSP Status Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
