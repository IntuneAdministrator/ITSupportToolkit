...
<#
.SYNOPSIS
    Common problem fix: Identity Credential Deep Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Identity-Credential-Deep-Evidence-Report.ps1
.EXAMPLE
    .\Identity-Credential-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Identity Credential Deep Evidence Report'; $totalSteps = 8; $Name = 'Identity-Credential-Deep-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Identity Credential Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Identity Credential Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
