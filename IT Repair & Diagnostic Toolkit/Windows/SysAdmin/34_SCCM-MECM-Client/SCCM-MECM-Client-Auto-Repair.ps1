...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Auto-Repair.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Auto Repair'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
