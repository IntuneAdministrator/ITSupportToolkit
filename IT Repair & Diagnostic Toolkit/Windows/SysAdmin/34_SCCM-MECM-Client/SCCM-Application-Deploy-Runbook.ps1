...
<#
.SYNOPSIS
    Common problem fix: SCCM Application Deploy Runbook
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-Application-Deploy-Runbook.ps1
.EXAMPLE
    .\SCCM-Application-Deploy-Runbook.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM Application Deploy Runbook'; $totalSteps = 8; $Name = 'SCCM-Application-Deploy-Runbook'
try {
    ...
    Export-HtmlReport -Title 'SCCM Application Deploy Runbook' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM Application Deploy Runbook' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
