...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Stack-Reset.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Stack Reset'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
