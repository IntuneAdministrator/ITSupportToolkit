...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Evidence-Report.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Evidence Report'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
