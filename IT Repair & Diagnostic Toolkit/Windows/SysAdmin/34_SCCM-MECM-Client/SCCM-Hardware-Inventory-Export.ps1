...
<#
.SYNOPSIS
    Common problem fix: SCCM Hardware Inventory Export
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-Hardware-Inventory-Export.ps1
.EXAMPLE
    .\SCCM-Hardware-Inventory-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM Hardware Inventory Export'; $totalSteps = 8; $Name = 'SCCM-Hardware-Inventory-Export'
try {
    ...
    Export-HtmlReport -Title 'SCCM Hardware Inventory Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM Hardware Inventory Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
