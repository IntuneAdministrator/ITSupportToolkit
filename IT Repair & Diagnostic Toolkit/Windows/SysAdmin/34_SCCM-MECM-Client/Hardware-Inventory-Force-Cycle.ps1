...
<#
.SYNOPSIS
    Enterprise IT tool: Hardware Inventory Force Cycle
.DESCRIPTION
...
.EXAMPLE
    .\Hardware-Inventory-Force-Cycle.ps1
.EXAMPLE
    .\Hardware-Inventory-Force-Cycle.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Hardware Inventory Force Cycle'; $totalSteps = 8; $Name = 'Hardware-Inventory-Force-Cycle'
try {
    ...
    Export-HtmlReport -Title 'Hardware Inventory Force Cycle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware Inventory Force Cycle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
