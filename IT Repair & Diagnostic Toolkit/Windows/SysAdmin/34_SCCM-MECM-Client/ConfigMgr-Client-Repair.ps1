...
<#
.SYNOPSIS
    Enterprise IT tool: ConfigMgr Client Repair
.DESCRIPTION
...
.EXAMPLE
    .\ConfigMgr-Client-Repair.ps1
.EXAMPLE
    .\ConfigMgr-Client-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'ConfigMgr Client Repair'; $totalSteps = 8; $Name = 'ConfigMgr-Client-Repair'
try {
    ...
    Export-HtmlReport -Title 'ConfigMgr Client Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'ConfigMgr Client Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
