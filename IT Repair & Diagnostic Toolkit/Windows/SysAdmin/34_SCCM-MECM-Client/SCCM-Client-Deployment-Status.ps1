...
<#
.SYNOPSIS
    Common problem fix: SCCM Client Deployment Status
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-Client-Deployment-Status.ps1
.EXAMPLE
    .\SCCM-Client-Deployment-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM Client Deployment Status'; $totalSteps = 8; $Name = 'SCCM-Client-Deployment-Status'
try {
    ...
    Export-HtmlReport -Title 'SCCM Client Deployment Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM Client Deployment Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
