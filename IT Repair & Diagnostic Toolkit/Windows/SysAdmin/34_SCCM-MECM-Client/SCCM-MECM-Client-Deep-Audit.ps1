...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Deep-Audit.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Deep Audit'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
