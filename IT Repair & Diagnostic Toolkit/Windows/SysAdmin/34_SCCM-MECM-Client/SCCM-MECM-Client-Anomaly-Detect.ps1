...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Anomaly-Detect.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Anomaly Detect'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
