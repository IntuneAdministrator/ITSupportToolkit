...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Health-Triage.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Health Triage'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
