...
<#
.SYNOPSIS
    Enterprise IT tool: Software Center Reset
.DESCRIPTION
...
.EXAMPLE
    .\Software-Center-Reset.ps1
.EXAMPLE
    .\Software-Center-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Software Center Reset'; $totalSteps = 8; $Name = 'Software-Center-Reset'
try {
    ...
    Export-HtmlReport -Title 'Software Center Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Software Center Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
