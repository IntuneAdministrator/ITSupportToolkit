...
<#
.SYNOPSIS
    Common problem fix: SCCM Co Management Intune Sync Status
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-Co-Management-Intune-Sync-Status.ps1
.EXAMPLE
    .\SCCM-Co-Management-Intune-Sync-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM Co Management Intune Sync Status'; $totalSteps = 8; $Name = 'SCCM-Co-Management-Intune-Sync-Status'
try {
    ...
    Export-HtmlReport -Title 'SCCM Co Management Intune Sync Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM Co Management Intune Sync Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
