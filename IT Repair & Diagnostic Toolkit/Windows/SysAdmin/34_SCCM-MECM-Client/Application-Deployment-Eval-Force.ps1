...
<#
.SYNOPSIS
    Enterprise IT tool: Application Deployment Eval Force
.DESCRIPTION
...
.EXAMPLE
    .\Application-Deployment-Eval-Force.ps1
.EXAMPLE
    .\Application-Deployment-Eval-Force.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Application Deployment Eval Force'; $totalSteps = 8; $Name = 'Application-Deployment-Eval-Force'
try {
    ...
    Export-HtmlReport -Title 'Application Deployment Eval Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Application Deployment Eval Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
