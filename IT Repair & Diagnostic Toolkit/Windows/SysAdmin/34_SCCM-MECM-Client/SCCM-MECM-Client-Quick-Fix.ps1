...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Quick-Fix.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Quick Fix'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
