...
<#
.SYNOPSIS
    Common problem fix: SCCM MECM Client Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\SCCM-MECM-Client-Policy-Validate.ps1
.EXAMPLE
    .\SCCM-MECM-Client-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM MECM Client Policy Validate'; $totalSteps = 8; $Name = 'SCCM-MECM-Client-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'SCCM MECM Client Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM MECM Client Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
