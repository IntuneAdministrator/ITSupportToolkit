...
<#
.SYNOPSIS
    Enterprise IT tool: Client Certificate Repair CM
.DESCRIPTION
...
.EXAMPLE
    .\Client-Certificate-Repair-CM.ps1
.EXAMPLE
    .\Client-Certificate-Repair-CM.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Client Certificate Repair CM'; $totalSteps = 8; $Name = 'Client-Certificate-Repair-CM'
try {
    ...
    Export-HtmlReport -Title 'Client Certificate Repair CM' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Client Certificate Repair CM' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
