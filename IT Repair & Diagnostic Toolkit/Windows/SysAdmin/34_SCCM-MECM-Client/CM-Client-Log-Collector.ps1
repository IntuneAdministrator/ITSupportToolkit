...
<#
.SYNOPSIS
    Enterprise IT tool: CM Client Log Collector
.DESCRIPTION
...
.EXAMPLE
    .\CM-Client-Log-Collector.ps1
.EXAMPLE
    .\CM-Client-Log-Collector.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CM Client Log Collector'; $totalSteps = 8; $Name = 'CM-Client-Log-Collector'
try {
    ...
    Export-HtmlReport -Title 'CM Client Log Collector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CM Client Log Collector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
