...
<#
.SYNOPSIS
    Enterprise IT tool: MECM Policy Force Cycle
.DESCRIPTION
...
.EXAMPLE
    .\MECM-Policy-Force-Cycle.ps1
.EXAMPLE
    .\MECM-Policy-Force-Cycle.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'MECM Policy Force Cycle'; $totalSteps = 8; $Name = 'MECM-Policy-Force-Cycle'
try {
    ...
    Export-HtmlReport -Title 'MECM Policy Force Cycle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'MECM Policy Force Cycle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
