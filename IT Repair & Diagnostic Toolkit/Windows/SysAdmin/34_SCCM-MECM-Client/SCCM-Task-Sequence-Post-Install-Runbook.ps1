...
<#
.SYNOPSIS
    Common problem fix: SCCM Task Sequence Post Install Runbook
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 34_SCCM-MECM-Client). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\SCCM-Task-Sequence-Post-Install-Runbook.ps1
.EXAMPLE
    .\SCCM-Task-Sequence-Post-Install-Runbook.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SCCM Task Sequence Post Install Runbook'; $totalSteps = 8; $Name = 'SCCM-Task-Sequence-Post-Install-Runbook'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '34_SCCM-MECM-Client' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '34_SCCM-MECM-Client'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '34_SCCM-MECM-Client'
    ...
    Export-HtmlReport -Title 'SCCM Task Sequence Post Install Runbook' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SCCM Task Sequence Post Install Runbook' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
