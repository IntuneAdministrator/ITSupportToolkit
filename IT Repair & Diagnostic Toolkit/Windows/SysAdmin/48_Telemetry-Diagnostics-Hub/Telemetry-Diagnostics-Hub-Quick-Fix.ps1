...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Quick-Fix.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Quick Fix'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
