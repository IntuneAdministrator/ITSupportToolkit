...
<#
.SYNOPSIS
    Enterprise IT tool: Delivery Optimization Stats
.DESCRIPTION
    Phase 10 expansion (SysAdmin/48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Delivery-Optimization-Stats.ps1
.EXAMPLE
    .\Delivery-Optimization-Stats.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Delivery Optimization Stats'; $totalSteps=8; $Name='Delivery-Optimization-Stats'; $Folder='48_Telemetry-Diagnostics-Hub'
try {
    ...
    Export-HtmlReport -Title 'Delivery Optimization Stats' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Delivery Optimization Stats' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
