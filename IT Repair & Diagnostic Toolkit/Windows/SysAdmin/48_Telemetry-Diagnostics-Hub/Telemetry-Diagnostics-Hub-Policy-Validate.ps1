...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Policy-Validate.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Policy Validate'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
