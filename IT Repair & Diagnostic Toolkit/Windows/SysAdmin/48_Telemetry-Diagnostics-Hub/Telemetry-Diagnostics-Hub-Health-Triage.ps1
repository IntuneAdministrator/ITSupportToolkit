...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Health-Triage.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Health Triage'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
