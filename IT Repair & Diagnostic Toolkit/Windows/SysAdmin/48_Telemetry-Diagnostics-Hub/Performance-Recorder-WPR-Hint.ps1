...
<#
.SYNOPSIS
    Enterprise IT tool: Performance Recorder WPR Hint
.DESCRIPTION
    Phase 10 expansion (SysAdmin/48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Performance-Recorder-WPR-Hint.ps1
.EXAMPLE
    .\Performance-Recorder-WPR-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Performance Recorder WPR Hint'; $totalSteps=8; $Name='Performance-Recorder-WPR-Hint'; $Folder='48_Telemetry-Diagnostics-Hub'
try {
    ...
    Export-HtmlReport -Title 'Performance Recorder WPR Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Recorder WPR Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
