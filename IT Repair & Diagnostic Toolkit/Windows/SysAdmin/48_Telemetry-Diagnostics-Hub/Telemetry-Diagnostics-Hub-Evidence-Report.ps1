...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Evidence-Report.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Evidence Report'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '48_Telemetry-Diagnostics-Hub' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '48_Telemetry-Diagnostics-Hub'
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
