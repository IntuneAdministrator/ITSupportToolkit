...
<#
.SYNOPSIS
    Enterprise IT tool: Windows Error Reporting Queue Clear
.DESCRIPTION
    Phase 10 expansion (SysAdmin/48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Windows-Error-Reporting-Queue-Clear.ps1
.EXAMPLE
    .\Windows-Error-Reporting-Queue-Clear.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Windows Error Reporting Queue Clear'; $totalSteps=8; $Name='Windows-Error-Reporting-Queue-Clear'; $Folder='48_Telemetry-Diagnostics-Hub'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '48_Telemetry-Diagnostics-Hub' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'TELEMETRY / DIAGNOSTICS HUB'
        $dt = Get-Service DiagTrack,dmsvc,WerSvc -EA SilentlyContinue
        foreach ($s in $dt) { Add-ReportRow -Check $s.Name -Result "$($s.Status)/$($s.StartType)" -Status 'INFO' -Detail $s.DisplayName }
        Write-ProgressStep 2 $totalSteps $Activity 'Services'
        if ($Name -match 'WER|Queue' -and $PSCmdlet.ShouldProcess('WER','Clear queue sample')) {
            Remove-Item "$env:ProgramData\Microsoft\Windows\WER\ReportQueue\*" -Recurse -Force -EA SilentlyContinue
            Add-ReportRow -Check 'WER ReportQueue' -Result 'Cleared' -Status 'PASS' -Detail ''
        }
        if ($Name -match 'Reliability') {
            $rel = Get-CimInstance -Namespace root\cimv2 -ClassName Win32_ReliabilityStabilityMetrics -EA SilentlyContinue | Select-Object -First 5
            Add-ReportRow -Check 'Reliability metrics' -Result "$(@($rel).Count) samples" -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Delivery-Optimization') {
            try { Get-DeliveryOptimizationStatus -EA SilentlyContinue | Out-String | ForEach-Object { Add-ReportRow -Check 'DO status' -Result 'Captured' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(300,$_.Length)) } } catch {}
        }
        if ($Name -match 'DiagTrack|Tune' -and $PSCmdlet.ShouldProcess('DiagTrack','Restart')) {
            Restart-Service DiagTrack -Force -EA SilentlyContinue
            Add-ReportRow -Check 'DiagTrack restart' -Result 'Done' -Status 'PASS' -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Diagnostics done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Windows Error Reporting Queue Clear' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Error Reporting Queue Clear' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
