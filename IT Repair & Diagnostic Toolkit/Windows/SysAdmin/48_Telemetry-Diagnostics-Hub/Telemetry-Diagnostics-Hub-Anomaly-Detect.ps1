...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Anomaly-Detect.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Anomaly Detect'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
