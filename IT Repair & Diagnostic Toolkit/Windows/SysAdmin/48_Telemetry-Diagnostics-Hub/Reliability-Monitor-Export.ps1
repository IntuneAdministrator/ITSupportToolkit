...
<#
.SYNOPSIS
    Enterprise IT tool: Reliability Monitor Export
.DESCRIPTION
    Phase 10 expansion (SysAdmin/48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Reliability-Monitor-Export.ps1
.EXAMPLE
    .\Reliability-Monitor-Export.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Reliability Monitor Export'; $totalSteps=8; $Name='Reliability-Monitor-Export'; $Folder='48_Telemetry-Diagnostics-Hub'
try {
    ...
    Export-HtmlReport -Title 'Reliability Monitor Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Reliability Monitor Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
