...
<#
.SYNOPSIS
    Enterprise IT tool: Diagnostic Data Viewer Prep
.DESCRIPTION
    Phase 10 expansion (SysAdmin/48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Diagnostic-Data-Viewer-Prep.ps1
.EXAMPLE
    .\Diagnostic-Data-Viewer-Prep.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Diagnostic Data Viewer Prep'; $totalSteps=8; $Name='Diagnostic-Data-Viewer-Prep'; $Folder='48_Telemetry-Diagnostics-Hub'
try {
    ...
    Export-HtmlReport -Title 'Diagnostic Data Viewer Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Diagnostic Data Viewer Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
