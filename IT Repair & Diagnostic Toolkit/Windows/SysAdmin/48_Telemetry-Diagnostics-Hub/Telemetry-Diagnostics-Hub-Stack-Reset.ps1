...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Stack-Reset.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Stack Reset'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
