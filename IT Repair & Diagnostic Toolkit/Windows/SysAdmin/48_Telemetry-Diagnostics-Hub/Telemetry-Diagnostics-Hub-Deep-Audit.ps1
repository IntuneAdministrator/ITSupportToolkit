...
<#
.SYNOPSIS
    Common problem fix: Telemetry Diagnostics Hub Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Deep-Audit.ps1
.EXAMPLE
    .\Telemetry-Diagnostics-Hub-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Telemetry Diagnostics Hub Deep Audit'; $totalSteps = 8; $Name = 'Telemetry-Diagnostics-Hub-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Telemetry Diagnostics Hub Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Telemetry Diagnostics Hub Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
