...
<#
.SYNOPSIS
    Enterprise IT tool: Connected User Experiences Audit
.DESCRIPTION
    Phase 10 expansion (SysAdmin/48_Telemetry-Diagnostics-Hub). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Connected-User-Experiences-Audit.ps1
.EXAMPLE
    .\Connected-User-Experiences-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Connected User Experiences Audit'; $totalSteps=8; $Name='Connected-User-Experiences-Audit'; $Folder='48_Telemetry-Diagnostics-Hub'
try {
    ...
    Export-HtmlReport -Title 'Connected User Experiences Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Connected User Experiences Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
