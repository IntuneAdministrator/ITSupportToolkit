...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 15_Performance-Optimization). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Performance-Optimization-Policy-Validate.ps1
.EXAMPLE
    .\Performance-Optimization-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Policy Validate'; $totalSteps = 8; $Name = 'Performance-Optimization-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '15_Performance-Optimization' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Performance Optimization Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
