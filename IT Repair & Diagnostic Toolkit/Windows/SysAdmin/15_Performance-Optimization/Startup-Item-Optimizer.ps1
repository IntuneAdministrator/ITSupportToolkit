...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Startup Item Optimizer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Startup-Item-Optimizer.
...
$exitCode = 0
$Activity = 'Startup Item Optimizer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Startup-Item-Optimizer'
        Add-ReportRow -Check 'Tool' -Result 'Startup-Item-Optimizer' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Startup Item Optimizer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Startup Item Optimizer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
