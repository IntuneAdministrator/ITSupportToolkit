...
<#
.SYNOPSIS
    Common problem fix: Memory Leak Process Audit
.DESCRIPTION
...
.EXAMPLE
    .\Memory-Leak-Process-Audit.ps1
.EXAMPLE
    .\Memory-Leak-Process-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Memory Leak Process Audit'; $totalSteps = 8; $Name = 'Memory-Leak-Process-Audit'
try {
    ...
    Export-HtmlReport -Title 'Memory Leak Process Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Memory Leak Process Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
