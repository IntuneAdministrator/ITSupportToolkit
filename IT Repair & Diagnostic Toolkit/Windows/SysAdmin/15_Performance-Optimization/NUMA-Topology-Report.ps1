...
<#
.SYNOPSIS
    Common problem fix: NUMA Topology Report
.DESCRIPTION
...
.EXAMPLE
    .\NUMA-Topology-Report.ps1
.EXAMPLE
    .\NUMA-Topology-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NUMA Topology Report'; $totalSteps = 8; $Name = 'NUMA-Topology-Report'
try {
    ...
    Export-HtmlReport -Title 'NUMA Topology Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NUMA Topology Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
