...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Interrupt Affinity Tuner
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Interrupt-Affinity-Tuner.
...
.EXAMPLE
    .\Interrupt-Affinity-Tuner.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Interrupt-Affinity-Tuner.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Interrupt Affinity Tuner'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Interrupt-Affinity-Tuner'
        Add-ReportRow -Check 'Tool' -Result 'Interrupt-Affinity-Tuner' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Interrupt Affinity Tuner' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Interrupt Affinity Tuner' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
