...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Power Plan Optimizer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Power-Plan-Optimizer.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Power-Plan-Optimizer'
        Add-ReportRow -Check 'Tool' -Result 'Power-Plan-Optimizer' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Power Plan Optimizer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Power Plan Optimizer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
