...
<#
.SYNOPSIS
    Common problem fix: CAD App Memory Pressure Audit
.DESCRIPTION
...
.EXAMPLE
    .\CAD-App-Memory-Pressure-Audit.ps1
.EXAMPLE
    .\CAD-App-Memory-Pressure-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CAD App Memory Pressure Audit'; $totalSteps = 8; $Name = 'CAD-App-Memory-Pressure-Audit'
try {
    ...
    Export-HtmlReport -Title 'CAD App Memory Pressure Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CAD App Memory Pressure Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
