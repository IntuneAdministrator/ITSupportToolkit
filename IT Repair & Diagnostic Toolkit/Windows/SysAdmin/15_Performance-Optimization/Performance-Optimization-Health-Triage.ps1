...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Health-Triage.ps1
.EXAMPLE
    .\Performance-Optimization-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Health Triage'; $totalSteps = 8; $Name = 'Performance-Optimization-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
