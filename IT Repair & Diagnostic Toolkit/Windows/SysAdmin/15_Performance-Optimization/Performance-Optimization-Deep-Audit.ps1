...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Deep-Audit.ps1
.EXAMPLE
    .\Performance-Optimization-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Deep Audit'; $totalSteps = 8; $Name = 'Performance-Optimization-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
