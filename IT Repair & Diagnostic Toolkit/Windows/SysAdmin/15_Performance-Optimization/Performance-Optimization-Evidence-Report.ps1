...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Evidence-Report.ps1
.EXAMPLE
    .\Performance-Optimization-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Evidence Report'; $totalSteps = 8; $Name = 'Performance-Optimization-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
