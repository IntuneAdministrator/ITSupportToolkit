...
<#
.SYNOPSIS
    Common problem fix: Server Disk Queue Length Alert
.DESCRIPTION
...
.EXAMPLE
    .\Server-Disk-Queue-Length-Alert.ps1
.EXAMPLE
    .\Server-Disk-Queue-Length-Alert.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Server Disk Queue Length Alert'; $totalSteps = 8; $Name = 'Server-Disk-Queue-Length-Alert'
try {
    ...
    Export-HtmlReport -Title 'Server Disk Queue Length Alert' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Server Disk Queue Length Alert' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
