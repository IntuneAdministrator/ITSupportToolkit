...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Memory Compression Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Memory-Compression-Audit.
...
.EXAMPLE
    .\Memory-Compression-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Memory-Compression-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Memory Compression Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Memory-Compression-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Memory-Compression-Audit' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Memory Compression Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Memory Compression Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
