...
<#
.SYNOPSIS
    Common problem fix: High CPU Process Report
.DESCRIPTION
...
.EXAMPLE
    .\High-CPU-Process-Report.ps1
.EXAMPLE
    .\High-CPU-Process-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'High CPU Process Report'; $totalSteps = 8; $Name = 'High-CPU-Process-Report'
try {
    ...
    Export-HtmlReport -Title 'High CPU Process Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'High CPU Process Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
