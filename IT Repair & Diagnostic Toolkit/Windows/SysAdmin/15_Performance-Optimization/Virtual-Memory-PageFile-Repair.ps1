...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Virtual Memory PageFile Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Virtual-Memory-PageFile-Repair.
...
.EXAMPLE
    .\Virtual-Memory-PageFile-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Virtual-Memory-PageFile-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Virtual Memory PageFile Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Virtual-Memory-PageFile-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Virtual-Memory-PageFile-Repair' -Status 'INFO' -Detail '15_Performance-Optimization'
        Write-SectionBanner 'MEMORY'
        $os = Get-CimInstance Win32_OperatingSystem
        $totalGB = [math]::Round($os.TotalVisibleMemorySize/1MB, 2)
        $freeGB = [math]::Round($os.FreePhysicalMemory/1MB, 2)
        Add-ReportRow -Check 'Physical memory' -Result ("{0} GB free / {1} GB" -f $freeGB, $totalGB) -Status $(if ($freeGB -gt 1) {'PASS'} else {'WARN'}) -Detail ''
        Get-Process | Sort-Object WorkingSet64 -Descending | Select-Object -First 12 | ForEach-Object {
            Add-ReportRow -Check $_.ProcessName -Result ("{0:N0} MB" -f ($_.WorkingSet64/1MB)) -Status $(if ($_.WorkingSet64 -gt 2GB) {'WARN'} else {'INFO'}) -Detail "PID=$($_.Id)"
        }
        $pf = Get-CimInstance Win32_PageFileUsage -ErrorAction SilentlyContinue
        foreach ($p in @($pf)) {
            Add-ReportRow -Check "Pagefile $($p.Name)" -Result ("Allocated={0}MB InUse={1}MB" -f $p.AllocatedBaseSize, $p.CurrentUsage) -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Memory'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Virtual Memory PageFile Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Virtual Memory PageFile Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
