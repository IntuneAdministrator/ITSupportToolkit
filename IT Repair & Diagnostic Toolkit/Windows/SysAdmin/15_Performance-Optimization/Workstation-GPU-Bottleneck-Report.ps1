...
<#
.SYNOPSIS
    Common problem fix: Workstation GPU Bottleneck Report
.DESCRIPTION
...
.EXAMPLE
    .\Workstation-GPU-Bottleneck-Report.ps1
.EXAMPLE
    .\Workstation-GPU-Bottleneck-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Workstation GPU Bottleneck Report'; $totalSteps = 8; $Name = 'Workstation-GPU-Bottleneck-Report'
try {
    ...
    Export-HtmlReport -Title 'Workstation GPU Bottleneck Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Workstation GPU Bottleneck Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
