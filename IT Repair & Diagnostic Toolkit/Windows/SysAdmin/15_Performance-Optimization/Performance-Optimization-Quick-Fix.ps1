...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Quick-Fix.ps1
.EXAMPLE
    .\Performance-Optimization-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Quick Fix'; $totalSteps = 8; $Name = 'Performance-Optimization-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
