...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: CPU Core Parking Disable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for CPU-Core-Parking-Disable.
...
.EXAMPLE
    .\CPU-Core-Parking-Disable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\CPU-Core-Parking-Disable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'CPU Core Parking Disable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'CPU-Core-Parking-Disable'
        Add-ReportRow -Check 'Tool' -Result 'CPU-Core-Parking-Disable' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'CPU Core Parking Disable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'CPU Core Parking Disable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
