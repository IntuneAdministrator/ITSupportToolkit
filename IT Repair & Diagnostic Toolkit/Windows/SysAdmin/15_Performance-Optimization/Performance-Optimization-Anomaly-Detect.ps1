...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Anomaly-Detect.ps1
.EXAMPLE
    .\Performance-Optimization-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Anomaly Detect'; $totalSteps = 8; $Name = 'Performance-Optimization-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
