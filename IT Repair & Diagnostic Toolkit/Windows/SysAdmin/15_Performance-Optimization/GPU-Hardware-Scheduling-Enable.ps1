...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPU Hardware Scheduling Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPU-Hardware-Scheduling-Enable.
Category: 15_Performance-Optimization. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\GPU-Hardware-Scheduling-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\GPU-Hardware-Scheduling-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'GPU Hardware Scheduling Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PERFORMANCE OPTIMIZATION'
        $Name = 'GPU-Hardware-Scheduling-Enable'
        Add-ReportRow -Check 'Tool' -Result 'GPU-Hardware-Scheduling-Enable' -Status 'INFO' -Detail '15_Performance-Optimization'
        Write-SectionBanner 'POWER / CPU TUNING'
        $plans = powercfg /list 2>&1 | Out-String
        Add-ReportRow -Check 'Power plans' -Result 'Listed' -Status 'INFO' -Detail ($plans.Substring(0, [Math]::Min(400, $plans.Length)))
        if ($Name -match 'Power-Plan|Gaming|CPU-Throttle' -and $PSCmdlet.ShouldProcess('Power', 'Set High Performance')) {
            powercfg /setactive SCHEME_MIN 2>$null
            # SCHEME_MIN is Power saver on some builds — use GUID for High Performance
            powercfg /setactive 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c 2>$null
            Add-ReportRow -Check 'High Performance plan' -Result 'Activated (if present)' -Status 'PASS' -Detail '8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c'
        }
        if ($Name -match 'Core-Parking' -and $PSCmdlet.ShouldProcess('Core parking', 'Disable via powercfg')) {
            powercfg -setacvalueindex scheme_current sub_processor CPMINCORES 100 2>$null
            powercfg -setactive scheme_current 2>$null
            Add-ReportRow -Check 'CPMINCORES' -Result '100%' -Status 'PASS' -Detail 'Core parking minimized'
        }
        if ($Name -match 'Network-Throttling') {
            $backupDir = 'C:\IT-Logs\Backups'
            $mmcss = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile'
            Backup-RegistryKey -KeyPath $mmcss -BackupDir $backupDir | Out-Null
            if ($PSCmdlet.ShouldProcess('NetworkThrottlingIndex', 'Set to 0xFFFFFFFF')) {
                New-ItemProperty $mmcss -Name NetworkThrottlingIndex -Value 0xFFFFFFFF -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'NetworkThrottlingIndex' -Result '0xFFFFFFFF' -Status 'PASS' -Detail 'Rollback via .reg'
            }
        }
        if ($Name -match 'GPU-Hardware') {
            $hwSch = 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers'
            Backup-RegistryKey -KeyPath $hwSch -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            if ($PSCmdlet.ShouldProcess('HwSchMode', 'Enable hardware-accelerated GPU scheduling')) {
                New-ItemProperty $hwSch -Name HwSchMode -Value 2 -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'HwSchMode' -Result '2' -Status 'PASS' -Detail 'Reboot required'
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Power/CPU'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'GPU Hardware Scheduling Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPU Hardware Scheduling Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
