...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System Responsiveness Tune
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System-Responsiveness-Tune.
...
.EXAMPLE
    .\System-Responsiveness-Tune.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\System-Responsiveness-Tune.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'System Responsiveness Tune'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'System-Responsiveness-Tune'
        Add-ReportRow -Check 'Tool' -Result 'System-Responsiveness-Tune' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'System Responsiveness Tune' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System Responsiveness Tune' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
