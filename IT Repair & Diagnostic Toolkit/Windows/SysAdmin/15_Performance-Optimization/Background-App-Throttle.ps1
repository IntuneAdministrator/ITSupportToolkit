...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Background App Throttle
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Background-App-Throttle.
...
.EXAMPLE
    .\Background-App-Throttle.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Background-App-Throttle.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Background App Throttle'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Background-App-Throttle'
        Add-ReportRow -Check 'Tool' -Result 'Background-App-Throttle' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Background App Throttle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Background App Throttle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
