...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Auto-Repair.ps1
.EXAMPLE
    .\Performance-Optimization-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Auto Repair'; $totalSteps = 8; $Name = 'Performance-Optimization-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
