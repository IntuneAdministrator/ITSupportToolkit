...
<#
.SYNOPSIS
    Common problem fix: Superfetch SysMain Triage
.DESCRIPTION
...
.EXAMPLE
    .\Superfetch-SysMain-Triage.ps1
.EXAMPLE
    .\Superfetch-SysMain-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Superfetch SysMain Triage'; $totalSteps = 8; $Name = 'Superfetch-SysMain-Triage'
try {
    ...
    Export-HtmlReport -Title 'Superfetch SysMain Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Superfetch SysMain Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
