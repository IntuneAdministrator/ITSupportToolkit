...
<#
.SYNOPSIS
    Common problem fix: Performance Optimization Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Performance-Optimization-Stack-Reset.ps1
.EXAMPLE
    .\Performance-Optimization-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Performance Optimization Stack Reset'; $totalSteps = 8; $Name = 'Performance-Optimization-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Performance Optimization Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Performance Optimization Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
