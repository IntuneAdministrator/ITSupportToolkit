...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NUMA Topology Optimizer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NUMA-Topology-Optimizer.
...
.EXAMPLE
    .\NUMA-Topology-Optimizer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\NUMA-Topology-Optimizer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'NUMA Topology Optimizer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'NUMA-Topology-Optimizer'
        Add-ReportRow -Check 'Tool' -Result 'NUMA-Topology-Optimizer' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'NUMA Topology Optimizer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NUMA Topology Optimizer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
