...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Storage IO Scheduler Optimize
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Storage-IO-Scheduler-Optimize.
...
.EXAMPLE
    .\Storage-IO-Scheduler-Optimize.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Storage-IO-Scheduler-Optimize.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Storage IO Scheduler Optimize'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Storage-IO-Scheduler-Optimize'
        Add-ReportRow -Check 'Tool' -Result 'Storage-IO-Scheduler-Optimize' -Status 'INFO' -Detail '15_Performance-Optimization'
        Write-SectionBanner 'STARTUP / IO PERFORMANCE'
        try {
            $startup = Get-CimInstance Win32_StartupCommand -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Startup commands' -Result "$($startup.Count)" -Status $(if ($startup.Count -lt 15) {'PASS'} else {'WARN'}) -Detail (($startup | Select-Object -First 10 Name) -join '; ')
        } catch {}
        $sysMain = Get-Service SysMain -ErrorAction SilentlyContinue
        if ($sysMain) {
            Add-ReportRow -Check 'SysMain (Superfetch)' -Result "$($sysMain.Status)/$($sysMain.StartType)" -Status 'INFO' -Detail ''
            if ($Name -match 'Superfetch' -and $PSCmdlet.ShouldProcess('SysMain', 'Restart')) {
                Restart-Service SysMain -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'SysMain restart' -Result 'Done' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Background-App') {
            $bak = 'C:\IT-Logs\Backups'
            $bg = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\BackgroundAccessApplications'
            if (Test-Path $bg) { Backup-RegistryKey -KeyPath $bg -BackupDir $bak | Out-Null }
            Add-ReportRow -Check 'Background apps key' -Result $(if (Test-Path $bg) {'Present'} else {'Absent'}) -Status 'INFO' -Detail $bg
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Startup/IO'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Storage IO Scheduler Optimize' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Storage IO Scheduler Optimize' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
