...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Thermal Throttle Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Thermal-Throttle-Detector.
...
.EXAMPLE
    .\Thermal-Throttle-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Thermal-Throttle-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Thermal Throttle Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Thermal-Throttle-Detector' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Thermal Throttle Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Thermal Throttle Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
