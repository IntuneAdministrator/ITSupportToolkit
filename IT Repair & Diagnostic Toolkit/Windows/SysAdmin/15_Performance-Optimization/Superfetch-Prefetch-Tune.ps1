...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Superfetch Prefetch Tune
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Superfetch-Prefetch-Tune.
...
.EXAMPLE
    .\Superfetch-Prefetch-Tune.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Superfetch-Prefetch-Tune.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Superfetch Prefetch Tune'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Superfetch-Prefetch-Tune'
        Add-ReportRow -Check 'Tool' -Result 'Superfetch-Prefetch-Tune' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Superfetch Prefetch Tune' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Superfetch Prefetch Tune' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
