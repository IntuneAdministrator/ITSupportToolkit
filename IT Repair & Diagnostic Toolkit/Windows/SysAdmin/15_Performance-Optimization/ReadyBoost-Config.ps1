...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ReadyBoost Config
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ReadyBoost-Config.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'ReadyBoost-Config'
        Add-ReportRow -Check 'Tool' -Result 'ReadyBoost-Config' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'ReadyBoost Config' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ReadyBoost Config' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
