...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Process Priority Manager
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Process-Priority-Manager.
...
.EXAMPLE
    .\Process-Priority-Manager.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Process-Priority-Manager.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Process Priority Manager'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Process-Priority-Manager'
        Add-ReportRow -Check 'Tool' -Result 'Process-Priority-Manager' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Process Priority Manager' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Process Priority Manager' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
