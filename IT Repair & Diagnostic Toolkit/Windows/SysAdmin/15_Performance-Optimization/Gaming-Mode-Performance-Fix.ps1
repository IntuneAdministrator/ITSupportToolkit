...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Gaming Mode Performance Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Gaming-Mode-Performance-Fix.
...
.EXAMPLE
    .\Gaming-Mode-Performance-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Gaming-Mode-Performance-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Gaming Mode Performance Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Gaming-Mode-Performance-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Gaming-Mode-Performance-Fix' -Status 'INFO' -Detail '15_Performance-Optimization'
    ...
    Export-HtmlReport -Title 'Gaming Mode Performance Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Gaming Mode Performance Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
