...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Thunderbolt Controller Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Thunderbolt-Controller-Audit.
...
.EXAMPLE
    .\Thunderbolt-Controller-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Thunderbolt-Controller-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Thunderbolt Controller Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Thunderbolt-Controller-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Thunderbolt-Controller-Audit' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Thunderbolt Controller Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Thunderbolt Controller Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
