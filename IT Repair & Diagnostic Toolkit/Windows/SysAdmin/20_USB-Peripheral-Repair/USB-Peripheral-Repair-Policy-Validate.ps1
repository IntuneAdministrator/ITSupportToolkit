...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Policy-Validate.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Policy Validate'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
