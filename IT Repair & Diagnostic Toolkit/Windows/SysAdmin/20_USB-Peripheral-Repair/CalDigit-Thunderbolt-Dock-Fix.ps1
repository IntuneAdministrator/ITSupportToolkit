...
<#
.SYNOPSIS
    Common problem fix: CalDigit Thunderbolt Dock Fix
.DESCRIPTION
...
.EXAMPLE
    .\CalDigit-Thunderbolt-Dock-Fix.ps1
.EXAMPLE
    .\CalDigit-Thunderbolt-Dock-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CalDigit Thunderbolt Dock Fix'; $totalSteps = 8; $Name = 'CalDigit-Thunderbolt-Dock-Fix'
try {
    ...
    Export-HtmlReport -Title 'CalDigit Thunderbolt Dock Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CalDigit Thunderbolt Dock Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
