...
<#
.SYNOPSIS
    Common problem fix: Avocent KVM Display Switch Fix
.DESCRIPTION
...
.EXAMPLE
    .\Avocent-KVM-Display-Switch-Fix.ps1
.EXAMPLE
    .\Avocent-KVM-Display-Switch-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Avocent KVM Display Switch Fix'; $totalSteps = 8; $Name = 'Avocent-KVM-Display-Switch-Fix'
try {
    ...
    Export-HtmlReport -Title 'Avocent KVM Display Switch Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Avocent KVM Display Switch Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
