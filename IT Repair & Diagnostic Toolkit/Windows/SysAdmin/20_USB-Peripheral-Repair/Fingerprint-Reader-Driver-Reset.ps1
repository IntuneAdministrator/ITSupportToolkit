...
<#
.SYNOPSIS
    Common problem fix: Fingerprint Reader Driver Reset
.DESCRIPTION
...
.EXAMPLE
    .\Fingerprint-Reader-Driver-Reset.ps1
.EXAMPLE
    .\Fingerprint-Reader-Driver-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Fingerprint Reader Driver Reset'; $totalSteps = 8; $Name = 'Fingerprint-Reader-Driver-Reset'
try {
    ...
    Export-HtmlReport -Title 'Fingerprint Reader Driver Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Fingerprint Reader Driver Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
