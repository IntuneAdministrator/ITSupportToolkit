...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Webcam App Conflict Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Webcam-App-Conflict-Detector.
...
.EXAMPLE
    .\Webcam-App-Conflict-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-App-Conflict-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam App Conflict Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Webcam-App-Conflict-Detector'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-App-Conflict-Detector' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Webcam App Conflict Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam App Conflict Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
