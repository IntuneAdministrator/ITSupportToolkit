...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Stack Deep Reset
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Stack-Deep-Reset.ps1
.EXAMPLE
    .\Bluetooth-Stack-Deep-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Stack Deep Reset'; $totalSteps = 8; $Name = 'Bluetooth-Stack-Deep-Reset'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Stack Deep Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Stack Deep Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
