...
<#
.SYNOPSIS
    Common problem fix: KVM EDID Emulator Missing Hint
.DESCRIPTION
...
.EXAMPLE
    .\KVM-EDID-Emulator-Missing-Hint.ps1
.EXAMPLE
    .\KVM-EDID-Emulator-Missing-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'KVM EDID Emulator Missing Hint'; $totalSteps = 8; $Name = 'KVM-EDID-Emulator-Missing-Hint'
try {
    ...
    Export-HtmlReport -Title 'KVM EDID Emulator Missing Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'KVM EDID Emulator Missing Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
