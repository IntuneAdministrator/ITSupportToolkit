...
<#
.SYNOPSIS
    Common problem fix: Dual Monitor KVM Black Screen
.DESCRIPTION
...
.EXAMPLE
    .\Dual-Monitor-KVM-Black-Screen.ps1
.EXAMPLE
    .\Dual-Monitor-KVM-Black-Screen.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dual Monitor KVM Black Screen'; $totalSteps = 8; $Name = 'Dual-Monitor-KVM-Black-Screen'
try {
    ...
    Export-HtmlReport -Title 'Dual Monitor KVM Black Screen' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dual Monitor KVM Black Screen' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
