...
<#
.SYNOPSIS
    Common problem fix: Raritan KVM USB Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\Raritan-KVM-USB-Conflict-Fix.ps1
.EXAMPLE
    .\Raritan-KVM-USB-Conflict-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Raritan KVM USB Conflict Fix'; $totalSteps = 8; $Name = 'Raritan-KVM-USB-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'Raritan KVM USB Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Raritan KVM USB Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
