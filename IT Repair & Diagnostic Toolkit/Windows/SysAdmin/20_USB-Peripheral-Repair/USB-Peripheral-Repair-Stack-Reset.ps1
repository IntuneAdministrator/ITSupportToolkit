...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Stack-Reset.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Stack Reset'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
