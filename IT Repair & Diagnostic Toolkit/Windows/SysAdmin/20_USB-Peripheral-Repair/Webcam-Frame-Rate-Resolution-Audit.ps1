...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Webcam Frame Rate Resolution Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Webcam-Frame-Rate-Resolution-Audit.
...
.EXAMPLE
    .\Webcam-Frame-Rate-Resolution-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-Frame-Rate-Resolution-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam Frame Rate Resolution Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Webcam-Frame-Rate-Resolution-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-Frame-Rate-Resolution-Audit' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
        Write-SectionBanner 'PERIPHERALS'
        $classes = @('Camera','Image','HIDClass','Keyboard','Mouse','SmartCardReader')
        foreach ($c in $classes) {
            $devs = @(Get-PnpDevice -Class $c -ErrorAction SilentlyContinue)
            if ($devs.Count -eq 0) { continue }
            Add-ReportRow -Check "Class $c" -Result "$($devs.Count) devices" -Status 'INFO' -Detail (($devs | Select-Object -First 5 FriendlyName) -join '; ')
            foreach ($d in ($devs | Where-Object Status -ne 'OK')) {
                Add-ReportRow -Check $d.FriendlyName -Result $d.Status -Status 'WARN' -Detail $d.InstanceId
                if ($PSCmdlet.ShouldProcess($d.FriendlyName, 'Re-enumerate device')) {
                    Disable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                    Start-Sleep -Seconds 1
                    Enable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                    Add-ReportRow -Check "Reset $($d.FriendlyName)" -Result 'Toggled' -Status 'PASS' -Detail ''
                }
            }
        }
        if ($Name -match 'Webcam-Permission|Privacy') {
            $cam = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\webcam'
            if (Test-Path $cam) {
                Backup-RegistryKey -KeyPath $cam -BackupDir 'C:\IT-Logs\Backups' | Out-Null
                if ($PSCmdlet.ShouldProcess('Webcam consent', 'Set Allow')) {
                    Set-ItemProperty $cam -Name Value -Value 'Allow' -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Webcam capability' -Result 'Allow' -Status 'PASS' -Detail $cam
                }
            } else {
                Add-ReportRow -Check 'Webcam capability' -Result 'Key missing' -Status 'WARN' -Detail $cam
            }
        }
        if ($Name -match 'Webcam-App-Conflict') {
            $cams = @(Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.ProcessName -match 'Teams|Zoom|Camera|Skype|WebEx|obs' })
            foreach ($p in $cams) {
                Add-ReportRow -Check "Camera app $($p.ProcessName)" -Result "PID $($p.Id)" -Status 'INFO' -Detail 'May lock webcam'
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Peripherals'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Webcam Frame Rate Resolution Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Frame Rate Resolution Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
