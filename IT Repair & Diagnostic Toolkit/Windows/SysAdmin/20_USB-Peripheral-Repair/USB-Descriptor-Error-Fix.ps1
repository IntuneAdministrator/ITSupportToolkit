...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Descriptor Error Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Descriptor-Error-Fix.
...
.EXAMPLE
    .\USB-Descriptor-Error-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Descriptor-Error-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Descriptor Error Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-Descriptor-Error-Fix'
        Add-ReportRow -Check 'Tool' -Result 'USB-Descriptor-Error-Fix' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'USB Descriptor Error Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Descriptor Error Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
