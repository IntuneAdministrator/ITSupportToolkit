...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Webcam Permission Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Webcam-Permission-Reset.
...
.EXAMPLE
    .\Webcam-Permission-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-Permission-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam Permission Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Webcam-Permission-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-Permission-Reset' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Webcam Permission Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Permission Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
