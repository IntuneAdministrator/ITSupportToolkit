...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Mass Storage Policy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Mass-Storage-Policy-Fix.
...
.EXAMPLE
    .\USB-Mass-Storage-Policy-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Mass-Storage-Policy-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Mass Storage Policy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-Mass-Storage-Policy-Fix'
        Add-ReportRow -Check 'Tool' -Result 'USB-Mass-Storage-Policy-Fix' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'USB Mass Storage Policy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Mass Storage Policy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
