...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Smart Card Reader Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Smart-Card-Reader-Repair.
...
.EXAMPLE
    .\Smart-Card-Reader-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Smart-Card-Reader-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Smart Card Reader Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Smart-Card-Reader-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Smart-Card-Reader-Repair' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Smart Card Reader Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Smart Card Reader Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
