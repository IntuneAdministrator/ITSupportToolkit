...
<#
.SYNOPSIS
    Common problem fix: Drawing Tablet Driver Repair
.DESCRIPTION
...
.EXAMPLE
    .\Drawing-Tablet-Driver-Repair.ps1
.EXAMPLE
    .\Drawing-Tablet-Driver-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Drawing Tablet Driver Repair'; $totalSteps = 8; $Name = 'Drawing-Tablet-Driver-Repair'
try {
    ...
    Export-HtmlReport -Title 'Drawing Tablet Driver Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Drawing Tablet Driver Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
