...
<#
.SYNOPSIS
    Common problem fix: USB C Thunderbolt Dock Reset
.DESCRIPTION
...
.EXAMPLE
    .\USB-C-Thunderbolt-Dock-Reset.ps1
.EXAMPLE
    .\USB-C-Thunderbolt-Dock-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB C Thunderbolt Dock Reset'; $totalSteps = 8; $Name = 'USB-C-Thunderbolt-Dock-Reset'
try {
    ...
    Export-HtmlReport -Title 'USB C Thunderbolt Dock Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB C Thunderbolt Dock Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
