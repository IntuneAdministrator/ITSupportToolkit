...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Bandwidth Saturation Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Bandwidth-Saturation-Detect.
Category: 20_USB-Peripheral-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\USB-Bandwidth-Saturation-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Bandwidth-Saturation-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Bandwidth Saturation Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'USB PERIPHERAL REPAIR'
        $Name = 'USB-Bandwidth-Saturation-Detect'
        Add-ReportRow -Check 'Tool' -Result 'USB-Bandwidth-Saturation-Detect' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
        Write-SectionBanner 'USB CONTROLLERS / POWER'
        $usb = @(Get-PnpDevice -Class USB -ErrorAction SilentlyContinue)
        $bad = @($usb | Where-Object Status -ne 'OK')
        Add-ReportRow -Check 'USB devices' -Result "$($usb.Count) total / $($bad.Count) problem" -Status $(if ($bad.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ''
        foreach ($d in ($bad + ($usb | Select-Object -First 8) | Select-Object -Unique -First 20)) {
            Add-ReportRow -Check $d.FriendlyName -Result $d.Status -Status $(if ($d.Status -eq 'OK') {'PASS'} else {'WARN'}) -Detail $d.InstanceId
        }
        Write-ProgressStep 2 $totalSteps $Activity 'USB inventory'

        if ($Name -match 'Controller-Reset|Descriptor|Composite' -and $PSCmdlet.ShouldProcess('USB', 'Re-enumerate problem USB devices')) {
            foreach ($d in $bad) {
                Disable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 1
                Enable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
            }
            Add-ReportRow -Check 'USB re-enumerate' -Result "Toggled $($bad.Count) devices" -Status 'PASS' -Detail ''
        }
        if ($Name -match 'Selective-Suspend|Power') {
            $backupDir = 'C:\IT-Logs\Backups'
            # Disable USB selective suspend via powercfg
            if ($PSCmdlet.ShouldProcess('USB selective suspend', 'powercfg disable')) {
                ...
                Add-ReportRow -Check 'USB selective suspend' -Result 'Disabled for current scheme' -Status 'PASS' -Detail 'AC+DC index 0'
            }
        }
        if ($Name -match 'Mass-Storage') {
            $stor = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\RemovableStorageDevices'
            Add-ReportRow -Check 'RemovableStorageDevices policy' -Result $(if (Test-Path $stor) {'Present'} else {'Not configured'}) -Status 'INFO' -Detail $stor
        }
        if ($Name -match 'Thunderbolt') {
            $tb = @(Get-PnpDevice -ErrorAction SilentlyContinue | Where-Object { $_.FriendlyName -match 'Thunderbolt|USB4' })
            Add-ReportRow -Check 'Thunderbolt/USB4 devices' -Result "$($tb.Count)" -Status 'INFO' -Detail (($tb | ForEach-Object FriendlyName) -join '; ')
        }
        Write-ProgressStep 3 $totalSteps $Activity 'USB power/policy'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'USB Bandwidth Saturation Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Bandwidth Saturation Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
