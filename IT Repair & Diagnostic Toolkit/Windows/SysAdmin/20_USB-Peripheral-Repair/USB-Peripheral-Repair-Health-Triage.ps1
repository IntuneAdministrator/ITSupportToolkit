...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Health-Triage.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Health Triage'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
