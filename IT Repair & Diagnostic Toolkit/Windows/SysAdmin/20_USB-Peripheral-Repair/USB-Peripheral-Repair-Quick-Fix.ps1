...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Quick-Fix.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Quick Fix'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
