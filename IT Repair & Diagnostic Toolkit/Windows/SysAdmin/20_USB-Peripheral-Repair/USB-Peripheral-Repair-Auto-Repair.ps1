...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Auto-Repair.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Auto Repair'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
