...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Evidence-Report.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Evidence Report'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
