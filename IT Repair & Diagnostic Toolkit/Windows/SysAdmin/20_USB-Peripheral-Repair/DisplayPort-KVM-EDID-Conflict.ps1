...
<#
.SYNOPSIS
    Common problem fix: DisplayPort KVM EDID Conflict
.DESCRIPTION
...
.EXAMPLE
    .\DisplayPort-KVM-EDID-Conflict.ps1
.EXAMPLE
    .\DisplayPort-KVM-EDID-Conflict.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DisplayPort KVM EDID Conflict'; $totalSteps = 8; $Name = 'DisplayPort-KVM-EDID-Conflict'
try {
    ...
    Export-HtmlReport -Title 'DisplayPort KVM EDID Conflict' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DisplayPort KVM EDID Conflict' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
