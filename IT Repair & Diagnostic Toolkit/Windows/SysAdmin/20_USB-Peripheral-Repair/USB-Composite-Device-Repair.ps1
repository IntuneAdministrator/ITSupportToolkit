...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Composite Device Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Composite-Device-Repair.
...
.EXAMPLE
    .\USB-Composite-Device-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Composite-Device-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Composite Device Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-Composite-Device-Repair'
        Add-ReportRow -Check 'Tool' -Result 'USB-Composite-Device-Repair' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'USB Composite Device Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Composite Device Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
