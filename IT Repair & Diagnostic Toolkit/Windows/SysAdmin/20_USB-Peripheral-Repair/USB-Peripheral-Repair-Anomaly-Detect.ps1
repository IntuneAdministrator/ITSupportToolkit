...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Anomaly Detect'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
