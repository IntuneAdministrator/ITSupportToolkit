...
<#
.SYNOPSIS
    Common problem fix: USB Over IP KVM Network Audit
.DESCRIPTION
...
.EXAMPLE
    .\USB-Over-IP-KVM-Network-Audit.ps1
.EXAMPLE
    .\USB-Over-IP-KVM-Network-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Over IP KVM Network Audit'; $totalSteps = 8; $Name = 'USB-Over-IP-KVM-Network-Audit'
try {
    ...
    Export-HtmlReport -Title 'USB Over IP KVM Network Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Over IP KVM Network Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
