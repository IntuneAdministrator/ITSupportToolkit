...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Power Management Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Power-Management-Fix.
...
.EXAMPLE
    .\USB-Power-Management-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Power-Management-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Power Management Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-Power-Management-Fix'
        Add-ReportRow -Check 'Tool' -Result 'USB-Power-Management-Fix' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'USB Power Management Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Power Management Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
