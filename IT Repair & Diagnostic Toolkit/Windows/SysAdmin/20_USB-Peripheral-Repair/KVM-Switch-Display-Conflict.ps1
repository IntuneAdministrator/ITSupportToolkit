...
<#
.SYNOPSIS
    Common problem fix: KVM Switch Display Conflict
.DESCRIPTION
...
.EXAMPLE
    .\KVM-Switch-Display-Conflict.ps1
.EXAMPLE
    .\KVM-Switch-Display-Conflict.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'KVM Switch Display Conflict'; $totalSteps = 8; $Name = 'KVM-Switch-Display-Conflict'
try {
    ...
    Export-HtmlReport -Title 'KVM Switch Display Conflict' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'KVM Switch Display Conflict' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
