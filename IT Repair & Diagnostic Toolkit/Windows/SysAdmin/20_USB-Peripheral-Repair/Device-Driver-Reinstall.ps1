...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Device Driver Reinstall
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Device-Driver-Reinstall.
...
.EXAMPLE
    .\Device-Driver-Reinstall.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Device-Driver-Reinstall.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Device Driver Reinstall'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Device-Driver-Reinstall'
        Add-ReportRow -Check 'Tool' -Result 'Device-Driver-Reinstall' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Device Driver Reinstall' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Device Driver Reinstall' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
