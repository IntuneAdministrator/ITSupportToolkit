...
<#
.SYNOPSIS
    Common problem fix: ATEN KVM Switch Reset
.DESCRIPTION
...
.EXAMPLE
    .\ATEN-KVM-Switch-Reset.ps1
.EXAMPLE
    .\ATEN-KVM-Switch-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'ATEN KVM Switch Reset'; $totalSteps = 8; $Name = 'ATEN-KVM-Switch-Reset'
try {
    ...
    Export-HtmlReport -Title 'ATEN KVM Switch Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'ATEN KVM Switch Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
