...
<#
.SYNOPSIS
    Common problem fix: Lenovo ThinkPad Dock Reset
.DESCRIPTION
...
.EXAMPLE
    .\Lenovo-ThinkPad-Dock-Reset.ps1
.EXAMPLE
    .\Lenovo-ThinkPad-Dock-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Lenovo ThinkPad Dock Reset'; $totalSteps = 8; $Name = 'Lenovo-ThinkPad-Dock-Reset'
try {
    ...
    Export-HtmlReport -Title 'Lenovo ThinkPad Dock Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Lenovo ThinkPad Dock Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
