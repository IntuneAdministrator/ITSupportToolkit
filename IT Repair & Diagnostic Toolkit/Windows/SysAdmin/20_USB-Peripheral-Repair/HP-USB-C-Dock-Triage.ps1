...
<#
.SYNOPSIS
    Common problem fix: HP USB C Dock Triage
.DESCRIPTION
...
.EXAMPLE
    .\HP-USB-C-Dock-Triage.ps1
.EXAMPLE
    .\HP-USB-C-Dock-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HP USB C Dock Triage'; $totalSteps = 8; $Name = 'HP-USB-C-Dock-Triage'
try {
    ...
    Export-HtmlReport -Title 'HP USB C Dock Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HP USB C Dock Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
