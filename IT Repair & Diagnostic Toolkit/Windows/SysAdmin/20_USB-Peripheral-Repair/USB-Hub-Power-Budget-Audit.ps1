...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Hub Power Budget Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Hub-Power-Budget-Audit.
...
.EXAMPLE
    .\USB-Hub-Power-Budget-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Hub-Power-Budget-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Hub Power Budget Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-Hub-Power-Budget-Audit'
        Add-ReportRow -Check 'Tool' -Result 'USB-Hub-Power-Budget-Audit' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'USB Hub Power Budget Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Hub Power Budget Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
