...
<#
.SYNOPSIS
    Common problem fix: YubiKey SmartCard Detect
.DESCRIPTION
...
.EXAMPLE
    .\YubiKey-SmartCard-Detect.ps1
.EXAMPLE
    .\YubiKey-SmartCard-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'YubiKey SmartCard Detect'; $totalSteps = 8; $Name = 'YubiKey-SmartCard-Detect'
try {
    ...
    Export-HtmlReport -Title 'YubiKey SmartCard Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'YubiKey SmartCard Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
