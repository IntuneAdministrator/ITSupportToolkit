...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB 3 Compatibility Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-3-Compatibility-Fix.
...
.EXAMPLE
    .\USB-3-Compatibility-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-3-Compatibility-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB 3 Compatibility Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-3-Compatibility-Fix'
        Add-ReportRow -Check 'Tool' -Result 'USB-3-Compatibility-Fix' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'USB 3 Compatibility Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB 3 Compatibility Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
