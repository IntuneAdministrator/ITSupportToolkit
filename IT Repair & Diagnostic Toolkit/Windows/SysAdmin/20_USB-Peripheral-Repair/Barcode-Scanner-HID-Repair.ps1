...
<#
.SYNOPSIS
    Common problem fix: Barcode Scanner HID Repair
.DESCRIPTION
...
.EXAMPLE
    .\Barcode-Scanner-HID-Repair.ps1
.EXAMPLE
    .\Barcode-Scanner-HID-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Barcode Scanner HID Repair'; $totalSteps = 8; $Name = 'Barcode-Scanner-HID-Repair'
try {
    ...
    Export-HtmlReport -Title 'Barcode Scanner HID Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Barcode Scanner HID Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
