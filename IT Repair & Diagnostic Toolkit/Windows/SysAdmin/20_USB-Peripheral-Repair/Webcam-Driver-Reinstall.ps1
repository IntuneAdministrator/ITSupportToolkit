...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Webcam Driver Reinstall
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Webcam-Driver-Reinstall.
...
.EXAMPLE
    .\Webcam-Driver-Reinstall.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-Driver-Reinstall.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam Driver Reinstall'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Webcam-Driver-Reinstall'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-Driver-Reinstall' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Webcam Driver Reinstall' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Driver Reinstall' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
