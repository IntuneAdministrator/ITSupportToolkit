...
<#
.SYNOPSIS
    Common problem fix: Dell WD19 Dock Firmware Hint
.DESCRIPTION
...
.EXAMPLE
    .\Dell-WD19-Dock-Firmware-Hint.ps1
.EXAMPLE
    .\Dell-WD19-Dock-Firmware-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Dell WD19 Dock Firmware Hint'; $totalSteps = 8; $Name = 'Dell-WD19-Dock-Firmware-Hint'
try {
    ...
    Export-HtmlReport -Title 'Dell WD19 Dock Firmware Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Dell WD19 Dock Firmware Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
