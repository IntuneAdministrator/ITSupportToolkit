...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Webcam Privacy Shutter Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Webcam-Privacy-Shutter-Audit.
...
.EXAMPLE
    .\Webcam-Privacy-Shutter-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Webcam-Privacy-Shutter-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Webcam Privacy Shutter Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Webcam-Privacy-Shutter-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Webcam-Privacy-Shutter-Audit' -Status 'INFO' -Detail '20_USB-Peripheral-Repair'
    ...
    Export-HtmlReport -Title 'Webcam Privacy Shutter Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Webcam Privacy Shutter Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
