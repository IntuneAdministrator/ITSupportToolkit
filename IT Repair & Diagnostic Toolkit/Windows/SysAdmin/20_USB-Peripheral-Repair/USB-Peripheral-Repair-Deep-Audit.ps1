...
<#
.SYNOPSIS
    Common problem fix: USB Peripheral Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\USB-Peripheral-Repair-Deep-Audit.ps1
.EXAMPLE
    .\USB-Peripheral-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Peripheral Repair Deep Audit'; $totalSteps = 8; $Name = 'USB-Peripheral-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'USB Peripheral Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Peripheral Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
