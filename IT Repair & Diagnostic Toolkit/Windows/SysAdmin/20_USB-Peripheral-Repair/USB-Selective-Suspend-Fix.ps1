...
<#
.SYNOPSIS
    Common problem fix: USB Selective Suspend Fix
.DESCRIPTION
...
.EXAMPLE
    .\USB-Selective-Suspend-Fix.ps1
.EXAMPLE
    .\USB-Selective-Suspend-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'USB Selective Suspend Fix'; $totalSteps = 8; $Name = 'USB-Selective-Suspend-Fix'
try {
    ...
    Export-HtmlReport -Title 'USB Selective Suspend Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'USB Selective Suspend Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
