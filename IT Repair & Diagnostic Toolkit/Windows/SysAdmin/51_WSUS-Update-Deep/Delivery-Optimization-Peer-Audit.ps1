...
<#
.SYNOPSIS
    Enterprise IT tool: Delivery Optimization Peer Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Delivery-Optimization-Peer-Audit.ps1
.EXAMPLE
    .\Delivery-Optimization-Peer-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Delivery Optimization Peer Audit'; $totalSteps=8; $Name='Delivery-Optimization-Peer-Audit'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '51_WSUS-Update-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'WSUS / UPDATE DEEP'
        ...
        Add-ReportRow -Check 'WUServer' -Result $(if($wu.WUServer){$wu.WUServer}else{'Public WU / Intune'}) -Status 'INFO' -Detail $(if($au){"AUOptions=$($au.AUOptions)"}else{''})
        Write-ProgressStep 2 $totalSteps $Activity 'Update client'
        $uso = Get-Service UsoSvc -EA SilentlyContinue
        Add-ReportRow -Check 'UsoSvc' -Result $(if($uso){"$($uso.Status)"}else{'N/A'}) -Status $(if($uso -and $uso.Status -eq 'Running'){'PASS'}else{'WARN'}) -Detail ''
        if ($Name -match 'SoftwareDistribution|Reset') {
            if ($PSCmdlet.ShouldProcess('SoftwareDistribution', 'safe reset')) {
                Stop-Service wuauserv,bits,cryptsvc -Force -EA SilentlyContinue
                Rename-Item "$env:SystemRoot\SoftwareDistribution" "SoftwareDistribution.bak_$stamp" -EA SilentlyContinue
                Start-Service cryptsvc,bits,wuauserv -EA SilentlyContinue
                Add-ReportRow -Check 'SoftwareDistribution' -Result 'Renamed + services restarted' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Scan-Force|WUAUCLT') {
            if ($PSCmdlet.ShouldProcess('Windows Update', 'scan')) {
                UsoClient StartInteractiveScan 2>$null
                Add-ReportRow -Check 'UsoClient scan' -Result 'Triggered' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Pending-Reboot') {
            ...
            Add-ReportRow -Check 'Reboot pending' -Result "CBS=$pending WU=$wuPending" -Status $(if($pending -or $wuPending){'WARN'}else{'PASS'}) -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Update done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Delivery Optimization Peer Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Delivery Optimization Peer Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
