...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Stack-Reset.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Stack Reset'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
