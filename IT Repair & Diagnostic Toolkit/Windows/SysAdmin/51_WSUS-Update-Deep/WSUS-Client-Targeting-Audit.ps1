...
<#
.SYNOPSIS
    Enterprise IT tool: WSUS Client Targeting Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\WSUS-Client-Targeting-Audit.ps1
.EXAMPLE
    .\WSUS-Client-Targeting-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='WSUS Client Targeting Audit'; $totalSteps=8; $Name='WSUS-Client-Targeting-Audit'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'WSUS Client Targeting Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Client Targeting Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
