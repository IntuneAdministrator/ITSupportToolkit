...
<#
.SYNOPSIS
    Enterprise IT tool: Update History HTML Export
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Update-History-HTML-Export.ps1
.EXAMPLE
    .\Update-History-HTML-Export.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Update History HTML Export'; $totalSteps=8; $Name='Update-History-HTML-Export'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'Update History HTML Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Update History HTML Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
