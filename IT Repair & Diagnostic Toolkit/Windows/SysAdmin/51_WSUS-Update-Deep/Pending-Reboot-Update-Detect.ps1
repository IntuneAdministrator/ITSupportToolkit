...
<#
.SYNOPSIS
    Enterprise IT tool: Pending Reboot Update Detect
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Pending-Reboot-Update-Detect.ps1
.EXAMPLE
    .\Pending-Reboot-Update-Detect.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Pending Reboot Update Detect'; $totalSteps=8; $Name='Pending-Reboot-Update-Detect'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'Pending Reboot Update Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Pending Reboot Update Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
