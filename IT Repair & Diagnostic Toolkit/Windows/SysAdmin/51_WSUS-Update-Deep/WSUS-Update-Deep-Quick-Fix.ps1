...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Quick-Fix.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Quick Fix'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
