...
<#
.SYNOPSIS
    Enterprise IT tool: Update Orchestrator Health
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Update-Orchestrator-Health.ps1
.EXAMPLE
    .\Update-Orchestrator-Health.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Update Orchestrator Health'; $totalSteps=8; $Name='Update-Orchestrator-Health'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'Update Orchestrator Health' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Update Orchestrator Health' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
