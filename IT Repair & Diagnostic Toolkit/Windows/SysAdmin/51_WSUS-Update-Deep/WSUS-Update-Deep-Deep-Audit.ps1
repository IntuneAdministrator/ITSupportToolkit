...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Deep-Audit.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Deep Audit'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
