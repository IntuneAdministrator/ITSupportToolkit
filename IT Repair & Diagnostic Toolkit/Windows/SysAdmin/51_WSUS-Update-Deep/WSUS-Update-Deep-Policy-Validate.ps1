...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Policy-Validate.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Policy Validate'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
