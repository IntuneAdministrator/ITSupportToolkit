...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Auto-Repair.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Auto Repair'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
