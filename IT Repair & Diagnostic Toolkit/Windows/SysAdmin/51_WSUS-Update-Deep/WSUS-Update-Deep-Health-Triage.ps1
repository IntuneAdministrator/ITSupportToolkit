...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Health-Triage.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Health Triage'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
