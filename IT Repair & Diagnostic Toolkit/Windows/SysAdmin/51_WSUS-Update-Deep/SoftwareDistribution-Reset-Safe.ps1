...
<#
.SYNOPSIS
    Enterprise IT tool: SoftwareDistribution Reset Safe
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\SoftwareDistribution-Reset-Safe.ps1
.EXAMPLE
    .\SoftwareDistribution-Reset-Safe.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='SoftwareDistribution Reset Safe'; $totalSteps=8; $Name='SoftwareDistribution-Reset-Safe'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'SoftwareDistribution Reset Safe' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SoftwareDistribution Reset Safe' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
