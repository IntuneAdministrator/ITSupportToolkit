...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\WSUS-Update-Deep-Anomaly-Detect.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Anomaly Detect'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
