...
<#
.SYNOPSIS
    Common problem fix: WSUS Update Deep Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 51_WSUS-Update-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\WSUS-Update-Deep-Evidence-Report.ps1
.EXAMPLE
    .\WSUS-Update-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'WSUS Update Deep Evidence Report'; $totalSteps = 8; $Name = 'WSUS-Update-Deep-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '51_WSUS-Update-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '51_WSUS-Update-Deep'
    ...
    Export-HtmlReport -Title 'WSUS Update Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WSUS Update Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
