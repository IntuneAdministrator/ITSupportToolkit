...
<#
.SYNOPSIS
    Enterprise IT tool: Feature Update Blockers Scan
.DESCRIPTION
    Phase 11 expansion (SysAdmin/51_WSUS-Update-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Feature-Update-Blockers-Scan.ps1
.EXAMPLE
    .\Feature-Update-Blockers-Scan.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Feature Update Blockers Scan'; $totalSteps=8; $Name='Feature-Update-Blockers-Scan'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'Feature Update Blockers Scan' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Feature Update Blockers Scan' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
