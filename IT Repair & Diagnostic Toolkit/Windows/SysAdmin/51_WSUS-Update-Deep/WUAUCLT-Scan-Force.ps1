...
$exitCode=0; $Activity='WUAUCLT Scan Force'; $totalSteps=8; $Name='WUAUCLT-Scan-Force'; $Folder='51_WSUS-Update-Deep'
try {
    ...
    Export-HtmlReport -Title 'WUAUCLT Scan Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WUAUCLT Scan Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
