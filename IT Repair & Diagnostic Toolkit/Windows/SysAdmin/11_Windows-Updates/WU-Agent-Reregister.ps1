...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WU Agent Reregister
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WU-Agent-Reregister.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WU-Agent-Reregister'
        Add-ReportRow -Check 'Tool' -Result 'WU-Agent-Reregister' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'WU Agent Reregister' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WU Agent Reregister' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
