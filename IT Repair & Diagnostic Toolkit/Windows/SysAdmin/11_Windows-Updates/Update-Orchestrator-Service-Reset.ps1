...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Update Orchestrator Service Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Update-Orchestrator-Service-Reset.
...
.EXAMPLE
    .\Update-Orchestrator-Service-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Update-Orchestrator-Service-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Update Orchestrator Service Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Update-Orchestrator-Service-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Update-Orchestrator-Service-Reset' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Update Orchestrator Service Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Update Orchestrator Service Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
