...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Update Proxy Config
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Update-Proxy-Config.
...
.EXAMPLE
    .\Windows-Update-Proxy-Config.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Update-Proxy-Config.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Update Proxy Config'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Update-Proxy-Config'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Update-Proxy-Config' -Status 'INFO' -Detail '11_Windows-Updates'
        Write-SectionBanner 'WSUS / UPDATE PROXY'
        $wuKey = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
        $auKey = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU'
        Backup-RegistryKey -KeyPath $wuKey -BackupDir 'C:\IT-Logs\Backups' | Out-Null
        if (Test-Path $wuKey) {
            $wu = Get-ItemProperty $wuKey -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'WUServer' -Result "$($wu.WUServer)" -Status 'INFO' -Detail "$($wu.WUStatusServer)"
        } else {
            Add-ReportRow -Check 'WU policy key' -Result 'Not present' -Status 'INFO' -Detail 'Likely Microsoft Update / Intune'
        }
        if ($Join.IsAzureADJoined -and -not $Join.IsDomainJoined) {
            Add-ReportRow -Check 'Join branch' -Result 'Entra-only — prefer Intune update rings' -Status 'WARN' -Detail $Join.Label
        } elseif ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
            Add-ReportRow -Check 'Join branch' -Result 'Domain/Hybrid — WSUS/GPO applicable' -Status 'PASS' -Detail $Join.Label
            if ($Name -match 'WSUS' -and $PSCmdlet.ShouldProcess('WSUS client', 'Reset Automatic Updates AU')) {
                if (-not (Test-Path $auKey)) { New-Item $auKey -Force | Out-Null }
                New-ItemProperty $auKey -Name UseWUServer -Value 1 -PropertyType DWord -Force -ErrorAction SilentlyContinue | Out-Null
                Restart-Service wuauserv -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'WSUS client reset' -Result 'AU refreshed' -Status 'PASS' -Detail 'Rollback via .reg backup'
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'WSUS/proxy'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Update Proxy Config' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Update Proxy Config' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
