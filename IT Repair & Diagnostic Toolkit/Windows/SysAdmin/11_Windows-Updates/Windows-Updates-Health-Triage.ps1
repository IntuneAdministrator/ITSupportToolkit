...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Health-Triage.ps1
.EXAMPLE
    .\Windows-Updates-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Health Triage'; $totalSteps = 8; $Name = 'Windows-Updates-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
