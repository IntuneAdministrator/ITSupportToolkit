...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Anomaly-Detect.ps1
.EXAMPLE
    .\Windows-Updates-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Anomaly Detect'; $totalSteps = 8; $Name = 'Windows-Updates-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
