...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Stack-Reset.ps1
.EXAMPLE
    .\Windows-Updates-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Stack Reset'; $totalSteps = 8; $Name = 'Windows-Updates-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
