...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Auto-Repair.ps1
.EXAMPLE
    .\Windows-Updates-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Auto Repair'; $totalSteps = 8; $Name = 'Windows-Updates-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
