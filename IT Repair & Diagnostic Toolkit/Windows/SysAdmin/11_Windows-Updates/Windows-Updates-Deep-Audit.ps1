...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Deep-Audit.ps1
.EXAMPLE
    .\Windows-Updates-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Deep Audit'; $totalSteps = 8; $Name = 'Windows-Updates-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
