...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Quick-Fix.ps1
.EXAMPLE
    .\Windows-Updates-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Quick Fix'; $totalSteps = 8; $Name = 'Windows-Updates-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
