...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 11_Windows-Updates). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Windows-Updates-Evidence-Report.ps1
.EXAMPLE
    .\Windows-Updates-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Evidence Report'; $totalSteps = 8; $Name = 'Windows-Updates-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '11_Windows-Updates' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Windows Updates Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
