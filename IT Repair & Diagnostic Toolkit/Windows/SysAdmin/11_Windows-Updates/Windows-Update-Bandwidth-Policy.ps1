...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Update Bandwidth Policy
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Update-Bandwidth-Policy.
...
.EXAMPLE
    .\Windows-Update-Bandwidth-Policy.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Update-Bandwidth-Policy.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Update Bandwidth Policy'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Update-Bandwidth-Policy'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Update-Bandwidth-Policy' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Windows Update Bandwidth Policy' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Update Bandwidth Policy' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
