...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Update Log Convert
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Update-Log-Convert.
...
.EXAMPLE
    .\Windows-Update-Log-Convert.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Update-Log-Convert.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Update Log Convert'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Update-Log-Convert'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Update-Log-Convert' -Status 'INFO' -Detail '11_Windows-Updates'
        Write-SectionBanner 'WINDOWS UPDATE STATUS'
        $wu = Get-Service wuauserv -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'wuauserv' -Result $(if ($wu) { "$($wu.Status)/$($wu.StartType)" } else {'Missing'}) -Status $(if ($wu -and $wu.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'WU service'
        try {
            $session = New-Object -ComObject Microsoft.Update.Session
            $result = $session.CreateUpdateSearcher().Search('IsInstalled=0')
            Add-ReportRow -Check 'Pending updates' -Result "$($result.Updates.Count)" -Status $(if ($result.Updates.Count -eq 0) {'PASS'} else {'INFO'}) -Detail 'IsInstalled=0 search'
        } catch {
            Add-ReportRow -Check 'Pending updates' -Result 'Search failed' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Pending'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Update Log Convert' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Update Log Convert' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
