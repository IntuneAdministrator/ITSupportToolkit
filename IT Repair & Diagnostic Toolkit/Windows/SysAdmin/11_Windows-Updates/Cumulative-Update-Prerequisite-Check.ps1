...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Cumulative Update Prerequisite Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Cumulative-Update-Prerequisite-Check.
Category: 11_Windows-Updates. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Cumulative-Update-Prerequisite-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Cumulative-Update-Prerequisite-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Cumulative Update Prerequisite Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'WINDOWS UPDATES'
        $Name = 'Cumulative-Update-Prerequisite-Check'
        Add-ReportRow -Check 'Tool' -Result 'Cumulative-Update-Prerequisite-Check' -Status 'INFO' -Detail '11_Windows-Updates'
        Write-SectionBanner 'UPDATE AUDIT / HISTORY'
        try {
            $session = New-Object -ComObject Microsoft.Update.Session
            $searcher = $session.CreateUpdateSearcher()
            $historyCount = $searcher.GetTotalHistoryCount()
            $history = $searcher.QueryHistory(0, [Math]::Min(30, $historyCount))
            Add-ReportRow -Check 'Update history entries' -Result "$historyCount" -Status 'INFO' -Detail 'Microsoft.Update.Session'
            foreach ($h in ($history | Select-Object -First 15)) {
                $r = switch ($h.ResultCode) { 2 {'Succeeded'} 3 {'SucceededWithErrors'} 4 {'Failed'} 5 {'Aborted'} default {"Code $($h.ResultCode)"} }
                Add-ReportRow -Check ($h.Title) -Result $r -Status $(if ($h.ResultCode -eq 2) {'PASS'} elseif ($h.ResultCode -eq 4) {'FAIL'} else {'WARN'}) -Detail "$($h.Date)"
            }
        } catch {
            Add-ReportRow -Check 'Update history' -Result 'COM unavailable' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 2 $totalSteps $Activity 'History'

        $cbs = "$env:SystemRoot\Logs\CBS\CBS.log"
        if (Test-Path $cbs) {
            $errs = @(Select-String -Path $cbs -Pattern 'Error|failed' -SimpleMatch -ErrorAction SilentlyContinue | Select-Object -Last 10)
            Add-ReportRow -Check 'CBS.log errors (tail)' -Result "$($errs.Count) matches" -Status $(if ($errs.Count -gt 5) {'WARN'} else {'INFO'}) -Detail (($errs | ForEach-Object Line) -join ' | ').Substring(0, [Math]::Min(400, (($errs | ForEach-Object Line) -join ' | ').Length))
        }
        Write-ProgressStep 3 $totalSteps $Activity 'CBS'

        if ($Name -match 'Delivery-Optimization|Bandwidth') {
            $do = Get-Service dosvc -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Delivery Optimization' -Result $(if ($do) { $do.Status } else {'N/A'}) -Status 'INFO' -Detail 'dosvc'
            if ($PSCmdlet.ShouldProcess('dosvc', 'Restart Delivery Optimization')) {
                Restart-Service dosvc -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'dosvc restart' -Result 'Done' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Insider|Feature-Update-Block') {
            $backupDir = 'C:\IT-Logs\Backups'
            $insider = 'HKLM:\SOFTWARE\Microsoft\WindowsSelfHost\Applicability'
            if (Test-Path $insider) {
                Backup-RegistryKey -KeyPath $insider -BackupDir $backupDir | Out-Null
                Add-ReportRow -Check 'Insider applicability' -Result 'Key present' -Status 'WARN' -Detail 'Review ring membership'
            } else {
                Add-ReportRow -Check 'Insider applicability' -Result 'Not enrolled (key absent)' -Status 'PASS' -Detail ''
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Policies'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Cumulative Update Prerequisite Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Cumulative Update Prerequisite Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
