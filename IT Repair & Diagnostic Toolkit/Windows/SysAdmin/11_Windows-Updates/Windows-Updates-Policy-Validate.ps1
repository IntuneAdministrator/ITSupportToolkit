...
<#
.SYNOPSIS
    Common problem fix: Windows Updates Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Windows-Updates-Policy-Validate.ps1
.EXAMPLE
    .\Windows-Updates-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows Updates Policy Validate'; $totalSteps = 8; $Name = 'Windows-Updates-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Windows Updates Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows Updates Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
