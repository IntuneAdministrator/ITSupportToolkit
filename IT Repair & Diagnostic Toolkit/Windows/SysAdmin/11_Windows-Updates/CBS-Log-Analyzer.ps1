...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: CBS Log Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for CBS-Log-Analyzer.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'CBS-Log-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'CBS-Log-Analyzer' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'CBS Log Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'CBS Log Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
