...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Insider Unenroll
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Insider-Unenroll.
...
.EXAMPLE
    .\Windows-Insider-Unenroll.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Insider-Unenroll.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Insider Unenroll'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Insider-Unenroll'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Insider-Unenroll' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Windows Insider Unenroll' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Insider Unenroll' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
