...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Update Cache Rebuild
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Update-Cache-Rebuild.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Update-Cache-Rebuild'
        Add-ReportRow -Check 'Tool' -Result 'Update-Cache-Rebuild' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Update Cache Rebuild' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Update Cache Rebuild' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
