...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Installed Update Inventory
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Installed-Update-Inventory.
...
.EXAMPLE
    .\Installed-Update-Inventory.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Installed-Update-Inventory.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Installed Update Inventory'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Installed-Update-Inventory' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Installed Update Inventory' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Installed Update Inventory' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
