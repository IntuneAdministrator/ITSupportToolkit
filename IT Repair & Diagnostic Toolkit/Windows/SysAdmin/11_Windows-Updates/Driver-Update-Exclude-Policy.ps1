...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Driver Update Exclude Policy
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Driver-Update-Exclude-Policy.
...
.EXAMPLE
    .\Driver-Update-Exclude-Policy.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Driver-Update-Exclude-Policy.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Driver Update Exclude Policy'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Driver-Update-Exclude-Policy'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Update-Exclude-Policy' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Driver Update Exclude Policy' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Update Exclude Policy' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
