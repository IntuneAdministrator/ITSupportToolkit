...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Servicing Stack Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Servicing-Stack-Repair.
...
$exitCode = 0
$Activity = 'Servicing Stack Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Servicing-Stack-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Servicing-Stack-Repair' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Servicing Stack Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Servicing Stack Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
