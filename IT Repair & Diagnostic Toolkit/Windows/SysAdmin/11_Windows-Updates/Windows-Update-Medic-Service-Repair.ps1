...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Update Medic Service Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Update-Medic-Service-Repair.
...
.EXAMPLE
    .\Windows-Update-Medic-Service-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Update-Medic-Service-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Update Medic Service Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Update-Medic-Service-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Update-Medic-Service-Repair' -Status 'INFO' -Detail '11_Windows-Updates'
        Write-SectionBanner 'WINDOWS UPDATE RESET'
        $wuSvcs = @('wuauserv','bits','cryptsvc','msiserver','dosvc','UsoSvc','WaaSMedicSvc')
        foreach ($s in $wuSvcs) {
            $svc = Get-Service -Name $s -ErrorAction SilentlyContinue
            if ($svc) { Add-ReportRow -Check "Service $s" -Result "$($svc.Status)/$($svc.StartType)" -Status 'INFO' -Detail $svc.DisplayName }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'WU services'

        if ($PSCmdlet.ShouldProcess('Windows Update', 'Stop services, rename SoftwareDistribution/catroot2, restart')) {
            foreach ($s in @('wuauserv','bits','cryptsvc','dosvc','UsoSvc')) {
            ...
            $stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
            if (Test-Path $sd) {
                Rename-Item $sd "SoftwareDistribution.bak_$stamp" -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'SoftwareDistribution' -Result 'Renamed' -Status 'PASS' -Detail "SoftwareDistribution.bak_$stamp"
            }
            if (Test-Path $cr) {
                Rename-Item $cr "catroot2.bak_$stamp" -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'catroot2' -Result 'Renamed' -Status 'PASS' -Detail "catroot2.bak_$stamp"
            }
            foreach ($s in @('cryptsvc','bits','wuauserv','dosvc','UsoSvc')) {
                Start-Service -Name $s -ErrorAction SilentlyContinue
            }
            Add-ReportRow -Check 'WU services restart' -Result 'Started' -Status 'PASS' -Detail 'Rollback: rename .bak folders back if needed'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Cache rebuild'

        try {
            UsoClient StartInteractiveScan 2>$null
            Add-ReportRow -Check 'Update scan' -Result 'UsoClient triggered' -Status 'INFO' -Detail 'StartInteractiveScan'
        } catch {
            Add-ReportRow -Check 'Update scan' -Result 'Manual check needed' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Scan'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Update Medic Service Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Update Medic Service Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
