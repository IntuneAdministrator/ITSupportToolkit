...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Delivery Optimization Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Delivery-Optimization-Reset.
...
.EXAMPLE
    .\Delivery-Optimization-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Delivery-Optimization-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Delivery Optimization Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Delivery-Optimization-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Delivery-Optimization-Reset' -Status 'INFO' -Detail '11_Windows-Updates'
    ...
    Export-HtmlReport -Title 'Delivery Optimization Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Delivery Optimization Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
