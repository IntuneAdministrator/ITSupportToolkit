...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Administrative Share Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Administrative-Share-Audit.
...
.EXAMPLE
    .\Administrative-Share-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Administrative-Share-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Administrative Share Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Administrative-Share-Audit'
    ...
    Export-HtmlReport -Title 'Administrative Share Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Administrative Share Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
