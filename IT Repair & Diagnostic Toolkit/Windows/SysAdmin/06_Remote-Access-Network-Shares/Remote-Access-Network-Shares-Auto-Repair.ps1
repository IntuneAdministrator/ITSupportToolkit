...
<#
.SYNOPSIS
    Common problem fix: Remote Access Network Shares Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Access-Network-Shares-Auto-Repair.ps1
.EXAMPLE
    .\Remote-Access-Network-Shares-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Access Network Shares Auto Repair'; $totalSteps = 8; $Name = 'Remote-Access-Network-Shares-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Remote Access Network Shares Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Access Network Shares Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
