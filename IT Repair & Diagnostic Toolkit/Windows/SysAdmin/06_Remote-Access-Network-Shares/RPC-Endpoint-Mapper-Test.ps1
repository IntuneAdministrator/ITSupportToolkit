...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: RPC Endpoint Mapper Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for RPC-Endpoint-Mapper-Test.
...
.EXAMPLE
    .\RPC-Endpoint-Mapper-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\RPC-Endpoint-Mapper-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'RPC Endpoint Mapper Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — RPC-Endpoint-Mapper-Test'
    ...
    Export-HtmlReport -Title 'RPC Endpoint Mapper Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RPC Endpoint Mapper Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
