...
<#
.SYNOPSIS
    Common problem fix: IP KVM Remote Console Hint
.DESCRIPTION
...
.EXAMPLE
    .\IP-KVM-Remote-Console-Hint.ps1
.EXAMPLE
    .\IP-KVM-Remote-Console-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'IP KVM Remote Console Hint'; $totalSteps = 8; $Name = 'IP-KVM-Remote-Console-Hint'
try {
    ...
    Export-HtmlReport -Title 'IP KVM Remote Console Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IP KVM Remote Console Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
