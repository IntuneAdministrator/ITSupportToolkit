...
<#
.SYNOPSIS
    Common problem fix: Remote Access Network Shares Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Access-Network-Shares-Stack-Reset.ps1
.EXAMPLE
    .\Remote-Access-Network-Shares-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Access Network Shares Stack Reset'; $totalSteps = 8; $Name = 'Remote-Access-Network-Shares-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Remote Access Network Shares Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Access Network Shares Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
