...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Share Permissions Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Share-Permissions-Fix.
...
.EXAMPLE
    .\Network-Share-Permissions-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Share-Permissions-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Share Permissions Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Network-Share-Permissions-Fix'
    ...
    Export-HtmlReport -Title 'Network Share Permissions Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Share Permissions Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
