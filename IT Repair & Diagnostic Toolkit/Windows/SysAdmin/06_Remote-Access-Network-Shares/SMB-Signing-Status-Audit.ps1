...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SMB Signing Status Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SMB-Signing-Status-Audit.
...
.EXAMPLE
    .\SMB-Signing-Status-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SMB-Signing-Status-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SMB Signing Status Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — SMB-Signing-Status-Audit'
    ...
    Export-HtmlReport -Title 'SMB Signing Status Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SMB Signing Status Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
