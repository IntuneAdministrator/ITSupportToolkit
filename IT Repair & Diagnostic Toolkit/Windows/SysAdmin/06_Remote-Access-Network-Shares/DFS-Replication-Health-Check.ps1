...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DFS Replication Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DFS-Replication-Health-Check.
...
.EXAMPLE
    .\DFS-Replication-Health-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DFS-Replication-Health-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DFS Replication Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — DFS-Replication-Health-Check'
    ...
    Export-HtmlReport -Title 'DFS Replication Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DFS Replication Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
