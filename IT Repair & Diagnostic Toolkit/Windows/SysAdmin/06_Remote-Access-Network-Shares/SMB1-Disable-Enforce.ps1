...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SMB1 Disable Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SMB1-Disable-Enforce.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — SMB1-Disable-Enforce'
        ...
        try {
            ...
            Add-ReportRow -Check 'SMB1 feature' -Result $smb1.State -Status $(if ($smb1.State -eq 'Disabled') {'PASS'} else {'WARN'}) -Detail 'SMB1 should be disabled'
            if ($smb1.State -ne 'Disabled' -and $PSCmdlet.ShouldProcess('SMB1', 'Disable-WindowsOptionalFeature')) {
                Disable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol -NoRestart -ErrorAction SilentlyContinue | Out-Null
                Add-ReportRow -Check 'Disable SMB1' -Result 'Disabled (reboot may be needed)' -Status 'PASS' -Detail 'SMB1Protocol'
            }
        } catch {
            Add-ReportRow -Check 'SMB1 feature' -Result 'Check failed' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 3 $totalSteps $Activity 'SMB1'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'SMB1 Disable Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SMB1 Disable Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
