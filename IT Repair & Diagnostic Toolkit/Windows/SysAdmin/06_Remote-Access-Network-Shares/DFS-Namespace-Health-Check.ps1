...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DFS Namespace Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DFS-Namespace-Health-Check.
...
.EXAMPLE
    .\DFS-Namespace-Health-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DFS-Namespace-Health-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DFS Namespace Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — DFS-Namespace-Health-Check'
    ...
    Export-HtmlReport -Title 'DFS Namespace Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DFS Namespace Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
