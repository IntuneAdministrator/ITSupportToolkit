...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PsExec Remote Session Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PsExec-Remote-Session-Test.
...
.EXAMPLE
    .\PsExec-Remote-Session-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PsExec-Remote-Session-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PsExec Remote Session Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — PsExec-Remote-Session-Test'
    ...
    Export-HtmlReport -Title 'PsExec Remote Session Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PsExec Remote Session Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
