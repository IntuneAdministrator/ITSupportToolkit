...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Remote Desktop Gateway Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Remote-Desktop-Gateway-Test.
...
.EXAMPLE
    .\Remote-Desktop-Gateway-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Remote-Desktop-Gateway-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Remote Desktop Gateway Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Remote-Desktop-Gateway-Test'
        ...
        Add-ReportRow -Check 'LanmanServer' -Result (Get-Service LanmanServer).Status -Status 'INFO' -Detail 'Server service'
        Write-ProgressStep 3 $totalSteps $Activity 'Server service'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Remote Desktop Gateway Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Desktop Gateway Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
