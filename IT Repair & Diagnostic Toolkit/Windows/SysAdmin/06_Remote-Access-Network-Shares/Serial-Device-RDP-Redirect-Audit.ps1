...
<#
.SYNOPSIS
    Common problem fix: Serial Device RDP Redirect Audit
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 06_Remote-Access-Network-Shares). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Serial-Device-RDP-Redirect-Audit.ps1
.EXAMPLE
    .\Serial-Device-RDP-Redirect-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Serial Device RDP Redirect Audit'; $totalSteps = 8; $Name = 'Serial-Device-RDP-Redirect-Audit'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '06_Remote-Access-Network-Shares' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '06_Remote-Access-Network-Shares'
    ...
    Export-HtmlReport -Title 'Serial Device RDP Redirect Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Serial Device RDP Redirect Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
