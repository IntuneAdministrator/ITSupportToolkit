...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Null Session Restriction Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Null-Session-Restriction-Enforce.
...
.EXAMPLE
    .\Null-Session-Restriction-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Null-Session-Restriction-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Null Session Restriction Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Null-Session-Restriction-Enforce'
    ...
    Export-HtmlReport -Title 'Null Session Restriction Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Null Session Restriction Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
