...
<#
.SYNOPSIS
    Common problem fix: Remote Access Network Shares Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Access-Network-Shares-Deep-Audit.ps1
.EXAMPLE
    .\Remote-Access-Network-Shares-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Access Network Shares Deep Audit'; $totalSteps = 8; $Name = 'Remote-Access-Network-Shares-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Remote Access Network Shares Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Access Network Shares Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
