...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Mapped Drive Reconnect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Mapped-Drive-Reconnect.
...
$exitCode = 0
$Activity = 'Mapped Drive Reconnect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Mapped-Drive-Reconnect'
    ...
    Export-HtmlReport -Title 'Mapped Drive Reconnect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Mapped Drive Reconnect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
