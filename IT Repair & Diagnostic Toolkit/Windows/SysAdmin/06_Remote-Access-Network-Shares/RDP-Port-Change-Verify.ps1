...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: RDP Port Change Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for RDP-Port-Change-Verify.
...
$exitCode = 0
$Activity = 'RDP Port Change Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — RDP-Port-Change-Verify'
    ...
    Export-HtmlReport -Title 'RDP Port Change Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RDP Port Change Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
