...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Share Offline Files Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Share-Offline-Files-Repair.
...
.EXAMPLE
    .\Network-Share-Offline-Files-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Share-Offline-Files-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Share Offline Files Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Network-Share-Offline-Files-Repair'
        ...
        try {
            ...
            foreach ($sh in $shares) {
                Add-ReportRow -Check "Share $($sh.Name)" -Result $sh.Path -Status 'INFO' -Detail "Encrypt=$($sh.EncryptData)"
            }
        } catch {
            Add-ReportRow -Check 'SMB shares' -Result 'Error' -Status 'WARN' -Detail $_.Exception.Message
        }
        try {
            $smbServer = Get-SmbServerConfiguration
            Add-ReportRow -Check 'SMB encrypt data' -Result "$($smbServer.EncryptData)" -Status 'INFO' -Detail "RejectUnencrypted=$($smbServer.RejectUnencryptedAccess)"
        } catch {}
        Write-ProgressStep 3 $totalSteps $Activity 'SMB audit'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Network Share Offline Files Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Share Offline Files Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
