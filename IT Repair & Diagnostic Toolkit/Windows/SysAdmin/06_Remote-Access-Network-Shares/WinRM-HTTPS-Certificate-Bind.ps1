...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WinRM HTTPS Certificate Bind
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WinRM-HTTPS-Certificate-Bind.
...
.EXAMPLE
    .\WinRM-HTTPS-Certificate-Bind.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WinRM-HTTPS-Certificate-Bind.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WinRM HTTPS Certificate Bind'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — WinRM-HTTPS-Certificate-Bind'
        ...
        if ($PSCmdlet.ShouldProcess('WinRM', 'winrm quickconfig -quiet + service start')) {
            try {
                Start-Service WinRM -ErrorAction SilentlyContinue
                & winrm quickconfig -quiet 2>&1 | Out-Null
                $svc = Get-Service WinRM
                Add-ReportRow -Check 'WinRM service' -Result $svc.Status -Status $(if ($svc.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail $svc.StartType
                $listeners = & winrm enumerate winrm/config/listener 2>&1 | Out-String
                Add-ReportRow -Check 'WinRM listeners' -Result 'Enumerated' -Status 'INFO' -Detail $listeners.Substring(0, [Math]::Min(300, $listeners.Length))
            } catch {
                Add-ReportRow -Check 'WinRM repair' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'WinRM'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'WinRM HTTPS Certificate Bind' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WinRM HTTPS Certificate Bind' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
