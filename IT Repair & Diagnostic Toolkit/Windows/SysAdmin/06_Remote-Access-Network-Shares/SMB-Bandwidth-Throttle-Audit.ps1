...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SMB Bandwidth Throttle Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SMB-Bandwidth-Throttle-Audit.
...
.EXAMPLE
    .\SMB-Bandwidth-Throttle-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SMB-Bandwidth-Throttle-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SMB Bandwidth Throttle Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — SMB-Bandwidth-Throttle-Audit'
    ...
    Export-HtmlReport -Title 'SMB Bandwidth Throttle Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SMB Bandwidth Throttle Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
