...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: RDP Certificate Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for RDP-Certificate-Reset.
...
$exitCode = 0
$Activity = 'RDP Certificate Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — RDP-Certificate-Reset'
    ...
    Export-HtmlReport -Title 'RDP Certificate Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RDP Certificate Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
