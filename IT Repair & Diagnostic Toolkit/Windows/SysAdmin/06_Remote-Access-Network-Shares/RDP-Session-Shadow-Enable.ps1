...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: RDP Session Shadow Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for RDP-Session-Shadow-Enable.
Category: 06_Remote-Access-Network-Shares. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\RDP-Session-Shadow-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\RDP-Session-Shadow-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'RDP Session Shadow Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — RDP-Session-Shadow-Enable'
        Add-ReportRow -Check 'Join state context' -Result $Join.Label -Status 'INFO' -Detail $Join.DomainName
        Write-ProgressStep 2 $totalSteps $Activity 'Identity'
        $backupDir = 'C:\IT-Logs\Backups'
        Backup-RegistryKey -KeyPath 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server' -BackupDir $backupDir | Out-Null
        $ts = Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server'
        Add-ReportRow -Check 'fDenyTSConnections' -Result "$($ts.fDenyTSConnections)" -Status $(if ($ts.fDenyTSConnections -eq 0) {'PASS'} else {'WARN'}) -Detail '0=RDP allowed'
        if ($PSCmdlet.ShouldProcess('RDP', 'Enable Remote Desktop + firewall rule')) {
            Set-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server' -Name fDenyTSConnections -Value 0
            Enable-NetFirewallRule -DisplayGroup 'Remote Desktop' -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Enable RDP' -Result 'Configured' -Status 'PASS' -Detail 'Firewall group Remote Desktop enabled'
        }
        ...
        Add-ReportRow -Check 'RDP port' -Result "$port" -Status 'INFO' -Detail 'RDP-Tcp PortNumber'
        Write-ProgressStep 3 $totalSteps $Activity 'RDP'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'RDP Session Shadow Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RDP Session Shadow Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
