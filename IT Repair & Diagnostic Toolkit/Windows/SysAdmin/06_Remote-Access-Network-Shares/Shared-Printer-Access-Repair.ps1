...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Shared Printer Access Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Shared-Printer-Access-Repair.
...
.EXAMPLE
    .\Shared-Printer-Access-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Shared-Printer-Access-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Shared Printer Access Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Shared-Printer-Access-Repair'
    ...
    Export-HtmlReport -Title 'Shared Printer Access Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Shared Printer Access Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
