...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Remote Registry Service Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Remote-Registry-Service-Enable.
...
.EXAMPLE
    .\Remote-Registry-Service-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Remote-Registry-Service-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Remote Registry Service Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'REMOTE ACCESS / SHARES — Remote-Registry-Service-Enable'
        ...
        if ($PSCmdlet.ShouldProcess('RemoteRegistry', 'Set Automatic and Start')) {
            ...
            Add-ReportRow -Check 'RemoteRegistry' -Result "$($s.Status)/$($s.StartType)" -Status $(if ($s.Status -eq 'Running') {'PASS'} else {'WARN'}) -Detail 'Enabled for support scenarios'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Remote Registry'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Remote Registry Service Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Registry Service Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
