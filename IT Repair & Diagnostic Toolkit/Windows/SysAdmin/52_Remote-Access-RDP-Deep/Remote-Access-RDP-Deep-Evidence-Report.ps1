...
<#
.SYNOPSIS
    Common problem fix: Remote Access RDP Deep Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Access-RDP-Deep-Evidence-Report.ps1
.EXAMPLE
    .\Remote-Access-RDP-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Access RDP Deep Evidence Report'; $totalSteps = 8; $Name = 'Remote-Access-RDP-Deep-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Remote Access RDP Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Access RDP Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
