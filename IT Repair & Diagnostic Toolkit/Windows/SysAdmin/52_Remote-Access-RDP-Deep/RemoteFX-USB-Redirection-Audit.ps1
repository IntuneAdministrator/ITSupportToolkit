...
<#
.SYNOPSIS
    Enterprise IT tool: RemoteFX USB Redirection Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\RemoteFX-USB-Redirection-Audit.ps1
.EXAMPLE
    .\RemoteFX-USB-Redirection-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='RemoteFX USB Redirection Audit'; $totalSteps=8; $Name='RemoteFX-USB-Redirection-Audit'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '52_Remote-Access-RDP-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'REMOTE ACCESS / RDP DEEP'
        $ts = Get-Service TermService -EA SilentlyContinue
        Add-ReportRow -Check 'TermService' -Result $(if($ts){"$($ts.Status)"}else{'N/A'}) -Status $(if($ts -and $ts.Status -eq 'Running'){'PASS'}else{'WARN'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'RDP'
        $fDeny = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server' -Name fDenyTSConnections -EA SilentlyContinue).fDenyTSConnections
        Add-ReportRow -Check 'RDP enabled' -Result $(if($fDeny -eq 0){'Yes'}else{'Denied/Disabled'}) -Status $(if($fDeny -eq 0){'PASS'}else{'WARN'}) -Detail "fDenyTSConnections=$fDeny"
        if ($Name -match 'NLA|CredSSP') {
            ...
            Add-ReportRow -Check 'NLA' -Result "UserAuthentication=$nla" -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Certificate|Thumbprint') {
            $cert = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp' -Name SSLCertificateSHA1Hash -EA SilentlyContinue).SSLCertificateSHA1Hash
            Add-ReportRow -Check 'RDP cert hash' -Result $(if($cert){[BitConverter]::ToString($cert)}else{'Default self-signed'}) -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Quick-Assist') {
            Add-ReportRow -Check 'Quick Assist' -Result 'Check Appx MicrosoftCorporationII.QuickAssist + Intune allow' -Status 'INFO' -Detail ''
        }
        if ($PSCmdlet.ShouldProcess('TermService', 'ensure running') -and $ts -and $ts.Status -ne 'Running') {
            Start-Service TermService -EA SilentlyContinue
            Add-ReportRow -Check 'Start TermService' -Result 'Attempted' -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 5 $totalSteps $Activity 'RDP done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'RemoteFX USB Redirection Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RemoteFX USB Redirection Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
