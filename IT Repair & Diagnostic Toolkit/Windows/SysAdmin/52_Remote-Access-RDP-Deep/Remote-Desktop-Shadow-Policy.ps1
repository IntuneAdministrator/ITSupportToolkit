...
<#
.SYNOPSIS
    Enterprise IT tool: Remote Desktop Shadow Policy
.DESCRIPTION
    Phase 11 expansion (SysAdmin/52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Remote-Desktop-Shadow-Policy.ps1
.EXAMPLE
    .\Remote-Desktop-Shadow-Policy.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Remote Desktop Shadow Policy'; $totalSteps=8; $Name='Remote-Desktop-Shadow-Policy'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Export-HtmlReport -Title 'Remote Desktop Shadow Policy' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Desktop Shadow Policy' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
