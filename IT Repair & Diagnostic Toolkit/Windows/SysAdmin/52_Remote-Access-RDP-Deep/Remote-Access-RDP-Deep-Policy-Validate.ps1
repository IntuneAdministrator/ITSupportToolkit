...
<#
.SYNOPSIS
    Common problem fix: Remote Access RDP Deep Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Remote-Access-RDP-Deep-Policy-Validate.ps1
.EXAMPLE
    .\Remote-Access-RDP-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Access RDP Deep Policy Validate'; $totalSteps = 8; $Name = 'Remote-Access-RDP-Deep-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '52_Remote-Access-RDP-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '52_Remote-Access-RDP-Deep'
    ...
    Export-HtmlReport -Title 'Remote Access RDP Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Access RDP Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
