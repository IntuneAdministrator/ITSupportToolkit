...
$exitCode=0; $Activity='RDS License Grace Hint'; $totalSteps=8; $Name='RDS-License-Grace-Hint'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Export-HtmlReport -Title 'RDS License Grace Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDS License Grace Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
