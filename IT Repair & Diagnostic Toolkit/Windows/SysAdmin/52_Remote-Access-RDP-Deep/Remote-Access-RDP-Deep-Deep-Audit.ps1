...
<#
.SYNOPSIS
    Common problem fix: Remote Access RDP Deep Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Remote-Access-RDP-Deep-Deep-Audit.ps1
.EXAMPLE
    .\Remote-Access-RDP-Deep-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Remote Access RDP Deep Deep Audit'; $totalSteps = 8; $Name = 'Remote-Access-RDP-Deep-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Remote Access RDP Deep Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Remote Access RDP Deep Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
