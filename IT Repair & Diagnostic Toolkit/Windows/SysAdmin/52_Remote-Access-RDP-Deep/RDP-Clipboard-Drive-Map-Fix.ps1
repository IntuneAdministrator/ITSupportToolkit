...
<#
.SYNOPSIS
    Enterprise IT tool: RDP Clipboard Drive Map Fix
.DESCRIPTION
    Phase 11 expansion (SysAdmin/52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\RDP-Clipboard-Drive-Map-Fix.ps1
.EXAMPLE
    .\RDP-Clipboard-Drive-Map-Fix.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='RDP Clipboard Drive Map Fix'; $totalSteps=8; $Name='RDP-Clipboard-Drive-Map-Fix'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Export-HtmlReport -Title 'RDP Clipboard Drive Map Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Clipboard Drive Map Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
