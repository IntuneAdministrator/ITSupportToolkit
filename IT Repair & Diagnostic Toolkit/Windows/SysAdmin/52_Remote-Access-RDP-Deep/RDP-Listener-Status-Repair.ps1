...
<#
.SYNOPSIS
    Enterprise IT tool: RDP Listener Status Repair
.DESCRIPTION
    Phase 11 expansion (SysAdmin/52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\RDP-Listener-Status-Repair.ps1
.EXAMPLE
    .\RDP-Listener-Status-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='RDP Listener Status Repair'; $totalSteps=8; $Name='RDP-Listener-Status-Repair'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Export-HtmlReport -Title 'RDP Listener Status Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Listener Status Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
