...
<#
.SYNOPSIS
    Enterprise IT tool: Quick Assist Policy Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Quick-Assist-Policy-Check.ps1
.EXAMPLE
    .\Quick-Assist-Policy-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Quick Assist Policy Check'; $totalSteps=8; $Name='Quick-Assist-Policy-Check'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Export-HtmlReport -Title 'Quick Assist Policy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quick Assist Policy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
