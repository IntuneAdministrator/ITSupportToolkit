...
<#
.SYNOPSIS
    Enterprise IT tool: RDP Certificate Thumbprint
.DESCRIPTION
    Phase 11 expansion (SysAdmin/52_Remote-Access-RDP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\RDP-Certificate-Thumbprint.ps1
.EXAMPLE
    .\RDP-Certificate-Thumbprint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='RDP Certificate Thumbprint'; $totalSteps=8; $Name='RDP-Certificate-Thumbprint'; $Folder='52_Remote-Access-RDP-Deep'
try {
    ...
    Export-HtmlReport -Title 'RDP Certificate Thumbprint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Certificate Thumbprint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
