...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth Power State Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-Power-State-Audit.
...
.EXAMPLE
    .\Bluetooth-Power-State-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bluetooth-Power-State-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bluetooth Power State Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bluetooth-Power-State-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Bluetooth-Power-State-Audit' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Bluetooth Power State Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth Power State Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
