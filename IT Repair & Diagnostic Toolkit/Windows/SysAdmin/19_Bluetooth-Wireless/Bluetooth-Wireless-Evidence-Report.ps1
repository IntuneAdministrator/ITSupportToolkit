...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Evidence-Report.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Evidence Report'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
