...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Headset Multipoint Fix
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Headset-Multipoint-Fix.ps1
.EXAMPLE
    .\Bluetooth-Headset-Multipoint-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Headset Multipoint Fix'; $totalSteps = 8; $Name = 'Bluetooth-Headset-Multipoint-Fix'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Headset Multipoint Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Headset Multipoint Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
