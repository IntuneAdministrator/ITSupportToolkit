...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WLAN AutoConfig Service Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WLAN-AutoConfig-Service-Repair.
...
.EXAMPLE
    .\WLAN-AutoConfig-Service-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WLAN-AutoConfig-Service-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WLAN AutoConfig Service Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WLAN-AutoConfig-Service-Repair'
        Add-ReportRow -Check 'Tool' -Result 'WLAN-AutoConfig-Service-Repair' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'WLAN AutoConfig Service Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WLAN AutoConfig Service Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
