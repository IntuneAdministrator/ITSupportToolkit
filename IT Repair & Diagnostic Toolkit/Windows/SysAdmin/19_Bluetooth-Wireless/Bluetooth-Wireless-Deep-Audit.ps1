...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Deep-Audit.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Deep Audit'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
