...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Medical Device Pair Fix
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Medical-Device-Pair-Fix.ps1
.EXAMPLE
    .\Bluetooth-Medical-Device-Pair-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Medical Device Pair Fix'; $totalSteps = 8; $Name = 'Bluetooth-Medical-Device-Pair-Fix'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Medical Device Pair Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Medical Device Pair Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
