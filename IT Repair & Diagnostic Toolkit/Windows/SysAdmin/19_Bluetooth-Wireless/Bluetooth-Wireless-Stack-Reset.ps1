...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Stack-Reset.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Stack Reset'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
