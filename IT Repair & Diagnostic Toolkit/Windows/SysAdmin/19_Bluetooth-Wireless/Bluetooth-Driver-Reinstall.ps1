...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth Driver Reinstall
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-Driver-Reinstall.
...
.EXAMPLE
    .\Bluetooth-Driver-Reinstall.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bluetooth-Driver-Reinstall.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bluetooth Driver Reinstall'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bluetooth-Driver-Reinstall'
        Add-ReportRow -Check 'Tool' -Result 'Bluetooth-Driver-Reinstall' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Bluetooth Driver Reinstall' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth Driver Reinstall' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
