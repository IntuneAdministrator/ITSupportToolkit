...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth FIDO Key Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-FIDO-Key-Repair.
...
.EXAMPLE
    .\Bluetooth-FIDO-Key-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bluetooth-FIDO-Key-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bluetooth FIDO Key Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bluetooth-FIDO-Key-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Bluetooth-FIDO-Key-Repair' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Bluetooth FIDO Key Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth FIDO Key Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
