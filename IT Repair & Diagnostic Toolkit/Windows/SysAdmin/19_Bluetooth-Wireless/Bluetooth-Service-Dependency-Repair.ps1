...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth Service Dependency Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-Service-Dependency-Repair.
...
.EXAMPLE
    .\Bluetooth-Service-Dependency-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bluetooth-Service-Dependency-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bluetooth Service Dependency Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bluetooth-Service-Dependency-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Bluetooth-Service-Dependency-Repair' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
        Write-SectionBanner 'BLUETOOTH'
        $btSvcs = @('bthserv','BTAGService','BthAvctpSvc','BluetoothUserService')
        foreach ($s in $btSvcs) {
        ...
        Write-ProgressStep 2 $totalSteps $Activity 'BT services'
        ...
        if ($Name -match 'Stack-Reset|Service-Dependency' -and $PSCmdlet.ShouldProcess('Bluetooth', 'Restart bthserv + radio devices')) {
            Restart-Service bthserv -Force -ErrorAction SilentlyContinue
            foreach ($d in ($btDevs | Where-Object Status -ne 'OK')) {
                Disable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 1
                Enable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
            }
            Add-ReportRow -Check 'BT stack reset' -Result 'Services/devices recycled' -Status 'PASS' -Detail ''
        }
        Write-ProgressStep 3 $totalSteps $Activity 'BT devices'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Bluetooth Service Dependency Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth Service Dependency Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
