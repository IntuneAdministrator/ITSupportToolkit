...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wi Fi Driver Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wi-Fi-Driver-Reset.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Wi-Fi-Driver-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Wi-Fi-Driver-Reset' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Wi Fi Driver Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wi Fi Driver Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
