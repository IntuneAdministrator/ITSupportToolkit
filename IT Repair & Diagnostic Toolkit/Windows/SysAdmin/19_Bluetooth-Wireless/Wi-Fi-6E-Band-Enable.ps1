...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wi Fi 6E Band Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wi-Fi-6E-Band-Enable.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Wi-Fi-6E-Band-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Wi-Fi-6E-Band-Enable' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Wi Fi 6E Band Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wi Fi 6E Band Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
