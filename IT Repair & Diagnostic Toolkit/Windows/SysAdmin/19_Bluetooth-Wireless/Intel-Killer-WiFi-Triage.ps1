...
<#
.SYNOPSIS
    Common problem fix: Intel Killer WiFi Triage
.DESCRIPTION
...
.EXAMPLE
    .\Intel-Killer-WiFi-Triage.ps1
.EXAMPLE
    .\Intel-Killer-WiFi-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intel Killer WiFi Triage'; $totalSteps = 8; $Name = 'Intel-Killer-WiFi-Triage'
try {
    ...
    Export-HtmlReport -Title 'Intel Killer WiFi Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intel Killer WiFi Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
