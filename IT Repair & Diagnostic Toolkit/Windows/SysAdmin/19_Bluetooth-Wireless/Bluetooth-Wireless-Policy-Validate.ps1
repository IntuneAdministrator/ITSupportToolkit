...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Policy-Validate.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Policy Validate'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
