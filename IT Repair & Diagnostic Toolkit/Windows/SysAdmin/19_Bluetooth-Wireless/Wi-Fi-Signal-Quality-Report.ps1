...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wi Fi Signal Quality Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wi-Fi-Signal-Quality-Report.
...
.EXAMPLE
    .\Wi-Fi-Signal-Quality-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Wi-Fi-Signal-Quality-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Wi Fi Signal Quality Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Wi-Fi-Signal-Quality-Report'
        Add-ReportRow -Check 'Tool' -Result 'Wi-Fi-Signal-Quality-Report' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Wi Fi Signal Quality Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wi Fi Signal Quality Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
