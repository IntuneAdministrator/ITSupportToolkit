...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Anomaly-Detect.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Anomaly Detect'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
