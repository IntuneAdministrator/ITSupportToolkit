...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Auto-Repair.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Auto Repair'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
