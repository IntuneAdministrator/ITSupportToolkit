...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Health-Triage.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Health Triage'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
