...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wireless Profile Export Import
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wireless-Profile-Export-Import.
...
.EXAMPLE
    .\Wireless-Profile-Export-Import.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Wireless-Profile-Export-Import.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Wireless Profile Export Import'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Wireless-Profile-Export-Import'
        Add-ReportRow -Check 'Tool' -Result 'Wireless-Profile-Export-Import' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Wireless Profile Export Import' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wireless Profile Export Import' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
