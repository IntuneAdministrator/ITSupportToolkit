...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth Codec Quality Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-Codec-Quality-Audit.
...
.EXAMPLE
    .\Bluetooth-Codec-Quality-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Bluetooth-Codec-Quality-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Bluetooth Codec Quality Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bluetooth-Codec-Quality-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Bluetooth-Codec-Quality-Audit' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Bluetooth Codec Quality Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth Codec Quality Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
