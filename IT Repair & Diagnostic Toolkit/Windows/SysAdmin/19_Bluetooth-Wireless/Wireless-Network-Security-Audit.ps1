...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wireless Network Security Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wireless-Network-Security-Audit.
...
.EXAMPLE
    .\Wireless-Network-Security-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Wireless-Network-Security-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Wireless Network Security Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Wireless-Network-Security-Audit' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Wireless Network Security Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wireless Network Security Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
