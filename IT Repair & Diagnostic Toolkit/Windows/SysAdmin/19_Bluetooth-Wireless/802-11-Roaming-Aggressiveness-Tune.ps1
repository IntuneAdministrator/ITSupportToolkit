...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: 802 11 Roaming Aggressiveness Tune
.DESCRIPTION
    Production-grade Windows SysAdmin tool for 802-11-Roaming-Aggressiveness-Tune.
Category: 19_Bluetooth-Wireless. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\802-11-Roaming-Aggressiveness-Tune.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\802-11-Roaming-Aggressiveness-Tune.ps1 -WhatIf
...
$exitCode = 0
$Activity = '802 11 Roaming Aggressiveness Tune'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'BLUETOOTH WIRELESS'
        $Name = '802-11-Roaming-Aggressiveness-Tune'
        Add-ReportRow -Check 'Tool' -Result '802-11-Roaming-Aggressiveness-Tune' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
        Write-SectionBanner 'WI-FI / WLAN'
        $wlan = Get-Service WlanSvc -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'WlanSvc' -Result $(if ($wlan) { "$($wlan.Status)/$($wlan.StartType)" } else {'Missing'}) -Status $(if ($wlan -and $wlan.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail ''
        if ($PSCmdlet.ShouldProcess('WlanSvc', 'Restart WLAN AutoConfig')) {
            Restart-Service WlanSvc -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'WlanSvc restart' -Result (Get-Service WlanSvc).Status -Status 'PASS' -Detail ''
        }
        Write-ProgressStep 2 $totalSteps $Activity 'WlanSvc'

        $wifi = @(Get-NetAdapter | Where-Object { $_.InterfaceDescription -match 'Wi-?Fi|Wireless|802\.11' -or $_.Name -match 'Wi-?Fi' })
        foreach ($w in $wifi) {
            Add-ReportRow -Check $w.Name -Result "$($w.Status) $($w.LinkSpeed)" -Status $(if ($w.Status -eq 'Up') {'PASS'} else {'WARN'}) -Detail $w.InterfaceDescription
            if ($Name -match 'Driver-Reset' -and $PSCmdlet.ShouldProcess($w.Name, 'Disable/Enable Wi-Fi adapter')) {
                Disable-NetAdapter -Name $w.Name -Confirm:$false -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 3
                Enable-NetAdapter -Name $w.Name -Confirm:$false -ErrorAction SilentlyContinue
                Add-ReportRow -Check "Reset $($w.Name)" -Result 'Toggled' -Status 'PASS' -Detail ''
            }
        }
        $profiles = & netsh wlan show profiles 2>&1 | Out-String
        Add-ReportRow -Check 'WLAN profiles' -Result 'Enumerated' -Status 'INFO' -Detail ($profiles.Substring(0, [Math]::Min(400, $profiles.Length)))
        if ($Name -match 'Signal') {
            $iface = & netsh wlan show interfaces 2>&1 | Out-String
            Add-ReportRow -Check 'WLAN interfaces' -Result 'Signal dump' -Status 'INFO' -Detail ($iface.Substring(0, [Math]::Min(400, $iface.Length)))
        }
        if ($Name -match 'Profile-Export') {
            $exp = 'C:\IT-Logs\WLAN-Profiles'
            New-Item -ItemType Directory -Path $exp -Force | Out-Null
            if ($PSCmdlet.ShouldProcess($exp, 'netsh wlan export profile')) {
                & netsh wlan export profile folder="$exp" key=clear 2>&1 | Out-Null
                Add-ReportRow -Check 'Export profiles' -Result 'Done' -Status 'PASS' -Detail "$exp (contains keys - secure/delete after use)"
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Wi-Fi'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title '802 11 Roaming Aggressiveness Tune' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title '802 11 Roaming Aggressiveness Tune' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
