...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Bluetooth Stack Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Bluetooth-Stack-Reset.
...
$exitCode = 0
$Activity = 'Bluetooth Stack Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Bluetooth-Stack-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Bluetooth-Stack-Reset' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Bluetooth Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Bluetooth Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
