...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Paired Device Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Paired-Device-Cleanup.
...
$exitCode = 0
$Activity = 'Paired Device Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Paired-Device-Cleanup'
        Add-ReportRow -Check 'Tool' -Result 'Paired-Device-Cleanup' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
        Add-ReportRow -Check 'Radio services' -Result 'Audit' -Status 'INFO' -Detail 'Bluetooth/WLAN'
        Write-ProgressStep 2 $totalSteps $Activity 'Radios'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Paired Device Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Paired Device Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
