...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Hearing Aid Workaround
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Hearing-Aid-Workaround.ps1
.EXAMPLE
    .\Bluetooth-Hearing-Aid-Workaround.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Hearing Aid Workaround'; $totalSteps = 8; $Name = 'Bluetooth-Hearing-Aid-Workaround'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Hearing Aid Workaround' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Hearing Aid Workaround' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
