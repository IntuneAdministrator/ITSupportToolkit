...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Wi Fi Adapter Country Region Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Wi-Fi-Adapter-Country-Region-Fix.
...
.EXAMPLE
    .\Wi-Fi-Adapter-Country-Region-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Wi-Fi-Adapter-Country-Region-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Wi Fi Adapter Country Region Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Wi-Fi-Adapter-Country-Region-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Wi-Fi-Adapter-Country-Region-Fix' -Status 'INFO' -Detail '19_Bluetooth-Wireless'
    ...
    Export-HtmlReport -Title 'Wi Fi Adapter Country Region Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Wi Fi Adapter Country Region Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
