...
<#
.SYNOPSIS
    Common problem fix: Bluetooth Wireless Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Bluetooth-Wireless-Quick-Fix.ps1
.EXAMPLE
    .\Bluetooth-Wireless-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Bluetooth Wireless Quick Fix'; $totalSteps = 8; $Name = 'Bluetooth-Wireless-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Bluetooth Wireless Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Bluetooth Wireless Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
