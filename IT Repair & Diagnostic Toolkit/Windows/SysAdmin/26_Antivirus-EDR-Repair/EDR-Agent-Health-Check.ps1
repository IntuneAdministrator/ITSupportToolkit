...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: EDR Agent Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for EDR-Agent-Health-Check.
...
$exitCode = 0
$Activity = 'EDR Agent Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'EDR-Agent-Health-Check'
        Add-ReportRow -Check 'Tool' -Result 'EDR-Agent-Health-Check' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'EDR Agent Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'EDR Agent Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
