...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Stack Reset'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
