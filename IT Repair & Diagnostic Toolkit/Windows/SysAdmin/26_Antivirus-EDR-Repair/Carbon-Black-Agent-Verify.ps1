...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Carbon Black Agent Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Carbon-Black-Agent-Verify.
...
.EXAMPLE
    .\Carbon-Black-Agent-Verify.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Carbon-Black-Agent-Verify.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Carbon Black Agent Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Carbon-Black-Agent-Verify'
        Add-ReportRow -Check 'Tool' -Result 'Carbon-Black-Agent-Verify' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'Carbon Black Agent Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Carbon Black Agent Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
