...
<#
.SYNOPSIS
    Common problem fix: Quarantine Restore Guide Fix
.DESCRIPTION
...
.EXAMPLE
    .\Quarantine-Restore-Guide-Fix
.EXAMPLE
    .\Quarantine-Restore-Guide-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Quarantine Restore Guide Fix'; $totalSteps = 8; $Name = 'Quarantine-Restore-Guide-Fix'
try {
    ...
    Export-HtmlReport -Title 'Quarantine Restore Guide Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Quarantine Restore Guide Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
