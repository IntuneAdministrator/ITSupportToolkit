...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Auto Repair'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
