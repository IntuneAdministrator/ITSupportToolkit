...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: CrowdStrike Sensor Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for CrowdStrike-Sensor-Health-Check.
...
.EXAMPLE
    .\CrowdStrike-Sensor-Health-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\CrowdStrike-Sensor-Health-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'CrowdStrike Sensor Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'CrowdStrike-Sensor-Health-Check'
        Add-ReportRow -Check 'Tool' -Result 'CrowdStrike-Sensor-Health-Check' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'CrowdStrike Sensor Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'CrowdStrike Sensor Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
