...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Health-Triage.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Health Triage'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
