...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AV Definition Age Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AV-Definition-Age-Audit.
...
.EXAMPLE
    .\AV-Definition-Age-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AV-Definition-Age-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AV Definition Age Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AV-Definition-Age-Audit'
        Add-ReportRow -Check 'Tool' -Result 'AV-Definition-Age-Audit' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'AV Definition Age Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AV Definition Age Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
