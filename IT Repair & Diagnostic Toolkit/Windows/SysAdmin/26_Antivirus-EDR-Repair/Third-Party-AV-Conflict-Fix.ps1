...
<#
.SYNOPSIS
    Common problem fix: Third Party AV Conflict Fix
.DESCRIPTION
...
.EXAMPLE
    .\Third-Party-AV-Conflict-Fix
.EXAMPLE
    .\Third-Party-AV-Conflict-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Third Party AV Conflict Fix'; $totalSteps = 8; $Name = 'Third-Party-AV-Conflict-Fix'
try {
    ...
    Export-HtmlReport -Title 'Third Party AV Conflict Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Third Party AV Conflict Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
