...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: EDR Isolation Mode Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for EDR-Isolation-Mode-Check.
...
.EXAMPLE
    .\EDR-Isolation-Mode-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\EDR-Isolation-Mode-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'EDR Isolation Mode Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'EDR-Isolation-Mode-Check'
        Add-ReportRow -Check 'Tool' -Result 'EDR-Isolation-Mode-Check' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'EDR Isolation Mode Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'EDR Isolation Mode Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
