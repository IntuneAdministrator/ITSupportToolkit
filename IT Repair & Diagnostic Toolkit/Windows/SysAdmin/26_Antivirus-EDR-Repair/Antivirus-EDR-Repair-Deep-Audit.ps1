...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Deep Audit'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
