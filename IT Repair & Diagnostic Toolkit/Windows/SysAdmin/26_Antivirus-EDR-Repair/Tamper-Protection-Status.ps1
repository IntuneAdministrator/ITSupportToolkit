...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Tamper Protection Status
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Tamper-Protection-Status.
...
.EXAMPLE
    .\Tamper-Protection-Status.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Tamper-Protection-Status.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Tamper Protection Status'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Tamper-Protection-Status'
        Add-ReportRow -Check 'Tool' -Result 'Tamper-Protection-Status' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'Tamper Protection Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Tamper Protection Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
