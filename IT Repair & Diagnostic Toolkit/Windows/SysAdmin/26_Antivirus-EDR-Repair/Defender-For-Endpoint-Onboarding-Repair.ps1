...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Defender For Endpoint Onboarding Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Defender-For-Endpoint-Onboarding-Repair.
Category: 26_Antivirus-EDR-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Defender-For-Endpoint-Onboarding-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Defender-For-Endpoint-Onboarding-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Defender For Endpoint Onboarding Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'ANTIVIRUS EDR REPAIR'
        $Name = 'Defender-For-Endpoint-Onboarding-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Defender-For-Endpoint-Onboarding-Repair' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
        Write-SectionBanner 'DEFENDER / EDR'
        try {
            $mp = Get-MpComputerStatus -ErrorAction Stop
            Add-ReportRow -Check 'Real-time protection' -Result "$($mp.RealTimeProtectionEnabled)" -Status $(if ($mp.RealTimeProtectionEnabled) {'PASS'} else {'FAIL'}) -Detail "AMService=$($mp.AMServiceEnabled)"
            Add-ReportRow -Check 'Antivirus signature age' -Result "$($mp.AntivirusSignatureAge) days" -Status $(if ($mp.AntivirusSignatureAge -le 2) {'PASS'} else {'WARN'}) -Detail "$($mp.AntivirusSignatureLastUpdated)"
            Add-ReportRow -Check 'Tamper protection' -Result "$($mp.IsTamperProtected)" -Status 'INFO' -Detail ''
        } catch {
            Add-ReportRow -Check 'Defender status' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Defender status'

        if ($Name -match 'Real-Time|Full-Reset|Signature|Network-Protection|Definition|Scan-History|Onboarding') {
            if ($PSCmdlet.ShouldProcess('Defender', 'Re-enable protections / update signatures')) {
                try {
                    Set-MpPreference -DisableRealtimeMonitoring $false -ErrorAction SilentlyContinue
                    Update-MpSignature -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Realtime + signature update' -Result 'Applied' -Status 'PASS' -Detail 'Set-MpPreference / Update-MpSignature'
                } catch {
                    Add-ReportRow -Check 'Defender repair' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
                if ($Name -match 'Network-Protection') {
                    Set-MpPreference -EnableNetworkProtection Enabled -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Network Protection' -Result 'Enabled' -Status 'PASS' -Detail ''
                }
                if ($Name -match 'Full-Reset') {
                    Start-MpScan -ScanType QuickScan -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Quick scan started' -Result 'OK' -Status 'INFO' -Detail ''
                }
            }
            try {
                $threats = @(Get-MpThreatDetection -ErrorAction SilentlyContinue | Select-Object -First 15)
                Add-ReportRow -Check 'Threat detections' -Result "$($threats.Count)" -Status $(if ($threats.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ''
            } catch {}
        }

        if ($Name -match 'Third-Party|CrowdStrike|SentinelOne|Carbon-Black|EDR') {
            $patterns = @{
                'CrowdStrike' = 'CSFalcon|CSAgent|CrowdStrike'
                'SentinelOne' = 'SentinelAgent|SentinelOne'
                'Carbon Black' = 'CbDefense|CarbonBlack|Sense'
                'Defender' = 'MsMpEng|WinDefend'
            }
            foreach ($k in $patterns.Keys) {
                $procs = @(Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.ProcessName -match $patterns[$k] })
                $svcs = @(Get-Service -ErrorAction SilentlyContinue | Where-Object { $_.Name -match $patterns[$k] -or $_.DisplayName -match $k })
                Add-ReportRow -Check "$k presence" -Result "Procs=$($procs.Count) Svcs=$($svcs.Count)" -Status $(if ($procs.Count -gt 0 -or $svcs.Count -gt 0) {'PASS'} else {'INFO'}) -Detail (($svcs | Select-Object -First 3 Name) -join ', ')
            }
            if ($Name -match 'Isolation') {
                Add-ReportRow -Check 'EDR isolation' -Result 'Check console / netsh interface connectivity' -Status 'INFO' -Detail 'Host isolation is typically EDR-console controlled'
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'AV/EDR'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Defender For Endpoint Onboarding Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Defender For Endpoint Onboarding Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
