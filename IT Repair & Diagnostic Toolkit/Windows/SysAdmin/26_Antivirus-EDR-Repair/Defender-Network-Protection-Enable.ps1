...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Defender Network Protection Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Defender-Network-Protection-Enable.
...
.EXAMPLE
    .\Defender-Network-Protection-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Defender-Network-Protection-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Defender Network Protection Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Defender-Network-Protection-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Defender-Network-Protection-Enable' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'Defender Network Protection Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Defender Network Protection Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
