...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Third Party AV Conflict Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Third-Party-AV-Conflict-Detect.
...
.EXAMPLE
    .\Third-Party-AV-Conflict-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Third-Party-AV-Conflict-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Third Party AV Conflict Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Third-Party-AV-Conflict-Detect'
        Add-ReportRow -Check 'Tool' -Result 'Third-Party-AV-Conflict-Detect' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'Third Party AV Conflict Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Third Party AV Conflict Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
