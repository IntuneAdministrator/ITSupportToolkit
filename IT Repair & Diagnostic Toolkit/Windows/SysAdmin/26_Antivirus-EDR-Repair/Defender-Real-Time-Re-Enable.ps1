...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Defender Real Time Re Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Defender-Real-Time-Re-Enable.
...
.EXAMPLE
    .\Defender-Real-Time-Re-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Defender-Real-Time-Re-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Defender Real Time Re Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Defender-Real-Time-Re-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Defender-Real-Time-Re-Enable' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'Defender Real Time Re Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Defender Real Time Re Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
