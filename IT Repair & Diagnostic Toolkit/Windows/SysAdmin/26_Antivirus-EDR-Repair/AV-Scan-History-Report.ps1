...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AV Scan History Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AV-Scan-History-Report.
...
$exitCode = 0
$Activity = 'AV Scan History Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AV-Scan-History-Report'
        Add-ReportRow -Check 'Tool' -Result 'AV-Scan-History-Report' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'AV Scan History Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AV Scan History Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
