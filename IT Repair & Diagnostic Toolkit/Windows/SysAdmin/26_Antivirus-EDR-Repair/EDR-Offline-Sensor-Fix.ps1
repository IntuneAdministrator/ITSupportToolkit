...
<#
.SYNOPSIS
    Common problem fix: EDR Offline Sensor Fix
.DESCRIPTION
...
.EXAMPLE
    .\EDR-Offline-Sensor-Fix
.EXAMPLE
    .\EDR-Offline-Sensor-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'EDR Offline Sensor Fix'; $totalSteps = 8; $Name = 'EDR-Offline-Sensor-Fix'
try {
    ...
    Export-HtmlReport -Title 'EDR Offline Sensor Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'EDR Offline Sensor Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
