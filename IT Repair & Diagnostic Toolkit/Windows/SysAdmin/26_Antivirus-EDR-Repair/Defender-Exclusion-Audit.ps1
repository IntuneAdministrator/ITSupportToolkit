...
<#
.SYNOPSIS
    Common problem fix: Defender Exclusion Audit
.DESCRIPTION
...
.EXAMPLE
    .\Defender-Exclusion-Audit -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Defender Exclusion Audit'; $totalSteps = 8; $Name = 'Defender-Exclusion-Audit'
try {
    ...
    Export-HtmlReport -Title 'Defender Exclusion Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Defender Exclusion Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
