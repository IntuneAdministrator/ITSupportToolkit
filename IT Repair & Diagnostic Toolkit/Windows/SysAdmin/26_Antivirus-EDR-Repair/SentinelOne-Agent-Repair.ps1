...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SentinelOne Agent Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SentinelOne-Agent-Repair.
...
.EXAMPLE
    .\SentinelOne-Agent-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SentinelOne-Agent-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SentinelOne Agent Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SentinelOne-Agent-Repair'
        Add-ReportRow -Check 'Tool' -Result 'SentinelOne-Agent-Repair' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'SentinelOne Agent Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SentinelOne Agent Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
