...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Quick Fix'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
