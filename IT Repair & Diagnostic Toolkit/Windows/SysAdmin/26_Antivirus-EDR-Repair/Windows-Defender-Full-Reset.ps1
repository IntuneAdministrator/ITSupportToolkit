...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Defender Full Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Defender-Full-Reset.
...
.EXAMPLE
    .\Windows-Defender-Full-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Defender-Full-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Defender Full Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Defender-Full-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Defender-Full-Reset' -Status 'INFO' -Detail '26_Antivirus-EDR-Repair'
    ...
    Export-HtmlReport -Title 'Windows Defender Full Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Defender Full Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
