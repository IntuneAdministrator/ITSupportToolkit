...
<#
.SYNOPSIS
    Common problem fix: Tamper Protection Block Fix
.DESCRIPTION
...
.EXAMPLE
    .\Tamper-Protection-Block-Fix
.EXAMPLE
    .\Tamper-Protection-Block-Fix -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Tamper Protection Block Fix'; $totalSteps = 8; $Name = 'Tamper-Protection-Block-Fix'
try {
    ...
    Export-HtmlReport -Title 'Tamper Protection Block Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Tamper Protection Block Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
