...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
