...
<#
.SYNOPSIS
    Common problem fix: Antivirus EDR Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Antivirus-EDR-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Antivirus-EDR-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Antivirus EDR Repair Evidence Report'; $totalSteps = 8; $Name = 'Antivirus-EDR-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Antivirus EDR Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Antivirus EDR Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
