...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Health-Triage.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Health Triage'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
