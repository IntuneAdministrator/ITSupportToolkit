...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Auto Enroll GPO Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Auto-Enroll-GPO-Verify.
...
$exitCode = 0
$Activity = 'Auto Enroll GPO Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Auto-Enroll-GPO-Verify'
        Add-ReportRow -Check 'Tool' -Result 'Auto-Enroll-GPO-Verify' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'Auto Enroll GPO Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Auto Enroll GPO Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
