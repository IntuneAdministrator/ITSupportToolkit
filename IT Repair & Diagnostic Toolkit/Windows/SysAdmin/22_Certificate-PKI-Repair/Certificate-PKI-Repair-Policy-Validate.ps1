...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Policy Validate'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
