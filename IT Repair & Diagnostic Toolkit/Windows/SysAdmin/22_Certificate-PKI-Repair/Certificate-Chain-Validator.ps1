...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Certificate Chain Validator
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Certificate-Chain-Validator.
...
.EXAMPLE
    .\Certificate-Chain-Validator.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Certificate-Chain-Validator.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Certificate Chain Validator'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Certificate-Chain-Validator' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'Certificate Chain Validator' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Certificate Chain Validator' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
