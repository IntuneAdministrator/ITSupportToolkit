...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: CRL Distribution Point Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for CRL-Distribution-Point-Test.
...
.EXAMPLE
    .\CRL-Distribution-Point-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\CRL-Distribution-Point-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'CRL Distribution Point Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'CRL-Distribution-Point-Test'
        Add-ReportRow -Check 'Tool' -Result 'CRL-Distribution-Point-Test' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'CRL Distribution Point Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'CRL Distribution Point Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
