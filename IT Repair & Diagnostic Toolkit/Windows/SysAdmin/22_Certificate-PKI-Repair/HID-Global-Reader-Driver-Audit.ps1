...
<#
.SYNOPSIS
    Common problem fix: HID Global Reader Driver Audit
.DESCRIPTION
...
.EXAMPLE
    .\HID-Global-Reader-Driver-Audit.ps1
.EXAMPLE
    .\HID-Global-Reader-Driver-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HID Global Reader Driver Audit'; $totalSteps = 8; $Name = 'HID-Global-Reader-Driver-Audit'
try {
    ...
    Export-HtmlReport -Title 'HID Global Reader Driver Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HID Global Reader Driver Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
