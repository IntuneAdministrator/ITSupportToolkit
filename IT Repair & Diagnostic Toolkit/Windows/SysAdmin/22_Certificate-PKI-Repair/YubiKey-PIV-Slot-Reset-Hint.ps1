...
<#
.SYNOPSIS
    Common problem fix: YubiKey PIV Slot Reset Hint
.DESCRIPTION
...
.EXAMPLE
    .\YubiKey-PIV-Slot-Reset-Hint.ps1
.EXAMPLE
    .\YubiKey-PIV-Slot-Reset-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'YubiKey PIV Slot Reset Hint'; $totalSteps = 8; $Name = 'YubiKey-PIV-Slot-Reset-Hint'
try {
    ...
    Export-HtmlReport -Title 'YubiKey PIV Slot Reset Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'YubiKey PIV Slot Reset Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
