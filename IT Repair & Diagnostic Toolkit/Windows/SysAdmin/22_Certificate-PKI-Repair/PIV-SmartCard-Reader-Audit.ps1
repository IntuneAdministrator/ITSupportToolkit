...
<#
.SYNOPSIS
    Common problem fix: PIV SmartCard Reader Audit
.DESCRIPTION
...
.EXAMPLE
    .\PIV-SmartCard-Reader-Audit.ps1
.EXAMPLE
    .\PIV-SmartCard-Reader-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'PIV SmartCard Reader Audit'; $totalSteps = 8; $Name = 'PIV-SmartCard-Reader-Audit'
try {
    ...
    Export-HtmlReport -Title 'PIV SmartCard Reader Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'PIV SmartCard Reader Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
