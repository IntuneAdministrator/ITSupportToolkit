...
<#
.SYNOPSIS
    Common problem fix: Gemalto SmartCard Reader Triage
.DESCRIPTION
...
.EXAMPLE
    .\Gemalto-SmartCard-Reader-Triage.ps1
.EXAMPLE
    .\Gemalto-SmartCard-Reader-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Gemalto SmartCard Reader Triage'; $totalSteps = 8; $Name = 'Gemalto-SmartCard-Reader-Triage'
try {
    ...
    Export-HtmlReport -Title 'Gemalto SmartCard Reader Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Gemalto SmartCard Reader Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
