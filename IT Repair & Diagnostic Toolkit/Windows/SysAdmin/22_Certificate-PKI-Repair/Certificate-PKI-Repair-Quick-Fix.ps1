...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Quick Fix'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
