...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
