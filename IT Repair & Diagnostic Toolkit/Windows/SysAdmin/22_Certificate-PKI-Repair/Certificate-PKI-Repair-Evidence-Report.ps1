...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Evidence Report'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
