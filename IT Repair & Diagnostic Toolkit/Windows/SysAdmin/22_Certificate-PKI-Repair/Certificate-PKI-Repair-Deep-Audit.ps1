...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Deep Audit'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
