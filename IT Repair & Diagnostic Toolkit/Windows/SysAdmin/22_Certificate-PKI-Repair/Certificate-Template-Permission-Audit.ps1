...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Certificate Template Permission Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Certificate-Template-Permission-Audit.
Category: 22_Certificate-PKI-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Certificate-Template-Permission-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Certificate-Template-Permission-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Certificate Template Permission Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'CERTIFICATE PKI REPAIR'
        $Name = 'Certificate-Template-Permission-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Certificate-Template-Permission-Audit' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
        Write-SectionBanner 'CERTIFICATE / PKI'
        $stores = @('My','Root','CA','AuthRoot','TrustedPublisher')
        foreach ($st in $stores) {
            try {
                $certs = @(Get-ChildItem "Cert:\LocalMachine\$st" -ErrorAction SilentlyContinue)
                $expired = @($certs | Where-Object { $_.NotAfter -lt (Get-Date) })
                Add-ReportRow -Check "LM\$st" -Result "$($certs.Count) certs ($($expired.Count) expired)" -Status $(if ($expired.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ''
            } catch {
                Add-ReportRow -Check "LM\$st" -Result 'Error' -Status 'WARN' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Stores'

        if ($Name -match 'Expired|Store-Audit|PKI-Health') {
            $expSoon = @(Get-ChildItem Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Where-Object { $_.NotAfter -lt (Get-Date).AddDays(30) })
            foreach ($c in ($expSoon | Select-Object -First 20)) {
                $st = if ($c.NotAfter -lt (Get-Date)) {'FAIL'} else {'WARN'}
                Add-ReportRow -Check $c.Subject -Result "Expires $($c.NotAfter)" -Status $st -Detail $c.Thumbprint
            }
        }

        if ($Name -match 'Root-CA|Chain') {
            $roots = @(Get-ChildItem Cert:\LocalMachine\Root | Select-Object -First 15)
            Add-ReportRow -Check 'Root sample' -Result "$($roots.Count) listed" -Status 'INFO' -Detail (($roots | ForEach-Object { $_.Subject.Substring(0, [Math]::Min(60, $_.Subject.Length)) }) -join ' | ')
        }

        if ($Name -match 'SSL-TLS|Weak-Cipher') {
            $schannel = 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols'
            Backup-RegistryKey -KeyPath 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            foreach ($proto in @('SSL 2.0','SSL 3.0','TLS 1.0','TLS 1.1','TLS 1.2','TLS 1.3')) {
                $p = Join-Path $schannel "$proto\Server"
                $enabled = $null
                if (Test-Path $p) { $enabled = (Get-ItemProperty $p -Name Enabled -ErrorAction SilentlyContinue).Enabled }
                Add-ReportRow -Check "SCHANNEL $proto Server" -Result $(if ($null -eq $enabled) {'Default'} else {"Enabled=$enabled"}) -Status 'INFO' -Detail $p
            }
        }

        if ($Name -match 'CRL|OCSP') {
            try {
                $my = Get-ChildItem Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Select-Object -First 5
                foreach ($c in $my) {
                    $cdp = $c.Extensions | Where-Object { $_.Oid.FriendlyName -match 'CRL Distribution' }
                    Add-ReportRow -Check "CDP $($c.Thumbprint.Substring(0,8))" -Result $(if ($cdp) {'Present'} else {'None'}) -Status 'INFO' -Detail $c.Subject
                }
            } catch {
                Add-ReportRow -Check 'CRL/OCSP' -Result 'Check failed' -Status 'WARN' -Detail $_.Exception.Message
            }
        }

        if ($Name -match 'Auto-Enroll|SCEP|Machine-Certificate|Private-Key|Template') {
            if ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
                Add-ReportRow -Check 'Auto-enroll context' -Result 'Domain/Hybrid' -Status 'PASS' -Detail $Join.Label
                if ($PSCmdlet.ShouldProcess('certutil', 'certutil -pulse')) {
                    $pulse = & certutil -pulse 2>&1 | Out-String
                    Add-ReportRow -Check 'certutil -pulse' -Result 'Triggered' -Status 'INFO' -Detail ($pulse.Substring(0, [Math]::Min(300, $pulse.Length)))
                }
            } else {
                Add-ReportRow -Check 'Auto-enroll context' -Result $Join.Label -Status 'WARN' -Detail 'GPO auto-enroll typically requires domain join; Intune SCEP/PKCS for cloud'
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'PKI'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Certificate Template Permission Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Certificate Template Permission Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
