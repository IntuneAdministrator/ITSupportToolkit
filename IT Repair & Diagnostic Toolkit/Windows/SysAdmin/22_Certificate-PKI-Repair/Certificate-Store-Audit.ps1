...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Certificate Store Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Certificate-Store-Audit.
...
.EXAMPLE
    .\Certificate-Store-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Certificate-Store-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Certificate Store Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Certificate-Store-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Certificate-Store-Audit' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'Certificate Store Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Certificate Store Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
