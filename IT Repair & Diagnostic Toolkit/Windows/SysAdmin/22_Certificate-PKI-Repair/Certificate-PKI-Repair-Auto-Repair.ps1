...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Auto Repair'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
