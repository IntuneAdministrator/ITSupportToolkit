...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Weak Cipher Suite Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Weak-Cipher-Suite-Audit.
...
.EXAMPLE
    .\Weak-Cipher-Suite-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Weak-Cipher-Suite-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Weak Cipher Suite Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Weak-Cipher-Suite-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Weak-Cipher-Suite-Audit' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'Weak Cipher Suite Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Weak Cipher Suite Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
