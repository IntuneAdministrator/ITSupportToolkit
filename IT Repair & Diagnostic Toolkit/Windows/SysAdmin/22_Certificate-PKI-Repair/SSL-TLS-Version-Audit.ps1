...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SSL TLS Version Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SSL-TLS-Version-Audit.
...
$exitCode = 0
$Activity = 'SSL TLS Version Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SSL-TLS-Version-Audit'
        Add-ReportRow -Check 'Tool' -Result 'SSL-TLS-Version-Audit' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'SSL TLS Version Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SSL TLS Version Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
