...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Expired Certificate Finder
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Expired-Certificate-Finder.
...
.EXAMPLE
    .\Expired-Certificate-Finder.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Expired-Certificate-Finder.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Expired Certificate Finder'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Expired-Certificate-Finder'
        Add-ReportRow -Check 'Tool' -Result 'Expired-Certificate-Finder' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'Expired Certificate Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Expired Certificate Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
