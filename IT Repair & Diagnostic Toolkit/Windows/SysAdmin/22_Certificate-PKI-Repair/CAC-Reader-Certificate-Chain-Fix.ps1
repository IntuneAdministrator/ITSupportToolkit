...
<#
.SYNOPSIS
    Common problem fix: CAC Reader Certificate Chain Fix
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 22_Certificate-PKI-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\CAC-Reader-Certificate-Chain-Fix.ps1
.EXAMPLE
    .\CAC-Reader-Certificate-Chain-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CAC Reader Certificate Chain Fix'; $totalSteps = 8; $Name = 'CAC-Reader-Certificate-Chain-Fix'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '22_Certificate-PKI-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'CAC Reader Certificate Chain Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CAC Reader Certificate Chain Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
