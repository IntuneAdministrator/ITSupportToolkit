...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Machine Certificate Renewal Force
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Machine-Certificate-Renewal-Force.
...
.EXAMPLE
    .\Machine-Certificate-Renewal-Force.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Machine-Certificate-Renewal-Force.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Machine Certificate Renewal Force'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Machine-Certificate-Renewal-Force'
        Add-ReportRow -Check 'Tool' -Result 'Machine-Certificate-Renewal-Force' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'Machine Certificate Renewal Force' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Machine Certificate Renewal Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
