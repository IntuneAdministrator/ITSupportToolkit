...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SCEP NDES Enrollment Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SCEP-NDES-Enrollment-Fix.
...
.EXAMPLE
    .\SCEP-NDES-Enrollment-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SCEP-NDES-Enrollment-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SCEP NDES Enrollment Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SCEP-NDES-Enrollment-Fix'
        Add-ReportRow -Check 'Tool' -Result 'SCEP-NDES-Enrollment-Fix' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'SCEP NDES Enrollment Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SCEP NDES Enrollment Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
