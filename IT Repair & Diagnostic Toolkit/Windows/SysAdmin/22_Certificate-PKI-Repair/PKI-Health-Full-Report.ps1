...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PKI Health Full Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PKI-Health-Full-Report.
...
$exitCode = 0
$Activity = 'PKI Health Full Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PKI-Health-Full-Report'
        Add-ReportRow -Check 'Tool' -Result 'PKI-Health-Full-Report' -Status 'INFO' -Detail '22_Certificate-PKI-Repair'
    ...
    Export-HtmlReport -Title 'PKI Health Full Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PKI Health Full Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
