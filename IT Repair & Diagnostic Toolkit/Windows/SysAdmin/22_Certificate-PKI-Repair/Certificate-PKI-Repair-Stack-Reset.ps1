...
<#
.SYNOPSIS
    Common problem fix: Certificate PKI Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Certificate-PKI-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Certificate-PKI-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Certificate PKI Repair Stack Reset'; $totalSteps = 8; $Name = 'Certificate-PKI-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Certificate PKI Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Certificate PKI Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
