...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Deep-Audit.ps1
.EXAMPLE
    .\Group-Policy-Deep-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Deep Audit'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
