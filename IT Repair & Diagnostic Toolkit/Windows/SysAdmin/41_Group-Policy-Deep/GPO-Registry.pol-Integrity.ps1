...
<#
.SYNOPSIS
    Enterprise IT tool: GPO Registry.pol Integrity
.DESCRIPTION
    Phase 10 expansion (SysAdmin/41_Group-Policy-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\GPO-Registry.pol-Integrity.ps1
.EXAMPLE
    .\GPO-Registry.pol-Integrity.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='GPO Registry.pol Integrity'; $totalSteps=8; $Name='GPO-Registry.pol-Integrity'; $Folder='41_Group-Policy-Deep'
try {
    ...
    Export-HtmlReport -Title 'GPO Registry.pol Integrity' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'GPO Registry.pol Integrity' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
