...
$exitCode=0; $Activity='WMI Filter Eval Test'; $totalSteps=8; $Name='WMI-Filter-Eval-Test'; $Folder='41_Group-Policy-Deep'
try {
    ...
    Export-HtmlReport -Title 'WMI Filter Eval Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WMI Filter Eval Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
