...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Policy-Validate.ps1
.EXAMPLE
    .\Group-Policy-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Policy Validate'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
