...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Anomaly-Detect.ps1
.EXAMPLE
    .\Group-Policy-Deep-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Anomaly Detect'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
