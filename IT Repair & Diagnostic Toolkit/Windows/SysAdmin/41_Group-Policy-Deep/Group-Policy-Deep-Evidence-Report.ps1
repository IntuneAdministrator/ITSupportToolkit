...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Evidence-Report.ps1
.EXAMPLE
    .\Group-Policy-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Evidence Report'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
