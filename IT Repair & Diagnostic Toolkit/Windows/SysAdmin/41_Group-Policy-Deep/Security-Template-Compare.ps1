...
<#
.SYNOPSIS
    Enterprise IT tool: Security Template Compare
.DESCRIPTION
    Phase 10 expansion (SysAdmin/41_Group-Policy-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Security-Template-Compare.ps1
.EXAMPLE
    .\Security-Template-Compare.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Security Template Compare'; $totalSteps=8; $Name='Security-Template-Compare'; $Folder='41_Group-Policy-Deep'
try {
    ...
    Export-HtmlReport -Title 'Security Template Compare' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Template Compare' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
