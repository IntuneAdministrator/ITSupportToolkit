...
<#
.SYNOPSIS
    Enterprise IT tool: GPO Loopback Mode Audit
.DESCRIPTION
    Phase 10 expansion (SysAdmin/41_Group-Policy-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\GPO-Loopback-Mode-Audit.ps1
.EXAMPLE
    .\GPO-Loopback-Mode-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='GPO Loopback Mode Audit'; $totalSteps=8; $Name='GPO-Loopback-Mode-Audit'; $Folder='41_Group-Policy-Deep'
try {
    ...
    Export-HtmlReport -Title 'GPO Loopback Mode Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'GPO Loopback Mode Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
