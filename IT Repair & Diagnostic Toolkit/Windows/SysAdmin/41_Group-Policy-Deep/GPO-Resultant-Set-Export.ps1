...
<#
.SYNOPSIS
    Enterprise IT tool: GPO Resultant Set Export
.DESCRIPTION
    Phase 10 expansion (SysAdmin/41_Group-Policy-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\GPO-Resultant-Set-Export.ps1
.EXAMPLE
    .\GPO-Resultant-Set-Export.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='GPO Resultant Set Export'; $totalSteps=8; $Name='GPO-Resultant-Set-Export'; $Folder='41_Group-Policy-Deep'
try {
    ...
    Export-HtmlReport -Title 'GPO Resultant Set Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'GPO Resultant Set Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
