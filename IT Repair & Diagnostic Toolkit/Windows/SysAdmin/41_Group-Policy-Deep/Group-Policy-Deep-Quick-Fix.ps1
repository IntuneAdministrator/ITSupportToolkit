...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Quick-Fix.ps1
.EXAMPLE
    .\Group-Policy-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Quick Fix'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
