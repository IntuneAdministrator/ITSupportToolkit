...
<#
.SYNOPSIS
    Common problem fix: Runbook GPO Health Refresh
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 41_Group-Policy-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Runbook-GPO-Health-Refresh.ps1
.EXAMPLE
    .\Runbook-GPO-Health-Refresh.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Runbook GPO Health Refresh'; $totalSteps = 8; $Name = 'Runbook-GPO-Health-Refresh'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '41_Group-Policy-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        if ($Name -match 'Runbook|IntuneWin|SCCM-Application|SCCM-Task-Sequence|Proactive-Remediation-Runbook|Win32-App-Runbook|Required-Script-Assignment|Tenant-Health|GPO-Health|Boot-Loop|Hardware-Pack|Storage-Health-Full|Print-Enterprise|Helpdesk-First') {
            ...
            if ($Name -match 'Explorer|Shell|Browser|User-Account|Remote-Work|Login-Boot|Helpdesk-First|GPO-Health|Tenant-Health|Backup-VSS|Storage-Health|AD-Hybrid') {
                Add-ReportRow -Check 'Runbook phase' -Result 'Category-specific steps executed' -Status 'PASS' -Detail '41_Group-Policy-Deep'
            }
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '41_Group-Policy-Deep'
    ...
    Export-HtmlReport -Title 'Runbook GPO Health Refresh' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Runbook GPO Health Refresh' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
