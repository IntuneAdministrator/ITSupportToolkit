...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Auto-Repair.ps1
.EXAMPLE
    .\Group-Policy-Deep-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Auto Repair'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
