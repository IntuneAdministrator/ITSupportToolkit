...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Health-Triage.ps1
.EXAMPLE
    .\Group-Policy-Deep-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Health Triage'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
