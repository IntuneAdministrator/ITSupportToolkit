...
<#
.SYNOPSIS
    Common problem fix: Group Policy Deep Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Group-Policy-Deep-Stack-Reset.ps1
.EXAMPLE
    .\Group-Policy-Deep-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Group Policy Deep Stack Reset'; $totalSteps = 8; $Name = 'Group-Policy-Deep-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Group Policy Deep Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Group Policy Deep Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
