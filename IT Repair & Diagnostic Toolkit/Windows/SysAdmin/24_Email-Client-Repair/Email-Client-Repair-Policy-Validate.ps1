...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Email-Client-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Policy Validate'; $totalSteps = 8; $Name = 'Email-Client-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
