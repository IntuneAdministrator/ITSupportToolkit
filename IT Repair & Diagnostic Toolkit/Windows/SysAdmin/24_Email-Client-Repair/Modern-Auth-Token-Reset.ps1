...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Modern Auth Token Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Modern-Auth-Token-Reset.
...
.EXAMPLE
    .\Modern-Auth-Token-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Modern-Auth-Token-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Modern Auth Token Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Modern-Auth-Token-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Modern-Auth-Token-Reset' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Modern Auth Token Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Modern Auth Token Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
