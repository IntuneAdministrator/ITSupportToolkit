...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Profile Export Backup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Profile-Export-Backup.
...
.EXAMPLE
    .\Outlook-Profile-Export-Backup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Profile-Export-Backup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Profile Export Backup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Profile-Export-Backup'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Profile-Export-Backup' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Profile Export Backup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Profile Export Backup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
