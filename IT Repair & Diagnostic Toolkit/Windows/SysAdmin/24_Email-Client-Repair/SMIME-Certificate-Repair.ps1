...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: SMIME Certificate Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for SMIME-Certificate-Repair.
...
.EXAMPLE
    .\SMIME-Certificate-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\SMIME-Certificate-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'SMIME Certificate Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'SMIME-Certificate-Repair'
        Add-ReportRow -Check 'Tool' -Result 'SMIME-Certificate-Repair' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'SMIME Certificate Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'SMIME Certificate Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
