...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Email-Client-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Auto Repair'; $totalSteps = 8; $Name = 'Email-Client-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
