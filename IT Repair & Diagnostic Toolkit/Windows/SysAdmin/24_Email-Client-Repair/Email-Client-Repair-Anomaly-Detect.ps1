...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Email-Client-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Email-Client-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
