...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Delegate Permission Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Delegate-Permission-Fix.
...
.EXAMPLE
    .\Outlook-Delegate-Permission-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Delegate-Permission-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Delegate Permission Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Delegate-Permission-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Delegate-Permission-Fix' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Delegate Permission Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Delegate Permission Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
