...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook PST Integrity Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-PST-Integrity-Check.
...
.EXAMPLE
    .\Outlook-PST-Integrity-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-PST-Integrity-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook PST Integrity Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-PST-Integrity-Check'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-PST-Integrity-Check' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook PST Integrity Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook PST Integrity Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
