...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Exchange Autodiscover Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Exchange-Autodiscover-Test.
...
.EXAMPLE
    .\Exchange-Autodiscover-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Exchange-Autodiscover-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Exchange Autodiscover Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Exchange-Autodiscover-Test'
        Add-ReportRow -Check 'Tool' -Result 'Exchange-Autodiscover-Test' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Exchange Autodiscover Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Exchange Autodiscover Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
