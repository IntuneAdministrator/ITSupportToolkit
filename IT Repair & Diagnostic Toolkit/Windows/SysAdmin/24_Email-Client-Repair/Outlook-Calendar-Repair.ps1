...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Calendar Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Calendar-Repair.
...
.EXAMPLE
    .\Outlook-Calendar-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Calendar-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Calendar Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Calendar-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Calendar-Repair' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Calendar Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Calendar Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
