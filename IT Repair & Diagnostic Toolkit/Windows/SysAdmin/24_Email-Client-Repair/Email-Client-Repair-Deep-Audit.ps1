...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Email-Client-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Deep Audit'; $totalSteps = 8; $Name = 'Email-Client-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
