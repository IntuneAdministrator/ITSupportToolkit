...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Safe Mode Launch
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Safe-Mode-Launch.
...
.EXAMPLE
    .\Outlook-Safe-Mode-Launch.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Safe-Mode-Launch.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Safe Mode Launch'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Safe-Mode-Launch'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Safe-Mode-Launch' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Safe Mode Launch' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Safe Mode Launch' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
