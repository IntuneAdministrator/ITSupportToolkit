...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Email-Client-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Quick Fix'; $totalSteps = 8; $Name = 'Email-Client-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
