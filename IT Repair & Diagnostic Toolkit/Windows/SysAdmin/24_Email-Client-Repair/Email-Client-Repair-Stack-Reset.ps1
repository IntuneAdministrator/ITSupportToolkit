...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Email-Client-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Stack Reset'; $totalSteps = 8; $Name = 'Email-Client-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
