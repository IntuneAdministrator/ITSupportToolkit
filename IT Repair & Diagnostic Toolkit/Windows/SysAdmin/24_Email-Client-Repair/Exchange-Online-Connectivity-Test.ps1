...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Exchange Online Connectivity Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Exchange-Online-Connectivity-Test.
Category: 24_Email-Client-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Exchange-Online-Connectivity-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Exchange-Online-Connectivity-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Exchange Online Connectivity Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'EMAIL CLIENT REPAIR'
        $Name = 'Exchange-Online-Connectivity-Test'
        Add-ReportRow -Check 'Tool' -Result 'Exchange-Online-Connectivity-Test' -Status 'INFO' -Detail '24_Email-Client-Repair'
        Write-SectionBanner 'OUTLOOK / EXCHANGE CLIENT'
        $outlook = Get-Process OUTLOOK -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'Outlook running' -Result $(if ($outlook) {"Yes ($($outlook.Count))"} else {'No'}) -Status 'INFO' -Detail ''

        if ($Name -match 'Safe-Mode' -and $PSCmdlet.ShouldProcess('Outlook', 'Launch /safe')) {
            $exe = @(
                "$env:ProgramFiles\Microsoft Office\root\Office16\OUTLOOK.EXE",
                "${env:ProgramFiles(x86)}\Microsoft Office\root\Office16\OUTLOOK.EXE"
            ) | Where-Object { Test-Path $_ } | Select-Object -First 1
            if ($exe) {
                Start-Process $exe -ArgumentList '/safe'
                Add-ReportRow -Check 'Outlook /safe' -Result 'Launched' -Status 'PASS' -Detail $exe
            } else {
                Add-ReportRow -Check 'Outlook.exe' -Result 'Not found' -Status 'FAIL' -Detail ''
            }
        }

        if ($Name -match 'Add-in') {
            $addinKey = 'HKCU:\Software\Microsoft\Office\16.0\Outlook\AddinLoadTimes'
            $disabled = 'HKCU:\Software\Microsoft\Office\16.0\Outlook\Resiliency\DisabledItems'
            Backup-RegistryKey -KeyPath 'HKCU:\Software\Microsoft\Office\16.0\Outlook' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            if (Test-Path $disabled) {
                Add-ReportRow -Check 'DisabledItems' -Result 'Present' -Status 'WARN' -Detail $disabled
                if ($PSCmdlet.ShouldProcess('Outlook resiliency', 'Clear DisabledItems')) {
                    Remove-Item $disabled -Recurse -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Clear DisabledItems' -Result 'Cleared' -Status 'PASS' -Detail 'Rollback via backup'
                }
            } else {
                Add-ReportRow -Check 'DisabledItems' -Result 'None' -Status 'PASS' -Detail ''
            }
        }

        if ($Name -match 'NK2|Autocomplete') {
            $nk2 = Get-ChildItem "$env:APPDATA\Microsoft\Outlook" -Filter *.nk2 -ErrorAction SilentlyContinue
            $autocomplete = Get-ChildItem "$env:LOCALAPPDATA\Microsoft\Outlook" -Filter Stream_Autocomplete* -ErrorAction SilentlyContinue
            if ($PSCmdlet.ShouldProcess('Autocomplete', 'Rename NK2/autocomplete caches')) {
                foreach ($f in @($nk2 + $autocomplete)) {
                    Rename-Item $f.FullName ($f.FullName + '.bak_' + (Get-Date -Format 'yyyyMMdd')) -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Reset autocomplete' -Result $f.Name -Status 'PASS' -Detail 'Renamed to .bak'
                }
                if (-not $nk2 -and -not $autocomplete) {
                    Add-ReportRow -Check 'Autocomplete cache' -Result 'Not found' -Status 'INFO' -Detail 'RoamCache may use different path'
                }
            }
        }

        if ($Name -match 'Search-Index') {
            if ($PSCmdlet.ShouldProcess('WSearch', 'Rebuild Outlook search via Windows Search restart')) {
                Restart-Service WSearch -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'WSearch restart' -Result (Get-Service WSearch).Status -Status 'PASS' -Detail 'Re-index may take time'
            }
        }

        if ($Name -match 'Autodiscover|Exchange-Online|Connectivity|Modern-Auth') {
            $urls = @('https://outlook.office365.com','https://autodiscover-s.outlook.com/autodiscover/autodiscover.svc','https://login.microsoftonline.com')
            foreach ($u in $urls) {
                try {
                    $r = Invoke-WebRequest -Uri $u -UseBasicParsing -TimeoutSec 15 -Method Head
                    Add-ReportRow -Check $u -Result $r.StatusCode -Status $(if ($r.StatusCode -lt 500) {'PASS'} else {'WARN'}) -Detail 'HEAD'
                } catch {
                    Add-ReportRow -Check $u -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
            }
            if ($Name -match 'Modern-Auth|Token' -and $PSCmdlet.ShouldProcess('AAD token cache', 'Hint clear Identity cache')) {
                Add-ReportRow -Check 'Modern auth' -Result 'Sign out Office apps / clear TokenBroker if needed' -Status 'INFO' -Detail "$env:LOCALAPPDATA\Microsoft\TokenBroker\Cache"
            }
        }

        if ($Name -match 'PST|OST|Profile-Export|Cached-Mode|Rules|Calendar|Shared|Delegate|MAPI|SMIME') {
            $mail = Get-ChildItem "$env:LOCALAPPDATA\Microsoft\Outlook" -ErrorAction SilentlyContinue
            $ost = @($mail | Where-Object { $_.Extension -eq '.ost' })
            $pst = @(Get-ChildItem "$env:USERPROFILE" -Filter *.pst -Recurse -ErrorAction SilentlyContinue | Select-Object -First 10)
            foreach ($o in $ost) {
                Add-ReportRow -Check 'OST' -Result ("{0:N1} MB" -f ($o.Length/1MB)) -Status 'INFO' -Detail $o.FullName
            }
            foreach ($p in $pst) {
                Add-ReportRow -Check 'PST' -Result ("{0:N1} MB" -f ($p.Length/1MB)) -Status 'INFO' -Detail $p.FullName
            }
            $scanpst = Get-ChildItem "$env:ProgramFiles*\Microsoft Office\*\Office*\SCANPST.EXE" -ErrorAction SilentlyContinue | Select-Object -First 1
            Add-ReportRow -Check 'SCANPST' -Result $(if ($scanpst) {$scanpst.FullName} else {'Not found'}) -Status 'INFO' -Detail 'Run manually against PST/OST'
            if ($Name -match 'Profile-Export') {
                $dest = Join-Path 'C:\IT-Logs' ("OutlookProfile_{0}.reg" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
                if ($PSCmdlet.ShouldProcess('Outlook profile', 'Export HKCU Outlook profiles')) {
                    & reg.exe export 'HKCU\Software\Microsoft\Office\16.0\Outlook\Profiles' $dest /y 2>$null
                    Add-ReportRow -Check 'Profile export' -Result $(if (Test-Path $dest) {'Saved'} else {'Failed'}) -Status $(if (Test-Path $dest) {'PASS'} else {'WARN'}) -Detail $dest
                }
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Email client'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Exchange Online Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Exchange Online Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
