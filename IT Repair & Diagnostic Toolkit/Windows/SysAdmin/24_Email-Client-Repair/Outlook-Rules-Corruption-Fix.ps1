...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Rules Corruption Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Rules-Corruption-Fix.
...
.EXAMPLE
    .\Outlook-Rules-Corruption-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Rules-Corruption-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Rules Corruption Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Rules-Corruption-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Rules-Corruption-Fix' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Rules Corruption Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Rules Corruption Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
