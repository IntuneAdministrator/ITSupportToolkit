...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 24_Email-Client-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Email-Client-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Email-Client-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Evidence Report'; $totalSteps = 8; $Name = 'Email-Client-Repair-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '24_Email-Client-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Email Client Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
