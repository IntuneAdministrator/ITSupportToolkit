...
<#
.SYNOPSIS
    Common problem fix: Email Client Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Email-Client-Repair-Health-Triage.ps1
.EXAMPLE
    .\Email-Client-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Email Client Repair Health Triage'; $totalSteps = 8; $Name = 'Email-Client-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Email Client Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Email Client Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
