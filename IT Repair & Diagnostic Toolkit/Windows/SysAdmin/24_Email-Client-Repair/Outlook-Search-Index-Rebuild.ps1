...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Search Index Rebuild
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Search-Index-Rebuild.
...
.EXAMPLE
    .\Outlook-Search-Index-Rebuild.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Search-Index-Rebuild.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Search Index Rebuild'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Search-Index-Rebuild'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Search-Index-Rebuild' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Search Index Rebuild' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Search Index Rebuild' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
