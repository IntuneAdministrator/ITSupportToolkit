...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Shared Mailbox Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Shared-Mailbox-Repair.
...
.EXAMPLE
    .\Outlook-Shared-Mailbox-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Shared-Mailbox-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Shared Mailbox Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Shared-Mailbox-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Shared-Mailbox-Repair' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Shared Mailbox Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Shared Mailbox Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
