...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook NK2 Autocomplete Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-NK2-Autocomplete-Reset.
...
.EXAMPLE
    .\Outlook-NK2-Autocomplete-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-NK2-Autocomplete-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook NK2 Autocomplete Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-NK2-Autocomplete-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-NK2-Autocomplete-Reset' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook NK2 Autocomplete Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook NK2 Autocomplete Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
