...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Cached Mode Toggle
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Cached-Mode-Toggle.
...
.EXAMPLE
    .\Outlook-Cached-Mode-Toggle.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Cached-Mode-Toggle.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Cached Mode Toggle'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Cached-Mode-Toggle'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Cached-Mode-Toggle' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Cached Mode Toggle' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Cached Mode Toggle' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
