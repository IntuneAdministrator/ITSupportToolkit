...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Outlook Add in Disable All
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Outlook-Add-in-Disable-All.
...
.EXAMPLE
    .\Outlook-Add-in-Disable-All.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Outlook-Add-in-Disable-All.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Outlook Add in Disable All'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Outlook-Add-in-Disable-All'
        Add-ReportRow -Check 'Tool' -Result 'Outlook-Add-in-Disable-All' -Status 'INFO' -Detail '24_Email-Client-Repair'
    ...
    Export-HtmlReport -Title 'Outlook Add in Disable All' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Outlook Add in Disable All' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
