...
<#
.SYNOPSIS
    Enterprise IT tool: Cloud PC Connectivity Test
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-PC-Connectivity-Test.ps1
.EXAMPLE
    .\Cloud-PC-Connectivity-Test.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud PC Connectivity Test'; $totalSteps = 8; $Name = 'Cloud-PC-Connectivity-Test'
try {
    ...
    Export-HtmlReport -Title 'Cloud PC Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud PC Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
