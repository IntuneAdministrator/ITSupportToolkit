...
<#
.SYNOPSIS
    Enterprise IT tool: RDP Graphics Cloud PC Fix
.DESCRIPTION
...
.EXAMPLE
    .\RDP-Graphics-Cloud-PC-Fix.ps1
.EXAMPLE
    .\RDP-Graphics-Cloud-PC-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'RDP Graphics Cloud PC Fix'; $totalSteps = 8; $Name = 'RDP-Graphics-Cloud-PC-Fix'
try {
    ...
    Export-HtmlReport -Title 'RDP Graphics Cloud PC Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'RDP Graphics Cloud PC Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
