...
<#
.SYNOPSIS
    Common problem fix: Windows 365 Cloud PC Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 39_Windows-365-Cloud-PC). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Windows-365-Cloud-PC-Evidence-Report.ps1
.EXAMPLE
    .\Windows-365-Cloud-PC-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows 365 Cloud PC Evidence Report'; $totalSteps = 8; $Name = 'Windows-365-Cloud-PC-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '39_Windows-365-Cloud-PC' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '39_Windows-365-Cloud-PC'
    ...
    Export-HtmlReport -Title 'Windows 365 Cloud PC Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows 365 Cloud PC Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
