...
<#
.SYNOPSIS
    Enterprise IT tool: W365 Remote Desktop Client Repair
.DESCRIPTION
    Expansion module (SysAdmin / 39_Windows-365-Cloud-PC) for complete IT operations coverage.
    Join-state aware, Intune-compatible, HTML report + transcript logging.
.EXAMPLE
    .\W365-Remote-Desktop-Client-Repair.ps1
.EXAMPLE
    .\W365-Remote-Desktop-Client-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'W365 Remote Desktop Client Repair'; $totalSteps = 8; $Name = 'W365-Remote-Desktop-Client-Repair'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '39_Windows-365-Cloud-PC' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'WINDOWS 365 / AVD / FSLOGIX'
        $msrdc = Get-ChildItem "$env:LOCALAPPDATA\Apps\Remote Desktop" -Filter msrdcw.exe -Recurse -EA SilentlyContinue | Select-Object -First 1
        Add-ReportRow -Check 'Remote Desktop client' -Result $(if ($msrdc) {'Found'} else {'Not found'}) -Status $(if ($msrdc) {'PASS'} else {'WARN'}) -Detail $msrdc.FullName
        Write-ProgressStep 2 $totalSteps $Activity 'W365 client'
        foreach ($u in @('https://rdweb.wvd.microsoft.com','https://windows365.microsoft.com','https://login.microsoftonline.com')) {
            try { $r = Invoke-WebRequest -Uri $u -UseBasicParsing -Method Head -TimeoutSec 15; Add-ReportRow -Check $u -Result $r.StatusCode -Status 'PASS' -Detail '' }
            catch { Add-ReportRow -Check $u -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message }
        }
        if ($Name -match 'FSLogix') {
            $fx = Get-Service frxsvc -EA SilentlyContinue
            Add-ReportRow -Check 'FSLogix frxsvc' -Result $(if ($fx) { $fx.Status } else {'Not installed'}) -Status 'INFO' -Detail ''
            $fxLog = 'C:\ProgramData\FSLogix\Logs'
            Add-ReportRow -Check 'FSLogix logs' -Result $(if (Test-Path $fxLog) {'Present'} else {'N/A'}) -Status 'INFO' -Detail $fxLog
        }
        if ($Name -match 'Repair|Feed-Reset|SSO' -and $PSCmdlet.ShouldProcess('MSRDC', 'Reset AVD/W365 feed cache')) {
            Get-Process msrdc,msrdcw -EA SilentlyContinue | Stop-Process -Force -EA SilentlyContinue
            $feed = "$env:LOCALAPPDATA\Temp\DiagOutputDir"
            Add-ReportRow -Check 'Client reset' -Result 'Processes stopped; reopen Remote Desktop app' -Status 'PASS' -Detail ''
        }
        if ($Name -match 'Shortpath|UDP|Graphics') {
            Add-ReportRow -Check 'RDP Shortpath' -Result 'Verify UDP 3390 / STUN path on network' -Status 'INFO' -Detail 'Cloud PC networking'
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Cloud PC done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'W365 Remote Desktop Client Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'W365 Remote Desktop Client Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
