...
<#
.SYNOPSIS
    Enterprise IT tool: Cloud PC Shortpath UDP Check
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-PC-Shortpath-UDP-Check.ps1
.EXAMPLE
    .\Cloud-PC-Shortpath-UDP-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud PC Shortpath UDP Check'; $totalSteps = 8; $Name = 'Cloud-PC-Shortpath-UDP-Check'
try {
    ...
    Export-HtmlReport -Title 'Cloud PC Shortpath UDP Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud PC Shortpath UDP Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
