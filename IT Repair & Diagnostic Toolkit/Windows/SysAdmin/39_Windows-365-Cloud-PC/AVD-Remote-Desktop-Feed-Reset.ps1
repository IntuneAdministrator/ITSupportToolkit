...
<#
.SYNOPSIS
    Enterprise IT tool: AVD Remote Desktop Feed Reset
.DESCRIPTION
...
.EXAMPLE
    .\AVD-Remote-Desktop-Feed-Reset.ps1
.EXAMPLE
    .\AVD-Remote-Desktop-Feed-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AVD Remote Desktop Feed Reset'; $totalSteps = 8; $Name = 'AVD-Remote-Desktop-Feed-Reset'
try {
    ...
    Export-HtmlReport -Title 'AVD Remote Desktop Feed Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AVD Remote Desktop Feed Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
