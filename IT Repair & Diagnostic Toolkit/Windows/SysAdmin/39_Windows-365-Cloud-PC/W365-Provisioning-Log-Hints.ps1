...
<#
.SYNOPSIS
    Enterprise IT tool: W365 Provisioning Log Hints
.DESCRIPTION
...
.EXAMPLE
    .\W365-Provisioning-Log-Hints.ps1
.EXAMPLE
    .\W365-Provisioning-Log-Hints.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'W365 Provisioning Log Hints'; $totalSteps = 8; $Name = 'W365-Provisioning-Log-Hints'
try {
    ...
    Export-HtmlReport -Title 'W365 Provisioning Log Hints' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'W365 Provisioning Log Hints' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
