...
<#
.SYNOPSIS
    Enterprise IT tool: FSLogix Profile Health Check
.DESCRIPTION
...
.EXAMPLE
    .\FSLogix-Profile-Health-Check.ps1
.EXAMPLE
    .\FSLogix-Profile-Health-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'FSLogix Profile Health Check'; $totalSteps = 8; $Name = 'FSLogix-Profile-Health-Check'
try {
    ...
    Export-HtmlReport -Title 'FSLogix Profile Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'FSLogix Profile Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
