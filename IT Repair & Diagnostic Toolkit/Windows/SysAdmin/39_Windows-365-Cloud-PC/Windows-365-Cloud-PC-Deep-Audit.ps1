...
<#
.SYNOPSIS
    Common problem fix: Windows 365 Cloud PC Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Windows-365-Cloud-PC-Deep-Audit.ps1
.EXAMPLE
    .\Windows-365-Cloud-PC-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Windows 365 Cloud PC Deep Audit'; $totalSteps = 8; $Name = 'Windows-365-Cloud-PC-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Windows 365 Cloud PC Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Windows 365 Cloud PC Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
