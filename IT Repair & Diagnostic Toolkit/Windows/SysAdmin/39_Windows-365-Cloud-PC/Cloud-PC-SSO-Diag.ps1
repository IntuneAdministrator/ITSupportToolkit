...
<#
.SYNOPSIS
    Enterprise IT tool: Cloud PC SSO Diag
.DESCRIPTION
...
.EXAMPLE
    .\Cloud-PC-SSO-Diag.ps1
.EXAMPLE
    .\Cloud-PC-SSO-Diag.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Cloud PC SSO Diag'; $totalSteps = 8; $Name = 'Cloud-PC-SSO-Diag'
try {
    ...
    Export-HtmlReport -Title 'Cloud PC SSO Diag' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Cloud PC SSO Diag' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
