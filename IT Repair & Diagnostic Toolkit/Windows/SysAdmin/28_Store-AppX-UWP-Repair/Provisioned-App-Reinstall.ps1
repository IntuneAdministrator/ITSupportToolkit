...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Provisioned App Reinstall
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Provisioned-App-Reinstall.
...
.EXAMPLE
    .\Provisioned-App-Reinstall.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Provisioned-App-Reinstall.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Provisioned App Reinstall'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Provisioned-App-Reinstall'
        Add-ReportRow -Check 'Tool' -Result 'Provisioned-App-Reinstall' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'Provisioned App Reinstall' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Provisioned App Reinstall' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
