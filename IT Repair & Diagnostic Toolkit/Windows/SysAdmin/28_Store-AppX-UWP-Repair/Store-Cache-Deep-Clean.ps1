...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Store Cache Deep Clean
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Store-Cache-Deep-Clean.
...
$exitCode = 0
$Activity = 'Store Cache Deep Clean'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Store-Cache-Deep-Clean'
        Add-ReportRow -Check 'Tool' -Result 'Store-Cache-Deep-Clean' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'Store Cache Deep Clean' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Store Cache Deep Clean' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
