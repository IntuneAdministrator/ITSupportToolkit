...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: UWP Background Task Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for UWP-Background-Task-Audit.
...
.EXAMPLE
    .\UWP-Background-Task-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\UWP-Background-Task-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'UWP Background Task Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'UWP-Background-Task-Audit'
        Add-ReportRow -Check 'Tool' -Result 'UWP-Background-Task-Audit' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'UWP Background Task Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'UWP Background Task Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
