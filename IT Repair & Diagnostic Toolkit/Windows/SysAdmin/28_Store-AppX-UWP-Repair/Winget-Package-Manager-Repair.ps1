...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Winget Package Manager Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Winget-Package-Manager-Repair.
...
.EXAMPLE
    .\Winget-Package-Manager-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Winget-Package-Manager-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Winget Package Manager Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Winget-Package-Manager-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Winget-Package-Manager-Repair' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'Winget Package Manager Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Winget Package Manager Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
