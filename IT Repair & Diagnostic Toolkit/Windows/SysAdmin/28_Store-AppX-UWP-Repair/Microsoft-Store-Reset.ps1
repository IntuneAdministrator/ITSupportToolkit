...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Microsoft Store Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Microsoft-Store-Reset.
...
$exitCode = 0
$Activity = 'Microsoft Store Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Microsoft-Store-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Microsoft-Store-Reset' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'Microsoft Store Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Microsoft Store Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
