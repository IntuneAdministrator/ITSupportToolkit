...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Sideload Policy Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Sideload-Policy-Enable.
...
$exitCode = 0
$Activity = 'Sideload Policy Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Sideload-Policy-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Sideload-Policy-Enable' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'Sideload Policy Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Sideload Policy Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
