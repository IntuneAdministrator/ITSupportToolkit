...
<#
.SYNOPSIS
    Common problem fix: Store AppX UWP Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Store-AppX-UWP-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Store-AppX-UWP-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Store AppX UWP Repair Quick Fix'; $totalSteps = 8; $Name = 'Store-AppX-UWP-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Store AppX UWP Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Store AppX UWP Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
