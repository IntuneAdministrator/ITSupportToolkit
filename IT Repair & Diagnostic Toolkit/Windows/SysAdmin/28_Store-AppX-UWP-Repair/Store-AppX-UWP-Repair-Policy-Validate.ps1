...
<#
.SYNOPSIS
    Common problem fix: Store AppX UWP Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Store-AppX-UWP-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Store-AppX-UWP-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Store AppX UWP Repair Policy Validate'; $totalSteps = 8; $Name = 'Store-AppX-UWP-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Store AppX UWP Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Store AppX UWP Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
