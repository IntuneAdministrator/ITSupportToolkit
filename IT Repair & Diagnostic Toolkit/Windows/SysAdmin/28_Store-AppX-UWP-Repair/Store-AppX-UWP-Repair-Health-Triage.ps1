...
<#
.SYNOPSIS
    Common problem fix: Store AppX UWP Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Store-AppX-UWP-Repair-Health-Triage.ps1
.EXAMPLE
    .\Store-AppX-UWP-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Store AppX UWP Repair Health Triage'; $totalSteps = 8; $Name = 'Store-AppX-UWP-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Store AppX UWP Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Store AppX UWP Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
