...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Sandbox Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Sandbox-Enable.
...
$exitCode = 0
$Activity = 'Windows Sandbox Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Sandbox-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Sandbox-Enable' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'Windows Sandbox Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Sandbox Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
