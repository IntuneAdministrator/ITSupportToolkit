...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AppX Manifest Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AppX-Manifest-Repair.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AppX-Manifest-Repair'
        Add-ReportRow -Check 'Tool' -Result 'AppX-Manifest-Repair' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'AppX Manifest Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AppX Manifest Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
