...
<#
.SYNOPSIS
    Common problem fix: Store AppX UWP Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Store-AppX-UWP-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Store-AppX-UWP-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Store AppX UWP Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Store-AppX-UWP-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Store AppX UWP Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Store AppX UWP Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
