...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AppX Deployment Service Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AppX-Deployment-Service-Repair.
...
.EXAMPLE
    .\AppX-Deployment-Service-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\AppX-Deployment-Service-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'AppX Deployment Service Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AppX-Deployment-Service-Repair'
        Add-ReportRow -Check 'Tool' -Result 'AppX-Deployment-Service-Repair' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
    ...
    Export-HtmlReport -Title 'AppX Deployment Service Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AppX Deployment Service Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
