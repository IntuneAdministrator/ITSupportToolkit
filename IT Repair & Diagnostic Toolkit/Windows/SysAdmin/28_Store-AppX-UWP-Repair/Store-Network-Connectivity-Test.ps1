...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Store Network Connectivity Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Store-Network-Connectivity-Test.
Category: 28_Store-AppX-UWP-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Store-Network-Connectivity-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Store-Network-Connectivity-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Store Network Connectivity Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'STORE APPX UWP REPAIR'
        $Name = 'Store-Network-Connectivity-Test'
        Add-ReportRow -Check 'Tool' -Result 'Store-Network-Connectivity-Test' -Status 'INFO' -Detail '28_Store-AppX-UWP-Repair'
        Write-SectionBanner 'STORE / APPX / UWP'
        $appXSvc = @('AppXSvc','ClipSVC','InstallService','WinHttpAutoProxySvc')
        foreach ($s in $appXSvc) {
            $svc = Get-Service $s -ErrorAction SilentlyContinue
            if ($svc) { Add-ReportRow -Check $s -Result "$($svc.Status)/$($svc.StartType)" -Status 'INFO' -Detail $svc.DisplayName }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Store services'

        if ($Name -match 'Microsoft-Store-Reset|Store-Cache|AppX-Package|UWP-App-Reset|Manifest|License|Deployment') {
            if ($PSCmdlet.ShouldProcess('Store', 'wsreset + AppX re-register sample')) {
                Start-Process wsreset.exe -WindowStyle Hidden -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'wsreset' -Result 'Triggered' -Status 'INFO' -Detail 'Store cache reset'
                try {
                    Get-AppxPackage Microsoft.WindowsStore -ErrorAction SilentlyContinue | ForEach-Object {
                        Add-ReportRow -Check 'Windows Store package' -Result $_.Version -Status 'PASS' -Detail $_.PackageFullName
                    }
                } catch {
                    Add-ReportRow -Check 'Windows Store package' -Result 'Query failed' -Status 'WARN' -Detail $_.Exception.Message
                }
            }
            if ($Name -match 'AppX-Package-Rebuild|UWP-App-Reset' -and $PSCmdlet.ShouldProcess('AppX', 'Re-register Microsoft.WindowsStore')) {
                $manifest = Get-ChildItem "$env:ProgramFiles\WindowsApps\Microsoft.WindowsStore*" -Filter AppxManifest.xml -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
                if ($manifest) {
                    Add-AppxPackage -DisableDevelopmentMode -Register $manifest.FullName -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Re-register Store' -Result 'Attempted' -Status 'PASS' -Detail $manifest.FullName
                } else {
                    Add-ReportRow -Check 'Store manifest' -Result 'Not found' -Status 'WARN' -Detail 'WindowsApps ACL may block enumeration'
                }
            }
        }

        if ($Name -match 'Winget') {
            $winget = Get-Command winget -ErrorAction SilentlyContinue
            if ($winget) {
                $v = & winget --version 2>&1 | Out-String
                Add-ReportRow -Check 'winget' -Result $v.Trim() -Status 'PASS' -Detail $winget.Source
                if ($PSCmdlet.ShouldProcess('winget', 'winget source reset')) {
                    & winget source reset --force 2>&1 | Out-Null
                    Add-ReportRow -Check 'winget source reset' -Result 'Done' -Status 'PASS' -Detail ''
                }
            } else {
                Add-ReportRow -Check 'winget' -Result 'Not found' -Status 'WARN' -Detail 'Install App Installer from Store'
            }
        }

        if ($Name -match 'Sideload|Sandbox|Provisioned|Background|Network') {
            if ($Name -match 'Sideload' -and $PSCmdlet.ShouldProcess('Sideload', 'Enable Developer/Sideload')) {
                Set-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppModelUnlock' -Name AllowAllTrustedApps -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'AllowAllTrustedApps' -Result '1' -Status 'PASS' -Detail 'Sideload trusted apps'
            }
            if ($Name -match 'Sandbox') {
                $sb = Get-WindowsOptionalFeature -Online -FeatureName Containers-DisposableClientVM -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Windows Sandbox feature' -Result $(if ($sb) { $sb.State } else {'N/A'}) -Status 'INFO' -Detail ''
                if ($sb -and $sb.State -ne 'Enabled' -and $PSCmdlet.ShouldProcess('Sandbox', 'Enable feature')) {
                    ...
                    Add-ReportRow -Check 'Enable Sandbox' -Result 'Requested' -Status 'PASS' -Detail 'Reboot may be required'
                }
            }
            if ($Name -match 'Provisioned') {
                $prov = @(Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue | Select-Object -First 15)
                Add-ReportRow -Check 'Provisioned packages' -Result "$($prov.Count) sampled" -Status 'INFO' -Detail (($prov | ForEach-Object DisplayName) -join '; ')
            }
            if ($Name -match 'Network') {
                try {
                    $r = Invoke-WebRequest -Uri 'https://www.microsoft.com/store' -UseBasicParsing -TimeoutSec 15 -Method Head
                    Add-ReportRow -Check 'Store endpoint' -Result $r.StatusCode -Status 'PASS' -Detail 'microsoft.com/store'
                } catch {
                    Add-ReportRow -Check 'Store endpoint' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Store/AppX'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Store Network Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Store Network Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
