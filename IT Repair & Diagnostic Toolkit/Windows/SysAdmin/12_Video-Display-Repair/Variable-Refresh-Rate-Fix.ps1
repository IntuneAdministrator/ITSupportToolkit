...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Variable Refresh Rate Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Variable-Refresh-Rate-Fix.
...
.EXAMPLE
    .\Variable-Refresh-Rate-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Variable-Refresh-Rate-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Variable Refresh Rate Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Variable-Refresh-Rate-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Variable-Refresh-Rate-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Variable Refresh Rate Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Variable Refresh Rate Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
