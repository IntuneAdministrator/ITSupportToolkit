...
<#
.SYNOPSIS
    Common problem fix: Monitor EDID Reset
.DESCRIPTION
...
.EXAMPLE
    .\Monitor-EDID-Reset.ps1
.EXAMPLE
    .\Monitor-EDID-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Monitor EDID Reset'; $totalSteps = 8; $Name = 'Monitor-EDID-Reset'
try {
    ...
    Export-HtmlReport -Title 'Monitor EDID Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Monitor EDID Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
