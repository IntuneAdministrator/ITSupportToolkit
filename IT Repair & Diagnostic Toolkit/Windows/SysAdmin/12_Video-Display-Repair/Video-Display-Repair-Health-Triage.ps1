...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Health-Triage.ps1
.EXAMPLE
    .\Video-Display-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Health Triage'; $totalSteps = 8; $Name = 'Video-Display-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
