...
<#
.SYNOPSIS
    Common problem fix: Docking Dual Display Failure
.DESCRIPTION
...
.EXAMPLE
    .\Docking-Dual-Display-Failure.ps1
.EXAMPLE
    .\Docking-Dual-Display-Failure.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Docking Dual Display Failure'; $totalSteps = 8; $Name = 'Docking-Dual-Display-Failure'
try {
    ...
    Export-HtmlReport -Title 'Docking Dual Display Failure' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Docking Dual Display Failure' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
