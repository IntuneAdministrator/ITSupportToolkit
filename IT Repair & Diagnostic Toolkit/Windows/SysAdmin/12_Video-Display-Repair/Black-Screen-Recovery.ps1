...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Black Screen Recovery
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Black-Screen-Recovery.
...
$exitCode = 0
$Activity = 'Black Screen Recovery'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Black-Screen-Recovery'
        Add-ReportRow -Check 'Tool' -Result 'Black-Screen-Recovery' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Black Screen Recovery' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Black Screen Recovery' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
