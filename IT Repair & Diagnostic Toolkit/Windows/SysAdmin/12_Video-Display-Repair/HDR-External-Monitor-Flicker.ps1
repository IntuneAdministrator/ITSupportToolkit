...
<#
.SYNOPSIS
    Common problem fix: HDR External Monitor Flicker
.DESCRIPTION
...
.EXAMPLE
    .\HDR-External-Monitor-Flicker.ps1
.EXAMPLE
    .\HDR-External-Monitor-Flicker.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HDR External Monitor Flicker'; $totalSteps = 8; $Name = 'HDR-External-Monitor-Flicker'
try {
    ...
    Export-HtmlReport -Title 'HDR External Monitor Flicker' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HDR External Monitor Flicker' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
