...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Display Scaling DPI Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Display-Scaling-DPI-Fix.
...
.EXAMPLE
    .\Display-Scaling-DPI-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Display-Scaling-DPI-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Display Scaling DPI Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Display-Scaling-DPI-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Display-Scaling-DPI-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Display Scaling DPI Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Display Scaling DPI Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
