...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPU Driver DDU Prep
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPU-Driver-DDU-Prep.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'GPU-Driver-DDU-Prep'
        Add-ReportRow -Check 'Tool' -Result 'GPU-Driver-DDU-Prep' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'GPU Driver DDU Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPU Driver DDU Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
