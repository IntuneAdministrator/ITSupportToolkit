...
<#
.SYNOPSIS
    Common problem fix: Multi GPU Workstation Triage
.DESCRIPTION
...
.EXAMPLE
    .\Multi-GPU-Workstation-Triage.ps1
.EXAMPLE
    .\Multi-GPU-Workstation-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Multi GPU Workstation Triage'; $totalSteps = 8; $Name = 'Multi-GPU-Workstation-Triage'
try {
    ...
    Export-HtmlReport -Title 'Multi GPU Workstation Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Multi GPU Workstation Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
