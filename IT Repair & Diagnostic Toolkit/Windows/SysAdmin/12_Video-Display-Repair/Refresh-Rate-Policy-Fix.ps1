...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Refresh Rate Policy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Refresh-Rate-Policy-Fix.
...
.EXAMPLE
    .\Refresh-Rate-Policy-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Refresh-Rate-Policy-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Refresh Rate Policy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Refresh-Rate-Policy-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Refresh-Rate-Policy-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Refresh Rate Policy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Refresh Rate Policy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
