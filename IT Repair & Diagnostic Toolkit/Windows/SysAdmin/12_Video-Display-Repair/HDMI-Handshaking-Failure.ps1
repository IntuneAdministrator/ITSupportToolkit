...
<#
.SYNOPSIS
    Common problem fix: HDMI Handshaking Failure
.DESCRIPTION
...
.EXAMPLE
    .\HDMI-Handshaking-Failure.ps1
.EXAMPLE
    .\HDMI-Handshaking-Failure.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'HDMI Handshaking Failure'; $totalSteps = 8; $Name = 'HDMI-Handshaking-Failure'
try {
    ...
    Export-HtmlReport -Title 'HDMI Handshaking Failure' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'HDMI Handshaking Failure' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
