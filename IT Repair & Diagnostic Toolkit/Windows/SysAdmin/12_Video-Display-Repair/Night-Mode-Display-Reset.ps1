...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Night Mode Display Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Night-Mode-Display-Reset.
...
.EXAMPLE
    .\Night-Mode-Display-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Night-Mode-Display-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Night Mode Display Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Night-Mode-Display-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Night-Mode-Display-Reset' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Night Mode Display Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Night Mode Display Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
