...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DisplayPort HDMI Handshake Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DisplayPort-HDMI-Handshake-Fix.
Category: 12_Video-Display-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\DisplayPort-HDMI-Handshake-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DisplayPort-HDMI-Handshake-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DisplayPort HDMI Handshake Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'VIDEO DISPLAY REPAIR'
        $Name = 'DisplayPort-HDMI-Handshake-Fix'
        Add-ReportRow -Check 'Tool' -Result 'DisplayPort-HDMI-Handshake-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
        Write-SectionBanner 'DISPLAY POLICY / MONITORS'
        try {
            Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue
            $screens = [System.Windows.Forms.Screen]::AllScreens
            foreach ($s in $screens) {
                $b = $s.Bounds
                Add-ReportRow -Check $(if ($s.Primary) {'Primary monitor'} else {'Monitor'}) -Result "$($b.Width)x$($b.Height)" -Status 'INFO' -Detail "X=$($b.X) Y=$($b.Y)"
            }
        } catch {
            Add-ReportRow -Check 'Monitor enum' -Result 'Fallback CIM' -Status 'WARN' -Detail $_.Exception.Message
            Get-CimInstance Win32_DesktopMonitor -ErrorAction SilentlyContinue | ForEach-Object {
                Add-ReportRow -Check $_.Name -Result $_.Status -Status 'INFO' -Detail "ScreenHeight=$($_.ScreenHeight)"
            }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Monitors'

        $dpiKey = 'HKCU:\Control Panel\Desktop'
        Backup-RegistryKey -KeyPath $dpiKey -BackupDir 'C:\IT-Logs\Backups' | Out-Null
        $lse = (Get-ItemProperty $dpiKey -Name LogPixels -ErrorAction SilentlyContinue).LogPixels
        Add-ReportRow -Check 'LogPixels' -Result $(if ($null -eq $lse) {'System default'} else {"$lse"}) -Status 'INFO' -Detail 'DPI related'

        if ($Name -match 'Night-Mode|Brightness') {
            $night = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\CloudStore'
            Add-ReportRow -Check 'Night light / brightness' -Result 'Review Settings > System > Display' -Status 'INFO' -Detail 'Policy keys vary by build'
        }
        if ($Name -match 'Multiple-Monitor|Resolution' -and $PSCmdlet.ShouldProcess('Display', 'Restart Display Policy service')) {
            Restart-Service DispBrokerDesktopSvc -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'DispBrokerDesktopSvc' -Result 'Restarted' -Status 'PASS' -Detail 'Display Policy Service'
        }
        if ($Name -match 'Color-Profile') {
            $profiles = Get-ChildItem "$env:SystemRoot\System32\spool\drivers\color" -ErrorAction SilentlyContinue | Select-Object -First 10
            Add-ReportRow -Check 'Color profiles' -Result "$($profiles.Count)+ files" -Status 'INFO' -Detail (($profiles | ForEach-Object Name) -join ', ')
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Display policy'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'DisplayPort HDMI Handshake Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DisplayPort HDMI Handshake Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
