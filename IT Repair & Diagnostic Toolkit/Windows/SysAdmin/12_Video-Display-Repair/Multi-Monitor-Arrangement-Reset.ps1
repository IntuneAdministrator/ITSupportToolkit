...
<#
.SYNOPSIS
    Common problem fix: Multi Monitor Arrangement Reset
.DESCRIPTION
...
.EXAMPLE
    .\Multi-Monitor-Arrangement-Reset.ps1
.EXAMPLE
    .\Multi-Monitor-Arrangement-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Multi Monitor Arrangement Reset'; $totalSteps = 8; $Name = 'Multi-Monitor-Arrangement-Reset'
try {
    ...
    Export-HtmlReport -Title 'Multi Monitor Arrangement Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Multi Monitor Arrangement Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
