...
<#
.SYNOPSIS
    Common problem fix: NVIDIA Quadro Workstation Audit
.DESCRIPTION
...
.EXAMPLE
    .\NVIDIA-Quadro-Workstation-Audit.ps1
.EXAMPLE
    .\NVIDIA-Quadro-Workstation-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NVIDIA Quadro Workstation Audit'; $totalSteps = 8; $Name = 'NVIDIA-Quadro-Workstation-Audit'
try {
    ...
    Export-HtmlReport -Title 'NVIDIA Quadro Workstation Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NVIDIA Quadro Workstation Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
