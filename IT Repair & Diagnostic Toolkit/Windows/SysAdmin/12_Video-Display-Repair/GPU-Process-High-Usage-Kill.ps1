...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPU Process High Usage Kill
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPU-Process-High-Usage-Kill.
...
.EXAMPLE
    .\GPU-Process-High-Usage-Kill.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\GPU-Process-High-Usage-Kill.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'GPU Process High Usage Kill'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'GPU-Process-High-Usage-Kill'
        Add-ReportRow -Check 'Tool' -Result 'GPU-Process-High-Usage-Kill' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'GPU Process High Usage Kill' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPU Process High Usage Kill' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
