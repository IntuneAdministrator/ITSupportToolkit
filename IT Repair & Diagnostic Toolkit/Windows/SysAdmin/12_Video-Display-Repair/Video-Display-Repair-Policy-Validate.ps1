...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Video-Display-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Policy Validate'; $totalSteps = 8; $Name = 'Video-Display-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
