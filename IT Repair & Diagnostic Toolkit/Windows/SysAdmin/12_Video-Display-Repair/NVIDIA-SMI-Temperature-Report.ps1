...
<#
.SYNOPSIS
    Common problem fix: NVIDIA SMI Temperature Report
.DESCRIPTION
...
.EXAMPLE
    .\NVIDIA-SMI-Temperature-Report.ps1
.EXAMPLE
    .\NVIDIA-SMI-Temperature-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NVIDIA SMI Temperature Report'; $totalSteps = 8; $Name = 'NVIDIA-SMI-Temperature-Report'
try {
    ...
    Export-HtmlReport -Title 'NVIDIA SMI Temperature Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NVIDIA SMI Temperature Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
