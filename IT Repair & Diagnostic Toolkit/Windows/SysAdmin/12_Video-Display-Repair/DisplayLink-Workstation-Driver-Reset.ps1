...
<#
.SYNOPSIS
    Common problem fix: DisplayLink Workstation Driver Reset
.DESCRIPTION
...
.EXAMPLE
    .\DisplayLink-Workstation-Driver-Reset.ps1
.EXAMPLE
    .\DisplayLink-Workstation-Driver-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DisplayLink Workstation Driver Reset'; $totalSteps = 8; $Name = 'DisplayLink-Workstation-Driver-Reset'
try {
    ...
    Export-HtmlReport -Title 'DisplayLink Workstation Driver Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DisplayLink Workstation Driver Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
