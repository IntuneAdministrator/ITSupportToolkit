...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPU Overclock Conflict Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPU-Overclock-Conflict-Detect.
...
.EXAMPLE
    .\GPU-Overclock-Conflict-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\GPU-Overclock-Conflict-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'GPU Overclock Conflict Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'GPU-Overclock-Conflict-Detect'
        Add-ReportRow -Check 'Tool' -Result 'GPU-Overclock-Conflict-Detect' -Status 'INFO' -Detail '12_Video-Display-Repair'
        Write-SectionBanner 'DISPLAY / GPU'
        $gpus = @(Get-CimInstance Win32_VideoController -ErrorAction SilentlyContinue)
        foreach ($g in $gpus) {
            Add-ReportRow -Check $g.Name -Result $g.Status -Status $(if ($g.Status -eq 'OK') {'PASS'} else {'WARN'}) -Detail "Driver=$($g.DriverVersion) Mode=$($g.VideoModeDescription)"
        }
        Write-ProgressStep 2 $totalSteps $Activity 'GPU inventory'

        $disp = @(Get-PnpDevice -Class Display -ErrorAction SilentlyContinue)
        foreach ($d in $disp) {
            Add-ReportRow -Check "PnP $($d.FriendlyName)" -Result $d.Status -Status $(if ($d.Status -eq 'OK') {'PASS'} else {'FAIL'}) -Detail $d.InstanceId
            if ($d.Status -ne 'OK' -and $Name -match 'Driver|Reset|Black-Screen' -and $PSCmdlet.ShouldProcess($d.FriendlyName, 'Disable/Enable display device')) {
                Disable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 2
                Enable-PnpDevice -InstanceId $d.InstanceId -Confirm:$false -ErrorAction SilentlyContinue
                Add-ReportRow -Check "Re-enum $($d.FriendlyName)" -Result 'Toggled' -Status 'PASS' -Detail ''
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Display PnP'

        if ($Name -match 'DirectX') {
            $dx = Join-Path 'C:\IT-Logs' ("dxdiag_{0}.txt" -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
            if ($PSCmdlet.ShouldProcess($dx, 'dxdiag /t export')) {
                Start-Process dxdiag.exe -ArgumentList "/t `"$dx`"" -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'dxdiag export' -Result $(if (Test-Path $dx) {'Saved'} else {'Pending/Failed'}) -Status 'INFO' -Detail $dx
            }
        }
        if ($Name -match 'GPU-Process') {
            Get-Process | Where-Object { $_.ProcessName -match 'dwm|nvidia|amd|intel|graphics' } | Select-Object -First 10 | ForEach-Object {
                Add-ReportRow -Check "GPU-related $($_.ProcessName)" -Result ("WS={0:N0}MB" -f ($_.WorkingSet64/1MB)) -Status 'INFO' -Detail "PID=$($_.Id)"
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'GPU extras'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'GPU Overclock Conflict Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPU Overclock Conflict Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
