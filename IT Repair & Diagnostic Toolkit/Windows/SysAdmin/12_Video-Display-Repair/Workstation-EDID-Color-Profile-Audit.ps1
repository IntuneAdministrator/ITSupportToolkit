...
<#
.SYNOPSIS
    Common problem fix: Workstation EDID Color Profile Audit
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 12_Video-Display-Repair). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Workstation-EDID-Color-Profile-Audit.ps1
.EXAMPLE
    .\Workstation-EDID-Color-Profile-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Workstation EDID Color Profile Audit'; $totalSteps = 8; $Name = 'Workstation-EDID-Color-Profile-Audit'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '12_Video-Display-Repair' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Workstation EDID Color Profile Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Workstation EDID Color Profile Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
