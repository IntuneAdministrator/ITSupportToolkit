...
<#
.SYNOPSIS
    Common problem fix: AMD Radeon Pro Driver Audit
.DESCRIPTION
...
.EXAMPLE
    .\AMD-Radeon-Pro-Driver-Audit.ps1
.EXAMPLE
    .\AMD-Radeon-Pro-Driver-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AMD Radeon Pro Driver Audit'; $totalSteps = 8; $Name = 'AMD-Radeon-Pro-Driver-Audit'
try {
    ...
    Export-HtmlReport -Title 'AMD Radeon Pro Driver Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AMD Radeon Pro Driver Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
