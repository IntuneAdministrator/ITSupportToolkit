...
<#
.SYNOPSIS
    Common problem fix: CUDA Runtime Health Check
.DESCRIPTION
...
.EXAMPLE
    .\CUDA-Runtime-Health-Check.ps1
.EXAMPLE
    .\CUDA-Runtime-Health-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CUDA Runtime Health Check'; $totalSteps = 8; $Name = 'CUDA-Runtime-Health-Check'
try {
    ...
    Export-HtmlReport -Title 'CUDA Runtime Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CUDA Runtime Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
