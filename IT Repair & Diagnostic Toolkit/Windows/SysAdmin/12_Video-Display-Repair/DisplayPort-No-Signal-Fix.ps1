...
<#
.SYNOPSIS
    Common problem fix: DisplayPort No Signal Fix
.DESCRIPTION
...
.EXAMPLE
    .\DisplayPort-No-Signal-Fix.ps1
.EXAMPLE
    .\DisplayPort-No-Signal-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DisplayPort No Signal Fix'; $totalSteps = 8; $Name = 'DisplayPort-No-Signal-Fix'
try {
    ...
    Export-HtmlReport -Title 'DisplayPort No Signal Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DisplayPort No Signal Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
