...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Video-Display-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Quick Fix'; $totalSteps = 8; $Name = 'Video-Display-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
