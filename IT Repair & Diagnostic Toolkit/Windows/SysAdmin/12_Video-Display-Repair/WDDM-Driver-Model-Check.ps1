...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WDDM Driver Model Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WDDM-Driver-Model-Check.
...
.EXAMPLE
    .\WDDM-Driver-Model-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WDDM-Driver-Model-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WDDM Driver Model Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WDDM-Driver-Model-Check'
        Add-ReportRow -Check 'Tool' -Result 'WDDM-Driver-Model-Check' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'WDDM Driver Model Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WDDM Driver Model Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
