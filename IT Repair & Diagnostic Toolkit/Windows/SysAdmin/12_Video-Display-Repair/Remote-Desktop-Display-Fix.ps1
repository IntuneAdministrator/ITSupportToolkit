...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Remote Desktop Display Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Remote-Desktop-Display-Fix.
...
.EXAMPLE
    .\Remote-Desktop-Display-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Remote-Desktop-Display-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Remote Desktop Display Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Remote-Desktop-Display-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Remote-Desktop-Display-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Remote Desktop Display Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Remote Desktop Display Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
