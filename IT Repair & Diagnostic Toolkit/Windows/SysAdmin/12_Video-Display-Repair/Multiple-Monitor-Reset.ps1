...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Multiple Monitor Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Multiple-Monitor-Reset.
...
$exitCode = 0
$Activity = 'Multiple Monitor Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Multiple-Monitor-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Multiple-Monitor-Reset' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Multiple Monitor Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Multiple Monitor Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
