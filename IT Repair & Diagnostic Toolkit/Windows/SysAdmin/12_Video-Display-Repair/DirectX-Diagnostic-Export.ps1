...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DirectX Diagnostic Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DirectX-Diagnostic-Export.
...
.EXAMPLE
    .\DirectX-Diagnostic-Export.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DirectX-Diagnostic-Export.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DirectX Diagnostic Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'DirectX-Diagnostic-Export'
        Add-ReportRow -Check 'Tool' -Result 'DirectX-Diagnostic-Export' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'DirectX Diagnostic Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DirectX Diagnostic Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
