...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Video-Display-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Video-Display-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
