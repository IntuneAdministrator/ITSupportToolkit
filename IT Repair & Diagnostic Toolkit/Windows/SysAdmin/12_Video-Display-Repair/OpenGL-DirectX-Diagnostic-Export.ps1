...
<#
.SYNOPSIS
    Common problem fix: OpenGL DirectX Diagnostic Export
.DESCRIPTION
...
.EXAMPLE
    .\OpenGL-DirectX-Diagnostic-Export.ps1
.EXAMPLE
    .\OpenGL-DirectX-Diagnostic-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'OpenGL DirectX Diagnostic Export'; $totalSteps = 8; $Name = 'OpenGL-DirectX-Diagnostic-Export'
try {
    ...
    Export-HtmlReport -Title 'OpenGL DirectX Diagnostic Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'OpenGL DirectX Diagnostic Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
