...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Video-Display-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Evidence Report'; $totalSteps = 8; $Name = 'Video-Display-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
