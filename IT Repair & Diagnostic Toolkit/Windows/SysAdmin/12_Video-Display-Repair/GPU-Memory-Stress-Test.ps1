...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: GPU Memory Stress Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for GPU-Memory-Stress-Test.
...
$exitCode = 0
$Activity = 'GPU Memory Stress Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'GPU-Memory-Stress-Test'
        Add-ReportRow -Check 'Tool' -Result 'GPU-Memory-Stress-Test' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'GPU Memory Stress Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'GPU Memory Stress Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
