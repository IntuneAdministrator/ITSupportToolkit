...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Video-Display-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Auto Repair'; $totalSteps = 8; $Name = 'Video-Display-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
