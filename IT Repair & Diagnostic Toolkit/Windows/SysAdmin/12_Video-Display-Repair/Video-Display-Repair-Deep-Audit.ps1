...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Video-Display-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Deep Audit'; $totalSteps = 8; $Name = 'Video-Display-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
