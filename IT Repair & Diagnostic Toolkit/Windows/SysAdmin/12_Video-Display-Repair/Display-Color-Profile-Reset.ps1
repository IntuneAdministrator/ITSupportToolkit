...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Display Color Profile Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Display-Color-Profile-Reset.
...
.EXAMPLE
    .\Display-Color-Profile-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Display-Color-Profile-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Display Color Profile Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Display-Color-Profile-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Display-Color-Profile-Reset' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Display Color Profile Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Display Color Profile Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
