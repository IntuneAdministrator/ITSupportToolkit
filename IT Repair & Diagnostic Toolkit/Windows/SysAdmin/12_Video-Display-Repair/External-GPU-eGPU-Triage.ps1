...
<#
.SYNOPSIS
    Common problem fix: External GPU eGPU Triage
.DESCRIPTION
...
.EXAMPLE
    .\External-GPU-eGPU-Triage.ps1
.EXAMPLE
    .\External-GPU-eGPU-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'External GPU eGPU Triage'; $totalSteps = 8; $Name = 'External-GPU-eGPU-Triage'
try {
    ...
    Export-HtmlReport -Title 'External GPU eGPU Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'External GPU eGPU Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
