...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Resolution Policy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Resolution-Policy-Fix.
...
$exitCode = 0
$Activity = 'Resolution Policy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Resolution-Policy-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Resolution-Policy-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Resolution Policy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Resolution Policy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
