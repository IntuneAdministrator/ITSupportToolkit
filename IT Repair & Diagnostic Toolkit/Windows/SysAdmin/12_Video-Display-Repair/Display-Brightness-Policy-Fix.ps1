...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Display Brightness Policy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Display-Brightness-Policy-Fix.
...
.EXAMPLE
    .\Display-Brightness-Policy-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Display-Brightness-Policy-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Display Brightness Policy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Display-Brightness-Policy-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Display-Brightness-Policy-Fix' -Status 'INFO' -Detail '12_Video-Display-Repair'
    ...
    Export-HtmlReport -Title 'Display Brightness Policy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Display Brightness Policy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
