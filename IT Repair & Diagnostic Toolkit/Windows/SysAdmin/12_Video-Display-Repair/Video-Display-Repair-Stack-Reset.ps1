...
<#
.SYNOPSIS
    Common problem fix: Video Display Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Video-Display-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Video-Display-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Video Display Repair Stack Reset'; $totalSteps = 8; $Name = 'Video-Display-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Video Display Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Video Display Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
