...
<#
.SYNOPSIS
    Enterprise IT tool: Hardware RTC Drift Hint
.DESCRIPTION
    Phase 11 expansion (SysAdmin/54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Hardware-RTC-Drift-Hint.ps1
.EXAMPLE
    .\Hardware-RTC-Drift-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Hardware RTC Drift Hint'; $totalSteps=8; $Name='Hardware-RTC-Drift-Hint'; $Folder='54_Time-Sync-NTP-Deep'
try {
    ...
    Export-HtmlReport -Title 'Hardware RTC Drift Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Hardware RTC Drift Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
