...
<#
.SYNOPSIS
    Enterprise IT tool: NTP Stratum Peer Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\NTP-Stratum-Peer-Check.ps1
.EXAMPLE
    .\NTP-Stratum-Peer-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='NTP Stratum Peer Check'; $totalSteps=8; $Name='NTP-Stratum-Peer-Check'; $Folder='54_Time-Sync-NTP-Deep'
try {
    ...
    Export-HtmlReport -Title 'NTP Stratum Peer Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NTP Stratum Peer Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
