...
<#
.SYNOPSIS
    Enterprise IT tool: Domain Time Skew Detect
.DESCRIPTION
    Phase 11 expansion (SysAdmin/54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Domain-Time-Skew-Detect.ps1
.EXAMPLE
    .\Domain-Time-Skew-Detect.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Domain Time Skew Detect'; $totalSteps=8; $Name='Domain-Time-Skew-Detect'; $Folder='54_Time-Sync-NTP-Deep'
try {
    ...
    Export-HtmlReport -Title 'Domain Time Skew Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Domain Time Skew Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
