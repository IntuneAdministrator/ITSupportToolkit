...
<#
.SYNOPSIS
    Enterprise IT tool: VMICTimeProvider HyperV Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\VMICTimeProvider-HyperV-Check.ps1
.EXAMPLE
    .\VMICTimeProvider-HyperV-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='VMICTimeProvider HyperV Check'; $totalSteps=8; $Name='VMICTimeProvider-HyperV-Check'; $Folder='54_Time-Sync-NTP-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '54_Time-Sync-NTP-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'TIME SYNC / NTP DEEP'
        Write-ProgressStep 2 $totalSteps $Activity 'w32time'
        $w32 = Get-Service W32Time -EA SilentlyContinue
        Add-ReportRow -Check 'W32Time' -Result $(if($w32){"$($w32.Status)"}else{'N/A'}) -Status $(if($w32 -and $w32.Status -eq 'Running'){'PASS'}else{'FAIL'}) -Detail ''
        $src = w32tm /query /source 2>&1 | Out-String
        Add-ReportRow -Check 'Time source' -Result $src.Trim() -Status 'INFO' -Detail ''
        if ($Name -match 'Resync|Force') {
            if ($PSCmdlet.ShouldProcess('W32Time', 'resync')) {
                w32tm /resync /force 2>&1 | Out-Null
                Add-ReportRow -Check 'w32tm /resync' -Result 'Forced' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'Skew|Domain') {
            $st = w32tm /query /status 2>&1 | Out-String
            Add-ReportRow -Check 'w32tm status' -Result 'Captured' -Status 'INFO' -Detail $st.Substring(0,[Math]::Min(400,$st.Length))
        }
        if ($Name -match 'Timezone|DST') {
            $tz = Get-TimeZone
            Add-ReportRow -Check 'Timezone' -Result $tz.Id -Status 'INFO' -Detail $tz.DisplayName
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Time done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'VMICTimeProvider HyperV Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'VMICTimeProvider HyperV Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
