...
<#
.SYNOPSIS
    Common problem fix: Time Sync NTP Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Time-Sync-NTP-Deep-Quick-Fix.ps1
.EXAMPLE
    .\Time-Sync-NTP-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time Sync NTP Deep Quick Fix'; $totalSteps = 8; $Name = 'Time-Sync-NTP-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Time Sync NTP Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time Sync NTP Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
