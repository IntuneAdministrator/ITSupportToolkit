...
<#
.SYNOPSIS
    Enterprise IT tool: Timezone DST Policy Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Timezone-DST-Policy-Audit.ps1
.EXAMPLE
    .\Timezone-DST-Policy-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Timezone DST Policy Audit'; $totalSteps=8; $Name='Timezone-DST-Policy-Audit'; $Folder='54_Time-Sync-NTP-Deep'
try {
    ...
    Export-HtmlReport -Title 'Timezone DST Policy Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Timezone DST Policy Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
