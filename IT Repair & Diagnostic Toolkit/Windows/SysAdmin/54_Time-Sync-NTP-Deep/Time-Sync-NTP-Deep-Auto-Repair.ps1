...
<#
.SYNOPSIS
    Common problem fix: Time Sync NTP Deep Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Time-Sync-NTP-Deep-Auto-Repair.ps1
.EXAMPLE
    .\Time-Sync-NTP-Deep-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time Sync NTP Deep Auto Repair'; $totalSteps = 8; $Name = 'Time-Sync-NTP-Deep-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Time Sync NTP Deep Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time Sync NTP Deep Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
