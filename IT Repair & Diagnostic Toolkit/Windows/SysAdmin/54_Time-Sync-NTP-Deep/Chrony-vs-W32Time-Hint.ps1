...
<#
.SYNOPSIS
    Enterprise IT tool: Chrony vs W32Time Hint
.DESCRIPTION
    Phase 11 expansion (SysAdmin/54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Chrony-vs-W32Time-Hint.ps1
.EXAMPLE
    .\Chrony-vs-W32Time-Hint.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Chrony vs W32Time Hint'; $totalSteps=8; $Name='Chrony-vs-W32Time-Hint'; $Folder='54_Time-Sync-NTP-Deep'
try {
    ...
    Export-HtmlReport -Title 'Chrony vs W32Time Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Chrony vs W32Time Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
