...
<#
.SYNOPSIS
    Common problem fix: Time Sync NTP Deep Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Time-Sync-NTP-Deep-Evidence-Report.ps1
.EXAMPLE
    .\Time-Sync-NTP-Deep-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time Sync NTP Deep Evidence Report'; $totalSteps = 8; $Name = 'Time-Sync-NTP-Deep-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Time Sync NTP Deep Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time Sync NTP Deep Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
