...
<#
.SYNOPSIS
    Common problem fix: Time Sync NTP Deep Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 54_Time-Sync-NTP-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Time-Sync-NTP-Deep-Policy-Validate.ps1
.EXAMPLE
    .\Time-Sync-NTP-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time Sync NTP Deep Policy Validate'; $totalSteps = 8; $Name = 'Time-Sync-NTP-Deep-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '54_Time-Sync-NTP-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '54_Time-Sync-NTP-Deep'
    ...
    Export-HtmlReport -Title 'Time Sync NTP Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time Sync NTP Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
