...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Event Log Size Policy Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Event-Log-Size-Policy-Enforce.
...
.EXAMPLE
    .\Event-Log-Size-Policy-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Event-Log-Size-Policy-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Event Log Size Policy Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Event-Log-Size-Policy-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'Event-Log-Size-Policy-Enforce' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Event Log Size Policy Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Event Log Size Policy Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
