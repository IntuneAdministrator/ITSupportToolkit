...
<#
.SYNOPSIS
    Common problem fix: Application Crash 7Day Report
.DESCRIPTION
...
.EXAMPLE
    .\Application-Crash-7Day-Report
.EXAMPLE
    .\Application-Crash-7Day-Report -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Application Crash 7Day Report'; $totalSteps = 8; $Name = 'Application-Crash-7Day-Report'
try {
    ...
    Export-HtmlReport -Title 'Application Crash 7Day Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Application Crash 7Day Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
