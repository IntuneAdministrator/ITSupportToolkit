...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Critical Event Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Critical-Event-Report.
...
$exitCode = 0
$Activity = 'Critical Event Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Critical-Event-Report'
        Add-ReportRow -Check 'Tool' -Result 'Critical-Event-Report' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Critical Event Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Critical Event Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
