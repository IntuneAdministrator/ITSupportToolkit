...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Object Access Anomaly Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Object-Access-Anomaly-Report.
...
.EXAMPLE
    .\Object-Access-Anomaly-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Object-Access-Anomaly-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Object Access Anomaly Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Object-Access-Anomaly-Report'
        Add-ReportRow -Check 'Tool' -Result 'Object-Access-Anomaly-Report' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Object Access Anomaly Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Object Access Anomaly Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
