...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Deep-Audit.ps1
.EXAMPLE
    .\Event-Log-Analysis-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Deep Audit'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
