...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Lateral Movement Indicator Scan
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Lateral-Movement-Indicator-Scan.
...
.EXAMPLE
    .\Lateral-Movement-Indicator-Scan.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Lateral-Movement-Indicator-Scan.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Lateral Movement Indicator Scan'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Lateral-Movement-Indicator-Scan'
        Add-ReportRow -Check 'Tool' -Result 'Lateral-Movement-Indicator-Scan' -Status 'INFO' -Detail '14_Event-Log-Analysis'
        Write-SectionBanner 'SECURITY EVENT ANALYSIS'
        ...
        $idMap = @{
            ...
            'Lateral' = @(4648,4624,4672)
            'USB-Insertion' = @(2003,2100,2101)
            'Service-Installation' = @(7045)
            'Audit-Policy-Change' = @(4719)
            'PowerShell-Script-Block' = @(4104)
        }
        $ids = @(4625,4624,4740,7045)
        foreach ($k in $idMap.Keys) { if ($Name -match $k) { $ids = $idMap[$k]; break } }

        $logName = if ($Name -match 'PowerShell-Script-Block') { 'Microsoft-Windows-PowerShell/Operational' } elseif ($Name -match 'USB-Insertion') { 'Microsoft-Windows-DriverFrameworks-UserMode/Operational' } else { 'Security' }
        try {
            $evts = @(Get-WinEvent -FilterHashtable @{LogName=$logName; Id=$ids; StartTime=$start} -MaxEvents 50 -ErrorAction SilentlyContinue)
            Add-ReportRow -Check "$logName ids $($ids -join ',')" -Result "$($evts.Count) events (3d)" -Status $(if ($evts.Count -gt 25 -and $Name -match 'Failed-Login|Lockout') {'WARN'} else {'INFO'}) -Detail ''
            foreach ($e in ($evts | Select-Object -First 20)) {
                Add-ReportRow -Check "Id $($e.Id) $($e.TimeCreated.ToString('g'))" -Result $e.ProviderName -Status 'INFO' -Detail ($e.Message.Substring(0, [Math]::Min(140, $e.Message.Length)))
            }
        } catch {
            Add-ReportRow -Check 'Security query' -Result 'Failed/empty' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Security events'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Lateral Movement Indicator Scan' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Lateral Movement Indicator Scan' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
