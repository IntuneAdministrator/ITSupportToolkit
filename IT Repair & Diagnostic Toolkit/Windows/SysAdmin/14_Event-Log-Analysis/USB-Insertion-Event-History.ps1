...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: USB Insertion Event History
.DESCRIPTION
    Production-grade Windows SysAdmin tool for USB-Insertion-Event-History.
...
.EXAMPLE
    .\USB-Insertion-Event-History.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\USB-Insertion-Event-History.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'USB Insertion Event History'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'USB-Insertion-Event-History'
        Add-ReportRow -Check 'Tool' -Result 'USB-Insertion-Event-History' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'USB Insertion Event History' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'USB Insertion Event History' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
