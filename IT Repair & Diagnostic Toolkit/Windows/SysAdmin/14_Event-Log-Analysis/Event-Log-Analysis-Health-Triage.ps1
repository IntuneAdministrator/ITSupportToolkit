...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Health-Triage.ps1
.EXAMPLE
    .\Event-Log-Analysis-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Health Triage'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
