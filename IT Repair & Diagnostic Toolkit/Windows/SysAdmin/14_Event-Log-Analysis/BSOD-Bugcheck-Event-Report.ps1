...
<#
.SYNOPSIS
    Common problem fix: BSOD Bugcheck Event Report
.DESCRIPTION
...
.EXAMPLE
    .\BSOD-Bugcheck-Event-Report
.EXAMPLE
    .\BSOD-Bugcheck-Event-Report -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BSOD Bugcheck Event Report'; $totalSteps = 8; $Name = 'BSOD-Bugcheck-Event-Report'
try {
    ...
    Export-HtmlReport -Title 'BSOD Bugcheck Event Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BSOD Bugcheck Event Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
