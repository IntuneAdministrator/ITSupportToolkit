...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Auto-Repair.ps1
.EXAMPLE
    .\Event-Log-Analysis-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Auto Repair'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
