...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Privilege Use Event Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Privilege-Use-Event-Report.
...
.EXAMPLE
    .\Privilege-Use-Event-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Privilege-Use-Event-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Privilege Use Event Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Privilege-Use-Event-Report'
        Add-ReportRow -Check 'Tool' -Result 'Privilege-Use-Event-Report' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Privilege Use Event Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Privilege Use Event Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
