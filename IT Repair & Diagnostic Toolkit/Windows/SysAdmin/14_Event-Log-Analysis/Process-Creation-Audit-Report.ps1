...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Process Creation Audit Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Process-Creation-Audit-Report.
...
.EXAMPLE
    .\Process-Creation-Audit-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Process-Creation-Audit-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Process Creation Audit Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Process-Creation-Audit-Report'
        Add-ReportRow -Check 'Tool' -Result 'Process-Creation-Audit-Report' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Process Creation Audit Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Process Creation Audit Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
