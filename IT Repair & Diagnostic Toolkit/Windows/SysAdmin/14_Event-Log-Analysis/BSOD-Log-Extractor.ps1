...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: BSOD Log Extractor
.DESCRIPTION
    Production-grade Windows SysAdmin tool for BSOD-Log-Extractor.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'BSOD-Log-Extractor'
        Add-ReportRow -Check 'Tool' -Result 'BSOD-Log-Extractor' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'BSOD Log Extractor' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'BSOD Log Extractor' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
