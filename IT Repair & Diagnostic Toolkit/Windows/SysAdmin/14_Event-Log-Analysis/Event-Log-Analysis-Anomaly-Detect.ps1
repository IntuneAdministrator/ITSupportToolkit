...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Anomaly-Detect.ps1
.EXAMPLE
    .\Event-Log-Analysis-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Anomaly Detect'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
