...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PowerShell Script Block Log Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PowerShell-Script-Block-Log-Analyzer.
...
.EXAMPLE
    .\PowerShell-Script-Block-Log-Analyzer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PowerShell-Script-Block-Log-Analyzer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PowerShell Script Block Log Analyzer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'PowerShell-Script-Block-Log-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'PowerShell-Script-Block-Log-Analyzer' -Status 'INFO' -Detail '14_Event-Log-Analysis'
        Write-SectionBanner 'CRITICAL / CRASH EVENTS'
        $start = (Get-Date).AddDays(-7)
        try {
            $crit = @(Get-WinEvent -FilterHashtable @{LogName='System'; Level=1,2; StartTime=$start} -MaxEvents 40 -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'System Critical/Error (7d)' -Result "$($crit.Count)" -Status $(if ($crit.Count -lt 10) {'PASS'} else {'WARN'}) -Detail ''
            foreach ($e in ($crit | Select-Object -First 15)) {
                Add-ReportRow -Check "Event $($e.Id)" -Result $e.ProviderName -Status $(if ($e.LevelDisplayName -eq 'Critical') {'FAIL'} else {'WARN'}) -Detail ($e.Message.Substring(0, [Math]::Min(160, $e.Message.Length)))
            }
        } catch {
            Add-ReportRow -Check 'System events' -Result 'Query failed' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 2 $totalSteps $Activity 'System errors'

        $minidump = "$env:SystemRoot\Minidump"
        if (Test-Path $minidump) {
            $dumps = @(Get-ChildItem $minidump -Filter *.dmp -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 10)
            Add-ReportRow -Check 'Minidumps' -Result "$($dumps.Count) recent" -Status $(if ($dumps.Count -eq 0) {'PASS'} else {'WARN'}) -Detail (($dumps | ForEach-Object Name) -join ', ')
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Dumps'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'PowerShell Script Block Log Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PowerShell Script Block Log Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
