...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Application Crash Log Digest
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Application-Crash-Log-Digest.
...
.EXAMPLE
    .\Application-Crash-Log-Digest.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Application-Crash-Log-Digest.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Application Crash Log Digest'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Application-Crash-Log-Digest'
        Add-ReportRow -Check 'Tool' -Result 'Application-Crash-Log-Digest' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Application Crash Log Digest' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Application Crash Log Digest' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
