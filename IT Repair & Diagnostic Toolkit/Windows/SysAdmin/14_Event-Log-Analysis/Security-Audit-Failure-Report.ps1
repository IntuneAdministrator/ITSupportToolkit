...
<#
.SYNOPSIS
    Common problem fix: Security Audit Failure Report
.DESCRIPTION
...
.EXAMPLE
    .\Security-Audit-Failure-Report -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Audit Failure Report'; $totalSteps = 8; $Name = 'Security-Audit-Failure-Report'
try {
    ...
    Export-HtmlReport -Title 'Security Audit Failure Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Audit Failure Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
