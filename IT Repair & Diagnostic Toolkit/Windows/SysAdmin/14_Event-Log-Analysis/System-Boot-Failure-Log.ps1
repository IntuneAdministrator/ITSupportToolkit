...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: System Boot Failure Log
.DESCRIPTION
    Production-grade Windows SysAdmin tool for System-Boot-Failure-Log.
...
.EXAMPLE
    .\System-Boot-Failure-Log.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\System-Boot-Failure-Log.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'System Boot Failure Log'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'System-Boot-Failure-Log'
        Add-ReportRow -Check 'Tool' -Result 'System-Boot-Failure-Log' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'System Boot Failure Log' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'System Boot Failure Log' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
