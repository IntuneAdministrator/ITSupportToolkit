...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 14_Event-Log-Analysis). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Event-Log-Analysis-Evidence-Report.ps1
.EXAMPLE
    .\Event-Log-Analysis-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Evidence Report'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '14_Event-Log-Analysis' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Event Log Analysis Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
