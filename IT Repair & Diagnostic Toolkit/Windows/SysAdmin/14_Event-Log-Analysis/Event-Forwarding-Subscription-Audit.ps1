...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Event Forwarding Subscription Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Event-Forwarding-Subscription-Audit.
Category: 14_Event-Log-Analysis. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Event-Forwarding-Subscription-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Event-Forwarding-Subscription-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Event Forwarding Subscription Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'EVENT LOG ANALYSIS'
        $Name = 'Event-Forwarding-Subscription-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Event-Forwarding-Subscription-Audit' -Status 'INFO' -Detail '14_Event-Log-Analysis'
        Write-SectionBanner 'EVENT LOG MAINTENANCE'
        $logs = @('Application','System','Security','Setup')
        $backupRoot = 'C:\IT-Logs\EventLogBackups'
        New-Item -ItemType Directory -Path $backupRoot -Force | Out-Null
        foreach ($l in $logs) {
            $log = Get-WinEvent -ListLog $l -ErrorAction SilentlyContinue
            if ($log) {
                $pct = if ($log.MaximumSizeInBytes -gt 0) { [int](($log.FileSize / $log.MaximumSizeInBytes) * 100) } else { 0 }
                Add-ReportRow -Check "Log $l" -Result "$pct% full" -Status $(if ($pct -lt 80) {'PASS'} else {'WARN'}) -Detail "MaxMB=$([math]::Round($log.MaximumSizeInBytes/1MB,0)) Records=$($log.RecordCount)"
            }
            if ($Name -match 'Event-Log-Clear' -and $PSCmdlet.ShouldProcess($l, 'Backup then clear log')) {
                $bak = Join-Path $backupRoot ("{0}_{1}.evtx" -f $l, (Get-Date -Format 'yyyyMMdd_HHmmss'))
                wevtutil epl $l $bak 2>$null
                wevtutil cl $l 2>$null
                Add-ReportRow -Check "Cleared $l" -Result 'Backed up + cleared' -Status 'PASS' -Detail $bak
            }
            if ($Name -match 'Event-Log-Size' -and $PSCmdlet.ShouldProcess($l, 'Set max size 256MB')) {
                wevtutil sl $l /ms:268435456 2>$null
                Add-ReportRow -Check "Size policy $l" -Result '256MB' -Status 'PASS' -Detail 'wevtutil sl /ms'
            }
        }
        if ($Name -match 'Forwarding') {
            try {
                $subs = wecutil es 2>&1
                Add-ReportRow -Check 'Forwarding subscriptions' -Result 'Enumerated' -Status 'INFO' -Detail ($subs | Out-String).Trim()
            } catch {
                Add-ReportRow -Check 'Forwarding' -Result 'wecutil unavailable' -Status 'WARN' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Log maintenance'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Event Forwarding Subscription Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Event Forwarding Subscription Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
