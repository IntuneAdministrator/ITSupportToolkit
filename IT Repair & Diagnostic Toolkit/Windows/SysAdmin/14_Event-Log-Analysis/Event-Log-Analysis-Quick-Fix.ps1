...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Quick-Fix.ps1
.EXAMPLE
    .\Event-Log-Analysis-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Quick Fix'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
