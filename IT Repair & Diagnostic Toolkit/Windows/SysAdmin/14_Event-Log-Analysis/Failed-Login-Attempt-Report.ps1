...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Failed Login Attempt Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Failed-Login-Attempt-Report.
...
.EXAMPLE
    .\Failed-Login-Attempt-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Failed-Login-Attempt-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Failed Login Attempt Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Failed-Login-Attempt-Report'
        Add-ReportRow -Check 'Tool' -Result 'Failed-Login-Attempt-Report' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Failed Login Attempt Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Failed Login Attempt Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
