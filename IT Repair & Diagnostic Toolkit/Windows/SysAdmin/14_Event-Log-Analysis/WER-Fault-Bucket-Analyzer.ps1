...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WER Fault Bucket Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WER-Fault-Bucket-Analyzer.
...
.EXAMPLE
    .\WER-Fault-Bucket-Analyzer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WER-Fault-Bucket-Analyzer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WER Fault Bucket Analyzer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WER-Fault-Bucket-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'WER-Fault-Bucket-Analyzer' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'WER Fault Bucket Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WER Fault Bucket Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
