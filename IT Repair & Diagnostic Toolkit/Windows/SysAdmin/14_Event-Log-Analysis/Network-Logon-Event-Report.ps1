...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Logon Event Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Logon-Event-Report.
...
.EXAMPLE
    .\Network-Logon-Event-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Logon-Event-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Logon Event Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Network-Logon-Event-Report'
        Add-ReportRow -Check 'Tool' -Result 'Network-Logon-Event-Report' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Network Logon Event Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Logon Event Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
