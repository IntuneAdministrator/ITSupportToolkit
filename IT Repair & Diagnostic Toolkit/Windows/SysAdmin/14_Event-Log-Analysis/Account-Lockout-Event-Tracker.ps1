...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Account Lockout Event Tracker
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Account-Lockout-Event-Tracker.
...
.EXAMPLE
    .\Account-Lockout-Event-Tracker.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Account-Lockout-Event-Tracker.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Account Lockout Event Tracker'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Account-Lockout-Event-Tracker'
        Add-ReportRow -Check 'Tool' -Result 'Account-Lockout-Event-Tracker' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Account Lockout Event Tracker' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Account Lockout Event Tracker' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
