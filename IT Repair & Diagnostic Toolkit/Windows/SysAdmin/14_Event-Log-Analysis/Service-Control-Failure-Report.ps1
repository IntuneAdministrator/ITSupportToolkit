...
<#
.SYNOPSIS
    Common problem fix: Service Control Failure Report
.DESCRIPTION
...
.EXAMPLE
    .\Service-Control-Failure-Report
.EXAMPLE
    .\Service-Control-Failure-Report -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Service Control Failure Report'; $totalSteps = 8; $Name = 'Service-Control-Failure-Report'
try {
    ...
    Export-HtmlReport -Title 'Service Control Failure Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Service Control Failure Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
