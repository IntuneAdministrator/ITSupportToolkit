...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Policy-Validate.ps1
.EXAMPLE
    .\Event-Log-Analysis-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Policy Validate'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
