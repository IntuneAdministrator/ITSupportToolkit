...
<#
.SYNOPSIS
    Common problem fix: Event Log Analysis Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Event-Log-Analysis-Stack-Reset.ps1
.EXAMPLE
    .\Event-Log-Analysis-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Event Log Analysis Stack Reset'; $totalSteps = 8; $Name = 'Event-Log-Analysis-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Event Log Analysis Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Event Log Analysis Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
