...
<#
.SYNOPSIS
    Common problem fix: Disk Error Event Report
.DESCRIPTION
...
.EXAMPLE
    .\Disk-Error-Event-Report
.EXAMPLE
    .\Disk-Error-Event-Report -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Disk Error Event Report'; $totalSteps = 8; $Name = 'Disk-Error-Event-Report'
try {
    ...
    Export-HtmlReport -Title 'Disk Error Event Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Disk Error Event Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
