...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Installation Event Hunt
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Installation-Event-Hunt.
...
.EXAMPLE
    .\Service-Installation-Event-Hunt.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Service-Installation-Event-Hunt.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Service Installation Event Hunt'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Service-Installation-Event-Hunt'
        Add-ReportRow -Check 'Tool' -Result 'Service-Installation-Event-Hunt' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Service Installation Event Hunt' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Installation Event Hunt' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
