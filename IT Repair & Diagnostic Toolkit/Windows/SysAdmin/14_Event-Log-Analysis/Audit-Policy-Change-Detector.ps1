...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audit Policy Change Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audit-Policy-Change-Detector.
...
.EXAMPLE
    .\Audit-Policy-Change-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audit-Policy-Change-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audit Policy Change Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Audit-Policy-Change-Detector'
        Add-ReportRow -Check 'Tool' -Result 'Audit-Policy-Change-Detector' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Audit Policy Change Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audit Policy Change Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
