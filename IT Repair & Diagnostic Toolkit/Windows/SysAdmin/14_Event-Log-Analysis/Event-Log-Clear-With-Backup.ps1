...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Event Log Clear With Backup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Event-Log-Clear-With-Backup.
...
.EXAMPLE
    .\Event-Log-Clear-With-Backup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Event-Log-Clear-With-Backup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Event Log Clear With Backup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Event-Log-Clear-With-Backup'
        Add-ReportRow -Check 'Tool' -Result 'Event-Log-Clear-With-Backup' -Status 'INFO' -Detail '14_Event-Log-Analysis'
    ...
    Export-HtmlReport -Title 'Event Log Clear With Backup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Event Log Clear With Backup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
