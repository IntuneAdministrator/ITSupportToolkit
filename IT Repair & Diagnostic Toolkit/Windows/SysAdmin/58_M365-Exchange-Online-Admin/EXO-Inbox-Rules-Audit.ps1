...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: EXO Inbox Rules Audit
.DESCRIPTION
...
.EXAMPLE
    .\EXO-Inbox-Rules-Audit.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\EXO-Inbox-Rules-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'EXO Inbox Rules Audit'; $totalSteps = 8; $Name = 'EXO-Inbox-Rules-Audit'; $Folder = '58_M365-Exchange-Online-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'EXO Inbox Rules Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'EXO Inbox Rules Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
