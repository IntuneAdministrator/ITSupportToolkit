...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: EXO Shared Mailbox Permissions
.DESCRIPTION
    Phase 12 M365 tenant admin (SysAdmin/58_M365-Exchange-Online-Admin). Uses Microsoft Graph / EXO modules when installed.
...
.EXAMPLE
    .\EXO-Shared-Mailbox-Permissions.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\EXO-Shared-Mailbox-Permissions.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'EXO Shared Mailbox Permissions'; $totalSteps = 8; $Name = 'EXO-Shared-Mailbox-Permissions'; $Folder = '58_M365-Exchange-Online-Admin'
...
try {
    ...
    Add-ReportRow -Check 'Module' -Result '58_M365-Exchange-Online-Admin' -Status 'INFO' -Detail 'SysAdmin Phase12 M365'
    Add-M365PortalRows
    ...
    if ($UPN) { Add-ReportRow -Check 'Target UPN' -Result $UPN -Status 'INFO' -Detail '' }
        ...
        Write-SectionBanner 'EXCHANGE ONLINE ADMIN'
        Write-ProgressStep 3 $totalSteps $Activity 'EXO'
        $exo = Get-Module -ListAvailable ExchangeOnlineManagement -EA SilentlyContinue
        Add-ReportRow -Check 'ExchangeOnlineManagement' -Result $(if($exo){"v$($exo[0].Version)"}else{'Missing'}) -Status $(if($exo){'PASS'}else{'WARN'}) -Detail 'Install-Module ExchangeOnlineManagement -Scope CurrentUser'
        $exoOk = $false
        if ($exo -and $PSCmdlet.ShouldProcess('Exchange Online', 'Connect-ExchangeOnline')) {
            try {
                Import-Module ExchangeOnlineManagement -EA Stop
                if (-not (Get-ConnectionInformation -EA SilentlyContinue)) {
                    Connect-ExchangeOnline -ShowBanner:$false -EA Stop
                }
                $exoOk = $true
                Add-ReportRow -Check 'EXO connect' -Result 'Connected' -Status 'PASS' -Detail ''
            } catch { Add-ReportRow -Check 'EXO connect' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
        }
        if ($exoOk -and $UPN) {
            try {
                if ($Name -match 'Mailbox-Lookup|Mailbox-Quota|Litigation') {
                    $mbx = Get-EXOMailbox -Identity $UPN -Properties DisplayName,RecipientTypeDetails,LitigationHoldEnabled,ProhibitSendQuota -EA Stop
                    Add-ReportRow -Check 'Mailbox type' -Result $mbx.RecipientTypeDetails -Status 'PASS' -Detail $mbx.DisplayName
                    Add-ReportRow -Check 'LitigationHold' -Result "$($mbx.LitigationHoldEnabled)" -Status 'INFO' -Detail ''
                    $st = Get-EXOMailboxStatistics -Identity $UPN -EA SilentlyContinue
                    if ($st) { Add-ReportRow -Check 'Mailbox size' -Result $st.TotalItemSize -Status 'INFO' -Detail "Items=$($st.ItemCount)" }
                }
                if ($Name -match 'Shared-Mailbox' ) {
                    $perms = Get-EXOMailboxPermission -Identity $UPN -EA SilentlyContinue | Select-Object -First 15
                    Add-ReportRow -Check 'Mailbox permissions' -Result "$($perms.Count) ACEs sampled" -Status 'INFO' -Detail (($perms | ForEach-Object { "$($_.User):$($_.AccessRights)" }) -join '; ')
                }
                if ($Name -match 'Forwarding') {
                    $mbx = Get-EXOMailbox -Identity $UPN -Properties ForwardingSmtpAddress,ForwardingAddress,DeliverToMailboxAndForward -EA Stop
                    Add-ReportRow -Check 'ForwardingSmtp' -Result $(if($mbx.ForwardingSmtpAddress){$mbx.ForwardingSmtpAddress}else{'(none)'}) -Status $(if($mbx.ForwardingSmtpAddress){'WARN'}else{'PASS'}) -Detail "DeliverAndForward=$($mbx.DeliverToMailboxAndForward)"
                }
                if ($Name -match 'Inbox-Rules') {
                    $rules = Get-InboxRule -Mailbox $UPN -EA SilentlyContinue | Select-Object -First 20
                    Add-ReportRow -Check 'Inbox rules' -Result "$($rules.Count)" -Status 'INFO' -Detail (($rules | ForEach-Object Name) -join '; ')
                }
                if ($Name -match 'Distribution-Group') {
                    $dg = Get-EXORecipient -Identity $UPN -EA SilentlyContinue
                    Add-ReportRow -Check 'Recipient' -Result $(if($dg){$dg.RecipientTypeDetails}else{'N/A'}) -Status 'INFO' -Detail 'For DG use DG identity as -UserPrincipalName'
                }
            } catch { Add-ReportRow -Check 'EXO query' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message }
        } elseif (-not $exo) {
            Add-ReportRow -Check 'Portal' -Result 'https://admin.exchange.microsoft.com' -Status 'INFO' -Detail 'Install EXO module for automation'
        }
        Write-ProgressStep 5 $totalSteps $Activity 'EXO done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'EXO Shared Mailbox Permissions' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'EXO Shared Mailbox Permissions' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
