...
<#
.SYNOPSIS
    Common problem fix: M365 Exchange Online Admin Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\M365-Exchange-Online-Admin-Stack-Reset.ps1
.EXAMPLE
    .\M365-Exchange-Online-Admin-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'M365 Exchange Online Admin Stack Reset'; $totalSteps = 8; $Name = 'M365-Exchange-Online-Admin-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'M365 Exchange Online Admin Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'M365 Exchange Online Admin Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
