...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Deep-Audit.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Deep Audit'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
