...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Health-Triage.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Health Triage'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
