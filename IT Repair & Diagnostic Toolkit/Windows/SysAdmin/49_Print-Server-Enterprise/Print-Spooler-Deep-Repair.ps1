...
<#
.SYNOPSIS
    Enterprise IT tool: Print Spooler Deep Repair
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Print-Spooler-Deep-Repair.ps1
.EXAMPLE
    .\Print-Spooler-Deep-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Print Spooler Deep Repair'; $totalSteps=8; $Name='Print-Spooler-Deep-Repair'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Deep Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Deep Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
