...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Quick-Fix.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Quick Fix'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
