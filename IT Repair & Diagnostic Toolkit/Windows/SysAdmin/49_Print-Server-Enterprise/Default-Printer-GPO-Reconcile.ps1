...
<#
.SYNOPSIS
    Enterprise IT tool: Default Printer GPO Reconcile
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Default-Printer-GPO-Reconcile.ps1
.EXAMPLE
    .\Default-Printer-GPO-Reconcile.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Default Printer GPO Reconcile'; $totalSteps=8; $Name='Default-Printer-GPO-Reconcile'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'Default Printer GPO Reconcile' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Default Printer GPO Reconcile' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
