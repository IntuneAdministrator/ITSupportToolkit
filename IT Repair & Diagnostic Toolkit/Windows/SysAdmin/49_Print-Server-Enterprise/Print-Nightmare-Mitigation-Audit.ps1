...
<#
.SYNOPSIS
    Enterprise IT tool: Print Nightmare Mitigation Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Print-Nightmare-Mitigation-Audit.ps1
.EXAMPLE
    .\Print-Nightmare-Mitigation-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Print Nightmare Mitigation Audit'; $totalSteps=8; $Name='Print-Nightmare-Mitigation-Audit'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'Print Nightmare Mitigation Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Nightmare Mitigation Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
