...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Anomaly-Detect.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Anomaly Detect'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
