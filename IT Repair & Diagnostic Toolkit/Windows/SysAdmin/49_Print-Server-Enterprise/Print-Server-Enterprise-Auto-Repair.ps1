...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Auto-Repair.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Auto Repair'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
