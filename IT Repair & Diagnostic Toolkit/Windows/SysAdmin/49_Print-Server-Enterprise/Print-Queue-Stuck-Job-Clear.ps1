...
<#
.SYNOPSIS
    Enterprise IT tool: Print Queue Stuck Job Clear
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Print-Queue-Stuck-Job-Clear.ps1
.EXAMPLE
    .\Print-Queue-Stuck-Job-Clear.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Print Queue Stuck Job Clear'; $totalSteps=8; $Name='Print-Queue-Stuck-Job-Clear'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'Print Queue Stuck Job Clear' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Queue Stuck Job Clear' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
