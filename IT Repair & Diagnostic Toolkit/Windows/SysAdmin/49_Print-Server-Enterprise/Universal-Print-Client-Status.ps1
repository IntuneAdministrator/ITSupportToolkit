...
<#
.SYNOPSIS
    Enterprise IT tool: Universal Print Client Status
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Universal-Print-Client-Status.ps1
.EXAMPLE
    .\Universal-Print-Client-Status.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Universal Print Client Status'; $totalSteps=8; $Name='Universal-Print-Client-Status'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'Universal Print Client Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Universal Print Client Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
