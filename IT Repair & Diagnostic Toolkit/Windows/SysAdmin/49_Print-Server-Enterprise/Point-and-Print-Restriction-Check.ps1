...
<#
.SYNOPSIS
    Enterprise IT tool: Point and Print Restriction Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Point-and-Print-Restriction-Check.ps1
.EXAMPLE
    .\Point-and-Print-Restriction-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Point and Print Restriction Check'; $totalSteps=8; $Name='Point-and-Print-Restriction-Check'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '49_Print-Server-Enterprise' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'PRINT SERVER ENTERPRISE'
        $spool = Get-Service Spooler -EA SilentlyContinue
        Add-ReportRow -Check 'Spooler' -Result $(if($spool){"$($spool.Status)"}else{'N/A'}) -Status $(if($spool -and $spool.Status -eq 'Running'){'PASS'}else{'FAIL'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'Print stack'
        if ($PSCmdlet.ShouldProcess('Spooler', 'restart if needed')) {
            if ($spool -and $spool.Status -ne 'Running') { Start-Service Spooler -EA SilentlyContinue }
            elseif ($Name -match 'Stuck-Job|Deep-Repair') {
                Stop-Service Spooler -Force -EA SilentlyContinue
                Get-ChildItem "$env:SystemRoot\System32\spool\PRINTERS" -EA SilentlyContinue | Remove-Item -Force -EA SilentlyContinue
                Start-Service Spooler -EA SilentlyContinue
                Add-ReportRow -Check 'Queue clear' -Result 'Spool PRINTERS cleaned' -Status 'PASS' -Detail ''
            }
        }
        $printers = @(Get-Printer -EA SilentlyContinue)
        Add-ReportRow -Check 'Printers' -Result "$($printers.Count)" -Status 'INFO' -Detail (($printers | Select-Object -First 8 -ExpandProperty Name) -join ', ')
        if ($Name -match 'Point-and-Print|Nightmare') {
            $ppp = Get-ItemProperty 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Printers\PointAndPrint' -EA SilentlyContinue
            Add-ReportRow -Check 'PointAndPrint policy' -Result $(if($ppp){'Present'}else{'Not set'}) -Status 'INFO' -Detail 'Review RestrictDriverInstallationToAdministrators'
        }
        if ($Name -match 'Universal-Print') {
            $up = Get-Service PrintWorkflowUserSvc* -EA SilentlyContinue | Select-Object -First 1
            Add-ReportRow -Check 'Universal Print / Workflow' -Result $(if($up){"$($up.Status)"}else{'Check Azure UP client'}) -Status 'INFO' -Detail ''
        }
        if ($Name -match 'IPP|HTTP') {
            Add-ReportRow -Check 'IPP' -Result 'Use Test-NetConnection printer:631 / https' -Status 'INFO' -Detail 'Enterprise IPP/HTTPS printers'
        }
        Write-ProgressStep 5 $totalSteps $Activity 'Print done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Point and Print Restriction Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Point and Print Restriction Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
