...
$exitCode=0; $Activity='IPP HTTP Printer Probe'; $totalSteps=8; $Name='IPP-HTTP-Printer-Probe'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'IPP HTTP Printer Probe' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'IPP HTTP Printer Probe' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
