...
<#
.SYNOPSIS
    Enterprise IT tool: Printer Driver Package Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/49_Print-Server-Enterprise). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Printer-Driver-Package-Audit.ps1
.EXAMPLE
    .\Printer-Driver-Package-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Printer Driver Package Audit'; $totalSteps=8; $Name='Printer-Driver-Package-Audit'; $Folder='49_Print-Server-Enterprise'
try {
    ...
    Export-HtmlReport -Title 'Printer Driver Package Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Printer Driver Package Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
