...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Stack-Reset.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Stack Reset'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
