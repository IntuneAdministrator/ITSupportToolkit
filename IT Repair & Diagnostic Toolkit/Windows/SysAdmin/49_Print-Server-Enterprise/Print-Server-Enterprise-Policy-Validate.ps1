...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Print-Server-Enterprise-Policy-Validate.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Policy Validate'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
