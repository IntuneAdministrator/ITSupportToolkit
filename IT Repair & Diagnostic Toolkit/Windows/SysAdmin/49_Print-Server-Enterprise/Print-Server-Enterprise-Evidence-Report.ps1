...
<#
.SYNOPSIS
    Common problem fix: Print Server Enterprise Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 49_Print-Server-Enterprise). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Print-Server-Enterprise-Evidence-Report.ps1
.EXAMPLE
    .\Print-Server-Enterprise-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Server Enterprise Evidence Report'; $totalSteps = 8; $Name = 'Print-Server-Enterprise-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '49_Print-Server-Enterprise' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '49_Print-Server-Enterprise'
    ...
    Export-HtmlReport -Title 'Print Server Enterprise Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Server Enterprise Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
