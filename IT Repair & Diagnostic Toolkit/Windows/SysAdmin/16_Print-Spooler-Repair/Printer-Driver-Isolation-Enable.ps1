...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Printer Driver Isolation Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Printer-Driver-Isolation-Enable.
...
.EXAMPLE
    .\Printer-Driver-Isolation-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Printer-Driver-Isolation-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Printer Driver Isolation Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Printer-Driver-Isolation-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Printer-Driver-Isolation-Enable' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Printer Driver Isolation Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Printer Driver Isolation Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
