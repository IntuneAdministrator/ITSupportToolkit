...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Policy-Validate.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Policy Validate'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
