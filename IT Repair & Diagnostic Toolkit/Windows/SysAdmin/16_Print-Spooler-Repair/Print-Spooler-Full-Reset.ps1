...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Print Spooler Full Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Print-Spooler-Full-Reset.
...
.EXAMPLE
    .\Print-Spooler-Full-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Print-Spooler-Full-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Print Spooler Full Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Print-Spooler-Full-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Print-Spooler-Full-Reset' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
        Write-SectionBanner 'PRINT SPOOLER RESET'
        if ($PSCmdlet.ShouldProcess('Spooler', 'Stop service, clear queue, restart')) {
            Stop-Service Spooler -Force -ErrorAction SilentlyContinue
            $queue = "$env:SystemRoot\System32\spool\PRINTERS"
            if (Test-Path $queue) {
                $files = @(Get-ChildItem $queue -Force -ErrorAction SilentlyContinue)
                Remove-Item "$queue\*" -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Clear print queue' -Result "Removed $($files.Count) files" -Status 'PASS' -Detail $queue
            }
        ...
        foreach ($p in $printers) {
            Add-ReportRow -Check "Printer $($p.Name)" -Result $p.PrinterStatus -Status 'INFO' -Detail "$($p.DriverName) @ $($p.PortName)"
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Printers'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Print Spooler Full Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Print Spooler Full Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
