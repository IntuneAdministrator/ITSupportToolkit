...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: RAW vs EMF Spool Mode Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for RAW-vs-EMF-Spool-Mode-Fix.
...
.EXAMPLE
    .\RAW-vs-EMF-Spool-Mode-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\RAW-vs-EMF-Spool-Mode-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'RAW vs EMF Spool Mode Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'RAW-vs-EMF-Spool-Mode-Fix'
        Add-ReportRow -Check 'Tool' -Result 'RAW-vs-EMF-Spool-Mode-Fix' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'RAW vs EMF Spool Mode Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RAW vs EMF Spool Mode Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
