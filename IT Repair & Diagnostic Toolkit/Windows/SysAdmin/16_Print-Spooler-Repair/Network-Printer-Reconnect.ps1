...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Network Printer Reconnect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Network-Printer-Reconnect.
...
.EXAMPLE
    .\Network-Printer-Reconnect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Network-Printer-Reconnect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Network Printer Reconnect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Network-Printer-Reconnect'
        Add-ReportRow -Check 'Tool' -Result 'Network-Printer-Reconnect' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
        $s = Get-Service Spooler
        Add-ReportRow -Check 'Spooler' -Result $s.Status -Status $(if ($s.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'Spooler'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Network Printer Reconnect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Network Printer Reconnect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
