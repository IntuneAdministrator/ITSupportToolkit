...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Print Spooler Event Log Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Print-Spooler-Event-Log-Analyzer.
Category: 16_Print-Spooler-Repair. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Print-Spooler-Event-Log-Analyzer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Print-Spooler-Event-Log-Analyzer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Print Spooler Event Log Analyzer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PRINT SPOOLER REPAIR'
        $Name = 'Print-Spooler-Event-Log-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'Print-Spooler-Event-Log-Analyzer' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
        Write-SectionBanner 'PRINTER DRIVERS / POLICY'
        try {
            $drivers = @(Get-PrinterDriver -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'Printer drivers' -Result "$($drivers.Count)" -Status 'INFO' -Detail (($drivers | Select-Object -First 10 Name) -join '; ')
        } catch {
            Add-ReportRow -Check 'Printer drivers' -Result 'Query failed' -Status 'WARN' -Detail $_.Exception.Message
        }
        $ports = @(Get-PrinterPort -ErrorAction SilentlyContinue)
        Add-ReportRow -Check 'Printer ports' -Result "$($ports.Count)" -Status 'INFO' -Detail (($ports | Select-Object -First 10 Name) -join '; ')

        if ($Name -match 'Point-And-Print') {
            $pnpr = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Printers\PointAndPrint'
            Backup-RegistryKey -KeyPath $pnpr -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            if ($PSCmdlet.ShouldProcess('Point and Print', 'Restrict driver installation to admins')) {
                if (-not (Test-Path $pnpr)) { New-Item $pnpr -Force | Out-Null }
                New-ItemProperty $pnpr -Name RestrictDriverInstallationToAdministrators -Value 1 -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'PointAndPrint restrict' -Result '1' -Status 'PASS' -Detail 'CVE mitigation style policy'
            }
        }
        if ($Name -match 'Isolation' -and $PSCmdlet.ShouldProcess('Printer isolation', 'Set driver isolation where supported')) {
            Add-ReportRow -Check 'Driver isolation' -Result 'Review per-driver Isolation mode' -Status 'INFO' -Detail 'Print Management MMC'
        }
        if ($Name -match 'Event-Log') {
            $ev = @(Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-PrintService/Admin'; StartTime=(Get-Date).AddDays(-7)} -MaxEvents 30 -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'PrintService Admin events' -Result "$($ev.Count)" -Status 'INFO' -Detail ''
            foreach ($e in ($ev | Select-Object -First 10)) {
                Add-ReportRow -Check "Print evt $($e.Id)" -Result $e.LevelDisplayName -Status $(if ($e.Level -le 2) {'WARN'} else {'INFO'}) -Detail ($e.Message.Substring(0, [Math]::Min(120, $e.Message.Length)))
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Print config'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Print Spooler Event Log Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Print Spooler Event Log Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
