...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: XPS Document Writer Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for XPS-Document-Writer-Repair.
...
.EXAMPLE
    .\XPS-Document-Writer-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\XPS-Document-Writer-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'XPS Document Writer Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'XPS-Document-Writer-Repair'
        Add-ReportRow -Check 'Tool' -Result 'XPS-Document-Writer-Repair' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'XPS Document Writer Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'XPS Document Writer Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
