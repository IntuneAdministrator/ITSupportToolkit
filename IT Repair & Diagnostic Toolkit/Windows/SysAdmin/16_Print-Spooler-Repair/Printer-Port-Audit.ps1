...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Printer Port Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Printer-Port-Audit.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Printer-Port-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Printer-Port-Audit' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Printer Port Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Printer Port Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
