...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Evidence-Report.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Evidence Report'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
