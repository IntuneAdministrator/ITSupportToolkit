...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Auto-Repair.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Auto Repair'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
