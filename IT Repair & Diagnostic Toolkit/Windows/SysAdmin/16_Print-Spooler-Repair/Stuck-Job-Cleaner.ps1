...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Stuck Job Cleaner
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Stuck-Job-Cleaner.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Stuck-Job-Cleaner'
        Add-ReportRow -Check 'Tool' -Result 'Stuck-Job-Cleaner' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Stuck Job Cleaner' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Stuck Job Cleaner' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
