...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Printer Security Descriptor Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Printer-Security-Descriptor-Fix.
...
.EXAMPLE
    .\Printer-Security-Descriptor-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Printer-Security-Descriptor-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Printer Security Descriptor Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Printer-Security-Descriptor-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Printer-Security-Descriptor-Fix' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Printer Security Descriptor Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Printer Security Descriptor Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
