...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Point And Print Policy Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Point-And-Print-Policy-Fix.
...
.EXAMPLE
    .\Point-And-Print-Policy-Fix.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Point-And-Print-Policy-Fix.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Point And Print Policy Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Point-And-Print-Policy-Fix'
        Add-ReportRow -Check 'Tool' -Result 'Point-And-Print-Policy-Fix' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Point And Print Policy Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Point And Print Policy Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
