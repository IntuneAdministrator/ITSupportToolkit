...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Anomaly-Detect.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Anomaly Detect'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
