...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Health-Triage.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Health Triage'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
