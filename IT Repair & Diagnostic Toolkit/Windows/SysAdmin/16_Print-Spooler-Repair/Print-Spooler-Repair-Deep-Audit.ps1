...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Deep-Audit.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Deep Audit'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
