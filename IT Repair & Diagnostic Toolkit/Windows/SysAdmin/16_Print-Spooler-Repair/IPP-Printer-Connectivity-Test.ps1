...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: IPP Printer Connectivity Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for IPP-Printer-Connectivity-Test.
...
.EXAMPLE
    .\IPP-Printer-Connectivity-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\IPP-Printer-Connectivity-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'IPP Printer Connectivity Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'IPP-Printer-Connectivity-Test'
        Add-ReportRow -Check 'Tool' -Result 'IPP-Printer-Connectivity-Test' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'IPP Printer Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'IPP Printer Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
