...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Universal Print Connector Status
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Universal-Print-Connector-Status.
...
.EXAMPLE
    .\Universal-Print-Connector-Status.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Universal-Print-Connector-Status.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Universal Print Connector Status'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Universal-Print-Connector-Status'
        Add-ReportRow -Check 'Tool' -Result 'Universal-Print-Connector-Status' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Universal Print Connector Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Universal Print Connector Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
