...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Stack-Reset.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Stack Reset'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
