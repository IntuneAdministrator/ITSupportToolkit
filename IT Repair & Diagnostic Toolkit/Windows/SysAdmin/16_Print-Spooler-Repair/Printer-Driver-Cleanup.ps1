...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Printer Driver Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Printer-Driver-Cleanup.
...
$exitCode = 0
$Activity = 'Printer Driver Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Printer-Driver-Cleanup'
        Add-ReportRow -Check 'Tool' -Result 'Printer-Driver-Cleanup' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Printer Driver Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Printer Driver Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
