...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Print Server Migration Prep
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Print-Server-Migration-Prep.
...
.EXAMPLE
    .\Print-Server-Migration-Prep.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Print-Server-Migration-Prep.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Print Server Migration Prep'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Print-Server-Migration-Prep'
        Add-ReportRow -Check 'Tool' -Result 'Print-Server-Migration-Prep' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Print Server Migration Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Print Server Migration Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
