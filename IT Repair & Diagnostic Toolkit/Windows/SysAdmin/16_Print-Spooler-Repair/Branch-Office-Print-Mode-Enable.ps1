...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Branch Office Print Mode Enable
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Branch-Office-Print-Mode-Enable.
...
.EXAMPLE
    .\Branch-Office-Print-Mode-Enable.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Branch-Office-Print-Mode-Enable.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Branch Office Print Mode Enable'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Branch-Office-Print-Mode-Enable'
        Add-ReportRow -Check 'Tool' -Result 'Branch-Office-Print-Mode-Enable' -Status 'INFO' -Detail '16_Print-Spooler-Repair'
    ...
    Export-HtmlReport -Title 'Branch Office Print Mode Enable' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Branch Office Print Mode Enable' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
