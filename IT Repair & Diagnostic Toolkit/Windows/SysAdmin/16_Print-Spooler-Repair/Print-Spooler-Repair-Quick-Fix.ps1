...
<#
.SYNOPSIS
    Common problem fix: Print Spooler Repair Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Print-Spooler-Repair-Quick-Fix.ps1
.EXAMPLE
    .\Print-Spooler-Repair-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Print Spooler Repair Quick Fix'; $totalSteps = 8; $Name = 'Print-Spooler-Repair-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Print Spooler Repair Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Print Spooler Repair Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
