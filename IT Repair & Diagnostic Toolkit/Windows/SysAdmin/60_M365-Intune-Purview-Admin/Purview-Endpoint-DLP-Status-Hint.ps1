...
<#
.SYNOPSIS
    Common problem fix: Purview Endpoint DLP Status Hint
.DESCRIPTION
...
.EXAMPLE
    .\Purview-Endpoint-DLP-Status-Hint.ps1
.EXAMPLE
    .\Purview-Endpoint-DLP-Status-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Purview Endpoint DLP Status Hint'; $totalSteps = 8; $Name = 'Purview-Endpoint-DLP-Status-Hint'
try {
    ...
    Export-HtmlReport -Title 'Purview Endpoint DLP Status Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Purview Endpoint DLP Status Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
