...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Secure Score Portal Snapshot
.DESCRIPTION
...
.EXAMPLE
    .\Secure-Score-Portal-Snapshot.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Secure-Score-Portal-Snapshot.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Secure Score Portal Snapshot'; $totalSteps = 8; $Name = 'Secure-Score-Portal-Snapshot'; $Folder = '60_M365-Intune-Purview-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Secure Score Portal Snapshot' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Secure Score Portal Snapshot' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
