...
<#
.SYNOPSIS
    Common problem fix: Intune Windows Update Ring Status
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Windows-Update-Ring-Status.ps1
.EXAMPLE
    .\Intune-Windows-Update-Ring-Status.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Windows Update Ring Status'; $totalSteps = 8; $Name = 'Intune-Windows-Update-Ring-Status'
try {
    ...
    Export-HtmlReport -Title 'Intune Windows Update Ring Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Windows Update Ring Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
