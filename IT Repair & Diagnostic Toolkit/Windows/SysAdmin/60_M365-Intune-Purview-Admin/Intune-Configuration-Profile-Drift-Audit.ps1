...
<#
.SYNOPSIS
    Common problem fix: Intune Configuration Profile Drift Audit
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 60_M365-Intune-Purview-Admin). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Intune-Configuration-Profile-Drift-Audit.ps1
.EXAMPLE
    .\Intune-Configuration-Profile-Drift-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Configuration Profile Drift Audit'; $totalSteps = 8; $Name = 'Intune-Configuration-Profile-Drift-Audit'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '60_M365-Intune-Purview-Admin' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '60_M365-Intune-Purview-Admin'
    ...
    Export-HtmlReport -Title 'Intune Configuration Profile Drift Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Configuration Profile Drift Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
