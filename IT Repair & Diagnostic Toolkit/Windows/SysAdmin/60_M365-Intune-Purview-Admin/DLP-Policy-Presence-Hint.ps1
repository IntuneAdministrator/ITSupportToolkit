...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: DLP Policy Presence Hint
.DESCRIPTION
...
.EXAMPLE
    .\DLP-Policy-Presence-Hint.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\DLP-Policy-Presence-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DLP Policy Presence Hint'; $totalSteps = 8; $Name = 'DLP-Policy-Presence-Hint'; $Folder = '60_M365-Intune-Purview-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'DLP Policy Presence Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DLP Policy Presence Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
