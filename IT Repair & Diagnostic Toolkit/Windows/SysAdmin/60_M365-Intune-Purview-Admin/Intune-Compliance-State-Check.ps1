...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Intune Compliance State Check
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Compliance-State-Check.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Intune-Compliance-State-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Compliance State Check'; $totalSteps = 8; $Name = 'Intune-Compliance-State-Check'; $Folder = '60_M365-Intune-Purview-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Intune Compliance State Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Compliance State Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
