...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Intune Managed Device Lookup
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Managed-Device-Lookup.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Intune-Managed-Device-Lookup.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Managed Device Lookup'; $totalSteps = 8; $Name = 'Intune-Managed-Device-Lookup'; $Folder = '60_M365-Intune-Purview-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Intune Managed Device Lookup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Managed Device Lookup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
