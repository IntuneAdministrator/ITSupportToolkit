...
<#
.SYNOPSIS
    Microsoft 365 cloud admin tool: Purview Sensitivity Label Client Hint
.DESCRIPTION
...
.EXAMPLE
    .\Purview-Sensitivity-Label-Client-Hint.ps1 -UserPrincipalName user@contoso.com
.EXAMPLE
    .\Purview-Sensitivity-Label-Client-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Purview Sensitivity Label Client Hint'; $totalSteps = 8; $Name = 'Purview-Sensitivity-Label-Client-Hint'; $Folder = '60_M365-Intune-Purview-Admin'
...
try {
    ...
    Export-HtmlReport -Title 'Purview Sensitivity Label Client Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Purview Sensitivity Label Client Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
