...
<#
.SYNOPSIS
    Common problem fix: Intune Hardware Inventory Graph Export
.DESCRIPTION
...
.EXAMPLE
    .\Intune-Hardware-Inventory-Graph-Export.ps1
.EXAMPLE
    .\Intune-Hardware-Inventory-Graph-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Intune Hardware Inventory Graph Export'; $totalSteps = 8; $Name = 'Intune-Hardware-Inventory-Graph-Export'
try {
    ...
    Export-HtmlReport -Title 'Intune Hardware Inventory Graph Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Intune Hardware Inventory Graph Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
