...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Kernel Memory Dump Config
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Kernel-Memory-Dump-Config.
...
.EXAMPLE
    .\Kernel-Memory-Dump-Config.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Kernel-Memory-Dump-Config.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Kernel Memory Dump Config'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Kernel-Memory-Dump-Config'
        Add-ReportRow -Check 'Tool' -Result 'Kernel-Memory-Dump-Config' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'Kernel Memory Dump Config' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Kernel Memory Dump Config' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
