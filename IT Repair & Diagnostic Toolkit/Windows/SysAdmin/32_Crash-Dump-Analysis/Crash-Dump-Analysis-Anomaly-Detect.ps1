...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Anomaly-Detect.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Anomaly Detect'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
