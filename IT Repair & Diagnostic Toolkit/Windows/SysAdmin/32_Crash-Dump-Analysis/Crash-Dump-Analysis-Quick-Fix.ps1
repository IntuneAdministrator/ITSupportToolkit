...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Quick-Fix.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Quick Fix'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
