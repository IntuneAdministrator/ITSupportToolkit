...
<#
.SYNOPSIS
    Common problem fix: BugCheck Code Lookup Report
.DESCRIPTION
...
.EXAMPLE
    .\BugCheck-Code-Lookup-Report.ps1
.EXAMPLE
    .\BugCheck-Code-Lookup-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'BugCheck Code Lookup Report'; $totalSteps = 8; $Name = 'BugCheck-Code-Lookup-Report'
try {
    ...
    Export-HtmlReport -Title 'BugCheck Code Lookup Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BugCheck Code Lookup Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
