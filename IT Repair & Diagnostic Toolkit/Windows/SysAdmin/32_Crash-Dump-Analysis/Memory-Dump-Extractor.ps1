...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Memory Dump Extractor
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Memory-Dump-Extractor.
...
$exitCode = 0
$Activity = 'Memory Dump Extractor'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Memory-Dump-Extractor'
        Add-ReportRow -Check 'Tool' -Result 'Memory-Dump-Extractor' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'Memory Dump Extractor' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Memory Dump Extractor' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
