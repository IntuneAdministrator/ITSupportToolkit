...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Health-Triage.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Health Triage'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
