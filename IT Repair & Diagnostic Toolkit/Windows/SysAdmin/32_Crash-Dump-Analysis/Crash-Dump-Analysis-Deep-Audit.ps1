...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Deep-Audit.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Deep Audit'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
