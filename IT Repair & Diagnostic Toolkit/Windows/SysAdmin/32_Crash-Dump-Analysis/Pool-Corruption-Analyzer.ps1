...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Pool Corruption Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Pool-Corruption-Analyzer.
...
.EXAMPLE
    .\Pool-Corruption-Analyzer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Pool-Corruption-Analyzer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Pool Corruption Analyzer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Pool-Corruption-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'Pool-Corruption-Analyzer' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'Pool Corruption Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Pool Corruption Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
