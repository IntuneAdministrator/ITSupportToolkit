...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: BSOD Stop Code Lookup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for BSOD-Stop-Code-Lookup.
...
$exitCode = 0
$Activity = 'BSOD Stop Code Lookup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'BSOD-Stop-Code-Lookup'
        Add-ReportRow -Check 'Tool' -Result 'BSOD-Stop-Code-Lookup' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'BSOD Stop Code Lookup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'BSOD Stop Code Lookup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
