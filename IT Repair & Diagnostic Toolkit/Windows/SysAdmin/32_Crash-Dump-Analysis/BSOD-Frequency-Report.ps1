...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: BSOD Frequency Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for BSOD-Frequency-Report.
...
$exitCode = 0
$Activity = 'BSOD Frequency Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'BSOD-Frequency-Report'
        Add-ReportRow -Check 'Tool' -Result 'BSOD-Frequency-Report' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'BSOD Frequency Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'BSOD Frequency Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
