...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Crash Dump Archive Manager
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Crash-Dump-Archive-Manager.
...
.EXAMPLE
    .\Crash-Dump-Archive-Manager.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Crash-Dump-Archive-Manager.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Crash Dump Archive Manager'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Crash-Dump-Archive-Manager'
        Add-ReportRow -Check 'Tool' -Result 'Crash-Dump-Archive-Manager' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'Crash Dump Archive Manager' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Crash Dump Archive Manager' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
