...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Hardware Fault Attribution
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Hardware-Fault-Attribution.
...
.EXAMPLE
    .\Hardware-Fault-Attribution.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Hardware-Fault-Attribution.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Hardware Fault Attribution'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Hardware-Fault-Attribution'
        Add-ReportRow -Check 'Tool' -Result 'Hardware-Fault-Attribution' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'Hardware Fault Attribution' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Hardware Fault Attribution' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
