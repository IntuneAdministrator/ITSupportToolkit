...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Auto-Repair.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Auto Repair'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
