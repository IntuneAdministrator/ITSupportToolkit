...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Automated Symbol Download
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Automated-Symbol-Download.
...
.EXAMPLE
    .\Automated-Symbol-Download.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Automated-Symbol-Download.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Automated Symbol Download'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Automated-Symbol-Download'
        Add-ReportRow -Check 'Tool' -Result 'Automated-Symbol-Download' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'Automated Symbol Download' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Automated Symbol Download' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
