...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: WinDbg Automated Analysis
.DESCRIPTION
    Production-grade Windows SysAdmin tool for WinDbg-Automated-Analysis.
...
.EXAMPLE
    .\WinDbg-Automated-Analysis.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\WinDbg-Automated-Analysis.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'WinDbg Automated Analysis'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'WinDbg-Automated-Analysis'
        Add-ReportRow -Check 'Tool' -Result 'WinDbg-Automated-Analysis' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
    ...
    Export-HtmlReport -Title 'WinDbg Automated Analysis' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'WinDbg Automated Analysis' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
