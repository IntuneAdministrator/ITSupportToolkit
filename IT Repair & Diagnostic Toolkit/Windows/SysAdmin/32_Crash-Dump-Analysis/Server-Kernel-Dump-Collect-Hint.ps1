...
<#
.SYNOPSIS
    Common problem fix: Server Kernel Dump Collect Hint
.DESCRIPTION
...
.EXAMPLE
    .\Server-Kernel-Dump-Collect-Hint.ps1
.EXAMPLE
    .\Server-Kernel-Dump-Collect-Hint.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Server Kernel Dump Collect Hint'; $totalSteps = 8; $Name = 'Server-Kernel-Dump-Collect-Hint'
try {
    ...
    Export-HtmlReport -Title 'Server Kernel Dump Collect Hint' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Server Kernel Dump Collect Hint' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
