...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Evidence-Report.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Evidence Report'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
