...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Driver Verifier Log Extract
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Driver-Verifier-Log-Extract.
Category: 32_Crash-Dump-Analysis. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Driver-Verifier-Log-Extract.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Driver-Verifier-Log-Extract.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Driver Verifier Log Extract'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
        $Name = 'Driver-Verifier-Log-Extract'
        Add-ReportRow -Check 'Tool' -Result 'Driver-Verifier-Log-Extract' -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
        Write-SectionBanner 'CRASH DUMP ANALYSIS'
        $crashControl = Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\CrashControl' -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'CrashDumpEnabled' -Result "$($crashControl.CrashDumpEnabled)" -Status 'INFO' -Detail '0=None 1=Complete 2=Kernel 3=Small 7=Automatic'
        $mini = "$env:SystemRoot\Minidump"
        $memdmp = "$env:SystemRoot\MEMORY.DMP"
        $dumps = @()
        if (Test-Path $mini) { $dumps = @(Get-ChildItem $mini -Filter *.dmp -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending) }
        Add-ReportRow -Check 'Minidumps' -Result "$($dumps.Count)" -Status $(if ($dumps.Count -eq 0) {'PASS'} else {'WARN'}) -Detail (($dumps | Select-Object -First 8 Name) -join ', ')
        Add-ReportRow -Check 'MEMORY.DMP' -Result $(if (Test-Path $memdmp) { "{0:N1} MB" -f ((Get-Item $memdmp).Length/1MB) } else {'Absent'}) -Status 'INFO' -Detail $memdmp
        Write-ProgressStep 2 $totalSteps $Activity 'Dump inventory'

        if ($Name -match 'BSOD-Stop|Frequency|Trend|MiniDump|Memory-Dump|Pool|Hardware-Fault|WinDbg|Symbol|Verifier|Archive|Kernel-Memory') {
            $bugchecks = @(Get-WinEvent -FilterHashtable @{LogName='System'; ProviderName='Microsoft-Windows-WER-SystemErrorReporting'; StartTime=(Get-Date).AddDays(-90)} -MaxEvents 30 -ErrorAction SilentlyContinue)
            if (-not $bugchecks) {
                $bugchecks = @(Get-WinEvent -FilterHashtable @{LogName='System'; Id=1001; StartTime=(Get-Date).AddDays(-90)} -MaxEvents 30 -ErrorAction SilentlyContinue)
            }
            Add-ReportRow -Check 'Bugcheck-related events' -Result "$($bugchecks.Count) (90d)" -Status $(if ($bugchecks.Count -eq 0) {'PASS'} else {'WARN'}) -Detail ''
            foreach ($e in ($bugchecks | Select-Object -First 15)) {
                Add-ReportRow -Check "WER/Bugcheck $($e.TimeCreated.ToString('g'))" -Result "Id $($e.Id)" -Status 'WARN' -Detail ($e.Message.Substring(0, [Math]::Min(160, $e.Message.Length)))
            }

            if ($Name -match 'Kernel-Memory-Dump-Config' -and $PSCmdlet.ShouldProcess('CrashControl', 'Set automatic memory dump')) {
                Backup-RegistryKey -KeyPath 'HKLM:\SYSTEM\CurrentControlSet\Control\CrashControl' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
                Set-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\CrashControl' -Name CrashDumpEnabled -Value 7
                Add-ReportRow -Check 'CrashDumpEnabled' -Result '7 (Automatic)' -Status 'PASS' -Detail 'Rollback via .reg'
            }

            if ($Name -match 'Archive' -and $dumps.Count -gt 0 -and $PSCmdlet.ShouldProcess('Minidumps', 'Archive to C:\IT-Logs\CrashDumps')) {
                $arch = 'C:\IT-Logs\CrashDumps'
                New-Item $arch -ItemType Directory -Force | Out-Null
                Copy-Item ($dumps | Select-Object -First 20 | ForEach-Object FullName) $arch -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Dump archive' -Result $arch -Status 'PASS' -Detail 'Copied recent minidumps'
            }

            if ($Name -match 'WinDbg|Symbol') {
                $windbg = Get-Command windbg -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'WinDbg' -Result $(if ($windbg) { $windbg.Source } else {'Not in PATH'}) -Status 'INFO' -Detail 'Install Windows SDK / WinDbg Preview for automated analysis'
            }

            if ($Name -match 'Verifier-Log') {
                $vlog = & verifier /query 2>&1 | Out-String
                Add-ReportRow -Check 'Verifier query' -Result 'Captured' -Status 'INFO' -Detail ($vlog.Substring(0, [Math]::Min(300, $vlog.Length)))
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Crash analysis'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Driver Verifier Log Extract' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Driver Verifier Log Extract' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
