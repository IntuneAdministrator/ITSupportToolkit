...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Crash-Dump-Analysis-Stack-Reset.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Stack Reset'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
