...
<#
.SYNOPSIS
    Common problem fix: Crash Dump Analysis Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 32_Crash-Dump-Analysis). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Crash-Dump-Analysis-Policy-Validate.ps1
.EXAMPLE
    .\Crash-Dump-Analysis-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Crash Dump Analysis Policy Validate'; $totalSteps = 8; $Name = 'Crash-Dump-Analysis-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '32_Crash-Dump-Analysis' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '32_Crash-Dump-Analysis'
    ...
    Export-HtmlReport -Title 'Crash Dump Analysis Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Crash Dump Analysis Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
