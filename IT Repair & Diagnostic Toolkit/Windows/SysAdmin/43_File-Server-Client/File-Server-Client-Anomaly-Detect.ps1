...
<#
.SYNOPSIS
    Common problem fix: File Server Client Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Anomaly-Detect.ps1
.EXAMPLE
    .\File-Server-Client-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Anomaly Detect'; $totalSteps = 8; $Name = 'File-Server-Client-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
