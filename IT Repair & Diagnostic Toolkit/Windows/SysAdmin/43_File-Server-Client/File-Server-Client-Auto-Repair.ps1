...
<#
.SYNOPSIS
    Common problem fix: File Server Client Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Auto-Repair.ps1
.EXAMPLE
    .\File-Server-Client-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Auto Repair'; $totalSteps = 8; $Name = 'File-Server-Client-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
