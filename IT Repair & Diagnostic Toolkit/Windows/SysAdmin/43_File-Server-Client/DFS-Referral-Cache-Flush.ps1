...
<#
.SYNOPSIS
    Enterprise IT tool: DFS Referral Cache Flush
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\DFS-Referral-Cache-Flush.ps1
.EXAMPLE
    .\DFS-Referral-Cache-Flush.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='DFS Referral Cache Flush'; $totalSteps=8; $Name='DFS-Referral-Cache-Flush'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'DFS Referral Cache Flush' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DFS Referral Cache Flush' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
