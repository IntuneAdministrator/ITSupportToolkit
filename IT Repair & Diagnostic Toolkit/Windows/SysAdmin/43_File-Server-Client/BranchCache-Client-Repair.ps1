...
<#
.SYNOPSIS
    Enterprise IT tool: BranchCache Client Repair
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\BranchCache-Client-Repair.ps1
.EXAMPLE
    .\BranchCache-Client-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='BranchCache Client Repair'; $totalSteps=8; $Name='BranchCache-Client-Repair'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'BranchCache Client Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'BranchCache Client Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
