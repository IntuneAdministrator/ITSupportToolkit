...
<#
.SYNOPSIS
    Enterprise IT tool: File Lock Handle Finder
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\File-Lock-Handle-Finder.ps1
.EXAMPLE
    .\File-Lock-Handle-Finder.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='File Lock Handle Finder'; $totalSteps=8; $Name='File-Lock-Handle-Finder'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'File Lock Handle Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Lock Handle Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
