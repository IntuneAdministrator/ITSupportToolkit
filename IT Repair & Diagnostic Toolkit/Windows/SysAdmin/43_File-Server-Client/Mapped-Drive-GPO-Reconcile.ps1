...
<#
.SYNOPSIS
    Enterprise IT tool: Mapped Drive GPO Reconcile
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Mapped-Drive-GPO-Reconcile.ps1
.EXAMPLE
    .\Mapped-Drive-GPO-Reconcile.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Mapped Drive GPO Reconcile'; $totalSteps=8; $Name='Mapped-Drive-GPO-Reconcile'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'Mapped Drive GPO Reconcile' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Mapped Drive GPO Reconcile' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
