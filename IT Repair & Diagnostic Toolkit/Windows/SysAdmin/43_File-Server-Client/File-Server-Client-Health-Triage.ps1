...
<#
.SYNOPSIS
    Common problem fix: File Server Client Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Health-Triage.ps1
.EXAMPLE
    .\File-Server-Client-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Health Triage'; $totalSteps = 8; $Name = 'File-Server-Client-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
