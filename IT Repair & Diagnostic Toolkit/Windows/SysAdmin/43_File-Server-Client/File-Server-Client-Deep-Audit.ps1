...
<#
.SYNOPSIS
    Common problem fix: File Server Client Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Deep-Audit.ps1
.EXAMPLE
    .\File-Server-Client-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Deep Audit'; $totalSteps = 8; $Name = 'File-Server-Client-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
