...
<#
.SYNOPSIS
    Enterprise IT tool: SMB Multichannel Status
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\SMB-Multichannel-Status.ps1
.EXAMPLE
    .\SMB-Multichannel-Status.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='SMB Multichannel Status'; $totalSteps=8; $Name='SMB-Multichannel-Status'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'SMB Multichannel Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SMB Multichannel Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
