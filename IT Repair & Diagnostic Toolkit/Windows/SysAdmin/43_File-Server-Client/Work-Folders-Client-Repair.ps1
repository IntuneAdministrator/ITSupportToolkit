...
<#
.SYNOPSIS
    Enterprise IT tool: Work Folders Client Repair
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Work-Folders-Client-Repair.ps1
.EXAMPLE
    .\Work-Folders-Client-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Work Folders Client Repair'; $totalSteps=8; $Name='Work-Folders-Client-Repair'; $Folder='43_File-Server-Client'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '43_File-Server-Client' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'FILE SERVER CLIENT'
        try {
            $smb = Get-SmbClientConfiguration
            Add-ReportRow -Check 'SMB multichannel' -Result "$($smb.EnableMultiChannel)" -Status 'INFO' -Detail "BandwidthThresh=$($smb.ConnectionCountPerRssNetworkInterface)"
        } catch { Add-ReportRow -Check 'SMB client' -Result 'Error' -Status 'WARN' -Detail $_.Exception.Message }
        Write-ProgressStep 2 $totalSteps $Activity 'SMB'
        $csc = Get-Service CscService -EA SilentlyContinue
        if ($csc) {
            Add-ReportRow -Check 'Offline Files' -Result "$($csc.Status)/$($csc.StartType)" -Status 'INFO' -Detail ''
            if ($Name -match 'Offline|CSC' -and $PSCmdlet.ShouldProcess('CscService', 'Restart')) {
                Restart-Service CscService -Force -EA SilentlyContinue
                Add-ReportRow -Check 'CscService restart' -Result 'Done' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'BranchCache') {
            $bc = Get-Service PeerDistSvc -EA SilentlyContinue
            Add-ReportRow -Check 'BranchCache' -Result $(if($bc){$bc.Status}else{'N/A'}) -Status 'INFO' -Detail 'PeerDistSvc'
        }
        if ($Name -match 'DFS') {
            dfsutil.exe /pktinfo 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'DFS pktinfo' -Result 'Captured' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(300,$_.Length)) }
        }
        if ($Name -match 'WebDAV') {
            $w = Get-Service WebClient -EA SilentlyContinue
            Add-ReportRow -Check 'WebClient' -Result $(if($w){"$($w.Status)"}else{'Missing'}) -Status $(if($w -and $w.Status -eq 'Running'){'PASS'}else{'WARN'}) -Detail ''
            if ($w -and $PSCmdlet.ShouldProcess('WebClient', 'Restart')) { Restart-Service WebClient -Force -EA SilentlyContinue }
        }
        if ($Name -match 'Work-Folders') {
            Add-ReportRow -Check 'Work Folders' -Result (Get-Service WorkFoldersSvc -EA SilentlyContinue).Status -Status 'INFO' -Detail ''
        }
        if ($Name -match 'Mapped-Drive|File-Lock') {
            net use 2>&1 | Out-String | ForEach-Object { Add-ReportRow -Check 'net use' -Result 'Listed' -Status 'INFO' -Detail $_.Substring(0,[Math]::Min(400,$_.Length)) }
        }
        Write-ProgressStep 5 $totalSteps $Activity 'File client done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'Work Folders Client Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Work Folders Client Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
