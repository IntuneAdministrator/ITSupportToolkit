...
<#
.SYNOPSIS
    Common problem fix: File Server Client Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Evidence-Report.ps1
.EXAMPLE
    .\File-Server-Client-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Evidence Report'; $totalSteps = 8; $Name = 'File-Server-Client-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
