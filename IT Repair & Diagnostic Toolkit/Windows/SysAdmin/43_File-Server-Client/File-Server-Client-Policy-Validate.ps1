...
<#
.SYNOPSIS
    Common problem fix: File Server Client Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 43_File-Server-Client). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\File-Server-Client-Policy-Validate.ps1
.EXAMPLE
    .\File-Server-Client-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Policy Validate'; $totalSteps = 8; $Name = 'File-Server-Client-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '43_File-Server-Client' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '43_File-Server-Client'
    ...
    Export-HtmlReport -Title 'File Server Client Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
