...
<#
.SYNOPSIS
    Enterprise IT tool: Offline Files CSC Reset
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Offline-Files-CSC-Reset.ps1
.EXAMPLE
    .\Offline-Files-CSC-Reset.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Offline Files CSC Reset'; $totalSteps=8; $Name='Offline-Files-CSC-Reset'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'Offline Files CSC Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Offline Files CSC Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
