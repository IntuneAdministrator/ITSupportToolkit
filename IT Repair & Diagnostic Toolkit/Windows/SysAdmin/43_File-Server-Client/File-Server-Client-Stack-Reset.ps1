...
<#
.SYNOPSIS
    Common problem fix: File Server Client Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Stack-Reset.ps1
.EXAMPLE
    .\File-Server-Client-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Stack Reset'; $totalSteps = 8; $Name = 'File-Server-Client-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
