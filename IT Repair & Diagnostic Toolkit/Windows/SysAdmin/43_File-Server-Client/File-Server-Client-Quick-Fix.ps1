...
<#
.SYNOPSIS
    Common problem fix: File Server Client Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\File-Server-Client-Quick-Fix.ps1
.EXAMPLE
    .\File-Server-Client-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'File Server Client Quick Fix'; $totalSteps = 8; $Name = 'File-Server-Client-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'File Server Client Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'File Server Client Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
