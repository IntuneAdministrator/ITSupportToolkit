...
<#
.SYNOPSIS
    Enterprise IT tool: WebDAV Redirector Repair
.DESCRIPTION
    Phase 10 expansion (SysAdmin/43_File-Server-Client). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\WebDAV-Redirector-Repair.ps1
.EXAMPLE
    .\WebDAV-Redirector-Repair.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='WebDAV Redirector Repair'; $totalSteps=8; $Name='WebDAV-Redirector-Repair'; $Folder='43_File-Server-Client'
try {
    ...
    Export-HtmlReport -Title 'WebDAV Redirector Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'WebDAV Redirector Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
