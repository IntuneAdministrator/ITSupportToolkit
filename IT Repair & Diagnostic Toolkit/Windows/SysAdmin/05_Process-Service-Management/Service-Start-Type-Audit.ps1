...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Start Type Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Start-Type-Audit.
...
.EXAMPLE
    .\Service-Start-Type-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Service-Start-Type-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Service Start Type Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Service-Start-Type-Audit'
    ...
    Export-HtmlReport -Title 'Service Start Type Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Start Type Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
