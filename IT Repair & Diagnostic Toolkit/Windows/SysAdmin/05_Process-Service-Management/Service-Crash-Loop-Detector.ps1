...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Crash Loop Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Crash-Loop-Detector.
...
.EXAMPLE
    .\Service-Crash-Loop-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Service-Crash-Loop-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Service Crash Loop Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Service-Crash-Loop-Detector'
    ...
    Export-HtmlReport -Title 'Service Crash Loop Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Crash Loop Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
