...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Process CPU Affinity Manager
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Process-CPU-Affinity-Manager.
...
.EXAMPLE
    .\Process-CPU-Affinity-Manager.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Process-CPU-Affinity-Manager.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Process CPU Affinity Manager'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Process-CPU-Affinity-Manager'
    ...
    Export-HtmlReport -Title 'Process CPU Affinity Manager' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Process CPU Affinity Manager' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
