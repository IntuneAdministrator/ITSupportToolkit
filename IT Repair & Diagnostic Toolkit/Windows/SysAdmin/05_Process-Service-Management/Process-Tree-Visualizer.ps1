...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Process Tree Visualizer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Process-Tree-Visualizer.
...
.EXAMPLE
    .\Process-Tree-Visualizer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Process-Tree-Visualizer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Process Tree Visualizer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Process-Tree-Visualizer'
    ...
    Export-HtmlReport -Title 'Process Tree Visualizer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Process Tree Visualizer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
