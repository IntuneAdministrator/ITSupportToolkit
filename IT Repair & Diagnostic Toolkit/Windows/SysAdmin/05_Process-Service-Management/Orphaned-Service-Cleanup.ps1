...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Orphaned Service Cleanup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Orphaned-Service-Cleanup.
...
.EXAMPLE
    .\Orphaned-Service-Cleanup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Orphaned-Service-Cleanup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Orphaned Service Cleanup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Orphaned-Service-Cleanup'
    ...
    Export-HtmlReport -Title 'Orphaned Service Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Orphaned Service Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
