...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Account Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Account-Audit.
...
.EXAMPLE
    .\Service-Account-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Service Account Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Service-Account-Audit'
    ...
    Export-HtmlReport -Title 'Service Account Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Account Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
