...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Critical Service Restart
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Critical-Service-Restart.
...
.EXAMPLE
    .\Critical-Service-Restart.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Critical-Service-Restart.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Critical Service Restart'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Critical-Service-Restart'
    ...
    Export-HtmlReport -Title 'Critical Service Restart' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Critical Service Restart' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
