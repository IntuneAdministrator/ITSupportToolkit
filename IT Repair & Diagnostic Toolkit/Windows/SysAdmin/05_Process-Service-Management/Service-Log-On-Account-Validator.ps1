...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Log On Account Validator
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Log-On-Account-Validator.
...
.EXAMPLE
    .\Service-Log-On-Account-Validator.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Service-Log-On-Account-Validator.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Service Log On Account Validator'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Service-Log-On-Account-Validator'
    ...
    Export-HtmlReport -Title 'Service Log On Account Validator' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Log On Account Validator' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
