...
<#
.SYNOPSIS
    Common problem fix: Process Service Management Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Process-Service-Management-Stack-Reset.ps1
.EXAMPLE
    .\Process-Service-Management-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Process Service Management Stack Reset'; $totalSteps = 8; $Name = 'Process-Service-Management-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Process Service Management Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Process Service Management Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
