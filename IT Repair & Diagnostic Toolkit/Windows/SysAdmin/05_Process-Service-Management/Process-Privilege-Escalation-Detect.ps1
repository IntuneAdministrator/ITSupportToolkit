...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Process Privilege Escalation Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Process-Privilege-Escalation-Detect.
...
.EXAMPLE
    .\Process-Privilege-Escalation-Detect.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Process-Privilege-Escalation-Detect.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Process Privilege Escalation Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Process-Privilege-Escalation-Detect'
        ...
        foreach ($p in $procs) {
            Add-ReportRow -Check "Process $($p.ProcessName)" -Result ("{0:N0} MB" -f ($p.WorkingSet64/1MB)) -Status $(if (($p.WorkingSet64/1MB) -gt 2000) {'WARN'} else {'INFO'}) -Detail "PID=$($p.Id) Handles=$($p.Handles)"
        }
        $notResponding = @(Get-Process | Where-Object { $_.Responding -eq $false -and $_.ProcessName -notmatch 'Idle|System' })
        foreach ($nr in $notResponding) {
            Add-ReportRow -Check "Not responding $($nr.ProcessName)" -Result "PID $($nr.Id)" -Status 'WARN' -Detail 'Consider Hung-Process-Killer with -WhatIf first'
            if ($Name -match 'Hung-Process' -and $PSCmdlet.ShouldProcess("$($nr.ProcessName) ($($nr.Id))", 'Stop-Process')) {
                Stop-Process -Id $nr.Id -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check "Killed $($nr.ProcessName)" -Result 'Stopped' -Status 'PASS' -Detail "PID $($nr.Id)"
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Processes'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Process Privilege Escalation Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Process Privilege Escalation Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
