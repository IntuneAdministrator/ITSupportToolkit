...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Zombie Process Detector
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Zombie-Process-Detector.
...
.EXAMPLE
    .\Zombie-Process-Detector.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Zombie-Process-Detector.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Zombie Process Detector'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Zombie-Process-Detector'
    ...
    Export-HtmlReport -Title 'Zombie Process Detector' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Zombie Process Detector' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
