...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Recovery Action Configurator
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Recovery-Action-Configurator.
Category: 05_Process-Service-Management. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\Service-Recovery-Action-Configurator.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Service-Recovery-Action-Configurator.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Service Recovery Action Configurator'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Service-Recovery-Action-Configurator'
        $services = @(Get-Service -ErrorAction SilentlyContinue)
        $running = @($services | Where-Object Status -eq 'Running').Count
        Add-ReportRow -Check 'Services running' -Result "$running / $($services.Count)" -Status 'INFO' -Detail 'Get-Service snapshot'
        Write-ProgressStep 2 $totalSteps $Activity 'Service snapshot'
        $critical = @('WinManage','RpcSs','EventLog','Dhcp','Dnscache','nsi','gpsvc','WinDefend','mpssvc','LanmanServer','LanmanWorkstation','Schedule','ProfSvc')
        # Fix typo WinManage -> Winmgmt
        $critical = @('Winmgmt','RpcSs','EventLog','Dhcp','Dnscache','nsi','gpsvc','Schedule','ProfSvc','LanmanWorkstation')
        foreach ($c in $critical) {
            $s = Get-Service -Name $c -ErrorAction SilentlyContinue
            if (-not $s) { Add-ReportRow -Check "Service $c" -Result 'Not found' -Status 'INFO' -Detail ''; continue }
            Add-ReportRow -Check "Service $c" -Result "$($s.Status)/$($s.StartType)" -Status $(if ($s.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail $s.DisplayName
            if ($s.Status -ne 'Running' -and $s.StartType -ne 'Disabled' -and $PSCmdlet.ShouldProcess($c, 'Start service')) {
                try { Start-Service $c -ErrorAction Stop; Add-ReportRow -Check "Start $c" -Result 'Started' -Status 'PASS' -Detail '' } catch {
                    Add-ReportRow -Check "Start $c" -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Critical services'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Service Recovery Action Configurator' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Recovery Action Configurator' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
