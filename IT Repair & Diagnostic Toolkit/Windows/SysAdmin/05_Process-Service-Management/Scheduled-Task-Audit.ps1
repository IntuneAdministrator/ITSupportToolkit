...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Scheduled Task Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Scheduled-Task-Audit.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Scheduled-Task-Audit'
        ...
        $tasks = @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object State -eq 'Ready')
        $failed = @(Get-ScheduledTask | ForEach-Object {
            $i = $_ | Get-ScheduledTaskInfo
            if ($i.LastTaskResult -ne 0 -and $i.LastTaskResult -ne 267009) { [pscustomobject]@{ Task=$_.TaskName; Result=$i.LastTaskResult } }
        } | Select-Object -First 20)
        Add-ReportRow -Check 'Ready tasks' -Result "$($tasks.Count)" -Status 'INFO' -Detail ''
        foreach ($f in $failed) {
            Add-ReportRow -Check "Task fail $($f.Task)" -Result "Code $($f.Result)" -Status 'WARN' -Detail 'LastTaskResult'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Tasks'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Scheduled Task Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Scheduled Task Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
