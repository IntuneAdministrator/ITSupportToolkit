...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Critical Process Watchdog Setup
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Critical-Process-Watchdog-Setup.
...
.EXAMPLE
    .\Critical-Process-Watchdog-Setup.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Critical-Process-Watchdog-Setup.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Critical Process Watchdog Setup'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Critical-Process-Watchdog-Setup'
        ...
        Add-ReportRow -Check 'Process count' -Result "$((Get-Process).Count)" -Status 'INFO' -Detail 'Get-Process'
        Write-ProgressStep 3 $totalSteps $Activity 'Process count'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Critical Process Watchdog Setup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Critical Process Watchdog Setup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
