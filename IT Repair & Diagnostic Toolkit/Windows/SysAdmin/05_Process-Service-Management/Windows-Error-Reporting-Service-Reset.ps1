...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Error Reporting Service Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Error-Reporting-Service-Reset.
...
.EXAMPLE
    .\Windows-Error-Reporting-Service-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Error-Reporting-Service-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Error Reporting Service Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Windows-Error-Reporting-Service-Reset'
        ...
        if ($PSCmdlet.ShouldProcess('WerSvc', 'Restart Windows Error Reporting')) {
            Restart-Service WerSvc -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'WerSvc' -Result (Get-Service WerSvc).Status -Status 'PASS' -Detail 'Restarted'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'WER'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Windows Error Reporting Service Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Error Reporting Service Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
