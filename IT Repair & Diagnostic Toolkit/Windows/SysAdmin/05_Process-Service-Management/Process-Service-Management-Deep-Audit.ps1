...
<#
.SYNOPSIS
    Common problem fix: Process Service Management Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Process-Service-Management-Deep-Audit.ps1
.EXAMPLE
    .\Process-Service-Management-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Process Service Management Deep Audit'; $totalSteps = 8; $Name = 'Process-Service-Management-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Process Service Management Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Process Service Management Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
