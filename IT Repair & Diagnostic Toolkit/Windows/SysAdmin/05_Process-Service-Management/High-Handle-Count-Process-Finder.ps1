...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: High Handle Count Process Finder
.DESCRIPTION
    Production-grade Windows SysAdmin tool for High-Handle-Count-Process-Finder.
...
.EXAMPLE
    .\High-Handle-Count-Process-Finder.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\High-Handle-Count-Process-Finder.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'High Handle Count Process Finder'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — High-Handle-Count-Process-Finder'
    ...
    Export-HtmlReport -Title 'High Handle Count Process Finder' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'High Handle Count Process Finder' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
