...
<#
.SYNOPSIS
    Common problem fix: Process Service Management Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 05_Process-Service-Management). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Process-Service-Management-Policy-Validate.ps1
.EXAMPLE
    .\Process-Service-Management-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Process Service Management Policy Validate'; $totalSteps = 8; $Name = 'Process-Service-Management-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '05_Process-Service-Management' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '05_Process-Service-Management'
    ...
    Export-HtmlReport -Title 'Process Service Management Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Process Service Management Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
