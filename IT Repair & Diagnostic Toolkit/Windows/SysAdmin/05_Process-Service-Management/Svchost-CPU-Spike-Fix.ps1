...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Svchost CPU Spike Fix
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Svchost-CPU-Spike-Fix.
...
$exitCode = 0
$Activity = 'Svchost CPU Spike Fix'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Svchost-CPU-Spike-Fix'
        ...
        $svchost = Get-Process svchost -ErrorAction SilentlyContinue | Sort-Object CPU -Descending | Select-Object -First 10
        foreach ($p in $svchost) {
            Add-ReportRow -Check "svchost PID $($p.Id)" -Result ("CPU={0:N1} WS={1:N0}MB" -f $p.CPU, ($p.WorkingSet64/1MB)) -Status $(if ($p.CPU -gt 100) {'WARN'} else {'INFO'}) -Detail ''
        }
        Write-ProgressStep 3 $totalSteps $Activity 'svchost'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Svchost CPU Spike Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Svchost CPU Spike Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
