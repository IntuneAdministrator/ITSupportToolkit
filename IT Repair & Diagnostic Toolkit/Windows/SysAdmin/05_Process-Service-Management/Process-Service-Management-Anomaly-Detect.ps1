...
<#
.SYNOPSIS
    Common problem fix: Process Service Management Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Process-Service-Management-Anomaly-Detect.ps1
.EXAMPLE
    .\Process-Service-Management-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Process Service Management Anomaly Detect'; $totalSteps = 8; $Name = 'Process-Service-Management-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Process Service Management Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Process Service Management Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
