...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Service Dependency Map
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Service-Dependency-Map.
...
$exitCode = 0
$Activity = 'Service Dependency Map'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'PROCESS / SERVICE — Service-Dependency-Map'
    ...
    Export-HtmlReport -Title 'Service Dependency Map' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Service Dependency Map' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
