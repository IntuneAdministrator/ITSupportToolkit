...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Activation Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Activation-Repair.
...
.EXAMPLE
    .\Windows-Activation-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Activation-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Activation Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Activation-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Activation-Repair' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'Windows Activation Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Activation Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
