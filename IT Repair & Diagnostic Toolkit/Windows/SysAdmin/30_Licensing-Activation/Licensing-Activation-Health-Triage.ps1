...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Health-Triage.ps1
.EXAMPLE
    .\Licensing-Activation-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Health Triage'; $totalSteps = 8; $Name = 'Licensing-Activation-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
