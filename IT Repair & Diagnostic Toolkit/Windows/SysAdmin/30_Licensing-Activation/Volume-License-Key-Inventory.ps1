...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Volume License Key Inventory
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Volume-License-Key-Inventory.
...
.EXAMPLE
    .\Volume-License-Key-Inventory.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Volume-License-Key-Inventory.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Volume License Key Inventory'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Volume-License-Key-Inventory'
        Add-ReportRow -Check 'Tool' -Result 'Volume-License-Key-Inventory' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'Volume License Key Inventory' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Volume License Key Inventory' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
