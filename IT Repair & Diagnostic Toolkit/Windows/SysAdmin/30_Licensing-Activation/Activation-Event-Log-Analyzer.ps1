...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Activation Event Log Analyzer
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Activation-Event-Log-Analyzer.
...
.EXAMPLE
    .\Activation-Event-Log-Analyzer.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Activation-Event-Log-Analyzer.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Activation Event Log Analyzer'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Activation-Event-Log-Analyzer'
        Add-ReportRow -Check 'Tool' -Result 'Activation-Event-Log-Analyzer' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'Activation Event Log Analyzer' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Activation Event Log Analyzer' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
