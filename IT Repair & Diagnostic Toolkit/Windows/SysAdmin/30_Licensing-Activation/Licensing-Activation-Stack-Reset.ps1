...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Stack-Reset.ps1
.EXAMPLE
    .\Licensing-Activation-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Stack Reset'; $totalSteps = 8; $Name = 'Licensing-Activation-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
