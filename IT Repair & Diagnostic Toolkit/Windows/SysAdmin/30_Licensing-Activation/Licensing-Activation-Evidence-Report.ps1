...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 30_Licensing-Activation). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Licensing-Activation-Evidence-Report.ps1
.EXAMPLE
    .\Licensing-Activation-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Evidence Report'; $totalSteps = 8; $Name = 'Licensing-Activation-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '30_Licensing-Activation' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '30_Licensing-Activation'
    ...
    Export-HtmlReport -Title 'Licensing Activation Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
