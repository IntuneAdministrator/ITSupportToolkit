...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Quick-Fix.ps1
.EXAMPLE
    .\Licensing-Activation-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Quick Fix'; $totalSteps = 8; $Name = 'Licensing-Activation-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
