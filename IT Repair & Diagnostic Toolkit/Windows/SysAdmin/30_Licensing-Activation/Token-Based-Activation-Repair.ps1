...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Token Based Activation Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Token-Based-Activation-Repair.
...
.EXAMPLE
    .\Token-Based-Activation-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Token-Based-Activation-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Token Based Activation Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Token-Based-Activation-Repair'
        Add-ReportRow -Check 'Tool' -Result 'Token-Based-Activation-Repair' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'Token Based Activation Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Token Based Activation Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
