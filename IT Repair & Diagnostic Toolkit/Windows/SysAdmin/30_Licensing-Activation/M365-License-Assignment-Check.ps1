...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: M365 License Assignment Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for M365-License-Assignment-Check.
Category: 30_Licensing-Activation. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\M365-License-Assignment-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\M365-License-Assignment-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'M365 License Assignment Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'LICENSING ACTIVATION'
        $Name = 'M365-License-Assignment-Check'
        Add-ReportRow -Check 'Tool' -Result 'M365-License-Assignment-Check' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
        $slmgr = & cscript.exe //Nologo "$env:SystemRoot\System32\slmgr.vbs" /dli 2>&1 | Out-String
        Add-ReportRow -Check 'slmgr /dli' -Result 'Captured' -Status 'INFO' -Detail ($slmgr.Substring(0, [Math]::Min(500, $slmgr.Length)))
        $xpr = & cscript.exe //Nologo "$env:SystemRoot\System32\slmgr.vbs" /xpr 2>&1 | Out-String
        $licensed = $xpr -match 'permanently|licensed'
        Add-ReportRow -Check 'Activation' -Result ($xpr.Trim()) -Status $(if ($licensed) {'PASS'} else {'WARN'}) -Detail ''
        Write-ProgressStep 2 $totalSteps $Activity 'License status'

        if ($Name -match 'KMS|Token-Based|ADBA|Server-Connectivity') {
            $dns = & cscript.exe //Nologo "$env:SystemRoot\System32\slmgr.vbs" /dlv 2>&1 | Out-String
            Add-ReportRow -Check 'slmgr /dlv' -Result 'Detailed' -Status 'INFO' -Detail ($dns.Substring(0, [Math]::Min(500, $dns.Length)))
            if ($dns -match 'Key Management Service') {
                Add-ReportRow -Check 'KMS channel' -Result 'Detected in dlv' -Status 'INFO' -Detail ''
            }
            if ($Join.IsDomainJoined) {
                try {
                    $kms = Resolve-DnsName -Name ("_vlmcs._tcp.{0}" -f $Join.DomainName) -Type SRV -ErrorAction Stop
                    Add-ReportRow -Check 'KMS SRV DNS' -Result "$($kms.Count) records" -Status 'PASS' -Detail (($kms | ForEach-Object NameTarget) -join ', ')
                } catch {
                    Add-ReportRow -Check 'KMS SRV DNS' -Result 'Not found' -Status 'WARN' -Detail $_.Exception.Message
                }
            }
        }

        if ($Name -match 'SLMGR-Full-Reset|Activation-Repair' -and $PSCmdlet.ShouldProcess('Software Licensing', 'slmgr /rearm + ato')) {
            & cscript.exe //Nologo "$env:SystemRoot\System32\slmgr.vbs" /rearm 2>&1 | Out-Null
            & cscript.exe //Nologo "$env:SystemRoot\System32\slmgr.vbs" /ato 2>&1 | Out-Null
            Add-ReportRow -Check 'slmgr rearm/ato' -Result 'Invoked' -Status 'INFO' -Detail 'Reboot may be required after rearm'
        }

        if ($Name -match 'OEM|MAK|Volume-License|Grace') {
            $oa3 = & cscript.exe //Nologo "$env:SystemRoot\System32\slmgr.vbs" /dli 2>&1 | Out-String
            Add-ReportRow -Check 'Channel info' -Result 'See detail' -Status 'INFO' -Detail ($oa3.Substring(0, [Math]::Min(400, $oa3.Length)))
            if ($Name -match 'OEM-License-Key') {
                try {
                    $key = (Get-CimInstance -Query "SELECT * FROM SoftwareLicensingService").OA3xOriginalProductKey
                    Add-ReportRow -Check 'OA3 OEM key' -Result $(if ($key) { ($key.Substring(0,5) + '-*****-*****-*****-*****') } else {'Not in firmware'}) -Status 'INFO' -Detail 'Partial mask only - never log full key to shared reports if policy forbids'
                } catch {
                    Add-ReportRow -Check 'OA3 OEM key' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
                }
            }
        }

        if ($Name -match 'M365-License') {
            Add-ReportRow -Check 'M365 license' -Result 'Check Microsoft 365 admin center / dspp' -Status 'INFO' -Detail $Join.Label
            $ospp = Get-ChildItem "$env:ProgramFiles*\Microsoft Office\Office*\OSPP.VBS" -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($ospp) {
                $st = & cscript.exe //Nologo $ospp.FullName /dstatus 2>&1 | Out-String
                Add-ReportRow -Check 'OSPP dstatus' -Result 'Captured' -Status 'INFO' -Detail ($st.Substring(0, [Math]::Min(400, $st.Length)))
            }
        }

        if ($Name -match 'Activation-Event') {
            $ev = @(Get-WinEvent -FilterHashtable @{LogName='Application'; ProviderName='Microsoft-Windows-Security-SPP'; StartTime=(Get-Date).AddDays(-14)} -MaxEvents 20 -ErrorAction SilentlyContinue)
            Add-ReportRow -Check 'SPP events' -Result "$($ev.Count)" -Status 'INFO' -Detail ''
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Licensing'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'M365 License Assignment Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'M365 License Assignment Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
