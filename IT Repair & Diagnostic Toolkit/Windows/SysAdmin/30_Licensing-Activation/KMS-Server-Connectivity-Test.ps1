...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: KMS Server Connectivity Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for KMS-Server-Connectivity-Test.
...
.EXAMPLE
    .\KMS-Server-Connectivity-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\KMS-Server-Connectivity-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'KMS Server Connectivity Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'KMS-Server-Connectivity-Test'
        Add-ReportRow -Check 'Tool' -Result 'KMS-Server-Connectivity-Test' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'KMS Server Connectivity Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'KMS Server Connectivity Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
