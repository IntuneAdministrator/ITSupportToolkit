...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Policy-Validate.ps1
.EXAMPLE
    .\Licensing-Activation-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Policy Validate'; $totalSteps = 8; $Name = 'Licensing-Activation-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
