...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Anomaly-Detect.ps1
.EXAMPLE
    .\Licensing-Activation-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Anomaly Detect'; $totalSteps = 8; $Name = 'Licensing-Activation-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
