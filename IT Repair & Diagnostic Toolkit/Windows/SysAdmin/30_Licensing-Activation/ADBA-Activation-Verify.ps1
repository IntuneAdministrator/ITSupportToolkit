...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: ADBA Activation Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for ADBA-Activation-Verify.
...
$exitCode = 0
$Activity = 'ADBA Activation Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'ADBA-Activation-Verify'
        Add-ReportRow -Check 'Tool' -Result 'ADBA-Activation-Verify' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'ADBA Activation Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'ADBA Activation Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
