...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Deep-Audit.ps1
.EXAMPLE
    .\Licensing-Activation-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Deep Audit'; $totalSteps = 8; $Name = 'Licensing-Activation-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
