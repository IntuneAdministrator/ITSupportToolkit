...
<#
.SYNOPSIS
    Common problem fix: Licensing Activation Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Licensing-Activation-Auto-Repair.ps1
.EXAMPLE
    .\Licensing-Activation-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Licensing Activation Auto Repair'; $totalSteps = 8; $Name = 'Licensing-Activation-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Licensing Activation Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Licensing Activation Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
