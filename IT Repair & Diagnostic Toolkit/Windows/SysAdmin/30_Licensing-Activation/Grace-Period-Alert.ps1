...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Grace Period Alert
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Grace-Period-Alert.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Grace-Period-Alert' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'Grace Period Alert' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Grace Period Alert' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
