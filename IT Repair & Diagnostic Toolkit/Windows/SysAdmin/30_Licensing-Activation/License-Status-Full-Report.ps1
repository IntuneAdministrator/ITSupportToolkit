...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: License Status Full Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for License-Status-Full-Report.
...
.EXAMPLE
    .\License-Status-Full-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\License-Status-Full-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'License Status Full Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'License-Status-Full-Report'
        Add-ReportRow -Check 'Tool' -Result 'License-Status-Full-Report' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'License Status Full Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'License Status Full Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
