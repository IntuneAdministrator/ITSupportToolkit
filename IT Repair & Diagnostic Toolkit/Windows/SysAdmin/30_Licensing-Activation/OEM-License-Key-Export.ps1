...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: OEM License Key Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for OEM-License-Key-Export.
...
$exitCode = 0
$Activity = 'OEM License Key Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'OEM-License-Key-Export'
        Add-ReportRow -Check 'Tool' -Result 'OEM-License-Key-Export' -Status 'INFO' -Detail '30_Licensing-Activation'
        Write-SectionBanner 'LICENSING / ACTIVATION'
    ...
    Export-HtmlReport -Title 'OEM License Key Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'OEM License Key Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
