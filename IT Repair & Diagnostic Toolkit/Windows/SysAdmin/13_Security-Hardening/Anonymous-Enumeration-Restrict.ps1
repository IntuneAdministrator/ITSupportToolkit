...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Anonymous Enumeration Restrict
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Anonymous-Enumeration-Restrict.
...
.EXAMPLE
    .\Anonymous-Enumeration-Restrict.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Anonymous-Enumeration-Restrict.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Anonymous Enumeration Restrict'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Anonymous-Enumeration-Restrict'
        Add-ReportRow -Check 'Tool' -Result 'Anonymous-Enumeration-Restrict' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Anonymous Enumeration Restrict' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Anonymous Enumeration Restrict' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
