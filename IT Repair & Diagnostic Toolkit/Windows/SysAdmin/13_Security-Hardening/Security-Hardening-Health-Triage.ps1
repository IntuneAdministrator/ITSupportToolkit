...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Health-Triage.ps1
.EXAMPLE
    .\Security-Hardening-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Health Triage'; $totalSteps = 8; $Name = 'Security-Hardening-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
