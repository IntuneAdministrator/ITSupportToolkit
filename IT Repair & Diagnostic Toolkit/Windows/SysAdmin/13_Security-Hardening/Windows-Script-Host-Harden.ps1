...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Script Host Harden
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Script-Host-Harden.
...
.EXAMPLE
    .\Windows-Script-Host-Harden.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Script-Host-Harden.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Script Host Harden'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Script-Host-Harden'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Script-Host-Harden' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Windows Script Host Harden' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Script Host Harden' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
