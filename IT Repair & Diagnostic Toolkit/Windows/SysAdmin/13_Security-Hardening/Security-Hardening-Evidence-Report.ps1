...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 13_Security-Hardening). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Security-Hardening-Evidence-Report.ps1
.EXAMPLE
    .\Security-Hardening-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Evidence Report'; $totalSteps = 8; $Name = 'Security-Hardening-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '13_Security-Hardening' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Security Hardening Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
