...
<#
.SYNOPSIS
    Common problem fix: LLMNR Disable Check
.DESCRIPTION
...
$exitCode = 0; $Activity = 'LLMNR Disable Check'; $totalSteps = 8; $Name = 'LLMNR-Disable-Check'
try {
    ...
    Export-HtmlReport -Title 'LLMNR Disable Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'LLMNR Disable Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
