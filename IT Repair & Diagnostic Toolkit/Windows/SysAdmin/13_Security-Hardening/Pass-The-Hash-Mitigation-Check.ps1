...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Pass The Hash Mitigation Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Pass-The-Hash-Mitigation-Check.
...
.EXAMPLE
    .\Pass-The-Hash-Mitigation-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Pass-The-Hash-Mitigation-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Pass The Hash Mitigation Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Pass-The-Hash-Mitigation-Check'
        Add-ReportRow -Check 'Tool' -Result 'Pass-The-Hash-Mitigation-Check' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Pass The Hash Mitigation Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Pass The Hash Mitigation Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
