...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PowerShell Constrained Language Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PowerShell-Constrained-Language-Enforce.
Category: 13_Security-Hardening. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\PowerShell-Constrained-Language-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PowerShell-Constrained-Language-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PowerShell Constrained Language Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'SECURITY HARDENING'
        $Name = 'PowerShell-Constrained-Language-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'PowerShell-Constrained-Language-Enforce' -Status 'INFO' -Detail '13_Security-Hardening'
        Write-SectionBanner 'SECURITY BASELINE CHECKS'
        Get-NetFirewallProfile | ForEach-Object {
            Add-ReportRow -Check "Firewall $($_.Name)" -Result $(if ($_.Enabled) {'Enabled'} else {'Disabled'}) -Status $(if ($_.Enabled) {'PASS'} else {'FAIL'}) -Detail "In=$($_.DefaultInboundAction) Out=$($_.DefaultOutboundAction)"
            if (-not $_.Enabled -and $Name -match 'Firewall' -and $PSCmdlet.ShouldProcess($_.Name, 'Enable firewall profile')) {
                Set-NetFirewallProfile -Name $_.Name -Enabled True
                Add-ReportRow -Check "Enable $($_.Name)" -Result 'Enabled' -Status 'PASS' -Detail ''
            }
        }
        Write-ProgressStep 2 $totalSteps $Activity 'Firewall'
        ...
        Add-ReportRow -Check 'UAC EnableLUA' -Result "$uac" -Status $(if ($uac -eq 1) {'PASS'} else {'FAIL'}) -Detail '1=enabled'
        if ($Name -match 'UAC' -and $uac -ne 1 -and $PSCmdlet.ShouldProcess('UAC', 'Set EnableLUA=1')) {
            Backup-RegistryKey -KeyPath 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            Set-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name EnableLUA -Value 1
            Add-ReportRow -Check 'Enforce UAC' -Result 'EnableLUA=1' -Status 'PASS' -Detail 'Reboot may be required'
        }

        if ($Name -match 'SMB-Signing') {
            $smb = Get-SmbServerConfiguration -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'SMB RequireSecuritySignature' -Result "$($smb.RequireSecuritySignature)" -Status $(if ($smb.RequireSecuritySignature) {'PASS'} else {'WARN'}) -Detail "EncryptData=$($smb.EncryptData)"
            if ($PSCmdlet.ShouldProcess('SMB', 'RequireSecuritySignature=$true')) {
                Set-SmbServerConfiguration -RequireSecuritySignature $true -Force -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Enforce SMB signing' -Result 'Required' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'RDP-Security') {
            ...
            Add-ReportRow -Check 'RDP NLA' -Result "$nla" -Status $(if ($nla -eq 1) {'PASS'} else {'WARN'}) -Detail 'UserAuthentication=1'
            if ($PSCmdlet.ShouldProcess('RDP', 'Enable NLA')) {
                Backup-RegistryKey -KeyPath 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp' -BackupDir 'C:\IT-Logs\Backups' | Out-Null
                Set-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp' -Name UserAuthentication -Value 1
                Add-ReportRow -Check 'Enforce NLA' -Result 'Enabled' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'WSH') {
            $wsh = 'HKLM:\SOFTWARE\Microsoft\Windows Script Host\Settings'
            if (-not (Test-Path $wsh)) { New-Item $wsh -Force | Out-Null }
            Backup-RegistryKey -KeyPath $wsh -BackupDir 'C:\IT-Logs\Backups' | Out-Null
            if ($PSCmdlet.ShouldProcess('WSH', 'Disable Windows Script Host')) {
                New-ItemProperty $wsh -Name Enabled -Value 0 -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'WSH Enabled' -Result '0 (disabled)' -Status 'PASS' -Detail 'Rollback via .reg'
            }
        }
        if ($Name -match 'ASR|Exploit') {
            try {
                $pref = Get-MpPreference
                Add-ReportRow -Check 'ASR rules configured' -Result "$($pref.AttackSurfaceReductionRules_Ids.Count)" -Status 'INFO' -Detail (($pref.AttackSurfaceReductionRules_Ids | Select-Object -First 5) -join ', ')
            } catch {
                Add-ReportRow -Check 'ASR' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
            }
        }
        if ($Name -match 'AppLocker|WDAC') {
            try {
                $pol = Get-AppLockerPolicy -Effective -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'AppLocker effective' -Result $(if ($pol) {'Present'} else {'None'}) -Status 'INFO' -Detail 'Get-AppLockerPolicy -Effective'
            } catch {
                Add-ReportRow -Check 'AppLocker' -Result 'N/A' -Status 'INFO' -Detail $_.Exception.Message
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Hardening'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'PowerShell Constrained Language Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PowerShell Constrained Language Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
