...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Stack-Reset.ps1
.EXAMPLE
    .\Security-Hardening-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Stack Reset'; $totalSteps = 8; $Name = 'Security-Hardening-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
