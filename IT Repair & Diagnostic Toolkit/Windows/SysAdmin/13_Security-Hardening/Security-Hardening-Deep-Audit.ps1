...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Deep-Audit.ps1
.EXAMPLE
    .\Security-Hardening-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Deep Audit'; $totalSteps = 8; $Name = 'Security-Hardening-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
