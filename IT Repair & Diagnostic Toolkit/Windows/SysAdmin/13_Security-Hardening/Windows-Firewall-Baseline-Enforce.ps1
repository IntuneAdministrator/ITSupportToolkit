...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Windows Firewall Baseline Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Windows-Firewall-Baseline-Enforce.
...
.EXAMPLE
    .\Windows-Firewall-Baseline-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Windows-Firewall-Baseline-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Windows Firewall Baseline Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Windows-Firewall-Baseline-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'Windows-Firewall-Baseline-Enforce' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Windows Firewall Baseline Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Windows Firewall Baseline Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
