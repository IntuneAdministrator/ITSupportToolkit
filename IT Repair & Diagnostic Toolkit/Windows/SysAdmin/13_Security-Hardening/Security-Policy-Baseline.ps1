...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Security Policy Baseline
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Security-Policy-Baseline.
...
.EXAMPLE
    .\Security-Policy-Baseline.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Security-Policy-Baseline.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Security Policy Baseline'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'Security-Policy-Baseline' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Security Policy Baseline' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Security Policy Baseline' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
