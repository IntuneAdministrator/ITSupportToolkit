...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: BitLocker Status Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for BitLocker-Status-Report.
...
.EXAMPLE
    .\BitLocker-Status-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\BitLocker-Status-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'BitLocker Status Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Add-ReportRow -Check 'Tool' -Result 'BitLocker-Status-Report' -Status 'INFO' -Detail '13_Security-Hardening'
        Write-SectionBanner 'BITLOCKER'
        try {
            Get-BitLockerVolume | ForEach-Object {
                Add-ReportRow -Check "Volume $($_.MountPoint)" -Result $_.VolumeStatus -Status $(if ($_.ProtectionStatus -eq 'On') {'PASS'} else {'WARN'}) -Detail "Protection=$($_.ProtectionStatus) Encryption=$($_.EncryptionPercentage)%"
            }
        } catch {
            Add-ReportRow -Check 'BitLocker' -Result 'Unavailable' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 2 $totalSteps $Activity 'BitLocker'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'BitLocker Status Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'BitLocker Status Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
