...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Secure Boot Status Report
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Secure-Boot-Status-Report.
...
.EXAMPLE
    .\Secure-Boot-Status-Report.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Secure-Boot-Status-Report.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Secure Boot Status Report'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Secure-Boot-Status-Report'
        Add-ReportRow -Check 'Tool' -Result 'Secure-Boot-Status-Report' -Status 'INFO' -Detail '13_Security-Hardening'
        Write-SectionBanner 'PLATFORM SECURITY'
        try {
            $tpm = Get-Tpm
            Add-ReportRow -Check 'TPM present' -Result "$($tpm.TpmPresent)" -Status $(if ($tpm.TpmPresent) {'PASS'} else {'FAIL'}) -Detail "Ready=$($tpm.TpmReady) Enabled=$($tpm.TpmEnabled)"
        } catch {
            Add-ReportRow -Check 'TPM' -Result 'Query failed' -Status 'WARN' -Detail $_.Exception.Message
        }
        try {
            Confirm-SecureBootUEFI -ErrorAction Stop | Out-Null
            Add-ReportRow -Check 'Secure Boot' -Result 'Enabled' -Status 'PASS' -Detail 'Confirm-SecureBootUEFI'
        } catch {
            Add-ReportRow -Check 'Secure Boot' -Result 'Disabled or unsupported' -Status 'WARN' -Detail $_.Exception.Message
        }
        $dg = Get-CimInstance -ClassName Win32_DeviceGuard -Namespace root\Microsoft\Windows\DeviceGuard -ErrorAction SilentlyContinue
        if ($dg) {
            Add-ReportRow -Check 'Credential Guard' -Result "$($dg.SecurityServicesRunning -contains 1)" -Status 'INFO' -Detail "SecurityServicesConfigured=$($dg.SecurityServicesConfigured -join ',')"
        }
        if ($Name -match 'LSASS') {
            $backupDir = 'C:\IT-Logs\Backups'
            $lsassKey = 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa'
            Backup-RegistryKey -KeyPath $lsassKey -BackupDir $backupDir | Out-Null
            $runAsPPL = (Get-ItemProperty $lsassKey -Name RunAsPPL -ErrorAction SilentlyContinue).RunAsPPL
            Add-ReportRow -Check 'RunAsPPL' -Result $(if ($null -eq $runAsPPL) {'Not set'} else {"$runAsPPL"}) -Status $(if ($runAsPPL -eq 1) {'PASS'} else {'WARN'}) -Detail '1=LSASS as PPL'
            if ($PSCmdlet.ShouldProcess('LSASS PPL', 'Set RunAsPPL=1')) {
                New-ItemProperty $lsassKey -Name RunAsPPL -Value 1 -PropertyType DWord -Force | Out-Null
                Add-ReportRow -Check 'Enforce RunAsPPL' -Result 'Set to 1' -Status 'PASS' -Detail 'Reboot required; rollback via .reg'
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Platform security'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Secure Boot Status Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Secure Boot Status Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
