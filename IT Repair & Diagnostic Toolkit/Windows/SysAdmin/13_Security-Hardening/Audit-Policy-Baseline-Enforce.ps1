...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Audit Policy Baseline Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Audit-Policy-Baseline-Enforce.
...
.EXAMPLE
    .\Audit-Policy-Baseline-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Audit-Policy-Baseline-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Audit Policy Baseline Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Audit-Policy-Baseline-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'Audit-Policy-Baseline-Enforce' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'Audit Policy Baseline Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Audit Policy Baseline Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
