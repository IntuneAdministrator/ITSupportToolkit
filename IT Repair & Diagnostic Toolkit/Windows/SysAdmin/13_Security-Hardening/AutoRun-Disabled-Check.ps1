...
<#
.SYNOPSIS
    Common problem fix: AutoRun Disabled Check
.DESCRIPTION
...
.EXAMPLE
    .\AutoRun-Disabled-Check
.EXAMPLE
    .\AutoRun-Disabled-Check -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'AutoRun Disabled Check'; $totalSteps = 8; $Name = 'AutoRun-Disabled-Check'
try {
    ...
    Export-HtmlReport -Title 'AutoRun Disabled Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'AutoRun Disabled Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
