...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Quick-Fix.ps1
.EXAMPLE
    .\Security-Hardening-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Quick Fix'; $totalSteps = 8; $Name = 'Security-Hardening-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
