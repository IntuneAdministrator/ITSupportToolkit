...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Guest Account Disable Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Guest-Account-Disable-Verify.
...
.EXAMPLE
    .\Guest-Account-Disable-Verify.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Guest-Account-Disable-Verify.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Guest Account Disable Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Guest-Account-Disable-Verify'
        Add-ReportRow -Check 'Tool' -Result 'Guest-Account-Disable-Verify' -Status 'INFO' -Detail '13_Security-Hardening'
        Write-SectionBanner 'LOCAL ADMIN / ACCOUNTS'
        try {
            $admins = Get-LocalGroupMember -Group 'Administrators' -ErrorAction Stop
            foreach ($a in $admins) {
                Add-ReportRow -Check 'Administrators member' -Result $a.Name -Status 'INFO' -Detail $a.ObjectClass
            }
            Add-ReportRow -Check 'Admin count' -Result "$($admins.Count)" -Status $(if ($admins.Count -le 3) {'PASS'} else {'WARN'}) -Detail 'Review unexpected members'
        } catch {
            Add-ReportRow -Check 'Local Administrators' -Result 'Query failed' -Status 'WARN' -Detail $_.Exception.Message
        }
        try {
            $guest = Get-LocalUser -Name 'Guest' -ErrorAction SilentlyContinue
            if ($guest) {
                Add-ReportRow -Check 'Guest account' -Result $(if ($guest.Enabled) {'Enabled'} else {'Disabled'}) -Status $(if (-not $guest.Enabled) {'PASS'} else {'FAIL'}) -Detail ''
                if ($guest.Enabled -and $PSCmdlet.ShouldProcess('Guest', 'Disable-LocalUser')) {
                    Disable-LocalUser -Name 'Guest'
                    Add-ReportRow -Check 'Disable Guest' -Result 'Disabled' -Status 'PASS' -Detail ''
                }
            }
        } catch {}
        if ($Name -match 'LAPS') {
            ...
            Add-ReportRow -Check 'Windows LAPS config' -Result $(if ($laps) {'Present'} else {'Not found'}) -Status 'INFO' -Detail 'HKLM\...\LAPS\Config'
            Add-ReportRow -Check 'Legacy AdmPwd' -Result $(if ($legacy) {'Present'} else {'Not found'}) -Status 'INFO' -Detail 'Legacy LAPS GPO'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Accounts'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Guest Account Disable Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Guest Account Disable Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
