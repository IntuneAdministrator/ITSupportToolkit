...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: RDP Security Layer Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for RDP-Security-Layer-Enforce.
...
.EXAMPLE
    .\RDP-Security-Layer-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\RDP-Security-Layer-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'RDP Security Layer Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'RDP-Security-Layer-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'RDP-Security-Layer-Enforce' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'RDP Security Layer Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'RDP Security Layer Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
