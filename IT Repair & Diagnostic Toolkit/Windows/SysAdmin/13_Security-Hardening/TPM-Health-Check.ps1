...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: TPM Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for TPM-Health-Check.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'TPM-Health-Check'
        Add-ReportRow -Check 'Tool' -Result 'TPM-Health-Check' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'TPM Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'TPM Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
