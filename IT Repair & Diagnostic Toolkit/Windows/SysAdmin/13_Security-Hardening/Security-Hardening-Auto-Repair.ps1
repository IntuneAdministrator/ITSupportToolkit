...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Auto-Repair.ps1
.EXAMPLE
    .\Security-Hardening-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Auto Repair'; $totalSteps = 8; $Name = 'Security-Hardening-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
