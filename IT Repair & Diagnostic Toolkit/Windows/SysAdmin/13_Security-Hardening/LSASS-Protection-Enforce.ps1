...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: LSASS Protection Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for LSASS-Protection-Enforce.
...
.EXAMPLE
    .\LSASS-Protection-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\LSASS-Protection-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'LSASS Protection Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'LSASS-Protection-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'LSASS-Protection-Enforce' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'LSASS Protection Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'LSASS Protection Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
