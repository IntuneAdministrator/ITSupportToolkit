...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Policy-Validate.ps1
.EXAMPLE
    .\Security-Hardening-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Policy Validate'; $totalSteps = 8; $Name = 'Security-Hardening-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
