...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: AppLocker Rule Export
.DESCRIPTION
    Production-grade Windows SysAdmin tool for AppLocker-Rule-Export.
...
$exitCode = 0
$Activity = 'AppLocker Rule Export'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'AppLocker-Rule-Export'
        Add-ReportRow -Check 'Tool' -Result 'AppLocker-Rule-Export' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'AppLocker Rule Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'AppLocker Rule Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
