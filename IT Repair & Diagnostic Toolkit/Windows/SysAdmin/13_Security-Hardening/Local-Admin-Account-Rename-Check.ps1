...
<#
.SYNOPSIS
    Common problem fix: Local Admin Account Rename Check
.DESCRIPTION
...
.EXAMPLE
    .\Local-Admin-Account-Rename-Check
.EXAMPLE
    .\Local-Admin-Account-Rename-Check -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Local Admin Account Rename Check'; $totalSteps = 8; $Name = 'Local-Admin-Account-Rename-Check'
try {
    ...
    Export-HtmlReport -Title 'Local Admin Account Rename Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Local Admin Account Rename Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
