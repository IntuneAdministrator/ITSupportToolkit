...
<#
.SYNOPSIS
    Common problem fix: NetBIOS Over TCP Disable Check
.DESCRIPTION
...
.EXAMPLE
    .\NetBIOS-Over-TCP-Disable-Check
.EXAMPLE
    .\NetBIOS-Over-TCP-Disable-Check -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'NetBIOS Over TCP Disable Check'; $totalSteps = 8; $Name = 'NetBIOS-Over-TCP-Disable-Check'
try {
    ...
    Export-HtmlReport -Title 'NetBIOS Over TCP Disable Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'NetBIOS Over TCP Disable Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
