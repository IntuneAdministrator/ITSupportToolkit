...
<#
.SYNOPSIS
    Common problem fix: Guest Account Disabled Check
.DESCRIPTION
...
.EXAMPLE
    .\Guest-Account-Disabled-Check
.EXAMPLE
    .\Guest-Account-Disabled-Check -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Guest Account Disabled Check'; $totalSteps = 8; $Name = 'Guest-Account-Disabled-Check'
try {
    ...
    Export-HtmlReport -Title 'Guest Account Disabled Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Guest Account Disabled Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
