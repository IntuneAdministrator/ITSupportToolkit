...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NTLM Restriction Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NTLM-Restriction-Audit.
...
$exitCode = 0
$Activity = 'NTLM Restriction Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'NTLM-Restriction-Audit'
        Add-ReportRow -Check 'Tool' -Result 'NTLM-Restriction-Audit' -Status 'INFO' -Detail '13_Security-Hardening'
    ...
    Export-HtmlReport -Title 'NTLM Restriction Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NTLM Restriction Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
