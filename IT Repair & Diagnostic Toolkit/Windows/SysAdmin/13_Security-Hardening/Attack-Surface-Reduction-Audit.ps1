...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Attack Surface Reduction Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Attack-Surface-Reduction-Audit.
...
.EXAMPLE
    .\Attack-Surface-Reduction-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Attack-Surface-Reduction-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Attack Surface Reduction Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Attack-Surface-Reduction-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Attack-Surface-Reduction-Audit' -Status 'INFO' -Detail '13_Security-Hardening'
        Add-ReportRow -Check 'Security context' -Result $Join.Label -Status 'INFO' -Detail $env:COMPUTERNAME
        Write-ProgressStep 2 $totalSteps $Activity 'Context'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'Attack Surface Reduction Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Attack Surface Reduction Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
