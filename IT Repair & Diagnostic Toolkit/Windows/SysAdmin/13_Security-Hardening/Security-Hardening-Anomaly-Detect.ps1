...
<#
.SYNOPSIS
    Common problem fix: Security Hardening Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Security-Hardening-Anomaly-Detect.ps1
.EXAMPLE
    .\Security-Hardening-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Security Hardening Anomaly Detect'; $totalSteps = 8; $Name = 'Security-Hardening-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Security Hardening Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Security Hardening Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
