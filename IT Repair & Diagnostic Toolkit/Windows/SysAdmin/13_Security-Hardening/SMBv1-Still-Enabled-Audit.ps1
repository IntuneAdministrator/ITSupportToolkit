...
<#
.SYNOPSIS
    Common problem fix: SMBv1 Still Enabled Audit
.DESCRIPTION
...
.EXAMPLE
    .\SMBv1-Still-Enabled-Audit
.EXAMPLE
    .\SMBv1-Still-Enabled-Audit -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'SMBv1 Still Enabled Audit'; $totalSteps = 8; $Name = 'SMBv1-Still-Enabled-Audit'
try {
    ...
    Export-HtmlReport -Title 'SMBv1 Still Enabled Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'SMBv1 Still Enabled Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
