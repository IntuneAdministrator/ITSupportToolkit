...
<#
.SYNOPSIS
    Enterprise IT tool: Named Locations Proxy Check
.DESCRIPTION
...
.EXAMPLE
    .\Named-Locations-Proxy-Check.ps1
.EXAMPLE
    .\Named-Locations-Proxy-Check.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Named Locations Proxy Check'; $totalSteps = 8; $Name = 'Named-Locations-Proxy-Check'
try {
    ...
    Export-HtmlReport -Title 'Named Locations Proxy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Named Locations Proxy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
