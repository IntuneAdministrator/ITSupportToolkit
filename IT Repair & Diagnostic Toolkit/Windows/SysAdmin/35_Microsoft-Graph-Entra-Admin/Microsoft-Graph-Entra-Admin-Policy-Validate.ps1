...
<#
.SYNOPSIS
    Common problem fix: Microsoft Graph Entra Admin Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Microsoft-Graph-Entra-Admin-Policy-Validate.ps1
.EXAMPLE
    .\Microsoft-Graph-Entra-Admin-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft Graph Entra Admin Policy Validate'; $totalSteps = 8; $Name = 'Microsoft-Graph-Entra-Admin-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Microsoft Graph Entra Admin Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft Graph Entra Admin Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
