...
<#
.SYNOPSIS
    Common problem fix: Graph Autopilot Device Preparation
.DESCRIPTION
...
.EXAMPLE
    .\Graph-Autopilot-Device-Preparation.ps1
.EXAMPLE
    .\Graph-Autopilot-Device-Preparation.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Autopilot Device Preparation'; $totalSteps = 8; $Name = 'Graph-Autopilot-Device-Preparation'
try {
    ...
    Export-HtmlReport -Title 'Graph Autopilot Device Preparation' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Autopilot Device Preparation' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
