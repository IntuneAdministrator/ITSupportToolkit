...
<#
.SYNOPSIS
    Common problem fix: Entra Device Join Type Report
.DESCRIPTION
...
.EXAMPLE
    .\Entra-Device-Join-Type-Report.ps1
.EXAMPLE
    .\Entra-Device-Join-Type-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Entra Device Join Type Report'; $totalSteps = 8; $Name = 'Entra-Device-Join-Type-Report'
try {
    ...
    Export-HtmlReport -Title 'Entra Device Join Type Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Entra Device Join Type Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
