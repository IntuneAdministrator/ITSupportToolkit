...
<#
.SYNOPSIS
    Enterprise IT tool: Workplace Join Cleanup
.DESCRIPTION
...
.EXAMPLE
    .\Workplace-Join-Cleanup.ps1
.EXAMPLE
    .\Workplace-Join-Cleanup.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Workplace Join Cleanup'; $totalSteps = 8; $Name = 'Workplace-Join-Cleanup'
try {
    ...
    Export-HtmlReport -Title 'Workplace Join Cleanup' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Workplace Join Cleanup' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
