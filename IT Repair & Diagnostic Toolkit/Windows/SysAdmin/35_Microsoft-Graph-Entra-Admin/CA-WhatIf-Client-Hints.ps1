...
<#
.SYNOPSIS
    Enterprise IT tool: CA WhatIf Client Hints
.DESCRIPTION
...
.EXAMPLE
    .\CA-WhatIf-Client-Hints.ps1
.EXAMPLE
    .\CA-WhatIf-Client-Hints.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'CA WhatIf Client Hints'; $totalSteps = 8; $Name = 'CA-WhatIf-Client-Hints'
try {
    ...
    Export-HtmlReport -Title 'CA WhatIf Client Hints' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'CA WhatIf Client Hints' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
