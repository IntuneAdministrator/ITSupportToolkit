...
<#
.SYNOPSIS
    Common problem fix: Microsoft Graph Entra Admin Evidence Report
.DESCRIPTION
...
.EXAMPLE
    .\Microsoft-Graph-Entra-Admin-Evidence-Report.ps1
.EXAMPLE
    .\Microsoft-Graph-Entra-Admin-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft Graph Entra Admin Evidence Report'; $totalSteps = 8; $Name = 'Microsoft-Graph-Entra-Admin-Evidence-Report'
try {
    ...
    Export-HtmlReport -Title 'Microsoft Graph Entra Admin Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft Graph Entra Admin Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
