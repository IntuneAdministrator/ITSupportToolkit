...
<#
.SYNOPSIS
    Common problem fix: Graph Hardware Peripheral Inventory
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 35_Microsoft-Graph-Entra-Admin). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Graph-Hardware-Peripheral-Inventory.ps1
.EXAMPLE
    .\Graph-Hardware-Peripheral-Inventory.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Hardware Peripheral Inventory'; $totalSteps = 8; $Name = 'Graph-Hardware-Peripheral-Inventory'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '35_Microsoft-Graph-Entra-Admin' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '35_Microsoft-Graph-Entra-Admin'
    ...
    Export-HtmlReport -Title 'Graph Hardware Peripheral Inventory' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Hardware Peripheral Inventory' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
