...
<#
.SYNOPSIS
    Common problem fix: Graph Registered Owner Export
.DESCRIPTION
...
.EXAMPLE
    .\Graph-Registered-Owner-Export.ps1
.EXAMPLE
    .\Graph-Registered-Owner-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Registered Owner Export'; $totalSteps = 8; $Name = 'Graph-Registered-Owner-Export'
try {
    ...
    Export-HtmlReport -Title 'Graph Registered Owner Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Registered Owner Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
