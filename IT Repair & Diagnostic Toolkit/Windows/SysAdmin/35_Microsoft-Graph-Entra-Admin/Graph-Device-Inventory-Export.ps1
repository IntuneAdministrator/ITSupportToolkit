...
<#
.SYNOPSIS
    Common problem fix: Graph Device Inventory Export
.DESCRIPTION
...
.EXAMPLE
    .\Graph-Device-Inventory-Export.ps1
.EXAMPLE
    .\Graph-Device-Inventory-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Device Inventory Export'; $totalSteps = 8; $Name = 'Graph-Device-Inventory-Export'
try {
    ...
    Export-HtmlReport -Title 'Graph Device Inventory Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Device Inventory Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
