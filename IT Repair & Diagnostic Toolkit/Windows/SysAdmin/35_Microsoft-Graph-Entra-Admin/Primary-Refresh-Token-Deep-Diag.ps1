...
<#
.SYNOPSIS
    Enterprise IT tool: Primary Refresh Token Deep Diag
.DESCRIPTION
...
.EXAMPLE
    .\Primary-Refresh-Token-Deep-Diag.ps1
.EXAMPLE
    .\Primary-Refresh-Token-Deep-Diag.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Primary Refresh Token Deep Diag'; $totalSteps = 8; $Name = 'Primary-Refresh-Token-Deep-Diag'
try {
    ...
    Export-HtmlReport -Title 'Primary Refresh Token Deep Diag' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Primary Refresh Token Deep Diag' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
