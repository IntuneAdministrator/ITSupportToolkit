...
<#
.SYNOPSIS
    Enterprise IT tool: Graph Device Lookup Prep
.DESCRIPTION
...
.EXAMPLE
    .\Graph-Device-Lookup-Prep.ps1
.EXAMPLE
    .\Graph-Device-Lookup-Prep.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Device Lookup Prep'; $totalSteps = 8; $Name = 'Graph-Device-Lookup-Prep'
try {
    ...
    Export-HtmlReport -Title 'Graph Device Lookup Prep' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Device Lookup Prep' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
