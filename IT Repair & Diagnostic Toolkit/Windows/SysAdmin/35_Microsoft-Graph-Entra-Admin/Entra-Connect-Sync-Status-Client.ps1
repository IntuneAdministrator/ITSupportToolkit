...
<#
.SYNOPSIS
    Enterprise IT tool: Entra Connect Sync Status Client
.DESCRIPTION
...
.EXAMPLE
    .\Entra-Connect-Sync-Status-Client.ps1
.EXAMPLE
    .\Entra-Connect-Sync-Status-Client.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Entra Connect Sync Status Client'; $totalSteps = 8; $Name = 'Entra-Connect-Sync-Status-Client'
try {
    ...
    Export-HtmlReport -Title 'Entra Connect Sync Status Client' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Entra Connect Sync Status Client' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
