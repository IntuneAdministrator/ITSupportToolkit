...
<#
.SYNOPSIS
    Common problem fix: Graph Managed App Status Export
.DESCRIPTION
...
.EXAMPLE
    .\Graph-Managed-App-Status-Export.ps1
.EXAMPLE
    .\Graph-Managed-App-Status-Export.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Graph Managed App Status Export'; $totalSteps = 8; $Name = 'Graph-Managed-App-Status-Export'
try {
    ...
    Export-HtmlReport -Title 'Graph Managed App Status Export' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Graph Managed App Status Export' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
