...
<#
.SYNOPSIS
    Common problem fix: Microsoft Graph Entra Admin Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Microsoft-Graph-Entra-Admin-Stack-Reset.ps1
.EXAMPLE
    .\Microsoft-Graph-Entra-Admin-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Microsoft Graph Entra Admin Stack Reset'; $totalSteps = 8; $Name = 'Microsoft-Graph-Entra-Admin-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Microsoft Graph Entra Admin Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Microsoft Graph Entra Admin Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
