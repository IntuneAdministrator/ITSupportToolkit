...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Time Service Full Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Time-Service-Full-Reset.
...
.EXAMPLE
    .\Time-Service-Full-Reset.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Time-Service-Full-Reset.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Time Service Full Reset'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Time-Service-Full-Reset'
        Add-ReportRow -Check 'Tool' -Result 'Time-Service-Full-Reset' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Time Service Full Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Time Service Full Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
