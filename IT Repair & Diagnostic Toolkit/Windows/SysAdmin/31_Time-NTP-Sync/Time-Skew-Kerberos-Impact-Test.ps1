...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Time Skew Kerberos Impact Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Time-Skew-Kerberos-Impact-Test.
...
.EXAMPLE
    .\Time-Skew-Kerberos-Impact-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Time-Skew-Kerberos-Impact-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Time Skew Kerberos Impact Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Time-Skew-Kerberos-Impact-Test'
        Add-ReportRow -Check 'Tool' -Result 'Time-Skew-Kerberos-Impact-Test' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Time Skew Kerberos Impact Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Time Skew Kerberos Impact Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
