...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 31_Time-NTP-Sync). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\Time-NTP-Sync-Evidence-Report.ps1
.EXAMPLE
    .\Time-NTP-Sync-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Evidence Report'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '31_Time-NTP-Sync' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Time NTP Sync Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
