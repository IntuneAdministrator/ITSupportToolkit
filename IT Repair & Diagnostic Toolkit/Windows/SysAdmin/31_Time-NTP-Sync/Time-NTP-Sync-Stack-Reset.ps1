...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Stack-Reset.ps1
.EXAMPLE
    .\Time-NTP-Sync-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Stack Reset'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
