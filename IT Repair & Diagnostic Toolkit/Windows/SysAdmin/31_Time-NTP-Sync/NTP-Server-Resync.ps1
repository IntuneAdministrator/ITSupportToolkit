...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NTP Server Resync
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NTP-Server-Resync.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'NTP-Server-Resync'
        Add-ReportRow -Check 'Tool' -Result 'NTP-Server-Resync' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'NTP Server Resync' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NTP Server Resync' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
