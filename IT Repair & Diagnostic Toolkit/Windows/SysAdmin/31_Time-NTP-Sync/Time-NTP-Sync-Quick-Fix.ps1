...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Quick-Fix.ps1
.EXAMPLE
    .\Time-NTP-Sync-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Quick Fix'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
