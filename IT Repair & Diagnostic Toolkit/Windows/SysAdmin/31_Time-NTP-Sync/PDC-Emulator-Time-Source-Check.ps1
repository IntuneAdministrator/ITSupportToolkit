...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: PDC Emulator Time Source Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for PDC-Emulator-Time-Source-Check.
Category: 31_Time-NTP-Sync. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\PDC-Emulator-Time-Source-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\PDC-Emulator-Time-Source-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'PDC Emulator Time Source Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'TIME NTP SYNC'
        $Name = 'PDC-Emulator-Time-Source-Check'
        Add-ReportRow -Check 'Tool' -Result 'PDC-Emulator-Time-Source-Check' -Status 'INFO' -Detail '31_Time-NTP-Sync'
        Write-SectionBanner 'TIME / NTP'
        $w32 = Get-Service W32Time
        Add-ReportRow -Check 'W32Time' -Result "$($w32.Status)/$($w32.StartType)" -Status $(if ($w32.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail ''
        $status = & w32tm /query /status 2>&1 | Out-String
        Add-ReportRow -Check 'w32tm status' -Result 'Captured' -Status 'INFO' -Detail ($status.Substring(0, [Math]::Min(400, $status.Length)))
        Write-ProgressStep 2 $totalSteps $Activity 'Time status'

        if ($Name -match 'Resync|Full-Reset|NTP-Source|Timezone') {
            if ($PSCmdlet.ShouldProcess('W32Time', 'Restart and resync')) {
                if ($Name -match 'Full-Reset') {
                    & w32tm /unregister 2>&1 | Out-Null
                    & w32tm /register 2>&1 | Out-Null
                    Start-Service W32Time -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'w32tm re-register' -Result 'Done' -Status 'PASS' -Detail ''
                }
                Restart-Service W32Time -Force -ErrorAction SilentlyContinue
                & w32tm /resync /force 2>&1 | Out-Null
                Add-ReportRow -Check 'Resync' -Result 'Forced' -Status 'PASS' -Detail 'w32tm /resync /force'
            }
            if ($Name -match 'Timezone') {
                $tz = Get-TimeZone
                Add-ReportRow -Check 'Time zone' -Result $tz.Id -Status 'INFO' -Detail $tz.DisplayName
            }
            if ($Name -match 'NTP-Source-Change' -and $PSCmdlet.ShouldProcess('NTP', 'Set time.windows.com,0x9')) {
                if ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
                    Add-ReportRow -Check 'NTP source' -Result 'Domain members should follow domain hierarchy' -Status 'WARN' -Detail 'Skipping external NTP peer override'
                } else {
                    & w32tm /config /manualpeerlist:'time.windows.com,0x9' /syncfromflags:manual /reliable:YES /update 2>&1 | Out-Null
                    Restart-Service W32Time -Force -ErrorAction SilentlyContinue
                    Add-ReportRow -Check 'Manual NTP peer' -Result 'time.windows.com' -Status 'PASS' -Detail 'Workgroup/Entra-only path'
                }
            }
        }

        if ($Name -match 'Domain-Time|PDC|Kerberos|Stripchart|Secure-Time|Event-Log') {
            if ($Join.IsDomainJoined -or $Join.IsHybridJoined) {
                $src = & w32tm /query /source 2>&1 | Out-String
                Add-ReportRow -Check 'Time source' -Result $src.Trim() -Status 'INFO' -Detail 'Domain hierarchy expected'
                $peers = & w32tm /query /peers 2>&1 | Out-String
                Add-ReportRow -Check 'Peers' -Result 'Listed' -Status 'INFO' -Detail ($peers.Substring(0, [Math]::Min(300, $peers.Length)))
            } else {
                Add-ReportRow -Check 'Domain time hierarchy' -Result 'N/A - not domain joined' -Status 'INFO' -Detail $Join.Label
            }
            if ($Name -match 'Stripchart') {
                $sc = & w32tm /stripchart /computer:time.windows.com /samples:3 /dataonly 2>&1 | Out-String
                Add-ReportRow -Check 'Stripchart' -Result 'Samples' -Status 'INFO' -Detail ($sc.Substring(0, [Math]::Min(300, $sc.Length)))
            }
            if ($Name -match 'Event-Log') {
                $ev = @(Get-WinEvent -FilterHashtable @{LogName='System'; ProviderName='Microsoft-Windows-Time-Service'; StartTime=(Get-Date).AddDays(-7)} -MaxEvents 20 -ErrorAction SilentlyContinue)
                Add-ReportRow -Check 'Time-Service events' -Result "$($ev.Count)" -Status 'INFO' -Detail ''
            }
        }
        Write-ProgressStep 4 $totalSteps $Activity 'NTP'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'PDC Emulator Time Source Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'PDC Emulator Time Source Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
