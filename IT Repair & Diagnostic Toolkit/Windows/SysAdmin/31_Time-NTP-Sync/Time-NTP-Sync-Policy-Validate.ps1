...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Policy-Validate.ps1
.EXAMPLE
    .\Time-NTP-Sync-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Policy Validate'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
