...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Health-Triage.ps1
.EXAMPLE
    .\Time-NTP-Sync-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Health Triage'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
