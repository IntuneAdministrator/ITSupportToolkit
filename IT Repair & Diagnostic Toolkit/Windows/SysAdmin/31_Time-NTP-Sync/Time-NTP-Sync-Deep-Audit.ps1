...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Deep-Audit.ps1
.EXAMPLE
    .\Time-NTP-Sync-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Deep Audit'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
