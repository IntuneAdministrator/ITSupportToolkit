...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Domain Time Hierarchy Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Domain-Time-Hierarchy-Check.
...
.EXAMPLE
    .\Domain-Time-Hierarchy-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Domain-Time-Hierarchy-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Domain Time Hierarchy Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Domain-Time-Hierarchy-Check'
        Add-ReportRow -Check 'Tool' -Result 'Domain-Time-Hierarchy-Check' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Domain Time Hierarchy Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Domain Time Hierarchy Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
