...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Timezone Auto Correct
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Timezone-Auto-Correct.
...
$exitCode = 0
$Activity = 'Timezone Auto Correct'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Timezone-Auto-Correct'
        Add-ReportRow -Check 'Tool' -Result 'Timezone-Auto-Correct' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Timezone Auto Correct' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Timezone Auto Correct' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
