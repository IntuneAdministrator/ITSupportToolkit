...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: NTP Source Change Enforce
.DESCRIPTION
    Production-grade Windows SysAdmin tool for NTP-Source-Change-Enforce.
...
.EXAMPLE
    .\NTP-Source-Change-Enforce.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\NTP-Source-Change-Enforce.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'NTP Source Change Enforce'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'NTP-Source-Change-Enforce'
        Add-ReportRow -Check 'Tool' -Result 'NTP-Source-Change-Enforce' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'NTP Source Change Enforce' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'NTP Source Change Enforce' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
