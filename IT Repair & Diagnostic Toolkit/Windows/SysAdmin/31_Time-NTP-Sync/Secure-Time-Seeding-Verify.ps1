...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Secure Time Seeding Verify
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Secure-Time-Seeding-Verify.
...
.EXAMPLE
    .\Secure-Time-Seeding-Verify.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Secure-Time-Seeding-Verify.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Secure Time Seeding Verify'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Secure-Time-Seeding-Verify'
        Add-ReportRow -Check 'Tool' -Result 'Secure-Time-Seeding-Verify' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Secure Time Seeding Verify' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Secure Time Seeding Verify' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
