...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Auto-Repair.ps1
.EXAMPLE
    .\Time-NTP-Sync-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Auto Repair'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
