...
<#
.SYNOPSIS
    Common problem fix: Time NTP Sync Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\Time-NTP-Sync-Anomaly-Detect.ps1
.EXAMPLE
    .\Time-NTP-Sync-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'Time NTP Sync Anomaly Detect'; $totalSteps = 8; $Name = 'Time-NTP-Sync-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'Time NTP Sync Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Time NTP Sync Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
