...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: W32tm Stripchart Analysis
.DESCRIPTION
    Production-grade Windows SysAdmin tool for W32tm-Stripchart-Analysis.
...
.EXAMPLE
    .\W32tm-Stripchart-Analysis.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\W32tm-Stripchart-Analysis.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'W32tm Stripchart Analysis'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'W32tm-Stripchart-Analysis'
        Add-ReportRow -Check 'Tool' -Result 'W32tm-Stripchart-Analysis' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'W32tm Stripchart Analysis' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'W32tm Stripchart Analysis' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
