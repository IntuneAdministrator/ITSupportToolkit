...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Time Sync Event Log Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Time-Sync-Event-Log-Audit.
...
.EXAMPLE
    .\Time-Sync-Event-Log-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Time-Sync-Event-Log-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Time Sync Event Log Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        $Name = 'Time-Sync-Event-Log-Audit'
        Add-ReportRow -Check 'Tool' -Result 'Time-Sync-Event-Log-Audit' -Status 'INFO' -Detail '31_Time-NTP-Sync'
    ...
    Export-HtmlReport -Title 'Time Sync Event Log Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Time Sync Event Log Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
