...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Policy Validate
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Policy-Validate.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Policy Validate'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Policy-Validate'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
