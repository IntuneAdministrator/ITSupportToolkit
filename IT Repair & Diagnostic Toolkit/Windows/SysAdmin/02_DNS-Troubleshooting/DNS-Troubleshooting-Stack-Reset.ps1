...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Stack Reset
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Stack-Reset.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Stack-Reset.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Stack Reset'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Stack-Reset'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Stack Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Stack Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
