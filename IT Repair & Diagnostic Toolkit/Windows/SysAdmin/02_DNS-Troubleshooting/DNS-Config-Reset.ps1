...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Config Reset
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Config-Reset.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Config-Reset'
        ...
        if ($PSCmdlet.ShouldProcess('DNS client config', 'Reset adapters to DHCP DNS')) {
            Get-NetAdapter | Where-Object Status -eq 'Up' | ForEach-Object {
                Set-DnsClientServerAddress -InterfaceIndex $_.ifIndex -ResetServerAddresses -ErrorAction SilentlyContinue
                Add-ReportRow -Check "DNS reset $($_.Name)" -Result 'DHCP DNS' -Status 'PASS' -Detail "ifIndex=$($_.ifIndex)"
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'DNS reset'
    ...
    Export-HtmlReport -Title 'DNS Config Reset' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Config Reset' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
