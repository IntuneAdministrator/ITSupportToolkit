...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Evidence Report
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 02_DNS-Troubleshooting). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\DNS-Troubleshooting-Evidence-Report.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Evidence-Report.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Evidence Report'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Evidence-Report'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '02_DNS-Troubleshooting' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '02_DNS-Troubleshooting'
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Evidence Report' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Evidence Report' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
