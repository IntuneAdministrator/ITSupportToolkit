...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Anomaly Detect
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Anomaly-Detect.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Anomaly-Detect.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Anomaly Detect'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Anomaly-Detect'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Anomaly Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Anomaly Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
