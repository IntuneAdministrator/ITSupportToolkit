...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Client Service Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Client-Service-Repair.
...
.EXAMPLE
    .\DNS-Client-Service-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Client-Service-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Client Service Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Client-Service-Repair'
        ...
        if ($PSCmdlet.ShouldProcess('DNS', 'Flush cache and restart Dnscache')) {
            ipconfig /flushdns | Out-Null
            Clear-DnsClientCache -ErrorAction SilentlyContinue
            Restart-Service Dnscache -Force -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'DNS cache repair' -Result 'Flushed + service restarted' -Status 'PASS' -Detail 'ipconfig /flushdns'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Flush/repair'
    ...
    Export-HtmlReport -Title 'DNS Client Service Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Client Service Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
