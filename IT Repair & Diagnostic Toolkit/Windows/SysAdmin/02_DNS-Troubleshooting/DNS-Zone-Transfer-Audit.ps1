...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Zone Transfer Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Zone-Transfer-Audit.
...
.EXAMPLE
    .\DNS-Zone-Transfer-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Zone-Transfer-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Zone Transfer Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Zone-Transfer-Audit'
    ...
    Export-HtmlReport -Title 'DNS Zone Transfer Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Zone Transfer Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
