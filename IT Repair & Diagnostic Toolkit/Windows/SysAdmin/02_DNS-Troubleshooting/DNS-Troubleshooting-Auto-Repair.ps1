...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Auto-Repair.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Auto Repair'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
