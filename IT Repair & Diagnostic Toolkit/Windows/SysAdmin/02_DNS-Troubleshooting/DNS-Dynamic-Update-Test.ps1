...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Dynamic Update Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Dynamic-Update-Test.
...
.EXAMPLE
    .\DNS-Dynamic-Update-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Dynamic-Update-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Dynamic Update Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Dynamic-Update-Test'
    ...
    Export-HtmlReport -Title 'DNS Dynamic Update Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Dynamic Update Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
