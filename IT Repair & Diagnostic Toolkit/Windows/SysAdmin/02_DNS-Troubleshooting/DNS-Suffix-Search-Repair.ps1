...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Suffix Search Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Suffix-Search-Repair.
Category: 02_DNS-Troubleshooting. Detects AD / Entra ID / Hybrid join state at runtime,
...
.EXAMPLE
    .\DNS-Suffix-Search-Repair.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Suffix-Search-Repair.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Suffix Search Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Suffix-Search-Repair'
        $dnsServers = @(Get-DnsClientServerAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object { $_.ServerAddresses })
        foreach ($d in $dnsServers) {
            Add-ReportRow -Check "DNS on $($d.InterfaceAlias)" -Result (($d.ServerAddresses) -join ', ') -Status 'INFO' -Detail "Index=$($d.InterfaceIndex)"
        }
        Write-ProgressStep 2 $totalSteps $Activity 'DNS servers'
        $suffix = (Get-DnsClientGlobalSetting).SuffixSearchList
        Add-ReportRow -Check 'DNS suffix search' -Result (($suffix) -join ', ') -Status $(if ($suffix) {'PASS'} else {'WARN'}) -Detail 'Get-DnsClientGlobalSetting'
        if ($Join.IsDomainJoined -and $Join.DomainName -and $PSCmdlet.ShouldProcess('DNS suffix', "Ensure $($Join.DomainName) in search list")) {
            $list = @($suffix)
            if ($list -notcontains $Join.DomainName) {
                Set-DnsClientGlobalSetting -SuffixSearchList ($list + $Join.DomainName) -ErrorAction SilentlyContinue
                Add-ReportRow -Check 'Append domain suffix' -Result $Join.DomainName -Status 'PASS' -Detail 'Added to search list'
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Suffix'
        Write-ProgressStep $($totalSteps - 1) $totalSteps $Activity 'Verify resolve'
        try {
            $r = Resolve-DnsName -Name www.microsoft.com -Type A -ErrorAction Stop | Select-Object -First 1
            Add-ReportRow -Check 'Resolve www.microsoft.com' -Result $r.IPAddress -Status 'PASS' -Detail $r.Name
        } catch {
            Add-ReportRow -Check 'Resolve www.microsoft.com' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
        }

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'DNS Suffix Search Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Suffix Search Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
