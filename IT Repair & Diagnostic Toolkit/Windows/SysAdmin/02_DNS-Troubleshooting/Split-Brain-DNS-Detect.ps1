...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Split Brain DNS Detect
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Split-Brain-DNS-Detect.
...
$exitCode = 0
$Activity = 'Split Brain DNS Detect'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — Split-Brain-DNS-Detect'
    ...
    Export-HtmlReport -Title 'Split Brain DNS Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Split Brain DNS Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
