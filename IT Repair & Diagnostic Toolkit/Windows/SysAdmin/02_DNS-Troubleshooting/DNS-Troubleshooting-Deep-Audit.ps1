...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Deep Audit
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Deep-Audit.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Deep-Audit.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Deep Audit'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Deep-Audit'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Deep Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Deep Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
