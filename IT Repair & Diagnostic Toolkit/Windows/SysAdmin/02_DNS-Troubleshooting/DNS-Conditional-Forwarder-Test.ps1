...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Conditional Forwarder Test
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Conditional-Forwarder-Test.
...
.EXAMPLE
    .\DNS-Conditional-Forwarder-Test.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Conditional-Forwarder-Test.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Conditional Forwarder Test'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Conditional-Forwarder-Test'
        ...
        ipconfig /displaydns 2>&1 | Select-Object -First 5 | Out-Null
        Add-ReportRow -Check 'DNS client cache' -Result 'Readable' -Status 'PASS' -Detail 'ipconfig /displaydns'
        Write-ProgressStep 3 $totalSteps $Activity 'Cache display'
    ...
    Export-HtmlReport -Title 'DNS Conditional Forwarder Test' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Conditional Forwarder Test' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
