...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Lookup Validator
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Lookup-Validator.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Lookup-Validator'
    ...
    Export-HtmlReport -Title 'DNS Lookup Validator' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Lookup Validator' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
