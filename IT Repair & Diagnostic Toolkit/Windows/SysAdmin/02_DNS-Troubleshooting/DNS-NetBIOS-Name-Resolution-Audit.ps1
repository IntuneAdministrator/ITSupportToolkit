...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS NetBIOS Name Resolution Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-NetBIOS-Name-Resolution-Audit.
...
.EXAMPLE
    .\DNS-NetBIOS-Name-Resolution-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-NetBIOS-Name-Resolution-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS NetBIOS Name Resolution Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-NetBIOS-Name-Resolution-Audit'
        ...
        $nb = & nbtstat -n 2>&1 | Out-String
        Add-ReportRow -Check 'NetBIOS names' -Result 'nbtstat -n' -Status 'INFO' -Detail ($nb.Substring(0, [Math]::Min(400, $nb.Length)))
        $lmhosts = "$env:SystemRoot\System32\drivers\etc\lmhosts"
        Add-ReportRow -Check 'lmhosts exists' -Result (Test-Path $lmhosts) -Status 'INFO' -Detail $lmhosts
        Write-ProgressStep 3 $totalSteps $Activity 'NetBIOS'
    ...
    Export-HtmlReport -Title 'DNS NetBIOS Name Resolution Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS NetBIOS Name Resolution Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
