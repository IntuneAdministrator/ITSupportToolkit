...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Cache Flush Repair
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Cache-Flush-Repair.
...
$exitCode = 0
$Activity = 'DNS Cache Flush Repair'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS CLIENT SERVICE'
        $svc = Get-Service -Name Dnscache -ErrorAction SilentlyContinue
        Add-ReportRow -Check 'Dnscache service' -Result $(if ($svc) { $svc.Status } else {'Missing'}) -Status $(if ($svc -and $svc.Status -eq 'Running') {'PASS'} else {'FAIL'}) -Detail $svc.StartType
        Write-ProgressStep 2 $totalSteps $Activity 'Service check'

        if ($PSCmdlet.ShouldProcess('DNS cache', 'Flush and restart Dnscache')) {
            try {
                ipconfig /flushdns | Out-Null
                Add-ReportRow -Check 'ipconfig /flushdns' -Result 'Flushed' -Status 'PASS' -Detail 'Client resolver cache cleared'
            } catch {
                Add-ReportRow -Check 'ipconfig /flushdns' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
            }
            try {
                Restart-Service -Name Dnscache -Force -ErrorAction Stop
                Add-ReportRow -Check 'Restart Dnscache' -Result 'Restarted' -Status 'PASS' -Detail 'Service recycled'
            } catch {
                Add-ReportRow -Check 'Restart Dnscache' -Result 'Failed' -Status 'WARN' -Detail $_.Exception.Message
            }
            Clear-DnsClientCache -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'Clear-DnsClientCache' -Result 'Cleared' -Status 'PASS' -Detail 'PowerShell DNS client cache'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'Flush'

        try {
            $r = Resolve-DnsName www.microsoft.com -ErrorAction Stop | Select-Object -First 1
            Add-ReportRow -Check 'Post-flush resolve' -Result 'OK' -Status 'PASS' -Detail "$($r.Name) -> $($r.IPAddress)"
        } catch {
            Add-ReportRow -Check 'Post-flush resolve' -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
        }
        Write-ProgressStep 4 $totalSteps $Activity 'Verify'

    Write-ProgressStep $totalSteps $totalSteps $Activity 'Export report'
    Export-HtmlReport -Title 'DNS Cache Flush Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Cache Flush Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
