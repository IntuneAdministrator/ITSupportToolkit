...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Forwarder Health Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Forwarder-Health-Check.
...
.EXAMPLE
    .\DNS-Forwarder-Health-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Forwarder-Health-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Forwarder Health Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Forwarder-Health-Check'
    ...
    Export-HtmlReport -Title 'DNS Forwarder Health Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Forwarder Health Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
