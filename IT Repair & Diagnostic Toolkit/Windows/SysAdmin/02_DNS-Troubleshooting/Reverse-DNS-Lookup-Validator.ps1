...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: Reverse DNS Lookup Validator
.DESCRIPTION
    Production-grade Windows SysAdmin tool for Reverse-DNS-Lookup-Validator.
...
.EXAMPLE
    .\Reverse-DNS-Lookup-Validator.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\Reverse-DNS-Lookup-Validator.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'Reverse DNS Lookup Validator'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — Reverse-DNS-Lookup-Validator'
        ...
        $servers = @(Get-DnsClientServerAddress -AddressFamily IPv4 | Select-Object -ExpandProperty ServerAddresses -Unique)
        $names = @('www.microsoft.com','google.com','cloudflare.com')
        foreach ($s in $servers) {
            foreach ($nm in $names) {
                $sw = [System.Diagnostics.Stopwatch]::StartNew()
                try {
                    Resolve-DnsName -Name $nm -Server $s -Type A -ErrorAction Stop | Out-Null
                    $sw.Stop()
                    Add-ReportRow -Check "Query $nm via $s" -Result "$($sw.ElapsedMilliseconds) ms" -Status $(if ($sw.ElapsedMilliseconds -lt 200) {'PASS'} else {'WARN'}) -Detail 'A record'
                } catch {
                    $sw.Stop()
                    Add-ReportRow -Check "Query $nm via $s" -Result 'Failed' -Status 'FAIL' -Detail $_.Exception.Message
                }
            }
        }
        Write-ProgressStep 3 $totalSteps $Activity 'DNS queries'
    ...
    Export-HtmlReport -Title 'Reverse DNS Lookup Validator' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'Reverse DNS Lookup Validator' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
