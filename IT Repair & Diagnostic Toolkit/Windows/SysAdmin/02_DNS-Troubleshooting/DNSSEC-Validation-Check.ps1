...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNSSEC Validation Check
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNSSEC-Validation-Check.
...
.EXAMPLE
    .\DNSSEC-Validation-Check.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNSSEC-Validation-Check.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNSSEC Validation Check'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNSSEC-Validation-Check'
        ...
        try {
            $r = Resolve-DnsName -Name dnssec-failed.org -Type A -DnsOnly -ErrorAction SilentlyContinue
            Add-ReportRow -Check 'DNSSEC probe' -Result 'Query attempted' -Status 'INFO' -Detail 'Validate corporate DNSSEC policy separately'
        } catch {
            Add-ReportRow -Check 'DNSSEC probe' -Result $_.Exception.Message -Status 'INFO' -Detail 'Failure may indicate DNSSEC validation'
        }
        Write-ProgressStep 3 $totalSteps $Activity 'DNSSEC'
    ...
    Export-HtmlReport -Title 'DNSSEC Validation Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNSSEC Validation Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
