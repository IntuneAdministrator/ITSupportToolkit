...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Over HTTPS Config Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Over-HTTPS-Config-Audit.
...
.EXAMPLE
    .\DNS-Over-HTTPS-Config-Audit.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Over-HTTPS-Config-Audit.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Over HTTPS Config Audit'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Over-HTTPS-Config-Audit'
        ...
        try {
            ...
            Add-ReportRow -Check 'DoH servers' -Result $(if ($doh) { ($doh | ForEach-Object ServerAddress) -join ', ' } else {'None configured'}) -Status 'INFO' -Detail 'Get-DnsClientDohServerAddress'
        } catch {
            Add-ReportRow -Check 'DoH servers' -Result 'Cmdlet unavailable' -Status 'WARN' -Detail $_.Exception.Message
        }
        Write-ProgressStep 3 $totalSteps $Activity 'DoH audit'
    ...
    Export-HtmlReport -Title 'DNS Over HTTPS Config Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Over HTTPS Config Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
