...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Quick-Fix.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Quick Fix'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
