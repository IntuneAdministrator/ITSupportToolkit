...
<#
.SYNOPSIS
    Common problem fix: DNS Troubleshooting Health Triage
.DESCRIPTION
...
.EXAMPLE
    .\DNS-Troubleshooting-Health-Triage.ps1
.EXAMPLE
    .\DNS-Troubleshooting-Health-Triage.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS Troubleshooting Health Triage'; $totalSteps = 8; $Name = 'DNS-Troubleshooting-Health-Triage'
try {
    ...
    Export-HtmlReport -Title 'DNS Troubleshooting Health Triage' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Troubleshooting Health Triage' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
