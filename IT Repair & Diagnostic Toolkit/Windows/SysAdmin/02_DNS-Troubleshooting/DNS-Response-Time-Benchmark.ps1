...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Response Time Benchmark
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Response-Time-Benchmark.
...
.EXAMPLE
    .\DNS-Response-Time-Benchmark.ps1
    Runs the full repair/diagnostic with logging and HTML report.
.EXAMPLE
    .\DNS-Response-Time-Benchmark.ps1 -WhatIf
...
$exitCode = 0
$Activity = 'DNS Response Time Benchmark'
$totalSteps = 8
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Response-Time-Benchmark'
    ...
    Export-HtmlReport -Title 'DNS Response Time Benchmark' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Response Time Benchmark' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
