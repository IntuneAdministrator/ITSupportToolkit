...
<#
.SYNOPSIS
    Enterprise IT repair/diagnostic: DNS Scavenging Audit
.DESCRIPTION
    Production-grade Windows SysAdmin tool for DNS-Scavenging-Audit.
...
try {
    ...
    Add-ReportRow -Check 'PowerShell' -Result $PSVersionTable.PSVersion.ToString() -Status 'INFO' -Detail $PSVersionTable.PSEdition
        ...
        Write-SectionBanner 'DNS — DNS-Scavenging-Audit'
    ...
    Export-HtmlReport -Title 'DNS Scavenging Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try {
        ...
        Export-HtmlReport -Title 'DNS Scavenging Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    } catch { }
    $exitCode = 2
}
finally {
