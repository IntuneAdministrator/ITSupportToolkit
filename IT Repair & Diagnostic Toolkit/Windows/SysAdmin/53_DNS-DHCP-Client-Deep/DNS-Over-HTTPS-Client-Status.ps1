...
<#
.SYNOPSIS
    Enterprise IT tool: DNS Over HTTPS Client Status
.DESCRIPTION
    Phase 11 expansion (SysAdmin/53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\DNS-Over-HTTPS-Client-Status.ps1
.EXAMPLE
    .\DNS-Over-HTTPS-Client-Status.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='DNS Over HTTPS Client Status'; $totalSteps=8; $Name='DNS-Over-HTTPS-Client-Status'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    Export-HtmlReport -Title 'DNS Over HTTPS Client Status' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Over HTTPS Client Status' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
