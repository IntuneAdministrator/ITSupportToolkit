...
<#
.SYNOPSIS
    Common problem fix: DNS DHCP Client Deep Auto Repair
.DESCRIPTION
...
.EXAMPLE
    .\DNS-DHCP-Client-Deep-Auto-Repair.ps1
.EXAMPLE
    .\DNS-DHCP-Client-Deep-Auto-Repair.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS DHCP Client Deep Auto Repair'; $totalSteps = 8; $Name = 'DNS-DHCP-Client-Deep-Auto-Repair'
try {
    ...
    Export-HtmlReport -Title 'DNS DHCP Client Deep Auto Repair' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS DHCP Client Deep Auto Repair' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
