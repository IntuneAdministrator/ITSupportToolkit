...
<#
.SYNOPSIS
    Enterprise IT tool: Static DNS vs DHCP Reconcile
.DESCRIPTION
    Phase 11 expansion (SysAdmin/53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\Static-DNS-vs-DHCP-Reconcile.ps1
.EXAMPLE
    .\Static-DNS-vs-DHCP-Reconcile.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='Static DNS vs DHCP Reconcile'; $totalSteps=8; $Name='Static-DNS-vs-DHCP-Reconcile'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    Export-HtmlReport -Title 'Static DNS vs DHCP Reconcile' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'Static DNS vs DHCP Reconcile' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
