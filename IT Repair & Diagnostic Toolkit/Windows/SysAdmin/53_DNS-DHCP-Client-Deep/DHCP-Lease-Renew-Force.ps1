...
<#
.SYNOPSIS
    Enterprise IT tool: DHCP Lease Renew Force
.DESCRIPTION
...
$exitCode=0; $Activity='DHCP Lease Renew Force'; $totalSteps=8; $Name='DHCP-Lease-Renew-Force'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DHCP Lease Renew Force' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
