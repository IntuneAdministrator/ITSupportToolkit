...
<#
.SYNOPSIS
    Enterprise IT tool: LLMNR NetBIOS Hardening Check
.DESCRIPTION
    Phase 11 expansion (SysAdmin/53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\LLMNR-NetBIOS-Hardening-Check.ps1
.EXAMPLE
    .\LLMNR-NetBIOS-Hardening-Check.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='LLMNR NetBIOS Hardening Check'; $totalSteps=8; $Name='LLMNR-NetBIOS-Hardening-Check'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '53_DNS-DHCP-Client-Deep' -Status 'INFO' -Detail 'SysAdmin'
        Write-SectionBanner 'DNS / DHCP CLIENT DEEP'
        Write-ProgressStep 2 $totalSteps $Activity 'Name resolution'
        if ($PSCmdlet.ShouldProcess('DNS', 'flush + register')) {
            ipconfig /flushdns | Out-Null
            ipconfig /registerdns | Out-Null
            Add-ReportRow -Check 'flushdns/registerdns' -Result 'Done' -Status 'PASS' -Detail ''
        }
        ...
        foreach ($a in $adapters | Select-Object -First 3) {
            ...
            Add-ReportRow -Check "DNS $($a.Name)" -Result ($dns -join ', ') -Status $(if($dns.Count){'PASS'}else{'WARN'}) -Detail ''
        }
        if ($Name -match 'NRPT') {
            $nrpt = @(Get-DnsClientNrptPolicy -EA SilentlyContinue)
            Add-ReportRow -Check 'NRPT entries' -Result "$($nrpt.Count)" -Status 'INFO' -Detail ''
        }
        if ($Name -match 'DHCP|Lease') {
            if ($PSCmdlet.ShouldProcess('DHCP', 'release/renew')) {
                ipconfig /release | Out-Null
                ipconfig /renew | Out-Null
                Add-ReportRow -Check 'DHCP renew' -Result 'release/renew' -Status 'PASS' -Detail ''
            }
        }
        if ($Name -match 'LLMNR|NetBIOS|mDNS') {
            Add-ReportRow -Check 'Name resolution hardening' -Result 'Review LLMNR/NetBIOS/mDNS policies' -Status 'INFO' -Detail 'Prefer DNS + DoH where approved'
        }
        Write-ProgressStep 5 $totalSteps $Activity 'DNS/DHCP done'
    Write-ProgressStep $totalSteps $totalSteps $Activity 'Report'
    Export-HtmlReport -Title 'LLMNR NetBIOS Hardening Check' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'LLMNR NetBIOS Hardening Check' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
