...
<#
.SYNOPSIS
    Enterprise IT tool: DNS Suffix Search List Audit
.DESCRIPTION
    Phase 11 expansion (SysAdmin/53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\DNS-Suffix-Search-List-Audit.ps1
.EXAMPLE
    .\DNS-Suffix-Search-List-Audit.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='DNS Suffix Search List Audit'; $totalSteps=8; $Name='DNS-Suffix-Search-List-Audit'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    Export-HtmlReport -Title 'DNS Suffix Search List Audit' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Suffix Search List Audit' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
