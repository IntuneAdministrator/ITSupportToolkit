...
<#
.SYNOPSIS
    Common problem fix: DNS DHCP Client Deep Quick Fix
.DESCRIPTION
...
.EXAMPLE
    .\DNS-DHCP-Client-Deep-Quick-Fix.ps1
.EXAMPLE
    .\DNS-DHCP-Client-Deep-Quick-Fix.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS DHCP Client Deep Quick Fix'; $totalSteps = 8; $Name = 'DNS-DHCP-Client-Deep-Quick-Fix'
try {
    ...
    Export-HtmlReport -Title 'DNS DHCP Client Deep Quick Fix' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS DHCP Client Deep Quick Fix' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
