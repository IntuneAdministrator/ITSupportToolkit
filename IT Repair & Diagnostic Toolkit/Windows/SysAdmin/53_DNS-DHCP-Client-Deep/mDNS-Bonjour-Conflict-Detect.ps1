...
<#
.SYNOPSIS
    Enterprise IT tool: mDNS Bonjour Conflict Detect
.DESCRIPTION
    Phase 11 expansion (SysAdmin/53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\mDNS-Bonjour-Conflict-Detect.ps1
.EXAMPLE
    .\mDNS-Bonjour-Conflict-Detect.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='mDNS Bonjour Conflict Detect'; $totalSteps=8; $Name='mDNS-Bonjour-Conflict-Detect'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    Export-HtmlReport -Title 'mDNS Bonjour Conflict Detect' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'mDNS Bonjour Conflict Detect' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
