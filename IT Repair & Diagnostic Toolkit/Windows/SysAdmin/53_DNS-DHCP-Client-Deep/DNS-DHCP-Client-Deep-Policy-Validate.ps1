...
<#
.SYNOPSIS
    Common problem fix: DNS DHCP Client Deep Policy Validate
.DESCRIPTION
    Phase 13 common-problems module (SysAdmin / 53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible,
    transcript log + dark HTML report. Use -WhatIf to preview changes.
.EXAMPLE
    .\DNS-DHCP-Client-Deep-Policy-Validate.ps1
.EXAMPLE
    .\DNS-DHCP-Client-Deep-Policy-Validate.ps1 -WhatIf
.NOTES
...
$exitCode = 0; $Activity = 'DNS DHCP Client Deep Policy Validate'; $totalSteps = 8; $Name = 'DNS-DHCP-Client-Deep-Policy-Validate'
try {
    ...
    Add-ReportRow -Check 'Module' -Result '53_DNS-DHCP-Client-Deep' -Status 'INFO' -Detail 'SysAdmin'
        ...
        Add-ReportRow -Check 'Script' -Result $Name -Status 'INFO' -Detail '53_DNS-DHCP-Client-Deep'
    ...
    Export-HtmlReport -Title 'DNS DHCP Client Deep Policy Validate' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null
    ...
    try { Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS DHCP Client Deep Policy Validate' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath | Out-Null } catch {}
    $exitCode = 2
