...
<#
.SYNOPSIS
    Enterprise IT tool: DNS Client Cache Poison Hunt
.DESCRIPTION
    Phase 11 expansion (SysAdmin/53_DNS-DHCP-Client-Deep). Join-state aware, Intune-compatible, HTML + transcript.
.EXAMPLE
    .\DNS-Client-Cache-Poison-Hunt.ps1
.EXAMPLE
    .\DNS-Client-Cache-Poison-Hunt.ps1 -WhatIf
.NOTES
...
$exitCode=0; $Activity='DNS Client Cache Poison Hunt'; $totalSteps=8; $Name='DNS-Client-Cache-Poison-Hunt'; $Folder='53_DNS-DHCP-Client-Deep'
try {
    ...
    Export-HtmlReport -Title 'DNS Client Cache Poison Hunt' -JoinState $Join.Label -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null
    ...
    try{Add-ReportRow -Check 'Fatal' -Result 'Exception' -Status 'FAIL' -Detail $_.Exception.Message; Export-HtmlReport -Title 'DNS Client Cache Poison Hunt' -JoinState 'Unknown' -ComputerName $env:COMPUTERNAME -LogPath $LogPath -ScriptFileName $ScriptName -ReportPath $ReportPath|Out-Null}catch{}
    $exitCode=2
